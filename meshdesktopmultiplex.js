...
function CreateDesktopMultiplexor(parent, domain, nodeid, id, func) {
  ...
  obj.addPeer = function (peer) {
    if (obj.viewers == null) {
      parent.parent.debug('relay', 'DesktopRelay: Error, addingPeer on disposed session');
      return;
    }
    ...
    if (peer.req.query.browser) {
      ...
      if (peer.user != null && obj.userIds.indexOf(peer.user._id) == -1) {
        obj.userIds.push(peer.user._id);
      }
      ...
      if (peer.req.query.slowrelay != null) {
        var sr = null;
        try {
          sr = parseInt(peer.req.query.slowrelay);
        } catch (ex) {}
      ...
      if (typeof domain.sessionrecording == 'object') {
        ...
        if (
          ...
          var user = null;
          if (peer.user != null) {
            user = parent.users[peer.user._id];
          }
          if (user != null && user.links != null && user.links[obj.meshid] == null && user.links[obj.nodeid] == null) {
            ...
            for (var i in user.links) {
              ...
              if (
                ...
                (ugrp.flags & 2) != 0 &&
                ugrp.links != null &&
                (ugrp.links[obj.meshid] != null || ugrp.links[obj.nodeid] != null)
              ) {
                startRecord = true;
              }
            }
      ...
      startRecording(domain, startRecord, function () {
        ...
        if (obj.lastDisplayInfoData != null) {
          obj.sendToViewer(peer, obj.lastDisplayInfoData);
        }
        if (obj.lastDisplayLocationData != null) {
          obj.sendToViewer(peer, obj.lastDisplayLocationData);
        }
        if (obj.lastConsoleMessage != null) {
          obj.sendToViewer(peer, obj.lastConsoleMessage);
        }
        if (obj.lastKeyState != null) {
          obj.sendToViewer(peer, obj.lastKeyState);
        }
        ...
        if (obj.startTime != null) {
          var event = {
            ...
            domain: domain.id,
            nodeid: obj.nodeid,
            userid: peer.user ? peer.user._id : null,
            username: peer.user.name,
            msgid: 143,
            msgArgs: [obj.id],
            msg: 'Joined desktop multiplex session "' + obj.id + '"',
            protocol: 2
          };
      ...
      if (obj.agent != null) {
        parent.parent.debug('relay', 'DesktopRelay: Error, duplicate agent connection');
        return false;
      }
      ...
      if (obj.viewerConnected == true) {
        if (obj.protocolOptions != null) {
          obj.sendToAgent(JSON.stringify(obj.protocolOptions));
    ...
    if (obj.agent != null && obj.viewers.length > 0 && obj.startTime == null) {
      var event = {
        ...
        msgid: 145,
        msgArgs: [obj.id],
        msg: 'Started desktop multiplex session "' + obj.id + '"',
        protocol: 2
      };
      if (obj.viewers[0].user != null) {
        event.userid = obj.viewers[0].user._id;
        event.username = obj.viewers[0].user.name;
      }
      const targets = ['*', obj.nodeid, obj.meshid];
      if (obj.viewers[0].user != null) {
        targets.push(obj.viewers[0].user._id);
      }
  ...
  obj.removePeer = function (peer) {
    ...
    if (peer == obj.agent) {
      ...
      if (peer.overflow == true) {
        ...
        peer.overflow = false;
        if (
          obj.viewersOverflowCount < obj.viewers.length &&
          obj.recordingFileWriting == false &&
          obj.agent &&
          obj.agent.paused == true
        ) {
      ...
      if (obj.startTime != null) {
        // Used to check if the agent has connected. If not, don't log this event since the session never really started.
        ...
        try {
          inTraffc = peer.ws._socket.bytesRead;
        } catch (ex) {}
        try {
          outTraffc = peer.ws._socket.bytesWritten;
        } catch (ex) {}

        // Add any previous agent traffic accounting
        if (peer.agentInTraffic) {
          inTraffc += peer.agentInTraffic;
        }
        if (peer.outTraffc) {
          inTraffc += peer.agentOutTraffic;
        }
        ...
        if (obj.agent != null) {
          ...
          try {
            agentInTraffc = agentInTraffc2 = obj.agent.ws._socket.bytesRead;
          } catch (ex) {}
          try {
            agentOutTraffc = agentOutTraffc2 = obj.agent.ws._socket.bytesWritten;
          } catch (ex) {}
          if (obj.agent.accountedBytesRead) {
            agentInTraffc -= obj.agent.accountedBytesRead;
          }
          if (obj.agent.accountedBytesWritten) {
            agentOutTraffc -= obj.agent.accountedBytesWritten;
          }
          ...
          for (var i in obj.viewers) {
            if (obj.viewers[i].agentInTraffic) {
              obj.viewers[i].agentInTraffic += viewerPartIn;
            } else {
              obj.viewers[i].agentInTraffic = viewerPartIn;
            }
            if (obj.viewers[i].agentOutTraffic) {
              obj.viewers[i].agentOutTraffic += viewerPartOut;
            } else {
              obj.viewers[i].agentOutTraffic = viewerPartOut;
            }
          }
        ...
        var event = {
          ...
          nodeid: obj.nodeid,
          msgid: 144,
          msgArgs: [obj.id, sessionSeconds],
          msg: 'Left the desktop multiplex session "' + obj.id + '" after ' + sessionSeconds + ' second(s).',
          protocol: 2,
          bytesin: inTraffc,
          bytesout: outTraffc
        };
        if (peer.user != null) {
          event.userid = peer.user._id;
          event.username = peer.user.name;
        }
        if (peer.guestName) {
      ...
      if (obj.viewers != null && obj.viewers.length == 0 && obj.agent != null) {
        obj.agent.close();
        dispose();
        return true;
      }
  ...
  function dispose() {
    ...
    if (obj.recordingFile != null) {
      ...
      if (obj.startTime != null) {
        obj.sessionStart = obj.startTime;
        obj.sessionLength = Math.round((Date.now() - obj.startTime) / 1000);
      }
      ...
      recordingEntry(
        ...
        function (fd, filename) {
          ...
          if (domain.sessionrecording.index && domain.sessionrecording.index !== false) {
            parent.parent.certificateOperations.acceleratorPerformOperation('indexMcRec', filename);
          }
          ...
          var event = {
            ...
            nodeid: obj.nodeid,
            msgid: 146,
            msgArgs: [obj.id, obj.sessionLength],
            msg: 'Finished recording session "' + obj.id + '", ' + obj.sessionLength + ' second(s)',
            filename: basefile,
            size: obj.recordingFileSize,
            protocol: 2,
            icon: obj.icon,
            name: obj.name,
            meshid: obj.meshid,
          ...
          if (obj.sessionStart) {
            event.startTime = obj.sessionStart;
            event.lengthTime = obj.sessionLength;
          }
    ...
    if (obj.startTime != null) {
      var event = {
        ...
        nodeid: obj.nodeid,
        msgid: 147,
        msgArgs: [obj.id, Math.floor((Date.now() - obj.startTime) / 1000)],
        msg:
          'Closed desktop multiplex session "' +
          obj.id +
          '", ' +
          Math.floor((Date.now() - obj.startTime) / 1000) +
          ' second(s)',
        protocol: 2
      };
    ...
    if (obj.viewers != null) {
      for (var i in obj.viewers) {
        ...
        if (v.user != null && v.user._id != null) {
          var id = v.user._id;
          if (v.guestName) {
            id += '/guest:' + Buffer.from(v.guestName).toString('base64');
          } // If this is a guest connect, add the Base64 guest name.
          if (allUsers[id] == null) {
            allUsers[id] = 1;
          } else {
            allUsers[id]++;
      ...
      obj.sendToAllViewers(
        JSON.stringify({ type: 'metadata', ctrlChannel: '102938', users: allUsers, startTime: obj.startTime })
      );
    }
  ...
  obj.sendToAllViewers = function (data) {
    if (obj.viewers == null) return;
    for (var i in obj.viewers) {
      obj.sendToViewer(obj.viewers[i], data);
    }
  };
  ...
  obj.sendToAllInputViewers = function (data) {
    if (obj.viewers == null) return;
    for (var i in obj.viewers) {
      if (obj.viewers[i].viewOnly != true) {
        obj.sendToViewer(obj.viewers[i], data);
      }
    }
  };
  ...
  obj.sendToViewer = function (viewer, data) {
    ...
    if (viewer.sending) {
      ...
      if (viewer.slowRelay) {
        setTimeout(function () {
          try {
            viewer.ws.send(data, function () {
              sendViewerNext(viewer);
            });
          } catch (ex) {}
        }, viewer.slowRelay);
      } else {
  ...
  function checkViewerOverflow(viewer) {
    ...
    if (viewer.sendQueue.length > 5 || (viewer.dataPtr != null && viewer.dataPtr != obj.lastData)) {
      ...
      obj.viewersOverflowCount++;
      if (obj.viewersOverflowCount >= obj.viewers.length && obj.agent && obj.agent.paused == false) {
        obj.agent.paused = true;
        obj.agent.ws._socket.pause();
      }
    }
  }
  ...
  function sendViewerNext(viewer) {
    ...
    if (viewer.sendQueue.length > 0) {
      ...
      if (viewer.slowRelay) {
        setTimeout(function () {
          try {
            viewer.ws.send(viewer.sendQueue.shift(), function () {
              sendViewerNext(viewer);
            });
          } catch (ex) {}
        }, viewer.slowRelay);
      ...
      if (viewer.dataPtr != null) {
        ...
        if (viewer.slowRelay) {
          setTimeout(function () {
            try {
              viewer.ws.send(image.data, function () {
                sendViewerNext(viewer);
              });
            } catch (ex) {}
          }, viewer.slowRelay);
  ...
  obj.processData = function (peer, data) {
    ...
    if (peer == obj.agent) {
      ...
      recordData(true, data, function () {
        if (obj.viewers == null) return;
        obj.recordingFileWriting = false;
        if (obj.viewersOverflowCount < obj.viewers.length && obj.agent && obj.agent.paused == true) {
          obj.agent.paused = false;
          obj.agent.ws._socket.resume();
        }
  ...
  obj.processViewerData = function (viewer, data) {
    if (typeof data == 'string') {
      ...
      if (json == null) return;
      if (json.type == 'options' && obj.protocolOptions == null) {
        obj.protocolOptions = json;
      }
      if (json.ctrlChannel == '102938') {
        if (json.type == 'lock' && viewer.viewOnly == false) {
          obj.sendToAgent('{"ctrlChannel":"102938","type":"lock"}');
        } // Account lock support
        if (json.type == 'autolock' && viewer.viewOnly == false && typeof json.value == 'boolean') {
          obj.sendToAgent('{"ctrlChannel":"102938","type":"autolock","value":' + json.value + '}');
        } // Lock on disconnect
      }
      return;
    }
    ...
    switch (command) {
      ...
      case 5: // Compression
        ...
        for (var i in obj.viewers) {
          if (viewersimageType == null) {
            viewersimageType = obj.viewers[i].imageType;
          } else if (obj.viewers[i].imageType != viewersimageType) {
            viewersimageType = 1;
          } // Default to JPEG if viewers has different image formats
          if (viewersimageCompression == null || obj.viewers[i].imageCompression > viewersimageCompression) {
            viewersimageCompression = obj.viewers[i].imageCompression;
          }
          if (viewersimageScaling == null || obj.viewers[i].imageScaling > viewersimageScaling) {
            viewersimageScaling = obj.viewers[i].imageScaling;
          }
          if (viewersimageFrameRate == null || obj.viewers[i].imageFrameRate < viewersimageFrameRate) {
            viewersimageFrameRate = obj.viewers[i].imageFrameRate;
          }
        }
        if (
      ...
      case 8: // Pause and unpause
        ...
        for (var i in obj.viewers) {
          if (obj.viewers[i].desktopPaused == false) {
            viewersPaused = false;
          }
        }
      ...
      case 82: // Request display information
        if (obj.lastDisplayLocationData != null) {
          obj.sendToAgent(obj.lastDisplayLocationData);
        }
        break;
  ...
  obj.processAgentData = function (data) {
    if (typeof data != 'object' || data.length < 4) {
      if (typeof data == 'string') {
        ...
        if (json.type == 'console') {
          ...
          if (json.msg != null) {
            obj.lastConsoleMessage = data;
          } else {
            obj.lastConsoleMessage = null;
          }
    ...
    switch (command) {
      case 3: // Tile, check dimentions and store
        ...
        var sx = x / 16,
          sy = y / 16,
          sw = dimensions.width / 16,
          sh = dimensions.height / 16;
        obj.counter++;
        ...
        obj.imagesCount++;
        if (obj.imagesCount == 2000000000) {
          obj.imagesCount = 1;
        } // Loop the counter if needed
        ...
        for (var i = 0; i < sw; i++) {
          for (var j = 0; j < sh; j++) {
            ...
            if (oi != null && --obj.imagesCounters[oi] == 0) {
              ...
              for (var l in obj.viewers) {
                const v = obj.viewers[l];
                if (v.dataPtr == oi) {
                  v.dataPtr = d.next;
                }
              }
      ...
      case 7: // Screen Size, clear the screen state and compute the tile count
        ...
        if (Math.floor(obj.swidth) != obj.swidth) {
          obj.swidth = Math.floor(obj.swidth) + 1;
        }
        if (Math.floor(obj.sheight) != obj.sheight) {
          obj.sheight = Math.floor(obj.sheight) + 1;
        }
  ...
  function startRecording(domain, start, func) {
    if (obj.pendingRecording == 1 || obj.recordingFile != null) {
      func(true);
      return;
  ...
  function recordingSetup(domain, func) {
    ...
    if (
      ...
      (typeof domain.sessionrecording == 'object' &&
        (domain.sessionrecording.protocols == null || domain.sessionrecording.protocols.indexOf(2) >= 0))
    ) {
      record = true;
  ...
  function recordData(isAgent, data, func) {
    try {
      if (obj.recordingFile != null) {
        ...
        recordingEntry(obj.recordingFile.fd, 2, isAgent ? 0 : 2, data, function () {
          func(data);
        });
      } else {
        func(data);
  ...
  function recordingEntry(fd, type, flags, data, func, tag) {
    try {
      if (typeof data == 'string') {
        ...
        parent.parent.fs.write(fd, block, 0, block.length, function () {
          func(fd, tag);
        });
  ...
  parent.db.Get(nodeid, function (err, nodes) {
    ...
    recordingSetup(domain, function () {
      func(obj);
    });
  });
  return obj;
...
function CreateMeshRelayEx(parent, ws, req, domain, user, cookie) {
  // Do validation work
  if (cookie) {
    if (typeof cookie.expire == 'number' && cookie.expire <= Date.now()) {
      delete req.query.nodeid;
    } else if (typeof cookie.nid == 'string') {
      req.query.nodeid = cookie.nid;
    }
  }
  if (req.query.nodeid == null || req.query.p != '2' || req.query.id == null || domain == null) {
    try {
      ws.close();
    } catch (e) {}
    return;
  } // Not is not a valid remote desktop connection.
...
function CreateMeshRelayEx2(parent, ws, req, domain, user, cookie) {
  ...
  if (parent.trafficStats.desktopMultiplex == null) {
    parent.trafficStats.desktopMultiplex = { connections: 1, sessions: 0, in: 0, out: 0 };
  } else {
    parent.trafficStats.desktopMultiplex.connections++;
  }
  ...
  if (user == null && obj.req.query != null && obj.req.query.rauth != null) {
    ...
    if (rcookie.nodeid != null) {
      obj.nodeid = rcookie.nodeid;
    }
  }

  // If there is no authentication, drop this connection
  if (obj.id != null && obj.user == null && obj.ruserid == null && obj.nouser !== true && obj.rnouser !== true) {
    try {
      ws.close();
      parent.parent.debug('relay', 'DesktopRelay: Connection with no authentication (' + obj.req.clientIp + ')');
    } catch (e) {
      console.log(e);
    }
    return;
  ...
  obj.close = function (arg) {
    ...
    if (arg == 1 || arg == null) {
      try {
        ws.close();
        parent.parent.debug('relay', 'DesktopRelay: Soft disconnect (' + obj.req.clientIp + ')');
      } catch (e) {
        console.log(e);
      }
    ...
    if (arg == 2) {
      try {
        ...
        parent.parent.debug('relay', 'DesktopRelay: Hard disconnect (' + obj.req.clientIp + ')');
      } catch (e) {
        console.log(e);
      }
    ...
    if (
      obj.deskMultiplexor != null &&
      typeof obj.deskMultiplexor == 'object' &&
      obj.deskMultiplexor.removePeer(obj) == true
    ) {
  ...
  obj.sendAgentMessage = function (command, userid, domainid) {
    ...
    if (splitnodeid[0] == 'node' && splitnodeid[1] == domainid) {
      ...
      if (agent != null) {
        ...
        if ((rights != null && mesh != null) || (rights & 16) != 0) {
          // TODO: 16 is console permission, may need more gradular permission checking
          if (ws.sessionId) {
          ...
          if (command.rights != 0xffffffff && (command.rights & 0x100) != 0) {
        ...
        if (routing != null) {
          ...
          if (rights != null || (rights & 16) != 0) {
            // TODO: 16 is console permission, may need more gradular permission checking
            if (ws.sessionId) {
              command.fromSessionid = ws.sessionId;
            } // Set the session id, required for responses.
            command.rights = rights; // Add user rights flags to the message
            if (command.rights != 0xffffffff && (command.rights & 0x00000100) != 0) {
              command.rights -= 0x00000100;
            } // Since the multiplexor will enforce view-only, remove MESHRIGHT_REMOTEVIEWONLY
            if (typeof command.consent == 'number') {
              command.consent = command.consent | mesh.consent;
            } else {
  ...
  ws.on('message', function (data) {
    ...
    if (this.me.deskMultiplexor != null) {
      this.me.deskMultiplexor.processData(this.me, data);
    }
  });
  ...
  if (obj.authenticated) {
    ...
    if (cookie != null && cookie.nodeid != null && cookie.tcpport != null && cookie.domainid != null) {
      ...
      parent.db.Get(cookie.nodeid, function (err, docs) {
        ...
        if (obj.nouser !== true && (parent.GetNodeRights(user, node.meshid, node._id) & MESHRIGHT_REMOTECONTROL) == 0) {
        ...
        if (obj.id == undefined) {
          obj.id = ('' + Math.random()).substring(2);
        } // If there is no connection id, generate one.
        const command = {
          nodeid: node._id,
          action: 'msg',
          type: 'tunnel',
          value: '*/meshrelay.ashx?id=' + obj.id + '&rauth=' + rcookie,
      ...
      parent.db.Get(cookie.nid, function (err, docs) {
        ...
        const command = {
          ...
          action: 'msg',
          type: 'tunnel',
          value: '*/meshrelay.ashx?p=2&id=' + obj.id + '&rauth=' + rcookie + '&nodeid=' + node._id,
          soptions: {},
          usage: 2,
          rights: cookie.r,
  ...
  if (obj.user != null && obj.viewOnly == false) {
    ...
    parent.db.Get(obj.nodeid, function (err, docs) {
      ...
      if ((rights & 0x00000008) == 0) {
        try {
          obj.close();
        } catch (e) {}
        return;
      } // Check MESHRIGHT_ADMIN or MESHRIGHT_REMOTECONTROL
      if (rights != 0xffffffff && (rights & 0x00010000) != 0) {
        try {
          obj.close();
        } catch (e) {}
        return;
      } // Check MESHRIGHT_NODESKTOP
      if (rights == 0xffffffff || (rights & 0x00000100) == 0) {
        obj.viewOnly = false;
      } // Check MESHRIGHT_REMOTEVIEWONLY
      performRelay(0);
    });
  } else {
