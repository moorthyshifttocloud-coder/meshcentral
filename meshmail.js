...
module.exports.CreateMeshMail = function (parent, domain) {
  ...
  if (obj.config.sendgrid != null) {
    ...
    if (obj.config.sendgrid.verifyemail == true) {
      obj.verifyemail = true;
    }
    if (typeof obj.config.sendgrid.emaildelayseconds == 'number' && obj.config.sendgrid.emaildelayseconds > 0) {
      obj.emailDelay = obj.config.sendgrid.emaildelayseconds * 1000;
    }
  } else if (obj.config.smtp != null) {
    // Setup SMTP mail server
    if (typeof obj.config.smtp.emaildelayseconds == 'number' && obj.config.smtp.emaildelayseconds > 0) {
      obj.emailDelay = obj.config.smtp.emaildelayseconds * 1000;
    }
    if (obj.config.smtp.name == 'console') {
      ...
      var options = {
        name: obj.config.smtp.name,
        host: obj.config.smtp.host,
        secure: obj.config.smtp.tls == true,
        tls: {}
      };
      ...
      if (obj.config.smtp.port != null) {
        options.port = obj.config.smtp.port;
      }
      if (obj.config.smtp.tlscertcheck === false) {
        options.tls.rejectUnauthorized = false;
      }
      if (obj.config.smtp.tlsstrict === true) {
        options.tls.secureProtocol = 'SSLv23_method';
        options.tls.ciphers = 'RSA+AES:!aNULL:!MD5:!DSS';
      ...
      if (obj.config.smtp.auth != null && typeof obj.config.smtp.auth == 'object') {
        ...
        if (user == null && obj.config.smtp.user != null) {
          user = obj.config.smtp.user;
        }
        if (obj.config.smtp.auth.user != null && typeof obj.config.smtp.auth.user == 'string') {
          user = obj.config.smtp.auth.user;
        }
        if (user.toLowerCase().endsWith('@gmail.com')) {
          options = { service: 'gmail', auth: { user: user } };
          obj.config.smtp.host = 'gmail';
        } else {
          options.auth = { user: user };
        }
        if (obj.config.smtp.auth.type) {
          options.auth.type = obj.config.smtp.auth.type;
        }
        if (obj.config.smtp.auth.clientid) {
          options.auth.clientId = obj.config.smtp.auth.clientid;
          options.auth.type = 'OAuth2';
        }
        if (obj.config.smtp.auth.clientsecret) {
          options.auth.clientSecret = obj.config.smtp.auth.clientsecret;
        }
        if (obj.config.smtp.auth.refreshtoken) {
          options.auth.refreshToken = obj.config.smtp.auth.refreshtoken;
        }
      ...
      if (obj.config.smtp.verifyemail == true) {
        obj.verifyemail = true;
      }
    ...
    // Setup Sendmail
    if (typeof obj.config.sendmail.emaildelayseconds == 'number' && obj.config.sendmail.emaildelayseconds > 0) {
      obj.emailDelay = obj.config.sendmail.emaildelayseconds * 1000;
    }
    const nodemailer = require('nodemailer');
    var options = { sendmail: true };
    if (typeof obj.config.sendmail.newline == 'string') {
      options.newline = obj.config.sendmail.newline;
    }
    if (typeof obj.config.sendmail.path == 'string') {
      options.path = obj.config.sendmail.path;
    }
    if (Array.isArray(obj.config.sendmail.args)) {
      options.args = obj.config.sendmail.args;
    }
    obj.smtpServer = nodemailer.createTransport(options);
  }
  ...
  function getTemplate(name, domain, lang) {
    ...
    if (lang != null && lang != 'en') {
      ...
      if (
        obj.parent.fs.existsSync(translationsPath) &&
        obj.parent.fs.existsSync(translationsPathHtml) &&
        obj.parent.fs.existsSync(translationsPathTxt)
      ) {
    ...
    if ((htmlfile == null || txtfile == null) && lang != null && lang != 'en') {
      ...
      var translationsPathHtml = obj.parent.path.join(
    ...
    if (r.htmlSubject.startsWith('<div>')) {
      r.htmlSubject = r.htmlSubject.substring(5);
    }
    if (r.htmlSubject.endsWith('</div>')) {
      r.htmlSubject = r.htmlSubject.substring(0, r.htmlSubject.length - 6);
    }
    r.html = lines.join('\r\n');
  ...
  function strZone(str, marker, keep) {
    var lines = str.split('\r\n'),
      linesEx = [],
      removing = false;
    const startMarker = '<area-' + marker + '>',
      endMarker = '</area-' + marker + '>';
    for (var i in lines) {
      var line = lines[i];
      if (removing) {
  ...
  function mailReplacements(text, domain, options) {
    ...
    if (options.serverurl.endsWith('/')) {
      options.serverurl = options.serverurl.substring(0, options.serverurl.length - 1);
  ...
  obj.sendAccountLoginMail = function (domain, email, token, language, loginkey) {
    obj.checkEmail(email, function (checked) {
      if (checked) {
        ...
        if (loginkey != null) {
          options.urlargs1 = '?key=' + loginkey;
          options.urlargs2 = '&key=' + loginkey;
        } else {
          options.urlargs1 = '';
          options.urlargs2 = '';
        }
        ...
        // Get from field
        var from = null;
        if (obj.config.sendgrid && typeof obj.config.sendgrid.from == 'string') {
          from = obj.config.sendgrid.from;
        } else if (obj.config.smtp && typeof obj.config.smtp.from == 'string') {
          from = obj.config.smtp.from;
        }

        // Send the email
        obj.pendingMails.push({
          to: email,
          from: from,
          subject: mailReplacements(template.htmlSubject, domain, options),
          text: mailReplacements(template.txt, domain, options),
          html: mailReplacements(template.html, domain, options)
        });
        sendNextMail();
      }
    });
    ...
    obj.checkEmail(email, function (checked) {
      if (checked) {
        ...
        var options = {
          ...
          email: email,
          servername: domain.title ? domain.title : 'MeshCentral',
          password: password
        };
        if (loginkey != null) {
          options.urlargs1 = '?key=' + loginkey;
  ...
  obj.sendAgentInviteMail = function (
    domain,
    username,
    email,
    meshid,
    ...
    obj.checkEmail(email, function (checked) {
      if (checked) {
        ...
        const optionsHtml = {
          ...
          servername: EscapeHtml(domain.title ? domain.title : 'MeshCentral'),
          header: true,
          footer: false
        };
        const optionsTxt = {
        ...
        obj.pendingMails.push({
          to: email,
          from: from,
          subject: mailReplacements(template.htmlSubject, domain, optionsTxt),
          text: mailReplacements(template.txt, domain, optionsTxt),
          html: mailReplacements(template.html, domain, optionsHtml)
        });
        sendNextMail();
      }
    });
    ...
    obj.checkEmail(email, function (checked) {
      if (checked) {
        ...
        const optionsHtml = {
          ...
          helpusername: EscapeHtml(helpusername),
          helprequest: EscapeHtml(helprequest),
  ...
  function sendNextMail() {
    if (obj.sendingMail == true || obj.pendingMails.length == 0) {
      return;
    }
    ...
    if (obj.sendGridServer != null) {
      ...
      obj.sendGridServer.send(mailToSend).then(
        function () {
          obj.sendingMail = false;
      ...
      if (obj.smtpServer == 'console') {
        ...
        if (mailToSend.from == null) {
          delete mailToSend.from;
        }
        if (mailToSend.html == null) {
          delete mailToSend.html;
        }
  ...
  obj.parent.db.Get('MailCookieEncryptionKey', function (err, docs) {
    if (docs.length > 0 && docs[0].key != null && obj.parent.mailtokengen == null) {
      ...
      obj.parent.db.Set({
        _id: 'MailCookieEncryptionKey',
        key: obj.mailCookieEncryptionKey.toString('hex'),
        time: Date.now()
      });
    }
  });
  ...
  function createInviteLink(domain, meshid, flags, expirehours) {
    return (
      '/agentinvite?c=' +
      parent.encodeCookie({ a: 4, mid: meshid, f: flags, expire: expirehours * 60 }, parent.invitationLinkEncryptionKey)
    );
  }
  ...
  obj.checkEmail = function (email, func) {
    ...
    if (obj.approvedEmailDomains[emailSplit[1]] === true) {
      func(true);
      return;
    }
    require('dns').resolveMx(emailSplit[1], function (err, addresses) {
      ...
      if (err == null) {
        obj.approvedEmailDomains[emailSplit[1]] = true;
      }
      func(err == null);
    });
  };
  ...
  obj.notifyDeviceConnect = function (user, meshid, nodeid, connectTime, connectType, powerState, serverid, extraInfo) {
    ...
    if (obj.deviceNotifications[user._id] == null) {
      ...
      obj.deviceNotifications[user._id].timer = setTimeout(function () {
