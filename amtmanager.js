...
module.exports.CreateAmtManager = function (parent) {
  ...
  obj.rootCertBase64 = obj.parent.certificates.root.cert
    ...
    .split('\n')
    .join('');
  obj.rootCertCN = obj.parent.certificateOperations.forge.pki
    .certificateFromPem(obj.parent.certificates.root.cert)
    .subject.getField('CN').value;

  // 802.1x authentication protocols
  const netAuthStrings = [
    'eap-tls',
    'eap-ttls/mschapv2',
    'peapv0/eap-mschapv2',
    'peapv1/eap-gtc',
  ...
  for (var domainid in parent.config.domains) {
    ...
    if (typeof domain.amtmanager != 'object') {
      domain.amtmanager = {};
    }
    ...
    if (Array.isArray(domain.amtmanager.adminaccounts) == true) {
      for (var i = 0; i < domain.amtmanager.adminaccounts.length; i++) {
        var c = domain.amtmanager.adminaccounts[i],
          c2 = {};
        ...
        if (typeof c.pass == 'string') {
          c2.pass = c.pass;
          if (obj.amtAdminAccounts[domainid] == null) {
            obj.amtAdminAccounts[domainid] = [];
          }
    ...
    if (Array.isArray(domain.amtmanager.environmentdetection) == true) {
      ...
      for (var i = 0; i < domain.amtmanager.environmentdetection.length; i++) {
        ...
        if (typeof x == 'string' && x != '' && x.length < 64 && envDetect.indexOf(x) == -1) {
          envDetect.push(x);
        }
        if (envDetect.length >= 4) break; // Maximum of 4 DNS suffix
      }
      if (envDetect.length > 0) {
        domain.amtmanager.environmentdetection = envDetect;
      } else {
    ...
    if (domain.amtmanager['802.1x'] != null && typeof domain.amtmanager['802.1x'] == 'object') {
      if (domain.amtmanager['802.1x'].satellitecredentials != null) {
        if (typeof domain.amtmanager['802.1x'].satellitecredentials != 'string') {
          delete domain.amtmanager['802.1x'];
        } else {
          const userSplit = domain.amtmanager['802.1x'].satellitecredentials.split('/');
          if (userSplit.length > 3) {
            delete domain.amtmanager['802.1x'];
          } else if (userSplit.length == 2) {
            domain.amtmanager['802.1x'].satellitecredentials = 'user/' + domain.id + '/' + userSplit[1];
          } else if (userSplit.length == 1) {
            domain.amtmanager['802.1x'].satellitecredentials = 'user/' + domain.id + '/' + userSplit[0];
          }
        }
      }
      ...
      if (typeof domain.amtmanager['802.1x'].servercertificatename != 'string') {
        ...
        if (typeof domain.amtmanager['802.1x'].servercertificatenamecomparison == 'string') {
          domain.amtmanager['802.1x'].servercertificatenamecomparison = serverCertCompareStrings.indexOf(
            domain.amtmanager['802.1x'].servercertificatenamecomparison.toLowerCase()
          );
          if (domain.amtmanager['802.1x'].servercertificatenamecomparison == -1) {
            domain.amtmanager['802.1x'].servercertificatenamecomparison = 2;
          } // Default to full name compare
        }
      }

      if (typeof domain.amtmanager['802.1x'].authenticationprotocol == 'string') {
        domain.amtmanager['802.1x'].authenticationprotocol = netAuthStrings.indexOf(
          domain.amtmanager['802.1x'].authenticationprotocol.toLowerCase()
        );
        if (domain.amtmanager['802.1x'].authenticationprotocol == -1) {
          delete domain.amtmanager['802.1x'];
        }
      }
    }
    ...
    if (Array.isArray(domain.amtmanager.wifiprofiles) == true) {
      ...
      for (var i = 0; i < domain.amtmanager.wifiprofiles.length; i++) {
        ...
        if (typeof wifiProfile.ssid == 'string' && wifiProfile.ssid != '') {
          if (wifiProfile.name == null || wifiProfile.name == '') {
            wifiProfile.name = wifiProfile.ssid;
          }

          // Authentication
          if (typeof wifiProfile.authentication == 'string') {
            wifiProfile.authentication = wifiProfile.authentication.toLowerCase();
          }
          if (wifiProfile.authentication == 'wpa-psk') {
            wifiProfile.authentication = 4;
          }
          if (wifiProfile.authentication == 'wpa2-psk') {
            wifiProfile.authentication = 6;
          }
          if (wifiProfile.authentication == 'wpa-8021x') {
            wifiProfile.authentication = 5;
          }
          if (wifiProfile.authentication == 'wpa2-802.1x') {
            wifiProfile.authentication = 7;
          }
          if (wifiProfile.authentication == 'wpa3-sae-802.1x') {
            wifiProfile.authentication = 32768;
          }
          if (wifiProfile.authentication == 'wpa3-owe-802.1x') {
            wifiProfile.authentication = 32769;
          }
          if (typeof wifiProfile.authentication != 'number') {
            if (wifiProfile['802.1x']) {
              wifiProfile.authentication = 7;
            } // Default to WPA2-802.1x
            else {
              wifiProfile.authentication = 6;
            } // Default to WPA2-PSK
          }
          ...
          // Encyption
          if (typeof wifiProfile.encryption == 'string') {
            wifiProfile.encryption = wifiProfile.encryption.toLowerCase();
          }
          if (wifiProfile.encryption == 'ccmp-aes' || wifiProfile.encryption == 'ccmp') {
            wifiProfile.encryption = 4;
          }
          if (wifiProfile.encryption == 'tkip-rc4' || wifiProfile.encryption == 'tkip') {
            wifiProfile.encryption = 3;
          }
          if (typeof wifiProfile.encryption != 'number') {
            wifiProfile.encryption = 4;
          } // Default to CCMP-AES
          ...
          if ([4, 6].indexOf(wifiProfile.authentication) >= 0) {
            ...
            if (
              typeof wifiProfile.password != 'string' ||
              wifiProfile.password.length < 8 ||
              wifiProfile.password.length > 63
            )
              continue;
  ...
  function removeAmtDevice(dev, tag) {
    ...
    if (dev.taskid != null) {
      obj.parent.taskLimiter.completed(dev.taskid);
      delete dev.taskLimiter;
    }
    ...
    if (dev.polltimer != null) {
      clearInterval(dev.polltimer);
      delete dev.polltimer;
    }

    // Remove the device from the list
    devices.splice(i, 1);
    if (devices.length == 0) {
      delete obj.amtDevices[dev.nodeid];
    } else {
      obj.amtDevices[dev.nodeid] = devices;
    }
  ...
  function removeDevice(nodeid) {
    ...
    for (var i in devices) {
      ...
      // Clean up this device
      if (dev.amtstack != null) {
        dev.amtstack.wsman.comm.FailAllError = 999;
        delete dev.amtstack;
    ...
    if (domain.amtmanager['802.1x'] != null) {
      ...
      parent.DispatchEvent([domain.amtmanager['802.1x'].satellitecredentials], obj, {
        action: 'satellite',
        subaction: '802.1x-Profile-Remove',
        satelliteFlags: 2,
        nodeid: nodeid,
        domain: nodeid.split('/')[1],
        nolog: 1,
        ver: dev.intelamt.ver
      });
    }
  ...
  obj.startAmtManagement = function (nodeid, connType, connection) {
    ...
    if (devices != null) {
      for (var i in devices) {
        if (devices[i].mpsConnection == connection || devices[i].host == connection) {
          dev = devices[i];
        }
      }
    }
    ...
    if (typeof connection == 'object') {
      dev.mpsConnection = connection;
    }
    dev.consoleMsg = function deviceConsoleMsg(msg) {
      parent.debug('amt', deviceConsoleMsg.dev.name, msg);
      if (typeof deviceConsoleMsg.conn == 'object') {
        deviceConsoleMsg.conn.ControlMsg({ action: 'console', msg: msg });
      }
    };
    dev.consoleMsg.conn = connection;
    dev.consoleMsg.dev = dev;
    dev.controlMsg = function deviceControlMsg(msg) {
      if (typeof deviceControlMsg.conn == 'object') {
        deviceControlMsg.conn.ControlMsg(msg);
      }
    };
    ...
    obj.parent.taskLimiter.launch(
      function (dev, taskid, taskLimiterQueue) {
  ...
  obj.getStatusString = function () {
    ...
    for (var nodeid in obj.amtDevices) {
      ...
      for (var i in devices) {
        ...
        if (dev.polltimer != null) {
          items.push('Polling Power');
        }
        r += '  ' + items.join(', ') + '\r\n';
      }
    }
    if (r == '') {
      r = 'No managed Intel AMT devices';
    }
    return r;
  };
  ...
  obj.mpsControlMessage = function (nodeid, conn, connType, jsondata) {
    ...
    for (var i in devices) {
      if (devices[i].mpsConnection === conn) {
        dev = devices[i];
      }
    }
    if (dev == null) return;
    ...
    switch (jsondata.action) {
      ...
      case 'meiState':
        if (dev.acmactivate == 1) {
          ...
          var continueAcmFunc = function continueAcm() {
            if (isAmtDeviceValid(continueAcm.dev)) {
              activateIntelAmtAcmEx0(continueAcm.dev);
            }
          };
      ...
      case 'stopConfiguration':
        ...
        if (jsondata.value == 3) {
          delete dev.acmactivate;
          activateIntelAmtAcmEx0(dev);
        } // Intel AMT was already not in in-provisioning state, keep going right away.
          ...
          var askStateFunc = function askState() {
            if (isAmtDeviceValid(askState.dev)) {
              askState.dev.controlMsg({ action: 'mestate' });
            }
          };
          ...
          dev.consoleMsg(
            'Unknown stopConfiguration() state of ' + jsondata.value + '. Continuing with ACM activation...'
          );
          delete dev.acmactivate;
          activateIntelAmtAcmEx0(dev);
        }
    ...
    switch (event.action) {
      ...
      case 'wakedevices': {
        ...
        if (Array.isArray(event.nodeids)) {
          for (var i in event.nodeids) {
            performPowerAction(event.nodeids[i], 2);
          }
        }
        break;
      }
      case 'oneclickrecovery': {
        // React to Intel AMT One Click Recovery command
        if (event.noact == 1) return; // Take no action on these events. We are likely in peering mode and need to only act when the database signals the change in state.
        if (Array.isArray(event.nodeids)) {
          for (var i in event.nodeids) {
            performOneClickRecoveryAction(event.nodeids[i], event.file);
          }
        }
        break;
      }
      case 'amtpoweraction': {
        ...
        if (Array.isArray(event.nodeids)) {
          for (var i in event.nodeids) {
            performPowerAction(event.nodeids[i], event.actiontype);
          }
        }
        break;
      }
      case 'changenode': {
        // React to changes in a device
        ...
        if (devices != null) {
          for (var i in devices) {
            ...
            if (event.node.intelamt != null) {
              if (dev.intelamt == null) {
                dev.intelamt = {};
              }
              if (typeof event.node.intelamt.version == 'string' && event.node.intelamt.version != dev.intelamt.ver) {
                dev.intelamt.ver = event.node.intelamt.version;
              }
              if (typeof event.node.intelamt.user == 'string' && event.node.intelamt.user != dev.intelamt.user) {
                dev.intelamt.user = event.node.intelamt.user;
              }
              if (typeof event.node.intelamt.pass == 'string' && event.node.intelamt.pass != dev.intelamt.pass) {
                dev.intelamt.pass = event.node.intelamt.pass;
              }
              if (
                typeof event.node.intelamt.mpspass == 'string' &&
                event.node.intelamt.mpspass != dev.intelamt.mpspass
              ) {
                dev.intelamt.mpspass = event.node.intelamt.mpspass;
              }
              if (typeof event.node.intelamt.host == 'string' && event.node.intelamt.host != dev.intelamt.host) {
                dev.intelamt.host = event.node.intelamt.host;
              }
              if (typeof event.node.intelamt.realm == 'string' && event.node.intelamt.realm != dev.intelamt.realm) {
                dev.intelamt.realm = event.node.intelamt.realm;
              }
              if (typeof event.node.intelamt.hash == 'string' && event.node.intelamt.hash != dev.intelamt.hash) {
                dev.intelamt.hash = event.node.intelamt.hash;
              }
              if (typeof event.node.intelamt.tls == 'number' && event.node.intelamt.tls != dev.intelamt.tls) {
                dev.intelamt.tls = event.node.intelamt.tls;
              }
              if (typeof event.node.intelamt.state == 'number' && event.node.intelamt.state != dev.intelamt.state) {
                dev.intelamt.state = event.node.intelamt.state;
              }
            }
        ...
        if (rescan && parent.amtScanner != null) {
          parent.amtScanner.performSpecificScan(event.node);
        }
        break;
      }
      case 'meshchange': {
      ...
      case 'satelliteResponse': {
        ...
        if (devices != null) {
          for (var i in devices) {
            if (devices[i].netAuthSatReqId == event.reqid) {
              devFound = devices[i];
            }
          }
        }
        ...
        switch (event.subaction) {
          ...
          case '802.1x-Profile-Completed': {
            ...
            if (devFound.netAuthSatReqTimer != null) {
              clearTimeout(devFound.netAuthSatReqTimer);
              delete devFound.netAuthSatReqTimer;
            }
            ...
            if (typeof event.response.authProtocol != 'number') {
              delete devFound.netAuthSatReqId;
              break;
            }
  ...
  function fetchIntelAmtInformation(dev) {
    parent.db.Get(dev.nodeid, function (err, nodes) {
      if (nodes == null || nodes.length != 1) {
        removeAmtDevice(dev, 4);
        return;
      }
      const node = nodes[0];
      if (node.intelamt == null || node.meshid == null) {
        removeAmtDevice(dev, 5);
        return;
      }
      const mesh = parent.webserver.meshes[node.meshid];
      if (mesh == null) {
        removeAmtDevice(dev, 6);
        return;
      }
      if (dev == null) {
        return;
      ...
      if (mesh.amt != null) {
        ...
        if (mesh.amt.type == 4) {
          ...
          if (mesh.amt.cirasetup) {
            ciraPolicy = mesh.amt.cirasetup;
          }
          if (mesh.amt.badpass) {
            badPass = mesh.amt.badpass;
          }
          if (typeof mesh.amt.password == 'string' && mesh.amt.password != '') {
            password = mesh.amt.password;
          }
        }
      }
      if (amtPolicy == 0) {
        ciraPolicy = 0;
      } // If no policy, don't change CIRA state.
      if (amtPolicy == 1) {
        ciraPolicy = 1;
      } // If deactivation policy, clear CIRA.
      ...
      if (
        dev.connType == 2 &&
        dev.mpsConnection != null &&
        dev.mpsConnection.tag != null &&
        dev.mpsConnection.tag.meiState != null
      ) {
        dev.aquired = {};
        if (
          typeof dev.mpsConnection.tag.meiState.OsHostname == 'string' &&
          typeof dev.mpsConnection.tag.meiState.OsDnsSuffix == 'string'
        ) {
          dev.host = dev.aquired.host =
            dev.mpsConnection.tag.meiState.OsHostname + '.' + dev.mpsConnection.tag.meiState.OsDnsSuffix;
        }
        ...
        if (
          typeof dev.mpsConnection.tag.meiState['Versions'] == 'object' &&
          typeof dev.mpsConnection.tag.meiState['Versions']['AMT'] == 'string'
        ) {
        ...
        if (typeof dev.mpsConnection.tag.meiState['Flags'] == 'number') {
          ...
          if (flags & 2) {
            dev.aquired.controlMode = 1;
          } // CCM
          if (flags & 4) {
            dev.aquired.controlMode = 2;
          } // ACM
        }
        UpdateDevice(dev);
      }
  ...
  function attemptInitialContact(dev) {
    ...
    if (
      ...
      if (
        (dev.policy.amtPolicy == 3 || dev.policy.amtPolicy == 4) &&
        dev.mpsConnection.tag.meiState.ProvisioningState == 2 &&
        (dev.mpsConnection.tag.meiState.Flags & 2) != 0
      ) {
    ...
    if (dev.acctry == null) {
      ...
      if (
        ...
        dev.mpsConnection.tag != null &&
        dev.mpsConnection.tag.meiState != null &&
        typeof dev.mpsConnection.tag.meiState.amtuser == 'string' &&
        typeof dev.mpsConnection.tag.meiState.amtpass == 'string' &&
        dev.mpsConnection.tag.meiState.amtuser != '' &&
        dev.mpsConnection.tag.meiState.amtpass != ''
      ) {
      ...
      if (
        typeof dev.intelamt.user == 'string' &&
        typeof dev.intelamt.pass == 'string' &&
        dev.intelamt.user != '' &&
        dev.intelamt.pass != ''
      ) {
        dev.acctry.push([dev.intelamt.user, dev.intelamt.pass]);
      }

      // Add the policy password as an alternative
      if (typeof dev.policy.password == 'string' && dev.policy.password != '') {
        dev.acctry.push(['admin', dev.policy.password]);
      }

      // Add any configured admin account as alternatives
      if (obj.amtAdminAccounts[dev.domainid] != null) {
        for (var i in obj.amtAdminAccounts[dev.domainid]) {
          dev.acctry.push([obj.amtAdminAccounts[dev.domainid][i].user, obj.amtAdminAccounts[dev.domainid][i].pass]);
        }
      }

      // Add any previous passwords for the device UUID as alternative
      if (
        parent.amtPasswords != null &&
        dev.mpsConnection != null &&
        dev.mpsConnection.tag != null &&
        dev.mpsConnection.tag.meiState != null &&
        dev.mpsConnection.tag.meiState.UUID != null &&
        parent.amtPasswords[dev.mpsConnection.tag.meiState.UUID] != null
      ) {
      ...
      for (var i = 0; i < dev.acctry.length; i++) {
        var found = false;
        for (var j = 0; j < acctry2.length; j++) {
          if (dev.acctry[i][0] == acctry2[j][0] && dev.acctry[i][1] == acctry2[j][1]) {
            found = true;
          }
        }
        if (found == false) {
          acctry2.push(dev.acctry[i]);
        }
      }
      dev.acctry = acctry2;
    ...
    if (dev.acctry == null || dev.acctry.length == 0) {
      removeAmtDevice(dev, 9);
      return;
    } // No Intel AMT credentials to try
    var user = dev.acctry[0][0],
      pass = dev.acctry[0][1]; // Try the first user/pass in the list
    ...
    switch (dev.connType) {
      case 0: // CIRA
        ...
        if (ciraconn == null || ciraconn.tag == null || ciraconn.tag.boundPorts == null) {
          removeAmtDevice(dev, 9);
          return;
        } // CIRA connection is not on this server, no need to deal with this device anymore.

        // See if we need to perform TLS or not. We prefer not to do TLS within CIRA.
        var dotls = -1;
        if (ciraconn.tag.boundPorts.indexOf('16992')) {
          dotls = 0;
        } else if (ciraconn.tag.boundPorts.indexOf('16993')) {
          dotls = 1;
        }
        if (dotls == -1) {
          removeAmtDevice(dev, 10);
          return;
        } // The Intel AMT ports are not open, not a device we can deal with.
        ...
        dev.amtstack.BatchEnum(
          null,
          ['*AMT_GeneralSettings', '*IPS_HostBasedSetupService'],
          attemptLocalConnectResponse
        );
        break;
      case 1: // CIRA-Relay
      case 2: // CIRA-LMS
        ...
        var ciraconn = dev.mpsConnection;
        if (ciraconn == null || ciraconn.tag == null || ciraconn.tag.boundPorts == null) {
          removeAmtDevice(dev, 11);
          return;
        } // Relay connection not valid

        // Connect now
        var comm;
        if (
          dev.tlsfail !== true &&
          parent.config.domains[dev.domainid].amtmanager.tlsconnections !== false &&
          dev.intelamt.tls == 1
        ) {
      ...
      case 3: // Local LAN
        ...
        if (dev.intelamt == null || (dev.intelamt.state != null && dev.intelamt.state != 2)) {
          removeAmtDevice(dev, 12);
          return;
        }
        ...
        if (typeof dev.host != 'string') {
          removeAmtDevice(dev, 13);
          return;
        } // Local connection not valid
        ...
        if (obj.activeLocalConnections[dev.host] != null) {
          ...
          var tryAgainFunc = function tryAgainFunc() {
            if (obj.amtDevices[tryAgainFunc.dev.nodeid] != null) {
              attemptInitialContact(tryAgainFunc.dev);
            }
          };
  ...
  function attemptLocalConnectResponse(stack, name, responses, status) {
    ...
    if (dev.connType == 3) {
      delete obj.activeLocalConnections[dev.host];
    }
    ...
    if (
      status == 200 &&
      responses['AMT_GeneralSettings'] != null &&
      ...
      if (typeof stack.wsman.comm.amtVersion == 'string') {
        // Set the Intel AMT version using the HTTP header if present
      ...
      if (dev.connType == 1 || dev.connType == 3) {
        dev.aquired.tls = stack.wsman.comm.xtls;
      } // Only set the TLS state if in relay or local mode. When using CIRA, this is auto-detected.
      if (stack.wsman.comm.xtls == 1) {
        dev.aquired.hash = stack.wsman.comm.xtlsCertificate.fingerprint.split(':').join('').toLowerCase();
      } else {
        delete dev.aquired.hash;
      }
      UpdateDevice(dev);
      ...
      if (
        ...
        dev.mpsConnection.tag.meiState.UUID != null &&
        parent.amtPasswords[dev.mpsConnection.tag.meiState.UUID] != null &&
        parent.amtPasswords[dev.mpsConnection.tag.meiState.UUID][0] != dev.aquired.pass
      ) {
        parent.certificateOperations.logAmtActivation(parent.config.domains[dev.domainid], {
          time: new Date(),
          action: 'amtpassword',
          domain: dev.domainid,
          amtUuid: dev.mpsConnection.tag.meiState.UUID,
      ...
      attemptSyncClock(dev, function (dev) {
        ...
        attemptTlsSync(dev, function (dev) {
          ...
          if (dev.switchToTls == 1) {
            delete dev.switchToTls;
            attemptInitialContact(dev);
            return;
          }
                  ...
                  attemptCleanCertsSync(dev, function (dev) {
                    ...
                    attemptFetchHardwareInventory(dev, function (dev) {
                      ...
                      if (dev.connType != 2) {
                        ...
                        var ppfunc = function powerPoleFunction() {
                          fetchPowerState(powerPoleFunction.dev);
                        };
                        ppfunc.dev = dev;
                        if (dev.polltimer) {
                          clearInterval(dev.polltimer);
      ...
      if (dev.connType != 0 && dev.tlsfail !== false && status == 408) {
        // If not using CIRA and we get a 408 error while using non-TLS, try TLS.
        // non-TLS error on a local connection, try again with TLS
        dev.tlsfail = false;
        dev.intelamt.tls = 1;
        attemptInitialContact(dev);
        return;
      } else if (dev.connType != 0 && dev.tlsfail !== true && status == 408) {
        // If not using CIRA and we get a 408 error while using TLS, try non-TLS.
        ...
        if (
          dev.connType == 2 &&
          dev.policy.badPass == 1 &&
          dev.mpsConnection != null &&
          dev.mpsConnection.tag != null &&
          dev.mpsConnection.tag.meiState != null &&
          dev.mpsConnection.tag.meiState.Flags != null &&
          (dev.mpsConnection.tag.meiState.Flags & 2) != 0
        ) {
          deactivateIntelAmtCCM(dev);
          return;
  ...
  function UpdateDevice(dev) {
    ...
    parent.db.Get(dev.nodeid, function (err, nodes) {
      ...
      if (device.intelamt == null) {
        device.intelamt = {};
      }
      if (typeof dev.aquired.version == 'string' && dev.aquired.version != device.intelamt.ver) {
        change = 1;
        log = 1;
        device.intelamt.ver = dev.aquired.version;
        changes.push('AMT version');
      }
      if (typeof dev.aquired.user == 'string' && dev.aquired.user != device.intelamt.user) {
        change = 1;
        log = 1;
        device.intelamt.user = dev.aquired.user;
        changes.push('AMT user');
      }
      if (typeof dev.aquired.pass == 'string' && dev.aquired.pass != device.intelamt.pass) {
        change = 1;
        log = 1;
        device.intelamt.pass = dev.aquired.pass;
        changes.push('AMT pass');
      }
      if (typeof dev.aquired.mpspass == 'string' && dev.aquired.mpspass != device.intelamt.mpspass) {
        change = 1;
        log = 1;
        device.intelamt.mpspass = dev.aquired.mpspass;
        changes.push('AMT MPS pass');
      }
      if (typeof dev.aquired.host == 'string' && dev.aquired.host != device.intelamt.host) {
        change = 1;
        log = 1;
        device.intelamt.host = dev.aquired.host;
        changes.push('AMT host');
      }
      if (typeof dev.aquired.realm == 'string' && dev.aquired.realm != device.intelamt.realm) {
        change = 1;
        log = 1;
        device.intelamt.realm = dev.aquired.realm;
        changes.push('AMT realm');
      }
      if (typeof dev.aquired.hash == 'string' && dev.aquired.hash != device.intelamt.hash) {
        change = 1;
        log = 1;
        device.intelamt.hash = dev.aquired.hash;
        changes.push('AMT hash');
      }
      if (typeof dev.aquired.tls == 'number' && dev.aquired.tls != device.intelamt.tls) {
        change = 1;
        log = 1;
        device.intelamt.tls = dev.aquired.tls; /*changes.push('AMT TLS');*/
      }
      if (typeof dev.aquired.state == 'number' && dev.aquired.state != device.intelamt.state) {
        change = 1;
        log = 1;
        device.intelamt.state = dev.aquired.state;
      ...
      if (typeof dev.aquired.warn == 'number') {
        if (dev.aquired.warn == 0 && device.intelamt.warn != null) {
          delete device.intelamt.warn;
          change = 1;
        } else if (dev.aquired.warn != 0 && dev.aquired.warn != device.intelamt.warn) {
          device.intelamt.warn = dev.aquired.warn;
          change = 1;
        }
      }
      ...
      var flags = 0;
      if (typeof device.intelamt.flags == 'number') {
        flags = device.intelamt.flags;
      }
      if (dev.aquired.controlMode == 1) {
        if ((flags & 4) != 0) {
          flags -= 4;
        }
        if ((flags & 2) == 0) {
      ...
      if (device.intelamt.flags != flags) {
        change = 1;
        log = 1;
        device.intelamt.flags = flags;
        changes.push('AMT flags');
      }
      ...
      if (change == 1) {
        ...
        var event = {
          ...
          nodeid: device._id,
          domain: domain.id,
          node: parent.webserver.CloneSafeNode(device)
        };
        if (changes.length > 0) {
        ...
        if (
          log == 0 ||
          (obj.agentInfo && obj.agentInfo.capabilities && obj.agentInfo.capabilities & 0x20) ||
          changes.length == 0
        ) {
          event.nolog = 1;
  ...
  function ClearDeviceCredentials(dev) {
    ...
    parent.db.Get(dev.nodeid, function (err, nodes) {
      ...
      if (device.intelamt.user != null) {
        change = 1;
        log = 1;
        delete device.intelamt.user;
        changes.push('AMT user');
      }
      if (device.intelamt.pass != null) {
        change = 1;
        log = 1;
        delete device.intelamt.pass;
        changes.push('AMT pass');
      }
  ...
  function fetchPowerState(dev) {
    ...
    dev.amtstack.BatchEnum(null, ['CIM_ServiceAvailableToElement'], function (stack, name, responses, status) {
      ...
      if (
        status != 200 ||
        responses['CIM_ServiceAvailableToElement'] == null ||
        responses['CIM_ServiceAvailableToElement'].responses == null ||
        responses['CIM_ServiceAvailableToElement'].responses.length < 1
      )
        return; // If the polling fails, just skip it.
      ...
      if (powerstate == 2 && dev.aquired.majorver != null && dev.aquired.majorver > 9) {
        ...
        dev.amtstack.Get('IPS_PowerManagementService', function (stack, name, response, status) {
          ...
          if (response.Body.OSPowerSavingState == 2) {
            meshPowerState = 1;
          } // Fully powered (S0);
          else if (response.Body.OSPowerSavingState == 3) {
            meshPowerState = 2;
          } // Modern standby (We are going to call this S1);

          // Set OS power state - connType: 0 = CIRA, 1 = CIRA-Relay, 2 = CIRA-LMS, 3 = LAN
          if (meshPowerState >= 0) {
            parent.SetConnectivityState(
              dev.meshid,
              dev.nodeid,
              Date.now(),
              dev.connType == 3 ? 4 : 2,
        ...
        var meshPowerState = -1,
          powerConversionTable = [-1, -1, 1, 2, 3, 6, 6, 5, 6];
        if (powerstate < powerConversionTable.length) {
          meshPowerState = powerConversionTable[powerstate];
        } else {
          powerstate = 6;
        }
    ...
    for (var i in devices) {
      ...
      if (dev.connType != 2 && dev.state == 1 && dev.amtstack != null) {
        ...
        if (action <= 10) {
          ...
          try {
            dev.amtstack.RequestPowerStateChange(action, performPowerActionResponse);
          } catch (ex) {}
        } else {
  ...
  function performAdvancedPowerActionResponse(stack, name, responses, status) {
    ...
    dev.amtstack.Put(
      ...
      function (stack, name, response, status, tag) {
        ...
        // Set boot config
        dev.amtstack.SetBootConfigRole(
          1,
          function (stack, name, response, status, tag) {
            const dev = stack.dev;
            if (obj.amtDevices[dev.nodeid] == null || status != 200) return; // Device no longer exists or error
            ...
            var bootDevice =
              action === 15 || action === 16
                ? '<Address xmlns="http://schemas.xmlsoap.org/ws/2004/08/addressing">http://schemas.xmlsoap.org/ws/2004/08/addressing</Address><ReferenceParameters xmlns="http://schemas.xmlsoap.org/ws/2004/08/addressing"><ResourceURI xmlns="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd">http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_BootSourceSetting</ResourceURI><SelectorSet xmlns="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd"><Selector Name="InstanceID">Intel(r) AMT: Force PXE Boot</Selector></SelectorSet></ReferenceParameters>'
                : null;
            dev.amtstack.CIM_BootConfigSetting_ChangeBootOrder(
              bootDevice,
              function (stack, name, response, status) {
                ...
                try {
                  dev.amtstack.RequestPowerStateChange(dev.powerAction, performPowerActionResponse);
                } catch (ex) {}
              },
              0,
              1
  ...
  function performPowerActionResponse(stack, name, responses, status) {
    ...
    if (action == 2 && dev.aquired.majorver > 9) {
      try {
        dev.amtstack.RequestOSPowerStateChange(2, function (stack, name, response, status) {});
      } catch (ex) {}
    }
  }
  ...
  function performOneClickRecoveryAction(nodeid, file) {
    ...
    for (var i in devices) {
      ...
      if (dev.connType != 2 && dev.state == 1 && dev.amtstack != null) {
        ...
        dev.amtstack.BatchEnum(
          null,
          ['AMT_PublicKeyCertificate', '*AMT_BootCapabilities'],
          performOneClickRecoveryActionEx
        );
      }
    }
  }
  ...
  function performOneClickRecoveryActionEx(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to get security information (' + status + ').');
      delete dev.ocrfile;
      return;
    }
    ...
    if (responses['AMT_BootCapabilities'].response['ForceUEFIHTTPSBoot'] !== true) {
      dev.consoleMsg('This Intel AMT device does not support UEFI HTTPS boot  (' + status + ').');
      delete dev.ocrfile;
      return;
    }
    ...
    for (var i in xxCertificates) {
      ...
      xxCertificates[i].X509CertificateBin = Buffer.from(xxCertificates[i]['X509Certificate'], 'base64').toString(
        'binary'
      );
  ...
  function performOneClickRecoveryActionEx3(stack, name, response, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to get boot settings data (' + status + ').');
      delete dev.ocrfile;
      return;
    }
    ...
    var url =
      'https://' +
      parent.webserver.certificates.AmtMpsName +
      ':' +
      (parent.args.mpsaliasport != null ? parent.args.mpsaliasport : parent.args.mpsport) +
      '/c/' +
      cookie +
      '.efi';
    delete dev.ocrfile;
    ...
    r['UefiBootParametersArray'] = Buffer.from(
      makeUefiBootParam(1, url) + makeUefiBootParam(20, 1, 1) + makeUefiBootParam(30, 0, 2),
      'binary'
    ).toString('base64');
    ...
    dev.amtstack.CIM_BootConfigSetting_ChangeBootOrder(
      ...
      function (stack, name, response, status) {
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        if (status != 200) {
          dev.consoleMsg('Failed to set boot order (' + status + ').');
          return;
        }
  ...
  function performOneClickRecoveryActionEx4(stack, name, response, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to set boot settings data (' + status + ').');
      return;
    }
    dev.amtstack.SetBootConfigRole(
      1,
      function (stack, name, response, status) {
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        if (status != 200) {
          dev.consoleMsg('Failed to set boot config role (' + status + ').');
          return;
        }
        dev.amtstack.CIM_BootConfigSetting_ChangeBootOrder(
          '<Address xmlns="http://schemas.xmlsoap.org/ws/2004/08/addressing">http://schemas.xmlsoap.org/ws/2004/08/addressing</Address><ReferenceParameters xmlns="http://schemas.xmlsoap.org/ws/2004/08/addressing"><ResourceURI xmlns="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd">http://schemas.dmtf.org/wbem/wscim/1/cim-schema/2/CIM_BootSourceSetting</ResourceURI><SelectorSet xmlns="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd"><Selector Name="InstanceID">Intel(r) AMT: Force OCR UEFI HTTPS Boot</Selector></SelectorSet></ReferenceParameters>',
          function (stack, name, response, status) {
            if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
            if (status != 200) {
              dev.consoleMsg('Failed to set boot config (' + status + ').');
              return;
            }
            dev.amtstack.RequestPowerStateChange(10, function (stack, name, response, status) {
              // 10 = Reset, 2 = Power Up
              if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
              if (status != 200) {
                dev.consoleMsg('Failed to perform power action (' + status + ').');
                return;
              }
  ...
  function attemptSyncClock(dev, func) {
    ...
    if (dev.policy.amtPolicy == 0) {
      func(dev);
      return;
    } // If there is no Intel AMT policy, skip this operation.
    dev.taskCount = 1;
  ...
  function attemptSyncClockEx(stack, name, response, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to get clock (' + status + ').');
      removeAmtDevice(dev, 17);
      return;
    }
    ...
    if (Math.abs(t - now) > 10000) {
      // If the Intel AMT clock is more than 10 seconds off, set it.
      dev.consoleMsg('Performing clock sync.');
      var Tm1 = Math.round(now.getTime() / 1000);
      dev.amtstack.AMT_TimeSynchronizationService_SetHighAccuracyTimeSynch(
        response.Body['Ta0'],
        Tm1,
        Tm1,
        attemptSyncClockSet
  ...
  function attemptSyncClockSet(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to sync clock (' + status + ').');
      removeAmtDevice(dev, 18);
    }
    devTaskCompleted(dev);
  }
  ...
  function attemptTlsSync(dev, func) {
    ...
    dev.amtstack.BatchEnum(
      null,
      ['AMT_PublicKeyCertificate', 'AMT_PublicPrivateKeyPair', 'AMT_TLSSettingData', 'AMT_TLSCredentialContext'],
      attemptTlsSyncEx
    );
  }
  ...
  function generateKeyPairWithRetry(dev, func) {
    ...
    dev.amtstack.AMT_PublicKeyManagementService_GenerateKeyPair(0, 2048, function (stack, name, responses, status) {
      ...
      if (status == 200 || dev.keyPairAttempts > 19) {
        ...
        if (responses.Body != null && responses.Body.ReturnValue != null && responses.Body.ReturnValueStr != null) {
          dev.consoleMsg(
            'Failed to generate a key pair (' +
              status +
              ', ' +
              responses.Body.ReturnValue +
              ', "' +
              responses.Body.ReturnValueStr +
              '"), attempt ' +
              dev.keyPairAttempts +
        ...
        var f = function doManage() {
          generateKeyPairWithRetry(doManage.dev, doManage.func);
        };
        f.dev = dev;
        f.func = func;
        setTimeout(f, 10000);
  ...
  function attemptTlsSyncEx(stack, name, responses, status) {
    ...
    if (dev.policy.tlsCredentialContext.length > 0) {
      var certInstanceId =
        dev.policy.tlsCredentialContext[0]['ElementInContext']['ReferenceParameters']['SelectorSet']['Selector'][
          'Value'
        ];
      for (var i in dev.policy.certificates) {
        if (dev.policy.certificates[i]['InstanceID'] == certInstanceId) {
          xxTlsCurrentCert = i;
        }
      }
    }
    ...
    if (xxTlsCurrentCert == null) {
      ...
      generateKeyPairWithRetry(dev, function (stack, name, responses, status) {
        ...
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        if (status != 200) {
          dev.consoleMsg('Failed to generate a key pair (' + status + ').');
          removeAmtDevice(dev, 20);
          return;
        }
        ...
        var x = null;
        try {
          x = responses.Body['KeyPair']['ReferenceParameters']['SelectorSet']['Selector']['Value'];
        } catch (ex) {}
        if (x == null) {
          dev.consoleMsg('Unable to get key pair reference.');
          removeAmtDevice(dev, 21);
          return;
        }
        ...
        dev.amtstack.Enum(
          ...
          function (stack, name, responses, status, tag) {
            const dev = stack.dev;
            if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
            if (status != 200) {
              dev.consoleMsg('Failed to get a key pair list (' + status + ').');
              removeAmtDevice(dev, 22);
              return;
            }
            ...
            for (var i in responses) {
              if (responses[i]['InstanceID'] == tag) {
                DERKey = responses[i]['DERKey'];
              }
            }
            ...
            if (domain != null && domain.title != null) {
              serverName = domain.title;
            }
            ...
            const cert = obj.amtcert_createCertificate(
              certattributes,
              xxCaPrivateKey,
              DERKey,
              issuerattributes,
            ...
            if (cert == null) {
              dev.consoleMsg('Failed to sign the TLS certificate.');
              removeAmtDevice(dev, 23);
              return;
            }
            ...
            md.update(
              obj.parent.certificateOperations.forge.asn1
                .toDer(obj.parent.certificateOperations.forge.pki.certificateToAsn1(cert))
                .getBytes()
            );
            dev.aquired.xhash = md.digest().toHex();

            dev.amtstack.AMT_PublicKeyManagementService_AddCertificate(
              pem.substring(27, pem.length - 25),
              function (stack, name, responses, status) {
                const dev = stack.dev;
                if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
                if (status != 200) {
                  dev.consoleMsg('Failed to add TLS certificate (' + status + ').');
                  removeAmtDevice(dev, 24);
                  return;
                }
                var certInstanceId = null;
                try {
                  certInstanceId =
                    responses.Body['CreatedCertificate']['ReferenceParameters']['SelectorSet']['Selector']['Value'];
                } catch (ex) {}
                if (certInstanceId == null) {
                  dev.consoleMsg('Failed to get TLS certificate identifier.');
                  removeAmtDevice(dev, 25);
                  return;
                }
                ...
                if (dev.policy.tlsCredentialContext.length > 0) {
                  ...
                  newTLSCredentialContext['ElementInContext']['ReferenceParameters']['SelectorSet']['Selector'][
                    'Value'
                  ] = certInstanceId;
                  ...
                  dev.amtstack.Create(
                    ...
                    {
                      ElementInContext:
                        '<a:Address>/wsman</a:Address><a:ReferenceParameters><w:ResourceURI>' +
                        dev.amtstack.CompleteName('AMT_PublicKeyCertificate') +
                      ...
                      ElementProvidingContext:
                        '<a:Address>/wsman</a:Address><a:ReferenceParameters><w:ResourceURI>' +
                        dev.amtstack.CompleteName('AMT_TLSProtocolEndpointCollection') +
                        '</w:ResourceURI><w:SelectorSet><w:Selector Name="ElementName">TLSProtocolEndpointInstances Collection</w:Selector></w:SelectorSet></a:ReferenceParameters>'
                    },
                    amtSwitchToTls
                  );
                }
                ...
                var localNdx =
                    dev.policy != null &&
                    dev.policy.tlsSettings != null &&
                    dev.policy.tlsSettings[0] != null &&
                    dev.policy.tlsSettings[0]['InstanceID'] == 'Intel(r) AMT LMS TLS Settings'
                      ? 0
                      : 1,
                  remoteNdx = 1 - localNdx;
  ...
  function amtSwitchToTls(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed setup TLS (' + status + ').');
      removeAmtDevice(dev, 26);
      return;
    }
    ...
    if (--dev.setTlsSecurityPendingCalls == 0) {
      ...
      dev.amtstack.AMT_SetupAndConfigurationService_CommitChanges(null, function (stack, name, responses, status) {
        ...
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        if (status != 200) {
          dev.consoleMsg('Failed perform commit (' + status + ').');
          removeAmtDevice(dev, 27);
          return;
        }
  ...
  function amtcert_linkCertPrivateKey(certs, keys) {
    for (var i in certs) {
      ...
      try {
        ...
        var publicKeyPEM = forge.pki
          .publicKeyToPem(forge.pki.certificateFromAsn1(forge.asn1.fromDer(cert.X509Certificate)).publicKey)
          .substring(28 + 32)
          .replace(/(\r\n|\n|\r)/gm, '');
  ...
  function attemptWifiSync(dev, func) {
    ...
    if (dev.connType != 2) {
      func(dev);
      return;
    } // Only configure wireless over a CIRA-LMS link
    ...
    if (parent.config.domains[dev.domainid].amtmanager['802.1x'] != null) {
      objQuery.push('*AMT_8021XProfile');
    }
    dev.amtstack.BatchEnum(null, objQuery, function (stack, name, responses, status) {
      ...
      const domain = parent.config.domains[dev.domainid];
      if (
        responses['AMT_PublicKeyCertificate'] == null ||
        responses['AMT_PublicKeyCertificate'].status != 200 ||
        responses['AMT_PublicPrivateKeyPair'] == null ||
        responses['AMT_PublicPrivateKeyPair'].status != 200
      ) {
        devTaskCompleted(dev);
        return;
      } // We can't get the certificate list, fail and carry on.

      // See if we need to perform wired or wireless 802.1x configuration
      var wiredConfig =
        parent.config.domains[dev.domainid].amtmanager['802.1x'] != null &&
        responses['AMT_8021XProfile'] != null &&
        responses['AMT_8021XProfile'].status == 200;
      const wirelessConfig =
        responses['CIM_WiFiEndpointSettings'] != null &&
        responses['CIM_WiFiEndpointSettings'].status == 200 &&
        responses['AMT_WiFiPortConfigurationService'] != null &&
        responses['AMT_WiFiPortConfigurationService'].status == 200 &&
        responses['CIM_WiFiPort'] != null &&
      ...
      if (!wiredConfig && !wirelessConfig) {
        devTaskCompleted(dev);
        return;
      } // We can't get wired or wireless settings, ignore and carry on.
      ...
      for (var i in xxCertPrivateKeys) {
        if (!xxCertPrivateKeys[i].XCert) {
          dev.amtstack.Delete(
            'AMT_PublicPrivateKeyPair',
            { InstanceID: xxCertPrivateKeys[i]['InstanceID'] },
            function (stack, name, response, status) {
      ...
      if (wiredConfig) {
        ...
        if (srvNetAuthProfile === false && devNetAuthProfile != null) {
          ...
          if (devNetAuthProfile.AuthenticationProtocol != srvNetAuthProfile.authenticationprotocol) {
            wiredMatch = 2;
          }
          if (devNetAuthProfile.ServerCertificateName != srvNetAuthProfile.servercertificatename) {
            wiredMatch = 2;
          }
          if (devNetAuthProfile.ServerCertificateNameComparison != srvNetAuthProfile.servercertificatenamecomparison) {
            wiredMatch = 2;
          }
          if (devNetAuthProfile.ActiveInS0 != srvNetAuthProfile.availableins0) {
            wiredMatch = 2;
          }
          if (typeof srvNetAuthProfile.satellitecredentials != 'string') {
            // Credentials for this profile are in the config file
            if (devNetAuthProfile.RoamingIdentity != srvNetAuthProfile.roamingidentity) {
              wiredMatch = 2;
            }
            if (devNetAuthProfile.Username != srvNetAuthProfile.username) {
              wiredMatch = 2;
            }
            if (devNetAuthProfile.Domain != srvNetAuthProfile.domain) {
              wiredMatch = 2;
            }
          }
          // If the existing 802.1x profile has a certificate, remember the client certificate instance id for later checking
          if (devNetAuthProfile.ClientCertificate) {
            netAuthClientCertInstanceId =
              devNetAuthProfile.ClientCertificate.ReferenceParameters.SelectorSet.Selector.Value;
          }
        }
        if (wiredMatch == 2) {
          newNetAuthProfileRequested = true;
        }
      }
      ...
      if (wirelessConfig) {
        ...
        if (parent.config.domains[dev.domainid].amtmanager.wifiprofiles != null) {
          ...
          for (var i in sevProfiles) {
            ...
            for (var j in devProfiles) {
              ...
              if (
                ...
                if ([5, 7, 32768, 32769].indexOf(sevProfile.authentication) >= 0) {
                  ...
                  var netAuthProfile = null,
                    netAuthMatch = false;
                  for (var k in netAuthProfiles) {
                    if (netAuthProfiles[k].ElementName == devProfile.ElementName) {
                      netAuthProfile = netAuthProfiles[k];
                    }
                  }
                  if (netAuthProfile != null) {
                    netAuthMatch = true;
                    if (srvNetAuthProfile.authenticationprotocol != netAuthProfile['AuthenticationProtocol']) {
                      netAuthMatch = false;
                    }
                    if (srvNetAuthProfile.roamingidentity != netAuthProfile['RoamingIdentity']) {
                      netAuthMatch = false;
                    }
                    if (srvNetAuthProfile.servercertificatename != netAuthProfile['ServerCertificateName']) {
                      netAuthMatch = false;
                    }
                    if (
                      srvNetAuthProfile.servercertificatenamecomparison !=
                      netAuthProfile['ServerCertificateNameComparison']
                    ) {
                      netAuthMatch = false;
                    }
                    if (typeof srvNetAuthProfile.satellitecredentials != 'string') {
                      // Credentials for this profile are in the config file
                      if (srvNetAuthProfile.username != netAuthProfile['Username']) {
                        netAuthMatch = false;
                      }
                      if (srvNetAuthProfile.domain != netAuthProfile['Domain']) {
                        netAuthMatch = false;
                      }
                    }
                  }
            ...
            if (wirelessMatch == false) {
              profilesToAdd.push(sevProfile);
            } // Add non-matching profile
            if (wirelessMatch == false || [5, 7, 32768, 32769].indexOf(sevProfile.authentication) >= 0) {
              profilesToAdd2.push(sevProfile);
            } // Add non-matching profile or 802.1x profile
          }
          for (var j in devProfiles) {
            var devProfile = devProfiles[j];
            if (devProfile.InstanceID != null) {
              if (devProfile.match !== true) {
                profilesToRemove.push(devProfile);
              } // Missing profile to remove
              if (devProfile.match !== true || [5, 7, 32768, 32769].indexOf(devProfile.AuthenticationMethod) >= 0) {
                profilesToRemove2.push(devProfile);
              } // Missing profile to remove or 802.1x profile
            }
          }
          ...
          var prioritiesInUse = [];
          for (var j in devProfiles) {
            if (devProfiles[j].match == true) {
              prioritiesInUse.push(devProfiles[j].Priority);
            }
          }
          ...
          if (dev.netAuthCredentials == null) {
            for (var i in profilesToAdd) {
              if ([5, 7, 32768, 32769].indexOf(profilesToAdd[i].authentication) >= 0) {
                newNetAuthProfileRequested = true;
              }
            }
          }
          ...
          if (profilesToAdd.length > 0 || profilesToRemove.length > 0) {
            dev.consoleMsg(
              'Changing WIFI profiles, adding ' + profilesToAdd.length + ', removing ' + profilesToRemove.length + '.'
            );
          }
          ...
          for (var i in profilesToRemove) {
            dev.amtstack.Delete(
              'CIM_WiFiEndpointSettings',
              { InstanceID: 'Intel(r) AMT:WiFi Endpoint Settings ' + profilesToRemove[i].ElementName },
              function (stack, name, responses, status) {},
              0,
              1
            );
      ...
      if (netAuthClientCertInstanceId != null) {
        ...
        if (netAuthClientCert) {
          var cert = null;
          try {
            cert = obj.parent.certificateOperations.forge.pki.certificateFromAsn1(
              obj.parent.certificateOperations.forge.asn1.fromDer(
                obj.parent.certificateOperations.forge.util.decode64(netAuthClientCert.X509Certificate)
              )
            );
          } catch (ex) {}
          if (cert != null) {
            ...
            if (Date.now() > certMidPoint) {
              newNetAuthProfileRequested = true;
            } // Past mid-point or expired, request a new 802.1x certificate & profile
          }
        }
      }

      // Figure out if there are no changes to 802.1x wired configuration
      if (wiredMatch == 0 && newNetAuthProfileRequested == false) {
        wiredConfig = false;
      }
      ...
      if (newNetAuthProfileRequested && typeof srvNetAuthProfile.satellitecredentials == 'string') {
        ...
        dev.consoleMsg(
          'Requesting 802.1x credentials for ' +
            netAuthStrings[srvNetAuthProfile.authenticationprotocol] +
            ' from MeshCentral Satellite...'
        );
        dev.netAuthSatReqId = Buffer.from(parent.crypto.randomBytes(16), 'binary').toString('base64'); // Generate a crypto-secure request id.
        dev.netAuthSatReqData = {
          domain: domain,
          wiredConfig: wiredConfig,
          wirelessConfig: wirelessConfig,
          devNetAuthProfile: devNetAuthProfile,
          srvNetAuthProfile: srvNetAuthProfile,
          profilesToAdd: profilesToAdd,
          prioritiesInUse: prioritiesInUse,
          responses: responses,
          xxCertificates: xxCertificates,
          xxCertPrivateKeys: xxCertPrivateKeys
        };
        const request = {
          action: 'satellite',
          ...
          nodeid: dev.nodeid,
          icon: dev.icon,
          domain: dev.nodeid.split('/')[1],
          nolog: 1,
          reqid: dev.netAuthSatReqId,
          authProtocol: srvNetAuthProfile.authenticationprotocol,
          devname: dev.name,
          osname: dev.rname,
          ver: dev.intelamt.ver
        };
        if (netAuthClientCert != null) {
          request.cert = netAuthClientCert.X509Certificate;
          request.certid = netAuthClientCertInstanceId;
        }
        ...
        const netAuthTimeoutFunc = function netAuthTimeout() {
          ...
          if (dev.netAuthSatReqId != null) {
            ...
            netAuthTimeout.dev.consoleMsg(
              'MeshCentral Satellite did not respond in time, 802.1x profile will not be set.'
            );
  ...
  function perform8021xRootCertCheck(dev) {
    ...
    if (dev.netAuthCredentials.rootcert) {
      ...
      for (var i in dev.netAuthSatReqData.xxCertificates) {
        if (
          dev.netAuthSatReqData.xxCertificates[i].X509Certificate == dev.netAuthCredentials.rootcert &&
          dev.netAuthSatReqData.xxCertificates[i].TrustedRootCertficate
        ) {
      ...
      if (matchingRootCertId == null) {
        ...
        const f = function perform8021xRootCertCheckResponse(stack, name, response, status) {
          if (status != 200 || response.Body['ReturnValue'] != 0) {
            ...
            perform8021xRootCertCheckResponse.dev.netAuthSatReqData.rootCertInstanceId =
              response.Body.CreatedCertificate.ReferenceParameters.SelectorSet.Selector.Value;
  ...
  function perform8021xClientCertCheck(dev) {
    ...
    if (dev.netAuthCredentials.certificate) {
      ...
      delete dev.netAuthSatReqData;
      if (dev.netAuthCredentials.certid) {
        netAuthSatReqData.certInstanceId = dev.netAuthCredentials.certid;
      } // If we are reusing an existing certificate, set that now.
  ...
  function attempt8021xSyncEx(dev, devNetAuthData) {
    ...
    if (wiredConfig) {
      ...
      if (netAuthProfile['Enabled']) {
        ...
        if (srvNetAuthProfile.roamingidentity && srvNetAuthProfile.roamingidentity != '') {
          netAuthProfile['RoamingIdentity'] = srvNetAuthProfile.roamingidentity;
        } else {
          delete netAuthProfile['RoamingIdentity'];
        }
        ...
        if (srvNetAuthProfile.username && srvNetAuthProfile.username != '') {
          netAuthProfile['Username'] = srvNetAuthProfile.username;
        } else {
          delete netAuthProfile['Username'];
        }
        if (srvNetAuthProfile.password && srvNetAuthProfile.password != '') {
          netAuthProfile['Password'] = srvNetAuthProfile.password;
        } else {
          delete netAuthProfile['Password'];
        }
        if (srvNetAuthProfile.domain && srvNetAuthProfile.domain != '') {
          netAuthProfile['Domain'] = srvNetAuthProfile.domain;
        } else {
          delete netAuthProfile['Domain'];
        }
        ...
        if (devNetAuthData.certInstanceId) {
          netAuthProfile['ClientCertificate'] =
            '<a:Address>/wsman</a:Address><a:ReferenceParameters><w:ResourceURI>' +
            dev.amtstack.CompleteName('AMT_PublicKeyCertificate') +
            '</w:ResourceURI><w:SelectorSet><w:Selector Name="InstanceID">' +
            devNetAuthData.certInstanceId +
            '</w:Selector></w:SelectorSet></a:ReferenceParameters>';
        } else {
        ...
        if (devNetAuthData.rootCertInstanceId) {
          netAuthProfile['ServerCertificateIssuer'] =
            '<a:Address>/wsman</a:Address><a:ReferenceParameters><w:ResourceURI>' +
            dev.amtstack.CompleteName('AMT_PublicKeyCertificate') +
            '</w:ResourceURI><w:SelectorSet><w:Selector Name="InstanceID">' +
            devNetAuthData.rootCertInstanceId +
            '</w:Selector></w:SelectorSet></a:ReferenceParameters>';
        } else {
          delete netAuthProfile['ServerCertificateIssuer'];
        }

        netAuthProfile['PxeTimeout'] =
          typeof srvNetAuthProfile.pxetimeoutinseconds == 'number' ? srvNetAuthProfile.pxetimeoutinseconds : 120;

        // If we have a MeshCentral Satellite profile, use that
        if (dev.netAuthCredentials != null) {
          const srvNetAuthProfile2 = dev.netAuthCredentials;
          if (srvNetAuthProfile2.username && srvNetAuthProfile2.username != '') {
            netAuthProfile['Username'] = srvNetAuthProfile2.username;
          }
          if (srvNetAuthProfile2.password && srvNetAuthProfile2.password != '') {
            netAuthProfile['Password'] = srvNetAuthProfile2.password;
          }
          if (srvNetAuthProfile2.domain && srvNetAuthProfile2.domain != '') {
            netAuthProfile['Domain'] = srvNetAuthProfile2.domain;
          }
        }
      }
      ...
      dev.amtstack.Put('AMT_8021XProfile', netAuthProfile, function (stack, name, responses, status) {
        ...
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        if (status == 200) {
          dev.consoleMsg('802.1x wired profile set.');
        } else {
          dev.consoleMsg('Unable to set 802.1x wired profile.');
        }
  ...
  function attemptWifiSyncEx(dev, devNetAuthData) {
    ...
    if (wirelessConfig) {
      ...
      var nextPriority = 1;
      for (var i in profilesToAdd) {
        while (prioritiesInUse.indexOf(nextPriority) >= 0) {
          nextPriority++;
        } // Figure out the next available priority slot.
        ...
        if ([4, 6].indexOf(profileToAdd.authentication) >= 0) {
          wifiepsettinginput['PSKPassPhrase'] = profileToAdd.password;
        }
        if ([5, 7, 32768, 32769].indexOf(profileToAdd.authentication) >= 0) {
          ...
          if (domain.amtmanager['802.1x'].roamingidentity) {
            netAuthProfile['RoamingIdentity'] = domain.amtmanager['802.1x'].roamingidentity;
          }
          if (domain.amtmanager['802.1x'].servercertificatename) {
            netAuthProfile['ServerCertificateName'] = domain.amtmanager['802.1x'].servercertificatename;
            netAuthProfile['ServerCertificateNameComparison'] = profileToAdd['802.1x'].servercertificatenamecomparison;
          }
          if (domain.amtmanager['802.1x'].username) {
            netAuthProfile['Username'] = domain.amtmanager['802.1x'].username;
          }
          if (domain.amtmanager['802.1x'].password) {
            netAuthProfile['Password'] = domain.amtmanager['802.1x'].password;
          }
          if (domain.amtmanager['802.1x'].domain) {
            netAuthProfile['Domain'] = domain.amtmanager['802.1x'].domain;
          }
          if (domain.amtmanager['802.1x'].authenticationprotocol > 3) {
            domain.amtmanager['ProtectedAccessCredential'] = profileToAdd['802.1x'].protectedaccesscredentialhex;
            netAuthProfile['PACPassword'] = profileToAdd['802.1x'].pacpassword;
          }
        ...
        taskCounter++;
        dev.amtstack.AMT_WiFiPortConfigurationService_AddWiFiSettings(
          wifiep,
          wifiepsettinginput,
          netAuthProfile,
          netAuthSettingsClientCert,
          ...
          function (stack, name, response, status) {
            if (status != 200) {
              dev.consoleMsg('Unable to set WIFI profile.');
            }
            if (--taskCounter == 0) {
              attemptWifiSyncEx2(dev, devNetAuthData);
            } // All done, complete WIFI configuration
          }
        );
      }
    }
  ...
  function attemptWifiSyncEx2(dev, devNetAuthData) {
    ...
    if (wirelessConfig) {
      ...
      if (
        responses['AMT_WiFiPortConfigurationService'] != null &&
        responses['AMT_WiFiPortConfigurationService'].response != null &&
        responses['AMT_WiFiPortConfigurationService'].response['localProfileSynchronizationEnabled'] == 0
      ) {
        responses['AMT_WiFiPortConfigurationService'].response['localProfileSynchronizationEnabled'] = 1;
        dev.amtstack.Put(
          'AMT_WiFiPortConfigurationService',
          responses['AMT_WiFiPortConfigurationService'].response,
          function (stack, name, response, status) {
            if (status != 200) {
              dev.consoleMsg('Unable to enable local WIFI profile sync.');
            } else {
              dev.consoleMsg('Enabled local WIFI profile sync.');
            }
          }
        );
      }
  ...
  function attempt8021xKeyGeneration(dev) {
    ...
    dev.amtstack.AMT_PublicKeyManagementService_GenerateKeyPair(0, 2048, function (stack, name, response, status) {
      if (status != 200 || response.Body['ReturnValue'] != 0) {
        ...
        dev.amtstack.Enum(
          'AMT_PublicPrivateKeyPair',
          function (stack, name, xresponse, status, keyInstanceId) {
            if (status != 200) {
              ...
              if (DERKey == null) {
                dev.consoleMsg('Failed to match the generated RSA key pair.');
                return;
              }
              ...
              parent.DispatchEvent([domain.amtmanager['802.1x'].satellitecredentials], obj, {
                ...
                nolog: 1,
                reqid: dev.netAuthSatReqId,
                authProtocol: domain.amtmanager['802.1x'].authenticationprotocol,
                devname: dev.name,
                osname: dev.rname,
                DERKey: DERKey,
  ...
  function attempt8021xCRSRequest(dev, event) {
    ...
    var keyPair =
      ...
      '</w:Selector></w:SelectorSet></a:ReferenceParameters>'; // keyPair EPR Reference
    var signingAlgorithm = 1; // 0 = SHA1-RSA, 1 = SHA256-RSA
    var nullSignedCertificateRequest = event.response.csr; // DEREncodedRequest
    dev.amtstack.AMT_PublicKeyManagementService_GeneratePKCS10RequestEx(
      keyPair,
      signingAlgorithm,
      ...
      function (stack, name, response, status) {
        if (status != 200 || response.Body['ReturnValue'] != 0) {
          ...
          parent.DispatchEvent([domain.amtmanager['802.1x'].satellitecredentials], obj, {
            ...
            devname: dev.name,
            osname: dev.rname,
            signedcsr: response.Body['SignedCertificateRequest'],
            ver: dev.intelamt.ver
          });
        }
      }
  ...
  function attemptRootCertSync(dev, func, forced) {
    ...
    if (forced !== true) {
      if (dev.connType != 2 || dev.policy.ciraPolicy != 2) {
        func(dev);
        return;
      }
    } // Server root certificate does not need to be present if CIRA is not needed and "forced" is false
    if (parent.mpsserver.server == null) {
      func(dev);
      return;
    } // Root cert not needed if MPS is not active.

    // Find the current TLS certificate & MeshCentral root certificate
    var xxMeshCentralRoot = null;
    if (dev.policy.tlsCredentialContext.length > 0) {
      for (var i in dev.policy.certificates) {
        if (dev.policy.certificates[i]['X509Certificate'] == obj.rootCertBase64) {
          xxMeshCentralRoot = i;
        }
      }
    }
    ...
    if (xxMeshCentralRoot == null) {
      dev.taskCount = 1;
      dev.taskCompleted = func;
      dev.amtstack.AMT_PublicKeyManagementService_AddTrustedRootCertificate(
        obj.rootCertBase64,
        function (stack, name, responses, status) {
          const dev = stack.dev;
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to add server root certificate (' + status + ').');
            removeAmtDevice(dev, 28);
            return;
          }
  ...
  function attemptCiraSync(dev, func) {
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (dev.connType != 2 || (dev.policy.ciraPolicy != 1 && dev.policy.ciraPolicy != 2)) {
      func(dev);
      return;
    } // Only setup CIRA when LMS connection is used and a CIRA policy is enabled.
    ...
    if (dev.aquired.majorver != null && dev.aquired.majorver > 11) {
      requests.push('*IPS_HTTPProxyService', 'IPS_HTTPProxyAccessPoint');
    }
  ...
  function attemptCiraSyncResponse(stack, name, responses, status) {
    ...
    if (dev.aquired.majorver != null && dev.aquired.majorver > 11 && status == 400) {
      ...
      status = 200;
      if (responses['IPS_HTTPProxyAccessPoint'].status == 400) {
        delete responses['IPS_HTTPProxyAccessPoint'];
      }
      if (responses['IPS_HTTPProxyService'].status == 400) {
        delete responses['IPS_HTTPProxyService'];
      }
      for (var i in responses) {
    ...
    if (
      responses['AMT_UserInitiatedConnectionService'] == null ||
      responses['AMT_UserInitiatedConnectionService'].response == null
    ) {
    ...
    dev.cira.xxEnvironementDetection['DetectionStrings'] = MakeToArray(
      dev.cira.xxEnvironementDetection['DetectionStrings']
    );
    ...
    if (dev.cira.xxUserInitiatedCira['EnabledState'] != 32771) {
      // 32768: "Disabled", 32769: "BIOS enabled", 32770: "OS enable", 32771: "BIOS & OS enabled"
      dev.amtstack.AMT_UserInitiatedConnectionService_RequestStateChange(
        32771,
        null,
        function (stack, name, responses, status) {}
      ); // This is not a critical call.
    }
    ...
    for (var i in responses['AMT_RemoteAccessPolicyAppliesToMPS'].responses) {
      ...
      var server = Clone(
        getItem(
          dev.cira.xxCiraServers,
          'Name',
          getItem(policy['ManagedElement']['ReferenceParameters']['SelectorSet']['Selector'], '@Name', 'Name')['Value']
        )
      );
      server.MpsType = policy['MpsType']; // MpsType was added in Intel AMT 11.6
      var ptype = getItem(
        policy['PolicySet']['ReferenceParameters']['SelectorSet']['Selector'],
        '@Name',
        'PolicyRuleName'
      )['Value'].split(' ')[0];
    ...
    if (dev.policy.ciraPolicy == 2 && parent.mpsserver.server != null) {
      ...
      if (
        serverNameSplit.length == 4 &&
        parseInt(serverNameSplit[0]) == serverNameSplit[0] &&
        parseInt(serverNameSplit[1]) == serverNameSplit[1] &&
        parseInt(serverNameSplit[2]) == serverNameSplit[2] &&
        parseInt(serverNameSplit[3]) == serverNameSplit[3]
      ) {
        dev.cira.mpsAddressFormat = 3;
      }
      ...
      if (dev.cira.xxCiraServers != null && dev.cira.xxCiraServers.length > 0) {
        for (var i = 0; i < dev.cira.xxCiraServers.length; i++) {
          ...
          if (
            mpsServer.AccessInfo == dev.cira.mpsName &&
            mpsServer.Port == dev.cira.mpsPort &&
            mpsServer.InfoFormat == dev.cira.mpsAddressFormat
          ) {
            dev.cira.mpsPresent = mpsServer['Name'];
          }
        }
      }
    ...
    if (dev.cira.xxPolicies != null) {
      if (dev.cira.xxPolicies['User'] != null && dev.cira.xxPolicies['User'].length > 0) {
        dev.consoleMsg('Removing CIRA user trigger.');
        dev.amtstack.Delete(
          'AMT_RemoteAccessPolicyRule',
          { PolicyRuleName: 'User Initiated' },
          function (stack, name, responses, status) {}
        );
      }
      if (dev.cira.xxPolicies['Alert'] != null && dev.cira.xxPolicies['Alert'].length > 0) {
        dev.consoleMsg('Removing CIRA alert trigger.');
        dev.amtstack.Delete(
          'AMT_RemoteAccessPolicyRule',
          { PolicyRuleName: 'Alert' },
          function (stack, name, responses, status) {}
        );
      }
      if (
        dev.cira.xxPolicies['Periodic'] != null &&
        dev.cira.xxPolicies['Periodic'].length > 0 &&
        dev.cira.mpsPolicy == false
      ) {
        dev.consoleMsg('Removing CIRA periodic trigger.');
        dev.amtstack.Delete(
          'AMT_RemoteAccessPolicyRule',
  ...
  function addMpsServer(dev) {
    ...
    if (dev.cira.mpsPresent == null) {
      dev.amtstack.AMT_RemoteAccessService_AddMpServer(
        dev.cira.mpsName,
        dev.cira.mpsAddressFormat,
        dev.cira.mpsPort,
        2,
        null,
        dev.cira.meshidx,
        ...
        function (stack, name, response, status) {
          ...
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to create new MPS server (' + status + ').');
            removeAmtDevice(dev, 31);
            return;
          }
          if (
            response.Body.MpServer == null ||
            response.Body.MpServer.ReferenceParameters == null ||
            response.Body.MpServer.ReferenceParameters.SelectorSet == null ||
            response.Body.MpServer.ReferenceParameters.SelectorSet.Selector == null
          ) {
            dev.consoleMsg('Create new MPS server invalid response.');
            removeAmtDevice(dev, 32);
            return;
          }
          dev.cira.mpsPresent = getItem(
            response.Body.MpServer.ReferenceParameters.SelectorSet.Selector,
            '@Name',
            'Name'
          ).Value;
  ...
  function addMpsPolicy(dev) {
    if (dev.cira.mpsPolicy == false) {
      var cilaSupport =
        dev.aquired.majorver != null &&
        dev.aquired.minorver != null &&
        (dev.aquired.majorver > 11 || (dev.aquired.majorver == 11 && dev.aquired.minorver >= 6));
      ...
      var ciraServers = [],
        cilaServers = [];
      if (server1) {
        ciraServers.push(server1);
        if (server2) {
          ciraServers.push(server2);
        }
      }

      // Go ahead and create the new CIRA/CILA policy.
      dev.amtstack.AMT_RemoteAccessService_AddRemoteAccessPolicyRule(
        trigger,
        0,
        extendedData,
        ciraServers,
        ...
        function (stack, name, responses, status) {
          ...
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to create new MPS policy (' + status + ').');
            removeAmtDevice(dev, 33);
            return;
          }
  ...
  function checkEnvironmentDetection(dev) {
    ...
    if (currentEnvDetect == null) {
      currentEnvDetect = [];
    }

    if (dev.policy.ciraPolicy == 2 && parent.mpsserver.server != null) {
      // ciraPolicy: 0 = Do Nothing, 1 = Clear, 2 = Set
      ...
      if (newEnvDetect == null) {
        ...
        if (currentEnvDetect.length == 0) {
          editEnvironmentDetectionTmp = [Buffer.from(parent.crypto.randomBytes(6), 'binary').toString('hex')];
          changes = true;
        }
      } else {
        ...
        if (currentEnvDetect.length != newEnvDetect.length) {
          ...
          for (var i in currentEnvDetect) {
            if (newEnvDetect.indexOf(currentEnvDetect[i]) == -1) {
              mismatch = true;
            }
          }
          for (var i in newEnvDetect) {
            if (currentEnvDetect.indexOf(newEnvDetect[i]) == -1) {
              mismatch = true;
            }
          }
        }
        ...
        if (mismatch == true) {
          editEnvironmentDetectionTmp = newEnvDetect;
          changes = true;
        }
      }
    ...
    if (changes == true) {
      ...
      dev.amtstack.Put(
        ...
        function (stack, name, responses, status) {
          ...
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to set environement detection (' + status + ').');
            removeAmtDevice(dev, 34);
            return;
          }
          if (dev.cira.envclear) {
            dev.consoleMsg('Environment detection cleared.');
          } else {
  ...
  function attemptSettingsSync(dev, func) {
    ...
    if (dev.aquired.majorver != null && dev.aquired.majorver > 5) {
      query.push('*CIM_KVMRedirectionSAP', '*IPS_OptInService');
    }
  ...
  function attemptSettingsSyncResponse(stack, name, responses, status) {
    ...
    if (responses['CIM_KVMRedirectionSAP'] == null || responses['CIM_KVMRedirectionSAP'].status == 400) {
      responses['CIM_KVMRedirectionSAP'] = null;
    }

    // Set user consent requirement to match device group user consent
    const mesh = parent.webserver.meshes[dev.meshid];
    if (mesh == null) {
      removeAmtDevice(dev, 35);
      return;
    }
    ...
    if (
      responses['IPS_OptInService'] != null &&
      responses['IPS_OptInService'].response['OptInRequired'] != userConsentRequirement
    ) {
      ...
      dev.amtstack.Put(
        'IPS_OptInService',
        responses['IPS_OptInService'].response,
        function (stack, name, responses, status) {
          const dev = stack.dev;
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status == 200) {
    ...
    // Enable SOL & IDER
    if (
      responses['AMT_RedirectionService'].response['EnabledState'] != 32771 ||
      responses['AMT_RedirectionService'].response['ListenerEnabled'] == false
    ) {
      dev.redirObj = responses['AMT_RedirectionService'].response;
      dev.redirObj['ListenerEnabled'] = true;
      dev.redirObj['EnabledState'] = 32771;
      dev.taskCount++;
      dev.amtstack.AMT_RedirectionService_RequestStateChange(32771, function (stack, name, response, status) {
        const dev = stack.dev;
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        dev.amtstack.Put(
          ...
          function (stack, name, response, status) {
            ...
            if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
            if (status == 200) {
              dev.consoleMsg('Enabled redirection features.');
            }
            devTaskCompleted(dev);
          },
          0,
    ...
    if (dev.aquired.majorver != null && dev.aquired.majorver > 5 && responses['CIM_KVMRedirectionSAP'] != null) {
      const kvm =
        responses['CIM_KVMRedirectionSAP'].response['EnabledState'] == 2 ||
        responses['CIM_KVMRedirectionSAP'].response['EnabledState'] == 6;
      if (kvm == false) {
        // Enable KVM
        dev.taskCount++;
        dev.amtstack.CIM_KVMRedirectionSAP_RequestStateChange(2, 0, function (stack, name, response, status) {
          const dev = stack.dev;
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status == 200) {
            dev.consoleMsg('Enabled KVM.');
          }
          devTaskCompleted(dev);
        });
      }
      ...
      if (
        ...
        dev.amtstack.Put(
          ...
          function () {
            ...
            if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
            if (status == 200) {
              dev.consoleMsg('Changed device name: ' + dev.xname);
            }
            delete dev.xname;
            devTaskCompleted(dev);
          },
  ...
  function attemptCleanCertsSync(dev, func) {
    ...
    dev.amtstack.BatchEnum(
      null,
      ['AMT_PublicKeyCertificate', 'AMT_PublicPrivateKeyPair'],
      function (stack, name, responses, status) {
        const dev = stack.dev;
        if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
        const domain = parent.config.domains[dev.domainid];
        ...
        if (
          responses['AMT_PublicKeyCertificate'].responses.length == 0 ||
          responses['AMT_PublicPrivateKeyPair'].responses.length == 0
        ) {
          func(dev);
          return;
        } // Empty certificate list, fail and carry on.
        ...
        for (var i in xxCertPrivateKeys) {
          if (!xxCertPrivateKeys[i].XCert) {
            ...
            dev.amtstack.Delete(
              ...
              function (stack, name, response, status) {
                //if (status == 200) { dev.consoleMsg("Removed unassigned private key pair."); }
                if (--dev.certDeleteTasks == 0) {
                  delete dev.certDeleteTasks;
                  func(dev);
                }
              }
            );
        ...
        for (var i in xxCertificates) {
          if (xxCertificates[i].TrustedRootCertficate == false) {
            ...
            if (xxCertificates[i].XPrivateKey) {
              privateKeyInstanceId = { InstanceID: xxCertificates[i].XPrivateKey['InstanceID'] };
            }
            dev.certDeleteTasks++;
            dev.amtstack.Delete(
              'AMT_PublicKeyCertificate',
              { InstanceID: xxCertificates[i]['InstanceID'] },
              function (stack, name, response, status, tag) {
  ...
  function attemptFetchHardwareInventory(dev, func) {
    ...
    if (mesh.mtype == 1) {
      // If this is a Intel AMT only device group, pull the hardware inventory and network information for this device
      ...
      dev.amtstack.BatchEnum(
        ...
        attemptFetchHardwareInventoryResponse
      );
  ...
  function attemptFetchNetworkResponse(stack, name, responses, status) {
    ...
    if (responses['AMT_EthernetPortSettings'] == null || responses['AMT_EthernetPortSettings'].responses == null) {
      devTaskCompleted(dev);
      return;
    }
    ...
    for (var i in responses['AMT_EthernetPortSettings'].responses) {
      ...
      if (netif.MACAddress != null && netif.MACAddress != '00-00-00-00-00-00') {
        if (netif.WLANLinkProtectionLevel != null) {
          wireless = netif;
        } else {
          wired = netif;
        }
    ...
    parent.DispatchEvent(parent.webserver.CreateMeshDispatchTargets(dev.meshid, [dev.nodeid]), obj, {
      action: 'ifchange',
      nodeid: dev.nodeid,
      domain: dev.nodeid.split('/')[1],
      nolog: 1
  ...
  function attemptFetchHardwareInventoryResponse(stack, name, responses, status) {
    ...
    if (hw.PhysicalMemory != null && hw.PhysicalMemory.length > 0) {
      ...
      for (var i in hw.PhysicalMemory) {
        var m2 = {},
          m = hw.PhysicalMemory[i];
        m2.BankLabel = m.BankLabel;
        m2.Capacity = m.Capacity;
        if (typeof m.PartNumber == 'string') {
          m2.PartNumber = m.PartNumber.trim();
        }
        if (typeof m.PartNumber == 'number') {
          m2.PartNumber = m.PartNumber;
        }
        if (typeof m.SerialNumber == 'string') {
          m2.SerialNumber = m.SerialNumber.trim();
        }
        if (typeof m.SerialNumber == 'number') {
          m2.SerialNumber = m.SerialNumber;
        }
        if (typeof m.Manufacturer == 'string') {
          m2.Manufacturer = m.Manufacturer.trim();
        }
        if (typeof m.Manufacturer == 'number') {
          m2.Manufacturer = m.Manufacturer;
        }
        memory.push(m2);
      }
    ...
    if (hw.Bios != null) {
      if (typeof hw.Bios.Manufacturer == 'string') {
        hw2.hardware.identifiers.bios_vendor = hw.Bios.Manufacturer.trim();
      }
      if (typeof hw.Bios.Manufacturer == 'number') {
        hw2.hardware.identifiers.bios_vendor = hw.Bios.Manufacturer;
      }
      hw2.hardware.identifiers.bios_version = hw.Bios.Version;
      if (hw.Bios.ReleaseDate && hw.Bios.ReleaseDate.Datetime) {
        hw2.hardware.identifiers.bios_date = hw.Bios.ReleaseDate.Datetime;
      }
    }
    if (hw.PhysicalPackage != null) {
      if (typeof hw.Card.Model == 'string') {
        hw2.hardware.identifiers.board_name = hw.Card.Model.trim();
      }
      if (typeof hw.Card.Model == 'number') {
        hw2.hardware.identifiers.board_name = hw.Card.Model;
      }
      if (typeof hw.Card.Manufacturer == 'string') {
        hw2.hardware.identifiers.board_vendor = hw.Card.Manufacturer.trim();
      }
      if (typeof hw.Card.Manufacturer == 'number') {
        hw2.hardware.identifiers.board_vendor = hw.Card.Manufacturer;
      }
      if (typeof hw.Card.Version == 'string') {
        hw2.hardware.identifiers.board_version = hw.Card.Version.trim();
      }
      if (typeof hw.Card.Version == 'number') {
        hw2.hardware.identifiers.board_version = hw.Card.Version;
      }
      if (typeof hw.Card.SerialNumber == 'string') {
        hw2.hardware.identifiers.board_serial = hw.Card.SerialNumber.trim();
      }
      if (typeof hw.Card.SerialNumber == 'number') {
        hw2.hardware.identifiers.board_serial = hw.Card.SerialNumber;
      }
    }
  ...
  function activateIntelAmt(dev) {
    ...
    const mesh = parent.webserver.meshes[dev.meshid];
    if (mesh == null) {
      dev.consoleMsg('Unable to find device group (' + dev.meshid + ').');
      removeAmtDevice(dev, 36);
      return false;
    }
    ...
    if (mesh.amt != null) {
      if (typeof mesh.amt.type == 'number') {
        amtPolicy = mesh.amt.type;
      }
      if (typeof mesh.amt.ccm == 'number') {
        ccmPolicy = mesh.amt.ccm;
      }
    }
    if (
      typeof dev.mpsConnection.tag.meiState.OsAdmin != 'object' ||
      typeof dev.mpsConnection.tag.meiState.OsAdmin.user != 'string' ||
      typeof dev.mpsConnection.tag.meiState.OsAdmin.pass != 'string'
    ) {
      amtPolicy = 0;
    }
    if (amtPolicy == 0) {
      removeAmtDevice(dev, 37);
      return false;
    } // Do nothing, we should not have gotten this CIRA-LMS connection.
    if (amtPolicy == 2) {
      activateIntelAmtCcm(dev, mesh.amt.password);
    } // Activate to CCM policy
    if (amtPolicy == 3 || amtPolicy == 4) {
      // Activate to ACM policy
      ...
      if (acminfo == null || acminfo.err != null) {
        ...
        if ((dev.mpsConnection.tag.meiState.Flags & 2) != 0) {
          if (amtPolicy == 3 && ccmPolicy == 1) {
            deactivateIntelAmtCCM(dev);
          } // If we are in ACM policy and CCM is not allowed, deactivate it now.
          else {
            return true;
          } // We are in CCM, keep going
        } else {
          // We are not in CCM, go to CCM now
          if (amtPolicy == 4 || (amtPolicy == 3 && ccmPolicy == 2)) {
            activateIntelAmtCcm(dev, mesh.amt.password);
          } // If we are in full automatic or ACM with CCM allowed, setup CCM.
          else {
            // Unable to find an activation match.
            if (acminfo == null) {
              dev.consoleMsg('No opportunity for ACM activation.');
            } else {
              dev.consoleMsg('No opportunity for ACM activation: ' + acminfo.err);
            }
        ...
        if ((dev.mpsConnection.tag.meiState.Flags & 2) != 0) {
          ...
          var domain = parent.config.domains[dev.domainid];
          if (domain && domain.amtmanager && domain.amtmanager.tlsacmactivation == true) {
  ...
  function activateIntelAmtCcm(dev, password) {
    ...
    var comm = CreateWsmanComm(
      dev.nodeid,
      16992,
      dev.mpsConnection.tag.meiState.OsAdmin.user,
      dev.mpsConnection.tag.meiState.OsAdmin.pass,
      0,
      null,
      dev.mpsConnection
    ); // No TLS
  ...
  function activateIntelAmtCcmEx1(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to get Intel AMT state.');
      removeAmtDevice(dev, 39);
      return;
    }
    if (responses['IPS_HostBasedSetupService'].response['AllowedControlModes'].length != 2) {
      dev.consoleMsg('Client control mode activation not allowed.');
      removeAmtDevice(dev, 40);
      return;
    }
    ...
    var domain = parent.config.domains[dev.domainid];
    if (domain == null) {
      dev.consoleMsg('Invalid domain.');
      removeAmtDevice(dev, 41);
      return;
    }
    if (
      parent.certificateOperations.logAmtActivation(domain, {
        time: new Date(),
        action: 'ccmactivate',
        domain: dev.domainid,
        amtUuid: dev.mpsConnection.tag.meiState.UUID,
        amtRealm: responses['AMT_GeneralSettings'].response['DigestRealm'],
        user: 'admin',
        password: dev.temp.pass,
        ipport: dev.mpsConnection.remoteAddr + ':' + dev.mpsConnection.remotePort,
        nodeid: dev.nodeid,
        meshid: dev.meshid,
        computerName: dev.name
      }) == false
    ) {
      dev.consoleMsg('Unable to log operation.');
      removeAmtDevice(dev, 42);
      return;
    }

    // Perform CCM activation
    dev.amtstack.IPS_HostBasedSetupService_Setup(
      2,
      hex_md5('admin:' + responses['AMT_GeneralSettings'].response['DigestRealm'] + ':' + dev.temp.pass).substring(
        0,
        32
      ),
      null,
  ...
  function activateIntelAmtCcmEx2(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to activate Intel AMT to CCM.');
      removeAmtDevice(dev, 43);
      return;
    }
    ...
    var f = function doManage() {
      if (isAmtDeviceValid(dev)) {
        attemptInitialContact(doManage.dev);
      }
    };
    f.dev = dev;
    setTimeout(f, 10000);
  ...
  function checkAcmActivation(dev) {
    ...
    if (
      ...
      domain.amtacmactivation.certs == null ||
      domain.amtacmactivation.certs.length == 0
    )
      return { err: 'Server does not have any ACM activation certificates.' };
    const activationCerts = domain.amtacmactivation.certs;
    if (
      dev.mpsConnection.tag.meiState == null ||
      dev.mpsConnection.tag.meiState.Hashes == null ||
      dev.mpsConnection.tag.meiState.Hashes.length == 0
    )
      return { err: 'Intel AMT did not report any trusted hashes.' };
    const deviceHashes = dev.mpsConnection.tag.meiState.Hashes;

    // Get the trusted FQDN of the device
    var trustedFqdn = null;
    if (dev.mpsConnection.tag.meiState.OsDnsSuffix != null) {
      trustedFqdn = dev.mpsConnection.tag.meiState.OsDnsSuffix;
    }
    if (dev.mpsConnection.tag.meiState.DnsSuffix != null) {
      trustedFqdn = dev.mpsConnection.tag.meiState.DnsSuffix;
    }
      ...
      if (cert.cn == '*' || certDnsMatch) {
        for (var j in deviceHashes) {
          ...
          if (hashInfo != null && hashInfo.isActive == 1) {
            devValidHash = true;
            if (hashInfo.hashAlgorithmStr == 'SHA256' && hashInfo.certificateHash.toLowerCase() == cert.sha256) {
              return { cert: cert, fqdn: trustedFqdn, hash: cert.sha256 };
            } // Found a match
            else if (hashInfo.hashAlgorithmStr == 'SHA1' && hashInfo.certificateHash.toLowerCase() == cert.sha1) {
              return { cert: cert, fqdn: trustedFqdn, hash: cert.sha1 };
            } // Found a match
          }
        }
      }
    ...
    if (!devValidHash) {
      return { err: 'Intel AMT has no trusted root hashes for "' + trustedFqdn + '".' };
    } // Found no trusted root hashes
    if (gotSuffixMatch) {
      return { err: 'Certificate root hash matching failed for "' + trustedFqdn + '".' };
    } // Found a DNS suffix match, but root hash failed to match.
  ...
  function checkAcmActivationCertName(commonName, trustedFqdn) {
    ...
    if (commonName.startsWith('*.') && commonName.length > 2) {
      commonName = commonName.substring(2);
    }
  ...
  function activateIntelAmtTlsAcm(dev, password, acminfo) {
    ...
    if (
      ...
      dev.mpsConnection.tag != null &&
      dev.mpsConnection.tag.meiState != null &&
      typeof dev.mpsConnection.tag.meiState['core-ver'] == 'number' &&
      dev.mpsConnection.tag.meiState['core-ver'] > 0
    ) {
      // Generate a random Intel AMT password if needed
      if (password == null || password == '') {
        password = getRandomAmtPassword();
      ...
      var acmTlsInfo = parent.certificateOperations.getAcmCertChain(
        parent.config.domains[dev.domainid],
        dev.temp.acminfo.fqdn,
        dev.temp.acminfo.hash
      );
      if (acmTlsInfo.error == 1) {
        dev.consoleMsg(acmTlsInfo.errorText);
        removeAmtDevice(dev, 44);
        return;
      }
  ...
  function activateIntelAmtTlsAcmEx(dev, startConfigData) {
    if (startConfigData == null || startConfigData.status != 0 || typeof startConfigData.hash != 'string') {
      ...
      var comm = CreateWsmanComm(
        ...
        '',
        1,
        { cert: dev.acmTlsInfo.certs.join(''), key: dev.acmTlsInfo.signkey },
        dev.mpsConnection
      ); // TLS with client certificate chain and key.
      ...
      dev.amtstack.BatchEnum(
        null,
        ['*AMT_GeneralSettings', 'CIM_SoftwareIdentity', '*AMT_SetupAndConfigurationService'],
        activateIntelAmtTlsAcmEx1
      );
    }
  }
  ...
  function activateIntelAmtTlsAcmEx1(stack, name, responses, status) {
    ...
    if (
      responses != null &&
      responses['CIM_SoftwareIdentity'] != null &&
      responses['CIM_SoftwareIdentity'].responses != null
    ) {
      var amtlogicalelements = [];
      amtlogicalelements = responses['CIM_SoftwareIdentity'].responses;
      if (
        responses['AMT_SetupAndConfigurationService'] != null &&
        responses['AMT_SetupAndConfigurationService'].response != null
      ) {
      ...
      if (amtlogicalelements.length > 0) {
        ...
        if (vs != null) {
          ...
          if (version.length > 2) {
            dev.aquired.versionmaintenance = parseInt(version[2]);
          }
        }
      }
    }
    ...
    if (dev.aquired.version == null) {
      dev.consoleMsg('Could not get Intel AMT version.');
      removeAmtDevice(dev);
      return;
    ...
    if (
      responses['AMT_GeneralSettings'] &&
      responses['AMT_GeneralSettings'].response &&
      typeof responses['AMT_GeneralSettings'].response['DigestRealm'] == 'string'
    ) {
      // Set the realm in the stack since we are not doing HTTP digest and this will be checked later by different code.
      dev.aquired.realm = dev.amtstack.wsman.comm.digestRealm =
        responses['AMT_GeneralSettings'].response['DigestRealm'];
    } else {
      dev.consoleMsg('Could not get Intel AMT digest realm.');
      removeAmtDevice(dev);
      return;
    }
    ...
    var event = {
      ...
      domain: dev.domainid,
      msgid: 111,
      msgArgs: [dev.temp.acminfo.fqdn],
      msg: 'Device requested Intel(R) AMT ACM TLS activation, FQDN: ' + dev.temp.acminfo.fqdn
    };
    if (parent.db.changeStream) {
      event.noact = 1;
    } // If DB change stream is active, don't use this event to change the node. Another event will come.
    ...
    if (typeof dev.temp.mebxpass == 'string') {
      ...
      dev.amtstack.AMT_AuthorizationService_SetAdminAclEntryEx(
        dev.aquired.user,
        hex_md5(dev.aquired.user + ':' + dev.aquired.realm + ':' + dev.aquired.pass),
        activateIntelAmtTlsAcmEx3
      );
    }
  }
  ...
  function activateIntelAmtTlsAcmEx2(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to set MEBx password, status=' + status + '.');
      destroyDevice(dev);
      return;
    }
  ...
  function activateIntelAmtTlsAcmEx3(stack, name, responses, status) {
    ...
    if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
    if (status != 200) {
      dev.consoleMsg('Failed to set admin password, status=' + status + '.');
      removeAmtDevice(dev);
      return;
    }
    ...
    if (dev.mpsConnection != null && dev.mpsConnection.tag != null && dev.mpsConnection.tag.meiState != null) {
      dev.mpsConnection.tag.meiState.ProvisioningState = 2;
    }
  ...
  function activateIntelAmtAcmEx1(stack, name, responses, status) {
    ...
    if (status != 200) {
      dev.consoleMsg('Failed to get Intel AMT state.');
      removeAmtDevice(dev, 46);
      return;
    }

    // Sign the Intel AMT ACM activation request
    var info = {
      nonce: responses['IPS_HostBasedSetupService'].response['ConfigurationNonce'],
      realm: responses['AMT_GeneralSettings'].response['DigestRealm'],
      fqdn: dev.temp.acminfo.fqdn,
      hash: dev.temp.acminfo.hash,
      uuid: dev.mpsConnection.tag.meiState.UUID
    };
    var acmdata = parent.certificateOperations.signAcmRequest(
      parent.config.domains[dev.domainid],
      info,
      'admin',
      dev.temp.pass,
      dev.mpsConnection.remoteAddr + ':' + dev.mpsConnection.remotePort,
      dev.nodeid,
      dev.meshid,
      dev.name,
      0
    ...
    if (acmdata == null) {
      dev.consoleMsg('Failed to sign ACM nonce.');
      removeAmtDevice(dev, 47);
      return;
    }
    if (acmdata.error != null) {
      dev.consoleMsg(acmdata.errorText);
      removeAmtDevice(dev, 48);
      return;
    }
    ...
    var event = {
      ...
      msgid: 58,
      msgArgs: [dev.temp.acminfo.fqdn],
      msg: 'Device requested Intel(R) AMT ACM activation, FQDN: ' + dev.temp.acminfo.fqdn
    };
    if (parent.db.changeStream) {
      event.noact = 1;
    } // If DB change stream is active, don't use this event to change the node. Another event will come.
  ...
  function activateIntelAmtAcmEx2(dev) {
    ...
    var leaf = acmdata.index == 0,
      root = acmdata.index == acmdata.certs.length - 1;
    if (acmdata.index < acmdata.certs.length && acmdata.certs[acmdata.index] != null) {
      dev.amtstack.IPS_HostBasedSetupService_AddNextCertInChain(
        acmdata.certs[acmdata.index],
        leaf,
        root,
        function (stack, name, responses, status) {
          const dev = stack.dev;
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to set ACM certificate chain (' + status + ').');
            removeAmtDevice(dev, 49);
            return;
          }
          if (responses['Body']['ReturnValue'] != 0) {
            dev.consoleMsg('Failed to set ACM certificate chain (ERR/' + responses['Body']['ReturnValue'] + ').');
            removeAmtDevice(dev, 50);
            return;
          }
      ...
      dev.amtstack.IPS_HostBasedSetupService_AdminSetup(
        2,
        acmdata.password,
        acmdata.nonce,
        2,
        ...
        function (stack, name, responses, status) {
          ...
          if (isAmtDeviceValid(dev) == false) return; // Device no longer exists, ignore this request.
          if (status != 200) {
            dev.consoleMsg('Failed to complete ACM activation (' + status + ').');
            removeAmtDevice(dev, 51);
            return;
          }
          if (responses['Body']['ReturnValue'] != 0) {
            dev.consoleMsg('Failed to complete ACM activation (ERR/' + responses['Body']['ReturnValue'] + ').');
            removeAmtDevice(dev, 52);
            return;
          }
  ...
  function deactivateIntelAmtCCMEx(dev, state) {
    if (state != 0) {
      ...
      if (dev.policy.amtPolicy == 1) {
        // Deactivation policy, we are done.
  ...
  function devTaskCompleted(dev) {
    dev.taskCount--;
    if (dev.taskCount == 0) {
      var f = dev.taskCompleted;
      delete dev.taskCount;
      delete dev.taskCompleted;
      if (f != null) {
        f(dev);
      }
    }
  ...
  function amtcert_linkCertPrivateKey(certs, keys) {
    ...
    for (var i in certs) {
      var cert = certs[i];
      try {
        var publicKeyPEM = obj.parent.certificateOperations.forge.pki
          .publicKeyToPem(
            obj.parent.certificateOperations.forge.pki.certificateFromAsn1(
              obj.parent.certificateOperations.forge.asn1.fromDer(cert.X509CertificateBin)
            ).publicKey
          )
          .substring(28 + 32)
          .replace(/(\r\n|\n|\r)/gm, '');
        ...
        for (var j = 0; j < keys.length; j++) {
          if (publicKeyPEM === keys[j]['DERKey'] || publicKeyPEM == btoa(atob(keys[j]['DERKey']).substring(24))) {
            // Match directly or, new version of Intel AMT put the key type OID in the private key, skip that and match.
            keys[j].XCert = cert; // Link the key pair to the certificate
            cert.XPrivateKey = keys[j]; // Link the certificate to the key pair
          }
        }
  ...
  function getRandomLowerCase(len) {
    var r = '',
      random = parent.crypto.randomBytes(len);
    for (var i = 0; i < len; i++) {
  ...
  function getInstance(x, y) {
    for (var i in x) {
      if (x[i]['InstanceID'] == y) return x[i];
    }
    return null;
  }

  function hex_md5(str) {
    return parent.crypto.createHash('md5').update(str).digest('hex');
  }
  function Clone(v) {
    return JSON.parse(JSON.stringify(v));
  }
  function MakeToArray(v) {
    if (!v || v == null || typeof v == 'object') return v;
    return [v];
  }
  function getItem(x, y, z) {
    for (var i in x) {
      if (x[i][y] == z) return x[i];
    }
    return null;
  }
  function IntToStr(v) {
  ...
  function makeUefiBootParam(type, data, len) {
    if (typeof data == 'number') {
      ...
      if (len == 2) {
        data = parent.common.ShortToStrX(data);
      }
      if (len == 4) {
        data = parent.common.IntToStrX(data);
      }
    }
    return (
      parent.common.ShortToStrX(0x8086) + parent.common.ShortToStrX(type) + parent.common.IntToStrX(data.length) + data
    );
  }
  ...
  function parseCertName(x) {
    ...
    for (var i in xx) {
      j = xx[i].indexOf('=');
      r[xx[i].substring(0, j)] = xx[i].substring(j + 1);
    }
    return r;
  }
  ...
  obj.amtcert_createCertificate = function (certAttributes, caPrivateKey, DERKey, issuerAttributes, extKeyUsage) {
    ...
    var keys,
      cert = obj.parent.certificateOperations.forge.pki.createCertificate();
    cert.publicKey = obj.parent.certificateOperations.forge.pki.publicKeyFromPem(
      '-----BEGIN PUBLIC KEY-----' + DERKey + '-----END PUBLIC KEY-----'
    );
