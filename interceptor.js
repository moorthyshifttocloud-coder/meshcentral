...
module.exports.CreateHttpInterceptor = function (args) {
  ...
  obj.randomValueHex = function (len) {
    return crypto
  ...
  obj.processAmtDataEx = function () {
    ...
    if (obj.amt.mode == 0) {
      ...
      for (i in obj.amt.headers) {
        r += i + ': ' + obj.amt.headers[i] + '\r\n';
      }
      r += '\r\n';
      return r;
    } else if (obj.amt.mode == 1) {
      // Length Body Mode
  ...
  obj.processBrowserDataEx = function () {
    ...
    if (obj.ws.mode == 0) {
      ...
      if (
        obj.blockAmtStorage == true &&
        obj.ws.directive.length > 1 &&
        obj.ws.directive[1].indexOf('/amt-storage') == 0
      ) {
        obj.ws.directive[1] = obj.ws.directive[1].replace('/amt-storage', '/amt-dummy-storage');
      }
      var headers = headerlines.slice(1);
      obj.ws.headers = {};
      ...
      if (obj.args.user && obj.args.pass && HttpInterceptorAuthentications[obj.args.host + ':' + obj.args.port]) {
        ...
        var hash = obj.ComputeDigesthash(
          obj.args.user,
          obj.args.pass,
          AuthArgs.realm,
          obj.ws.directive[0],
        ...
        var authstr =
          ...
          '"';
        if (AuthArgs.opaque) {
          authstr += ',opaque="' + AuthArgs.opaque + '"';
        }
        obj.ws.headers.authorization = authstr;
        obj.ws.authCNonceCount++;
      } else {
        // We don't have authentication, clear it out of the header if needed.
        if (obj.ws.headers.authorization) {
          delete obj.ws.headers.authorization;
        }
      }

      // Reform the HTTP header
      r = obj.ws.directive.join(' ') + '\r\n';
      for (i in obj.ws.headers) {
        r += i + ': ' + obj.ws.headers[i] + '\r\n';
      }
      r += '\r\n';
      return r;
    } else if (obj.ws.mode == 1) {
      // Length Body Mode
      // Send the body of content-length size
      var rl = obj.ws.count;
...
module.exports.CreateRedirInterceptor = function (args) {
  ...
  obj.ws = {
    ...
    error: false,
    direct: false,
    authCNonce: obj.randomValueHex(10),
    authCNonceCount: 1
  };

  obj.RedirectCommands = {
  ...
  // Process data coming from Intel AMT
  obj.processAmtData = function (data) {
    if (obj.amt.direct == true && obj.amt.acc == '') {
      return data;
  ...
  // Process data coming from the Browser
  obj.processBrowserData = function (data) {
    if (obj.ws.direct == true && obj.ws.acc == '') {
      return data;
    } // Interceptor fast path
    obj.ws.acc += data.toString('binary'); // Add data to accumulator
    data = '';
        ...
        case obj.RedirectCommands.AuthenticateSession: {
          ...
          if (authType == obj.AuthenticationType.DIGEST && obj.args.user && obj.args.pass) {
            ...
            if (obj.amt.digestRealm) {
              ...
              obj.ws.authCNonceCount++;
              var digest = obj.ComputeDigesthash(
                obj.args.user,
                obj.args.pass,
                obj.amt.digestRealm,
                'POST',
              ...
              r += common.IntToStrX(
                obj.args.user.length +
                  obj.amt.digestRealm.length +
                  obj.amt.digestNonce.length +
                  authurl.length +
                  obj.ws.authCNonce.length +
                  nc.toString().length +
                  digest.length +
                  obj.amt.digestQOP.length +
                  8
              );
