...
if (args['_'].length == 0) {
  ...
  if (settings.cmd != 'run') {
    console.log('Invalid command. "run" command will start the bot.');
    return;
  } else {
    serverConnect();
  }
...
function serverConnect() {
  ...
  if (args.loginkey != null) {
    url += '?auth=' + args.loginkey;
  } // Cookie authentication
  else if (args.loginpass != null) {
  ...
  ws.on('message', function incoming(rawdata) {
    ...
    switch (data.action) {
      case 'serverinfo': {
        console.log('Connected to server: ' + data.serverinfo.name);
        break;
      }
      case 'userinfo': {
      ...
      case 'interuser': {
        ...
        if (args.targetuser == null && args.targetsession == null && typeof data.data == 'string') {
          // For testing, echo back the original message.
