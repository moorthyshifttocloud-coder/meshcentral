...
if (directRun && NodeJSVer < 8) {
  log('Translate.js requires Node v8 or above, current version is ' + process.version + '.');
  return;
}
...
if (directRun && NodeJSVer >= 12) {
  ...
  try {
    if (xworker.isMainThread == false) {
      ...
      if (minifyLib == 1) {
        log('minify-js is no longer used, please switch to "html-minifier-terser"');
        process.exit();
        return;
      }
      if (minifyLib == 2) {
...
function setup() {
  ...
  if (minifyLib == 2) {
    libs.push('html-minifier-terser@7.2.0');
  }
  InstallModules(libs, start);
}

function start() {
  startEx(process.argv);
}
...
async function startEx(argv) {
  ...
  if (command == 'extract') {
    if (argv.length < 4) {
      log('No language file specified.');
      process.exit();
      return;
    }
    var sources = [];
  ...
  if (command == 'fromtext') {
    if (argv.length == 6) {
      ...
      if (fs.existsSync(argv[4]) == false) {
        log('Unable to find: ' + argv[4]);
        return;
      }
  ...
  if (command == 'remove') {
    if (argv.length <= 3) {
      log('Usage: remove [language] (file)');
      return;
    }
    ...
    if (argv.length > 4) {
      if (fs.existsSync(argv[4]) == false) {
        ...
        translationFile = argv[4];
      }
    }
    sourceStrings = {};
    if (fs.existsSync(translationFile) == false) {
      log('Unable to find: ' + translationFile);
      return;
    }
    var langFileData = null;
    try {
      langFileData = JSON.parse(fs.readFileSync(translationFile));
    } catch (ex) {}
    if (langFileData == null || langFileData.strings == null) {
      log('Invalid language file.');
      process.exit();
      return;
    }
    for (var i in langFileData.strings) {
      delete langFileData.strings[i][lang];
    }
  ...
  if (command == 'translateall') {
    if (fs.existsSync('../views/translations') == false) {
      fs.mkdirSync('../views/translations');
    }
  ...
  if (command == 'translate') {
    ...
    var langFile = argv[4];
    if (fs.existsSync(langFile) == false) {
      log('Missing language file: ' + langFile);
      process.exit();
      return;
    }
  ...
  if (command == 'minifydir') {
    if (argv.length < 4) {
      log('Command source and/or destination folders missing.');
      process.exit();
      return;
    }
      ...
      if (sourceFiles[i].endsWith('.js') || sourceFiles[i].endsWith('.json')) {
        ...
        if (sourceFiles[i].endsWith('.js')) {
          ...
          const destinationFile = path.join(
            argv[4],
            sourceFiles[i].substring(0, sourceFiles[i].length - 3) + '.min.js'
          );
          if (minifyLib == 2) {
            var inFile = fs.readFileSync(sourceFile).toString();

            // Perform minification pre-processing
            if (sourceFile.endsWith('.handlebars') >= 0) {
              inFile = inFile.split('{{{pluginHandler}}}').join('"{{{pluginHandler}}}"');
            }
            if (sourceFile.endsWith('.js')) {
              inFile = '<script>' + inFile + '</script>';
            }
            ...
            if (sourceFile.endsWith('.js')) {
              minifiedOut = minifiedOut.substring(8, minifiedOut.length - 9);
            }
            if (sourceFile.endsWith('.handlebars') >= 0) {
              minifiedOut = minifiedOut.split('"{{{pluginHandler}}}"').join('{{{pluginHandler}}}');
            }
  ...
  if (command == 'minifyall') {
    for (var i in minifyMeshCentralSourceFiles) {
      ...
      if (minifyLib == 2) {
        ...
        // Perform minification pre-processing
        if (outname.endsWith('.handlebars') >= 0) {
          inFile = inFile.split('{{{pluginHandler}}}').join('"{{{pluginHandler}}}"');
        }
        ...
        try {
          minifiedOut = await minify(inFile, {
            ...
            } // Log errors from UglifyJS to console output
          });
        } catch (ex) {
          console.log(ex);
        }
...
function totext(source, target, lang) {
  ...
  try {
    sourceLangFileData = JSON.parse(fs.readFileSync(source));
  } catch (ex) {
    console.log(ex);
  }
  if (sourceLangFileData == null || sourceLangFileData.strings == null) {
    log('Invalid source language file.');
    process.exit();
    return;
  }
  ...
  for (var i in sourceLangFileData.strings) {
    if (
      sourceLangFileData.strings[i][lang] != null &&
      sourceLangFileData.strings[i][lang].indexOf('\r') == -1 &&
      sourceLangFileData.strings[i][lang].indexOf('\n') == -1
    ) {
      output.push(sourceLangFileData.strings[i][lang]);
      outputCharCount += sourceLangFileData.strings[i][lang].length + 2;
      if (outputCharCount > 4500) {
        outputCharCount = 0;
        splitOutputPtr++;
      }
      if (splitOutput[splitOutputPtr] == null) {
        splitOutput[splitOutputPtr] = [];
      }
...
function fromtext(source, target, lang) {
  ...
  if (sourceLangFileData.strings.length != rawTextArray.length) {
    log('String count mismatch, unable to import.');
    process.exit(1);
    return;
  }
  ...
  for (var i in sourceLangFileData.strings) {
    if (
      sourceLangFileData.strings[i]['en'] != null &&
      sourceLangFileData.strings[i]['en'].indexOf('\r') == -1 &&
      sourceLangFileData.strings[i]['en'].indexOf('\n') == -1
    ) {
...
function merge(source, target, lang) {
  ...
  try {
    targetLangFileData = JSON.parse(fs.readFileSync(target));
  } catch (ex) {
    console.log(ex);
  }
  if (targetLangFileData == null || targetLangFileData.strings == null) {
    log('Invalid target language file.');
    process.exit();
    return;
  }
  ...
  var index = {};
  for (var i in targetLangFileData.strings) {
    if (targetLangFileData.strings[i].en != null) {
      index[targetLangFileData.strings[i].en] = targetLangFileData.strings[i];
    }
  }
  ...
  for (var i in sourceLangFileData.strings) {
    if (
      sourceLangFileData.strings[i].en != null &&
      sourceLangFileData.strings[i][lang] != null &&
      index[sourceLangFileData.strings[i].en] != null
    ) {
      //if (sourceLangFileData.strings[i][lang] == null) {
  ...
  for (var i in index) {
    targetData.strings.push(index[i]);
  }
...
function translate(lang, langFile, sources, createSubDir) {
  if (directRun && NodeJSVer >= 12 && lang == null) {
    ...
    var langFileData = null;
    try {
      langFileData = JSON.parse(fs.readFileSync(langFile));
    } catch (ex) {
      console.log(ex);
    }
    if (langFileData == null || langFileData.strings == null) {
    ...
    function processNextTask() {
      if (activeWorkers < MAX_WORKERS && taskQueue.length > 0) {
        ...
        activeWorkers++;
        var worker = new Worker('./translate.js', {
          stdout: true,
          workerData: {
            op: 'translate',
            args: [nextTask.lang, nextTask.langFile, nextTask.sources, nextTask.createSubDir]
          }
        });
        worker.stdout.on('data', function (msg) {
          console.log('wstdio:', msg.toString());
        });
        ...
        worker.on('exit', function (code) {
          /*console.log('exit', code);*/ activeWorkers--;
          processNextTask();
        });
      }
    }
    ...
    for (var i = 0; i < Math.min(MAX_WORKERS, taskQueue.length); i++) {
      processNextTask();
    }
  } else {
  ...
  for (var i = 0; i < sources.length; i++) {
    if (sources[i].endsWith('.json')) {
      translateAllInJson(lang, langFile, sources[i]);
    }
  }
}
...
function translateSingleThreaded(lang, langFile, sources, createSubDir) {
  ...
  if (lang != null && lang != '*') {
    ...
    for (var i in langs) {
      translateEx(i, langFileData, sources, createSubDir);
    }
  }
...
function translateEx(lang, langFileData, sources, createSubDir) {
  ...
  for (var i in langFileData.strings) {
    var entry = langFileData.strings[i];
    if (entry['en'] != null && entry[lang] != null) {
      translationTable[entry['en']] = entry[lang];
    }
  }
  // Translate the files
  for (var i = 0; i < sources.length; i++) {
    if (sources[i].endsWith('.html') || sources[i].endsWith('.htm') || sources[i].endsWith('.handlebars')) {
      translateFromHtml(lang, sources[i], createSubDir);
    } else if (sources[i].endsWith('.txt')) {
      translateFromTxt(lang, sources[i], createSubDir);
    }
  }
}
...
function extract(langFile, sources) {
  ...
  for (var i = 0; i < sources.length; i++) {
    if (sources[i].endsWith('.html') || sources[i].endsWith('.htm') || sources[i].endsWith('.handlebars')) {
      ...
      extractFromTxt(sources[i]);
    } else if (sources[i].endsWith('.json')) {
      extractFromJson(sources[i]);
    }
  }
  var count = 0,
    output = [];
...
function extractFromTxt(file) {
  ...
  for (var i in lines) {
    ...
    if (line.length > 1 && line[0] != '~') {
      if (sourceStrings[line] == null) {
        sourceStrings[line] = { en: line, xloc: [name] };
      } else {
        if (sourceStrings[line].xloc == null) {
          sourceStrings[line].xloc = [];
        }
        sourceStrings[line].xloc.push(name);
      }
    }
  }
}
  ...
  for (var i in json.en) {
    if (typeof json.en[i] == 'string') {
      ...
      if (sourceStrings[str] == null) {
        ...
        if (sourceStrings[str].xloc == null) {
          sourceStrings[str].xloc = [];
        }
        sourceStrings[str].xloc.push(name);
      }
  ...
  for (var i = 0; i < node.childNodes.length; i++) {
    ...
    if (subnode.attributes != null && subnode.attributes.length > 0) {
      var subnodevalue = null,
        subnodeplaceholder = null,
        subnodetitle = null;
      for (var j in subnode.attributes) {
        if (subnode.attributes[j].name == 'notrans' && subnode.attributes[j].value == '1') {
          subnodeignore = true;
        }
        if (subnode.attributes[j].name == 'notransval' && subnode.attributes[j].value == '1') {
          subnodevalueignore = true;
        }
        if (subnode.attributes[j].name == 'type' && subnode.attributes[j].value == 'hidden') {
          subnodeignore = true;
        }
        if (subnode.attributes[j].name == 'value') {
          subnodevalue = subnode.attributes[j].value;
        }
        if (subnode.attributes[j].name == 'placeholder') {
          subnodeplaceholder = subnode.attributes[j].value;
        }
        if (subnode.attributes[j].name == 'title') {
          subnodetitle = subnode.attributes[j].value;
        }
      }
      if (subnodevalue != null && isNumber(subnodevalue) == true) {
        subnodevalue = null;
      }
      if (subnodeplaceholder != null && isNumber(subnodeplaceholder) == true) {
        subnodeplaceholder = null;
      }
      if (subnodetitle != null && isNumber(subnodetitle) == true) {
        subnodetitle = null;
      }
      if (subnodeignore == false && subnodevalueignore == false && subnodevalue != null) {
        // Add a new string to the list (value)
        if (sourceStrings[subnodevalue] == null) {
          sourceStrings[subnodevalue] = { en: subnodevalue, xloc: [name] };
        } else {
          if (sourceStrings[subnodevalue].xloc == null) {
            sourceStrings[subnodevalue].xloc = [];
          }
          sourceStrings[subnodevalue].xloc.push(name);
        }
      }
      if (subnodeplaceholder != null) {
        // Add a new string to the list (placeholder)
        if (sourceStrings[subnodeplaceholder] == null) {
          sourceStrings[subnodeplaceholder] = { en: subnodeplaceholder, xloc: [name] };
        } else {
          if (sourceStrings[subnodeplaceholder].xloc == null) {
            sourceStrings[subnodeplaceholder].xloc = [];
          }
          sourceStrings[subnodeplaceholder].xloc.push(name);
        }
      }
      if (subnodetitle != null) {
        // Add a new string to the list (title)
        if (sourceStrings[subnodetitle] == null) {
          sourceStrings[subnodetitle] = { en: subnodetitle, xloc: [name] };
        } else {
          if (sourceStrings[subnodetitle].xloc == null) {
            sourceStrings[subnodetitle].xloc = [];
          }
          sourceStrings[subnodetitle].xloc.push(name);
        }
      }
    }
      ...
      if (subnode.hasChildNodes()) {
        ...
        if (nodeValue.length > 0 && subnode.nodeType == 3) {
          if (
            node.tagName != 'SCRIPT' &&
            node.tagName != 'STYLE' &&
            nodeValue.length < 8000 &&
            nodeValue.startsWith('{{{') == false &&
            nodeValue != ' '
          ) {
            if (performCheck) {
              log('  "' + nodeValue + '"');
            ...
            if (sourceStrings[nodeValue] == null) {
              sourceStrings[nodeValue] = { en: nodeValue, xloc: [name] };
            } else {
              if (sourceStrings[nodeValue].xloc == null) {
                sourceStrings[nodeValue].xloc = [];
              }
              sourceStrings[nodeValue].xloc.push(name);
            }
...
function getStringFromJavaScript(name, script) {
  if (performCheck) {
    log(format('Processing JavaScript of {0} bytes: {1}', script.length, name));
  }
  var tokenScript = esprima.tokenize(script),
    count = 0;
  for (var i in tokenScript) {
    ...
    if (token.type == 'String' && token.value.length > 2 && token.value[0] == '"') {
      ...
      if (sourceStrings[str] == null) {
        sourceStrings[str] = { en: str, xloc: [name + '->' + ++count] };
      } else {
        if (sourceStrings[str].xloc == null) {
          sourceStrings[str].xloc = [];
        }
        sourceStrings[str].xloc.push(name + '->' + ++count);
      }
    }
  }
}
...
function translateFromTxt(lang, file, createSubDir) {
  ...
  for (var i in lines) {
    var line = lines[i];
    if (line.length > 1 && line[0] != '~') {
      if (translationTable[line] != null) {
        outlines.push(translationTable[line]);
      } else {
        outlines.push(line);
      }
    } else {
  ...
  for (var i in langFileData.strings) {
    ...
    for (var lang in entry) {
      ...
      if (languages.indexOf(lang) == -1) {
        languages.push(lang);
        xtranslationTable[lang] = {};
      }
      if (entry['en'] != null && entry[lang] != null) {
        xtranslationTable[lang][entry['en']] = entry[lang];
      }
    }
  }
    ...
    for (var j in languages) {
      ...
      for (var i in json.en) {
        if (typeof json.en[i] == 'string' && xtranslationTable[lang][json.en[i]] != null) {
          ...
          for (var k in json.en[i]) {
            var str = json.en[i][k];
            if (xtranslationTable[lang][str] != null) {
              r.push(xtranslationTable[lang][str]);
              translateCount++;
            } else {
              r.push(str);
            }
...
async function translateFromHtml(lang, file, createSubDir) {
  var data = fs.readFileSync(file);
  if (file.endsWith('.js')) {
    data = '<html><head></head><body><script>' + data + '</script></body></html>';
  }
  var { JSDOM } = jsdom;
  const dom = new JSDOM(data, { includeNodeLocations: true });
  ...
  if (outname.endsWith('.handlebars')) {
    ...
    if (out.startsWith('<html><head></head><body><script>')) {
      out = out.substring(33);
    }
    if (out.endsWith('</script></body></html>')) {
      out = out.substring(0, out.length - 23);
    }
  ...
  if (minifyLib == 1) {
    minify.file(
      ...
      function (e, compress) {
        ...
        compress.run((e) => {
          e ? log('Minification fail', e) : log('Minification success');
          minifyDone();
        });
      }
    );
  ...
  // Minify the file
  if (minifyLib == 2) {
    if (outnamemin.endsWith('.handlebars') >= 0) {
      out = out.split('{{{pluginHandler}}}').join('"{{{pluginHandler}}}"');
    }
  ...
  for (var i = 0; i < node.childNodes.length; i++) {
    ...
    if (subnode.attributes != null && subnode.attributes.length > 0) {
      var subnodevalue = null,
        subnodeindex = null,
        subnodeplaceholder = null,
        subnodeplaceholderindex = null,
        subnodetitle = null,
        subnodetitleindex = null;
      for (var j in subnode.attributes) {
        if (subnode.attributes[j].name == 'notrans' && subnode.attributes[j].value == '1') {
          subnodeignore = true;
        }
      ...
      if (subnodeignore == false && subnodevalue != null) {
        ...
        if (translationTable[subnodevalue] != null) {
          subnode.attributes[subnodeindex].value = translationTable[subnodevalue];
        }
      }
      if (subnodeplaceholder != null) {
        // Perform attribute translation for placeholder
        if (translationTable[subnodeplaceholder] != null) {
          subnode.attributes[subnodeplaceholderindex].value = translationTable[subnodeplaceholder];
        }
      }
      if (subnodetitle != null) {
        // Perform attribute translation for title
        if (translationTable[subnodetitle] != null) {
          subnode.attributes[subnodetitleindex].value = translationTable[subnodetitle];
        }
      }
    }
    ...
    if (subnodeignore == false) {
      ...
      if (subnode.hasChildNodes()) {
        ...
        if (x1 > 0) {
          frontTrim = subnode.nodeValue.substring(0, x1);
        }
        if (x1 != -1) {
          backTrim = subnode.nodeValue.substring(x1 + nodeValue.length);
        }
        ...
        if (nodeValue.length > 0 && subnode.nodeType == 3) {
          if (
            ...
            if (translationTable[nodeValue]) {
              subnode.nodeValue = frontTrim + translationTable[nodeValue] + backTrim;
            }
          } else if (node.tagName == 'SCRIPT') {
...
function translateStringsFromJavaScript(name, script) {
  if (performCheck) {
    log(format('Translating JavaScript of {0} bytes: {1}', script.length, name));
  }
  var tokenScript = esprima.tokenize(script, { range: true }),
    count = 0;
  var output = [],
    ptr = 0;
  for (var i in tokenScript) {
...
function isNumber(x) {
  return '' + parseInt(x) === x || '' + parseFloat(x) === x;
}
...
function InstallModuleEx(modulenames, func) {
  ...
  InstallModuleChildProcess = child_process.exec(
    ...
    function (error, stdout, stderr) {
      ...
      if (error != null && error != '') {
        log(
          'ERROR: Unable to install required modules. May not have access to npm, or npm may not have suffisent rights to load the new modules. Try "npm install ' +
            names +
            '" to manually install the modules.\r\n'
        );
        process.exit();
        return;
      }
