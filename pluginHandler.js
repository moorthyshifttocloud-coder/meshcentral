...
module.exports.pluginHandler = function (parent) {
    ...
    parent.db.getPlugins(function (err, plugins) {
      plugins.forEach(function (plugin) {
        ...
        if (obj.fs.existsSync(obj.pluginPath + '/' + plugin.shortName)) {
          try {
            obj.plugins[plugin.shortName] = require(
              obj.pluginPath + '/' + plugin.shortName + '/' + plugin.shortName + '.js'
            )[plugin.shortName](obj);
          ...
          try {
            // try loading local info about plugin to database (if it changed locally)
            ...
            console.log('Plugin config file for ' + plugin.name + ' could not be parsed.');
          }
        }
      });
  ...
  obj.addMeshCoreModules = function (modulesAdd) {
    for (var plugin in obj.plugins) {
      ...
      if (modulesDir == null) {
        try {
          moduleDirPath = obj.path.join(obj.pluginPath, plugin + '/modules_meshcore');
        ...
        for (var i in modulesDir) {
          if (modulesDir[i].toLowerCase().endsWith('.js')) {
            ...
            var moduleData = [
              ...
              moduleName,
              '", "',
              obj.parent.escapeCodeString(
                obj.fs.readFileSync(obj.path.join(moduleDirPath, modulesDir[i])).toString('binary')
              ),
              '"); addedModules.push("',
              moduleName,
              '"); } catch (e) { }\r\n'
  ...
  obj.deviceViewPanel = function () {
    ...
    for (var p in obj.plugins) {
      if (
        typeof obj.plugins[p].on_device_header === 'function' &&
        typeof obj.plugins[p].on_device_page === 'function'
      ) {
        try {
          panel[p] = {
  ...
  obj.isValidConfig = function (conf, url) {
  ...
  obj.getPluginConfig = function (configUrl) {
    return new Promise(function (resolve, reject) {
      ...
      if (
        typeof parent.config.settings.plugins.proxy == 'string' ||
        ...
        // Proxy support
        options.agent = new (require('https-proxy-agent').HttpsProxyAgent)(
          require('url').parse(parent.config.settings.plugins.proxy) ||
            process.env['HTTP_PROXY'] ||
            process.env['HTTPS_PROXY'] ||
            process.env['http_proxy'] ||
            process.env['https_proxy']
  ...
  obj.versionToNumber = function (ver) {
    var x = ver.split('-');
    if (x.length != 2) return ver;
    return x[0];
  };
  ...
  obj.versionCompare = function (current, minimal) {
    if (minimal.startsWith('>=')) {
      minimal = minimal.substring(2);
    }
    var c = obj.versionToNumber(current).split('.'),
      m = obj.versionToNumber(minimal).split('.');
    ...
    return new Promise(function (resolve, reject) {
      parent.db.getPlugins(function (err, plugins) {
        ...
        plugins.forEach(function (curconf) {
          proms.push(
            obj.getPluginConfig(curconf.configUrl).catch((e) => {
              return null;
            })
          );
        });
        ...
        Promise.all(proms)
          ...
          .catch((e) => {
            console.log('Error reaching plugins, update call aborted.', e);
          });
      });
    });
  };
    ...
    parent.db.getPlugin(id, function (err, docs) {
      ...
      if (plugin.repository.type == 'git') {
        ...
        if (
          ...
          // Proxy support
          opts.agent = new (require('https-proxy-agent').HttpsProxyAgent)(
            require('url').parse(parent.config.settings.plugins.proxy) ||
              process.env['HTTP_PROXY'] ||
              process.env['HTTPS_PROXY'] ||
              process.env['http_proxy'] ||
        ...
        var request = http.get(opts, function (response) {
          ...
          if (response.statusCode != null && response.statusCode >= 400) {
            return console.log('Error downloading plugin: HTTP ' + response.statusCode);
          }
          response.pipe(file);
              ...
              yauzl.open(fileName, { lazyEntries: true }, function (err, zipfile) {
                ...
                zipfile.on('entry', function (entry) {
                  ...
                  if (/\/$/.test(entry.fileName)) {
                    // dir
                    if (!obj.fs.existsSync(filePath)) obj.fs.mkdirSync(filePath);
                    zipfile.readEntry();
                  } else {
                    // file
                ...
                zipfile.on('end', function () {
                  setTimeout(function () {
                    ...
                    if (version_only == null || version_only === false) {
                      parent.db.setPluginStatus(id, 1, function () {
                        if (done) return;
                        done = true;
                      ...
                      parent.db.updatePlugin(id, { status: 1, version: version_only.name }, function () {
                        if (done) return;
                        done = true;
                        if (typeof func == 'function') {
                          func(null);
                    ...
                    try {
                      ...
                      obj.exports[plugin.shortName] = obj.plugins[plugin.shortName].exports;
                      if (typeof obj.plugins[plugin.shortName].server_startup == 'function')
                        obj.plugins[plugin.shortName].server_startup();
                    } catch (e) {
                      console.log('Error instantiating new plugin: ', e);
                    }
                    try {
                      var plugin_config = obj.fs.readFileSync(obj.pluginPath + '/' + plugin.shortName + '/config.json');
                      plugin_config = JSON.parse(plugin_config);
                ...
                zipfile.on('error', function (e) {
                  console.log('Error extracting plugin ZIP: ' + e.message);
                });
              });
            });
          });
        ...
        request.on('error', function (e) {
          console.log('Error downloading plugin: ' + e.message);
        });
        request.setTimeout(30000, function () {
          request.destroy(new Error('Timed out while downloading plugin'));
        });
            ...
            res.on('end', function () {
              if (versStr[0] == '{' || versStr[0] == '[') {
                ...
                try {
                  ...
                  reject('Version history problem.');
                }
              } else {
