...
module.exports.CreateMeshUser = function (parent, db, ws, req, args, domain, user) {
  ...
  const allowedCrossDomainMessages = [
    'accountcreate',
    'accountremove',
    'accountchange',
    'createusergroup',
  ...
  if (domain.maxdeviceview != null) {
    obj.deviceLimit = domain.maxdeviceview;
  }

  // Check if we are a cross-domain administrator
  if (
    parent.parent.config.settings.managecrossdomain &&
    parent.parent.config.settings.managecrossdomain.indexOf(user._id) >= 0
  ) {
  ...
  if (req.session != null && typeof req.session.expire == 'number') {
    ...
    if (delta <= 0) {
      ...
      return;
    } // Session is already expired, close now.
    obj.expireTimer = setTimeout(function () {
  ...
  function sendPing() {
    try {
      obj.ws.send('{"action":"ping"}');
    } catch (ex) {}
  }
  function sendPong() {
    try {
      obj.ws.send('{"action":"pong"}');
    } catch (ex) {}
  }

  // Setup the agent PING/PONG timers
  if (typeof args.browserping == 'number' && obj.pingtimer == null) {
    obj.pingtimer = setInterval(sendPing, args.browserping * 1000);
  } else if (typeof args.browserpong == 'number' && obj.pongtimer == null) {
    obj.pongtimer = setInterval(sendPong, args.browserpong * 1000);
  }
  ...
  obj.close = function (arg) {
    ...
    if (arg == 1 || arg == null) {
      try {
        obj.ws.close();
        parent.parent.debug('user', 'Soft disconnect');
      } catch (ex) {
        console.log(ex);
      }
    ...
    if (arg == 2) {
      try {
        obj.ws._socket._parent.end();
        parent.parent.debug('user', 'Hard disconnect');
      } catch (ex) {
        console.log(ex);
      }
    ...
    if (obj.expireTimer != null) {
      clearTimeout(obj.expireTimer);
    ...
    if (obj.serverStatsTimer != null) {
      clearInterval(obj.serverStatsTimer);
      delete obj.serverStatsTimer;
    }
    if (req.session && req.session.ws && req.session.ws == obj.ws) {
      delete req.session.ws;
    }
    if (parent.wssessions2[ws.sessionId]) {
      delete parent.wssessions2[ws.sessionId];
    }
    ...
    if (obj.user != null && parent.wssessions[obj.user._id]) {
      ...
      if (i >= 0) {
        ...
        if (user) {
          ...
          if (parent.parent.multiServer == null) {
            ...
            parent.parent.DispatchEvent(targets, obj, {
              ...
              userid: user._id,
              username: user.name,
              count: parent.wssessions[obj.user._id].length,
              nolog: 1,
              domain: domain.id
            });
          } else {
        ...
        if (parent.wssessions[obj.user._id].length == 0) {
          delete parent.wssessions[obj.user._id];
        }
      }
    }
    ...
    if (parent.parent.multiServer != null) {
      parent.parent.multiServer.DispatchMessage({ action: 'sessionEnd', sessionid: ws.sessionId });
    }
    ...
    if (obj.user != null) {
      ...
      if (obj.user.access < timeNow - 300) {
        // Only update user access time if longer than 5 minutes
        ...
        var message = {
          ...
          account: parent.CloneSafeUser(obj.user),
  ...
  function meshPathToRealPath(meshpath, user) {
    ...
    var rootfolder = meshpath[0],
      rootfoldersplit = rootfolder.split('/'),
      domainx = 'domain';
    if (rootfoldersplit[1].length > 0) domainx = 'domain-' + rootfoldersplit[1];
    var path = parent.path.join(
      parent.filespath,
      domainx,
      rootfoldersplit[0] + '-' + rootfoldersplit[2].replace(/[\\\/:\*\?"<>\|]/g, '_')
    );
    for (var i = 1; i < meshpath.length; i++) {
      if (common.IsFilenameValid(meshpath[i]) == false) {
        path = null;
        break;
      }
      path += '/' + meshpath[i];
  ...
  function copyFile(src, dest, func, tag) {
    if (fs.copyFile) {
      ...
      // Older NodeJS
      try {
        var ss = fs.createReadStream(src),
          ds = fs.createWriteStream(dest);
        ss.on('error', function () {
          func(tag);
        });
  ...
  function routeCommandToNode(command, requiredRights, requiredNonRights, func, options) {
    if (common.validateString(command.nodeid, 8, 128) == false) {
      if (func) {
        func(false);
      }
      return false;
    ...
    if (splitnodeid[0] == 'node' && splitnodeid[1] == domain.id) {
      ...
      if (agent != null) {
        ...
        parent.GetNodeWithRights(domain, user, agent.dbNodeKey, function (node, rights, visible) {
          ...
          if (node != null && mesh != null && (rights & MESHRIGHT_REMOTECONTROL || rights & MESHRIGHT_REMOTEVIEWONLY)) {
            // 8 is remote control permission, 256 is desktop read only
            if (requiredRights != null && (rights & requiredRights) == 0) {
              if (func) {
                func(false);
                return;
              }
            } // Check Required Rights
            if (requiredNonRights != null && rights != MESHRIGHT_ADMIN && (rights & requiredNonRights) != 0) {
              if (func) {
                func(false);
                return;
              }
            } // Check Required None Rights

            command.sessionid = ws.sessionId; // Set the session id, required for responses
            command.rights = rights; // Add user rights flags to the message
            if (
              options != null &&
              options.removeViewOnlyLimitation === true &&
              command.rights != 0xffffffff &&
              (command.rights & 0x100) != 0
            ) {
              command.rights -= 0x100;
            } // Since the multiplexor will enforce view-only, remove MESHRIGHT_REMOTEVIEWONLY
            command.consent = 0;
            if (typeof domain.userconsentflags == 'number') {
              command.consent |= domain.userconsentflags;
            } // Add server required consent flags
            if (typeof mesh.consent == 'number') {
              command.consent |= mesh.consent;
            } // Add device group user consent
            if (typeof node.consent == 'number') {
              command.consent |= node.consent;
            } // Add node user consent
            if (typeof user.consent == 'number') {
              command.consent |= user.consent;
            } // Add user consent

            // If desktop is viewonly, add this here.
            if (typeof domain.desktop == 'object' && domain.desktop.viewonly == true) {
              command.desktopviewonly = true;
            }
            ...
            if (typeof domain.desktopprivacybartext == 'string') {
              command.privacybartext = domain.desktopprivacybartext;
            } // Privacy bar text
        ...
        if (routing != null) {
          ...
          parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
            ...
            if (
              node != null &&
              mesh != null &&
              (rights & MESHRIGHT_REMOTECONTROL || rights & MESHRIGHT_REMOTEVIEWONLY)
            ) {
              // 8 is remote control permission
  ...
  function routeCommandToMesh(meshid, command) {
    ...
    if (parent.parent.multiServer != null) {
      parent.parent.multiServer.DispatchMessage({ action: 'agentMsgByMeshId', meshid: meshid, command: command });
    }

    // See if the node is connected
    for (var nodeid in parent.wsagents) {
      var agent = parent.wsagents[nodeid];
      if (agent.dbMeshKey == meshid) {
        try {
          agent.send(JSON.stringify(command));
        } catch (ex) {}
      }
  ...
  try {
    ...
    if (typeof domain.limits.maxusersessions == 'number' || typeof domain.limits.maxsingleusersessions == 'number') {
      ...
      var domainUserSessionCount = 0,
        selfUserSessionCount = 0;
      for (var i in parent.wssessions2) {
        if (parent.wssessions2[i].domainid == domain.id) {
          domainUserSessionCount++;
          if (parent.wssessions2[i].userid == user._id) {
            selfUserSessionCount++;
          }
        }
      }
      ...
      if (
        (typeof domain.limits.maxusersessions == 'number' && domainUserSessionCount >= domain.limits.maxusersessions) ||
        (typeof domain.limits.maxsingleusersessions == 'number' &&
          selfUserSessionCount >= domain.limits.maxsingleusersessions)
      ) {
        try {
    ...
    parent.crypto.randomBytes(20, function (err, randombuf) {
      ...
      if (!parent.wssessions[user._id]) {
        parent.wssessions[user._id] = [ws];
      } else {
        parent.wssessions[user._id].push(ws);
      }
      if (parent.parent.multiServer == null) {
        var targets = ['*', 'server-users'];
        if (obj.user.groups) {
        ...
        parent.parent.DispatchEvent(targets, obj, {
          ...
          userid: user._id,
          username: user.name,
          count: parent.wssessions[user._id].length,
          nolog: 1,
          domain: domain.id
        });
      } else {
      ...
      if (parent.parent.multiServer != null) {
        parent.parent.multiServer.DispatchMessage({ action: 'sessionStart', sessionid: ws.sessionId });
      }
      ...
      ws.HandleEvent = function (source, event, ids, id) {
        ...
        if (
          ...
          typeof event == 'object' &&
          event.action == 'loginTokenChanged' &&
          event.removed != null &&
          event.removed.indexOf(req.session.loginToken) >= 0
        ) {
          delete req.session;
          obj.close();
          return;
        ...
        if (
          event.domain == null ||
          event.domain == domain.id ||
          (obj.crossDomain === true && allowedCrossDomainMessages.indexOf(event.action) >= 0)
        ) {
          try {
            if (event == 'close') {
              try {
              ...
              obj.close();
              return;
            } else if (event == 'resubscribe') {
              user.subscriptions = parent.subscribe(user._id, ws);
            } else if (event == 'updatefiles') {
              ...
              if (
                (event.action == 'deviceShareUpdate' && Array.isArray(event.deviceShares)) ||
                (event.action == 'changenode' &&
                  event.node != null &&
                  (event.node.rdp != null || event.node.ssh != null))
              ) {
                event = common.Clone(event);
                if (event.action == 'deviceShareUpdate' && Array.isArray(event.deviceShares)) {
                  for (var i in event.deviceShares) {
                    if (event.deviceShares[i].userid != user._id) {
                      delete event.deviceShares[i].url;
                    }
                  }
                }
                if (
                  ...
                  if (event.node.rdp != null && typeof event.node.rdp[user._id] == 'number') {
                    event.node.rdp = event.node.rdp[user._id];
                  } else {
                    delete event.node.rdp;
                  }
                  if (event.node.ssh != null && typeof event.node.ssh[user._id] == 'number') {
                    event.node.ssh = event.node.ssh[user._id];
                  } else {
                    delete event.node.ssh;
                  }
                }
              }

              // This is a MeshCentral Satellite message
              if (event.action == 'satellite') {
                if ((obj.ws.satelliteFlags & event.satelliteFlags) != 0) {
                  try {
              ...
              if (id.startsWith('mesh/')) {
                ...
                if (
                  meshrights === MESHRIGHT_ADMIN ||
                  (meshrights & MESHRIGHT_LIMITEVENTS) == 0 ||
                  ids.indexOf(user._id) >= 0
                ) {
      ...
      obj.SendServerStats = function () {
        ...
        if (parent.parent.platform != 'win32') {
          try {
            stats.availablemem =
              1024 * Number(/MemAvailable:[ ]+(\d+)/.exec(fs.readFileSync('/proc/meminfo', 'utf8'))[1]);
          } catch (ex) {}
        }
        ...
        for (var i in parent.meshes) {
          if (parent.meshes[i].deleted == null) {
            activeDeviceGroups++;
          }
        } // This is not ideal for performance, we want to dome something better.
        var serverStats = {
        ...
        if (parent.relaySessionErrorCount != 0) {
          serverStats.RelayErrors = parent.relaySessionErrorCount;
        }
        if (parent.parent.mpsserver != null) {
          serverStats.ConnectedIntelAMTCira = 0;
          for (var i in parent.parent.mpsserver.ciraConnections) {
            serverStats.ConnectedIntelAMTCira += parent.parent.mpsserver.ciraConnections[i].length;
          }
        }
        for (var i in parent.parent.connectivityByNode) {
          const node = parent.parent.connectivityByNode[i];
          if (node && typeof node.connectivity !== 'undefined' && node.connectivity === 4) {
            serverStats.ConnectedIntelAMT++;
          }
        }
        ...
        var errorCounters = {},
          errorCountersCount = 0;
        if (agentstats.meshDoesNotExistCount > 0) {
          errorCountersCount++;
          errorCounters.UnknownGroup = agentstats.meshDoesNotExistCount;
        }
        if (agentstats.invalidPkcsSignatureCount > 0) {
          errorCountersCount++;
          errorCounters.InvalidPKCSsignature = agentstats.invalidPkcsSignatureCount;
        }
        if (agentstats.invalidRsaSignatureCount > 0) {
          errorCountersCount++;
          errorCounters.InvalidRSAsignature = agentstats.invalidRsaSignatureCount;
        }
        if (agentstats.invalidJsonCount > 0) {
          errorCountersCount++;
          errorCounters.InvalidJSON = agentstats.invalidJsonCount;
        }
        if (agentstats.unknownAgentActionCount > 0) {
          errorCountersCount++;
          errorCounters.UnknownAction = agentstats.unknownAgentActionCount;
        }
        if (agentstats.agentBadWebCertHashCount > 0) {
          errorCountersCount++;
          errorCounters.BadWebCertificate = agentstats.agentBadWebCertHashCount;
        }
        if (agentstats.agentBadSignature1Count + agentstats.agentBadSignature2Count > 0) {
          errorCountersCount++;
          errorCounters.BadSignature = agentstats.agentBadSignature1Count + agentstats.agentBadSignature2Count;
        }
        if (agentstats.agentMaxSessionHoldCount > 0) {
          errorCountersCount++;
          errorCounters.MaxSessionsReached = agentstats.agentMaxSessionHoldCount;
        }
        if (agentstats.invalidDomainMeshCount + agentstats.invalidDomainMesh2Count > 0) {
          errorCountersCount++;
          errorCounters.UnknownDeviceGroup = agentstats.invalidDomainMeshCount + agentstats.invalidDomainMesh2Count;
        }
        if (agentstats.invalidMeshTypeCount + agentstats.invalidMeshType2Count > 0) {
          errorCountersCount++;
          errorCounters.InvalidDeviceGroupType = agentstats.invalidMeshTypeCount + agentstats.invalidMeshType2Count;
        }
        ...
        if (errorCountersCount > 0) {
          stats.values.AgentErrorCounters = errorCounters;
        }
        try {
      ...
      var serverinfo = {
        ...
        emailcheck:
          ...
          domain.auth != 'ldap' &&
          args.lanonly != true &&
          parent.certificates.CommonName != null &&
          parent.certificates.CommonName.indexOf('.') != -1 &&
          user._id.split('/')[2].startsWith('~') == false,
      ...
      serverinfo.tlshash = Buffer.from(parent.webCertificateFullHashs[domain.id], 'binary')
        .toString('hex')
      ...
      if (typeof domain.sessionrecording == 'object') {
        if (domain.sessionrecording.onlyselectedusers === true) {
          serverinfo.usersSessionRecording = 1;
        } // Allow enabling of session recording for users
        if (domain.sessionrecording.onlyselectedusergroups === true) {
          serverinfo.userGroupsSessionRecording = 1;
        } // Allow enabling of session recording for user groups
        if (domain.sessionrecording.onlyselecteddevicegroups === true) {
          serverinfo.devGroupSessionRecording = 1;
        } // Allow enabling of session recording for device groups
      }
      if (
        parent.parent.config.domains[domain.id].amtacmactivation != null &&
        parent.parent.config.domains[domain.id].amtacmactivation.acmmatch != null
      ) {
        var matchingDomains = [];
        for (var i in parent.parent.config.domains[domain.id].amtacmactivation.acmmatch) {
          ...
          if (cn != '*' && matchingDomains.indexOf(cn) == -1) {
            matchingDomains.push(cn);
          }
        }
        if (matchingDomains.length > 0) {
          serverinfo.amtAcmFqdn = matchingDomains;
        }
      }
      if (typeof domain.devicemeshrouterlinks == 'object') {
        serverinfo.devicemeshrouterlinks = domain.devicemeshrouterlinks;
      }
      if (
        typeof domain.altmessenging == 'object' &&
        typeof domain.altmessenging.name == 'string' &&
        typeof domain.altmessenging.url == 'string'
      ) {
        serverinfo.altmessenging = [
          {
            name: domain.altmessenging.name,
            url: domain.altmessenging.url,
            localurl: domain.altmessenging.localurl,
            type: domain.altmessenging.type
          }
        ];
      }
      if (Array.isArray(domain.altmessenging)) {
        serverinfo.altmessenging = [];
        for (var i in domain.altmessenging) {
          if (
            typeof domain.altmessenging[i] == 'object' &&
            typeof domain.altmessenging[i].name == 'string' &&
            typeof domain.altmessenging[i].url == 'string'
          ) {
            serverinfo.altmessenging.push({
              name: domain.altmessenging[i].name,
              url: domain.altmessenging[i].url,
              type: domain.altmessenging[i].type
            });
          }
        }
      }
      ...
      if (parent.parent.webpush != null) {
        serverinfo.vapidpublickey = parent.parent.webpush.vapidPublicKey;
      } // Web push public key
      if (parent.parent.amtProvisioningServer != null) {
        serverinfo.amtProvServerMeshId = parent.parent.amtProvisioningServer.meshid;
      } // Device group that allows for bare-metal Intel AMT activation
      if (typeof domain.autoremoveinactivedevices == 'number' && domain.autoremoveinactivedevices > 0) {
        serverinfo.autoremoveinactivedevices = domain.autoremoveinactivedevices;
      } // Default number of days before inactive devices are removed
      if (domain.passwordrequirements) {
        if (domain.passwordrequirements.lock2factor == true) {
          serverinfo.lock2factor = true;
        } // Indicate 2FA change are not allowed
        if (typeof domain.passwordrequirements.maxfidokeys == 'number') {
          serverinfo.maxfidokeys = domain.passwordrequirements.maxfidokeys;
        }
      }
      if (parent.parent.msgserver != null) {
        // Setup messaging providers information
        serverinfo.userMsgProviders = parent.parent.msgserver.providers;
        if (parent.parent.msgserver.discordUrl != null) {
          serverinfo.discordUrl = parent.parent.msgserver.discordUrl;
        }
      }
      if (
        typeof parent.parent.config.messaging == 'object' &&
        typeof parent.parent.config.messaging.ntfy == 'object' &&
        typeof parent.parent.config.messaging.ntfy.userurl == 'string'
      ) {
        // nfty user url
      ...
      if (typeof parent.args.agentaliasdns == 'string') {
        agentServerName = parent.args.agentaliasdns;
      }
      ...
      if (parent.args.agentport != null) {
        agentHttpsPort = parent.args.agentport;
      } // If an agent only port is enabled, use that.
      if (parent.args.agentaliasport != null) {
        agentHttpsPort = parent.args.agentaliasport;
      } // If an agent alias port is specified, use that.
      serverinfo.magenturl =
        'mc://' +
        agentServerName +
      ...
      if (domain.guestdevicesharing === false) {
        serverinfo.guestdevicesharing = false;
      } else {
        if (typeof domain.guestdevicesharing == 'object') {
          if (typeof domain.guestdevicesharing.maxsessiontime == 'number') {
            serverinfo.guestdevicesharingmaxtime = domain.guestdevicesharing.maxsessiontime;
          }
        }
      }
      if (typeof domain.userconsentflags == 'number') {
        serverinfo.consent = domain.userconsentflags;
      }
      if (typeof domain.usersessionidletimeout == 'number' && domain.usersessionidletimeout > 0) {
        serverinfo.timeout = domain.usersessionidletimeout * 60 * 1000;
      }
      ...
      if (user.siteadmin === SITERIGHT_ADMIN) {
        if (
          parent.parent.config.settings.managealldevicegroups.indexOf(user._id) >= 0 ||
          (user.links &&
            Object.keys(user.links).some(
              (key) => parent.parent.config.settings.managealldevicegroups.indexOf(key) >= 0
            ))
        ) {
          serverinfo.manageAllDeviceGroups = true;
        }
        if (obj.crossDomain === true) {
          serverinfo.crossDomain = [];
          for (var i in parent.parent.config.domains) {
            serverinfo.crossDomain.push(i);
          }
        }
        if (typeof parent.webCertificateExpire[domain.id] == 'number') {
          serverinfo.certExpire = parent.webCertificateExpire[domain.id];
        }
      }
      if (typeof domain.terminal == 'object') {
        // Settings used for remote terminal feature
        if (typeof domain.terminal.linuxshell == 'string' && domain.terminal.linuxshell != 'any') {
          serverinfo.linuxshell = domain.terminal.linuxshell;
        }
      }
      if (Array.isArray(domain.preconfiguredremoteinput)) {
      ...
      if (Array.isArray(domain.preconfiguredscripts)) {
        ...
        for (var i in domain.preconfiguredscripts) {
          ...
          if (
            ...
            script.name.length <= 32 &&
            typeof script.type == 'string' &&
            (typeof script.file == 'string' || typeof script.cmd == 'string')
          ) {
      ...
      if (domain.maxdeviceview != null) {
        serverinfo.maxdeviceview = domain.maxdeviceview;
      } // Maximum number of devices a user can view at any given time
      ...
      try {
        ws.send(JSON.stringify({ action: 'userinfo', userinfo: parent.CloneSafeUser(parent.users[user._id]) }));
      } catch (ex) {}
      ...
      if (user.siteadmin === SITERIGHT_ADMIN) {
        ...
        if (domain.myserver !== false && (domain.myserver == null || domain.myserver.trace === true)) {
          ...
          try {
            ws.send(JSON.stringify({ action: 'traceinfo', traceSources: parent.parent.debugRemoteSources }));
          } catch (ex) {}
        }
        ...
        if (serverWarnings.length > 0) {
          try {
            ws.send(JSON.stringify({ action: 'serverwarnings', warnings: serverWarnings }));
          } catch (ex) {}
        }
      }
            ...
            try {
              ws.send(
                JSON.stringify({
                  ...
                  tag: 'ServerNotify',
                  id: Math.random(),
                  value: 'There has been ' + count + ' failed login attempts on this account since the last login.',
                  titleid: 3,
                  msgid: 12,
                  args: [count]
                })
      ...
      if (
        ...
        domain.id == '' &&
        typeof parent.parent.config.settings.autobackup == 'object' &&
        typeof parent.parent.config.settings.autobackup.googledrive == 'object'
      ) {
        db.Get('GoogleDriveBackup', function (err, docs) {
          if (err != null) return;
          if (docs.length == 0) {
            try {
              ws.send(JSON.stringify({ action: 'serverBackup', service: 'googleDrive', state: 1 }));
            } catch (ex) {}
          } else {
            try {
              ws.send(JSON.stringify({ action: 'serverBackup', service: 'googleDrive', state: docs[0].state }));
            } catch (ex) {}
          }
        });
      }
  ...
  function processWebSocketData(msg) {
    ...
    if (commandHandler != null) {
      try {
        ...
        return; // todo: remove returns when switch is gone
      }
    } else {
    }
    ...
    switch (command.action) {
      case 'nodes': {
        ...
        if (domain.maxdeviceview != null) {
          if (typeof command.skip == 'number' && command.skip >= 0) {
            obj.deviceSkip = command.skip;
          }
          if (typeof command.limit == 'number' && command.limit > 0) {
            obj.deviceLimit = command.limit;
          }
          if (obj.deviceLimit > domain.maxdeviceview) {
            obj.deviceLimit = domain.maxdeviceview;
          }
        }
          ...
          for (var i in parent.meshes) {
            ...
            if (m.mtype == 2 && m.name == command.meshname && parent.IsMeshViewable(user, m)) {
              if (command.meshid == null) {
                command.meshid = m._id;
              } else {
                err = 'Duplicate device groups found';
              }
            }
          }
          if (command.meshid == null) {
        ...
        if (err == null) {
          try {
            if (command.meshid == null) {
              ...
              if (common.validateString(meshid, 0, 128) == false) {
                err = 'Invalid group id';
              } else {
                if (meshid.split('/').length == 1) {
                  meshid = 'mesh/' + domain.id + '/' + command.meshid;
                }
                if (parent.IsMeshViewable(user, meshid)) {
                  links.push(meshid);
                } else {
                  err = 'Invalid group id';
                }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'nodes', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        db.GetAllTypeNoTypeFieldMeshFiltered(
          ...
          function (err, docs) {
            ...
            for (i in docs) {
              ...
              if (domain.maxdeviceview != null) {
                obj.visibleDevices[docs[i]._id] = 1;
              }
              ...
              if (state) {
                ...
                if ((state.connectivity & 1) != 0) {
                  var agent = parent.wsagents[docs[i]._id];
                  if (agent != null) {
                    docs[i].agct = agent.connectTime;
                  }
                }
                ...
                if ((state.connectivity & 2) != 0) {
                  ...
                  if (ciraConnection != null && ciraConnection.tag != null) {
                    docs[i].cict = ciraConnection.tag.connectTime;
                  }
                }
              }
              ...
              if (docs[i].ssh != null) {
                if (docs[i].ssh[user._id] != null && docs[i].ssh[user._id].u) {
                  if (docs[i].ssh.k && docs[i].ssh[user._id].kp) {
                    docs[i].ssh = 2;
                  } // Username, key and password
                  else if (docs[i].ssh[user._id].k) {
                    docs[i].ssh = 3;
                  } // Username and key. No password.
                  else if (docs[i].ssh[user._id].p) {
                    docs[i].ssh = 1;
              ...
              if (docs[i].intelamt != null) {
                if (docs[i].intelamt.pass != null) {
                  docs[i].intelamt.pass = 1;
                }
                if (docs[i].intelamt.mpspass != null) {
                  docs[i].intelamt.mpspass = 1;
                }
              }

              // If GeoLocation not enabled, remove any node location information
              if (domain.geolocation != true) {
                if (docs[i].iploc != null) {
                  delete docs[i].iploc;
                }
                if (docs[i].wifiloc != null) {
                  delete docs[i].wifiloc;
                }
                if (docs[i].gpsloc != null) {
                  delete docs[i].gpsloc;
                }
                if (docs[i].userloc != null) {
                  delete docs[i].userloc;
                }
              }

              // Add device sessions
              const xagent = parent.wsagents[docs[i]._id];
              if (xagent != null && xagent.sessions != null) {
                docs[i].sessions = xagent.sessions;
              }

              // Add IP-KVM sessions
              if (parent.parent.ipKvmManager != null) {
                const xipkvmport = parent.parent.ipKvmManager.managedPorts[docs[i]._id];
                if (xipkvmport != null && xipkvmport.sessions != null) {
                  docs[i].sessions = xipkvmport.sessions;
                }
              }
            ...
            if (domain.maxdeviceview != null) {
              ...
              if (nodeCount < response.limit) {
                ...
                if (db.CountAllTypeNoTypeFieldMeshFiltered) {
                  db.CountAllTypeNoTypeFieldMeshFiltered(
                    links,
                    extraids,
                    domain.id,
                    'node',
      ...
      case 'fileoperation': {
        ...
        if ((user.siteadmin & 8) != 0) {
          ...
          var sendUpdate = true,
            path = meshPathToRealPath(command.path, user); // This will also check access rights
          if (path == null) break;

          if (command.fileop == 'createfolder' && common.IsFilenameValid(command.newfolder) == true) {
            // Create a new folder
            try {
              fs.mkdirSync(parent.path.join(path, command.newfolder));
            } catch (ex) {
              try {
                fs.mkdirSync(path);
              } catch (ex) {}
            ...
            for (i in command.delfiles) {
              if (common.IsFilenameValid(command.delfiles[i]) == true) {
                ...
                if (command.rec == true) {
                  ...
                  } catch (ex) {} // TODO, make this an async function
                } else {
                  try {
            ...
            command.fileop == 'rename' &&
            common.IsFilenameValid(command.oldname) === true &&
            common.IsFilenameValid(command.newname) === true
          ) {
            // Rename
            try {
              fs.renameSync(parent.path.join(path, command.oldname), parent.path.join(path, command.newname));
            } catch (e) {}
            ...
            for (i in command.names) {
              if (common.IsFilenameValid(command.names[i]) === true) {
                var s = parent.path.join(scpath, command.names[i]),
                  d = parent.path.join(path, command.names[i]);
                sendUpdate = false;
                try {
                  fs.mkdirSync(path);
                } catch (ex) {} // try to create folder first incase folder is missing
                copyFile(
                  s,
                  d,
                  function (op) {
                    if (op != null) {
                      fs.unlink(op, function (err) {
                        parent.parent.DispatchEvent([user._id], obj, 'updatefiles');
                      });
                    } else {
                      parent.parent.DispatchEvent([user._id], obj, 'updatefiles');
                    }
                  },
                  command.fileop == 'move' ? s : null
                );
              }
            }
      ...
      case 'msg': {
        ...
        // Rights check
        var requiredRights = null,
          requiredNonRights = null,
          routingOptions = null;

        // Complete the nodeid if needed
        if (command.nodeid.indexOf('/') == -1) {
          command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
        }

        // Check if getting / setting clipboard data is allowed
        if (command.type == 'getclip' && domain.clipboardget == false) {
          console.log('CG-EXIT');
          break;
        }
        if (command.type == 'setclip' && domain.clipboardset == false) {
          console.log('CS-EXIT');
          break;
        }
        ...
        if (command.type == 'tunnel') {
          ...
          var url = null;
          try {
            url = require('url').parse(command.value, true);
          } catch (ex) {}
          ...
          // Check rights
          if (url.query.p == '1') {
            requiredNonRights = MESHRIGHT_NOTERMINAL;
          } else if (url.query.p == '4' || url.query.p == '5') {
            requiredNonRights = MESHRIGHT_NOFILES;
          }
          ...
          if (
            (parent.parent.config.settings.desktopmultiplex === true || domain.desktopmultiplex === true) &&
            url.query.p == '2'
          ) {
            routingOptions = { removeViewOnlyLimitation: true };
          }

          // Add server TLS cert hash
          var tlsCertHash = null;
          if (parent.parent.args.ignoreagenthashcheck == null || parent.parent.args.ignoreagenthashcheck === false) {
            ...
            if (tlsCertHash != null) {
              command.servertlshash = Buffer.from(tlsCertHash, 'binary').toString('hex');
            }
          }
          ...
          if (typeof domain.consentmessages == 'object') {
            if (typeof domain.consentmessages.title == 'string') {
              command.soptions.consentTitle = domain.consentmessages.title;
            }
            if (typeof domain.consentmessages.desktop == 'string') {
              command.soptions.consentMsgDesktop = domain.consentmessages.desktop;
            }
            if (typeof domain.consentmessages.terminal == 'string') {
              command.soptions.consentMsgTerminal = domain.consentmessages.terminal;
            }
            if (typeof domain.consentmessages.files == 'string') {
              command.soptions.consentMsgFiles = domain.consentmessages.files;
            }
            if (typeof domain.consentmessages.consenttimeout == 'number' && domain.consentmessages.consenttimeout > 0) {
              command.soptions.consentTimeout = domain.consentmessages.consenttimeout;
            }
            if (domain.consentmessages.autoacceptontimeout === true) {
              command.soptions.consentAutoAccept = true;
            }
            if (domain.consentmessages.autoacceptifnouser === true) {
              command.soptions.consentAutoAcceptIfNoUser = true;
            }
            if (domain.consentmessages.autoacceptifdesktopnouser === true) {
              command.soptions.consentAutoAcceptIfDesktopNoUser = true;
            }
            if (domain.consentmessages.autoacceptifterminalnouser === true) {
              command.soptions.consentAutoAcceptIfTerminalNoUser = true;
            }
            if (domain.consentmessages.autoacceptiffilenouser === true) {
              command.soptions.consentAautoAcceptIfFileNoUser = true;
            }
            if (domain.consentmessages.autoacceptiflocked === true) {
              command.soptions.consentAutoAcceptIfLocked = true;
            }
            if (domain.consentmessages.autoacceptifdesktoplocked === true) {
              command.soptions.consentAutoAcceptIfDesktopLocked = true;
            }
            if (domain.consentmessages.autoacceptifterminallocked === true) {
              command.soptions.consentAutoAcceptIfTerminalLocked = true;
            }
            if (domain.consentmessages.autoacceptiffilelocked === true) {
              command.soptions.consentAutoAcceptIfFileLocked = true;
            }
            if (domain.consentmessages.oldstyle === true) {
              command.soptions.oldStyle = true;
            }
          }
          if (typeof domain.notificationmessages == 'object') {
            if (typeof domain.notificationmessages.title == 'string') {
              command.soptions.notifyTitle = domain.notificationmessages.title;
            }
            if (typeof domain.notificationmessages.desktop == 'string') {
              command.soptions.notifyMsgDesktop = domain.notificationmessages.desktop;
            }
            if (typeof domain.notificationmessages.terminal == 'string') {
              command.soptions.notifyMsgTerminal = domain.notificationmessages.terminal;
            }
            if (typeof domain.notificationmessages.files == 'string') {
              command.soptions.notifyMsgFiles = domain.notificationmessages.files;
            }
          }
          ...
          if (typeof parent.parent.config.settings.agentwscompression == 'boolean') {
            command.perMessageDeflate = parent.parent.config.settings.agentwscompression;
          }
        }
      ...
      case 'events': {
        ...
        if (command.userid != null && (user.siteadmin & SITERIGHT_MANAGEUSERS) != 0) {
          ...
          if (command.filter != null) {
            if (
              ['agentlog', 'batchupload', 'changenode', 'manual', 'relaylog', 'removenode', 'runcommands'].includes(
                command.filter
              )
            )
              actionfilter = command.filter;
          }
          ...
          if (command.limit == null || typeof command.limit != 'number') {
            ...
            db.GetUserEvents(filter, domain.id, command.userid, actionfilter, function (err, docs) {
              if (err != null) return;
              try {
                ws.send(JSON.stringify({ action: 'events', events: docs, userid: command.userid, tag: command.tag }));
              } catch (ex) {}
            });
          } else {
            // Send the list of most recent events for this session, up to 'limit' count
            db.GetUserEventsWithLimit(
              filter,
              domain.id,
              command.userid,
              command.limit,
        ...
        } else if (command.nodeid != null) {
          // Device filtered events
          ...
          parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
            if (node == null) {
              try {
                ws.send(JSON.stringify({ action: 'events', events: [], nodeid: command.nodeid, tag: command.tag }));
              } catch (ex) {}
              return;
            }
            ...
            var limit = 10000;
            if (common.validateInt(command.limit, 1, 1000000) == true) {
              limit = command.limit;
            }
            ...
            if ((rights & MESHRIGHT_LIMITEVENTS) != 0 && rights != MESHRIGHT_ADMIN) {
              ...
              db.GetNodeEventsSelfWithLimit(node._id, domain.id, user._id, limit, filter, function (err, docs) {
                if (err != null) return;
                try {
                  ws.send(JSON.stringify({ action: 'events', events: docs, nodeid: node._id, tag: command.tag }));
                } catch (ex) {}
              });
            } else {
          ...
          var exGroupFilter2 = [],
            filter = [],
            filter2 = user.subscriptions;

          // Add all meshes for groups this user is part of
          // TODO (UserGroups)

          // Remove MeshID's that we do not have rights to see events for
          for (var link in obj.user.links) {
            if (
              (obj.user.links[link].rights & MESHRIGHT_LIMITEVENTS) != 0 &&
              obj.user.links[link].rights != MESHRIGHT_ADMIN
            ) {
              exGroupFilter2.push(link);
            }
          }
          for (var i in filter2) {
            if (exGroupFilter2.indexOf(filter2[i]) == -1) {
              filter.push(filter2[i]);
            }
          }
          ...
          if (command.limit == null || typeof command.limit != 'number') {
            ...
            db.GetEvents(filter, domain.id, actionfilter, function (err, docs) {
              if (err != null) return;
              try {
                ws.send(JSON.stringify({ action: 'events', events: docs, user: command.user, tag: command.tag }));
              } catch (ex) {}
            });
          } else {
            // Send the list of most recent events for this session, up to 'limit' count
      ...
      case 'recordings': {
        ...
        if (domain.sessionrecording.filepath) {
          ...
          recordingsPath = parent.parent.recordpath;
        }
        if (recordingsPath == null) return;
        fs.readdir(recordingsPath, function (err, files) {
          if (err != null) {
            try {
              ws.send(JSON.stringify({ action: 'recordings', error: 1, tag: command.tag }));
            } catch (ex) {}
            return;
          }
          if (command.limit == null || typeof command.limit != 'number') {
            ...
            db.GetEvents(['recording'], domain.id, null, function (err, docs) {
              if (err != null) {
                try {
                  ws.send(JSON.stringify({ action: 'recordings', error: 2, tag: command.tag }));
                } catch (ex) {}
                return;
              }
              for (var i in docs) {
                delete docs[i].action;
                delete docs[i].etype;
                delete docs[i].msg; // TODO: We could make a more specific query in the DB and never have these.
                if (files.indexOf(docs[i].filename) >= 0) {
                  docs[i].present = 1;
                }
              }
              try {
                ws.send(JSON.stringify({ action: 'recordings', events: docs, tag: command.tag }));
              } catch (ex) {}
            });
          } else {
      ...
      case 'wssessioncount': {
        ...
        if ((user.siteadmin & 2) == 0) {
          try {
            ws.send(JSON.stringify({ action: 'wssessioncount', wssessions: {}, tag: command.tag }));
          } catch (ex) {}
          break;
        }
        if (parent.parent.multiServer == null) {
        ...
        try {
          ws.send(JSON.stringify({ action: 'wssessioncount', wssessions: wssessions, tag: command.tag }));
        } catch (ex) {} // wssessions is: userid --> count
        break;
      }
      case 'deleteuser': {
        ...
        try {
          if ((user.siteadmin & 2) == 0) {
          ...
          } else if (common.validateString(command.userid, 1, 2048) == false) {
            err = 'Invalid userid';
          } else {
            if (command.userid.indexOf('/') < 0) {
              command.userid = 'user/' + domain.id + '/' + command.userid;
            }
            ...
            } else if (obj.crossDomain !== true && (delusersplit.length != 3 || delusersplit[1] != domain.id)) {
              err = 'Invalid domain';
            } // Invalid domain, operation only valid for current domain
            else if (deluser.siteadmin === SITERIGHT_ADMIN && user.siteadmin != SITERIGHT_ADMIN) {
              err = 'Permission denied';
            } // Need full admin to remote another administrator
            else if (
              obj.crossDomain !== true &&
              user.groups != null &&
              user.groups.length > 0 &&
              (deluser.groups == null || findOne(deluser.groups, user.groups) == false)
            ) {
        ...
        if (obj.crossDomain === true) {
          deluserdomain = parent.parent.config.domains[delusersplit[1]];
        }
        if (deluserdomain == null) {
          err = 'Invalid domain';
        }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'deleteuser', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
            ...
            if (i.startsWith('mesh/')) {
              ...
              if (mesh) {
                ...
                var event = {
                  ...
                  links: mesh.links,
                  msgid: 72,
                  msgArgs: [deluser.name, mesh.name],
                  msg: change,
                  domain: deluserdomain.id,
                  invite: mesh.invite
                };
                if (db.changeStream) {
                  event.noact = 1;
                ...
                parent.parent.DispatchEvent(
                  parent.CreateMeshDispatchTargets(mesh, [deluser._id, user._id]),
                  obj,
                  event
                );
              }
              ...
              parent.GetNodeWithRights(deluserdomain, deluser, i, function (node, rights, visible) {
                ...
                if (command.rights == 0) {
                  event = {
                    ...
                    node: parent.CloneSafeNode(node)
                  };
                } else {
                  event = {
                    etype: 'node',
                    ...
                    msgid: 61,
                    msgArgs: [node.name],
                    msg: 'Changed user device rights for ' + node.name,
                    node: parent.CloneSafeNode(node)
                  };
                }
                if (db.changeStream) {
              ...
              if (ugroup) {
                ...
                var event = {
                  ...
                  links: ugroup.links,
                  msgid: 62,
                  msgArgs: [deluser.name, ugroup.name],
                  msg: 'Removed user ' + deluser.name + ' from user group ' + ugroup.name,
                  addUserDomain: deluserdomain.id
                };
                if (db.changeStream) {
                  event.noact = 1;
        ...
        parent.parent.db.GetAllTypeNodeFiltered(
          ...
          function (err, docs) {
            if (err == null && docs != null) {
              for (var i = 0; i < docs.length; i++) {
                parent.parent.db.Remove(docs[i]._id, function () {});
              }
            }
          }
        );
        ...
        try {
          ...
          if (deluserpath != null) {
            parent.deleteFolderRec(deluserpath);
          }
        } catch (e) {}
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'deleteuser', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }

        // Log in the auth log
        if (parent.parent.authlog) {
          parent.parent.authLog('https', 'User ' + user.name + ' deleted user account ' + deluser.name);
        }
      ...
      case 'userbroadcast': {
        ...
        try {
          ...
          if ((user.siteadmin & 2) == 0) {
            err = 'Permission denied';
          } else if (common.validateString(command.msg, 1, 512) == false) {
            err = 'Message is too long';
          } // Notification message is between 1 and 256 characters
        } catch (ex) {
          err = 'Validation exception: ' + ex;
        }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'userbroadcast', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (typeof command.maxtime == 'number' && command.maxtime > 0) {
          notification.maxtime = command.maxtime;
        }
        ...
        for (var i in parent.wssessions2) {
          try {
            if (parent.wssessions2[i].domainid == domain.id) {
              ...
              if (
                command.userid != null &&
                command.userid != sessionUser._id &&
                command.userid != sessionUser._id.split('/')[2]
              ) {
                continue;
              }
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'userbroadcast', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'edituser': {
        ...
        if (typeof command.username == 'string' && command.userid == null) {
          command.userid = command.username;
        }
        if (typeof command.id == 'string' && command.userid == null) {
        ...
        try {
          if ((user.siteadmin & 2) == 0) {
            ...
            if (edituser == null) {
              err = 'User does not exists';
            } else if (obj.crossDomain !== true && (editusersplit.length != 3 || editusersplit[1] != domain.id)) {
              err = 'Invalid domain';
            } // Invalid domain, operation only valid for current domain
            else if (edituser.siteadmin === SITERIGHT_ADMIN && user.siteadmin != SITERIGHT_ADMIN) {
              err = 'Permission denied';
            } // Need full admin to remote another administrator
            else if (
              obj.crossDomain !== true &&
              user.groups != null &&
              user.groups.length > 0 &&
              (edituser.groups == null || findOne(edituser.groups, user.groups) == false)
            ) {
              err = 'Invalid user group';
            } // Can only perform this operation on other users of our group.
          }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'edituser', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (chguser) {
          ...
          if (user.siteadmin != SITERIGHT_ADMIN) {
            if (
              user.groups != null &&
              user.groups.length > 0 &&
              (chguser.groups == null || findOne(chguser.groups, user.groups) == false)
            )
              return;
          }
          ...
          if (edituserdomain.usernameisemail !== true) {
            if (common.validateString(command.email, 0, 1024) && chguser.email != command.email) {
              if (command.email == '') {
                ...
                delete chguser.email;
              } else {
                chguser.email = command.email.toLowerCase();
              }
              change = 1;
            }
          }
          ...
          if (common.validateString(command.realname, 0, 256) && chguser.realname != command.realname) {
            if (command.realname == '') {
              delete chguser.realname;
            } else {
              chguser.realname = command.realname;
            }
            change = 1;
          }
          ...
          // Make changes
          if (
            (command.emailVerified === true || command.emailVerified === false) &&
            chguser.emailVerified != command.emailVerified
          ) {
            chguser.emailVerified = command.emailVerified;
            change = 1;
          }
          if ((common.validateInt(command.quota, 0) || command.quota == null) && command.quota != chguser.quota) {
            chguser.quota = command.quota;
            if (chguser.quota == null) {
              delete chguser.quota;
            }
            change = 1;
          }
          if (command.resetNextLogin === true) {
            chguser.passchange = -1;
          }
          if (command.consent != null && typeof command.consent == 'number') {
            if (command.consent == 0) {
              delete chguser.consent;
            } else {
              chguser.consent = command.consent;
            }
            change = 1;
          }
          if (
            ...
            (command.phone == '' || isPhoneNumber(command.phone))
          ) {
            if (command.phone == '') {
              delete chguser.phone;
            } else {
              chguser.phone = command.phone;
            }
            change = 1;
          }
          if (command.msghandle != null && typeof command.msghandle == 'string') {
            if (command.msghandle.startsWith('callmebot:http')) {
              const h = parent.parent.msgserver.callmebotUrlToHandle(command.msghandle.substring(10));
              if (h) {
                command.msghandle = h;
              } else {
                command.msghandle = '';
            ...
            if (command.msghandle == '') {
              delete chguser.msghandle;
            } else {
              chguser.msghandle = command.msghandle;
            }
            change = 1;
          }
          if (command.flags != null && typeof command.flags == 'number') {
            // Flags: 1 = Account Image, 2 = Session Recording
            if (command.flags == 0 && chguser.flags != null) {
              delete chguser.flags;
              change = 1;
            } else {
              if (command.flags !== chguser.flags) {
                chguser.flags = command.flags;
                change = 1;
              }
            }
          }
          if (command.removeRights != null && typeof command.removeRights == 'number') {
            if (command.removeRights == 0) {
              if (chguser.removeRights != null) {
                delete chguser.removeRights;
                change = 1;
              }
            } else {
              if (command.removeRights !== chguser.removeRights) {
                chguser.removeRights = command.removeRights;
                change = 1;
              }
            }
          }
          ...
          if (
            common.validateInt(command.siteadmin) &&
            chguser._id !== user._id &&
            chguser.siteadmin != command.siteadmin
          ) {
            // We can't change our own siteadmin permissions.
            ...
            if (user.siteadmin === SITERIGHT_ADMIN) {
              ...
              if ((user.siteadmin & 256) != 0) {
                mask -= 256;
              } // Mask: Manage User Groups
              if ((user.siteadmin & 512) != 0) {
                mask -= 512;
              } // Mask: Manage Recordings
              if (((chgusersiteadmin ^ command.siteadmin) & mask) == 0) {
                chguser.siteadmin = command.siteadmin;
                change = 1;
              }
            }
          ...
          if (Array.isArray(command.groups) && (user._id != command.id || user.siteadmin === SITERIGHT_ADMIN)) {
            if (command.groups.length == 0) {
              ...
              if (chguser.groups != null) {
                delete chguser.groups;
                change = 1;
              }
            } else {
              ...
              for (var i in command.groups) {
                if (typeof command.groups[i] == 'string') {
                  ...
                  if (gname.length > 0 && gname.length <= 64 && groups2.indexOf(gname) == -1) {
                    groups2.push(gname);
                  }
                }
              }
              ...
              if (chguser.groups != groups2) {
                chguser.groups = groups2;
                change = 1;
              }
              ...
              for (var i in groups2) {
                if (allTargetGroups.indexOf(i) == -1) {
                  allTargetGroups.push(i);
                }
              }
            }
          }
          ...
          if (change == 1) {
            ...
            var event = {
              ...
              userid: user._id,
              username: user.name,
              account: parent.CloneSafeUser(chguser),
              action: 'accountchange',
              msgid: 66,
              msgArgs: [chguser.name],
              msg: 'Account changed: ' + chguser.name,
              domain: edituserdomain.id
            };
            if (db.changeStream) {
              event.noact = 1;
            } // If DB change stream is active, don't use this event to change the user. Another event will come.
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'edituser', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'usergroups': {
        ...
        if ((user.siteadmin & SITERIGHT_USERGROUPS) == 0) {
          ...
          var groups = {},
            groupCount = 0;
          for (var i in parent.userGroups) {
            if (parent.userGroups[i].domain == domain.id) {
              groupCount++;
              groups[i] = { name: parent.userGroups[i].name };
            }
          }
          try {
            ws.send(JSON.stringify({ action: 'usergroups', ugroups: groupCount ? groups : null, tag: command.tag }));
          } catch (ex) {}
        } else {
          ...
          var groups = {},
            groupCount = 0;
          for (var i in parent.userGroups) {
            if (obj.crossDomain == true || parent.userGroups[i].domain == domain.id) {
              groupCount++;
              groups[i] = parent.userGroups[i];
            }
          }
          try {
            ws.send(JSON.stringify({ action: 'usergroups', ugroups: groupCount ? groups : null, tag: command.tag }));
      ...
      case 'createusergroup': {
        ...
        try {
          ...
          if (
            ...
            (domain.authstrategies['oidc'].groups.sync == true ||
              (typeof domain.authstrategies['oidc'].groups.sync == 'object' &&
                domain.authstrategies['oidc'].groups.sync.enabled == true))
          ) {
          ...
          else if (common.validateString(command.name, 1, 64) == false) {
            err = 'Invalid group name';
          } // User group name is between 1 and 64 characters
          else if (command.desc != null && common.validateString(command.desc, 0, 1024) == false) {
            err = 'Invalid group description';
          } // User group description is between 0 and 1024 characters

          // If we are cloning from an existing user group, check that.
          if (command.clone) {
            if (common.validateString(command.clone, 1, 256) == false) {
              err = 'Invalid clone groupid';
            } else {
              var clonesplit = command.clone.split('/');
              if (
                clonesplit.length != 3 ||
                clonesplit[0] != 'ugrp' ||
                (command.domain == null && clonesplit[1] != domain.id)
              ) {
                err = 'Invalid clone groupid';
              } else if (parent.userGroups[command.clone] == null) {
                err = 'Invalid clone groupid';
              }
            }
            ...
            // Get new user group domain
            ugrpdomain = domain;
            if (obj.crossDomain === true && command.domain != null) {
              ugrpdomain = parent.parent.config.domains[command.domain];
            }
            if (ugrpdomain == null) {
              err = 'Invalid domain';
            }
          ...
          if (
            err == null &&
            domain.mailserver != null &&
            ugrpdomain.auth != 'sspi' &&
            ugrpdomain.auth != 'ldap' &&
            user.emailVerified !== true &&
            user.siteadmin != SITERIGHT_ADMIN
          ) {
            err = 'Email verification required';
          } // User must verify it's email first.
        } catch (ex) {
          err = 'Validation exception: ' + ex;
        }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'createusergroup', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.crypto.randomBytes(48, function (err, buf) {
                ...
                if (i.startsWith('user/')) {
                  ...
                  if (xuser != null && xuser.links != null) {
                    ...
                    var event = {
                      ...
                      userid: user._id,
                      username: user.name,
                      account: parent.CloneSafeUser(xuser),
                      action: 'accountchange',
                      msgid: 67,
                      msgArgs: [xuser.name],
                      msg: 'User group membership changed: ' + xuser.name,
                      domain: ugrpdomain.id
                    };
                    if (db.changeStream) {
                      event.noact = 1;
                  ...
                  if (xmesh && xmesh.links) {
                    ...
                    var event = {
                      ...
                      links: xmesh.links,
                      msgid: 68,
                      msgArgs: [ugrp.name, xmesh.name],
                      msg: 'Added user group ' + ugrp.name + ' to device group ' + xmesh.name,
                      domain: ugrpdomain.id,
                      invite: xmesh.invite
                    };
                    if (db.changeStream) {
          ...
          for (var i in pendingDispatchEvents) {
            var ev = pendingDispatchEvents[i];
            parent.parent.DispatchEvent(ev[0], ev[1], ev[2]);
          }

          // Log in the auth log
          if (parent.parent.authlog) {
            parent.parent.authLog('https', 'User ' + user.name + ' created user group ' + ugrp.name);
          }
      ...
      case 'deleteusergroup': {
        ...
        else if (common.validateString(command.ugrpid, 1, 1024) == false) {
          err = 'Invalid group id';
        } // Check the user group id
        else {
          var ugroupidsplit = command.ugrpid.split('/');
          if (
            ugroupidsplit.length != 3 ||
            ugroupidsplit[0] != 'ugrp' ||
            (obj.crossDomain !== true && ugroupidsplit[1] != domain.id)
          ) {
            err = 'Invalid domain id';
          }
        }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'deleteusergroup', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        db.Get(command.ugrpid, function (err, groups) {
              ...
              if (i.startsWith('user/')) {
                ...
                if (xmesh && xmesh.links) {
                  ...
                  var event = {
                    ...
                    links: xmesh.links,
                    msgid: 70,
                    msgArgs: [group.name, xmesh.name],
                    msg: 'Removed user group ' + group.name + ' from device group ' + xmesh.name,
                    domain: delGroupDomain.id
                  };
                  if (db.changeStream) {
                    event.noact = 1;
                  } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
          ...
          db.Remove(group._id);
          if (db.changeStream == false) {
            delete parent.userGroups[group._id];
          }
          ...
          // Log in the auth log
          if (parent.parent.authlog) {
            parent.parent.authLog('https', 'User ' + user.name + ' deleted user group ' + group.name);
          }
      ...
      case 'editusergroup': {
        ...
        if (group != null) {
          ...
          if (
            group.membershipType == null &&
            common.validateString(command.name, 1, 64) == true &&
            command.name != group.name
          ) {
            change = 'User group name changed from "' + group.name + '" to "' + command.name + '"';
            group.name = command.name;
          }
          if (common.validateString(command.desc, 0, 1024) == true && command.desc != group.desc) {
            if (change != '') change += ' and description changed';
            else change += 'User group "' + group.name + '" description changed';
            group.desc = command.desc;
          }
          if (typeof command.consent == 'number' && command.consent != group.consent) {
            if (change != '') change += ' and consent changed';
            else change += 'User group "' + group.name + '" consent changed';
            group.consent = command.consent;
          }

          if (command.flags != null && typeof command.flags == 'number') {
            // Flags: 2 = Session Recording
            if (command.flags == 0 && group.flags != null) {
              delete group.flags;
            } else {
              if (command.flags !== group.flags) {
                group.flags = command.flags;
              }
            }
            if (change == '') {
              change = 'User group features changed.';
            }
          }
          ...
          if (change != '') {
            ...
            var event = {
              ...
              name: group.name,
              desc: group.desc,
              consent: group.consent == null ? 0 : group.consent,
              action: 'usergroupchange',
              links: group.links,
              flags: group.flags,
              msg: change,
      ...
      case 'changemeshnotify': {
        ...
        try {
          ...
          if (common.validateString(command.meshid, 8, 134) == false) {
            err = 'Invalid group identifier';
          } // Check the meshid
          else if (command.meshid.indexOf('/') == -1) {
            command.meshid = 'mesh/' + domain.id + '/' + command.meshid;
          }
          if (common.validateInt(command.notify) == false) {
            err = 'Invalid notification flags';
          }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'changemeshnotify', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (command.notify == 0 && user.notify != null && user.notify[command.meshid] != null) {
          delete user.notify[command.meshid];
        }
        if (command.notify != 0 && user.links[command.meshid] == null) {
          if (user.notify == null) {
            user.notify = {};
          }
          user.notify[command.meshid] = command.notify;
        }
        ...
        var event = {
          ...
          action: 'accountchange',
          msgid: 73,
          msg: 'Device group notification changed',
          domain: domain.id
        };
        if (db.changeStream) {
          event.noact = 1;
      ...
      case 'changeusernotify': {
        ...
        try {
          // Change the current user's notification flags for a meshid
          if (common.validateString(command.nodeid, 1, 1024) == false) {
            err = 'Invalid device identifier';
          } // Check the meshid
          else if (command.nodeid.indexOf('/') == -1) {
            command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
          }
          if (common.validateInt(command.notify) == false) {
            err = 'Invalid notification flags';
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'changeusernotify', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (command.notify == 0) {
          delete user.notify[command.nodeid];
        } else {
          user.notify[command.nodeid] = command.notify;
        }
        if (Object.keys(user.notify).length == 0) {
          delete user.notify;
        }
        ...
        var event = {
          ...
          action: 'accountchange',
          msgid: 130,
          msg: 'User notifications changed',
          domain: domain.id
        };
        if (db.changeStream) {
          event.noact = 1;
                  ...
                  function (err, salt, hash, tag) {
                    if (err) {
                      ...
                      if (domain.passwordrequirements != null) {
                        ...
                        if (domain.passwordrequirements.hint === true && command.hint != null) {
                      ...
                      // Log in the auth log
                      if (parent.parent.authlog) {
                        parent.parent.authLog('https', 'User ' + user.name + ' changed this password');
                      }
                    }
                  },
                  0
      ...
      case 'changeuserpass': {
        ...
        if (command.pass != '' && common.checkPasswordRequirements(command.pass, domain.passwordrequirements) == false)
          break; // Password does not meet requirements
            ...
            function (err, salt, hash, tag) {
              if (!err) {
                ...
                var event = {
                  ...
                  action: 'accountchange',
                  msgid: 75,
                  msg: 'Changed account credentials',
                  domain: domain.id
                };
                if (db.changeStream) {
                  event.noact = 1;
                ...
                // Log in the auth log
                if (parent.parent.authlog) {
                  parent.parent.authLog(
                    'https',
                    'User ' + user.name + ' changed account password of user ' + chguser.name
                  );
                }
              } else {
      ...
      case 'notifyuser': {
        ...
        if (typeof command.url == 'string') {
          notification.url = command.url;
        }
        if (typeof command.maxtime == 'number' && command.maxtime > 0) {
          notification.maxtime = command.maxtime;
        }
        if (command.msgid == 11) {
          notification.value = 'Chat Request, Click here to accept.';
          notification.msgid = 11;
        } // Chat request
        ...
        if (sessions != null) {
          for (i in sessions) {
            try {
              sessions[i].send(JSON.stringify(notification));
            } catch (ex) {}
          }
        }
        ...
        if (parent.wssessions[chguser._id] == null && parent.sessionsCount[chguser._id] == null) {
          ...
          if (domain.title != null) {
            payload.title = domain.title;
          } else {
            payload.title = 'MeshCentral';
          }
      ...
      case 'meshmessenger': {
        ...
        if (common.validateString(command.userid, 1, 2048)) {
          ...
          var notification = {
            ...
            type: 'notify',
            id: Math.random(),
            value: 'Chat Request, Click here to accept.',
            title: user.name,
            userid: user._id,
            username: user.name,
            tag: 'meshmessenger/' + encodeURIComponent(command.userid) + '/' + encodeURIComponent(user._id),
            msgid: 11
          };
          ...
          if (parent.wssessions[chguser._id] == null && parent.sessionsCount[chguser._id] == null) {
            ...
            var url =
              'https://' +
              parent.getWebServerName(domain, req) +
              ':' +
              httpsPort +
              '/' +
              xdomain +
              'messenger?id=meshmessenger/' +
              encodeURIComponent(command.userid) +
              '/' +
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          var url =
            ...
            xdomain +
            'messenger?id=meshmessenger/' +
            encodeURIComponent(command.nodeid) +
            '/' +
            encodeURIComponent(user._id);
      ...
      case 'createmesh': {
        ...
        try {
          ...
          if (typeof command.meshtype == 'string') {
            command.meshtype = parseInt(command.meshtype);
          }

          // Check if we have new group restriction
          if (user.siteadmin != SITERIGHT_ADMIN && (user.siteadmin & 64) != 0) {
            err = 'Permission denied';
          }
          ...
          // Create mesh
          else if (common.validateString(command.meshname, 1, 128) == false) {
            err = 'Invalid group name';
          } // Meshname is between 1 and 128 characters
          else if (command.desc != null && common.validateString(command.desc, 0, 1024) == false) {
            err = 'Invalid group description';
          } // Mesh description is between 0 and 1024 characters
          else if (command.meshtype < 1 || command.meshtype > 4) {
            err = 'Invalid group type';
          } // Device group types are 1 = AMT, 2 = Agent, 3 = Local
          else if (
            (command.meshtype == 3 || command.meshtype == 4) &&
            parent.args.wanonly == true &&
            typeof command.relayid != 'string'
          ) {
            err = 'Invalid group type';
          } // Local device group type wihtout relay is not allowed in WAN mode
          else if (
            (command.meshtype == 3 || command.meshtype == 4) &&
            parent.args.lanonly == true &&
            typeof command.relayid == 'string'
          ) {
            err = 'Invalid group type';
          } // Local device group type with relay is not allowed in WAN mode
          else if (domain.ipkvm == null && command.meshtype == 4) {
            err = 'Invalid group type';
          } // IP KVM device group type is not allowed unless enabled
          if (err == null && command.meshtype == 4) {
            if (command.kvmmodel < 1 || command.kvmmodel > 2) {
              err = 'Invalid KVM model';
            } else if (common.validateString(command.kvmhost, 1, 128) == false) {
              err = 'Invalid KVM hostname';
            } else if (common.validateString(command.kvmuser, 1, 128) == false) {
              err = 'Invalid KVM username';
            } else if (common.validateString(command.kvmpass, 1, 128) == false) {
              err = 'Invalid KVM password';
            }
          }
        } catch (ex) {
          err = 'Validation exception: ' + ex;
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'createmesh', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.crypto.randomBytes(48, function (err, buf) {
          ...
          mesh = {
            type: 'mesh',
            _id: meshid,
            name: command.meshname,
            mtype: command.meshtype,
            desc: command.desc,
            domain: domain.id,
            links: links,
            creation: Date.now(),
          ...
          if (typeof command.flags == 'number') {
            mesh.flags = command.flags;
          }
          if (typeof command.consent == 'number') {
            mesh.consent = command.consent;
          }

          // Add KVM information if needed
          if (command.meshtype == 4) {
            mesh.kvm = { model: command.kvmmodel, host: command.kvmhost, user: command.kvmuser, pass: command.kvmpass };
          }
          ...
          if (
            ...
            if (relayIdSplit[0] == 'node' && relayIdSplit[1] == domain.id) {
              mesh.relayid = command.relayid;
            }
          }
          ...
          var event = {
            ...
            meshid: meshid,
            mtype: command.meshtype,
            mesh: parent.CloneSafeMesh(mesh),
            action: 'createmesh',
            msgid: 76,
            msgArgs: [command.meshname],
            msg: 'Device group created: ' + command.meshname,
            domain: domain.id
          };
          ...
          // Log in the auth log
          if (parent.parent.authlog) {
            parent.parent.authLog('https', 'User ' + user.name + ' created device group ' + mesh.name);
          }
          ...
          if (mesh.relayid != null) {
            ...
            parent.GetNodeWithRights(domain, user, mesh.relayid, function (node, rights, visible) {
              ...
              parent.parent.DispatchEvent(
                parent.CreateNodeDispatchTargets(node.meshid, node._id, [user._id]),
                obj,
                event
              );
            });
      ...
      case 'deletemesh': {
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'deletemesh', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (mesh == null) {
          if (command.responseid != null) {
            try {
              ws.send(
                JSON.stringify({ action: 'deletemesh', responseid: command.responseid, result: 'Unknown device group' })
              );
            } catch (ex) {}
          }
          return;
        ...
        // Check if this user has rights to do this
        var err = null;
        if (parent.GetMeshRights(user, mesh) != MESHRIGHT_ADMIN) {
          err = 'Access denied';
        }
        if (command.meshid.split('/').length != 3 || command.meshid.split('/')[1] != domain.id) {
          err = 'Invalid group';
        } // Invalid domain, operation only valid for current domain
        ...
        var event = {
          ...
          msgid: 77,
          msgArgs: [command.meshname],
          msg: 'Device group deleted: ' + command.meshname,
          domain: domain.id
        };
        parent.parent.DispatchEvent(
          parent.CreateMeshDispatchTargets(command.meshid, ['server-deletemesh']),
          obj,
          event
          ...
          if (j.startsWith('user/')) {
            ...
            if (xuser && xuser.links) {
              ...
              var event = {
                ...
                msgid: 78,
                msgArgs: [xuser.name],
                msg: 'Device group membership changed: ' + xuser.name,
                domain: domain.id
              };
              if (db.changeStream) {
                event.noact = 1;
            ...
            if (xgroup && xgroup.links) {
              ...
              var event = {
                ...
                msgid: 79,
                msgArgs: [xgroup.name],
                msg: 'User group changed: ' + xgroup.name,
                domain: domain.id
              };
              if (db.changeStream) {
                event.noact = 1;
        ...
        try {
          ...
          if (meshpath != null) {
            parent.deleteFolderRec(meshpath);
          }
        } catch (e) {}
        ...
        if (mesh.relayid != null) {
          ...
          parent.GetNodeWithRights(domain, user, mesh.relayid, function (node, rights, visible) {
            ...
            var event = {
              ...
              nodeid: node._id,
              domain: domain.id,
              msg: 'No longer a relay for ' + mesh.name + '.',
              msgid: 152,
              msgArgs: [mesh.name],
              node: parent.CloneSafeNode(node)
            };
        ...
        // Log in the auth log
        if (parent.parent.authlog) {
          parent.parent.authLog('https', 'User ' + user.name + ' deleted device group ' + mesh.name);
        }

        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'deletemesh', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'editmesh': {
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'editmesh', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        var changesids = [];
        if (common.validateString(command.meshname, 1, 128) == true && command.meshname != mesh.name) {
          change = 'Device group name changed from "' + mesh.name + '" to "' + command.meshname + '"';
          changesids.push(1);
          mesh.name = command.meshname;
        }
        if (common.validateString(command.desc, 0, 1024) == true && command.desc != mesh.desc) {
          if (change != '') change += ' and description changed';
          else change += 'Device group "' + mesh.name + '" description changed';
          changesids.push(2);
          mesh.desc = command.desc;
        }
        if (common.validateInt(command.flags) == true && command.flags != mesh.flags) {
          if (change != '') change += ' and flags changed';
          else change += 'Device group "' + mesh.name + '" flags changed';
          changesids.push(3);
          mesh.flags = command.flags;
        }
        if (common.validateInt(command.consent) == true && command.consent != mesh.consent) {
          if (change != '') change += ' and consent changed';
          else change += 'Device group "' + mesh.name + '" consent changed';
          changesids.push(4);
          mesh.consent = command.consent;
        }
        if (common.validateInt(command.expireDevs, 0, 2000) == true && command.expireDevs != mesh.expireDevs) {
          if (change != '') change += ' and auto-remove changed';
          else change += 'Device group "' + mesh.name + '" auto-remove changed';
          changesids.push(5);
          if (command.expireDevs == 0) {
            delete mesh.expireDevs;
          } else {
            mesh.expireDevs = command.expireDevs;
          }
        }

        var oldRelayNodeId = null,
          newRelayNodeId = null;
        if (
          typeof command.relayid == 'string' &&
          (mesh.mtype == 3 || mesh.mtype == 4) &&
          mesh.relayid != null &&
          command.relayid != mesh.relayid
        ) {
          var relayIdSplit = command.relayid.split('/');
          if (relayIdSplit.length == 3 && (relayIdSplit[0] = 'node') && relayIdSplit[1] == domain.id) {
            if (change != '') {
              change += ' and device relay changed';
            } else {
              change = 'Device relay changed';
            }
            changesids.push(7);
        ...
        if (mesh.mtype == 2) {
          if (command.invite === '*') {
            ...
            if (change != '') {
              change += ' and invite code changed';
            } else {
              change += 'Device group "' + mesh.name + '" invite code changed';
            }
            changesids.push(6);
          } else if (
            typeof command.invite == 'object' &&
            Array.isArray(command.invite.codes) &&
            typeof command.invite.flags == 'number'
          ) {
            // Set invite codes
            if (
              mesh.invite == null ||
              mesh.invite.codes != command.invite.codes ||
              mesh.invite.flags != command.invite.flags
            ) {
              ...
              for (var i in command.invite.codes) {
                for (var j in parent.meshes) {
                  if (
                    j != command.meshid &&
                    parent.meshes[j].deleted == null &&
                    parent.meshes[j].domain == domain.id &&
                    parent.meshes[j].invite != null &&
                    parent.meshes[j].invite.codes.indexOf(command.invite.codes[i]) >= 0
                  ) {
                    dup = command.invite.codes[i];
                    break;
                  }
                }
              }
              if (dup != null) {
                ...
                displayNotificationMessage(
                  'Error, invite code "' + dup + '" already in use.',
                  'Invite Codes',
                  null,
                  6,
                  22,
              ...
              if (typeof command.invite.ag == 'number') {
                mesh.invite.ag = command.invite.ag;
              }
              if (change != '') {
                change += ' and invite code changed';
              } else {
        ...
        if (change != '') {
          ...
          var event = {
            ...
            links: mesh.links,
            msgid: 142,
            msgArgs: [mesh.name, changesids],
            msg: change,
            domain: domain.id,
            invite: mesh.invite,
            expireDevs: command.expireDevs,
            relayid: mesh.relayid
          };
          if (db.changeStream) {
            event.noact = 1;
          } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
          parent.parent.DispatchEvent(
            parent.CreateMeshDispatchTargets(mesh, [user._id, 'server-editmesh']),
            obj,
            event
          );
        }
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'editmesh', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'removemeshuser': {
        ...
        try {
          if (common.validateString(command.userid, 1, 1024) == false) {
            err = 'Invalid userid';
          } // Check userid
          if (common.validateString(command.meshid, 8, 134) == false) {
            err = 'Invalid groupid';
          } // Check meshid
          if (command.userid.indexOf('/') == -1) {
            command.userid = 'user/' + domain.id + '/' + command.userid;
          }
          if (command.userid == obj.user._id) {
            err = "Can't remove self";
          } // Can't add of modify self
          if (
            command.userid.split('/').length != 3 ||
            (obj.crossDomain !== true && command.userid.split('/')[1] != domain.id)
          ) {
            err = 'Invalid userid';
          } // Invalid domain, operation only valid for current domain
          else {
            if (command.meshid.indexOf('/') == -1) {
              command.meshid = 'mesh/' + domain.id + '/' + command.meshid;
            }
            mesh = parent.meshes[command.meshid];
            var meshIdSplit = command.meshid.split('/');
            if (mesh == null) {
              err = 'Unknown device group';
            } else if ((parent.GetMeshRights(user, mesh) & MESHRIGHT_MANAGEUSERS) == 0) {
              err = 'Permission denied';
            } else if (meshIdSplit.length != 3) {
              err = 'Invalid domain';
            } // Invalid domain, operation only valid for current domain
            else {
              xdomain = domain;
              if (obj.crossDomain !== true) {
                xdomain = parent.parent.config.domains[meshIdSplit[1]];
              }
              if (xdomain == null) {
                err = 'Invalid domain';
              }
        ...
        if (err != null) {
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'removemeshuser', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        var deluserid = command.userid,
          deluser = null;
        if (deluserid.startsWith('user/')) {
          deluser = parent.users[deluserid];
        } else if (deluserid.startsWith('ugrp/')) {
          deluser = parent.userGroups[deluserid];
        }
        ...
        if (deluser == null && deluserid.startsWith('user/' + xdomain.id + '/*\\') == true) {
          ...
          for (var i in parent.users) {
            if (i.endsWith(search) && parent.users[i].domain == xdomain.id) {
              deluser = parent.users[i];
              command.userid = deluserid = deluser._id;
              break;
            }
          }
        }
          ...
          if (deluser.links != null && deluser.links[command.meshid] != null) {
            ...
            if (deluserid.startsWith('user/')) {
              ...
              var event = {
                ...
                userid: user._id,
                username: user.name,
                account: parent.CloneSafeUser(deluser),
                action: 'accountchange',
                msgid: 78,
                msgArgs: [deluser.name],
                msg: 'Device group membership changed: ' + deluser.name,
                domain: xdomain.id
              };
              if (db.changeStream) {
                event.noact = 1;
              ...
              var event = {
                ...
                msgid: 79,
                msgArgs: [deluser.name],
                msg: 'User group changed: ' + deluser.name,
                domain: xdomain.id
              };
              if (db.changeStream) {
                event.noact = 1;
        ...
        if (mesh.links[command.userid] != null) {
          ...
          if (deluser != null) {
            event = {
              ...
              msgid: 83,
              msgArgs: [deluser.name, mesh.name],
              msg: 'Removed user ' + deluser.name + ' from device group ' + mesh.name,
              domain: xdomain.id,
              invite: mesh.invite
            };
          } else {
            event = {
              ...
              links: mesh.links,
              msgid: 83,
              msgArgs: [deluserid.split('/')[2], mesh.name],
              msg: 'Removed user ' + deluserid.split('/')[2] + ' from device group ' + mesh.name,
              domain: xdomain.id,
              invite: mesh.invite
            };
          }
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'removemeshuser', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          }
        } else {
          if (command.responseid != null) {
      ...
      case 'meshamtpolicy': {
        ...
        if (command.amtpolicy.type === 2) {
          if (command.amtpolicy.password != null && common.validateString(command.amtpolicy.password, 0, 32) == false)
            break; // Check the amtpolicy.password
        ...
        if (mesh) {
          ...
          if (command.amtpolicy.type === 2 || command.amtpolicy.type === 3) {
            amtpolicy = {
              type: command.amtpolicy.type,
              badpass: command.amtpolicy.badpass,
              cirasetup: command.amtpolicy.cirasetup
            };
            if (command.amtpolicy.type === 3) {
              amtpolicy.ccm = command.amtpolicy.ccm;
            }
            if (command.amtpolicy.password == null && mesh.amt != null && typeof mesh.amt.password == 'string') {
              amtpolicy.password = mesh.amt.password;
            } // Keep the last password
            if (typeof command.amtpolicy.password == 'string' && command.amtpolicy.password.length >= 8) {
              amtpolicy.password = command.amtpolicy.password;
            } // Set a new password
          }
          mesh.amt = amtpolicy;
          db.Set(mesh);
          var amtpolicy2 = Object.assign({}, amtpolicy); // Shallow clone
          if (amtpolicy2.password != null) {
            amtpolicy2.password = 1;
          }
          var event = {
            etype: 'mesh',
            userid: user._id,
          ...
          if (parent.parent.multiServer != null) {
            parent.parent.multiServer.DispatchMessage({
              action: 'newIntelAmtPolicy',
              meshid: command.meshid,
              amtpolicy: amtpolicy
            });
          }
          ...
          for (var nodeid in parent.wsagents) {
            ...
            if (agent.dbMeshKey == command.meshid) {
              agent.sendUpdatedIntelAmtPolicy(amtpolicy);
            }
          }
        }
        break;
      ...
      case 'addlocaldevice': {
        ...
        try {
          if (common.validateString(command.meshid, 8, 134) == false) {
            err = 'Invalid device group id';
          } // Check meshid
          if (common.validateString(command.devicename, 1, 256) == false) {
            err = 'Invalid devicename';
          } // Check device name
          if (common.validateString(command.hostname, 1, 256) == false) {
            err = 'Invalid hostname';
          } // Check hostname
          if (typeof command.type != 'number') {
            err = 'Invalid type';
          } // Type must be a number
          if (command.type != 4 && command.type != 6 && command.type != 29) {
            err = 'Invalid type';
          } // Check device type
          else {
            if (command.meshid.indexOf('/') == -1) {
            ...
            if (mesh.mtype != 3) {
              err = 'Local device agentless mesh only allowed';
            } // This operation is only allowed for mesh type 3, local device agentless mesh.
            else if ((parent.GetMeshRights(user, mesh) & MESHRIGHT_MANAGECOMPUTERS) == 0) {
              err = 'Permission denied';
            } else if (command.meshid.split('/').length != 3 || command.meshid.split('/')[1] != domain.id) {
              err = 'Invalid domain';
            } // Invalid domain, operation only valid for current domain
          }
        } catch (ex) {
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'changeDeviceMesh', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.crypto.randomBytes(48, function (err, buf) {
          ...
          var device = {
            ...
            domain: domain.id,
            agent: { id: command.type, caps: 0 }
          };
          db.Set(device);

          // Event the new node
          parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(command.meshid, [nodeid]), obj, {
            etype: 'node',
            userid: user._id,
            username: user.name,
            action: 'addnode',
            ...
            msgid: 84,
            msgArgs: [command.devicename, mesh.name],
            msg: 'Added device ' + command.devicename + ' to device group ' + mesh.name,
            domain: domain.id
          });
          // Send response if required
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'addlocaldevice', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          }
        });
        break;
      ...
      case 'addamtdevice': {
        ...
        try {
          ...
          if (common.validateString(command.amtusername, 0, 16) == false) {
            err = 'Invalid amtusername';
          } // Check username
          if (common.validateString(command.amtpassword, 0, 16) == false) {
            err = 'Invalid amtpassword';
          } // Check password
          if (command.amttls == '0') {
            command.amttls = 0;
          } else if (command.amttls == '1') {
            command.amttls = 1;
          } // Check TLS flag
          if (command.amttls != 1 && command.amttls != 0) {
            err = 'Invalid amttls';
          } else {
            if (command.meshid.indexOf('/') == -1) {
              command.meshid = 'mesh/' + domain.id + '/' + command.meshid;
            ...
            if (mesh.mtype != 1) {
              err = 'Intel AMT agentless mesh only allowed';
            } // This operation is only allowed for mesh type 1, Intel AMT agentless mesh.
            // Check if this user has rights to do this
            else if ((parent.GetMeshRights(user, mesh) & MESHRIGHT_MANAGECOMPUTERS) == 0) {
              err = 'Permission denied';
            } else if (command.meshid.split('/').length != 3 || command.meshid.split('/')[1] != domain.id) {
        ...
        if (args.wanonly == true && command.hostname) {
          delete command.hostname;
        }
        ...
        parent.crypto.randomBytes(48, function (err, buf) {
          ...
          var device = {
            ...
            host: command.hostname,
            domain: domain.id,
            intelamt: { user: command.amtusername, pass: command.amtpassword, tls: command.amttls }
          };

          // Add optional feilds
          if (common.validateInt(command.state, 0, 3)) {
            device.intelamt.state = command.state;
          }
          if (common.validateString(command.ver, 1, 16)) {
            device.intelamt.ver = command.ver;
          }
          if (common.validateString(command.hash, 1, 256)) {
            device.intelamt.hash = command.hash;
          }
          if (common.validateString(command.realm, 1, 256)) {
            device.intelamt.realm = command.realm;
          }
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'addamtdevice', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          }
        });
      ...
      case 'scanamtdevice': {
        ...
        if (parent.parent.amtScanner) {
          if (parent.parent.amtScanner.performRangeScan(user._id, command.range) == false) {
            parent.parent.DispatchEvent(['*', user._id], obj, {
              action: 'scanamtdevice',
              range: command.range,
              results: null,
              nolog: 1
            });
      ...
      case 'changeDeviceMesh': {
        ...
        // Perform input validation
        try {
          if (common.validateStrArray(command.nodeids, 1, 256) == false) {
            err = 'Invalid nodeids';
          } // Check nodeids
          if (common.validateString(command.meshid, 8, 134) == false) {
            err = 'Invalid groupid';
        ...
        for (var i = 0; i < command.nodeids.length; i++) {
          ...
          if (xnodeid.indexOf('/') == -1) {
            xnodeid = 'node/' + domain.id + '/' + xnodeid;
          }
          ...
          parent.GetNodeWithRights(domain, user, xnodeid, function (node, rights, visible) {
                ...
                try {
                  ws.send(
                    JSON.stringify({
                      action: 'changeDeviceMesh',
                      responseid: command.responseid,
                      result: 'Device already in correct group'
                    })
                  );
                } catch (ex) {}
              }
            ...
            try {
              if (parent.meshes[node.meshid].mtype != parent.meshes[command.meshid].mtype) {
                changeDeviceShareMeshIdNodeCount--;
                return;
              }
            } catch (e) {
                ...
                try {
                  ws.send(
                    JSON.stringify({
                      action: 'changeDeviceMesh',
                      responseid: command.responseid,
                      result: 'Device groups are of different types'
                    })
                  );
                } catch (ex) {}
              }
            ...
            changeDeviceShareMeshIdNodeCount--;
            if (changeDeviceShareMeshIdNodeCount == 0) {
              changeDeviceShareMeshId(changeDeviceShareMeshIdNodeList, command.meshid);
            }
            ...
            if (parent.parent.mqttbroker != null) {
              parent.parent.mqttbroker.changeDeviceMesh(node._id, command.meshid);
            }

            // If any CIRA sessions are connected on this server, switch it now.
            if (parent.parent.mpsserver != null) {
              parent.parent.mpsserver.changeDeviceMesh(node._id, command.meshid);
            }
            ...
            if (state) {
              ...
              if ((state.connectivity & 2) != 0) {
                ...
                if (ciraConnection != null && ciraConnection.tag != null) {
                  node.cict = ciraConnection.tag.connectTime;
                }
              }
            }
            ...
            db.Get('lc' + node._id, function (err, xnodes) {
              if (xnodes != null && xnodes.length == 1 && xnodes[0].meshid != command.meshid) {
                xnodes[0].meshid = command.meshid;
                db.Set(xnodes[0]);
              }
            });
            ...
            var event = {
              ...
              node: node,
              oldMeshId: oldMeshId,
              newMeshId: command.meshid,
              msgid: 85,
              msgArgs: [node.name, newMesh.name],
              msg: 'Moved device ' + node.name + ' to group ' + newMesh.name,
              domain: domain.id
            };
            ...
            parent.parent.DispatchEvent(
              parent.CreateMeshDispatchTargets(command.meshid, [oldMeshId, node._id]),
              obj,
              event
            );
            ...
            if (command.responseid != null) {
              try {
                ws.send(JSON.stringify({ action: 'changeDeviceMesh', responseid: command.responseid, result: 'ok' }));
              } catch (ex) {}
            }
          });
        }
      ...
      case 'removedevices': {
        ...
        for (i in command.nodeids) {
          ...
          // Argument validation
          if (common.validateString(nodeid, 1, 1024) == false) {
            err = 'Invalid nodeid';
          } // Check nodeid
          else {
            if (nodeid.indexOf('/') == -1) {
              nodeid = 'node/' + domain.id + '/' + nodeid;
            }
            if (nodeid.split('/').length != 3 || nodeid.split('/')[1] != domain.id) {
              err = 'Invalid domain';
            } // Invalid domain, operation only valid for current domain
          }
          if (err != null) {
            if (command.responseid != null) {
              try {
                ws.send(JSON.stringify({ action: 'removedevices', responseid: command.responseid, result: err }));
              } catch (ex) {}
            }
            continue;
          }
          ...
          parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
            ...
            if ((rights & MESHRIGHT_UNINSTALL) == 0) {
              if (command.responseid != null) {
                try {
                  ws.send(
                    JSON.stringify({ action: 'removedevices', responseid: command.responseid, result: 'Denied' })
                  );
                } catch (ex) {}
              }
              return;
            ...
            if (typeof node.pmt == 'string') {
              db.Remove('pmt_' + node.pmt);
            } // Remove Push Messaging Token
            db.Get('ra' + node._id, function (err, nodes) {
              if (nodes != null && nodes.length == 1) {
                db.Remove('da' + nodes[0].daid);
              } // Remove diagnostic agent to real agent link
              ...
              for (var i in node.links) {
                if (i.startsWith('user/')) {
                  ...
                  if (cuser != null && cuser.links != null && cuser.links[node._id] != null) {
                    ...
                    if (Object.keys(cuser.links).length == 0) {
                      delete cuser.links;
                    }
                    db.SetUser(cuser);
                    ...
                    var event = {
                      ...
                      msgid: 86,
                      msgArgs: [cuser.name],
                      msg: 'Removed user device rights for ' + cuser.name,
                      domain: domain.id,
                      account: parent.CloneSafeUser(cuser)
                    };
                    if (db.changeStream) {
                      event.noact = 1;
                    } // If DB change stream is active, don't use this event to change the user. Another event will come.
                  ...
                  if (cusergroup != null && cusergroup.links != null && cusergroup.links[node._id] != null) {
                    ...
                    if (Object.keys(cusergroup.links).length == 0) {
                      delete cusergroup.links;
                    }
                    db.Set(cusergroup);
                    ...
                    var event = {
                      ...
                      userid: user._id,
                      username: user.name,
                      ugrpid: cusergroup._id,
                      name: cusergroup.name,
                      desc: cusergroup.desc,
                      action: 'usergroupchange',
                      links: cusergroup.links,
                      msgid: 163,
                      msgArgs: [node.name, cusergroup.name],
                      msg: 'Removed device ' + node.name + ' from user group ' + cusergroup.name
                    };
                    if (db.changeStream) {
                      event.noact = 1;
                    } // If DB change stream is active, don't use this event to change the user. Another event will come.
            ...
            var event = {
              ...
              nodeid: node._id,
              msgid: 87,
              msgArgs: [node.name, parent.meshes[node.meshid].name],
              msg: 'Removed device ' + node.name + ' from device group ' + parent.meshes[node.meshid].name,
              domain: domain.id
            };
            ...
            if (state != null && state.connectivity != null) {
              if ((state.connectivity & 1) != 0) {
                parent.wsagents[nodeid].close();
              } // Disconnect mesh agent
              if ((state.connectivity & 2) != 0) {
                parent.parent.mpsserver.closeAllForNode(nodeid);
              } // Disconnect CIRA/Relay/LMS connections
            }

            // Send response if required
            if (command.responseid != null) {
              try {
                ws.send(JSON.stringify({ action: 'removedevices', responseid: command.responseid, result: 'ok' }));
              } catch (ex) {}
            }
          });
        }
      ...
      case 'wakedevices': {
        ...
        parent.parent.DispatchEvent('*', obj, {
          action: 'wakedevices',
          userid: user._id,
          username: user.name,
        ...
        for (i in command.nodeids) {
          ...
          // Argument validation
          if (common.validateString(nodeid, 8, 128) == false) {
            // Check the nodeid
            if (command.nodeids.length == 1) {
              try {
                ws.send(
                  JSON.stringify({ action: 'wakedevices', responseid: command.responseid, result: 'Invalid nodeid' })
                );
              } catch (ex) {}
            }
            continue;
            ...
            nodeid = 'node/' + domain.id + '/' + nodeid;
          } else if (nodeid.split('/').length != 3 || nodeid.split('/')[1] != domain.id) {
            // Invalid domain, operation only valid for current domain
            if (command.nodeids.length == 1) {
              try {
                ws.send(
                  JSON.stringify({ action: 'wakedevices', responseid: command.responseid, result: 'Invalid domain' })
                );
              } catch (ex) {}
            }
            continue;
          ...
          parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
            ...
            if (parent.parent.mqttbroker != null) {
              parent.parent.mqttbroker.publish(node._id, 'powerAction', 'wake');
            }
            ...
            if (node.mtype == 4) {
              ...
              const targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, [
                'devport-operation',
                'server-users',
                user._id
              ]);
            ...
            db.Get('if' + node._id, function (err, nodeifs) {
              if (nodeifs != null && nodeifs.length == 1) {
                var macs = [],
                  nodeif = nodeifs[0];
                if (nodeif.netif) {
                  for (var j in nodeif.netif) {
                    if (
                      nodeif.netif[j].mac &&
                      nodeif.netif[j].mac != '00:00:00:00:00:00' &&
                      macs.indexOf(nodeif.netif[j].mac) == -1
                    ) {
                      macs.push(nodeif.netif[j].mac);
                    }
                  }
                } else if (nodeif.netif2) {
                  for (var j in nodeif.netif2) {
                    for (var k in nodeif.netif2[j]) {
                      if (
                        nodeif.netif2[j][k].mac &&
                        nodeif.netif2[j][k].mac != '00:00:00:00:00:00' &&
                        macs.indexOf(nodeif.netif2[j][k].mac) == -1
                      ) {
                        macs.push(nodeif.netif2[j][k].mac);
                      }
                    }
                  }
                }
                    ...
                    try {
                      ws.send(
                        JSON.stringify({
                          action: 'wakedevices',
                          responseid: command.responseid,
                          result: 'No known MAC addresses for this device'
                        })
                      );
                    } catch (ex) {}
                  }
                ...
                if (parent.parent.meshScanner != null) {
                  parent.parent.meshScanner.wakeOnLan(macs, node.host);
                }

                // Get the list of device groups this user as wake permissions on
                var targets = [],
                  targetDeviceGroups = parent.GetAllMeshWithRights(user, MESHRIGHT_WAKEDEVICE);
                for (j in targetDeviceGroups) {
                  targets.push(targetDeviceGroups[j]._id);
                }
                for (j in user.links) {
                  if (
                    j.startsWith('node/') &&
                    typeof user.links[j].rights == 'number' &&
                    (user.links[j].rights & MESHRIGHT_WAKEDEVICE) != 0
                  ) {
                    targets.push(j);
                  }
                }
                ...
                for (j in parent.wsagents) {
                  ...
                  if (
                    agent.authenticated == 2 &&
                    (targets.indexOf(agent.dbMeshKey) >= 0 || targets.indexOf(agent.dbNodeKey) >= 0)
                  ) {
                  ...
                  try {
                    ws.send(
                      JSON.stringify({
                        action: 'wakedevices',
                        responseid: command.responseid,
                        result: 'Used ' + wakeCount + ' device(s) to send wake packets'
                      })
                    );
                  } catch (ex) {}
                }
                  ...
                  try {
                    ws.send(
                      JSON.stringify({
                        action: 'wakedevices',
                        responseid: command.responseid,
                        result: 'No network information for this device'
                      })
                    );
                  } catch (ex) {}
                }
          ...
          if (command.nodeids.length > 1) {
            ...
            try {
              ws.send(JSON.stringify({ action: 'wakedevices', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          }
        }
        break;
      ...
      case 'webrelay': {
        if (common.validateString(command.nodeid, 8, 128) == false) {
          ...
          command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
        } else if (command.nodeid.split('/').length != 3 || command.nodeid.split('/')[1] != domain.id) {
          err = 'Invalid domain';
        } // Invalid domain, operation only valid for current domain
        else if (command.port != null && common.validateInt(command.port, 1, 65535) == false) {
          err = 'Invalid port value';
        } // Check the port if present
        else {
          if (command.nodeid.split('/').length == 1) {
            command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
          }
          var snode = command.nodeid.split('/');
          if (snode.length != 3 || snode[0] != 'node' || snode[1] != domain.id) {
            err = 'Invalid node id';
          }
        }
        // Handle any errors
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'webrelay', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          if (node == null) {
            if (command.responseid != null) {
              try {
                ws.send(
                  JSON.stringify({ action: 'webrelay', responseid: command.responseid, result: 'Invalid node id' })
                );
              } catch (ex) {}
            }
            return;
          ...
          if (webRelayPort == 0) {
            try {
              ws.send(
                JSON.stringify({ action: 'webrelay', responseid: command.responseid, result: 'WebRelay Disabled' })
              );
              return;
            } catch (ex) {}
          }
          const authRelayCookie = parent.parent.encodeCookie(
            { ruserid: user._id, x: req.session.x },
      ...
      case 'runcommands': {
        ...
        if (typeof command.presetcmd != 'number') {
          ...
          if (typeof command.runAsUser != 'number') {
            command.runAsUser = 0;
          } // Check runAsUser
        }
        ...
        const processRunCommand = function (command) {
          for (i in command.nodeids) {
            ...
            if (err != null) {
              if (command.responseid != null) {
                try {
                  ws.send(JSON.stringify({ action: 'runcommands', responseid: command.responseid, result: err }));
                } catch (ex) {}
              }
              continue;
            }
            ...
            parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
              ...
              if (command.type == 4) {
                ...
                if (parent.parent.multiServer != null) {
                  ...
                  if (command.responseid != null) {
                    try {
                      ws.send(JSON.stringify({ action: 'runcommands', responseid: command.responseid, result: 'OK' }));
                    } catch (ex) {}
                  }
                } else {
                  ...
                  if (agent != null && agent.authenticated == 2 && agent.agentInfo != null) {
                    try {
                      agent.send(JSON.stringify(theCommand));
                    } catch (ex) {}
                    if (command.responseid != null) {
                      try {
                        ws.send(
                          JSON.stringify({ action: 'runcommands', responseid: command.responseid, result: 'OK' })
                        );
                      } catch (ex) {}
                    }
                  } else {
                ...
                if (command.type == 2) {
                  msgid = 100;
                } // "Running commands as user if possible"
                ...
                if ((node.agent.id > 0 && node.agent.id < 5) || (node.agent.id > 41 && node.agent.id < 44)) {
                  ...
                  if (command.type == 1 || command.type == 2) {
                    ...
                    command.type = 1;
                    commandsOk = true;
                  } // Set the default type of this agent
                } else {
                  // Non-Windows Agent
                  if (command.type == 3) {
                    commandsOk = true;
                ...
                if (commandsOk == true) {
                  ...
                  if (agent != null && agent.authenticated == 2 && agent.agentInfo != null) {
                    ...
                    if (command.responseid != null && command.reply == false) {
                      try {
                        ws.send(
                          JSON.stringify({ action: 'runcommands', responseid: command.responseid, result: 'OK' })
                        );
        ...
        if (typeof command.presetcmd == 'number') {
          ...
          // Decode script type
          const types = ['', 'bat', 'ps1', 'sh', 'agent']; // 1 = Windows Command, 2 = Windows PowerShell, 3 = Linux, 4 = Agent
          if (typeof script.type == 'string') {
            const stype = types.indexOf(script.type.toLowerCase());
            if (stype > 0) {
              command.type = stype;
            }
          }
          ...
          if (command.type != 4) {
            ...
            if (typeof script.runas == 'string') {
              const srunas = runAsModes.indexOf(script.runas.toLowerCase());
              if (srunas >= 0) {
                command.runAsUser = srunas;
              }
            }
          }
        ...
        for (i in command.nodeids) {
          ...
          parent.GetNodeWithRights(domain, user, command.nodeids[i], function (node, rights, visible) {
            ...
            if (agent != null) {
              ...
              try {
                agent.send(JSON.stringify({ action: 'uninstallagent' }));
              } catch (ex) {}
            }
          });
        }
        ...
        for (i in command.nodeids) {
          ...
          parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
            if (command.actiontype >= 400 && (rights & MESHRIGHT_REMOTECONTROL) != 0) {
              // Flash and vibrate
              if (command.actiontype == 400 && common.validateInt(command.time, 1, 30000)) {
                routeCommandToNode(
                  { action: 'msg', type: 'console', nodeid: node._id, value: 'flash ' + command.time },
                  MESHRIGHT_ADMIN,
                  0
                );
              }
              if (command.actiontype == 401 && common.validateInt(command.time, 1, 30000)) {
                routeCommandToNode(
                  { action: 'msg', type: 'console', nodeid: node._id, value: 'vibrate ' + command.time },
                  MESHRIGHT_ADMIN,
                  0
                );
              }
              ...
              if (parent.parent.mqttbroker != null && command.actiontype >= 0 && command.actiontype <= 4) {
                parent.parent.mqttbroker.publish(
                  node._id,
                  'powerAction',
                  ['', '', 'poweroff', 'reset', 'sleep'][command.actiontype]
                );
              }
              ...
              if (command.actiontype >= 300 && command.actiontype < 400) {
                if (
                  command.actiontype != 302 &&
                  command.actiontype != 308 &&
                  command.actiontype < 310 &&
                  command.actiontype > 316
                )
                ...
                parent.parent.DispatchEvent('*', obj, {
                  ...
                  domain: domain.id,
                  nolog: 1,
                  actiontype: command.actiontype - 300
                });
              } else {
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'poweraction', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          } else {
            try {
      ...
      case 'toast': {
        ...
        try {
          ...
          else if (common.validateString(command.msg, 1, 4096) == false) {
            err = 'Invalid message';
          } // Check message
          else {
            var nodeids = [];
            for (i in command.nodeids) {
              if (command.nodeids[i].indexOf('/') == -1) {
                nodeids.push('node/' + domain.id + '/' + command.nodeids[i]);
              } else {
                nodeids.push(command.nodeids[i]);
              }
            }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'toast', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (common.validateString(command.title, 1, 512) == false) {
          delete command.title;
        } // Check title
        if (
          command.title == null &&
          typeof domain.notificationmessages == 'object' &&
          typeof domain.notificationmessages.title == 'string'
        ) {
          command.title = domain.notificationmessages.title;
        }
        if (command.title == null && typeof domain.title == 'string') {
          command.title = domain.title;
        }
        if (command.title == null) {
          command.title = 'MeshCentral';
        }
        ...
        for (i in command.nodeids) {
          ...
          parent.GetNodeWithRights(domain, user, command.nodeids[i], function (node, rights, visible) {
            ...
            if ((rights & MESHRIGHT_CHATNOTIFY) == 0) {
              if (command.responseid != null) {
                try {
                  ws.send(JSON.stringify({ action: 'toast', responseid: command.responseid, result: 'Access Denied' }));
                } catch (ex) {}
              }
              return;
            }
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'toast', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'changedevice': {
        ...
        try {
          if (common.validateString(command.nodeid, 1, 1024) == false) {
            ...
            if (command.nodeid.split('/').length != 3 || command.nodeid.split('/')[1] != domain.id) {
              err = 'Invalid nodeid';
            } // Invalid domain, operation only valid for current domain
            else if (command.userloc && command.userloc.length != 2 && command.userloc.length != 0) {
              err = 'Invalid user location';
            }
          }
        } catch (ex) {
          console.log(ex);
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'changedevice', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          if ((rights & MESHRIGHT_MANAGECOMPUTERS) == 0) {
            if (command.responseid != null) {
              try {
                ws.send(
                  JSON.stringify({ action: 'changedevice', responseid: command.responseid, result: 'Access Denied' })
                );
              } catch (ex) {}
            }
            return;
          ...
          if (args.wanonly == true && command.host && node.mtype != 3 && node.mtype != 4) {
            delete command.host;
          }

          // Look for a change
          if (typeof command.icon == 'number' && command.icon != node.icon) {
            change = 1;
            node.icon = command.icon;
            changes.push('icon');
          }
          if (typeof command.name == 'string' && command.name != node.name) {
            change = 1;
            node.name = command.name;
            changes.push('name');
          }
          if (typeof command.host == 'string' && command.host != node.host) {
            change = 1;
            node.host = command.host;
            changes.push('host');
          }
          if (typeof command.consent == 'number') {
            var oldConsent = node.consent;
            if (command.consent != node.consent) {
              node.consent = command.consent;
            }
            if (command.consent == 0) {
              delete node.consent;
            }
            if (oldConsent != node.consent) {
              change = 1;
          ...
          if (typeof command.rdpport == 'number' && command.rdpport > 0 && command.rdpport < 65536) {
            if (command.rdpport == 3389 && node.rdpport != null) {
              delete node.rdpport;
              change = 1;
              changes.push('rdpport'); // Delete the RDP port
            } else {
              node.rdpport = command.rdpport;
              change = 1;
              changes.push('rdpport'); // Set the RDP port
            }
          }
          ...
          if (typeof command.rfbport == 'number' && command.rfbport > 0 && command.rfbport < 65536) {
            if (command.rfbport == 5900 && node.rfbport != null) {
              delete node.rfbport;
              change = 1;
              changes.push('rfbport'); // Delete the RFB port
            } else {
              node.rfbport = command.rfbport;
              change = 1;
              changes.push('rfbport'); // Set the RFB port
            }
          }
          ...
          if (typeof command.sshport == 'number' && command.sshport > 0 && command.sshport < 65536) {
            if (command.sshport == 22 && node.sshport != null) {
              delete node.sshport;
              change = 1;
              changes.push('sshport'); // Delete the SSH port
            } else {
              node.sshport = command.sshport;
              change = 1;
              changes.push('sshport'); // Set the SSH port
            }
          }

          if (typeof command.httpport == 'number' && command.httpport > 0 && command.httpport < 65536) {
            if (command.httpport == 80 && node.httpport != null) {
              delete node.httpport;
              change = 1;
              changes.push('httpport'); // Delete the HTTP port
            } else {
              node.httpport = command.httpport;
              change = 1;
              changes.push('httpport'); // Set the HTTP port
            }
          }
          ...
          if (typeof command.httpsport == 'number' && command.httpsport > 0 && command.httpsport < 65536) {
            if (command.httpsport == 443 && node.httpsport != null) {
              delete node.httpsport;
              change = 1;
              changes.push('httpsport'); // Delete the HTTPS port
            } else {
              node.httpsport = command.httpsport;
              change = 1;
              changes.push('httpsport'); // Set the HTTPS port
            }
          }

          if (typeof command.ssh == 'number' && command.ssh == 0) {
            if (node.ssh != null && node.ssh[user._id] != null) {
              delete node.ssh[user._id];
              change = 1;
              changes.push('ssh');
            } // Delete the SSH cendentials
          }

          if (typeof command.rdp == 'number' && command.rdp == 0) {
            if (node.rdp != null && node.rdp[user._id] != null) {
              delete node.rdp[user._id];
              change = 1;
              changes.push('rdp');
            } // Delete the RDP cendentials
          }
          ...
          if (
            domain.geolocation &&
            command.userloc &&
            (node.userloc == null || command.userloc[0] != node.userloc[0] || command.userloc[1] != node.userloc[1])
          ) {
            change = 1;
          ...
          if (command.desc != null && command.desc != node.desc) {
            change = 1;
            node.desc = command.desc;
            changes.push('description');
          }
          if (command.intelamt != null) {
            if (
              parent.parent.amtManager == null ||
              node.intelamt.pass == null ||
              node.intelamt.pass == '' ||
              (node.intelamt.warn != null && (node.intelamt.warn & 9) != 0)
            ) {
              // Only allow changes to Intel AMT credentials if AMT manager is not running, or manager warned of unknown/trying credentials.
              if (
                command.intelamt.user != null &&
                command.intelamt.pass != null &&
                (command.intelamt.user != node.intelamt.user || command.intelamt.pass != node.intelamt.pass)
              ) {
                change = 1;
            ...
            if (
              parent.parent.amtManager != null &&
              command.intelamt.tls != null &&
              command.intelamt.tls != node.intelamt.tls
            ) {
              change = 1;
              node.intelamt.tls = command.intelamt.tls;
              changes.push('Intel AMT TLS');
            }
          }
          if (command.tags) {
            // Node grouping tag, this is a array of strings that can't be empty and can't contain a comma
            var ok = true,
              group2 = [];
            if (common.validateString(command.tags, 0, 4096) == true) {
              command.tags = command.tags.split(',');
            }
            for (var i in command.tags) {
              var tname = command.tags[i].trim();
              if (tname.length > 0 && tname.length < 64 && group2.indexOf(tname) == -1) {
                group2.push(tname);
              }
            }
            group2.sort();
          ...
          } else if (command.tags === '' && node.tags) {
            delete node.tags;
            change = 1;
          }
          ...
          if (change == 1) {
            ...
            if (amtchange == 1) {
              event.amtchange = 1;
            } // This will give a hint to the AMT Manager to reconnect using new AMT credentials
            if (command.rdpport == 3389) {
              event.node.rdpport = 3389;
            }
            if (command.rfbport == 5900) {
              event.node.rfbport = 5900;
            }
            if (db.changeStream) {
              event.noact = 1;
            } // If DB change stream is active, don't use this event to change the node. Another event will come.
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'changedevice', responseid: command.responseid, result: 'ok' }));
            } catch (ex) {}
          }
        });
        break;
        ...
        for (var i in command.nodeids) {
          ...
          parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
            ...
            if (command.type == 'default') {
              ...
              parent.parent.updateMeshCore(function () {
                parent.sendMeshAgentCore(user, domain, node._id, 'default');
              });
      ...
      case 'inviteAgent': {
        ...
        try {
          if (domain.mailserver == null || args.lanonly == true) {
            err = 'Unsupported feature';
          } // This operation requires the email server
          else if (
            parent.parent.certificates.CommonName == null ||
            parent.parent.certificates.CommonName.indexOf('.') == -1
          ) {
            err = 'Unsupported feature';
          } // Server name must be configured
          else if (common.validateString(command.meshid, 8, 134) == false) {
            err = 'Invalid group identifier';
          } // Check meshid
          else {
            if (command.meshid.split('/').length == 1) {
              command.meshid = 'mesh/' + domain.id + '/' + command.meshid;
            }
            if (command.meshid.split('/').length != 3 || command.meshid.split('/')[1] != domain.id) {
              err = 'Invalid group identifier';
            } // Invalid domain, operation only valid for current domain
            else if (common.validateString(command.email, 4, 1024) == false) {
              err = 'Invalid email';
            } // Check email
            else if (command.email.split('@').length != 2) {
              err = 'Invalid email';
            } // Check email
            else {
              mesh = parent.meshes[command.meshid];
              if (mesh == null) {
                ...
                err = 'Invalid group type';
              } // Check if this is the correct group type
              else if (parent.IsMeshViewable(user, mesh) == false) {
                err = 'Not allowed';
              } // Check if this user has rights to do this
            }
          }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'inviteAgent', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        domain.mailserver.sendAgentInviteMail(
          domain,
        ...
        if (command.responseid != null) {
          try {
            ws.send(JSON.stringify({ action: 'inviteAgent', responseid: command.responseid, result: 'ok' }));
          } catch (ex) {}
        }
        break;
      }
      case 'setDeviceEvent': {
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          var event = {
            ...
            nodeid: node._id,
            action: 'manual',
            msg: decodeURIComponent(command.msg),
            domain: domain.id
          };
          parent.parent.DispatchEvent(targets, obj, event);
        });
      ...
      case 'otpemail': {
        ...
        var event = {
          ...
          account: parent.CloneSafeUser(user),
          action: 'accountchange',
          msgid: command.enabled ? 88 : 89,
          msg: command.enabled
            ? 'Enabled email two-factor authentication.'
            : 'Disabled email two-factor authentication.',
          domain: domain.id
        };
        if (db.changeStream) {
          event.noact = 1;
      ...
      case 'otpduo': {
        ...
        if (
          typeof domain.duo2factor != 'object' ||
          typeof domain.duo2factor.integrationkey != 'string' ||
          typeof domain.duo2factor.secretkey != 'string' ||
          typeof domain.duo2factor.apihostname != 'string'
        )
          return;
        ...
        var event = {
          ...
          account: parent.CloneSafeUser(user),
          action: 'accountchange',
          msgid: command.enabled ? 160 : 161,
          msg: command.enabled ? 'Enabled duo two-factor authentication.' : 'Disabled duo two-factor authentication.',
          domain: domain.id
        };
        if (db.changeStream) {
          event.noact = 1;
      ...
      case 'otpauth-request': {
        // Do not allow this command if 2FA's are locked
        if (domain.passwordrequirements && domain.passwordrequirements.lock2factor == true) {
          ws.send(JSON.stringify({ action: 'otpauth-request', err: 1 }));
          return;
        }

        // Do not allow this command when logged in using a login token
        if (req.session.loginToken != null) {
          ws.send(JSON.stringify({ action: 'otpauth-request', err: 3 }));
          return;
        }

        // Check of OTP 2FA is allowed
        if (domain.passwordrequirements && domain.passwordrequirements.otp2factor == false) {
          ws.send(JSON.stringify({ action: 'otpauth-request', err: 4 }));
          return;
        }

        if (user.siteadmin != 0xffffffff && (user.siteadmin & 1024) != 0) {
          ws.send(JSON.stringify({ action: 'otpauth-request', err: 5 }));
          return;
        } // If this account is settings locked, return here.
        ...
        if (twoStepLoginSupported) {
          ...
          if (otplib == null) {
            ws.send(JSON.stringify({ action: 'otpauth-request', err: 6 }));
            return;
          }
          ...
          ws.send(
            JSON.stringify({
              action: 'otpauth-request',
              secret: secret,
              url: otplib.authenticator.keyuri(user.name, domainName, secret)
            })
          );
        }
        break;
        ...
        if (twoStepLoginSupported) {
          ...
          if (otplib.authenticator.check(command.token, command.secret) === true) {
            ...
            var event = {
              ...
              action: 'accountchange',
              msgid: 90,
              msg: 'Added authentication application',
              domain: domain.id
            };
            if (db.changeStream) {
              event.noact = 1;
        ...
        if (twoStepLoginSupported) {
          ...
          if (user.otpsecret) {
            ...
            var event = {
              ...
              action: 'accountchange',
              msgid: 91,
              msg: 'Removed authentication application',
              domain: domain.id
            };
            if (db.changeStream) {
              event.noact = 1;
      ...
      case 'otpauth-getpasswords': {
        ...
        var actionTaken = false,
          ...
          actionId = 0;
        if (user.siteadmin == 0xffffffff || (user.siteadmin & 1024) == 0) {
          // Don't allow generation of tokens if the account is settings locked
          // Perform a sub-action
          if (command.subaction == 1) {
            // Generate a new set of tokens
            var randomNumbers = [],
              v;
            for (var i = 0; i < 10; i++) {
              do {
                v = getRandomEightDigitInteger();
              } while (randomNumbers.indexOf(v) >= 0);
              randomNumbers.push(v);
            }
            user.otpkeys = { keys: [] };
            for (var i = 0; i < 10; i++) {
              user.otpkeys.keys[i] = { p: randomNumbers[i], u: true };
            }
            actionTaken = true;
            actionId = 92;
            actionText = 'New 2FA backup codes generated';
          } else if (command.subaction == 2) {
            // Clear all tokens
        ...
        if (count2factoraAuths() > 0) {
          ws.send(
            JSON.stringify({ action: 'otpauth-getpasswords', passwords: user.otpkeys ? user.otpkeys.keys : null })
          );
        }
      ...
      case 'otp-hkey-get': {
        ...
        var hkeys = [];
        if (user.otphkeys != null) {
          for (var i = 0; i < user.otphkeys.length; i++) {
            hkeys.push({ i: user.otphkeys[i].keyIndex, name: user.otphkeys[i].name, type: user.otphkeys[i].type });
          }
        }
      ...
      case 'otp-hkey-remove': {
        ...
        if (user.otphkeys != null) {
          for (var i = 0; i < user.otphkeys.length; i++) {
            if (user.otphkeys[i].keyIndex == command.index) {
              foundAtIndex = i;
            }
          }
        }
      ...
      case 'otp-hkey-yubikey-add': {
        ...
        if (domain.passwordrequirements) {
          ...
          if (
            typeof domain.passwordrequirements.maxfidokeys == 'number' &&
            user.otphkeys &&
            user.otphkeys.length >= domain.passwordrequirements.maxfidokeys
          )
            return;
        }
        ...
        if (
          typeof domain.yubikey != 'object' ||
          typeof domain.yubikey.id != 'string' ||
          typeof domain.yubikey.secret != 'string' ||
          command.otp.length != 44
        ) {
          ws.send(JSON.stringify({ action: 'otp-hkey-yubikey-add', result: false, name: command.name }));
          break;
        }
        ...
        yub.verify(command.otp, function (err, results) {
          if (results != null && results.status == 'OK') {
            ...
            var foundAtIndex = -1;
            for (var i = 0; i < user.otphkeys.length; i++) {
              if (user.otphkeys[i].keyid == keyId) {
                foundAtIndex = i;
              }
            }
            if (foundAtIndex != -1) {
            ...
            parent.db.SetUser(user);
            ws.send(
              JSON.stringify({ action: 'otp-hkey-yubikey-add', result: true, name: command.name, index: keyIndex })
            );
      ...
      case 'otpdev-clear': {
        ...
        if (user.otpdev != null) {
          ...
          var event = {
            ...
            action: 'accountchange',
            msgid: 114,
            msg: 'Removed push notification authentication device',
            domain: domain.id
          };
          if (db.changeStream) {
            event.noact = 1;
      ...
      case 'otpdev-set': {
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          var payload = {
            notification: { title: 'MeshCentral', body: user.name + ' authentication' },
            data: { url: '2fa://auth?code=' + code + '&c=' + authCookie }
          };
          ...
          parent.parent.firebase.sendToDevice(node, payload, options, function (id, err, errdesc) {
            if (err == null) {
              ...
              parent.parent.debug(
                'email',
                'Failed auth addition push message to device ' + node.name + ', error: ' + errdesc
              );
            }
          });
        });
      ...
      case 'webauthn-startregister': {
        ...
        var registrationOptions = parent.webauthn.generateRegistrationChallenge('Anonymous Service', {
          id: Buffer.from(user._id, 'binary').toString('base64'),
          name: user._id,
          displayName: user._id.split('/')[2]
        });
        //console.log('registrationOptions', registrationOptions);
        registrationOptions.userVerification =
          domain.passwordrequirements && domain.passwordrequirements.fidopininput
            ? domain.passwordrequirements.fidopininput
            : 'preferred'; // Use the domain setting if it exists, otherwise use 'preferred'.
        obj.webAuthnReqistrationRequest = {
          action: 'webauthn-startregister',
      ...
      case 'webauthn-endregister': {
        ...
        var regResult = null;
        try {
          regResult = parent.webauthn.verifyAuthenticatorAttestationResponse(command.response.response);
        } catch (ex) {
          regResult = { verified: false, error: ex };
        }
        if (regResult.verified === true) {
          // Since we are registering a WebAuthn/FIDO2 key, remove all U2F keys (Type 1).
          var otphkeys2 = [];
          if (user.otphkeys && Array.isArray(user.otphkeys)) {
            for (var i = 0; i < user.otphkeys.length; i++) {
              if (user.otphkeys[i].type != 1) {
                otphkeys2.push(user.otphkeys[i]);
              }
            }
          }
          ...
          user.otphkeys.push({
            name: obj.webAuthnReqistrationRequest.keyname,
            type: 3,
            publicKey: regResult.authrInfo.publicKey,
            counter: regResult.authrInfo.counter,
            keyIndex: keyIndex,
            keyId: regResult.authrInfo.keyId
          });
          parent.db.SetUser(user);
          ws.send(
            JSON.stringify({ action: 'otp-hkey-setup-response', result: true, name: command.name, index: keyIndex })
          );
          ...
          ws.send(
            JSON.stringify({
              action: 'otp-hkey-setup-response',
              result: false,
            ...
            db.Get('nt' + command.id, function (err, notes) {
              try {
                if (notes == null || notes.length != 1) {
                  ws.send(JSON.stringify({ action: 'getNotes', id: command.id, notes: null }));
                  return;
                }
      ...
      case 'createInviteLink': {
        ...
        if (common.validateString(command.meshid, 8, 134) == false) {
          err = 'Invalid group id';
        } // Check the meshid (Max length of a meshid is 134 bytes).
        else if (common.validateInt(command.expire, 0, 99999) == false) {
          err = 'Invalid expire time';
        } // Check the expire time in hours
        else if (common.validateInt(command.flags, 0, 256) == false) {
          err = 'Invalid flags';
        } // Check the flags
        else {
          if (command.meshid.split('/').length == 1) {
          ...
          if (smesh.length != 3 || smesh[0] != 'mesh' || smesh[1] != domain.id) {
            err = 'Invalid group id';
          }
          mesh = parent.meshes[command.meshid];
          if (mesh == null || parent.IsMeshViewable(user, mesh) == false) {
            err = 'Invalid group id';
          }
        }
        ...
        if (err != null) {
          ...
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'createInviteLink', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
            ...
            try {
              ws.send(
                JSON.stringify({
                  ...
                  responseid: command.responseid,
                  result: 'Unable to generate invitation cookie'
                })
              );
            } catch (ex) {}
          }
        ...
        if (serverName.split('.') == 1) {
          url = '/' + xdomain + 'agentinvite?c=' + inviteCookie;
        }
      ...
      case 'deviceMeshShares': {
        ...
        if (common.validateString(command.meshid, 8, 134) == false) {
          ...
          else if (parent.GetMeshRights(user, mesh) == 0) {
            err = 'Access denied';
          }
        }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'deviceShares', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
            ...
            if (doc.expireTime != null && doc.expireTime < now) {
              ...
              if (doc.xmeshid == null) {
                ...
                if (doc.userid != user._id) {
                  delete doc.url;
                } // If this is not the user who created this link, don't give the link.
                okDocs.push(doc);
              }
            }
          }
          try {
            ws.send(JSON.stringify({ action: 'deviceMeshShares', meshid: command.meshid, deviceShares: okDocs }));
          } catch (ex) {}
        });
        break;
      }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          if (node == null) {
            if (command.responseid != null) {
              try {
                ws.send(
                  JSON.stringify({ action: 'deviceShares', responseid: command.responseid, result: 'Invalid node id' })
                );
              } catch (ex) {}
            }
            return;
          ...
          if (rights != MESHRIGHT_ADMIN) {
            ...
            if (
              ...
              (rights & MESHRIGHT_DESKLIMITEDINPUT) != 0 ||
              (rights & MESHRIGHT_GUESTSHARING) == 0
            ) {
              if (command.responseid != null) {
                try {
                  ws.send(
                    JSON.stringify({ action: 'deviceShares', responseid: command.responseid, result: 'Access denied' })
                  );
                } catch (ex) {}
              }
              return;
          ...
          parent.db.GetAllTypeNodeFiltered([command.nodeid], domain.id, 'deviceshare', null, function (err, docs) {
            ...
            try {
              ws.send(JSON.stringify({ action: 'deviceShares', nodeid: command.nodeid, deviceShares: okDocs }));
            } catch (ex) {}
      ...
      case 'removeDeviceShare': {
        ...
        if (common.validateString(command.publicid, 1, 128) == false) {
          err = 'Invalid public id';
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'removeDeviceShare', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          parent.db.GetAllTypeNodeFiltered([command.nodeid], domain.id, 'deviceshare', null, function (err, docs) {
            ...
            for (var i = 0; i < docs.length; i++) {
              const doc = docs[i];
              if (doc.publicid == command.publicid) {
                parent.db.Remove(doc._id, function () {});
                removedExact = doc;
                removed = true;
            ...
            if (removedExact != null) {
              ...
              var targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, [
                'server-users',
                'server-shareremove',
                user._id
              ]);
              var event = {
                ...
                msgid: 102,
                msgArgs: [removedExact.guestName],
                domain: domain.id,
                publicid: command.publicid
              };
              parent.parent.DispatchEvent(targets, obj, event);
              ...
              if (command.publicid.startsWith('AS:node/')) {
                routeCommandToNode({
                  action: 'msg',
                  type: 'guestShare',
                  nodeid: command.publicid.substring(3),
                  flags: 0,
                  url: null,
                  viewOnly: false
                });
            ...
            if (removed == true) {
              ...
              if (command.responseid != null) {
                try {
                  ws.send(
                    JSON.stringify({ action: 'removeDeviceShare', responseid: command.responseid, result: 'OK' })
                  );
                } catch (ex) {}
              }
            } else {
                ...
                try {
                  ws.send(
                    JSON.stringify({
                      action: 'removeDeviceShare',
                      responseid: command.responseid,
                      result: 'Invalid device share identifier.'
                    })
                  );
                } catch (ex) {}
              }
      ...
      case 'createDeviceShareLink': {
        ...
        if (common.validateString(command.guestname, 1, 128) == false) {
          err = 'Invalid guest name';
        } // Check the guest name
        else if (command.expire != null && typeof command.expire != 'number') {
          err = 'Invalid expire time';
        } // Check the expire time in minutes
        else if (command.start != null && typeof command.start != 'number') {
          err = 'Invalid start time';
        } // Check the start time in UTC seconds
        else if (command.end != null && typeof command.end != 'number') {
          err = 'Invalid end time';
        } // Check the end time in UTC seconds
        else if (common.validateInt(command.consent, 0, 256) == false) {
          err = 'Invalid flags';
        } // Check the flags
        else if (common.validateInt(command.p, 1, 31) == false) {
          err = 'Invalid protocol';
        } // Check the protocol, 1 = Terminal, 2 = Desktop, 4 = Files, 8 = HTTP, 16 = HTTPS
        else if (command.recurring != null && common.validateInt(command.recurring, 1, 2) == false) {
          err = 'Invalid recurring value';
        } // Check the recurring value, 1 = Daily, 2 = Weekly
        else if (command.port != null && common.validateInt(command.port, 1, 65535) == false) {
          err = 'Invalid port value';
        } // Check the port if present
        else if (
          command.recurring != null &&
          (command.end != null || command.start == null || command.expire == null)
        ) {
          err = 'Invalid recurring command';
        } else if (
          command.expire == null &&
          (command.start == null || command.end == null || command.start > command.end)
        ) {
          err = 'No time specified';
        } // Check that a time range is present
        else {
          if (command.nodeid.split('/').length == 1) {
            command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
          }
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'createDeviceShareLink', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        if (
          typeof domain.guestdevicesharing == 'object' &&
          typeof domain.guestdevicesharing.maxsessiontime == 'number' &&
          domain.guestdevicesharing.maxsessiontime > 0
        ) {
          const maxtime = domain.guestdevicesharing.maxsessiontime;
          if (command.expire != null && command.expire > maxtime) {
            command.expire = maxtime;
          }
          if (command.start != null && command.end != null) {
            if (command.end - command.start > maxtime * 60) {
              command.end = command.start + maxtime * 60;
            }
          }
        }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          if (startTime != null && expireTime != null) {
            ...
            command.expire = cookie.e = expireTime;
              ...
              try {
                ws.send(
                  JSON.stringify({
                    ...
                    responseid: command.responseid,
                    result: 'Unable to generate shareing cookie'
                  })
                );
              } catch (ex) {}
            }
          ...
          if (startTime != null && expireTime != null) {
            shareEntry.startTime = startTime;
            shareEntry.expireTime = expireTime;
          } else if (startTime != null && duration != null) {
            shareEntry.startTime = startTime;
            shareEntry.duration = duration;
          }
          if (command.recurring) {
            shareEntry.recurring = command.recurring;
          }
          if (command.viewOnly === true) {
          ...
          if (command.recurring == 1) {
            event = {
              ...
              msg: 'Added device share ' + command.guestname + ' recurring daily.',
              msgid: 138,
              msgArgs: [command.guestname],
              domain: domain.id
            };
            ...
            event = {
              ...
              nodeid: node._id,
              action: 'addedDeviceShare',
              msg: 'Added device share ' + command.guestname + ' recurring weekly.',
              msgid: 139,
              msgArgs: [command.guestname],
              domain: domain.id
            };
            ...
            event = {
              ...
              nodeid: node._id,
              action: 'addedDeviceShare',
              msg: 'Added device share: ' + command.guestname + '.',
              msgid: 101,
              msgArgs: [command.guestname, 'DATETIME:' + startTime, 'DATETIME:' + expireTime],
              domain: domain.id
            };
          } else {
            event = {
              ...
              nodeid: node._id,
              action: 'addedDeviceShare',
              msg: 'Added device share ' + command.guestname + ' with unlimited time.',
              msgid: 131,
              msgArgs: [command.guestname],
              domain: domain.id
            };
      ...
      case 'traceinfo': {
        ...
        if (
          ...
          (domain.myserver != null && domain.myserver !== true && domain.myserver.trace !== true)
        )
          break;
        ...
        if (user.siteadmin === SITERIGHT_ADMIN && typeof command.traceSources == 'object') {
          ...
          parent.parent.DispatchEvent(['*'], obj, {
            ...
            userid: user._id,
            username: user.name,
            traceSources: command.traceSources,
            nolog: 1,
            domain: domain.id
          });
        }
      ...
      case 'sendmqttmsg': {
        if (parent.parent.mqttbroker == null) {
          err = 'MQTT not supported on this server';
        } // MQTT not available
        if (common.validateArray(command.nodeids, 1) == false) {
          err = 'Invalid nodeids';
        } // Check nodeid's
        if (common.validateString(command.topic, 1, 64) == false) {
          err = 'Invalid topic';
        } // Check the topic
        if (common.validateString(command.msg, 1, 4096) == false) {
          err = 'Invalid msg';
        } // Check the message
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'sendmqttmsg', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
      ...
      case 'getmqttlogin': {
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'getmqttlogin', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
          ...
          if (rights == MESHRIGHT_ADMIN) {
            ...
            // Add MPS URL
            if (parent.parent.mpsserver != null) {
              r.mpsCertHashSha384 = parent.parent.certificateOperations.getCertHash(
                parent.parent.mpsserver.certificates.mps.cert
              );
              r.mpsCertHashSha1 = parent.parent.certificateOperations.getCertHashSha1(
                parent.parent.mpsserver.certificates.mps.cert
              );
              r.mpsUrl =
                'mqtts://' + serverName + ':' + (args.mpsaliasport != null ? args.mpsaliasport : args.mpsport) + '/';
            }
      ...
      case 'plugins': {
        ...
        parent.db.getPlugins(function (err, docs) {
          try {
            ws.send(JSON.stringify({ action: 'updatePluginList', list: docs, result: err }));
          } catch (ex) {}
        });
        break;
      }
      case 'pluginLatestCheck': {
        ...
        parent.parent.pluginHandler.getPluginLatest().then(function (latest) {
          try {
            ws.send(JSON.stringify({ action: 'pluginVersionsAvailable', list: latest }));
          } catch (ex) {}
        });
        break;
      }
      case 'addplugin': {
        if (user.siteadmin != SITERIGHT_ADMIN || parent.parent.pluginHandler == null) break; // Must be full admin with plugins enabled
        try {
          parent.parent.pluginHandler
            .getPluginConfig(command.url)
            ...
            .catch(function (err) {
              ...
              try {
                ws.send(JSON.stringify({ action: 'pluginError', msg: err }));
              } catch (er) {}
            });
        } catch (ex) {
          console.log('Cannot add plugin: ' + e);
        }
        break;
      }
      case 'installplugin': {
      ...
      case 'reloadplugin': {
        ...
        if (command.plugin == 'ALL') {
          ...
          parent.parent.pluginHandler.reloadAllPlugins(function (result) {
            try {
              ws.send(JSON.stringify({ action: 'pluginReloaded', result: result }));
            } catch (ex) {}
          });
        } else {
      ...
      case 'getpluginpermissions': {
        ...
        try {
          ws.send(JSON.stringify({ action: 'pluginPermissions', plugin: command.plugin, permissions: perms }));
        } catch (ex) {}
        break;
      }
      case 'setpluginpermissions': {
        ...
        parent.parent.pluginHandler.setPluginPermissions(command.plugin, command.data, function (err) {
          try {
            ws.send(
              JSON.stringify({ action: 'pluginPermissionsSet', plugin: command.plugin, success: !err, error: err })
            );
          } catch (ex) {}
        });
        break;
          ...
          parent.db.GetAllType('ugrp', function (err, ugrps) {
            ...
            parent.db.GetAllType('mesh', function (err, meshes) {
              ...
              parent.db.GetAllType('node', function (err, nodes) {
                ...
                try {
                  ws.send(JSON.stringify({ action: 'pluginPermissionList', list: result }));
                } catch (ex) {}
              });
            });
          });
      ...
      case 'getpluginversions': {
        if (user.siteadmin != SITERIGHT_ADMIN || parent.parent.pluginHandler == null) break; // Must be full admin with plugins enabled
        parent.parent.pluginHandler
          .getPluginVersions(command.id)
          .then(function (versionInfo) {
            try {
              ws.send(JSON.stringify({ action: 'downgradePluginVersions', info: versionInfo, error: null }));
            } catch (ex) {}
          })
          .catch(function (e) {
            try {
              ws.send(JSON.stringify({ action: 'pluginError', msg: e }));
            } catch (ex) {}
          });
        ...
        if (command.src != null && Array.isArray(command.src.selectedDevices)) {
          ...
          parent.GetNodesWithRights(domain, user, command.src.selectedDevices, function (nodes) {
            ...
            var message = {
              ...
              domain: domain.id,
              uisection: command.section,
              element: command.element
            };
            if (nodeids.length == 1) {
              message.nodeid = nodeids[0];
            }
            if (command.selectedDevices != null) {
              message.selectedDevices = command.selectedDevices;
            }
            if (command.src != null) {
              message.src = command.src;
            }
            if (command.values != null) {
              message.values = command.values;
            }
            if (typeof command.logmsg == 'string') {
              message.msg = command.logmsg;
            } else {
              message.nolog = 1;
            }
      ...
      case 'serverBackup': {
        ...
        if (command.service == 'googleDrive') {
          if (command.state == 0) {
            parent.db.Remove('GoogleDriveBackup', function () {
              try {
                ws.send(JSON.stringify({ action: 'serverBackup', service: 'googleDrive', state: 1 }));
              } catch (ex) {}
            });
            ...
            obj.oAuth2Client = new google.auth.OAuth2(
              command.clientid,
            ...
            const authUrl = obj.oAuth2Client.generateAuthUrl({
              access_type: 'offline',
              scope: ['https://www.googleapis.com/auth/drive.file']
            });
            try {
              ws.send(JSON.stringify({ action: 'serverBackup', service: 'googleDrive', state: 2, url: authUrl }));
            } catch (ex) {}
          } else if (command.state == 2 && obj.oAuth2Client != null) {
            obj.oAuth2Client.getToken(command.code, function (err, token) {
              if (err != null) {
                console.log('GoogleDrive (getToken) error: ', err);
                return;
              }
              parent.db.Set({
                _id: 'GoogleDriveBackup',
                state: 3,
                clientid: obj.oAuth2Client.xxclientid,
                clientsecret: obj.oAuth2Client.xxclientsecret,
                token: token
              });
              try {
                ws.send(JSON.stringify({ action: 'serverBackup', service: 'googleDrive', state: 3 }));
              } catch (ex) {}
            });
          }
        }
      ...
      case 'twoFactorCookie': {
        try {
          ...
          if (typeof maxCookieAge != 'number' || maxCookieAge < 1) {
            maxCookieAge = 30;
          }
          const twoFactorCookie = parent.parent.encodeCookie(
            { userid: user._id, expire: maxCookieAge * 24 * 60 /*, ip: req.clientIp*/ },
            parent.parent.loginCookieEncryptionKey
          );
          try {
            ws.send(JSON.stringify({ action: 'twoFactorCookie', cookie: twoFactorCookie }));
          } catch (ex) {}
        } catch (ex) {
          console.log(ex);
        }
      ...
      case 'amtsetupbin': {
        ...
        if (command.baremetal && parent.parent.amtProvisioningServer != null) {
          ...
          var bin = parent.parent.certificateOperations.GetBareMetalSetupBinFile(
            domain.amtacmactivation,
            command.oldmebxpass,
            command.newmebxpass,
            domain,
            user
          ...
          try {
            ws.send(JSON.stringify({ action: 'amtsetupbin', file: Buffer.from(bin, 'binary').toString('base64') }));
          } catch (ex) {}
        } else {
          // Create standard setup.bin
          var bin = parent.parent.certificateOperations.GetSetupBinFile(
            domain.amtacmactivation,
            command.oldmebxpass,
            command.newmebxpass,
            domain,
      ...
      case 'pushmessage': {
        ...
        db.Get(command.nodeid, function (err, nodes) {
          // TODO: Make a NodeRights(user) method that also does not do a db call if agent is connected (???)
          if (err == null && nodes.length == 1) {
            const node = nodes[0];
            if (
              (parent.GetNodeRights(user, node.meshid, node._id) & MESHRIGHT_CHATNOTIFY) != 0 &&
              typeof node.pmt == 'string'
            ) {
              // Send out a push message to the device
              ...
              parent.parent.firebase.sendToDevice(node, payload, options, function (id, err, errdesc) {
                if (err == null) {
                  parent.parent.debug(
                    'email',
                    'Successfully send push message to device ' +
                      node.name +
                      ', title: ' +
                      command.title +
                      ', msg: ' +
                  ...
                  parent.parent.debug(
                    'email',
                    'Failed to send push message to device ' +
                      node.name +
                      ', title: ' +
                      command.title +
                      ', msg: ' +
            ...
            if (parent.GetNodeRights(user, node.meshid, node._id) == MESHRIGHT_ADMIN && typeof node.pmt == 'string') {
              ...
              parent.parent.firebase.sendToDevice(node, payload, options, function (id, err, errdesc) {
                if (err != null) {
                  try {
                    ws.send(
                      JSON.stringify({ action: 'msg', type: 'console', nodeid: node._id, value: 'Failed: ' + errdesc })
                    );
                  } catch (ex) {}
                  parent.parent.debug(
                    'email',
                    'Failed to send push console message to device ' +
                      node.name +
                      ', command: ' +
                      command.console +
                      ', error: ' +
      ...
      case 'webpush': {
        ...
        if (
          typeof command.sub != 'object' &&
          typeof command.sub.keys != 'object' &&
          typeof command.sub.endpoint != 'string'
        )
          break;
        ...
        var newWebPush = {
          endpoint: command.sub.endpoint,
          keys: { auth: command.sub.keys.auth, p256dh: command.sub.keys.p256dh }
        };
        ...
        if (user.webpush == null) {
          ...
          for (var i in user.webpush) {
            if (
              user.webpush[i].endpoint == newWebPush.endpoint &&
              user.webpush[i].keys.auth == newWebPush.keys.auth &&
              user.webpush[i].keys.p256dh == newWebPush.keys.p256dh
            ) {
              found = true;
            }
          }
        ...
        if (command.userid == null) {
          ...
          if (db.GetUserLoginEvents) {
            ...
            db.GetUserLoginEvents(domain.id, user._id, function (err, docs) {
              ...
              try {
                ws.send(JSON.stringify({ action: 'previousLogins', events: e }));
              } catch (ex) {}
            });
          } else {
            // Old way
            db.GetUserEvents([user._id], domain.id, user._id, null, function (err, docs) {
              ...
              for (var i in docs) {
                if (
                  docs[i].msgArgs &&
                  docs[i].userid == user._id &&
                  (docs[i].action == 'authfail' || docs[i].action == 'login')
                ) {
                  e.push({ t: docs[i].time, m: docs[i].msgid, a: docs[i].msgArgs, tn: docs[i].tokenName });
                }
              }
            ...
            if (obj.crossDomain === true || splitUser[1] === domain.id) {
              if (db.GetUserLoginEvents) {
                ...
                db.GetUserLoginEvents(splitUser[1], command.userid, function (err, docs) {
                  ...
                  try {
                    ws.send(JSON.stringify({ action: 'previousLogins', userid: command.userid, events: e }));
                  } catch (ex) {}
                });
              } else {
                // Old way
      ...
      case 'oneclickrecovery': {
        // Intel(R) AMT One Click Recovery (OCR)
      ...
      case 'loginTokens': {
        // Respond with the list of currently valid login tokens
        ...
        if (common.validateStrArray(command.remove, 1) == false) {
        ...
        parent.db.GetAllTypeNodeFiltered(
          ...
          function (err, docs) {
            ...
            for (var i = 0; i < docs.length; i++) {
              const doc = docs[i];
              if (
                (doc.expire != 0 && doc.expire < now) ||
                doc.tokenUser == null ||
                (command.remove != null && command.remove.indexOf(doc.tokenUser) >= 0)
              ) {
                // This share is expired.
                parent.db.Remove(doc._id, function () {});
                removed.push(doc.tokenUser);
              } else {
                // This share is ok, remove extra data we don't need to send.
                delete doc._id;
                delete doc.domain;
            ...
            try {
              ws.send(JSON.stringify({ action: 'loginTokens', loginTokens: okDocs }));
            } catch (ex) {}
      ...
      case 'createLoginToken': {
        ...
        else if (typeof domain.passwordrequirements == 'object' && domain.passwordrequirements.logintokens === false) {
          err = 'Not supported';
        } // Login tokens are not supported on this server
        else if (
          typeof domain.passwordrequirements == 'object' &&
          Array.isArray(domain.passwordrequirements.logintokens) &&
          domain.passwordrequirements.logintokens.indexOf(user._id) < 0 &&
          user.links &&
          Object.keys(user.links).some((key) => domain.passwordrequirements.logintokens.indexOf(key) < 0)
        ) {
          err = 'Not supported';
        } // Login tokens are not supported by this user
        else if (common.validateString(command.name, 1, 100) == false) {
          err = 'Invalid name';
        } // Check name
        else if (typeof command.expire != 'number' || command.expire < 0) {
          err = 'Invalid expire value';
        } // Check expire
        ...
        if (err != null) {
          if (command.responseid != null) {
            try {
              ws.send(JSON.stringify({ action: 'createLoginToken', responseid: command.responseid, result: err }));
            } catch (ex) {}
          }
          break;
        }
        ...
        while (tokenUser.indexOf('+') >= 0 || tokenUser.indexOf('/') >= 0) {
          tokenUser = '~t:' + Buffer.from(parent.parent.crypto.randomBytes(12), 'binary').toString('base64');
        }
        var tokenPass = Buffer.from(parent.parent.crypto.randomBytes(15), 'binary').toString('base64');
        while (tokenPass.indexOf('+') >= 0 || tokenPass.indexOf('/') >= 0) {
          tokenPass = Buffer.from(parent.parent.crypto.randomBytes(15), 'binary').toString('base64');
        }
        ...
        require('./pass').hash(tokenPass, function (err, salt, hash, tag) {
          ...
          var expire = 0;
          if (command.expire > 0) {
            expire = created + command.expire * 60000;
          }

          // Generate the token password
          const dbentry = {
            _id: 'logintoken-' + tokenUser,
            type: 'logintoken',
            nodeid: 'logintoken-' + user._id,
            userid: user._id,
            name: command.name,
            tokenUser: tokenUser,
            salt: salt,
          ...
          var event = {
            ...
            userid: user._id,
            username: user.name,
            action: 'loginTokenAdded',
            msgid: 115,
            msg: 'Added login token',
            domain: domain.id,
            newToken: { name: command.name, tokenUser: tokenUser, created: created, expire: expire }
          };
          parent.parent.DispatchEvent(targets, obj, event);
        });
        break;
        ...
        db.GetAllTypeNoTypeFieldMeshFiltered(
          ...
          function (err, docs) {
            ...
            db.GetAllIdsOfType(ids, domain.id, 'lastconnect', function (err, docs) {
              ...
              if (docs != null) {
                for (var i in docs) {
                  lastConnects[docs[i]._id] = docs[i];
                }
              }
              ...
              getDeviceDetailedInfo(command.nodeids, command.type, function (results, type) {
                for (var i = 0; i < results.length; i++) {
                  ...
                  if (
                    (parent.GetNodeRights(user, results[i].node.meshid, results[i].node._id) &
                      MESHRIGHT_DEVICEDETAILS) ==
                    0
                  ) {
                  ...
                  if (lc != null) {
                    ...
                    delete lc.meshid;
                    delete lc.domain;
                    results[i].lastConnect = lc;
                  }

                  // Remove any connectivity and power state information, that should not be in the database anyway.
                  // TODO: Find why these are sometimes saved in the db.
                  if (results[i].node.conn != null) {
                    delete results[i].node.conn;
                  }
                  if (results[i].node.pwr != null) {
                    delete results[i].node.pwr;
                  }
                  if (results[i].node.agct != null) {
                    delete results[i].node.agct;
                  }
                  if (results[i].node.cict != null) {
                    delete results[i].node.cict;
                  }
                  ...
                  if (state) {
                    ...
                    if ((state.connectivity & 1) != 0) {
                      var agent = parent.wsagents[results[i].node._id];
                      if (agent != null) {
                        results[i].node.agct = agent.connectTime;
                      }
                    }
                    ...
                    if ((state.connectivity & 2) != 0) {
                      ...
                      if (ciraConnection != null && ciraConnection.tag != null) {
                        results[i].node.cict = ciraConnection.tag.connectTime;
                      }
                    }
                  }
                }
                ...
                if (type == 'csv') {
                    ...
                    for (var i = 0; i < results.length; i++) {
                      ...
                      if (nodeinfo.node != null) {
                        ...
                        output +=
                          ...
                          csvClean(n.name) +
                          ',' +
                          csvClean(n.rname ? n.rname : '') +
                          ',' +
                          csvClean(n.host ? n.host : '') +
                          ',' +
                          (n.icon ? n.icon : 1) +
                          ',' +
                          (n.ip ? n.ip : '') +
                          ',' +
                          (n.osdesc ? csvClean(n.osdesc) : '') +
                          ',' +
                          csvClean(parent.meshes[n.meshid].name);
                        if (typeof n.wsc == 'object') {
                          output +=
                            ',' +
                            csvClean(n.wsc.antiVirus ? n.wsc.antiVirus : '') +
                            ',' +
                            csvClean(n.wsc.autoUpdate ? n.wsc.autoUpdate : '') +
                            ',' +
                            csvClean(n.wsc.firewall ? n.wsc.firewall : '');
                        } else {
                          output += ',,,';
                        }
                        if (typeof n.volumes == 'object') {
                          var bitlockerdetails = '',
                            firstbitlocker = true;
                          for (var a in n.volumes) {
                            if (typeof n.volumes[a].protectionStatus !== 'undefined') {
                              if (firstbitlocker) {
                                firstbitlocker = false;
                              } else {
                                bitlockerdetails += '|';
                              }
                              bitlockerdetails += a + '/' + n.volumes[a].volumeStatus;
                            }
                          }
                        ...
                        if (typeof n.av == 'object') {
                          var avdetails = '',
                            firstav = true;
                          for (var a in n.av) {
                            if (typeof n.av[a].product == 'string') {
                              if (firstav) {
                                firstav = false;
                              } else {
                                avdetails += '|';
                              ...
                              avdetails +=
                                n.av[a].product +
                                '/' +
                                (n.av[a].enabled ? 'enabled' : 'disabled') +
                                '/' +
                                (n.av[a].updated ? 'updated' : 'notupdated');
                            }
                          }
                        ...
                        if (typeof n.tags == 'object') {
                          ...
                          for (var a in n.tags) {
                            ...
                            tagsdetails += n.tags[a];
                          }
                          output += ',' + csvClean(tagsdetails);
                        } else {
                          output += ',';
                        }
                        if (typeof n.lastbootuptime == 'number') {
                          output += ',' + n.lastbootuptime;
                        } else {
                          output += ',';
                        }
                      } else {
                      ...
                      if (nodeinfo.sys && nodeinfo.sys.hardware && nodeinfo.sys.hardware.windows) {
                        ...
                        output += ',';
                        if (
                          nodeinfo.sys.hardware.windows.cpu &&
                          nodeinfo.sys.hardware.windows.cpu.length > 0 &&
                          typeof nodeinfo.sys.hardware.windows.cpu[0].Name == 'string'
                        ) {
                          output += csvClean(nodeinfo.sys.hardware.windows.cpu[0].Name);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.windows.osinfo && nodeinfo.sys.hardware.windows.osinfo.BuildNumber) {
                          output += csvClean(nodeinfo.sys.hardware.windows.osinfo.BuildNumber);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_date) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_date);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_vendor) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_vendor);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_version) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_version);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_serial) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_mode) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_mode);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.board_name) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.board_name);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.board_vendor) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.board_vendor);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.board_version) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.board_version);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.board_serial) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.board_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.chassis_serial) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.chassis_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.chassis_assettag) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.chassis_assettag);
                        }
                        output += ',';
                        if (
                          nodeinfo.sys.hardware.identifiers &&
                          nodeinfo.sys.hardware.identifiers.chassis_manufacturer
                        ) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.chassis_manufacturer);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.product_uuid) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.product_uuid);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.SpecVersion) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.SpecVersion);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.ManufacturerId) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.ManufacturerId);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.ManufacturerVersion) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.ManufacturerVersion);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.IsActivated) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.IsActivated ? 'true' : 'false');
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.IsEnabled) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.IsEnabled ? 'true' : 'false');
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.IsOwned) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.IsOwned ? 'true' : 'false');
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.windows.memory) {
                          ...
                          for (var j in nodeinfo.sys.hardware.windows.memory) {
                            if (nodeinfo.sys.hardware.windows.memory[j].Capacity) {
                              if (typeof nodeinfo.sys.hardware.windows.memory[j].Capacity == 'number') {
                                totalMemory += nodeinfo.sys.hardware.windows.memory[j].Capacity;
                              }
                              if (typeof nodeinfo.sys.hardware.windows.memory[j].Capacity == 'string') {
                                totalMemory += parseInt(nodeinfo.sys.hardware.windows.memory[j].Capacity);
                              }
                            }
                          }
                        ...
                        output += ',';
                        output += ',';
                        if (nodeinfo.sys.hardware.mobile && nodeinfo.sys.hardware.mobile.bootloader) {
                          output += csvClean(nodeinfo.sys.hardware.mobile.bootloader);
                        }
                        output += ',';
                        output += ',';
                        output += ',';
                        if (nodeinfo.sys.hardware.mobile && nodeinfo.sys.hardware.mobile.model) {
                          output += csvClean(nodeinfo.sys.hardware.mobile.model);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.mobile && nodeinfo.sys.hardware.mobile.brand) {
                          output += csvClean(nodeinfo.sys.hardware.mobile.brand);
                        }
                        output += ',';
                        output += ',';
                        output += ',';
                        ...
                        output += ',';
                        output += ',';
                        if (nodeinfo.sys.hardware.mobile && nodeinfo.sys.hardware.mobile.id) {
                          output += csvClean(nodeinfo.sys.hardware.mobile.id);
                        }
                        output += ',';
                        output += ',';
                        output += ',';
                        ...
                        // Linux
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.cpu_name) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.cpu_name);
                        }
                        output += ',,';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.bios_date) {
                          output += csvClean(nodeinfo.sys.hardware.linux.bios_date);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.bios_vendor) {
                          output += csvClean(nodeinfo.sys.hardware.linux.bios_vendor);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.bios_version) {
                          output += csvClean(nodeinfo.sys.hardware.linux.bios_version);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.product_serial) {
                          output += csvClean(nodeinfo.sys.hardware.linux.product_serial);
                        } else if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_serial) {
                          output += csvClean(nodeinfo.sys.hardware.identifiers.bios_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.identifiers && nodeinfo.sys.hardware.identifiers.bios_mode) {
                        ...
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.board_name) {
                          output += csvClean(nodeinfo.sys.hardware.linux.board_name);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.board_vendor) {
                          output += csvClean(nodeinfo.sys.hardware.linux.board_vendor);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.board_version) {
                          output += csvClean(nodeinfo.sys.hardware.linux.board_version);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.board_serial) {
                          output += csvClean(nodeinfo.sys.hardware.linux.board_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.chassis_serial) {
                          output += csvClean(nodeinfo.sys.hardware.linux.chassis_serial);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.chassis_assettag) {
                          output += csvClean(nodeinfo.sys.hardware.linux.chassis_assettag);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.chassis_manufacturer) {
                          output += csvClean(nodeinfo.sys.hardware.linux.chassis_manufacturer);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.linux && nodeinfo.sys.hardware.linux.product_uuid) {
                          output += csvClean(nodeinfo.sys.hardware.linux.product_uuid);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.tpm && nodeinfo.sys.hardware.tpm.SpecVersion) {
                          output += csvClean(nodeinfo.sys.hardware.tpm.SpecVersion);
                          ...
                          if (nodeinfo.sys.hardware.linux.memory.Memory_Device) {
                            ...
                            for (var j in nodeinfo.sys.hardware.linux.memory.Memory_Device) {
                              if (nodeinfo.sys.hardware.linux.memory.Memory_Device[j].Size) {
                                if (typeof nodeinfo.sys.hardware.linux.memory.Memory_Device[j].Size == 'number') {
                                  totalMemory += nodeinfo.sys.hardware.linux.memory.Memory_Device[j].Size;
                                }
                                if (typeof nodeinfo.sys.hardware.linux.memory.Memory_Device[j].Size == 'string') {
                                  totalMemory += parseInt(nodeinfo.sys.hardware.linux.memory.Memory_Device[j].Size);
                                }
                              }
                            }
                      ...
                      if (nodeinfo.sys && nodeinfo.sys.hardware && nodeinfo.sys.hardware.agentvers) {
                        output += ',';
                        if (nodeinfo.sys.hardware.agentvers.openssl) {
                          output += csvClean(nodeinfo.sys.hardware.agentvers.openssl);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.agentvers.commitDate) {
                          output += csvClean(nodeinfo.sys.hardware.agentvers.commitDate);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.agentvers.commitHash) {
                          output += csvClean(nodeinfo.sys.hardware.agentvers.commitHash);
                        }
                        output += ',';
                        if (nodeinfo.sys.hardware.agentvers.compileTime) {
                          output += csvClean(nodeinfo.sys.hardware.agentvers.compileTime);
                        }
                      } else {
                        output += ',,,,';
                      }
                        ...
                        for (var j in nodeinfo.net.netif2) {
                          if (Array.isArray(nodeinfo.net.netif2[j])) {
                            for (var k = 0; k < nodeinfo.net.netif2[j].length; k++) {
                              if (typeof nodeinfo.net.netif2[j][k].mac == 'string') {
                                macs.push(nodeinfo.net.netif2[j][k].mac);
                              }
                              if (typeof nodeinfo.net.netif2[j][k].address == 'string') {
                                addresses.push(nodeinfo.net.netif2[j][k].address);
                              }
                            }
                          }
                        }
                      ...
                      if (nodeinfo.lastConnect) {
                        ...
                        if (nodeinfo.lastConnect.time) {
                          ...
                          if (typeof command.l == 'string' && typeof command.tz == 'string') {
                            output += csvClean(
                              new Date(nodeinfo.lastConnect.time).toLocaleString(command.l, { timeZone: command.tz })
                            );
                          } else {
                        ...
                        output += ',';
                        if (typeof nodeinfo.lastConnect.addr == 'string') {
                          output += csvClean(nodeinfo.lastConnect.addr);
                        } // Last connection address and port
                      } else {
                        output += ',,';
                      }
                  ...
                  for (var i = 0; i < results.length; i++) {
                    ...
                    if (nodeinfo.node) {
                      ...
                      if (mesh) {
                        results[i].node.groupname = mesh.name;
                      }
                    }
                  }
                ...
                try {
                  ws.send(JSON.stringify({ action: 'getDeviceDetails', data: output, type: type }));
                } catch (ex) {}
              });
            });
          }
      ...
      case 'endDesktopMultiplex': {
        var err = null,
          xuser = null;
        try {
          if (command.xuserid.indexOf('/') < 0) {
            command.xuserid = 'user/' + domain.id + '/' + command.xuserid;
          }
          if (common.validateString(command.nodeid, 1, 1024) == false) {
            err = 'Invalid device identifier';
          } // Check the meshid
          ...
          if (xuser == null) {
            err = 'User does not exists';
          } else if (obj.crossDomain !== true && (xusersplit.length != 3 || xusersplit[1] != domain.id)) {
            err = 'Invalid domain';
          } // Invalid domain, operation only valid for current domain
        } catch (ex) {
          err = 'Validation exception: ' + ex;
        ...
        parent.GetNodeWithRights(domain, xuser, command.nodeid, function (node, rights, visible) {
          ...
          for (var i = 0; i < desktopRelay.viewers.length; i++) {
            ...
            if (viewer.user._id == command.xuserid && viewer.guestName == command.guestname) {
              viewersToClose.push(viewer);
            } // Only close viewers that match the userid and guestname if present.
          }
          for (var i = 0; i < viewersToClose.length; i++) {
            viewersToClose[i].close();
          } // Close any viewers we need closed.
          ...
          if (command.guestname != null) {
            xusername += '/' + command.guestname;
          }
          const event = {
            etype: 'user',
            userid: user._id,
            username: user.name,
            nodeid: command.nodeid,
            xuserid: command.xuserid,
            action: 'endsession',
            msgid: 134,
            msgArgs: [xusername],
            msg: 'Forcibly disconnected desktop session of user ' + xusername,
            domain: domain.id
          };
          if (command.guestname != null) {
      ...
      case 'satellite': {
        ...
        if (command.setFlags != null && typeof command.setFlags == 'number') {
          obj.ws.satelliteFlags = command.setFlags;
        }
        if (command.reqid != null && typeof command.satelliteFlags == 'number') {
          const event = {
            action: 'satelliteResponse',
            subaction: command.subaction,
            reqid: command.reqid,
            response: command.response,
            satelliteFlags: command.satelliteFlags,
            nolog: 1
          };
          if (typeof command.nodeid == 'string') {
      ...
      case 'importamtdevices': {
        if (
          command.amtdevices == null ||
          command.meshid == null ||
          typeof command.meshid != 'string' ||
          command.meshid.startsWith('mesh/' + domain.id + '/') == false
        )
          return;
        const mesh = parent.meshes[command.meshid];
        if (mesh == null || mesh.mtype != 1 || (parent.GetMeshRights(user, command.meshid) & MESHRIGHT_EDITMESH) == 0)
          return null; // This user must have mesh rights to edit the device group
        var amtDevices = [];

        // Decode a JSON file from the Intel SCS migration tool
        if (
          typeof command.amtdevices == 'object' &&
          typeof command.amtdevices.ApplicationData == 'object' &&
          command.amtdevices.ApplicationData.Application == 'Intel vPro(R) Manageability Migration Tool' &&
          typeof command.amtdevices['ManagedSystems'] == 'object' &&
          Array.isArray(command.amtdevices['ManagedSystems']['ManagedSystemsList'])
        ) {
          for (var i in command.amtdevices['ManagedSystems']['ManagedSystemsList']) {
            ...
            var host = null;
            if (typeof importDev.Fqdn == 'string' && importDev.Fqdn != '') {
              host = importDev.Fqdn;
            }
            if (host == null && typeof importDev.IPv4 == 'string' && importDev.IPv4 != '') {
              host = importDev.IPv4;
            }
            if (host != null) {
              ...
              const nodeid =
                ...
                domain.id +
                '/' +
                parent.crypto.randomBytes(48).toString('base64').replace(/\+/g, '@').replace(/\//g, '$');
              const device = {
                type: 'node',
                _id: nodeid,
                meshid: mesh._id,
                ...
                host: host,
                domain: domain.id,
                intelamt: { user: 'admin', state: 2 }
              };

              // Add optional fields
              if (typeof importDev.AmtVersion == 'string') {
                device.intelamt.ver = importDev.AmtVersion;
              }
              if (typeof importDev.ConfiguredPassword == 'string') {
                device.intelamt.pass = importDev.ConfiguredPassword;
              }
              if (typeof importDev.Uuid == 'string') {
                device.intelamt.uuid = importDev.Uuid;
              }
              if (importDev.ConnectionType == 'TLS') {
                device.intelamt.tls = 1;
              }

              // Check if we are already adding a device with the same hostname, if so, skip it.
              var skip = false;
              for (var i in amtDevices) {
                if (amtDevices[i].host.toLowerCase() == device.host.toLowerCase()) {
                  skip = true;
                }
              }
              if (skip == false) {
                amtDevices.push(device);
              }
            }
          }
        }
        ...
        if (
          typeof command.amtdevices == 'object' &&
          typeof command.amtdevices.webappversion == 'string' &&
          Array.isArray(command.amtdevices.computers)
        ) {
          for (var i in command.amtdevices.computers) {
            const importDev = command.amtdevices.computers[i];
            if (
              typeof importDev.host == 'string' &&
              importDev.host != '' &&
              importDev.host != '127.0.0.1' &&
              importDev.host.toLowerCase() != 'localhost'
            ) {
              // Create a new Intel AMT device
              const nodeid =
                'node/' +
              ...
              if (typeof importDev.name == 'string') {
                device.name = importDev.name;
              } else {
                device.name = importDev.host;
              }

              // Add optional fields
              if (typeof importDev.user == 'string') {
                device.intelamt.user = importDev.user;
              }
              if (typeof importDev.pass == 'string') {
                device.intelamt.pass = importDev.pass;
              }
              if (importDev.tls === true || importDev.tls === 1) {
                device.intelamt.tls = 1;
              }
              if (typeof importDev.digestrealm == 'string') {
                device.intelamt.realm = importDev.digestrealm;
              }
              if (typeof importDev.ver == 'string') {
                device.intelamt.ver = importDev.ver;
              }
              if (typeof importDev.uuid == 'string') {
                device.intelamt.uuid = importDev.uuid;
              }
              if (typeof importDev.pstate == 'number') {
                device.intelamt.state = importDev.pstate;
              }
              if (typeof importDev.tlscerthash == 'string') {
                device.intelamt.hash = importDev.tlscerthash;
              }
              if (typeof importDev.icon == 'number') {
                device.icon = importDev.icon;
              }
              if (typeof importDev.desc == 'string') {
                device.desc = importDev.desc;
              }
        ...
        if (Array.isArray(command.amtdevices)) {
          for (var i in command.amtdevices) {
            ...
            if (
              typeof importDev.fqdn == 'string' &&
              importDev.fqdn != '' &&
              importDev.fqdn != '127.0.0.1' &&
              importDev.fqdn.toLowerCase() != 'localhost'
            ) {
              // Create a new Intel AMT device
              const nodeid =
                'node/' +
              ...
              if (typeof importDev.name == 'string') {
                ...
                device.name = importDev.fqdn;
              }

              // Add optional fields
              if (typeof importDev.username == 'string') {
                device.intelamt.user = importDev.username;
              }
              if (typeof importDev.password == 'string') {
                device.intelamt.pass = importDev.password;
              }
              if (importDev.tls === true || importDev.tls === 1) {
                device.intelamt.tls = 1;
              }
              if (typeof importDev.version == 'string') {
                device.intelamt.ver = importDev.version;
              }
              if (typeof importDev.digestrealm == 'string') {
                device.intelamt.realm = importDev.digestrealm;
              }
        ...
        for (var i in amtDevices) {
          ...
          parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(command.meshid, [nodeid]), obj, {
            ...
            username: user.name,
            action: 'addnode',
            node: parent.CloneSafeNode(amtDevices[i]),
            msgid: 84,
            msgArgs: [amtDevices[i].name, mesh.name],
            msg: 'Added device ' + amtDevices[i].name + ' to device group ' + mesh.name,
            domain: domain.id
          });
        }
        break;
  ...
  const serverUserCommands = {
    ...
    info: [
      serverUserCommandInfo,
      'Returns the most immidiatly useful information about this server, including MeshCentral and NodeJS versions. This is often information required to file a bug. Optionally use info h for human readable form.'
    ],
    ...
    resetserver: [
      serverUserCommandResetServer,
      'Causes the server to reset, this is sometimes useful is the config.json file was changed.'
    ],
    serverupdate: [
      serverUserCommandServerUpdate,
      'Updates server to latest version. Optional version argument to install specific version. Example: serverupdate 0.8.49'
    ],
    ...
    tasklimiter: [
      serverUserCommandTaskLimiter,
      "Returns the internal status of the tasklimiter. This is a system used to smooth out work done by the server. It's used by, for example, agent updates so that not all agents are updated at the same time."
    ],
  ...
  function serverCommandAddDeviceUser(command) {
    if (typeof command.userid == 'string') {
      command.userids = [command.userid];
    }
    var err = null;
    try {
      if (common.validateString(command.nodeid, 1, 1024) == false) {
      ...
      else if (common.validateInt(command.rights) == false) {
        err = 'Invalid rights';
      } // Device rights must be an integer
      else if ((command.rights & 7) != 0) {
        err = 'Invalid rights';
      } // EDITMESH, MANAGEUSERS or MANAGECOMPUTERS rights can't be assigned to a user to device link
      else if (
        common.validateStrArray(command.usernames, 1, 128) == false &&
        common.validateStrArray(command.userids, 1, 128) == false
      ) {
        err = 'Invalid usernames';
      } // Username is between 1 and 128 characters
      else {
        if (command.nodeid.indexOf('/') == -1) {
          command.nodeid = 'node/' + domain.id + '/' + command.nodeid;
        } else if (command.nodeid.split('/').length != 3 || command.nodeid.split('/')[1] != domain.id) {
    ...
    if (err != null) {
      if (command.responseid != null) {
        obj.send({ action: 'adddeviceuser', responseid: command.responseid, result: err });
      }
      return;
    }
    ...
    if (command.userids == null) {
      ...
      for (var i in command.usernames) {
        if (command.usernames[i] != null) {
          if (parent.users['user/' + domain.id + '/' + command.usernames[i].toLowerCase()] != null) {
            command.userids.push('user/' + domain.id + '/' + command.usernames[i].toLowerCase());
          } else if (parent.users['user/' + domain.id + '/' + command.usernames[i]] != null) {
            command.userids.push('user/' + domain.id + '/' + command.usernames[i]);
          }
        }
      }
    }
    ...
    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
      ...
      for (var i in command.userids) {
        ...
        // Add a user
        var newuser = null;
        if (newuserid.startsWith('ugrp/')) {
          newuser = parent.userGroups[newuserid];
        }
        if (newuserid.startsWith('user/')) {
          ...
          if (newuser == null && newuserid.startsWith('user/' + domain.id + '/*\\') == true) {
            ...
            for (var i in parent.users) {
              if (i.endsWith(search) && parent.users[i].domain == domain.id) {
                newuser = parent.users[i];
                command.userids[i] = newuserid = newuser._id;
                break;
              }
            }
          }
        ...
        if (newuser != null) {
          ...
          if (command.remove === true) {
            ...
            if (newuser.links != null) {
              ...
              if (Object.keys(newuser.links).length == 0) {
                delete newuser.links;
              }
            }
          ...
          if (newuserid.startsWith('user/')) {
            ...
            if (command.rights == 0) {
              event = {
                ...
                action: 'accountchange',
                msgid: 81,
                msgArgs: [newuser.name],
                msg: 'Removed user device rights for ' + newuser.name,
                domain: domain.id,
                account: parent.CloneSafeUser(newuser),
                nodeListChange: newuserid
              };
            } else {
              event = {
                etype: 'user',
                ...
                msgid: 82,
                msgArgs: [newuser.name],
                msg: 'Changed user device rights for ' + newuser.name,
                domain: domain.id,
                account: parent.CloneSafeUser(newuser),
                nodeListChange: newuserid
              };
            ...
            var event = {
              ...
              msgid: 79,
              msgArgs: [newuser.name],
              msg: 'User group changed: ' + newuser.name,
              domain: domain.id
            };
            if (db.changeStream) {
              event.noact = 1;
      ...
      if (command.responseid != null) {
        obj.send({ action: 'adddeviceuser', responseid: command.responseid, result: 'ok' });
      }
    });
  }
  ...
  function serverCommandAddMeshUser(command) {
    ...
    try {
      if (common.validateString(command.meshid, 8, 134) == false) {
      ...
      else if (common.validateInt(command.meshadmin) == false) {
        err = 'Invalid group rights';
      } // Mesh rights must be an integer
      else if (
        common.validateStrArray(command.usernames, 1, 128) == false &&
        common.validateStrArray(command.userids, 1, 128) == false
      ) {
        ...
        if (mesh == null) {
          err = 'Unknown group';
        } else if (((selfMeshRights = parent.GetMeshRights(user, mesh)) & MESHRIGHT_MANAGEUSERS) == 0) {
          err = 'Permission denied';
        } else if (meshIdSplit.length != 3 || meshIdSplit[1] != domain.id) {
          err = 'Invalid domain';
        } // Invalid domain, operation only valid for current domain
      }
    } catch (ex) {
    ...
    if (err != null) {
      if (command.responseid != null) {
        obj.send({ action: 'addmeshuser', responseid: command.responseid, result: err });
      }
      return;
    }
    ...
    for (var i in command.userids) {
      ...
      if (newuser == null && newuserid.startsWith('user/' + domain.id + '/*\\') == true) {
        ...
        for (var i in parent.users) {
          if (i.endsWith(search) && parent.users[i].domain == domain.id) {
            newuser = parent.users[i];
            command.userids[i] = newuserid = parent.users[i]._id;
            break;
          }
        }
      }
      ...
      if (meshIdSplit[1] != newuserid.split('/')[1]) {
        msgs.push('Mismatch domains');
        continue;
      }

      if (newuser != null) {
        // Can't add or modify self
        if (newuserid == obj.user._id) {
          msgs.push("Can't change self");
          continue;
        }
        ...
        if (
          newuser.links != null &&
          newuser.links[command.meshid] != null &&
          newuser.links[command.meshid].rights != null
        ) {
          targetMeshRights = newuser.links[command.meshid].rights;
        }
        if (targetMeshRights === MESHRIGHT_ADMIN && selfMeshRights != MESHRIGHT_ADMIN) {
          msgs.push("Can't change rights of device group administrator");
          continue;
        } // A non-admin can't kick out an admin
        ...
        if (command.remove === true) {
          ...
          if (selfMeshRights != MESHRIGHT_ADMIN && command.meshadmin == MESHRIGHT_ADMIN) {
            msgs.push("Can't set device group administrator, if not administrator");
            continue;
          }
          if ((selfMeshRights & 2) == 0 && (command.meshadmin & 2) != 0 && (targetMeshRights & 2) == 0) {
            command.meshadmin -= 2;
          }
          ...
          if (newuser.links[command.meshid]) {
        ...
        if (newuserid.startsWith('user/')) {
          ...
          var event = {
            ...
            msgid: 78,
            msgArgs: [newuser.name],
            msg: 'Device group membership changed: ' + newuser.name,
            domain: domain.id
          };
          if (db.changeStream) {
            event.noact = 1;
        ...
        if (command.remove === true) {
          ...
          event = {
            ...
            action: 'meshchange',
            links: mesh.links,
            msg: 'Removed user ' + newuser.name + ' from device group ' + mesh.name,
            domain: domain.id,
            invite: mesh.invite
          };
        } else {
          ...
          event = {
            ...
            action: 'meshchange',
            links: mesh.links,
            msg: 'Added user ' + newuser.name + ' to device group ' + mesh.name,
            domain: domain.id,
            invite: mesh.invite
          };
        }
        ...
        if (command.remove === true) {
          msgs.push('Removed user ' + newuserid.split('/')[2]);
        } else {
    ...
    if (successCount == 0 && failCount == 0) {
      msgs.push('Nothing done');
    }

    if (unknownUsers.length > 0) {
      // Send error back, user not found.
      displayNotificationMessage(
        'User' + (unknownUsers.length > 1 ? 's' : '') + ' ' + EscapeHtml(unknownUsers.join(', ')) + ' not found.',
        'Device Group',
        'ServerNotify',
        5,
        unknownUsers.length > 1 ? 16 : 15,
        [EscapeHtml(unknownUsers.join(', '))]
      );
    }
  ...
  function serverCommandAddUser(command) {
    ...
    if (command.randomPassword === true) {
      command.pass = getRandomPassword();
    }
    ...
    var err = null,
      ...
      newuserdomain;
    try {
      if ((user.siteadmin & MESHRIGHT_MANAGEUSERS) == 0) {
        err = 'Permission denied';
        errid = 1;
      } else if (common.validateUsername(command.username, 1, 256) == false) {
        err = 'Invalid username';
        errid = 2;
      } // Username is between 1 and 64 characters, no spaces
      else if (command.username[0] == '~' || command.username.indexOf('/') >= 0) {
        err = 'Invalid username';
        errid = 2;
      } // Usernames cant' start with ~ and can't have '/'
      else if (common.validateString(command.pass, 1, 256) == false) {
        err = 'Invalid password';
        errid = 3;
      } // Password is between 1 and 256 characters
      else if (
        command.randomPassword !== true &&
        common.checkPasswordRequirements(command.pass, domain.passwordrequirements) == false
      ) {
        err = 'Invalid password';
        errid = 3;
      } // Password does not meet requirements
      else if (command.email != null && common.validateEmail(command.email, 1, 1024) == false) {
        err = 'Invalid email';
        errid = 4;
        ...
        obj.crossDomain === true &&
        command.domain != null &&
        (typeof command.domain != 'string' || parent.parent.config.domains[command.domain] == null)
      ) {
        err = 'Invalid domain';
        errid = 5;
        ...
        !common.validateEmailDomain(command.email, domain.newaccountemaildomains)
      ) {
        err = 'Email domain is not allowed. Only (' + domain.newaccountemaildomains.join(', ') + ') are allowed.';
        errid = 30;
        args = [common.getEmailDomain(command.email), domain.newaccountemaildomains.join(', ')];
      } else {
        newuserdomain = domain;
        if (obj.crossDomain === true && command.domain != null) {
          newuserdomain = parent.parent.config.domains[command.domain];
        }
        newusername = command.username;
        newuserid = 'user/' + newuserdomain.id + '/' + command.username.toLowerCase();
        if (command.siteadmin != null) {
          if (typeof command.siteadmin != 'number' || Number.isInteger(command.siteadmin) == false) {
            err = 'Invalid site permissions';
            errid = 6;
          } // Check permissions
          else if (user.siteadmin != SITERIGHT_ADMIN && (command.siteadmin & (SITERIGHT_ADMIN - 224)) != 0) {
            err = 'Invalid site permissions';
            errid = 6;
          }
        }
        if (parent.users[newuserid]) {
          err = 'User already exists';
          errid = 7;
        } // Account already exists
        else if (newuserdomain.auth == 'sspi' || newuserdomain.auth == 'ldap') {
          err = 'Unable to add user in this mode';
          errid = 8;
        }
      }
    } catch (ex) {
      err = 'Validation exception';
      errid = 9;
    }
    ...
    db.isMaxType(newuserdomain.limits.maxuseraccounts, 'user', newuserdomain.id, function (maxExceed) {
      if (maxExceed) {
        ...
        if (command.responseid != null) {
          ...
          if (sessions != null) {
            for (var i in sessions) {
              try {
                if (sessions[i].domainid == newuserdomain.id) {
                  sessions[i].send(JSON.stringify(notification));
                }
              } catch (ex) {}
            }
        ...
        if (command.removeEvents === true) {
          db.RemoveAllUserEvents(newuserdomain.id, newuserid);
        }
        ...
        if (command.siteadmin != null) {
          newuser.siteadmin = command.siteadmin;
        } else if (newuserdomain.newaccountsrights) {
          newuser.siteadmin = newuserdomain.newaccountsrights;
        }
        if (command.email != null) {
          newuser.email = command.email.toLowerCase();
          if (command.emailVerified === true) {
            newuser.emailVerified = true;
          }
        } // Email
        if (command.resetNextLogin === true) {
          newuser.passchange = -1;
        } else {
          newuser.passchange = Math.floor(Date.now() / 1000);
        }
        if (user.groups) {
          newuser.groups = user.groups;
        } // New accounts are automatically part of our groups (Realms).
        if (common.validateString(command.realname, 1, 256)) {
          newuser.realname = command.realname;
        }
        if (command.consent != null && typeof command.consent == 'number') {
          if (command.consent == 0) {
            delete chguser.consent;
          } else {
            newuser.consent = command.consent;
          }
          change = 1;
        }
        if (
        ...
        if (typeof newuserdomain.newaccountsusergroups == 'object') {
          for (var i in newuserdomain.newaccountsusergroups) {
            ...
            if (ugrpid.indexOf('/') < 0) {
              ugrpid = 'ugrp/' + newuserdomain.id + '/' + ugrpid;
            }
            ...
            if (ugroup != null) {
              ...
              var event = {
                ...
                links: ugroup.links,
                msgid: 80,
                msgArgs: [newuser.name, ugroup.name],
                msg: 'Added user ' + newuser.name + ' to user group ' + ugroup.name,
                addUserDomain: newuserdomain.id
              };
              if (db.changeStream) {
                event.noact = 1;
              } // If DB change stream is active, don't use this event to change the user group. Another event will come.
            ...
            if (err == null) {
              ...
              if (command.email == null) {
                event = {
                  ...
                  msgid: 64,
                  msgArgs: [command.username],
                  msg: 'Account created, username is ' + command.username,
                  domain: newuserdomain.id
                };
              } else {
                event = {
                  ...
                  action: 'accountcreate',
                  msgid: 65,
                  msgArgs: [command.email.toLowerCase()],
                  msg: 'Account created, email is ' + command.email.toLowerCase(),
                  domain: newuserdomain.id
                };
              }
              if (parent.db.changeStream) {
              ...
              // Log in the auth log
              if (parent.parent.authlog) {
                parent.parent.authLog('https', 'User ' + user.name + ' created a user account ' + newuser.name);
              }

              // OK Response
              if (command.responseid != null) {
                obj.send({ action: 'adduser', responseid: command.responseid, result: 'ok' });
              }
            } else {
              if (command.responseid != null) {
                obj.send({ action: 'adduser', responseid: command.responseid, result: 'passwordHashError' });
              }
            }
          },
          0
  ...
  function serverCommandAddUserBatch(command) {
    ...
    if ((user.siteadmin & 2) == 0) {
      err = 'Access denied';
    } else if (domain.auth == 'sspi' || domain.auth == 'ldap') {
      err = 'Unable to create users when in SSPI or LDAP mode';
    } else if (!Array.isArray(command.users)) {
      err = 'Invalid users';
    } else {
      var userCount = 0;
      for (var i in command.users) {
        if (domain.usernameisemail) {
          if (command.users[i].email) {
            command.users[i].user = command.users[i].email;
          } else {
            command.users[i].email = command.users[i].user;
          }
        } // If the email is the username, set this here.
        if (common.validateUsername(command.users[i].user, 1, 256) == false) {
          err = 'Invalid username';
        } // Username is between 1 and 64 characters, no spaces
        if (command.users[i].user[0] == '~' || command.users[i].user.indexOf('/') >= 0) {
          err = 'Invalid username';
        } // This is a reserved user name or invalid name
        if (common.validateString(command.users[i].pass, 1, 256) == false) {
          err = 'Invalid password';
        } // Password is between 1 and 256 characters
        if (common.checkPasswordRequirements(command.users[i].pass, domain.passwordrequirements) == false) {
          err = 'Invalid password';
        } // Password does not meet requirements
        if (command.users[i].email != null && common.validateEmail(command.users[i].email, 1, 1024) == false) {
          err = 'Invalid email';
        } // Check if this is a valid email address
        userCount++;
      }
    ...
    if (err != null) {
      if (command.responseid != null) {
        obj.send({ action: 'adduserbatch', responseid: command.responseid, result: err });
      }
      return;
    }
    ...
    db.isMaxType(domain.limits.maxuseraccounts + userCount, 'user', domain.id, function (maxExceed) {
      if (maxExceed) {
        ...
        if (sessions != null) {
          for (var i in sessions) {
            try {
              if (sessions[i].domainid == domain.id) {
                sessions[i].send(JSON.stringify(notification));
              }
            } catch (ex) {}
          }
        ...
        for (var i in command.users) {
          ...
          var newuser = {
            type: 'user',
            _id: newuserid,
            name: command.users[i].user,
            creation: Math.floor(Date.now() / 1000),
            domain: domain.id
          };
          if (domain.newaccountsrights) {
            newuser.siteadmin = domain.newaccountsrights;
          }
          if (common.validateString(command.users[i].realname, 1, 256)) {
            newuser.realname = command.users[i].realname;
          }
          if (command.users[i].email != null) {
            newuser.email = command.users[i].email.toLowerCase();
            if (command.users[i].emailVerified === true) {
              newuser.emailVerified = true;
            }
          } // Email, always lowercase
          if (command.users[i].resetNextLogin === true) {
            newuser.passchange = -1;
          } else {
            newuser.passchange = Math.floor(Date.now() / 1000);
          }
          ...
          if (parent.users[newuserid] == null) {
            ...
            // Create a user, generate a salt and hash the password
            require('./pass').hash(
              command.users[i].pass,
              function (err, salt, hash, newuser) {
                if (err) throw err;
                newuser.salt = salt;
                newuser.hash = hash;
                db.SetUser(newuser);
                ...
                if (newuser.email == null) {
                  event = {
                    ...
                    msgid: 64,
                    msgArgs: [newuser.name],
                    msg: 'Account created, username is ' + newuser.name,
                    domain: domain.id
                  };
                } else {
                  event = {
                    ...
                    action: 'accountcreate',
                    msgid: 65,
                    msgArgs: [newuser.email],
                    msg: 'Account created, email is ' + newuser.email,
                    domain: domain.id
                  };
                }
                if (parent.db.changeStream) {
                ...
                // Log in the auth log
                if (parent.parent.authlog) {
                  parent.parent.authLog('https', 'User ' + user.name + ' created user account ' + newuser.name);
                }
              },
              newuser
            );
  ...
  function serverCommandAddUserToUserGroup(command) {
    ...
    try {
      ...
      else if (common.validateStrArray(command.usernames, 1, 128) == false) {
        err = 'Invalid usernames';
      } // Username is between 1 and 128 characters
      else {
        var ugroupidsplit = command.ugrpid.split('/');
    ...
    if (obj.crossDomain === true) {
      addUserDomain = parent.parent.config.domains[ugroupidsplit[1]];
    }
    if (addUserDomain == null) {
      err = 'Invalid domain';
    }
    ...
    if (err != null) {
      if (command.responseid != null) {
        obj.send({ action: 'addusertousergroup', responseid: command.responseid, result: err });
      }
      return;
    }
    ...
    if (group != null) {
      ...
      for (var i in command.usernames) {
        ...
        if (chguser == null) {
          chguserid = 'user/' + addUserDomain.id + '/' + command.usernames[i];
          chguser = parent.users[chguserid];
        }
        if (chguser != null) {
          // Add usr group to user
          if (chguser.links == null) {
            chguser.links = {};
          }
          ...
          var event = {
            ...
            msgid: 67,
            msgArgs: [chguser.name],
            msg: 'User group membership changed: ' + chguser.name,
            domain: addUserDomain.id
          };
          if (db.changeStream) {
            event.noact = 1;
          } // If DB change stream is active, don't use this event to change the user. Another event will come.
      ...
      if (addedCount > 0) {
        ...
        var event = {
          ...
          links: group.links,
          msgid: 71,
          msgArgs: [knownUsers.map((u) => u.name), group.name],
          msg: 'Added user(s) ' + knownUsers.map((u) => u.name) + ' to user group ' + group.name,
  ...
  function serverCommandAuthCookie(command) {
    try {
      ws.send(
        JSON.stringify({
          action: 'authcookie',
          cookie: parent.parent.encodeCookie(
            { userid: user._id, domainid: domain.id, ip: req.clientIp },
            parent.parent.loginCookieEncryptionKey
          ),
          rcookie: parent.parent.encodeCookie(
            { ruserid: user._id, x: req.session.x },
            parent.parent.loginCookieEncryptionKey
          )
        })
      ...
      db.GetUserWithVerifiedEmail(domain.id, command.email, function (err, docs) {
        if (docs != null && docs.length > 0) {
          ...
          obj.send({
            ...
            id: Math.random(),
            tag: 'ServerNotify',
            value: 'Failed to change email address, another account already using: ' + command.email + '.',
            titleid: 4,
            msgid: 13,
            args: [command.email]
          });
          ...
          if (parent.parent.authlog) {
            parent.parent.authLog(
              'https',
              'User ' + user.name + ' changed email from ' + oldemail + ' to ' + user.email
            );
          }
  ...
  function serverCommandChangeLang(command) {
    ...
    var message = {
      ...
      domain: domain.id,
      msgid: 3,
      msgArgs: ['', oldlang ? oldlang : 'default', user.lang ? user.lang : 'default']
    };
    if (db.changeStream) {
      message.noact = 1;
    } // If DB change stream is active, don't use this event to change the user. Another event will come.
    message.msg =
      'Changed language from ' + (oldlang ? oldlang : 'default') + ' to ' + (user.lang ? user.lang : 'default');
  ...
  function serverCommandClose(command) {
    ...
    try {
      if (obj.req.session.ws == ws) delete obj.req.session.ws;
    } catch (e) {}
    try {
      ws.close();
    } catch (e) {}
  ...
  function serverCommandConfirmPhone(command) {
    ...
    if (user.siteadmin != 0xffffffff && (user.siteadmin & 1024) != 0) return; // If this account is settings locked, return here.
    if (
      parent.parent.smsserver == null ||
      typeof command.cookie != 'string' ||
    ...
    var event = {
      ...
      msgid: 96,
      msgArgs: [user.name],
      msg: 'Verified phone number of user ' + EscapeHtml(user.name),
      domain: domain.id
    };
    if (db.changeStream) {
      event.noact = 1;
  ...
  function serverCommandConfirmMessaging(command) {
    ...
    var event = {
      ...
      msgid: 156,
      msgArgs: [user.name],
      msg: 'Verified messaging account of user ' + EscapeHtml(user.name),
      domain: domain.id
    };
    if (db.changeStream) {
      event.noact = 1;
  ...
  function serverCommandEmailUser(command) {
    var errMsg = null,
      emailuser = null;
    if (domain.mailserver == null) {
      errMsg = 'Email server not enabled';
    } else if ((user.siteadmin & 2) == 0) {
      errMsg = 'No user management rights';
    } else if (common.validateString(command.userid, 1, 2048) == false) {
      errMsg = 'Invalid userid';
    } else if (common.validateString(command.subject, 1, 1000) == false) {
      errMsg = 'Invalid subject message';
    } else if (common.validateString(command.msg, 1, 10000) == false) {
      ...
      if (emailuser == null) {
        ...
        errMsg = 'No validated email address for this user';
      } else if (emailuser.emailVerified !== true) {
        errMsg = 'No validated email address for this user';
      }
    }

    if (errMsg != null) {
      displayNotificationMessage(errMsg);
      return;
    }
  ...
  function serverCommandGetClip(command) {
    ...
    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
      ...
      if (agent != null) {
        try {
          agent.send(JSON.stringify({ action: 'getClip' }));
        } catch (ex) {}
      }
    });
  }
  ...
  function serverCommandGetCookie(command) {
    ...
    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
      ...
      if (command.nodeid) {
        cookieContent.nodeid = command.nodeid;
      }
      if (command.tcpaddr) {
        cookieContent.tcpaddr = command.tcpaddr;
      } // Indicates the browser want the agent to TCP connect to a remote address
      if (command.tcpport) {
        cookieContent.tcpport = command.tcpport;
      } // Indicates the browser want the agent to TCP connect to a remote port
      if (command.tag == 'novnc') {
        cookieContent.p = 12;
      } // If tag is novnc we must encode a protocol for meshrelay logging
      if (node.mtype == 3) {
        cookieContent.lc = 1;
        command.localRelay = true;
      } // Indicate this is for a local connection
  ...
  function serverCommandGetNetworkInfo(command) {
    ...
    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
      ...
      db.Get('if' + node._id, function (err, netinfos) {
        if (netinfos == null || netinfos.length != 1) {
          obj.send({ action: 'getnetworkinfo', nodeid: node._id, netif: null, netif2: null });
          return;
        }
        ...
        if (netinfo.netif2 != null) {
          for (var i in netinfo.netif2) {
            ...
            if (esc !== i) {
              netinfo.netif2[esc] = netinfo.netif2[i];
              delete netinfo.netif2[i];
            }
          }
        }
        ...
        obj.send({
          action: 'getnetworkinfo',
          nodeid: node._id,
          updateTime: netinfo.updateTime,
          netif: netinfo.netif,
          netif2: netinfo.netif2
        });
      });
    });
  }
        ...
        if (docs != null && docs.length > 0) {
          ...
          if (rights != MESHRIGHT_ADMIN && doc.hardware && doc.hardware.windows && doc.hardware.windows.volumes) {
            for (var i in doc.hardware.windows.volumes) {
              delete doc.hardware.windows.volumes[i].recoveryPassword;
            }
          }
          ...
          if (command.nodeinfo === true) {
            ...
            // Remove any connectivity and power state information, that should not be in the database anyway.
            // TODO: Find why these are sometimes saved in the db.
            if (doc.node.conn != null) {
              delete doc.node.conn;
            }
            if (doc.node.pwr != null) {
              delete doc.node.pwr;
            }
            if (doc.node.agct != null) {
              delete doc.node.agct;
            }
            if (doc.node.cict != null) {
              delete doc.node.cict;
            }
            // Add the connection state
            ...
            if (state) {
              ...
              if ((state.connectivity & 1) != 0) {
                var agent = parent.wsagents[doc.nodeid];
                if (agent != null) {
                  doc.node.agct = agent.connectTime;
                }
              }
              // Use the connection time of the CIRA/Relay connection
              if ((state.connectivity & 2) != 0) {
                ...
                if (ciraConnection != null && ciraConnection.tag != null) {
                  doc.node.cict = ciraConnection.tag.connectTime;
                }
              }
            }
          }
  ...
  function serverCommandInterSession(command) {
    ...
    for (var i in sessions) {
      if (sessions[i] != obj.ws) {
        try {
          sessions[i].send(notification);
        } catch (ex) {}
      }
    }
  ...
  function serverCommandInterUser(command) {
    ...
    if (
      parent.parent.config.settings.interusermessaging == null ||
      parent.parent.config.settings.interusermessaging == false ||
      command.data == null
    )
      return;
    if (typeof command.sessionid == 'string') {
      var userSessionId = command.sessionid.split('/');
      if (userSessionId.length != 4) return;
      command.userid = userSessionId[0] + '/' + userSessionId[1] + '/' + userSessionId[2];
    }
    if (common.validateString(command.userid, 0, 2014) == false) return;
    var userSplit = command.userid.split('/');
    if (userSplit.length == 1) {
      command.userid = 'user/' + domain.id + '/' + command.userid;
      userSplit = command.userid.split('/');
    }
    if (
      userSplit.length != 3 ||
      userSplit[0] != 'user' ||
      userSplit[1] != domain.id ||
      parent.users[command.userid] == null
    )
      return; // Make sure the target userid is valid and within the domain
    const allowed =
      parent.parent.config.settings.interusermessaging === true ||
      parent.parent.config.settings.interusermessaging.indexOf(obj.user._id) >= 0 ||
      parent.parent.config.settings.interusermessaging.indexOf(command.userid) >= 0;
    ...
    var notification = JSON.stringify({
      ...
      sessionid: ws.sessionId,
      data: command.data,
      scope: command.sessionid != null ? 'session' : 'user'
    });
    for (var i in sessions) {
  ...
  function serverCommandLastConnects(command) {
    if (obj.visibleDevices == null) {
      ...
      db.GetAllTypeNoTypeFieldMeshFiltered(
        ...
        function (err, docs) {
          ...
          db.GetAllIdsOfType(ids, domain.id, 'lastconnect', function (err, docs) {
            ...
            const response = {};
            for (var j in docs) {
              response[docs[j]._id.substring(2)] = docs[j].time;
            }
      ...
      // Create a list of node ids for this user and query them for last device connection time
      const ids = [];
      for (var i in obj.visibleDevices) {
        ids.push('lc' + i);
      }
  ...
  function serverCommandLoginCookie(command) {
    ...
    if (parent.parent.config.settings.allowlogintoken === true) {
      obj.send({
        action: 'logincookie',
        cookie: parent.parent.encodeCookie({ u: user._id, a: 3 }, parent.parent.loginCookieEncryptionKey)
      });
    }
  }
  ...
  function serverCommandMeshes(command) {
    ...
    obj.send({
      action: 'meshes',
      meshes: parent.GetAllMeshWithRights(user).map(parent.CloneSafeMesh),
      tag: command.tag
    });
  }

  function serverCommandPing(command) {
    try {
      ws.send('{"action":"pong"}');
    } catch (ex) {}
  }
        ...
        if (err == null && docs != null && docs.length > 0) {
          ...
          for (i in docs) {
            ...
            if (time == null) {
              ...
              time = doc.time;
              if (doc.oldPower) {
                timeline.push(doc.oldPower, time / 1000, doc.power);
              } else {
                timeline.push(0, time / 1000, doc.power);
              }
            } else if (previousPower != doc.power) {
              // Delta element
          ...
          if (state != null) {
            obj.send({
              action: 'powertimeline',
              nodeid: node._id,
              timeline: [state.powerState, Date.now(), state.powerState],
              tag: command.tag
            });
          }
        }
  ...
  function serverCommandPrint(command) {
  ...
  function serverCommandRemovePhone(command) {
    ...
    var event = {
      ...
      msgid: 97,
      msgArgs: [user.name],
      msg: 'Removed phone number of user ' + EscapeHtml(user.name),
      domain: domain.id
    };
    if (db.changeStream) {
      event.noact = 1;
  ...
  function serverCommandRemoveMessaging(command) {
    ...
    var event = {
      ...
      msgid: 157,
      msgArgs: [user.name],
      msg: 'Removed messaging account of user ' + EscapeHtml(user.name),
      domain: domain.id
    };
    if (db.changeStream) {
      event.noact = 1;
  ...
  function serverCommandRemoveUserFromUserGroup(command) {
    ...
    try {
      if ((user.siteadmin & SITERIGHT_USERGROUPS) == 0) {
        ...
        err = 'Invalid groupid';
      } else if (common.validateString(command.userid, 1, 256) == false) {
        err = 'Invalid userid';
      } else {
        var ugroupidsplit = command.ugrpid.split('/');
        if (
    ...
    if (obj.crossDomain !== true) {
      removeUserDomain = parent.parent.config.domains[ugroupidsplit[1]];
    }
    if (removeUserDomain == null) {
      err = 'Invalid domain';
    }
    ...
    if (err != null) {
      if (command.responseid != null) {
        obj.send({ action: 'removeuserfromusergroup', responseid: command.responseid, result: err });
      }
      return;
    }
    ...
    if (command.userid.startsWith('user/') == false) {
      if (parent.users['user/' + removeUserDomain.id + '/' + command.userid.toLowerCase()] != null) {
        command.userid = 'user/' + removeUserDomain.id + '/' + command.userid.toLowerCase();
      } else if (parent.users['user/' + removeUserDomain.id + '/' + command.userid] != null) {
        command.userid = 'user/' + removeUserDomain.id + '/' + command.userid;
      }
    }
      ...
      if (group != null) {
        ...
        if (group.links != null && group.links[command.userid] != null) {
          ...
          var event = {
            ...
            links: group.links,
            msgid: 72,
            msgArgs: [chguser.name, group.name],
            msg: 'Removed user ' + chguser.name + ' from user group ' + group.name,
            domain: removeUserDomain.id
          };
          if (db.changeStream) {
            event.noact = 1;
    ...
    if (command.responseid != null) {
      obj.send({ action: 'removeuserfromusergroup', responseid: command.responseid, result: 'ok' });
    }
  }
  ...
  function serverCommandReport(command) {
    ...
    if (
      command.devGroup != null &&
      manageAllDeviceGroups == false &&
      (user.links == null || user.links[command.devGroup] == null)
    )
      return; // Asking for a device group that is not allowed
  ...
  function serverCommandServerConsole(command) {
    ...
    if (
      domain.myserver === false ||
      (domain.myserver != null && domain.myserver !== true && domain.myserver.console !== true)
    )
      return;
    ...
    if (cmdTableEntry != null) {
      try {
        cmdTableEntry[0](cmdData);
      } catch (ex) {
        cmdData.result = '' + ex;
      }
    } else {
    ...
    if (cmdData.result != '') {
      obj.send({ action: 'serverconsole', value: cmdData.result, tag: command.tag });
    }
  }
  ...
  function serverCommandServerErrors(command) {
    ...
    if (userHasSiteUpdate() && domainHasMyServerErrorLog())
      fs.readFile(parent.parent.getConfigFilePath('mesherrors.txt'), 'utf8', function (err, data) {
        obj.send({ action: 'servererrors', data: data });
      });
  }
  ...
  function serverCommandServerConfig(command) {
    ...
    if (
      userHasSiteUpdate() &&
      domain.myserver !== false &&
      (domain.myserver == null || domain.myserver.config === true)
    ) {
      const configFilePath = common.joinPath(
        parent.parent.datapath,
        parent.parent.args.configfile ? parent.parent.args.configfile : 'config.json'
      );
      fs.readFile(configFilePath, 'utf8', function (err, data) {
        obj.send({ action: 'serverconfig', data: data });
      });
    }
  }
  ...
  function serverCommandServerTimelineStats(command) {
    ...
    db.GetServerStats(command.hours, function (err, docs) {
      if (err == null) {
        obj.send({ action: 'servertimelinestats', events: docs });
      }
    });
  }
  ...
  function serverCommandServerUpdate(command) {
    ...
    if (
      userHasSiteUpdate() &&
      domainHasMyServerUpgrade() &&
      !(command.version != null && typeof command.version != 'string')
    )
  ...
  function serverCommandServerVersion(command) {
    ...
    if (userHasSiteUpdate() && domainHasMyServerUpgrade())
      parent.parent.getServerTags(function (tags, err) {
        obj.send({ action: 'serverversion', tags: tags });
      });
  }
  ...
  function serverCommandSetClip(command) {
    ...
    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
      ...
      if (agent != null) {
        try {
          agent.send(JSON.stringify({ action: 'setClip', data: command.data }));
        } catch (ex) {}
      }
    });
  }
  ...
  function serverCommandSmsUser(command) {
    ...
    if (parent.parent.smsserver == null) {
      errMsg = 'SMS gateway not enabled';
    } else if ((user.siteadmin & 2) == 0) {
      errMsg = 'No user management rights';
    } else if (common.validateString(command.userid, 1, 2048) == false) {
      errMsg = 'Invalid username';
    } else if (common.validateString(command.msg, 1, 160) == false) {
      errMsg = 'Invalid SMS message';
    } else {
      smsuser = parent.users[command.userid];
      if (smsuser == null) {
        errMsg = 'Invalid username';
      } else if (smsuser.phone == null) {
        errMsg = 'No phone number for this user';
      }
    }
    ...
    parent.parent.smsserver.sendSMS(smsuser.phone, command.msg, function (success, msg) {
      if (success) {
        ...
        if (typeof msg == 'string') {
          displayNotificationMessage('SMS error: ' + msg, null, null, null, 29, [msg]);
        } else {
          displayNotificationMessage('SMS error', null, null, null, 28);
        }
      }
    });
  }
  ...
  function serverCommandMsgUser(command) {
    ...
    if (parent.parent.msgserver == null || parent.parent.msgserver.providers == 0) {
      errMsg = 'Messaging server not enabled';
    } else if ((user.siteadmin & 2) == 0) {
      errMsg = 'No user management rights';
    } else if (common.validateString(command.userid, 1, 2048) == false) {
      errMsg = 'Invalid username';
      ...
      if (msguser == null) {
        errMsg = 'Invalid username';
      } else if (msguser.msghandle == null) {
        errMsg = 'No messaging service configured for this user';
      }
    }
    ...
    parent.parent.msgserver.sendMessage(msguser.msghandle, command.msg, domain, function (success, msg) {
      if (success) {
        ...
        if (typeof msg == 'string') {
          displayNotificationMessage('Messaging error: ' + msg, null, null, null, 34, [msg]);
        } else {
          displayNotificationMessage('Messaging error', null, null, null, 33);
        }
      }
    });
  }
  ...
  function serverCommandUpdateAgents(command) {
    ...
    for (var i in command.nodeids) {
      routeCommandToNode(
        { action: 'msg', type: 'console', nodeid: command.nodeids[i], value: 'agentupdate' },
        MESHRIGHT_ADMIN,
        0
      );
    }
  ...
  function serverCommandUpdateUserImage(command) {
    ...
    var uid = user._id;
    if (typeof command.userid == 'string' && (user.siteadmin & SITERIGHT_MANAGEUSERS) != 0) {
      uid = command.userid;
    }

    var chguser = parent.users[uid],
      flags = 0,
      change = 0;
    if (chguser == null) return;
    if (typeof chguser.flags == 'number') {
      flags = chguser.flags;
    }
    ...
    if (command.image == 0) {
      ...
      command.image.length < 600000 &&
      (command.image.startsWith('data:image/png;base64,') || command.image.startsWith('data:image/jpeg;base64,'))
    ) {
      // Save the new image
  ...
  function serverCommandUsers(command) {
    ...
    if ((user.siteadmin & 2) == 0) {
      if (command.responseid != null) {
        obj.send({ action: 'users', responseid: command.responseid, result: 'Access denied' });
      }
      return;
    }
    var docs = [];
    for (i in parent.users) {
      if ((obj.crossDomain === true || parent.users[i].domain == domain.id) && parent.users[i].name != '~') {
        ...
        if (
          ...
          user.groups == null ||
          user.groups.length == 0 ||
          (parent.users[i].groups != null && findOne(parent.users[i].groups, user.groups))
        ) {
  ...
  function serverCommandVerifyPhone(command) {
    ...
    parent.parent.smsserver.sendPhoneCheck(
      domain,
      command.phone,
      code,
      parent.getLanguageCodes(req),
  ...
  function serverCommandVerifyMessaging(command) {
    ...
    var handle = null;
    if (command.service == 1 && (parent.parent.msgserver.providers & 1) != 0) {
      handle = 'telegram:@' + command.handle;
    }
    if (command.service == 4 && (parent.parent.msgserver.providers & 4) != 0) {
      handle = 'discord:' + command.handle;
    }
    if (command.service == 8 && (parent.parent.msgserver.providers & 8) != 0) {
      handle = 'xmpp:' + command.handle;
    }
    if (command.service == 16 && (parent.parent.msgserver.providers & 16) != 0) {
      handle = parent.parent.msgserver.callmebotUrlToHandle(command.handle);
    }
    if (command.service == 32 && (parent.parent.msgserver.providers & 32) != 0) {
      handle = 'pushover:' + command.handle;
    }
    if (command.service == 64 && (parent.parent.msgserver.providers & 64) != 0) {
      handle = 'ntfy:' + command.handle;
    }
    if (command.service == 128 && (parent.parent.msgserver.providers & 128) != 0) {
      handle = 'zulip:' + command.handle;
    }
    if (command.service == 256 && (parent.parent.msgserver.providers & 256) != 0) {
      handle = 'slack:' + command.handle;
    }
  ...
  function serverUserCommandHelp(cmdData) {
    var fin = '',
      f = '',
      availcommands = [];
    for (var i in serverUserCommands) {
      availcommands.push(i);
    }
    ...
    if (parent.parent.webpush == null) {
      ...
      if (cmdData.cmdargs['_'].length != 1) {
        ...
        const pushSubscription = {
          endpoint:
            'https://updates.push.services.mozilla.com/wpush/v2/gAAAAABgIkO9hjXHWhMPiuk-ppNRw7r_pUZitddwCEK4ykdzeIxOIjFnYhIt_nr-qUca2mpZziwQsSEhYTUCiuYrhWnVDRweMtiUj16yJJq8V5jneaEaUYjEIe5jp3DOMNpoTm1aHgX74gCR8uTXSITcM97bNi-hRxcQ4f6Ie4WSAmoXpd89B_g',
          keys: {
            auth: 'UB2sbLVK7ALnSHw5P1dahg',
            p256dh: 'BIoRbcNSxBuTjN39CCCUCHo1f4NxBJ1YDdu_k4MbPW_q3NK1_RufnydUzLPDp8ibBVItSI72-s48QJvOjQ_S8Ok'
          }
        };
        parent.parent.webpush.sendNotification(pushSubscription, cmdData.cmdargs['_'][0]).then(
          function (value) {
            try {
              ws.send(JSON.stringify({ action: 'OK', value: cmdData.result, tag: cmdData.command.tag }));
            } catch (ex) {}
          },
          function (error) {
            try {
              ws.send(JSON.stringify({ action: 'Error', value: cmdData.result, tag: cmdData.command.tag }));
            } catch (ex) {}
          }
        );
      }
  ...
  function serverUserCommandCertHashes(cmdData) {
    ...
    for (var i in parent.webCertificateHashs) {
      cmdData.result += '\r\nwebCertificateHash (' + i + '): ' + common.rstr2hex(parent.webCertificateHashs[i]);
    }
    for (var i in parent.webCertificateFullHashs) {
  ...
  function serverUserCommandAmtAcm(cmdData) {
    if (
      domain.amtacmactivation == null ||
      domain.amtacmactivation.acmmatch == null ||
      domain.amtacmactivation.acmmatch.length == 0
    ) {
      cmdData.result = 'No Intel AMT activation certificates.';
    } else {
      if (domain.amtacmactivation.log != null) {
        cmdData.result += '--- Activation Log ---\r\nFile  : ' + domain.amtacmactivation.log + '\r\n';
      }
      ...
      try {
        ws.send(
          JSON.stringify({
            ...
            value: 'Generating dump file at: ' + dumpFileName,
            tag: cmdData.command.tag
          })
        );
      } catch (ex) {}
      require('heapdump').writeSnapshot(dumpFileName, (err, filename) => {
        try {
          ws.send(JSON.stringify({ action: 'serverconsole', value: 'Done.', tag: cmdData.command.tag }));
        } catch (ex) {}
      });
    } else {
  ...
  function serverUserCommandHeapDump2(cmdData) {
    ...
    if (heapdump == null) {
      ...
      heapdump.writeSnapshot(function (err, filename) {
        if (err != null) {
          try {
            ws.send(JSON.stringify({ action: 'serverconsole', value: 'Unable to write heapdump: ' + err }));
          } catch (ex) {}
        } else {
          try {
            ws.send(JSON.stringify({ action: 'serverconsole', value: 'Wrote heapdump at ' + filename }));
          } catch (ex) {}
        }
      });
    }
          ...
          if (typeof msg == 'string') {
            try {
              ws.send(
                JSON.stringify({
                  action: 'serverconsole',
                  value: status ? 'Success: ' + msg : 'Failed: ' + msg,
                  tag: cmdData.command.tag
                })
              );
            } catch (ex) {}
  ...
  function serverUserCommandMsg(cmdData) {
    if (parent.parent.msgserver == null || parent.parent.msgserver.providers == 0) {
      ...
      if (cmdData.cmdargs['_'].length != 2) {
        var r = [];
        if ((parent.parent.msgserver.providers & 1) != 0) {
          r.push('Usage: MSG "telegram:[@UserHandle]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 2) != 0) {
          r.push('Usage: MSG "signal:[UserHandle]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 4) != 0) {
          r.push('Usage: MSG "discord:[Username#0000]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 8) != 0) {
          r.push('Usage: MSG "xmpp:[username@server.com]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 32) != 0) {
          r.push('Usage: MSG "pushover:[userkey]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 64) != 0) {
          r.push('Usage: MSG "ntfy:[topic]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 128) != 0) {
          r.push('Usage: MSG "zulip:[topic]" "Message".');
        }
        if ((parent.parent.msgserver.providers & 256) != 0) {
          r.push('Usage: MSG "slack:[webhook]" "Message".');
        }
        cmdData.result = r.join('\r\n');
      } else {
        parent.parent.msgserver.sendMessage(
          cmdData.cmdargs['_'][0],
          cmdData.cmdargs['_'][1],
          domain,
          function (status, msg) {
    ...
    if (domain.mailserver == null) {
      ...
      for (var userid in domain.mailserver.deviceNotifications) {
        ...
        for (var nodeid in domain.mailserver.deviceNotifications[userid].nodes) {
          ...
          x +=
            '  ' + info.mn + ', ' + info.nn + ', c:' + (info.c ? info.c : 0) + ', d:' + (info.d ? info.d : 0) + '\r\n';
        }
      }
  ...
  function serverUserCommandLeCheck(cmdData) {
    if (parent.parent.letsencrypt == null) {
      ...
      cmdData.result = ['CertOK', 'Request:NoCert', 'Request:Expire', 'Request:MissingNames'][
        parent.parent.letsencrypt.checkRenewCertificate()
      ];
    }
  }
    ...
    if (parent.parent.config.settings.maxinvalidlogin == false) {
      ...
      if (cmdData.cmdargs['_'] == 'reset') {
        ...
        if (typeof parent.parent.config.settings.maxinvalidlogin.coolofftime == 'number') {
          cmdData.result =
            'Max is ' +
            parent.parent.config.settings.maxinvalidlogin.count +
            ' bad login(s) in ' +
            parent.parent.config.settings.maxinvalidlogin.time +
            ' minute(s), ' +
            parent.parent.config.settings.maxinvalidlogin.coolofftime +
            ' minute(s) cooloff.\r\n';
        } else {
          cmdData.result =
            'Max is ' +
        ...
        for (var i in parent.badLoginTable) {
          ...
          if (typeof parent.badLoginTable[i] == 'number') {
            cmdData.result +=
              i + ' - Cooloff for ' + Math.floor((parent.badLoginTable[i] - Date.now()) / 60000) + ' minute(s)\r\n';
          } else {
            cmdData.result += i + ' - ' + parent.badLoginTable[i].length + ' attempt(s) until Cooloff ban\r\n';
          }
        }
        if (badLoginCount == 0) {
          cmdData.result += 'No bad logins.';
        }
      } else {
    ...
    if (parent.parent.config.settings.maxinvalid2fa == false) {
      ...
      if (cmdData.cmdargs['_'] == 'reset') {
        ...
        if (typeof parent.parent.config.settings.maxinvalid2fa.coolofftime == 'number') {
          cmdData.result =
            'Max is ' +
            parent.parent.config.settings.maxinvalid2fa.count +
            ' bad 2FA(s) in ' +
            parent.parent.config.settings.maxinvalid2fa.time +
            ' minute(s), ' +
            parent.parent.config.settings.maxinvalid2fa.coolofftime +
            ' minute(s) cooloff.\r\n';
        } else {
          cmdData.result =
            'Max is ' +
        ...
        for (var i in parent.bad2faTable) {
          ...
          if (typeof parent.bad2faTable[i] == 'number') {
            cmdData.result +=
              'Cooloff for ' + Math.floor((parent.bad2faTable[i] - Date.now()) / 60000) + ' minute(s)\r\n';
          } else {
  ...
  function serverUserCommandDupAgents(cmdData) {
    ...
    if (cmdData.result == '') {
      cmdData.result = 'No duplicate agents in log.';
    }
  }
  ...
  function serverUserCommandAgentStats(cmdData) {
    ...
    for (var i in stats) {
      if (typeof stats[i] == 'object') {
        cmdData.result += i + ': ' + JSON.stringify(stats[i]) + '\r\n';
      } else {
        cmdData.result += i + ': ' + stats[i] + '\r\n';
      }
    }
  }
  ...
  function serverUserCommandTrafficDelta(cmdData) {
    ...
    for (var i in stats.delta) {
      if (typeof stats.delta[i] == 'object') {
        cmdData.result += i + ': ' + JSON.stringify(stats.delta[i]) + '\r\n';
      } else {
        cmdData.result += i + ': ' + stats.delta[i] + '\r\n';
      }
    }
  }
  ...
  function serverUserCommandWatchdog(cmdData) {
    if (parent.parent.watchdog == null) {
      ...
      if (parent.parent.watchdogmaxtime != null) {
        cmdData.result +=
          'Largest timeout was ' + parent.parent.watchdogmax + 'ms on ' + parent.parent.watchdogmaxtime + '\r\n';
      }
      for (var i in parent.parent.watchdogtable) {
        cmdData.result += parent.parent.watchdogtable[i] + '\r\n';
      }
    }
  }
  ...
  function serverUserCommand2faLock(cmdData) {
    ...
    if (arg == 'set') {
      ...
      parent.parent.DispatchEvent(['server-allusers'], obj, {
        action: 'serverinfochange',
        lock2factor: true,
        nolog: 1,
        domain: domain.id
      });
    ...
    if (parent.parent.mpsserver == null) {
      ...
      for (var nodeid in parent.parent.mpsserver.ciraConnections) {
        ...
        for (var i in connections) {
          cmdData.result += ', ' + connectionTypes[connections[i].tag.connType];
        }
        cmdData.result += '\r\n';
      }
      if (cmdData.result == '') {
        cmdData.result = 'MPS has not connections.';
      }
    }
  }
  ...
  function serverUserCommandDbStats(cmdData) {
    parent.parent.db.getDbStats(function (stats) {
      ...
      try {
        ws.send(JSON.stringify({ action: 'serverconsole', value: r2, tag: cmdData.command.tag }));
      } catch (ex) {}
    });
  }
  ...
  function serverUserCommandDbCounters(cmdData) {
    try {
      ws.send(
        JSON.stringify({
          action: 'serverconsole',
          value: JSON.stringify(parent.parent.db.dbCounters, null, 2),
          tag: cmdData.command.tag
        })
      );
    } catch (ex) {}
  ...
  function serverUserCommandDbCompact(cmdData) {
    if (user.siteadmin === SITERIGHT_ADMIN) {
      if (parent.parent.db.databaseType == 1) {
        // Only compact if using NeDB, other databases should handle compaction on their own
          ...
          if (err) {
            try {
              ws.send(
                JSON.stringify({
                  action: 'serverconsole',
                  value: 'Error compacting database: ' + err,
                  tag: cmdData.command.tag
                })
              );
            } catch (ex) {}
              ...
              if (err) {
                try {
                  ws.send(
                    JSON.stringify({
                      action: 'serverconsole',
                      value: 'Error compacting events database: ' + err,
                      tag: cmdData.command.tag
                    })
                  );
                } catch (ex) {}
                  ...
                  if (err) {
                    try {
                      ws.send(
                        JSON.stringify({
                          action: 'serverconsole',
                          value: 'Error compacting power database: ' + err,
                          tag: cmdData.command.tag
                        })
                      );
                    } catch (ex) {}
                      ...
                      if (err) {
                        try {
                          ws.send(
                            JSON.stringify({
                              action: 'serverconsole',
                              value: 'Error compacting server stats database: ' + err,
                              tag: cmdData.command.tag
                            })
                          );
                        } catch (ex) {}
                            ...
                            if (err) {
                              try {
                                ws.send(
                                  JSON.stringify({
                                    action: 'serverconsole',
                                    value: 'Error compacting plugins database: ' + err,
                                    tag: cmdData.command.tag
                                  })
                                );
                              } catch (ex) {}
                              ...
                              try {
                                ws.send(
                                  JSON.stringify({
                                    action: 'serverconsole',
                                    value: 'Database compacted successfully.',
                                    tag: cmdData.command.tag
                                  })
                                );
                              } catch (ex) {}
        ...
        parent.parent.getServerVersions((data) => {
          ...
          if (versions.includes(version)) {
            if (parent.parent.performServerUpdate(version) == false) {
              try {
                ws.send(JSON.stringify({ action: 'serverconsole', value: 'Server self-update not possible.' }));
              } catch (ex) {}
            }
          } else {
            try {
              ws.send(JSON.stringify({ action: 'serverconsole', value: 'Invalid version. Aborting update' }));
            } catch (ex) {}
          }
        });
      } else {
  ...
  function serverUserCommandAmtPasswords(cmdData) {
    if (parent.parent.amtPasswords == null) {
      ...
      for (var i in parent.parent.amtPasswords) {
        cmdData.result += i + ' - ' + parent.parent.amtPasswords[i].join(', ') + '\r\n';
      }
    }
  }
      ...
      if (err != null) {
        ...
        for (var i in docs) {
          ...
          if (node.intelamt != null) {
            amtData['total']++;
            if (node.intelamt.ver != null) {
              if (amtData.versions[node.intelamt.ver] == null) {
                amtData.versions[node.intelamt.ver] = 1;
              } else {
                amtData.versions[node.intelamt.ver]++;
              }
            }
            if (node.intelamt.state != null) {
              if (amtData.state[node.intelamt.state] == null) {
                amtData.state[node.intelamt.state] = 1;
              } else {
                amtData.state[node.intelamt.state]++;
              }
            }
          }
        }
        if (amtData.total == 0) {
          ...
          var amtVersions = [];
          for (var i in amtData.versions) {
            if (amtVersions.indexOf(i) == -1) {
              amtVersions.push(i);
            }
          }
          var collator = new Intl.Collator([], { numeric: true });
          amtVersions.sort((a, b) => collator.compare(a, b));
          for (var i in amtVersions) {
            r += '  ' + amtVersions[i] + ': ' + amtData.versions[amtVersions[i]] + '\r\n';
          }
        }
      }
      try {
        ws.send(JSON.stringify({ action: 'serverconsole', value: r, tag: cmdData.command.tag }));
      } catch (ex) {}
    });
  }
  ...
  function serverUserCommandMaintenance(cmdData) {
    var arg = null,
      changed = false;
    if (cmdData.cmdargs['_'] != null && cmdData.cmdargs['_'][0] != null) {
      arg = cmdData.cmdargs['_'][0].toLowerCase();
    }
    if (arg == 'enabled') {
      parent.parent.config.settings.maintenancemode = 1;
      changed = true;
    } else if (arg == 'disabled') {
      delete parent.parent.config.settings.maintenancemode;
      changed = true;
    }
    cmdData.result =
      'Maintenance mode: ' + (parent.parent.config.settings.maintenancemode == null ? 'Disabled' : 'Enabled');
    if (changed == false) {
  ...
  function serverUserCommandInfo(cmdData) {
    function convertSeconds(s, form) {
      ...
      if (form == 'long') {
        r =
          ...
          (t.h == 1 ? ' hour, ' : ' hours, ') +
          t.m +
          (t.m == 1 ? ' minute, ' : ' minutes, ') +
          t.s +
          (t.s == 1 ? ' second' : ' seconds');
        ...
        r =
          ...
          ':' +
          String((s % 60).toFixed(2)).padStart(5, '0') +
          's';
      }
      return r;
    }
    ...
    try {
      info.meshVersion = 'v' + parent.parent.currentVer;
    } catch (ex) {}
    try {
      info.nodeVersion = process.version;
    } catch (ex) {}
    try {
      info.runMode = ['Hybrid (LAN + WAN) mode', 'WAN mode', 'LAN mode'][args.lanonly ? 2 : args.wanonly ? 1 : 0];
    } catch (ex) {}
    try {
      info.productionMode = process.env.NODE_ENV != null && process.env.NODE_ENV == 'production';
    } catch (ex) {}
    try {
      info.database = ['Unknown', 'NeDB', 'MongoJS', 'MongoDB', 'MariaDB', 'MySQL', 'PostgreSQL', 'AceBase', 'SQLite'][
        parent.parent.db.databaseType
      ];
    } catch (ex) {}
    try {
      if (parent.db.databaseType == 3) {
        info.dbChangeStream = parent.db.changeStream;
        info.dbBulkOperations = parent.parent.config.settings.mongodbbulkoperations === true;
      }
    } catch (ex) {}
    try {
      if (parent.parent.multiServer != null) {
        info.serverId = parent.parent.multiServer.serverid;
      }
    } catch (ex) {}
    try {
      if (parent.parent.pluginHandler != null) {
        info.plugins = [];
        for (var i in parent.parent.pluginHandler.plugins) {
          info.plugins.push(i);
        }
      }
    } catch (ex) {}
    ...
    try {
      info.allDevGroupManagers = parent.parent.config.settings.managealldevicegroups;
    } catch (ex) {}
    try {
      if (process.traceDeprecation == true) {
        info.traceDeprecation = true;
      }
    } catch (ex) {}
    ...
    for (var i in parent.wssessions) {
      if (filter == null || filter == i) {
        ...
        for (var j in parent.wssessions[i]) {
          var extras = '';
          if (parent.wssessions[i][j].satelliteFlags) {
            extras += ', Satellite';
          }
          cmdData.result +=
            '    ' +
            parent.wssessions[i][j].clientIp +
            ' --> ' +
            parent.wssessions[i][j].sessionId +
            extras +
            (parent.wssessions[i][j].xclosed ? ', CLOSED-' + parent.wssessions[i][j].xclosed : '') +
            '\r\n';
        }
      }
    }
    ...
    if (filter == null) {
      ...
      for (var i in parent.wssessions) {
        if (filter == i) {
          userSessionCount++;
          for (var j in parent.wssessions[i]) {
            parent.wssessions[i][j].send(
              JSON.stringify({ action: 'stopped', msg: 'Administrator forced disconnection' })
            );
      ...
      if (userSessionCount < 2) {
        cmdData.result += 'Disconnected ' + userSessionCount + ' session.';
      } else {
        cmdData.result += 'Disconnected ' + userSessionCount + ' sessions.';
      }
    }
  }
  ...
  function serverUserCommandSetMaxTasks(cmdData) {
    if (
      cmdData.cmdargs['_'].length != 1 ||
      parseInt(cmdData.cmdargs['_'][0]) < 1 ||
      parseInt(cmdData.cmdargs['_'][0]) > 1000
    ) {
  ...
  function serverUserCommandShowPaths(cmdData) {
    ...
    if (parent.parent.webPublicOverridePath) {
      cmdData.result += 'XWebPublic: ' + parent.parent.webPublicOverridePath + '\r\n';
    }
    if (parent.parent.webViewsOverridePath) {
      cmdData.result += 'XWebViews:  ' + parent.parent.webViewsOverridePath + '\r\n';
    }
    if (parent.parent.webEmailsOverridePath) {
      cmdData.result += 'XWebEmails:  ' + parent.parent.webEmailsOverridePath + '\r\n';
    }
    if (domain.webpublicpath) {
      cmdData.result += 'DomainWebPublic: ' + domain.webpublicpath + '\r\n';
    }
    if (domain.webviewspath) {
      cmdData.result += 'DomainWebViews: ' + domain.webviewspath + '\r\n';
    }
    if (domain.webemailspath) {
      cmdData.result += 'DomainWebEmails: ' + domain.webemailspath + '\r\n';
    }
  }
      ...
      for (var i in parent.parent.swarmserver.migrationAgents) {
        ...
        for (var j in arch) {
          ...
          cmdData.result +=
            ...
            agent.ver +
            ', Size ' +
            (agent.binary == null ? 0 : agent.binary.length) +
            '<br />';
        }
      }
    }
  ...
  function serverUserCommandRelays(cmdData) {
    for (var i in parent.wsrelays) {
      ...
      if (parent.wsrelays[i].peer1 != null) {
        ...
        if (parent.wsrelays[i].peer1.user) {
          cmdData.result += ' (User:' + parent.wsrelays[i].peer1.user.name + ')';
        }
      }
      if (parent.wsrelays[i].peer2 != null) {
        cmdData.result += ' to ' + cleanRemoteAddr(parent.wsrelays[i].peer2.req.clientIp);
        if (parent.wsrelays[i].peer2.user) {
          cmdData.result += ' (User:' + parent.wsrelays[i].peer2.user.name + ')';
        }
      }
      cmdData.result += '\r\n';
    }
    if (cmdData.result == '') {
      cmdData.result = 'No relays.';
    }
  }
  ...
  function serverUserCommandRemoveInactiveDevices(cmdData) {
    ...
    if (arg == null && arg != 'showremoved' && arg != 'showall') {
      ...
      parent.db.removeInactiveDevices(arg == 'showall', function (msg) {
        try {
          ws.send(JSON.stringify({ action: 'serverconsole', value: msg, tag: cmdData.command.tag }));
        } catch (ex) {}
      });
    }
  }
  ...
  function userHasSiteUpdate() {
    return (user.siteadmin & SITERIGHT_SERVERUPDATE) > 0;
  }
  function domainHasMyServerErrorLog() {
    return !(
      domain.myserver === false ||
      (domain.myserver != null && domain.myserver !== true && domain.myserver.errorlog !== true)
    );
  }
  function domainHasMyServerUpgrade() {
    return !(
      domain.myserver === false ||
      (domain.myserver != null && domain.myserver !== true && domain.myserver.upgrade !== true)
    );
  }

  function csvClean(s) {
    return '\"' + s.split('\"').join('').split(',').join('').split('\r').join('').split('\n').join('') + '\"';
  }
  ...
  function remoteSessionReport(command, manageAllDeviceGroups, msgIdFilter) {
    ...
    db.GetEventsTimeRange(
      ids,
      domain.id,
      msgIdFilter,
      new Date(command.start * 1000),
      new Date(command.end * 1000),
      function (err, docs) {
        if (err != null) return;
        var data = { groups: {} };
        ...
        if (command.groupBy == 1) {
          ...
          data.columns = [
            { id: 'time', title: 'time', format: 'datetime' },
            { id: 'nodeid', title: 'device', format: 'node' },
            { id: 'meshid', title: 'devgroup', format: 'mesh' },
            { id: 'guestname', title: 'guest', align: 'center' },
            { id: 'protocol', title: 'session', format: 'protocol', align: 'center' },
            { id: 'length', title: 'length', format: 'seconds', align: 'center', sumBy: 'protocol' }
          ];
        } else if (command.groupBy == 2) {
          data.groupFormat = 'nodemesh';
          data.columns = [
            { id: 'time', title: 'time', format: 'datetime' },
            { id: 'userid', title: 'user', format: 'user' },
            { id: 'guestname', title: 'guest', align: 'center' },
            { id: 'protocol', title: 'session', format: 'protocol', align: 'center' },
            { id: 'length', title: 'length', format: 'seconds', align: 'center', sumBy: 'protocol' }
          ];
          ...
          data.columns = [
            { id: 'time', title: 'time', format: 'time' },
            { id: 'nodeid', title: 'device', format: 'node' },
            { id: 'meshid', title: 'devgroup', format: 'mesh' },
            { id: 'guestname', title: 'guest', align: 'center' },
            { id: 'userid', title: 'user', format: 'user' },
        ...
        for (var i in docs) {
          ...
          // UserID
          if (command.groupBy != 1) {
            entry.userid = docs[i].userid;
          }
          if (command.groupBy != 2) {
            entry.nodeid = docs[i].nodeid;
          }
          entry.protocol = docs[i].protocol;

          // Device Group
          if (docs[i].ids != null) {
            for (var j in docs[i].ids) {
              if (docs[i].ids[j].startsWith('mesh/')) {
                entry.meshid = docs[i].ids[j];
              }
            }
          }
          ...
          // Add traffic data
          if (command.showTraffic) {
            entry.bytesin = docs[i].bytesin;
            entry.bytesout = docs[i].bytesout;
          }

          // Add guest name if present
          if (docs[i].guestname != null) {
            entry.guestname = docs[i].guestname;
            guestNamePresent = true;
          }
          ...
          if (
            ...
            docs[i].msgid <= 12 &&
            docs[i].msgArgs != null &&
            typeof docs[i].msgArgs == 'object' &&
            typeof docs[i].msgArgs[3] == 'number'
          ) {
            entry.length = docs[i].msgArgs[3];
          } else if (
            docs[i].msgid >= 122 &&
            docs[i].msgid <= 126 &&
            docs[i].msgArgs != null &&
            typeof docs[i].msgArgs == 'object' &&
            typeof docs[i].msgArgs[0] == 'number'
          ) {
            entry.length = docs[i].msgArgs[0];
          } else if (
            docs[i].msgid == 144 &&
            docs[i].msgArgs != null &&
            typeof docs[i].msgArgs == 'object' &&
            typeof docs[i].msgArgs[1] == 'number'
          ) {
            entry.length = docs[i].msgArgs[1];
          }

          if (command.groupBy == 1) {
            // Add entry to per user
            if (data.groups[docs[i].userid] == null) {
              data.groups[docs[i].userid] = { entries: [] };
            }
            data.groups[docs[i].userid].entries.push(entry);
          } else if (command.groupBy == 2) {
            // Add entry to per mesh+device
            if (entry.meshid != null) {
              var k = docs[i].nodeid + '/' + entry.meshid;
              if (data.groups[k] == null) {
                data.groups[k] = { entries: [] };
              }
              data.groups[k].entries.push(entry);
            } else {
              if (data.groups[docs[i].nodeid] == null) {
                data.groups[docs[i].nodeid] = { entries: [] };
              }
              data.groups[docs[i].nodeid].entries.push(entry);
            }
          } else if (command.groupBy == 3) {
            // Add entry to per day
            var day;
            if (typeof command.l == 'string' && typeof command.tz == 'string') {
            ...
            if (data.groups[day] == null) {
              data.groups[day] = { entries: [] };
            }
        ...
        try {
          ws.send(JSON.stringify({ action: 'report', data: data }));
        } catch (ex) {}
      }
    );
  }
  ...
  function trafficUsageReport(command, msgIdFilter) {
    ...
    if ((user.siteadmin & SITERIGHT_MANAGEUSERS) != 0) {
      ids = ['*'];
    } // If user administrator, count traffic of all users.
    ...
    db.GetEventsTimeRange(
      ...
      function (err, docs) {
        ...
        data.columns = [
          { id: 'userid', title: 'user', format: 'user' },
          { id: 'length', title: 'length', format: 'seconds', align: 'center', sumBy: true },
          { id: 'bytesin', title: 'bytesin', format: 'bytes', align: 'center', sumBy: true },
          { id: 'bytesout', title: 'bytesout', format: 'bytes', align: 'center', sumBy: true }
        ];
        var userEntries = {};
        ...
        for (var i in docs) {
          ...
          if (userEntry == null) {
            userEntry = { userid: docs[i].userid, length: 0, bytesin: 0, bytesout: 0 };
          }
          if (docs[i].bytesin) {
            userEntry.bytesin += docs[i].bytesin;
          }
          if (docs[i].bytesout) {
            userEntry.bytesout += docs[i].bytesout;
          }
          ...
          if (
            ...
            userEntry.length += docs[i].msgArgs[3];
          } else if (
            docs[i].msgid >= 122 &&
            docs[i].msgid <= 126 &&
            docs[i].msgArgs != null &&
            ...
            userEntry.length += docs[i].msgArgs[0];
          } else if (
            docs[i].msgid == 144 &&
            docs[i].msgArgs != null &&
            typeof docs[i].msgArgs == 'object' &&
            ...
            userEntry.length += docs[i].msgArgs[1];
          }
        ...
        for (var i in userEntries) {
          userEntries2.push(userEntries[i]);
        }
  ...
  function userLoginReport(command) {
    ...
    // MySQL or MariaDB query will ignore the MsgID filter.
    var msgIdFilter = [107];
    if (showInvalidLoginAttempts) {
      msgIdFilter = [107, 108, 109, 110];
      ...
      function (err, docs) {
        ...
        if (command.groupBy == 1) {
          ...
          data.columns = [
            ...
            { id: 'os', title: 'os' },
            { id: 'twofactor', title: 'twofactor', format: '2fa' }
          ];
        } else if (command.groupBy == 3) {
          data.columns = [
            { id: 'time', title: 'time', format: 'time' },
        ...
        if (showInvalidLoginAttempts) {
          data.columns.push({ id: 'msg', title: 'msg', format: 'msg' });
        }
        ...
        for (var i in docs) {
          ...
          if (command.groupBy == 1) {
            ...
            const entry = {
              time: docs[i].time.valueOf(),
              ip: docs[i].msgArgs[0],
              browser: docs[i].msgArgs[1],
              os: docs[i].msgArgs[2],
              twofactor: docs[i].twoFactorType ? docs[i].twoFactorType : ''
            };
            if (showInvalidLoginAttempts) {
              entry.msg = docs[i].msgid;
            }
            data.groups[docs[i].userid].entries.push(entry);
          } else if (command.groupBy == 3) {
            // Add entry per day
  ...
  function databaseRecordsReport(command) {
    ...
    parent.parent.db.getDbStats(function (stats) {
      var data = { groups: { 0: { entries: [] } } };
      data.columns = [
        { id: 'record', title: 'Record', format: 'records' },
        { id: 'recordcount', title: 'Count', align: 'center', sumBy: true }
      ];
      for (var i in stats) {
        if (i != 'total' && stats[i] > 0) {
          data.groups[0].entries.push({ record: i, recordcount: stats[i] });
        }
      }
      try {
        ws.send(JSON.stringify({ action: 'report', data: data }));
  ...
  function getDeviceDetailedInfo(nodeids, type, func) {
    if (nodeids == null) {
      getAllDeviceDetailedInfo(type, func);
      return;
    }
    var results = [],
        ...
        if (node != null && visible == true) {
          const getNodeSysInfoFunc = function (err, docs) {
            const getNodeNetInfoFunc = function (err, docs) {
              ...
              resultPendingCount--;
              getNodeNetInfoFunc.results.push({
                node: parent.CloneSafeNode(getNodeNetInfoFunc.node),
                sys: getNodeNetInfoFunc.sysinfo,
                net: netinfo
              });
              if (resultPendingCount == 0) {
                func(getNodeFunc.results, type);
              }
            };
            ...
            if (err == null && docs != null && docs.length == 1) {
              getNodeNetInfoFunc.sysinfo = docs[0];
            }
  ...
  function readFilesRec(path) {
    ...
    for (var i in dir) {
      var f = { t: 3, d: 111 },
        stat = null;
      try {
        stat = fs.statSync(path + '/' + dir[i]);
      } catch (ex) {}
      if (stat != null) {
        if ((stat.mode & 0x004000) == 0) {
          f.s = stat.size;
          f.d = stat.mtime.getTime();
        } else {
          f.t = 2;
          f.f = readFilesRec(path + '/' + dir[i]);
        }
        r[dir[i]] = f;
      }
    }
  ...
  function updateUserFiles(user, ws, domain, meshid) {
    ...
    if (meshid == null) {
      ...
      try {
        files.filetree.f[user._id].f = readFilesRec(
          parent.path.join(parent.filespath, domainx + '/user-' + safeUserName)
        );
      } catch (e) {
        // TODO: We may want to fake this file structure until it's needed.
        // Got an error, try to create all the folders and try again...
        try {
          fs.mkdirSync(parent.filespath);
        } catch (e) {}
        try {
          fs.mkdirSync(parent.path.join(parent.filespath, domainx));
        } catch (e) {}
        try {
          fs.mkdirSync(parent.path.join(parent.filespath, domainx + '/user-' + safeUserName));
        } catch (e) {}
        try {
          fs.mkdirSync(parent.path.join(parent.filespath, domainx + '/user-' + safeUserName + '/Public'));
        } catch (e) {}
        try {
          files.filetree.f[user._id].f = readFilesRec(
            parent.path.join(parent.filespath, domainx + '/user-' + safeUserName)
    ...
    for (var i in meshes) {
      ...
      // Read all files recursively
      try {
        files.filetree.f[mesh._id].f = readFilesRec(
          parent.path.join(parent.filespath, domainx + '/mesh-' + meshsplit[2])
        );
      } catch (e) {
  ...
  function toNumberIfNumber(x) {
    if (typeof x == 'string' && +parseInt(x) === x) {
      x = parseInt(x);
    }
    return x;
  }
  ...
  function getRandomPassword() {
    return Buffer.from(parent.crypto.randomBytes(9), 'binary').toString('base64').split('/').join('@');
  }
  ...
  function count2factoraAuths() {
    var email2fa =
      (typeof domain.passwordrequirements != 'object' || domain.passwordrequirements.email2factor != false) &&
      domain.mailserver != null;
    var sms2fa =
      parent.parent.smsserver != null &&
      (typeof domain.passwordrequirements != 'object' || domain.passwordrequirements.sms2factor != false);
    var msg2fa =
      parent.parent.msgserver != null &&
      parent.parent.msgserver.providers != 0 &&
      (typeof domain.passwordrequirements != 'object' || domain.passwordrequirements.msg2factor != false);
    var duo2fa =
      typeof domain.passwordrequirements != 'object' || typeof domain.passwordrequirements.duo2factor == 'object';
    var authFactorCount = 0;
    if (typeof user.otpsecret == 'string') {
      authFactorCount++;
    } // Authenticator time factor
    if (email2fa && user.otpekey != null) {
      authFactorCount++;
    } // EMail factor
    if (sms2fa && user.phone != null) {
      authFactorCount++;
    } // SMS factor
    if (msg2fa && user.msghandle != null) {
      authFactorCount++;
    } // Messaging factor
    if (duo2fa && user.otpduo != null) {
      authFactorCount++;
    } // Duo authentication factor
    if (user.otphkeys != null) {
      authFactorCount += user.otphkeys.length;
    } // FIDO hardware factor
    if (authFactorCount > 0 && user.otpkeys != null) {
      authFactorCount++;
    } // Backup keys
  ...
  function isEventWithinPage(ids) {
    ...
    for (var i in ids) {
      ...
      if (ids[i].startsWith('node/')) {
        r = false;
        if (obj.visibleDevices[ids[i]] != null) return true;
      }
    }
