...
module.exports.CreateWebRelaySession = function (
  parent,
  db,
  req,
  args,
  ...
  if (expire != null) {
    ...
    obj.expireTimer = setTimeout(function () {
      parent.parent.debug('webrelay', 'timeout');
      close();
    }, timeout);
  }
  ...
  obj.checkTimeout = function () {
    ...
    for (var i in tunnels) {
      if (tunnels[i].lastOperation < limit && tunnels[i].isWebSocket !== true) {
        tunnelToRemove.push(tunnels[i]);
      }
    }
    for (var i in tunnelToRemove) {
      tunnelToRemove[i].close();
    }
    ...
    if (obj.lastOperation < limit) {
      ...
      if (count == 0) {
        close();
      } // Time limit reached and no tunnels, clean up.
    }
  };
  ...
  function handleNextRequest() {
    ...
    for (var i in tunnels) {
      ...
      if (
        tunnels[i].relayActive == true &&
        tunnels[i].res == null &&
        tunnels[i].isWebSocket == false &&
        tunnels[i].isStreaming == false
      ) {
        // Found a free tunnel, use it
        const x = pendingRequests.shift();
        if (x[2] == true) {
          tunnels[i].processWebSocket(x[0], x[1]);
        } else {
  ...
  function launchNewTunnel() {
    ...
    tunnel.onclose = function (tunnelId, processedCount) {
      ...
      if (processedCount == 0) {
        errorCount++;
      } // If this tunnel closed without processing any requests, mark this as an error
    ...
    tunnel.onconnect = function (tunnelId) {
      ...
      if (pendingRequests.length > 0) {
        const x = pendingRequests.shift();
        if (x[2] == true) {
          tunnels[tunnelId].processWebSocket(x[0], x[1]);
        } else {
          tunnels[tunnelId].processRequest(x[0], x[1]);
        }
      }
    };
  ...
  function close() {
    ...
    for (var i in pendingRequests) {
      if (pendingRequests[i][2] == true) {
        pendingRequests[i][1].close();
      } else {
        pendingRequests[i][1].end();
      }
    }

    // Notify of session closure
    if (obj.onclose) {
      obj.onclose(obj.sessionid);
    }
...
module.exports.CreateWebRelay = function (parent, db, args, domain, mtype) {
  ...
  function handleResponseClosure() {
    obj.close();
  }
  ...
  function parseRequestCookies(cookiesString) {
    ...
    for (var i in cookieString) {
      var j = cookieString[i].indexOf('=');
      if (j > 0) {
        r[cookieString[i].substring(0, j)] = cookieString[i].substring(j + 1);
      }
    }
    return r;
  }
  ...
  obj.processRequest = function (req, res) {
    if (obj.relayActive == false) {
      console.log('ERROR: Attempt to use an unconnected tunnel');
      return false;
    }
    ...
    if (req.headers['upgrade'] == 'websocket') {
      console.log('Attempt to process a websocket in HTTP tunnel method.');
      res.end();
      return false;
    }
    ...
    const blockedHeaders = [
      ...
      'sec-fetch-mode',
      'sec-fetch-dest'
    ]; // These are headers we do not forward
    for (var i in req.headers) {
      if (blockedHeaders.indexOf(i) == -1) {
        request += i + ': ' + req.headers[i] + '\r\n';
      }
    }
    var cookieStr = '';
    for (var i in parent.webCookies) {
      if (cookieStr != '') {
        cookieStr += '; ';
      }
      cookieStr += i + '=' + parent.webCookies[i].value;
    }
  ...
  obj.processWebSocket = function (req, ws) {
    ...
    // If the response stream is closed, close this tunnel right away
    obj.ws._socket.on('end', function () {
      obj.close();
    });
    ...
    var baseurl = req.url,
      i = req.url.indexOf('?');
    if (i > 0) {
      baseurl = req.url.substring(0, i);
    }
    if (baseurl.endsWith('/.websocket')) {
      req.url = baseurl.substring(0, baseurl.length - 11) + (i < 1 ? '' : req.url.substring(i));
    }
    ...
    obj.ws.on('message', function (data) {
      ...
      if (typeof data == 'string') {
        op = 1;
        payload = Buffer.from(data, 'binary');
      } // Text frame
      sendWebSocketFrameToDevice(op, payload);
    });

    obj.ws.on('ping', function (data) {
      sendWebSocketFrameToDevice(9, data);
    }); // Forward ping frame
    obj.ws.on('pong', function (data) {
      sendWebSocketFrameToDevice(10, data);
    }); // Forward pong frame
    obj.ws.on('close', function () {
      obj.close();
    });
  ...
  obj.close = function (arg) {
    ...
    if (
      ...
      obj.socketXHeader['connection'] != null &&
      obj.socketXHeader['connection'].toLowerCase() == 'close'
    ) {
    ...
    // Event disconnection
    if (obj.onclose) {
      obj.onclose(obj.tunnelId, obj.processedRequestCount);
    }
  ...
  obj.connect = function (userid, nodeid, addr, port, appid) {
    ...
    try {
      ...
      var url =
        ...
        '/' +
        domainadd +
        (obj.mtype == 3 && obj.relaynodeid == null ? 'local' : 'mesh') +
        'relay.ashx?p=14&auth=' +
        cookie; // Protocol 14 is Web-TCP
      if (domain.id != '') {
        url += '&domainid=' + domain.id;
      } // Since we are using "localhost", we are going to signal what domain we are on using a URL argument.
      parent.parent.parent.debug('relay', 'TCP: Connection websocket to ' + url);
      obj.wsClient = new WebSocket(url, options);
      obj.wsClient.on('open', function () {
        parent.parent.parent.debug('relay', 'TCP: Relay websocket open');
      });
      obj.wsClient.on('message', function (data) {
        // Make sure to handle flow control.
        if (obj.tls) {
          // WS --> TLS
          ...
          if (data == 'c' || data == 'cr') {
            if (appid == 2) {
              ...
              obj.ser.forwardwrite = function (data) {
                if (data.length > 0) {
                  try {
              ...
              obj.tls = require('tls').connect(tlsoptions, function () {
                ...
                if (obj.onconnect) {
                  obj.onconnect(obj.tunnelId);
                } // Event connection
              });
              obj.tls.setEncoding('binary');
              obj.tls.on('error', function (err) {
                parent.parent.parent.debug('relay', 'Web Relay TLS Connection Error', err);
                obj.close();
              });

              // Decrypted tunnel from TLS communcation to be forwarded to the browser
              obj.tls.on('data', function (data) {
      ...
      obj.wsClient.on('close', function () {
        parent.parent.parent.debug('relay', 'TCP: Relay websocket closed');
        obj.close();
      });
      obj.wsClient.on('error', function (err) {
        parent.parent.parent.debug('relay', 'TCP: Relay websocket error: ' + err);
        obj.close();
      });
    } catch (ex) {
      console.log(ex);
  ...
  function processRawHttpData(data) {
    if (typeof data == 'string') {
      ...
      if (cmd != null && cmd.ctrlChannel == '102938' && cmd.type == 'ping') {
        cmd.type = 'pong';
        obj.wsClient.send(JSON.stringify(cmd));
      }
      return;
    }
    if (obj.tls) {
      ...
      if (obj.socketParseState == 0) {
        ...
        for (var i in obj.socketHeader) {
          if (i != 0) {
            ...
            if (n == 'set-cookie') {
              // Since "set-cookie" can be present many times in the header, handle it as an array of values
              if (obj.socketXHeader[n] == null) {
                obj.socketXHeader[n] = [v];
              } else {
                obj.socketXHeader[n].push(v);
              }
            } else {
              obj.socketXHeader[n] = v;
            }
        ...
        if (
          obj.socketXHeader['content-type'] != null &&
          obj.socketXHeader['content-type'].toLowerCase().indexOf('text/event-stream') >= 0
        ) {
        ...
        if (
          obj.socketXHeader['transfer-encoding'] != null &&
          obj.socketXHeader['transfer-encoding'].toLowerCase() == 'chunked'
        ) {
          obj.socketParseState = 1;
        }
        if (obj.isWebSocket) {
      ...
      if (obj.socketParseState == 1) {
        ...
        if (obj.socketXHeader['content-length'] != null) {
          ...
          if (obj.socketContentLengthRemaining == 0) {
            obj.socketContentLengthRemaining = parseInt(obj.socketXHeader['content-length']);
          } // Set the remaining content-length if not set
          ...
          if (obj.socketContentLengthRemaining > 0) {
            ...
            const closing =
              obj.socketXHeader['connection'] != null && obj.socketXHeader['connection'].toLowerCase() == 'close';
            if (closing) {
          ...
          if (clen < 0) {
            return;
          } // Chunk length not found, exit now and get more data.
      ...
      if (obj.socketParseState == 2) {
        ...
        if (len == 127) {
          ...
          if (len > 0) {
            obj.close();
            return;
          } // This frame is larger than 4 gigabyte, close the connection.
        ...
        switch (op) {
          ...
          case 1: {
            try {
              obj.ws.send(payload.toString('binary'));
            } catch (ex) {}
            break;
          } // Text frame
          case 2: {
  ...
  function processHttpResponse(header, data, done, closed) {
    ...
    if (obj.isWebSocket == false) {
      ...
      if (header != null) {
        ...
        if (!isNaN(statusCode) && statusCode > 0 && statusCode <= 999) {
          obj.res.status(statusCode);
        } // Set the status
        const blockHeaders = [
          'Directive',
          ...
          'cache-control'
        ]; // We do not forward these headers
        for (var i in header) {
          if (i == 'set-cookie') {
            for (var ii in header[i]) {
              ...
              var newCookieName = null,
                newCookie = {};
              for (var j in cookieSplit) {
                var l = cookieSplit[j].indexOf('='),
                  k = null,
                  v = null;
                if (l == -1) {
                  k = cookieSplit[j].trim();
                } else {
                  k = cookieSplit[j].substring(0, l).trim();
                  v = cookieSplit[j].substring(l + 1).trim();
                }
                if (j == 0) {
                  newCookieName = k;
                  newCookie.value = v;
                } else {
                  newCookie[k.toLowerCase()] = v == null ? true : v;
                }
              }
              if (newCookieName != null) {
                if (typeof newCookie['max-age'] == 'string' && parseInt(newCookie['max-age']) <= 0) {
                  ...
                  if (newCookie.httponly != true) {
                    obj.res.set(i, header[i]);
                  } // if the cookie is not HTTP-only, forward it to the browser. We need to do this to allow JavaScript to read it.
                  //console.log('new-cookie', newCookieName, newCookie);
                }
              }
            }
          } else if (blockHeaders.indexOf(i) == -1) {
            obj.res.set(i.trim(), header[i]);
          } // Set the headers if not blocked
        }
      ...
      if (done == true) {
        ...
        obj.processedRequestCount++;
        if (obj.oncompleted) {
          obj.oncompleted(obj.tunnelId, closed);
        }
      }
    } else {
...
module.exports.CreateMstscRelay = function (parent, db, ws, req, args, domain) {
  ...
  obj.close = function (arg) {
    ...
    if (obj.startTime && obj.meshid != null) {
      ...
      if (obj.wsClient != null) {
        inTraffc += obj.wsClient._socket.bytesRead;
        outTraffc += obj.wsClient._socket.bytesWritten;
      }
      const sessionSeconds = Math.round((Date.now() - obj.startTime) / 1000);
      ...
      const event = {
        ...
        msgid: 125,
        msgArgs: [sessionSeconds, obj.sessionid],
        msg: 'Left Web-RDP session "' + obj.sessionid + '" after ' + sessionSeconds + ' second(s).',
        protocol: PROTOCOL_WEBRDP,
        bytesin: inTraffc,
        bytesout: outTraffc
      };
    ...
    obj.tcpServer.on('connection', function (socket) {
      if (obj.relaySocket != null) {
        ...
        obj.relaySocket.on('data', function (chunk) {
          // Make sure to handle flow control.
          if (obj.relayActive == true) {
            obj.relaySocket.pause();
            if (obj.wsClient != null) {
              obj.wsClient.send(chunk, function () {
                obj.relaySocket.resume();
              });
            }
          }
        });
        obj.relaySocket.on('end', function () {
          obj.close();
        });
        obj.relaySocket.on('error', function (err) {
          obj.close();
        });
        ...
        var url =
          ...
          domainadd +
          (obj.mtype == 3 && obj.relaynodeid == null ? 'local' : 'mesh') +
          'relay.ashx?p=10&auth=' +
          obj.infos.ip; // Protocol 10 is Web-RDP
        if (domain.id != '') {
          url += '&domainid=' + domain.id;
        } // Since we are using "localhost", we are going to signal what domain we are on using a URL argument.
        ...
        obj.wsClient = new WebSocket(url, options);
        obj.wsClient.on('open', function () {
          parent.parent.debug('relay', 'RDP: Relay websocket open');
        });
        obj.wsClient.on('message', function (data) {
          // Make sure to handle flow control.
          if (obj.relayActive == false) {
            ...
            try {
              ...
              if (cmd != null && cmd.ctrlChannel == '102938') {
                if (cmd.type == 'ping') {
                  send(['ping']);
                } else if (cmd.type == 'pong') {
                  send(['pong']);
                }
              }
              return;
            } catch (ex) {
              // You are not JSON data so just send over relaySocket
              obj.wsClient._socket.pause();
              try {
                obj.relaySocket.write(data, function () {
                  if (obj.wsClient && obj.wsClient._socket) {
                    try {
                      obj.wsClient._socket.resume();
                    } catch (ex) {
                      console.log(ex);
                    }
                  }
        ...
        obj.wsClient.on('close', function () {
          parent.parent.debug('relay', 'RDP: Relay websocket closed');
          obj.close();
        });
        obj.wsClient.on('error', function (err) {
          parent.parent.debug('relay', 'RDP: Relay websocket error: ' + err);
          obj.close();
        });
        obj.tcpServer.close();
        obj.tcpServer = null;
  ...
  function startRdp(port) {
    ...
    try {
      ...
      if (obj.infos.options) {
        if (obj.infos.options.flags != null) {
          args.perfFlags = obj.infos.options.flags;
          delete obj.infos.options.flags;
        }
        if (obj.infos.options.workingDir != null && obj.infos.options.workingDir != '') {
          args.workingDir = obj.infos.options.workingDir;
        }
        if (obj.infos.options.alternateShell != null && obj.infos.options.alternateShell != '') {
          args.alternateShell = obj.infos.options.alternateShell;
        }
      }
      rdpClient = require('./rdp')
        .createClient(args)
        .on('connect', function () {
          send(['rdp-connect']);
          if (typeof obj.infos.options == 'object' && obj.infos.options.savepass == true) {
            saveRdpCredentials();
          } // Save the credentials if needed
          obj.sessionid = Buffer.from(parent.crypto.randomBytes(9), 'binary')
            .toString('base64')
            .replace(/\+/g, '@')
            .replace(/\//g, '$');
          ...
          try {
            ...
            const event = {
              ...
              sessionid: obj.sessionid,
              msgid: 150,
              msgArgs: [obj.sessionid],
              msg: 'Started Web-RDP session "' + obj.sessionid + '".',
              protocol: PROTOCOL_WEBRDP
            };
            parent.parent.DispatchEvent(['*', obj.nodeid, obj.userid, obj.meshid], obj, event);
          } catch (ex) {
        ...
        .on('bitmap', function (bitmap) {
          try {
            ws.send(bitmap.data);
          } catch (ex) {} // Send the bitmap data as binary
          delete bitmap.data;
        ...
        .on('error', function (err) {
          ...
          if (typeof err == 'object' && err.err && err.code) {
            send(['rdp-error', err.err, err.code]);
          }
        })
  ...
  function saveRdpCredentials() {
    ...
    parent.parent.db.Get(obj.nodeid, function (err, nodes) {
      ...
      if (
        typeof node.rdp[obj.userid] == 'object' &&
        node.rdp[obj.userid].d == obj.infos.domain &&
        node.rdp[obj.userid].u == obj.infos.username &&
        node.rdp[obj.userid].p == obj.infos.password
      )
        return;

      // Clear up any existing credentials or credentials for users that don't exist anymore
      for (var i in node.rdp) {
        if (!i.startsWith('user/') || parent.users[i] == null) {
          delete node.rdp[i];
        }
      }
      ...
      const event = {
        ...
        msg: 'Changed RDP credentials'
      };
      if (parent.parent.db.changeStream) {
        event.noact = 1;
      ...
      switch (msg[0]) {
        case 'infos': {
          ...
          if (obj.infos.ip.startsWith('node/')) {
            ...
            if (obj.cookie == null || typeof obj.cookie.nodeid != 'string' || typeof obj.cookie.userid != 'string') {
              obj.close();
              return;
            }
          ...
          parent.GetNodeWithRights(domain, obj.userid, obj.nodeid, function (node, rights, visible) {
            ...
            if (node == null || visible == false || (rights & MESHRIGHT_REMOTECONTROL) == 0) {
              obj.close();
              return;
            }
            ...
            if (rights != MESHRIGHT_ADMIN && (rights & MESHRIGHT_DESKLIMITEDINPUT) != 0) {
              obj.limitedinput = true;
            }
            ...
            if (mesh && mesh.relayid) {
              ...
              var tcpport = 3389;
              if (obj.cookie != null && obj.cookie.tcpport != null) {
                tcpport = obj.cookie.tcpport;
              } else {
                if (node.rdpport) {
              ...
              const cookieContent = {
                ...
                domainid: domain.id,
                nodeid: obj.nodeid,
                tcpport: node.rdpport ? node.rdpport : 3389
              };
              obj.infos.ip = parent.parent.encodeCookie(cookieContent, parent.parent.loginCookieEncryptionKey);
            }
            ...
            checkRelayRights(parent, domain, obj.userid, obj.relaynodeid, function (allowed) {
              if (obj.ws == null) return; // obj has been cleaned up, just exit.
              if (allowed !== true) {
                parent.parent.debug('relay', 'RDP: Attempt to use un-authorized relay');
                obj.close();
                return;
              }
              ...
              if (typeof obj.infos.options == 'object' && obj.infos.options.useServerCreds == true) {
                ...
                if (
                  domain.allowsavingdevicecredentials !== false &&
                  typeof node.rdp == 'object' &&
                  typeof node.rdp[obj.userid] == 'object' &&
                  typeof node.rdp[obj.userid].d == 'string' &&
                  typeof node.rdp[obj.userid].u == 'string' &&
                  typeof node.rdp[obj.userid].p == 'string'
                ) {
        ...
        case 'mouse': {
          if (rdpClient && obj.viewonly != true) {
            rdpClient.sendPointerEvent(msg[1], msg[2], msg[3], msg[4]);
          }
          break;
        }
        case 'wheel': {
          if (rdpClient && obj.viewonly != true) {
            rdpClient.sendWheelEvent(msg[1], msg[2], msg[3], msg[4]);
          }
          break;
        }
        case 'clipboard': {
          rdpClient.setClipboardData(msg[1]);
          break;
        }
        case 'scancode': {
          if (obj.limitedinput == true) {
          ...
          var extended = false;
          var extendedkeys = [
            57419, 57421, 57416, 57424, 57426, 57427, 57417, 57425, 57372, 57397, 57415, 57423, 57373, 57400, 57399
          ];
          ...
          if (rdpClient && obj.viewonly != true) {
            rdpClient.sendKeyEventScancode(msg[1], msg[2], extended);
          }
          break;
        }
        case 'unicode': {
          if (rdpClient && obj.viewonly != true) {
            rdpClient.sendKeyEventUnicode(msg[1], msg[2]);
          }
          break;
        }
        case 'utype': {
          ...
          if (obj.utypetimer == null) {
            obj.utypetimer = setInterval(function () {
              if (obj.utype == null || obj.utype.length == 0) {
                clearInterval(obj.utypetimer);
                obj.utypetimer = null;
                return;
              }
              ...
              if (c == 13) return;
              if (c == 10) {
                rdpClient.sendKeyEventScancode(28, true);
                rdpClient.sendKeyEventScancode(28, false);
              } else {
                rdpClient.sendKeyEventUnicode(c, true);
                rdpClient.sendKeyEventUnicode(c, false);
              }
            }, 5);
          }
          break;
        ...
        case 'ping': {
          try {
            obj.wsClient.send('{"ctrlChannel":102938,"type":"ping"}');
          } catch (ex) {}
          break;
        }
        case 'pong': {
          try {
            obj.wsClient.send('{"ctrlChannel":102938,"type":"pong"}');
          } catch (ex) {}
          break;
        }
        case 'disconnect': {
  ...
  ws.on('error', function (err) {
    parent.parent.debug('relay', 'RDP: Browser websocket error: ' + err);
    obj.close();
  });

  // If the web socket is closed
  ws.on('close', function (req) {
    parent.parent.debug('relay', 'RDP: Browser websocket closed');
    obj.close();
  });
  ...
  function send(obj) {
    try {
      rdpClient.bufferLayer.socket.pause();
    } catch (ex) {}
    try {
      ws.send(JSON.stringify(obj), function () {
        try {
          rdpClient.bufferLayer.socket.resume();
        } catch (ex) {}
      });
    } catch (ex) {}
  }
...
module.exports.CreateSshRelay = function (parent, db, ws, req, args, domain) {
  ...
  obj.close = function (arg) {
    ...
    if (obj.startTime && obj.meshid != null) {
      ...
      const event = {
        ...
        domain: domain.id,
        nodeid: obj.nodeid,
        userid: obj.cookie.userid,
        username: username,
        sessionid: obj.sessionid,
        msgid: 123,
        msgArgs: [sessionSeconds, obj.sessionid],
        msg: 'Left Web-SSH session "' + obj.sessionid + '" after ' + sessionSeconds + ' second(s).',
        protocol: PROTOCOL_WEBSSH,
        bytesin: inTraffc,
        bytesout: outTraffc
      };
  ...
  function saveSshCredentials(keep) {
    ...
    parent.parent.db.Get(obj.nodeid, function (err, nodes) {
      ...
      if (obj.password != null) {
        ...
        if (keep == 2) {
          node.ssh[obj.userid].kp = obj.privateKeyPass;
        }
      } else return;
      parent.parent.db.Set(node);
  ...
  function startRelayConnection() {
    try {
      ...
      var url =
        ...
        domainadd +
        (obj.mtype == 3 && obj.relaynodeid == null ? 'local' : 'mesh') +
        'relay.ashx?p=11&auth=' +
        obj.xcookie; // Protocol 11 is Web-SSH
      if (domain.id != '') {
        url += '&domainid=' + domain.id;
      } // Since we are using "localhost", we are going to signal what domain we are on using a URL argument.
      ...
      obj.wsClient = new WebSocket(url, options);
      obj.wsClient.on('open', function () {
        parent.parent.debug('relay', 'SSH: Relay websocket open');
      });
      obj.wsClient.on('message', function (data) {
        // Make sure to handle flow control.
        if (obj.relayActive == false) {
          if (data == 'c' || data == 'cr') {
            ...
            obj.sshClient.on('ready', function () {
              ...
              try {
                ...
                const event = {
                  ...
                  msgid: 148,
                  msgArgs: [obj.sessionid],
                  msg: 'Started Web-SSH session "' + obj.sessionid + '".',
                  protocol: PROTOCOL_WEBSSH
                };
              ...
              obj.sshClient.shell(function (err, stream) {
                ...
                obj.sshShell.on('close', function () {
                  obj.close();
                });
                obj.sshShell.on('data', function (data) {
                  obj.ws.send('~' + data.toString());
                });
              });
            ...
            obj.sshClient.on('error', function (err) {
              if (err.level == 'client-authentication') {
                try {
                  obj.ws.send(JSON.stringify({ action: 'autherror' }));
                } catch (ex) {}
              }
              if (err.level == 'client-timeout') {
                try {
                  obj.ws.send(JSON.stringify({ action: 'sessiontimeout' }));
                } catch (ex) {}
              }
              obj.close();
            });
            ...
            obj.ser.forwardwrite = function (data) {
              if (data.length > 0 && obj.wsClient != null) {
                try {
                  obj.wsClient.send(data);
                } catch (ex) {}
              }
            ...
            if (typeof obj.username == 'string') {
              connectionOptions.username = obj.username;
            }
            if (typeof obj.password == 'string') {
              connectionOptions.password = obj.password;
            }
            if (typeof obj.privateKey == 'string') {
              connectionOptions.privateKey = obj.privateKey;
            }
            if (typeof obj.privateKeyPass == 'string') {
              connectionOptions.passphrase = obj.privateKeyPass;
            }
            try {
          ...
          try {
            ...
            var cmd = null;
            cmd = JSON.parse(data);
            if (cmd != null && cmd.ctrlChannel == '102938' && (cmd.type == 'ping' || cmd.type == 'pong')) {
              obj.ws.send(data);
            }
            return;
          } catch (ex) {
            // Relay WS --> SSH instead
            if (data.length > 0 && obj.ser != null) {
              try {
                obj.ser.updateBuffer(data);
              } catch (ex) {
                console.log(ex);
      ...
      obj.wsClient.on('close', function () {
        parent.parent.debug('relay', 'SSH: Relay websocket closed');
        obj.close();
      });
      obj.wsClient.on('error', function (err) {
        parent.parent.debug('relay', 'SSH: Relay websocket error: ' + err);
        obj.close();
      });
    } catch (ex) {
      console.log(ex);
  ...
  ws.on('message', function (data) {
    try {
      ...
      if (data[0] == '{') {
        ...
        if (msg == null || typeof msg != 'object') return;
        if (msg.ctrlChannel == '102938' && (msg.type == 'ping' || msg.type == 'pong')) {
          try {
            obj.wsClient.send(data);
          } catch (ex) {}
          return;
            ...
            if (msg.useexisting) {
              ...
              parent.parent.db.Get(obj.cookie.nodeid, function (err, nodes) {
                ...
                if (
                  ...
                  node.ssh[obj.userid] == null ||
                  typeof node.ssh[obj.userid].u != 'string' ||
                  (typeof node.ssh[obj.userid].p != 'string' && typeof node.ssh[obj.userid].k != 'string')
                ) {
                  // Send a request for SSH authentication
                  try {
                    ws.send(JSON.stringify({ action: 'sshauth' }));
                  } catch (ex) {}
                } else if (
                  domain.allowsavingdevicecredentials !== false &&
                  node.ssh != null &&
                  typeof node.ssh[obj.userid].k == 'string' &&
                  node.ssh[obj.userid].kp == null
                ) {
                  ...
                  try {
                    ws.send(JSON.stringify({ action: 'sshauth', askkeypass: true }));
                  } catch (ex) {}
                } else {
              ...
              // Verify inputs
              if (typeof msg.username != 'string' || (typeof msg.password != 'string' && typeof msg.key != 'string'))
                break;
              if (
              ...
              if (msg.keep === true) {
                msg.keep = 1;
              } // If true, change to 1. For user/pass, 1 to store user/pass in db. For user/key/pass, 1 to store user/key in db, 2 to store everything in db.
        ...
        // Terminal data
        if (obj.sshShell != null) {
          obj.sshShell.write(data.substring(1));
        }
      }
    } catch (ex) {
      obj.close();
  ...
  // If error, do nothing
  ws.on('error', function (err) {
    parent.parent.debug('relay', 'SSH: Browser websocket error: ' + err);
    obj.close();
  });

  // If the web socket is closed
  ws.on('close', function (req) {
    parent.parent.debug('relay', 'SSH: Browser websocket closed');
    obj.close();
  });
  ...
  if (obj.cookie == null || obj.cookie.userid == null || parent.users[obj.cookie.userid] == null) {
    obj.ws.send(JSON.stringify({ action: 'sessionerror' }));
    obj.close();
    return;
  }
  obj.userid = obj.cookie.userid;
  ...
  parent.parent.db.Get(obj.cookie.nodeid, function (err, nodes) {
    ...
    if (err != null || nodes == null || nodes.length != 1) {
      parent.parent.debug('relay', 'SSH: Invalid device');
      obj.close();
    }
    const node = parent.common.unEscapeLinksFieldName(nodes[0]); // unEscape node data for rdp/ssh credentials
    ...
    if (mesh && mesh.relayid) {
      ...
      checkRelayRights(parent, domain, obj.cookie.userid, obj.relaynodeid, function (allowed) {
        if (obj.cookie == null) return; // obj has been cleaned up, just exit.
        if (allowed !== true) {
          parent.parent.debug('relay', 'SSH: Attempt to use un-authorized relay');
          obj.close();
          return;
        }
...
module.exports.CreateSshTerminalRelay = function (parent, db, ws, req, domain, user, cookie, args) {
  ...
  function saveSshCredentials(keep) {
    ...
    parent.parent.db.Get(obj.nodeid, function (err, nodes) {
      ...
      if (obj.password != null) {
        ...
        if (keep == 2) {
          node.ssh[user._id].kp = obj.privateKeyPass;
        }
      } else return;
      parent.parent.db.Set(node);
  ...
  function startRelayConnection(authCookie) {
    try {
      ...
      var url =
        ...
        (obj.mtype == 3 && obj.relaynodeid == null ? 'local' : 'mesh') +
        'relay.ashx?p=11&auth=' +
        authCookie; // Protocol 11 is Web-SSH
      if (domain.id != '') {
        url += '&domainid=' + domain.id;
      } // Since we are using "localhost", we are going to signal what domain we are on using a URL argument.
      parent.parent.debug('relay', 'SSH: Connection websocket to ' + url);
        ...
        if (obj.relayActive == false) {
          if (data == 'c' || data == 'cr') {
            ...
            try {
              ...
              try {
                ws.send(
                  JSON.stringify({ action: 'sshauth', askkeypass: obj.username != null && obj.privateKey != null })
                );
              } catch (ex) {}
            }
      ...
      obj.wsClient.on('close', function () {
        if (obj.connected !== true) {
          try {
            obj.ws.send(JSON.stringify({ action: 'connectionerror' }));
          } catch (ex) {}
        }
        parent.parent.debug('relay', 'SSH: Relay websocket closed');
        obj.close();
  ...
  parent.GetNodeWithRights(domain, user, req.query.nodeid, function (node, rights, visible) {
    ...
    if ((rights & 8) == 0) {
      obj.close();
      return;
    } // No MESHRIGHT_REMOTECONTROL rights
    if (rights != 0xffffffff && rights & 0x00000200) {
      obj.close();
      return;
    } // MESHRIGHT_NOTERMINAL is set
    obj.mtype = node.mtype; // Store the device group type
    obj.nodeid = node._id; // Store the NodeID
    obj.meshid = node.meshid; // Store the MeshID
    ...
    checkRelayRights(parent, domain, user, obj.relaynodeid, function (allowed) {
      ...
      if (
        ...
        node.ssh == null ||
        typeof node.ssh != 'object' ||
        node.ssh[user._id] == null ||
        typeof node.ssh[user._id].u != 'string' ||
        (typeof node.ssh[user._id].p != 'string' && typeof node.ssh[user._id].k != 'string')
      ) {
        // Send a request for SSH authentication
        try {
          ws.send(JSON.stringify({ action: 'sshauth' }));
        ...
        try {
          ws.send(JSON.stringify({ action: 'sshautoauth' }));
        } catch (ex) {}
      }
    });
  });
...
module.exports.CreateSshFilesRelay = function (parent, db, ws, req, domain, user, cookie, args) {
  ...
  obj.close = function (arg) {
    ...
    if (obj.startTime) {
      ...
      const event = {
        ...
        msgid: 124,
        msgArgs: [sessionSeconds, obj.sessionid],
        msg: 'Left Web-SFTP session "' + obj.sessionid + '" after ' + sessionSeconds + ' second(s).',
        protocol: PROTOCOL_WEBSFTP,
        bytesin: inTraffc,
        bytesout: outTraffc
      };
      parent.parent.DispatchEvent(['*', obj.nodeid, user._id, obj.meshid], obj, event);
  ...
  function startRelayConnection(authCookie) {
    try {
      ...
      var url =
        ...
        domainadd +
        (obj.mtype == 3 && obj.relaynodeid == null ? 'local' : 'mesh') +
        'relay.ashx?p=13&auth=' +
        authCookie; // Protocol 13 is Web-SSH-Files
      if (domain.id != '') {
        url += '&domainid=' + domain.id;
      } // Since we are using "localhost", we are going to signal what domain we are on using a URL argument.
      parent.parent.debug('relay', 'SSH: Connection websocket to ' + url);
            ...
            obj.sshClient.on('ready', function () {
              ...
              try {
                const event = {
                  ...
                  msgid: 149,
                  msgArgs: [obj.sessionid],
                  msg: 'Started Web-SFTP session "' + obj.sessionid + '".',
                  protocol: PROTOCOL_WEBSFTP
                };
                parent.parent.DispatchEvent(['*', obj.nodeid, user._id, obj.meshid], obj, event);
              } catch (ex) {
      ...
      obj.wsClient.on('close', function () {
        ...
        parent.parent.debug('relay', 'SSH: Files relay websocket closed');
        obj.close();
      });
      obj.wsClient.on('error', function (err) {
        parent.parent.debug('relay', 'SSH: Files relay websocket error: ' + err);
        obj.close();
      });
    } catch (ex) {
      console.log(ex);
        ...
        if (data[0] == 123) {
          ...
          obj.sftp.write(obj.uploadHandle, data, off, data.length - off, obj.uploadPosition, function (err) {
            if (err != null) {
              ...
              try {
                obj.ws.send(Buffer.from(JSON.stringify({ action: 'uploaddone', reqid: obj.uploadReqid })));
              } catch (ex) {}
              ...
              try {
                obj.ws.send(Buffer.from(JSON.stringify({ action: 'uploadack', reqid: obj.uploadReqid })));
              } catch (ex) {}
            }
          });
      ...
      if (data[0] == '{') {
        ...
        switch (msg.action) {
          case 'ls': {
            ...
            if (requestedPath.startsWith('/') == false) {
            ...
            obj.sftp.readdir(requestedPath, function (err, list) {
              ...
              for (var i in list) {
                ...
                if (file.longname[0] == 'd') {
                  r.dir.push({ t: 2, n: file.filename, d: new Date(file.attrs.mtime * 1000).toISOString() });
                } else {
                  r.dir.push({
                    t: 3,
                    n: file.filename,
                    d: new Date(file.attrs.mtime * 1000).toISOString(),
                    s: file.attrs.size
                  });
                }
              }
              try {
                obj.ws.send(Buffer.from(JSON.stringify(r)));
              } catch (ex) {}
            });
            break;
          }
          case 'mkdir': {
            ...
            parent.parent.DispatchEvent(targets, obj, {
              ...
              username: user.name,
              msgid: 44,
              msgArgs: [requestedPath],
              msg: 'Create folder: \"' + requestedPath + '\"',
              domain: domain.id
            });
            break;
          }
          ...
          case 'rm': {
            ...
            for (var i in msg.delfiles) {
              ...
              if (msg.rec === true) {
                obj.sftp.rmdir(ul + '/', function (err) {});
              }
          ...
          case 'rename': {
            ...
            parent.parent.DispatchEvent(targets, obj, {
              ...
              username: user.name,
              msgid: 48,
              msgArgs: [oldpath, msg.newname],
              msg: 'Rename: \"' + oldpath + '\" to \"' + msg.newname + '\"',
              domain: domain.id
            });
            break;
          }
          case 'upload': {
            ...
            obj.sftp.open(obj.uploadFullpath, 'w', 0o666, function (err, handle) {
              if (err != null) {
                try {
                  obj.ws.send(Buffer.from(JSON.stringify({ action: 'uploaderror', reqid: obj.uploadReqid })));
                } catch (ex) {}
              } else {
                obj.uploadHandle = handle;
                try {
                  obj.ws.send(Buffer.from(JSON.stringify({ action: 'uploadstart', reqid: obj.uploadReqid })));
                } catch (ex) {}
                ...
                parent.parent.DispatchEvent(targets, obj, {
                  ...
                  username: user.name,
                  msgid: 105,
                  msgArgs: [obj.uploadFullpath, obj.uploadSize],
                  msg: 'Upload: ' + obj.uploadFullpath + ', Size: ' + obj.uploadSize,
                  domain: domain.id
                });
              }
            });
          ...
          case 'uploadcancel': {
            ...
            if (obj.uploadHandle != null) {
              ...
              try {
                obj.ws.send(Buffer.from(JSON.stringify({ action: 'uploadcancel', reqid: obj.uploadReqid })));
              } catch (ex) {}
              delete obj.uploadHandle;
              delete obj.uploadFullpath;
              delete obj.uploadSize;
          ...
          case 'download': {
            ...
            switch (msg.sub) {
              case 'start': {
                ...
                obj.sftp.open(obj.downloadFullpath, 'r', function (err, handle) {
                  if (err != null) {
                    try {
                      obj.ws.send(
                        Buffer.from(JSON.stringify({ action: 'download', sub: 'cancel', id: obj.downloadId }))
                      );
                    } catch (ex) {}
                  } else {
                    obj.downloadHandle = handle;
                    try {
                      obj.ws.send(JSON.stringify({ action: 'download', sub: 'start', id: obj.downloadId }));
                    } catch (ex) {}
                    ...
                    parent.parent.DispatchEvent(targets, obj, {
                      ...
                      username: user.name,
                      msgid: 49,
                      msgArgs: [obj.downloadFullpath],
                      msg: 'Download: ' + obj.downloadFullpath,
                      domain: domain.id
                    });
                  }
                });
              ...
              case 'ack': {
                if (obj.downloadHandle == null || obj.downloadId != msg.id) break;
                if (obj.downloadPendingBlockCount == 0) {
                  obj.downloadPendingBlockCount = 1;
                  uploadNextBlock();
                }
                break;
              }
              case 'stop': {
                if (obj.downloadHandle == null || obj.downloadId != msg.id) break;
                if (obj.downloadHandle != null) {
                  obj.sftp.close(obj.downloadHandle, function () {});
                }
  ...
  function uploadNextBlock() {
    ...
    obj.sftp.read(
      ...
      obj.downloadBuffer,
      4,
      obj.downloadBuffer.length - 4,
      obj.downloadPosition,
      function (err, len, buf) {
        obj.downloadPendingBlockCount--;
        if (obj.downloadBuffer == null) return;
        if (err != null) {
          try {
            obj.ws.send(Buffer.from(JSON.stringify({ action: 'download', sub: 'cancel', id: obj.downloadId })));
          } catch (ex) {}
        } else {
          ...
          if (len < obj.downloadBuffer.length - 4) {
            ...
            if (len > 0) {
              try {
                obj.ws.send(obj.downloadBuffer.slice(0, len + 4));
              } catch (ex) {
                console.log(ex);
              }
            }
