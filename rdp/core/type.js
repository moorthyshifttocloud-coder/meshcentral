...
Type.prototype.write = function (s) {
  ...
  if (this.opt.conditional && !this.opt.conditional()) return this;
...
Type.prototype.read = function (s) {
  ...
  if (this.opt.optional && s.availableLength() < this.size()) return this;
  ...
  if (this.opt.constant) {
    ...
    if (oldValue !== this.value) {
      ...
      throw new error.ProtocolError(
        'NODE_RDP_CORE_TYPE_CONSTANT_VALUE_MISMATCH, OLD:' + oldValue + ', NEW:' + this.value
      );
    }
  } else {
    try {
...
Type.prototype.size = function () {
  if (this.opt.conditional && !this.opt.conditional()) return 0;
  return this._size_();
};
...
SingleType.prototype.writeValue = function (s) {
  ...
  if (typeof safeValue === 'number') {
    // FIX: Sanitize coordinates to prevent crashes
