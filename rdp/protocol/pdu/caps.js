...
function orderCapability(orders, opt) {
  ...
  var self = {
    ...
    terminalDescriptor: new type.BinaryString(Buffer.from(Array(16 + 1).join('\x00'), 'binary'), {
      readLength: new type.CallableValue(16)
    }),
...
function pointerCapability(isServer, opt) {
  var self = {
    ...
    pointerCacheSize: new type.UInt16Le(null, {
...
function inputCapability(opt) {
  var self = {
    ...
    imeFileName: new type.BinaryString(Buffer.from(Array(64 + 1).join('\x00'), 'binary'), {
      readLength: new type.CallableValue(64)
    })
  };
      ...
      new type.Factory(function (s) {
        switch (self.capabilitySetType.value) {
          case CapsType.CAPSTYPE_GENERAL:
            self.capability = generalCapability({
          ...
          case CapsType.CAPSTYPE_BITMAP:
            self.capability = bitmapCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_ORDER:
            self.capability = orderCapability(null, {
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_BITMAPCACHE:
            self.capability = bitmapCacheCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_POINTER:
            self.capability = pointerCapability(false, {
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_BRUSH:
            self.capability = brushCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_GLYPHCACHE:
            self.capability = glyphCapability(null, {
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_OFFSCREENCACHE:
            self.capability = offscreenBitmapCacheCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_VIRTUALCHANNEL:
            self.capability = virtualChannelCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_SOUND:
            self.capability = soundCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_CONTROL:
            self.capability = controlCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_ACTIVATION:
            self.capability = windowActivationCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_FONT:
            self.capability = fontCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_COLORCACHE:
            self.capability = colorCacheCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSTYPE_SHARE:
            self.capability = shareCapability({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          case CapsType.CAPSETTYPE_MULTIFRAGMENTUPDATE:
            self.capability = multiFragmentUpdate({
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
          ...
          default:
            ...
            self.capability = new type.BinaryString(null, {
              readLength: new type.CallableValue(function () {
                return self.lengthCapability.value - 4;
              })
            }).read(s);
