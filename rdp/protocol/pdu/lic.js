...
function licenseBinaryBlob(blobType) {
  ...
  var self = {
    ...
    blobData: new type.BinaryString(null, {
...
function productInformation() {
  var self = {
    ...
    pbCompanyName: new type.BinaryString(Buffer.from('Microsoft Corporation', 'ucs2'), {
      readLength: new type.CallableValue(function () {
    ...
    pbProductId: new type.BinaryString(Buffer.from('A02', 'ucs2'), {
      readLength: new type.CallableValue(function () {
...
function serverLicenseRequest(opt) {
  var self = {
    ...
    serverRandom: new type.BinaryString(Buffer.from(Array(32 + 1).join('\x00')), {
...
function clientNewLicenseRequest(opt) {
  var self = {
    ...
    clientRandom: new type.BinaryString(Buffer.from(Array(32 + 1).join('\x00')), {
      readLength: new type.CallableValue(32)
    }),
...
function clientPLatformChallengeResponse(opt) {
  var self = {
    ...
    MACData: new type.BinaryString(Buffer.from(Array(16 + 1).join('\x00'), 'binary'), {
      ...
      new type.Factory(function (s) {
        switch (self.bMsgtype.value) {
          case MessageType.ERROR_ALERT:
            self.licensingMessage = licensingErrorMessage({
              readLength: new type.CallableValue(function () {
          ...
          case MessageType.LICENSE_REQUEST:
            self.licensingMessage = serverLicenseRequest({
              readLength: new type.CallableValue(function () {
                return self.wMsgSize.value - 4;
              })
            }).read(s);
          ...
          case MessageType.NEW_LICENSE_REQUEST:
            self.licensingMessage = clientNewLicenseRequest({
              readLength: new type.CallableValue(function () {
                return self.wMsgSize.value - 4;
              })
            }).read(s);
          ...
          case MessageType.PLATFORM_CHALLENGE_RESPONSE:
            self.licensingMessage = clientPLatformChallengeResponse({
              readLength: new type.CallableValue(function () {
                return self.wMsgSize.value - 4;
              })
            }).read(s);
