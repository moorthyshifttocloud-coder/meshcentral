...
function Client(transport, fastPathTransport) {
  ...
  var self = this;
  this.transport
    .once('connect', function (core, userId, channelId) {
...
Client.prototype.recvDemandActivePDU = function (s) {
  ...
  this.transport.enableSecureCheckSum = !!(
    this.serverCapabilities[caps.CapsType.CAPSTYPE_GENERAL].obj.extraFlags.value &
...
Client.prototype.recvServerSynchronizePDU = function (s) {
  var pdu = data.pdu().read(s);
  if (
    pdu.obj.shareControlHeader.obj.pduType.value !== data.PDUType.PDUTYPE_DATAPDU ||
...
Client.prototype.sendConfirmActivePDU = function () {
  ...
  inputCapability.inputFlags.value =
    caps.InputFlags.INPUT_FLAG_SCANCODES | caps.InputFlags.INPUT_FLAG_MOUSEX | caps.InputFlags.INPUT_FLAG_UNICODE;
...
Client.prototype.sendInputEvents = function (inputEvents) {
  var pdu = data.clientInputEventPDU(
    new type.Component(
      inputEvents.map(function (e) {
