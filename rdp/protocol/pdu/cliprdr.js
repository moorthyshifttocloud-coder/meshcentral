...
class Client extends Cliprdr {
  constructor(transport, fastPathTransport) {
    ...
    this.transport
      .once('connect', (gccCore, userId, channelId) => {
  ...
  sendClientTemporaryDirectoryPDU(path = '') {
    ...
    this.send(
      new type.Component({
        ...
        wszTempDir: new type.BinaryString(Buffer.from('D:\\Vectors' + Array(251).join('\x00'), 'ucs2'), {
          readLength: new type.CallableValue(520)
        })
      })
    );
  }
  ...
  sendFormatListPDU() {
    this.send(
      new type.Component({
        ...
        formatName6: new type.BinaryString(Buffer.from('Native\x00', 'ucs2'), {
          readLength: new type.CallableValue(14)
        }),
