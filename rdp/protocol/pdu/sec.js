...
function rdpInfos(extendedInfoConditional) {
  var self = {
    ...
    domain: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
        return self.cbDomain.value + 2;
      })
    }),
    userName: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
        return self.cbUserName.value + 2;
      })
    }),
    password: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
        return self.cbPassword.value + 2;
      })
    }),
    alternateShell: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
        return self.cbAlternateShell.value + 2;
      })
    }),
    workingDir: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
...
function rdpExtendedInfos(opt) {
  var self = {
    ...
    clientAddress: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
    ...
    clientDir: new type.BinaryString(Buffer.from('\x00', 'ucs2'), {
      readLength: new type.CallableValue(function () {
...
Sec.prototype.sendFlagged = function (flag, data) {
  this.transport.send('global', new type.Component([new type.UInt16Le(flag), new type.UInt16Le(), data]));
};
...
function Client(transport, fastPathTransport) {
  ...
  var self = this;
  this.transport
    .on('connect', function (gccClient, gccServer, userId, channels) {
...
Client.prototype.sendClientChallengeResponse = function (platformChallenge) {
  ...
  var hwid = new type.Component([new type.UInt32Le(2), new type.BinaryString(crypto.randomBytes(16))]).toStream()
    .buffer;
...
Sec.prototype.recvLicense = function (s) {
  ...
  // i'm accepted
  if (
    message.bMsgtype.value === lic.MessageType.NEW_LICENSE ||
