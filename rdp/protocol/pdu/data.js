...
function demandActivePDU(capabilities, opt) {
  var self = {
    ...
    sourceDescriptor: new type.BinaryString(Buffer.from('node-rdpjs', 'binary'), {
...
function confirmActivePDU(capabilities, shareId, opt) {
  var self = {
    ...
    sourceDescriptor: new type.BinaryString(Buffer.from('rdpy', 'binary'), {
      readLength: new type.CallableValue(function () {
        return self.lengthSourceDescriptor.value;
      })
    }),
...
function persistentListPDU(entries, opt) {
  var self = {
    ...
    entries:
      ...
      new type.Factory(function (s) {
        var numEntries =
          self.numEntriesCache0.value +
          self.numEntriesCache1.value +
          self.numEntriesCache2.value +
          self.numEntriesCache3.value +
          self.numEntriesCache4.value;
...
function bitmapData(coord, opt) {
  ...
  var self = {
    ...
    bitmapComprHdr: bitmapCompressedDataHeader({
      conditional: function () {
        return (
          self.flags.value & BitmapFlag.BITMAP_COMPRESSION && !(self.flags.value & BitmapFlag.NO_BITMAP_COMPRESSION_HDR)
        );
      }
    }),
    bitmapDataStream: new type.BinaryString(coord.data, {
      readLength: new type.CallableValue(function () {
        if (
          !self.flags.value & BitmapFlag.BITMAP_COMPRESSION ||
...
function dataPDU(pduData, shareId, opt) {
  var self = {
    ...
    shareDataHeader: shareDataHeader(
      new type.CallableValue(function () {
