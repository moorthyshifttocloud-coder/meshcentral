...
const RdpdrChannelDef = new type.Component({
  name: new type.BinaryString(Buffer.from('rdpdr' + '\x00\x00\x00', 'binary'), {
    readLength: new type.CallableValue(8)
  }),
...
function Client(transport) {
  ...
  this.clientNetworkData = gcc.clientNetworkData(
    new type.Component([RdpdrChannelDef, CliprdrChannelDef, RdpsndChannelDef])
  );
...
Client.prototype.recvChannelJoinConfirm = function (s) {
  ...
  if (
    confirm !== 0 &&
    (channelId === Channel.MCS_CLIPRDR_CHANNEL || channelId === Channel.MCS_GLOBAL_CHANNEL || channelId === this.userId)
  ) {
...
Client.prototype.sendErectDomainRequest = function () {
  this.transport.send(
    new type.Component([writeMCSPDUHeader(DomainMCSPDU.ERECT_DOMAIN_REQUEST), per.writeInteger(0), per.writeInteger(0)])
  );
};
...
function Server(transport) {
  ...
  var self = this;
  this.transport
    .on('connect', function (selectedProtocol) {
