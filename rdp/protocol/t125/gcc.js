...
function clientCoreData(opt) {
  var self = {
    ...
    clientName: new type.BinaryString(Buffer.from('node-rdpjs\x00\x00\x00\x00\x00\x00', 'ucs2'), {
      readLength: new type.CallableValue(32)
    }),
    ...
    imeFileName: new type.BinaryString(Buffer.from(Array(64 + 1).join('\x00')), {
    ...
    supportedColorDepths: new type.UInt16Le(
      Support.RNS_UD_15BPP_SUPPORT |
    ...
    clientDigProductId: new type.BinaryString(Buffer.from(Array(64 + 1).join('\x00')), {
      optional: true,
      readLength: new type.CallableValue(64)
    }),
...
function clientSecurityData(opt) {
  var self = {
    ...
    encryptionMethods: new type.UInt32Le(
...
function writeConferenceCreateRequest(userData) {
  ...
  return new type.Component([
    per.writeChoice(0),
    per.writeObjectIdentifier(t124_02_98_oid),
    per.writeLength(userData.size() + 14),
    per.writeChoice(0),
    per.writeSelection(0x08),
    per.writeNumericString('1', 1),
    per.writePadding(1),
    per.writeNumberOfSet(1),
    per.writeChoice(0xc0),
    per.writeOctetStream(Buffer.from(h221_cs_key), 4),
...
function writeConferenceCreateResponse(userData) {
  ...
  return new type.Component([
    ...
    per.writeObjectIdentifier(t124_02_98_oid),
    per.writeLength(userData.size() + 14),
    per.writeChoice(0x14),
    per.writeInteger16(0x79f3, 1001),
    per.writeInteger(1),
    per.writeEnumerates(0),
    per.writeNumberOfSet(1),
    per.writeChoice(0xc0),
    per.writeOctetStream(Buffer.from(h221_sc_key), 4),
    per.writeOctetStream(userDataStream.getValue())
  ]);
}
