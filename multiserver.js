...
module.exports.CreateMultiServer = function (parent, args) {
  ...
  obj.CreatePeerOutServer = function (parent, serverid, url) {
    ...
    function connect() {
      ...
      obj.ws = new WebSocket(obj.url + 'meshserver.ashx', {
        rejectUnauthorized: false,
        servername: obj.certificates.CommonName,
        cert: obj.certificates.agent.cert,
        key: obj.certificates.agent.key
      });
      obj.parent.parent.debug('peer', 'OutPeer ' + obj.serverid + ': Connecting to: ' + url + 'meshserver.ashx');

      // Register the connection failed event
      obj.ws.on('error', function (error) {
        obj.parent.parent.debug('peer', 'OutPeer ' + obj.serverid + ': Error: ' + error);
        disconnect();
      });
      obj.ws.on('close', function () {
        obj.parent.parent.debug('peer', 'OutPeer ' + obj.serverid + ': Disconnected');
        disconnect();
      });
      ...
      obj.ws.on('open', function () {
        ...
        if (obj.url.toLowerCase().startsWith('wss://')) {
          ...
          var serverCert = obj.forge.pki.certificateFromAsn1(
            obj.forge.asn1.fromDer(obj.ws._socket.getPeerCertificate().raw.toString('binary'))
          );
          obj.serverCertHash = obj.forge.pki.getPublicKeyFingerprint(serverCert.publicKey, {
            encoding: 'binary',
      ...
      obj.ws.on('message', function (msg) {
        ...
        if (msg.charCodeAt(0) == 123) {
          if ((obj.connectionState & 4) != 0) {
            processServerData(msg);
          } else {
            obj.pendingData.push(msg);
          }
        } else {
          ...
          switch (cmd) {
            case 1: {
              ...
              if (msg.length != 98) {
                obj.parent.parent.debug(
                  'peer',
                  'OutPeer: Bad server authentication message, length = ' +
                    msg.length +
                    ', should be 98. HEX: ' +
                    Buffer.from(msg.substring(0, 4096), 'binary').toString('hex')
                );
                return;
              }

              // Check that the server hash matches the TLS server certificate public key hash
              if (obj.url.toLowerCase().startsWith('wss://') && obj.serverCertHash != msg.substring(2, 50)) {
                obj.parent.parent.debug('peer', 'OutPeer: Server hash mismatch.');
                disconnect();
                return;
              }
              ...
              obj.parent.parent.certificateOperations.acceleratorPerformSignature(
                0,
                msg.substring(2) + obj.nonce,
                null,
                function (tag, signature) {
                  ...
                  if (obj.ws != null) {
                    obj.ws.send(
                      Buffer.from(
                        obj.common.ShortToStr(2) +
                          obj.common.ShortToStr(obj.agentCertificateAsn1.length) +
                          obj.agentCertificateAsn1 +
                          signature,
                        'binary'
                      )
                    );
                  } // Command 2, certificate + signature
                }
              );
            ...
            case 2: {
              ...
              var serverCertPem =
                ...
                '\r\n-----END CERTIFICATE-----';
              try {
                serverCert = obj.forge.pki.certificateFromAsn1(obj.forge.asn1.fromDer(msg.substring(4, 4 + certlen)));
              } catch (e) {}
              if (serverCert == null) {
                obj.parent.parent.debug('peer', 'OutPeer: Invalid server certificate.');
                disconnect();
                return;
              }
              var serverid = Buffer.from(
                obj.forge.pki.getPublicKeyFingerprint(serverCert.publicKey, {
                  encoding: 'binary',
                  md: obj.forge.md.sha384.create()
                }),
                'binary'
                ...
                .replace(/\+/g, '@')
                .replace(/\//g, '$');
              if (serverid !== obj.agentCertificateHashBase64) {
                obj.parent.parent.debug('peer', 'OutPeer: Server hash mismatch.');
                disconnect();
                return;
              }
              ...
              if (verify.verify(serverCertPem, Buffer.from(msg.substring(4 + certlen), 'binary')) !== true) {
                obj.parent.parent.debug('peer', 'OutPeer: Server sign check failed.');
                disconnect();
                return;
              }
              ...
              obj.serverCertHash = Buffer.from(obj.serverCertHash, 'binary')
                .toString('base64')
                .replace(/\+/g, '@')
                .replace(/\//g, '$'); // Change this value to base64
              ...
              if (obj.connectionState == 15) {
                obj.send({
                  action: 'info',
                  serverid: obj.parent.serverid,
                  dbid: obj.parent.parent.db.identifier,
                  key: obj.parent.parent.serverKey.toString('hex'),
                  serverCertHash: obj.parent.parent.webserver.webCertificateHashBase64
                });
                for (var i in obj.pendingData) {
                  processServerData(obj.pendingData[i]);
    ...
    function disconnect() {
      if (obj.authenticated == 3) {
        obj.parent.ClearPeerServer(obj, obj.peerServerId);
        obj.authenticated = 0;
      }
      if (obj.connectionState == 15 && obj.connectHandler != null) {
        obj.connectHandler(0);
      }
      if (obj.ws != null) {
        obj.ws.close();
        obj.ws = null;
      }
      if (obj.retryTimer != null) {
        clearTimeout(obj.retryTimer);
        obj.retryTimer = null;
      }
      // Re-try connection
      if (obj.connectionState >= 1) {
        obj.connectionState = 1;
        if (obj.retryTimer == null) {
          obj.retryTimer = setTimeout(connect, getConnectRetryTime());
        }
      }
    }
    ...
    function getConnectRetryTime() {
      ...
      if (obj.retryBackoff < 30000) {
        obj.retryBackoff += (require('crypto').randomBytes(4).readUInt32BE(0) & 0x1fff) + 1000;
      }
    ...
    obj.send = function (msg) {
      try {
        if (obj.ws == null || obj.connectionState != 15) {
          return;
        }
        if (typeof msg == 'string') {
          obj.ws.send(msg);
        ...
        if (typeof msg == 'object') {
          obj.ws.send(JSON.stringify(msg));
          return;
        }
      } catch (ex) {}
    };
    ...
    function processServerData(msg) {
      ...
      if (str[0] == '{') {
        try {
          ...
          obj.parent.parent.debug('peer', 'Unable to parse server JSON (' + obj.remoteaddr + ').');
          return;
        ...
        if (command.action == 'info') {
          if (obj.authenticated != 3) {
            ...
            if (command.serverid != null && command.dbid != null) {
              if (command.serverid == obj.parent.serverid) {
                console.log(
                  'ERROR: Same server ID, trying to peer with self. (' + obj.url + ', ' + command.serverid + ').'
                );
                return;
              }
              if (command.dbid != obj.parent.parent.db.identifier) {
                console.log(
                  'ERROR: Database ID mismatch. Trying to peer to a server with the wrong database. (' +
                    obj.url +
                    ', ' +
                    command.serverid +
                    ').'
              ...
              if (obj.url.toLowerCase().startsWith('wss://') && obj.serverCertHash != command.serverCertHash) {
                console.log('ERROR: Outer certificate hash mismatch (2). (' + obj.url + ', ' + command.serverid + ').');
                return;
              }
  ...
  obj.CreatePeerInServer = function (parent, ws, req, tls) {
    ...
    obj.serverCertHash = null;
    obj.pendingData = [];
    if (obj.remoteaddr.startsWith('::ffff:')) {
      obj.remoteaddr = obj.remoteaddr.substring(7);
    }
    ...
    obj.close = function (arg) {
      if (arg == 1 || arg == null) {
        try {
          obj.ws.close();
          obj.parent.parent.debug('peer', 'InPeer: Soft disconnect ' + obj.peerServerId + ' (' + obj.remoteaddr + ')');
        } catch (e) {
          console.log(e);
        }
      ...
      if (arg == 2) {
        try {
          ...
          obj.parent.parent.debug('peer', 'InPeer: Hard disconnect ' + obj.peerServerId + ' (' + obj.remoteaddr + ')');
        } catch (e) {
          console.log(e);
        }
    ...
    ws.on('message', function (msg) {
      ...
      if (msg.charCodeAt(0) == 123) {
        ...
        if (cmd == 1) {
          ...
          if (obj.tls == true && obj.webCertificateHash != msg.substring(2, 50)) {
            obj.close();
            return;
          }
          ...
          if (obj.unauthsign != null) {
            if (processPeerSignature(obj.unauthsign) == false) {
              obj.close();
              return;
            } else {
          ...
          if (msg.length < 4 || (obj.receivedCommands & 2) != 0) {
            obj.parent.parent.debug('peer', 'InPeer: Invalid command 2.');
            return;
          }
          ...
          obj.unauth = {};
          try {
            obj.unauth.nodeid = Buffer.from(
              obj.forge.pki.getPublicKeyFingerprint(
                obj.forge.pki.certificateFromAsn1(obj.forge.asn1.fromDer(msg.substring(4, 4 + certlen))).publicKey,
                { encoding: 'binary', md: obj.forge.md.sha384.create() }
              ),
              'binary'
            )
              .toString('base64')
          ...
          if (obj.peernonce == null) {
            ...
            if (processPeerSignature(msg.substring(4 + certlen)) == false) {
              obj.parent.parent.debug('peer', 'InPeer: Invalid signature.');
              obj.close();
              return;
            }
          }
          ...
          if (msg.length < 56 || (obj.receivedCommands & 4) != 0) {
            obj.parent.parent.debug('peer', 'InPeer: Invalid command 3.');
            return;
          }
    ...
    ws.on('error', function (err) {
      obj.parent.parent.debug('peer', 'InPeer: Connection Error: ' + err);
    });

    // If the peer server web socket is closed, clean up.
    ws.on('close', function (req) {
      obj.parent.parent.debug('peer', 'InPeer disconnect ' + obj.nodeid + ' (' + obj.remoteaddr + ')');
      obj.close(0);
    });
    ...
    function processPeerSignature(msg) {
      ...
      if (verify.verify(obj.unauth.nodeCertPem, Buffer.from(msg, 'binary')) !== true) {
        console.log('Peer sign fail 1');
        return false;
      }
      if (obj.unauth.nodeid !== obj.agentCertificateHashBase64) {
        console.log('Peer sign fail 2');
        return false;
      }
        ...
        if (command.action == 'info') {
          if (obj.authenticated != 3) {
            ...
            if (command.serverid != null && command.dbid != null) {
              if (command.serverid == obj.parent.serverid) {
                console.log(
                  'ERROR: Same server ID, trying to peer with self. (' + obj.remoteaddr + ', ' + command.serverid + ').'
                );
                return;
              }
              if (command.dbid != obj.parent.parent.db.identifier) {
              ...
              if (obj.parent.peerConfig.servers[command.serverid] == null) {
                console.log('ERROR: Unknown peer serverid: ' + command.serverid + ' (' + obj.remoteaddr + ').');
                return;
              }
              obj.peerServerId = command.serverid;
              obj.peerServerKey = Buffer.from(command.key, 'hex');
  ...
  if (obj.peerConfig == null) {
    return null;
  }
  obj.serverid = obj.parent.config.peers.serverid;
  if (obj.serverid == null) {
    obj.serverid = require('os').hostname().toLowerCase();
  } else {
    obj.serverid = obj.serverid.toLowerCase();
  }
  if (args.serverid != null) {
    obj.serverid = args.serverid.toLowerCase();
  }
  if (obj.parent.config.peers.servers[obj.serverid] == null) {
    console.log('Error: Unable to peer with other servers, "' + obj.serverid + '" not present in peer servers list.');
    return null;
  }
  ...
  obj.DispatchEvent = function (ids, source, event) {
    for (var serverid in obj.peerServers) {
      obj.peerServers[serverid].send({ action: 'bus', ids: ids, event: event });
    }
  };
  ...
  obj.DispatchMessage = function (msg) {
    for (var serverid in obj.peerServers) {
      obj.peerServers[serverid].send(msg);
    }
  };
  ...
  obj.ConnectToPeers = function () {
    for (var serverId in obj.peerConfig.servers) {
      ...
      if (
        ...
        obj.peerConfig.servers[serverId].url != null &&
        obj.outPeerServers[serverId] == null
      ) {
  ...
  obj.SetupPeerServer = function (server, peerServerId) {
    ...
    server.send({
      action: 'connectivityTable',
      connectivityTable: obj.parent.peerConnectivityByNode[obj.parent.serverId]
    });
  ...
  obj.ClearPeerServer = function (server, peerServerId) {
    ...
    for (var i in obj.parent.webserver.wsPeerSessions[peerServerId]) {
      delete obj.parent.webserver.wsPeerSessions2[obj.parent.webserver.wsPeerSessions[peerServerId][i]];
    }
  ...
  obj.ProcessPeerServerMessage = function (server, peerServerId, msg) {
    ...
    switch (msg.action) {
      case 'mqtt': {
        if (obj.parent.mqttbroker != null && msg.nodeid != null) {
          obj.parent.mqttbroker.publishNoPeers(msg.nodeid, msg.topic, msg.message);
        } // Dispatch in the MQTT broker
        break;
      }
      case 'bus': {
      ...
      case 'sessionsTable': {
        ...
        for (i in msg.sessionsTable) {
          ...
          if (userToSession[userid] == null) {
            userToSession[userid] = [sessionid];
          } else {
            userToSession[userid].push(sessionid);
          } // UserId -> [ SessionId ]
        }
      ...
      case 'sessionStart': {
        ...
        if (obj.parent.webserver.wsPeerSessions3[peerServerId] == null) {
          obj.parent.webserver.wsPeerSessions3[peerServerId] = {};
        }
        if (obj.parent.webserver.wsPeerSessions3[peerServerId][userid] == null) {
          obj.parent.webserver.wsPeerSessions3[peerServerId][userid] = [msg.sessionid];
        } else {
          obj.parent.webserver.wsPeerSessions3[peerServerId][userid].push(msg.sessionid);
        }
      ...
      case 'sessionEnd': {
        ...
        if (i >= 0) {
          obj.parent.webserver.wsPeerSessions[peerServerId].splice(i, 1);
        }
        ...
        if (obj.parent.webserver.wsPeerSessions3[peerServerId][userid] != null) {
          ...
          if (i >= 0) {
            ...
            if (obj.parent.webserver.wsPeerSessions3[peerServerId][userid].length == 0) {
              delete obj.parent.webserver.wsPeerSessions3[peerServerId][userid];
            }
          }
        }
        obj.parent.webserver.recountSessions(msg.sessionid); // Recount a specific user
      ...
      case 'relay': {
        ...
        if (rsession != null) {
          ...
          if (peerServerId > obj.parent.serverId) {
            ...
            userid = null;
            if (rsession.peer1.user != null) {
              userid = rsession.peer1.user._id;
            }
          ...
          for (i in obj.parent.webserver.wsPeerRelays) {
            if (obj.parent.webserver.wsPeerRelays[i].time < oneMinuteAgo) {
              delete obj.parent.webserver.wsPeerRelays[i];
            }
          }
        }
        break;
      ...
      case 'msg': {
        if (msg.sessionid != null) {
          ...
          if (msg.fromNodeid != null) {
            msg.nodeid = msg.fromNodeid;
            delete msg.fromNodeid;
          }
          ...
          if (msg.fromSessionid != null) {
            msg.sessionid = msg.fromSessionid;
            delete msg.fromSessionid;
          }
          var agent = obj.parent.webserver.wsagents[msg.nodeid];
          if (agent != null) {
            delete msg.nodeid;
            agent.send(JSON.stringify(msg));
          } // Remove the nodeid since it's implyed and send the message to the agent
          ...
          for (userid in obj.parent.webserver.wssessions) {
            // Find all connected users for this mesh and send the message
            if (parent.webserver.GetMeshRights(userid, msg.meshid) != 0) {
              // TODO: Look at what rights are needed for message routing
              var sessions = obj.parent.webserver.wssessions[userid];
              // Send the message to all users on this server
              for (i in sessions) {
                sessions[i].send(cmdstr);
              }
            }
          }
        }
      ...
      case 'newIntelAmtPolicy': {
        ...
        for (var nodeid in obj.parent.webserver.wsagents) {
          ...
          if (agent.dbMeshKey == msg.meshid) {
            agent.sendUpdatedIntelAmtPolicy(msg.amtpolicy);
          }
        }
        break;
      }
      ...
      case 'agentCommand': {
        if (msg.nodeid != null) {
          ...
          var agent = obj.parent.webserver.wsagents[msg.nodeid];
          if (agent != null) {
            agent.send(JSON.stringify(msg.command));
          }
        } else if (msg.meshid != null) {
  ...
  obj.createPeerRelay = function (ws, req, serverid, user) {
    var server = obj.peerServers[serverid];
    if (server == null || server.peerServerKey == null) {
      return null;
    }
    ...
    if (typeof user == 'string') {
      user = { _id: user, domain: user.split('/')[1] };
    }

    // Build the connection URL
    var path = req.path;
    if (path[0] == '/') path = path.substring(1);
    if (path.substring(path.length - 11) == '/.websocket') {
      path = path.substring(0, path.length - 11);
    }
    var queryStr = '';
    for (var i in req.query) {
      if (i.toLowerCase() != 'auth') {
        queryStr += (queryStr == '' ? '?' : '&') + i + '=' + req.query[i];
      }
    }
    if (user != null) {
      queryStr +=
        (queryStr == '' ? '?' : '&') +
        'auth=' +
        obj.parent.encodeCookie({ userid: user._id, domainid: user.domain, ps: 1 }, cookieKey);
    }
  ...
  obj.createPeerRelayEx = function (ws, url, serverid) {
    ...
    peerTunnel.connect = function () {
      ...
      peerTunnel.ws2 = new WebSocket(peerTunnel.url, {
        rejectUnauthorized: false,
        servername: this.parent.parent.certificates.CommonName,
        cert: this.parent.parent.certificates.agent.cert,
        key: this.parent.parent.certificates.agent.key
      });

      // Register the connection failed event
      peerTunnel.ws2.on('error', function (error) {
        peerTunnel.parent.parent.debug('peer', 'FTunnel ' + obj.serverid + ': Connection error');
        peerTunnel.close();
      });

      // If the peer server web socket is closed, clean up.
      peerTunnel.ws2.on('close', function (req) {
        peerTunnel.parent.parent.debug('peer', 'FTunnel disconnect ' + peerTunnel.serverid);
        peerTunnel.close();
      });

      // If a message is received from the peer, Peer ---> Browser (TODO: Pipe this?)
      peerTunnel.ws2.on('message', function (msg, isBinary) {
        try {
          peerTunnel.ws2._socket.pause();
          peerTunnel.ws1.send(isBinary ? msg : msg.toString('binary'), function () {
            peerTunnel.ws2._socket.resume();
          });
        } catch (e) {}
      });
      ...
      peerTunnel.ws2.on('open', function () {
        ...
        if (peerTunnel.ws2._socket.getPeerCertificate != null) {
          // Get the peer server's certificate and compute the server public key hash
          var serverCert = obj.forge.pki.certificateFromAsn1(
            obj.forge.asn1.fromDer(peerTunnel.ws2._socket.getPeerCertificate().raw.toString('binary'))
          );
          var serverCertHashHex = Buffer.from(
            obj.forge.pki.getPublicKeyFingerprint(serverCert.publicKey, {
              encoding: 'binary',
              md: obj.forge.md.sha384.create()
            }),
          ...
          if (obj.peerServers[serverid] == null || obj.peerServers[serverid].serverCertHash != serverCertHashHex) {
            console.log(
              'ERROR: Outer certificate hash mismatch (1). (' + peerTunnel.url + ', ' + peerTunnel.serverid + ').'
            );
            peerTunnel.close();
            return;
          }
      ...
      peerTunnel.ws1.on('message', function (msg) {
        try {
          peerTunnel.ws1._socket.pause();
          peerTunnel.ws2.send(msg, function () {
            peerTunnel.ws1._socket.resume();
          });
        } catch (e) {}
      });
      ...
      // If error, do nothing
      peerTunnel.ws1.on('error', function (err) {
        peerTunnel.close();
      });

      // If the web socket is closed, close the associated TCP connection.
      peerTunnel.ws1.on('close', function (req) {
        peerTunnel.parent.parent.debug('peer', 'FTunnel disconnect ' + peerTunnel.serverid);
        peerTunnel.close();
      });
    };
    ...
    peerTunnel.close = function (arg) {
      if (arg == 2) {
        // Hard close, close the TCP socket
        if (peerTunnel.ws1 != null) {
          try {
            peerTunnel.ws1._socket._parent.end();
            peerTunnel.parent.parent.debug('peer', 'FTunnel1: Hard disconnect');
          } catch (e) {
            console.log(e);
          }
          delete peerTunnel.ws1;
        }
        if (peerTunnel.ws2 != null) {
          try {
            peerTunnel.ws2._socket._parent.end();
            peerTunnel.parent.parent.debug('peer', 'FTunnel2: Hard disconnect');
          } catch (e) {
            console.log(e);
          }
          delete peerTunnel.ws2;
        }
      } else {
        // Soft close, close the websocket
        if (peerTunnel.ws1 != null) {
          try {
            peerTunnel.ws1.close();
            peerTunnel.parent.parent.debug('peer', 'FTunnel1: Soft disconnect ');
          } catch (e) {
            console.log(e);
          }
          delete peerTunnel.ws1;
        ...
        if (peerTunnel.ws2 != null) {
          try {
            peerTunnel.ws2.close();
            peerTunnel.parent.parent.debug('peer', 'FTunnel2: Soft disconnect ');
          } catch (e) {
            console.log(e);
          }
          delete peerTunnel.ws2;
  ...
  setTimeout(function () {
    obj.ConnectToPeers();
  }, 1000); // Delay this a little to make sure we are ready on our side.
  return obj;
};
