...
module.exports.CreateMpsServer = function (parent, db, args, certificates) {
  ...
  if (args.lanonly != true && args.mpsport !== 0) {
    if (obj.args.mpstlsoffload) {
      ...
      if (obj.args.mpshighsecurity) {
        ...
        obj.server = tls.createServer(
          {
            key: certificates.mps.key,
            cert: certificates.mps.cert,
            requestCert: true,
        ...
        obj.server = tls.createServer(
          {
            ...
            requestCert: true,
            rejectUnauthorized: false,
            ciphers: 'HIGH:!aNULL:!eNULL:!EXPORT:!DES:!RC4:!MD5:!PSK:!SRP:!CAMELLIA:@SECLEVEL=0',
      ...
      obj.server.on('newSession', function (id, data, cb) {
      ...
      obj.server.on('resumeSession', function (id, cb) {
        cb(null, tlsSessionStore[id.toString('hex')] || null);
      });
    }

    obj.server
      .listen(args.mpsport, args.mpsportbind, function () {
        console.log(
          'MeshCentral Intel(R) AMT server running on ' +
            certificates.AmtMpsName +
            ':' +
            args.mpsport +
            (args.mpsaliasport != null ? ', alias port ' + args.mpsaliasport : '') +
            '.'
        );
        obj.parent.authLog(
          'mps',
          'Server listening on ' +
            (args.mpsportbind != null ? args.mpsportbind : '0.0.0.0') +
            ' port ' +
            args.mpsport +
            '.'
        );
      ...
      .on('error', function (err) {
        console.error(
          'ERROR: MeshCentral Intel(R) AMT server port ' +
            args.mpsport +
            ' is not available. Check if the MeshCentral is already running.'
        );
        if (args.exactports) {
          process.exit();
        }
    ...
    obj.server.on('tlsClientError', function (err, tlssocket) {
      if (args.mpsdebug) {
        var remoteAddress = tlssocket.remoteAddress;
        if (tlssocket.remoteFamily == 'IPv6') {
          remoteAddress = '[' + remoteAddress + ']';
        }
        console.log('MPS:Invalid TLS connection from ' + remoteAddress + ':' + tlssocket.remotePort + '.');
      }
    });
  }
  ...
  if (args.mpsaliasport != null) {
    obj.parent.updateServerState('mps-alias-port', args.mpsaliasport);
  }
  ...
  function addCiraConnection(socket) {
    ...
    var sameType = false,
      connections = obj.ciraConnections[socket.tag.nodeid];
    if (connections != null) {
      for (var i in connections) {
        var conn = connections[i];
        if (conn.tag.connType === socket.tag.connType) {
          sameType = true;
        }
      }
    }
    ...
    if (connections == null) {
      ...
      obj.ciraConnections[socket.tag.nodeid].push(socket);
    }
    ...
    var f = function setConnFunc() {
      ...
      if (ciraArray != null && ciraArray.indexOf(setConnFunc.socket) >= 0) {
        ...
        if (setConnFunc.socket.tag.connType == 0) {
          ...
          obj.parent.SetConnectivityState(
            setConnFunc.socket.tag.meshid,
            setConnFunc.socket.tag.nodeid,
            setConnFunc.socket.tag.connectTime,
            2,
            7,
            null,
            { name: socket.tag.name }
          ); // 7 = Present
        ...
        if (obj.parent.amtManager != null) {
          obj.parent.amtManager.startAmtManagement(
            setConnFunc.socket.tag.nodeid,
            setConnFunc.socket.tag.connType,
            setConnFunc.socket
          );
        }
      }
  ...
  function removeCiraConnection(socket) {
    ...
    if (obj.parent.amtManager != null) {
      obj.parent.amtManager.stopAmtManagement(socket.tag.nodeid, socket.tag.connType, socket);
    }
    ...
    if (ciraArray.length == 0) {
      delete obj.ciraConnections[socket.tag.nodeid];
    } else {
      obj.ciraConnections[socket.tag.nodeid] = ciraArray;
    }
  ...
  obj.getStats = function () {
    ...
    for (var i in obj.ciraConnections) {
      ciraConnectionCount += obj.ciraConnections[i].length;
    }
    return {
  ...
  obj.onWebSocketConnection = function (socket, req) {
    ...
    socket.SetupChannel = function SetupChannel(targetport) {
      return SetupChannel.parent.SetupChannel(SetupChannel.conn, targetport);
    };
    socket.SetupChannel.parent = obj;
    socket.SetupChannel.conn = socket;
    socket.websocket = 1;
    socket.ControlMsg = function ControlMsg(message) {
      return ControlMsg.parent.SendJsonControl(ControlMsg.conn, message);
    };
    ...
    socket.on('message', function (data) {
      ...
      try {
        ...
        var l = 0;
        do {
          l = ProcessCommand(this);
          if (l > 0) {
            this.tag.accumulator = this.tag.accumulator.substring(l);
          }
        } while (l > 0);
        if (l < 0) {
          this.terminate();
  ...
  function onConnection(socket) {
    ...
    socket.on('timeout', () => {
      ciraTimeoutCount++;
      parent.debug('mps', 'CIRA timeout, disconnecting.');
      obj.close(socket);
    });
    ...
    socket.addListener('data', function (data) {
      ...
      if (socket.tag.first == true) {
        ...
        if (socket.tag.accumulator.substring(0, 4) == 'GET ' || socket.tag.accumulator.substring(0, 5) == 'HEAD ') {
          if (args.mpsdebug) {
            console.log('MPS Connection, HTTP request detected: ' + socket.remoteAddress);
          }
        ...
        if (parent.mqttbroker != null) {
          ...
          if (
            chunk.readUInt8(0) == 16 &&
            (chunk.slice(4, 8).toString() === 'MQTT' ||
              chunk.slice(5, 9).toString() === 'MQTT' ||
              chunk.slice(6, 10).toString() === 'MQTT' ||
              chunk.slice(7, 11).toString() === 'MQTT')
          ) {
            ...
            socket.on('data', function (b) {
              socket.serialtunnel.updateBuffer(Buffer.from(b, 'binary'));
            });
            socket.serialtunnel.forwardwrite = function (b) {
              socket.write(b, 'binary');
            };
            socket.on('close', function () {
              socket.serialtunnel.emit('end');
            });
        ...
        if (
          socket.tag.clientCert &&
          socket.tag.clientCert.subject &&
          socket.tag.clientCert.subject.O &&
          socket.tag.clientCert.subject.O.length == 64
        ) {
          ...
          if (domain == null) {
            console.log('CIRA connection for invalid domain. meshid: ' + meshid);
            obj.close(socket);
            return;
          }
          ...
          socket.tag.nodeid =
            ...
            require('crypto')
              .createHash('sha384')
              .update(common.hex2rstr(socket.tag.clientCert.modulus, 'binary'))
              .digest('base64')
              .replace(/\+/g, '@')
              .replace(/\//g, '$');
          ...
          obj.db.Get(socket.tag.nodeid, function (err, nodes) {
            if (err) {
              parent.debug('mps', 'CIRA db.Get error for ' + socket.tag.nodeid + ': ' + err);
              obj.close(socket);
              return;
            }
            if (nodes == null || nodes.length !== 1) {
                ...
                if (domain.limits && typeof domain.limits.maxdevices == 'number') {
                  db.isMaxType(domain.limits.maxdevices, 'node', domain.id, function (ismax, count) {
                    if (ismax == true) {
                      ...
                      console.log(
                        'Too many devices on this domain to accept the CIRA connection. meshid: ' + socket.tag.meshid
                      );
                      obj.close(socket);
                    } else {
                      ...
                      require('dns').reverse(socket.remoteAddr, function (err, hostnames) {
                        ...
                        var device = {
                          type: 'node',
                          mtype: 1,
                          _id: socket.tag.nodeid,
                          meshid: socket.tag.meshid,
                          name: socket.tag.name,
                          icon: socket.tag.meiState.isBatteryPowered ? 2 : 1,
                          host: hostname,
                          domain: domainid,
                          intelamt: {
                            user: typeof socket.tag.meiState.amtuser == 'string' ? socket.tag.meiState.amtuser : '',
                            pass: typeof socket.tag.meiState.amtpass == 'string' ? socket.tag.meiState.amtpass : '',
                            tls: 0,
                            state: 2
                          }
                        };
                        if (socket.tag.meiState != null) {
                          if (
                            typeof socket.tag.meiState.desc == 'string' &&
                            socket.tag.meiState.desc.length > 0 &&
                            socket.tag.meiState.desc.length < 1024
                          ) {
                            device.desc = socket.tag.meiState.desc;
                          }
                          if (
                            typeof socket.tag.meiState.Versions == 'object' &&
                            typeof socket.tag.meiState.Versions.Sku == 'string'
                          ) {
                            device.intelamt.sku = parseInt(socket.tag.meiState.Versions.Sku);
                          }
                        }
                        obj.db.Set(device);
                        ...
                        obj.parent.DispatchEvent(['*', socket.tag.meshid], obj, {
                          etype: 'node',
                          action: 'addnode',
              ...
              if (node.intelamt != null && node.intelamt.state == 2) {
                socket.tag.host = node.intelamt.host;
              }
            }
  ...
  function ProcessCommand(socket) {
    ...
    switch (cmd) {
      ...
      case APFProtocol.USERAUTH_REQUEST: {
        ...
        if (username == '**MeshAgentApfTunnel**' && password != null) {
          ...
          if (obj.parent.webserver.meshes) {
            for (var i in obj.parent.webserver.meshes) {
              if (obj.parent.webserver.meshes[i]._id.replace(/\@/g, 'X').replace(/\$/g, 'X').indexOf(meshIdStart) > 0) {
                initialMesh = obj.parent.webserver.meshes[i];
                break;
              }
            }
          }
          if (initialMesh == null) {
            ...
            socket.ControlMsg({
              action: 'console',
              msg: 'Device group not found (2): ' + meshIdStart + ', u: ' + username + ', p: ' + password
            });
        ...
        if (initialMesh.mtype == 1) {
          ...
          const nodeid = Buffer.from(systemid + systemid + systemid, 'hex')
            .toString('base64')
            .replace(/\+/g, '@')
            .replace(/\//g, '$');
              ...
              if (domain.limits && typeof domain.limits.maxdevices == 'number') {
                ...
                const reverseDnsLookupHandler = function (err, hostnames) {
                  ...
                  var device = {
                    ...
                    meshid: socket.tag.meshid,
                    name: socket.tag.name,
                    icon: socket.tag.meiState && socket.tag.meiState.isBatteryPowered ? 2 : 1,
                    host: hostname,
                    domain: initialMesh.domain,
                    intelamt: {
                      user:
                        socket.tag.meiState && typeof socket.tag.meiState.amtuser == 'string'
                          ? socket.tag.meiState.amtuser
                          : '',
                      pass:
                        socket.tag.meiState && typeof socket.tag.meiState.amtpass == 'string'
                          ? socket.tag.meiState.amtpass
                          : '',
                      tls: 0,
                      state: 2
                    }
                ...
                try {
                  require('dns').reverse(socket.remoteAddr, reverseDnsLookupHandler);
                } catch (ex) {
                  reverseDnsLookupHandler(ex, null);
                }
              }
            } else {
              // Node is already present
        ...
        } else if (initialMesh.mtype == 2) {
          // If this is a agent mesh, search the mesh for this device UUID
          // Intel AMT GUID (socket.tag.SystemId) will be used to search the node
          obj.db.getAmtUuidMeshNode(initialMesh.domain, initialMesh.mtype, socket.tag.SystemId, function (err, nodes) {
            // TODO: Need to optimize this request with indexes
            if (err) {
              parent.debug('mps', 'CIRA db.Get error for ' + socket.tag.SystemId + ': ' + err);
              obj.close(socket);
              return;
            }
            if (nodes == null || nodes.length === 0 || obj.parent.webserver.meshes == null) {
              ...
              unknownNodeCount++;
              console.log(
                'CIRA connection for unknown node. groupid: ' + initialMesh._id + ', uuid: ' + socket.tag.SystemId
              );
              //obj.close(socket);
              //return;
              ...
              if (domain.limits && typeof domain.limits.maxdevices == 'number') {
                ...
                require('dns').reverse(socket.remoteAddr, function (err, hostnames) {
                  ...
                  var device = {
                    ...
                    host: hostname,
                    domain: initialMesh.domain,
                    agent: { ver: 0, id: 0, caps: 0 },
                    intelamt: {
                      uuid: socket.tag.SystemId,
                      user:
                        socket.tag.meiState && typeof socket.tag.meiState.amtuser == 'string'
                          ? socket.tag.meiState.amtuser
                          : '',
            ...
            for (var i in nodes) {
              if (initialMesh.domain == nodes[i].domain) {
                ...
                if (nodemesh != null && nodemesh.mtype == 2) {
                  node = nodes[i];
                }
              }
            }
            ...
            if (node == null) {
              ...
              unknownNodeCount++;
              console.log(
                'CIRA connection for unknown node. candidate(s): ' +
                  nodes.length +
                  ', groupid: ' +
                  initialMesh._id +
                  ', uuid: ' +
      ...
      case APFProtocol.SERVICE_REQUEST: {
        ...
        if (xserviceName == 'pfwd@amt.intel.com') {
          SendServiceAccept(socket, 'pfwd@amt.intel.com');
        }
        if (xserviceName == 'auth@amt.intel.com') {
          SendServiceAccept(socket, 'auth@amt.intel.com');
        }
      ...
      case APFProtocol.GLOBAL_REQUEST: {
        ...
        if (request == 'tcpip-forward') {
          ...
          if (port === 5900) {
            SendKeepaliveOptionsRequest(socket, KEEPALIVE_INTERVAL, 0);
          }
          ...
          if (cirachannel.sendBuffer != null) {
            if (cirachannel.sendBuffer.length <= cirachannel.sendcredits) {
              ...
              SendChannelData(
                ...
                cirachannel.sendBuffer.slice(0, cirachannel.sendcredits)
              );
      ...
      case APFProtocol.CHANNEL_OPEN_FAILURE: {
        ...
        var cirachannel = socket.tag.channels[RecipientChannel];
        if (cirachannel == null) {
          console.log('MPS Error in CHANNEL_OPEN_FAILURE: Unable to find channelid ' + RecipientChannel);
          return 17;
        }
      ...
      case APFProtocol.CHANNEL_CLOSE: {
        ...
        var cirachannel = socket.tag.channels[RecipientChannel];
        if (cirachannel == null) {
          console.log('MPS Error in CHANNEL_CLOSE: Unable to find channelid ' + RecipientChannel);
          return 5;
        }
        socket.tag.activetunnels--;
        if (cirachannel.state > 0) {
      ...
      case APFProtocol.CHANNEL_WINDOW_ADJUST: {
        ...
        var cirachannel = socket.tag.channels[RecipientChannel];
        if (cirachannel == null) {
          console.log('MPS Error in CHANNEL_WINDOW_ADJUST: Unable to find channelid ' + RecipientChannel);
          return 9;
        }
      ...
      case APFProtocol.CHANNEL_DATA: {
        ...
        var cirachannel = socket.tag.channels[RecipientChannel];
        if (cirachannel == null) {
          console.log('MPS Error in CHANNEL_DATA: Unable to find channelid ' + RecipientChannel);
          return 9 + LengthOfData;
        }
        if (cirachannel.state > 0) {
          cirachannel.amtpendingcredits += LengthOfData;
          if (cirachannel.onData) {
            cirachannel.onData(cirachannel, Buffer.from(data.substring(9, 9 + LengthOfData), 'binary'));
          }
      ...
      case APFProtocol.JSON_CONTROL: {
        // This is a Mesh specific command that sends JSON to and from the MPS server.
        if (len < 5) return 0;
        var jsondatalen = common.ReadInt(data, 1);
        if (jsondatalen > 1048576) return -1;
        if (len < 5 + jsondatalen) return 0;
        var jsondata = null,
          jsondatastr = data.substring(5, 5 + jsondatalen);
        try {
        ...
        switch (jsondata.action) {
          ...
          case 'meiState':
            ...
            if ((socket.tag.name == '' || socket.tag.name == null) && typeof jsondata.value.OsHostname == 'string') {
              socket.tag.name = jsondata.value.OsHostname;
            }
            if (obj.parent.amtManager != null) {
              obj.parent.amtManager.mpsControlMessage(socket.tag.nodeid, socket, socket.tag.connType, jsondata);
            }
            break;
          case 'deactivate':
  ...
  obj.SendJsonControl = function (socket, data) {
    ...
    if (typeof data == 'object') {
      parent.debug('mpscmd', '<-- JSON_CONTROL', data.action);
  ...
  function WriteBuffer(socket, data) {
    try {
      if (args.mpsdebug) {
        console.log('MPS --> (' + buf.length + '):' + data.toString('hex'));
      } // Print out sent bytes
      if (socket.websocket == 1) {
        socket.send(data);
      } else {
  ...
  obj.GetConnectionToNode = function (nodeid, targetport, oob) {
    ...
    for (var i in connectionArray) {
      ...
      if (selectConn == null || conn.tag.connType < selectConn.tag.connType) {
        selectConn = conn;
      }
    }
    return selectConn;
  ...
  obj.SetupChannel = function (socket, targetport) {
    ...
    cirachannel.write = function (data) {
      ...
      } // Make sure we always handle buffers when sending data.
      if (cirachannel.state == 1 || cirachannel.sendcredits == 0 || cirachannel.sendBuffer != null) {
        ...
        if (cirachannel.sendBuffer == null) {
          ...
          cirachannel.sendBuffer = Buffer.concat([cirachannel.sendBuffer, data]);
        }
        return true;
      }
      // Compute how much data we can send
  ...
  obj.changeDeviceMesh = function (nodeid, newMeshId) {
    ...
    for (var i in connectionArray) {
      ...
      if (socket != null && socket.tag != null) {
        socket.tag.meshid = newMeshId;
      }
    }
  };
  ...
  function onHttpData(data) {
    ...
    for (var i = 1; i < headers.length; i++) {
      ...
      if (i > 0) {
        headerObj[headers[i].substring(0, j).toLowerCase()] = headers[i].substring(j + 2);
      }
    }
    var hostHeader = headerObj['host'] != null ? 'Host: ' + headerObj['host'] + '\r\n' : '';
    var directives = headers[0].split(' ');
    if (directives.length != 3 || (directives[0] != 'GET' && directives[0] != 'HEAD')) {
      this.end();
      return;
    }
    ...
    if (directives[1].startsWith('/c/')) {
      var cookie = obj.parent.decodeCookie(
        directives[1].substring(3).split('.')[0],
      ...
      if (cookie != null && cookie.a == 'f' && typeof cookie.f == 'string') {
        ...
        var filestats = null;
        try {
          filestats = obj.fs.statSync(cookie.f);
        } catch (ex) {}
        if (filestats == null || typeof filestats.size != 'number' || filestats.size <= 0) {
          ...
          if (directives[0] == 'GET') {
            obj.fs.createReadStream(cookie.f, { flags: 'r' }).pipe(this);
          }
          delete this.xdata;
          return;
        }
      ...
      if (obj.httpResponses != null) {
        r = obj.httpResponses[directives[1]];
      }
      if (r != null && r.maxtime != null && r.maxtime < Date.now()) {
        r = null;
        delete obj.httpResponses[directives[1]];
      ...
      if (r != null) {
        if (typeof r == 'string') {
          ...
          if (r.shortfile) {
            try {
              responseData = obj.fs.readFileSync(r.shortfile);
            } catch (ex) {
              responseCode = 404;
              responseType = 'text/html';
              responseData = 'File not found';
          ...
          if (r.file) {
            ...
            var filestats = null;
            try {
              filestats = obj.fs.statSync(r.file);
            } catch (ex) {}
            if (filestats == null || typeof filestats.size != 'number' || filestats.size <= 0) {
              responseCode = 404;
              responseType = 'text/html';
              ...
              if (directives[0] == 'GET') {
                ...
                if (typeof r.maxserve == 'number') {
                  r.maxserve--;
                  if (r.maxserve == 0) {
                    delete obj.httpResponses[directives[1]];
                  }
                } // Check if this entry was server the maximum amount of times.
              }
              delete this.xdata;
              return;
            }
  ...
  obj.addHttpFileResponse = function (path, file, maxserve, minutes) {
    var r = { file: file };
    if (typeof maxserve == 'number') {
      r.maxserve = maxserve;
    }
    if (typeof minutes == 'number') {
      r.maxtime = Date.now() + 60000 * minutes;
    }
    obj.httpResponses[path] = r;

    // Clean up any expired files
    const now = Date.now();
    for (var i in obj.httpResponses) {
      if (obj.httpResponses[i].maxtime != null && obj.httpResponses[i].maxtime < now) {
        delete obj.httpResponses[i];
      }
    }
  };
  ...
  obj.dropAllConnections = function () {
    ...
    for (var nodeid in obj.ciraConnections) {
      ...
      } // This will drop all TCP CIRA connections
    }
    return dropCount;
  };
