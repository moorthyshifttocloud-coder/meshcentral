...
function log() {
  ...
  } /*else { if (worker != null) { worker.parentPort.postMessage({ msg: arguments[0] }); } } */
}
function log2() {
  if (directRun) {
    console.log(...arguments);
...
function indexFile(infile) {
  ...
  if (fs.existsSync(infile) == false) {
    log2('Missing file: ' + infile);
    return;
  }
  state.recFileName = infile;
  state.recFileSize = fs.statSync(infile).size;
  if (state.recFileSize < 32) {
    log2('Invalid file size: ' + infile);
    return;
  }
  ...
  readLastBlock(state, function (state, result, time, extras) {
    if (result == false) {
      log2('Invalid file: ' + infile);
      return;
    }
    if (extras != null) {
      log2('File already indexed: ' + infile);
      return;
    }
...
function createIndex(state, ptr) {
  var index = [];
  for (var i in state.screen) {
    if (index.indexOf(state.screen[i]) == -1) {
      index.push(state.screen[i]);
    }
  }
...
function processBlock(state, block, err) {
  ...
  if (state.startTime != null) {
    elapseMilliSeconds = block.time - state.startTime;
  }
  var flagBinary = (block.flags & 1) != 0;
  var flagUser = (block.flags & 2) != 0;

  // Start indexing at the first type 2 block
  if (state.basePtr == null && block.type == 2) {
    state.basePtr = block.ptr;
    state.startTime = block.time;
  }

  // Check if we need to create one or more indexes
  while ((state.lastIndex + state.indexTime) * 1000 < elapseMilliSeconds) {
    createIndex(state, block.ptr);
  }

  if (block.type == 1) {
    // Metadata
    state.metadata = JSON.parse(block.data.toString());
    if (state.metadata.indexInterval != null) {
      log2('This file is already indexed.');
      return;
    }
    if (state.metadata.protocol != 2) {
      log2('Only remote desktop sessions can currently be indexed.');
      return;
    }
    ...
    if (typeof state.recFileProtocol == 'string') {
      state.recFileProtocol = parseInt(state.recFileProtocol);
    }
      ...
      if (block.data.length >= 4) {
        ...
        switch (command) {
          ...
          case 7: // Screen Size, clear the screen state and computer the tile count
            ...
            if (Math.floor(state.swidth) != state.swidth) {
              state.swidth = Math.floor(state.swidth) + 1;
            }
            if (Math.floor(state.sheight) != state.sheight) {
              state.sheight = Math.floor(state.sheight) + 1;
            }
            state.screen = {};
  ...
  recordingEntry(
    ...
    function (state, len) {
      recordingEntry(
        ...
        3,
        0,
        state.recFileSize - 32,
        'MeshCentralMCNDX',
        function (state) {
          func(state);
        },
        state,
        state.recFileSize - 32 + len
      );
    },
    state,
    state.recFileSize - 32
  ...
  try {
    if (typeof data == 'string') {
      ...
      if (typeof position == 'number') {
        fs.write(fd, block, 0, block.length, position, function () {
          func(tag, block.length);
        });
      } else {
        fs.write(fd, block, 0, block.length, function () {
          func(tag, block.length);
        });
      }
    } else {
...
function readNextBlock(state, func) {
  if (state.recFilePtr + 16 > state.recFileSize) {
    func(state, null);
    return;
  }
  var r = {},
  ...
  fs.read(state.recFile, buf, 0, 16, state.recFilePtr, function (err, bytesRead, buf) {
    ...
    try {
      ...
      if (state.recFilePtr + 16 + r.size > state.recFileSize) {
        func(state, null, true);
        return;
      } // Error
      if (r.size == 0) {
...
function InstallModules(modules, func) {
  var missingModules = [];
  if (previouslyInstalledModules == null) {
  ...
  if (modules.length > 0) {
    for (var i in modules) {
      try {
        ...
        if (previouslyInstalledModules[modules[i]] !== true) {
          missingModules.push(modules[i]);
        }
      }
    }
    if (missingModules.length > 0) {
      InstallModule(missingModules.shift(), InstallModules, modules, func);
    } else {
      func();
    }
  }
...
function InstallModule(modulename, func, tag1, tag2) {
  ...
  InstallModuleChildProcess = child_process.exec(
    'npm install --no-optional --save ' + modulename,
    { maxBuffer: 512000, timeout: 300000, cwd: parentpath },
    function (error, stdout, stderr) {
      InstallModuleChildProcess = null;
      if (error != null && error != '') {
        log(
          'ERROR: Unable to install required module "' +
            modulename +
            '". May not have access to npm, or npm may not have suffisent rights to load the new module. Try "npm install ' +
            modulename +
            '" to manualy install this module.\r\n'
        );
        process.exit();
        return;
      }
...
function setup() {
  InstallModules(['image-size'], start);
}
function start() {
