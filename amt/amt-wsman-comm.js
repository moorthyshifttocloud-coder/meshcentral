...
var CreateWsmanComm = function (host, port, user, pass, tls, tlsoptions, mpsConnection) {
  ...
  obj.PerformAjax = function (postdata, callback, tag, pri, url, action) {
    if (
      (obj.ActiveAjaxCount == 0 || (obj.ActiveAjaxCount < obj.MaxActiveAjaxCount && obj.challengeParams != null)) &&
      obj.PendingAjax.length == 0
    ) {
      ...
      if (pri == 1) {
        obj.PendingAjax.unshift([postdata, callback, tag, url, action]);
      } else {
        obj.PendingAjax.push([postdata, callback, tag, url, action]);
      }
    }
  };
  ...
  obj.PerformAjaxEx = function (postdata, callback, tag, url, action) {
    if (obj.FailAllError != 0) {
      obj.gotNextMessagesError({ status: obj.FailAllError }, 'error', null, [postdata, callback, tag, url, action]);
      return;
    }
  ...
  // NODE.js specific private method
  obj.PerformAjaxExNodeJS = function (postdata, callback, tag, url, action) {
    obj.PerformAjaxExNodeJS2(postdata, callback, tag, url, action, 5);
  };
  ...
  obj.PerformAjaxExNodeJS2 = function (postdata, callback, tag, url, action, retry) {
    if (retry <= 0 || obj.FailAllError != 0) {
      ...
      obj.ActiveAjaxCount--;
      if (obj.FailAllError != 999)
        obj.gotNextMessages(null, 'error', { status: obj.FailAllError == 0 ? 408 : obj.FailAllError }, [
          postdata,
          callback,
          tag,
          url,
          action
        ]); // 408 is timeout error
      obj.PerformNextAjax();
      return;
    }
    obj.pendingAjaxCall.push([postdata, callback, tag, url, action, retry]);
    if (obj.socketState == 0) {
      obj.xxConnectHttpSocket();
    } else if (obj.socketState == 2) {
      obj.sendRequest(postdata, url, action);
    }
  };
  ...
  obj.sendRequest = function (postdata, url, action) {
    ...
    if (obj.user == '*' && kerberos != null) {
      ...
      if (obj.kerberosDone == 0) {
        var ticketName =
          'HTTP' + (obj.tls == 1 ? 'S' : '') + '/' + (obj.pass == '' ? obj.host + ':' + obj.port : obj.pass);
        ...
        if (ticketReturn.returnCode == 0 || ticketReturn.returnCode == 0x90312) {
          ...
          if (process.platform.indexOf('win') >= 0) {
            ...
            try {
              require('child_process').exec('%windir%\\system32\\klist purge', function (error, stdout, stderr) {
                if (error) {
                  require('child_process').exec('%windir%\\sysnative\\klist purge', function (error, stdout, stderr) {
                    if (error) {
                      console.error('Unable to purge kerberos tickets');
                    }
                  });
                }
              });
      ...
      var response = hex_md5(
        hex_md5(obj.user + ':' + obj.challengeParams['realm'] + ':' + obj.pass) +
          ':' +
          obj.challengeParams['nonce'] +
          ':' +
          nonceHex(obj.noncecounter) +
          ':' +
          obj.cnonce +
          ':' +
          obj.challengeParams['qop'] +
          ':' +
          hex_md5(action + ':' + url + (obj.challengeParams['qop'] == 'auth-int' ? ':' + hex_md5(postdata) : ''))
      );
      h +=
        'Authorization: ' +
        obj.renderDigest({
          username: obj.user,
          realm: obj.challengeParams['realm'],
          nonce: obj.challengeParams['nonce'],
          uri: url,
          qop: obj.challengeParams['qop'],
          response: response,
          nc: nonceHex(obj.noncecounter++),
          cnonce: obj.cnonce
        }) +
        '\r\n';
    }
  ...
  obj.parseDigest = function (header) {
    return correctedQuoteSplit(header.substring(7)).reduce(function (obj, s) {
      var parts = s.trim().split('=');
      obj[parts[0]] = parts[1].replace(new RegExp('\"', 'g'), '');
      return obj;
    }, {});
  };

  // Split a string on quotes but do not do it when in quotes
  function correctedQuoteSplit(str) {
    return str.split(',').reduce(
      function (a, c) {
        if (a.ic) {
          a.st[a.st.length - 1] += ',' + c;
        } else {
          a.st.push(c);
        }
        if (c.split('"').length % 2 == 0) {
          a.ic = !a.ic;
        }
        return a;
      },
  ...
  obj.renderDigest = function (params) {
    ...
    return (
      'Digest ' +
      paramsnames
        .reduce(function (s1, ii) {
          return s1 + ',' + (ii == 'nc' || ii == 'qop' ? ii + '=' + params[ii] : ii + '="' + params[ii] + '"');
        }, '')
        .substring(1)
    );
  };
  ...
  obj.xxConnectHttpSocket = function () {
    ...
    if (obj.mpsConnection != null) {
      if (obj.xtls != 1) {
        ...
        obj.cirasocket.onData = function (ciraconn, data) {
          if (data.length > 0) {
            try {
              ser.updateBuffer(Buffer.from(data, 'binary'));
            } catch (e) {}
          }
        }; // CIRA ---> TLS
        ...
        obj.cirasocket.onStateChange = function (ciraconn, state) {
          ...
          if (state == 2) {
            ...
            var options = {
              ...
              secureOptions:
                obj.constants.SSL_OP_NO_SSLv2 |
                obj.constants.SSL_OP_NO_SSLv3 |
                obj.constants.SSL_OP_NO_COMPRESSION |
                obj.constants.SSL_OP_CIPHER_SERVER_PREFERENCE |
                obj.constants.SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION,
            ...
            if (obj.xtlsoptions) {
              if (obj.xtlsoptions.ca) {
                options.ca = obj.xtlsoptions.ca;
              }
              if (obj.xtlsoptions.cert) {
                options.cert = obj.xtlsoptions.cert;
              }
              if (obj.xtlsoptions.key) {
                options.key = obj.xtlsoptions.key;
              }
            }
      ...
      if (obj.xtls != 1) {
        ...
        obj.socket.on('timeout', obj.destroy);
        obj.socket.on('error', function (ex) {
          if (ex.message && ex.message.indexOf('sslv3 alert bad record mac') >= 0) {
            obj.xtlsMethod = 1 - obj.xtlsMethod;
          }
        });
      }
  ...
  obj.getPeerCertificate = function () {
    if (obj.xtls == 1) {
      return obj.socket.getPeerCertificate();
    }
    return null;
  };
  obj.getPeerCertificateFingerprint = function () {
    if (obj.xtls == 1) {
      return obj.socket.getPeerCertificate().fingerprint.split(':').join('').toLowerCase();
    }
    return null;
  };
  ...
  function checkCertHash(cert, hash) {
    ...
    // SHA256 compare
    if (hash.length == 64 && obj.crypto.createHash('sha256').update(cert.raw).digest('hex') == hash) {
      return true;
    }

    // SHA384 compare
    if (hash.length == 96 && obj.crypto.createHash('sha384').update(cert.raw).digest('hex') == hash) {
      return true;
    }
    ...
    if (obj.xtls == 1) {
      ...
      if (obj.xtlsoptions != null && obj.xtlsoptions.ca != null) {
        var forgeCert = forge.pki.certificateFromAsn1(
          forge.asn1.fromDer(atob(obj.xtlsCertificate.raw.toString('base64')))
        );
        ...
        for (var i in caStore.certs) {
          if (camatch == 0) {
            var c = caStore.certs[i],
              verified = false;
            try {
              verified = c.verify(forgeCert);
            } catch (e) {}
            if (verified == true) {
              camatch = c;
            }
        ...
        if (obj.xtlsSkipHostCheck == 0 && camatch != 0) {
          ...
          if (amtcertname.toLowerCase() != obj.host.toLowerCase()) {
            camatch = 0;
          }
        }
      }
      ...
      if (obj.xtlsFingerprint == 0 && camatch == 0) {
        obj.xtlsCheck = 3;
      } else {
        obj.xtlsCheck = camatch == 0 ? 2 : 1;
      }
    } else {
      obj.xtlsCheck = 0;
    }
  ...
  obj.xxOnSocketData = function (data) {
    ...
    while (true) {
      ...
      if (obj.socketParseState == 0) {
        ...
        if (obj.amtVersion == null) {
          for (var i in obj.socketHeader) {
            if (obj.socketHeader[i].indexOf('Server: Intel(R) Active Management Technology ') == 0) {
              obj.amtVersion = obj.socketHeader[i].substring(46);
            }
          }
        }
        ...
        for (i in obj.socketHeader) {
          if (i != 0) {
            ...
            obj.socketXHeader[obj.socketHeader[i].substring(0, x2).toLowerCase()] = obj.socketHeader[i].substring(
              x2 + 2
            );
          }
        }
      ...
      if (obj.socketParseState == 1) {
        var csize = -1;
        if (
          obj.socketXHeader['connection'] != undefined &&
          obj.socketXHeader['connection'].toLowerCase() == 'close' &&
          (obj.socketXHeader['transfer-encoding'] == undefined ||
          ...
          try {
            ...
            console.log(ex, typeof data, data.length);
          }
        }
        if (csize == 0) {
  ...
  obj.xxProcessHttpResponse = function (header, data) {
    ...
    if (s == 401 && ++obj.authcounter < 3) {
      ...
      if (obj.challengeParams['qop'] != null) {
        ...
        for (var i in qopList) {
          qopList[i] = qopList[i].trim();
        }
        if (qopList.indexOf('auth-int') >= 0) {
          obj.challengeParams['qop'] = 'auth-int';
        } else {
          obj.challengeParams['qop'] = 'auth';
        }
      }
      if (obj.mpsConnection == null) {
        obj.socket.end();
      ...
      if (r == null || r.length < 1) {
        /*console.log("pendingAjaxCall error, " + r);*/ return;
      } // Get a response without any pending requests.
  ...
  obj.gotNextMessages = function (data, status, request, callArgs) {
    ...
    if (obj.FailAllError != 0) {
      try {
        callArgs[1](null, obj.FailAllError, callArgs[2]);
      } catch (ex) {
        console.error(ex);
      }
      return;
    ...
    if (request.status != 200) {
      try {
        callArgs[1](null, request.status, callArgs[2]);
      } catch (ex) {
        console.error(ex);
      }
      return;
  ...
  function hex_md5(str) {
    return obj.crypto.createHash('md5').update(str).digest('hex');
  }
