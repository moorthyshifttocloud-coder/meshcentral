...
module.exports.CreateAmtIderSession = function (parent, db, ws, req, args, domain, user) {
  ...
  obj.close = function (arg) {
    if (arg == 1 || arg == null) {
      try {
        ws.close();
        parent.parent.debug(1, 'Soft disconnect');
      } catch (e) {
        console.log(e);
      }
    ...
    if (arg == 2) {
      try {
        ...
        parent.parent.debug(1, 'Hard disconnect');
      } catch (e) {
        console.log(e);
      }
  ...
  function processWebSocketData(msg) {
    ...
    switch (command.action) {
        ...
        if (fs.existsSync(userPath)) {
          ...
          readFsRec(userPath, function (err, results) {
            ...
            for (var i in results) {
              if (results[i].toLowerCase().endsWith('.img')) {
                floppyImages.push(results[i].substring(userPath.length + 1));
              } else if (results[i].toLowerCase().endsWith('.iso')) {
                cdromImages.push(results[i].substring(userPath.length + 1));
              }
            }
            var xx,
              sel = true,
              html = "<div style='margin:10px 5px 10px 5px'>Select disk images & start type.</div>";

            // Floppy image selection
            xx = "<select style=width:240px id=xxFloppyImagesSelect><option value=''>None</option>";
            for (var i in floppyImages) {
              xx +=
                "<option value='" +
                encodeURIComponent(floppyImages[i]) +
                "'" +
                (sel ? ' selected' : '') +
                '>' +
                floppyImages[i] +
            ...
            for (var i in cdromImages) {
              xx +=
                "<option value='" +
                encodeURIComponent(cdromImages[i]) +
                "'" +
                (sel ? ' selected' : '') +
                '>' +
                cdromImages[i] +
            ...
            // Start type
            xx =
              '<select style=width:240px id=xxIderStartType><option value=0>On next boot<option value=1>Graceful<option value=2>Immediate</select>';
            html += '<div style=margin:5px>' + addHtmlValue('Session Start', xx) + '</div>';

            var js =
              "function iderServerCall() { return { ider: 1, floppyPath: Q('xxFloppyImagesSelect').value, cdromPath: Q('xxCdromImagesSelect').value, iderStart: Q('xxIderStartType').value }; }";

            try {
              ws.send(JSON.stringify({ action: 'dialog', args: { html: html, js: js }, buttons: 3 }));
            } catch (ex) {}
          });
        } else {
          // No user folder
          try {
            ws.send(
              JSON.stringify({
                action: 'dialog',
                args: {
                  html: 'No disk images found on remote server. Upload .img and .iso files to server "My Files" folder to enable this feature.'
                },
                buttons: 2
              })
            );
      ...
      case 'dialogResponse': {
        if (command.args.ider == 1) {
          // Start IDER Session
          // Decode and validate file paths
          if (command.args.floppyPath != null && typeof command.args.floppyPath != 'string') {
            command.args.floppyPath = null;
          } else {
            command.args.floppyPath = decodeURIComponent(command.args.floppyPath);
          }
          if (command.args.cdromPath != null && typeof command.args.cdromPath != 'string') {
            command.args.cdromPath = null;
          } else {
            command.args.cdromPath = decodeURIComponent(command.args.cdromPath);
          }
          // TODO: Double check that "." or ".." are not used.
          if (command.args.floppyPath != null && command.args.floppyPath.indexOf('..') >= 0) {
            delete command.args.floppyPath;
          }
          if (command.args.cdromPath != null && command.args.cdromPath.indexOf('..') >= 0) {
            delete command.args.cdromPath;
          }
          ...
          if (command.args.floppyPath) {
            floppyPath = parent.parent.path.join(
              parent.parent.filespath,
              domainx,
              'user-' + useridx,
              command.args.floppyPath
            );
          }
          if (command.args.cdromPath) {
            cdromPath = parent.parent.path.join(
              parent.parent.filespath,
              domainx,
              'user-' + useridx,
              command.args.cdromPath
            );
          }

          // Setup the IDER session
          obj.ider = amtMeshRedirModule.CreateAmtRedirect(
            amtMeshIderModule.CreateAmtRemoteIder(parent, parent.parent),
            domain,
            user,
            parent,
            parent.parent
          ...
          if (iderError != 0) {
            try {
              ws.send(JSON.stringify({ action: 'error', code: iderError }));
            } catch (ex) {}
            break;
          }
  ...
  function onIderStateChange(sender, state) {
    try {
      ws.send(JSON.stringify({ action: 'state', state: state }));
    } catch (ex) {}
    switch (state) {
      case 0:
    ...
    try {
      ws.send(
        JSON.stringify({
          ...
          start: start,
          len: len,
          toAmt: obj.ider.m.bytesToAmt,
          fromAmt: obj.ider.m.bytesFromAmt
        })
      );
    } catch (ex) {}
  }
      ...
      list.forEach(function (file) {
        ...
        fs.stat(file, function (err, stat) {
          if (stat && stat.isDirectory()) {
            readFsRec(file, function (err, res) {
  ...
  function addHtmlValue(t, v) {
    return (
      '<div style=height:20px><div style=float:right;width:240px;overflow:hidden><b title="' +
      v +
      '">' +
      v +
      '</b></div><div>' +
