...
module.exports.CreateAmtRedirect = function (module, domain, user, webserver, meshcentral) {
      ...
      if (ciraconn != null) {
        ...
        if (port == 16993 || port == 16995) {
          ...
          tlsock.on('error', function (err) {
            Debug(1, 'CIRA TLS Connection Error ', err);
          });
          tlsock.on('secureConnect', function () {
            Debug(2, 'CIRA Secure TLS Connection');
            ws._socket.resume();
          });
    ...
    if (!obj.xtlsoptions || !obj.xtlsoptions.meshServerConnect) {
      if (obj.xtls == true) {
        ...
        if (
          ...
          obj.xtlsCertificate.fingerprint.split(':').join('').toLowerCase() != obj.xtlsFingerprint
        ) {
          obj.Stop();
          return;
        }
  ...
  obj.xxOnSocketData = function (data) {
    ...
    if (obj.redirTrace) {
      console.log('REDIR-RECV(' + data.length + '): ' + webserver.common.rstr2hex(data));
    }
    //obj.Debug("Recv(" + data.length + "): " + webserver.common.rstr2hex(data));
    if (obj.protocol > 1 && obj.connectstate == 1) {
      ...
      switch (obj.amtaccumulator.charCodeAt(0)) {
        ...
        case 0x14: // AuthenticateSessionReply (20)
          ...
          if (authType == 0) {
            ...
            if (authData.indexOf(4) >= 0) {
              ...
              obj.xxSend(
                String.fromCharCode(0x13, 0x00, 0x00, 0x00, 0x04) +
                  webserver.common.IntToStrX(obj.amtuser.length + obj.authuri.length + 8) +
                  String.fromCharCode(obj.amtuser.length) +
                  obj.amtuser +
            ...
            var digest = hex_md5(
              hex_md5(obj.amtuser + ':' + realm + ':' + obj.amtpass) +
                ':' +
                nonce +
                ':' +
                extra +
            ...
            var buf =
              ...
              webserver.common.IntToStrX(totallen) +
              String.fromCharCode(obj.amtuser.length) +
              obj.amtuser +
            ...
            if (obj.protocol == 3) {
              ...
              obj.connectstate = 1;
              obj.m.Start();
              if (obj.amtaccumulator.length > cmdsize) {
                obj.m.ProcessData(obj.amtaccumulator.substring(cmdsize));
              }
        ...
        default:
          console.log(
            'Unknown Intel AMT command: ' + obj.amtaccumulator.charCodeAt(0) + ' acclen=' + obj.amtaccumulator.length
          );
          obj.Stop();
          return;
      }
  ...
  obj.xxSend = function (x) {
    if (typeof x == 'string') {
      if (obj.redirTrace) {
        console.log('REDIR-SEND(' + x.length + '): ' + Buffer.from(x, 'binary').toString('hex'), typeof x);
      }
      ...
      if (obj.redirTrace) {
        console.log('REDIR-SEND(' + x.length + '): ' + x.toString('hex'), typeof x);
      }
      //obj.Debug("Send(" + x.length + "): " + webserver.common.rstr2hex(x));
      //obj.forwardclient.write(x); // FIXES CIRA
  ...
  obj.Stop = function () {
    ...
    if (obj.forwardclient != null) {
      try {
        obj.forwardclient.destroy();
      } catch (ex) {}
      delete obj.forwardclient;
    }
  ...
  function hex_md5(str) {
    return meshcentral.certificateOperations.forge.md.md5.create().update(str).digest().toHex();
  }
