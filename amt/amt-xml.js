...
function _ParseWsmanRec(node) {
  ...
  for (var i = 0; i < node.childNodes.length; i++) {
    ...
    } else if (r[child.localName] == null) {
  ...
  for (var i in x1) {
    ...
    if (elementName.length > 0 && elementName[0] != '?') {
      if (elementName[0] != '/') {
        var attributes = [],
          localName,
          localname2 = elementName.split(' ')[0].split(':'),
          localName = localname2.length > 1 ? localname2[1] : localname2[0];
        Object.defineProperty(attributes, 'get', {
          value: function () {
            if (arguments.length == 1) {
              for (var a in this) {
                if (this[a].name == arguments[0]) {
                  return this[a];
                }
              }
            } else if (arguments.length == 2) {
              for (var a in this) {
                if (this[a].name == arguments[1] && (arguments[0] == '*' || this[a].namespace == arguments[0])) {
                  return this[a];
                }
              }
            } else {
        ...
        elementStack.push({
          ...
          getChildElementsByTagName: _getChildElementsByTagName,
          getElementsByTagNameNS: _getElementsByTagNameNS,
          getChildElementsByTagNameNS: _getChildElementsByTagNameNS,
            ...
            if (x3[j] == '/') {
              ...
              elementStack.peek().namespace =
                elementStack.peek().name == elementStack.peek().localName
                  ? elementStack.getNamespace('*')
                  : elementStack.getNamespace(
                      elementStack.peek().name.substring(0, elementStack.peek().name.indexOf(':'))
                    );
        ...
        elementStack.peek().namespace =
          elementStack.peek().name == elementStack.peek().localName
            ? elementStack.getNamespace('*')
            : elementStack.getNamespace(elementStack.peek().name.substring(0, elementStack.peek().name.indexOf(':')));
        if (x2[1]) {
