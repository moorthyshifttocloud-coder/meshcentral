...
var CreateAmtSetupBinStack = function () {
  ...
  var AmtSetupBinVarIds = {
    1: {
      ...
      3: [1, 'Manageability Feature Selection', { 0: 'None', 1: 'Intel AMT' }],
      4: [
        1,
        'Firmware Local Update', // 0 = Disabled, 1 = Enabled, 2 = Password Protected
        { 0: 'Disabled', 1: 'Enabled', 2: 'Password Protected' }
      ],
      5: [
        1,
        'Firmware Update Qualifier', // 0 = Always, 1 = Never, 2 = Restricted
    ...
    2: {
      ...
      5: [
        1,
        'Remote Configuration Enabled (RCFG)', // 0 = Off, 1 = On
        { 0: 'Off', 1: 'On' }
      ],
      6: [
        1,
        'Pre-Installed Certificates Enabled', // 0 = Off, 1 = On
        { 0: 'Off', 1: 'On' }
      ],
      7: [
        1,
        'User Defined Certificate Configuration', // 0 = Disabled, 1 = Enabled, 2 = Delete
      ...
      10: [
        1,
        'SOL/IDER Redirection Configuration',
        {
          0: 'None',
      ...
      14: [
        1,
        'Secure Firmware Update (SFWU)', // 0 = Disabled, 1 = Enabled
        { 0: 'Disabled', 1: 'Enabled' }
      ],
      15: [0, 'ITO'],
      16: [
        1,
        'Provisioning Mode (PM)', // 1 = Enterprise, 2 = Small Buisness (SMB)
      ...
      23: [
        1,
        'Shared/Dedicated FQDN', // 0 = Dedicated, 1 = Shared. This option is valid only if configuring the hostname as well
        { 0: 'Dedicated', 1: 'Shared' }
      ],
      24: [
        1,
        'Dynamic DNS Update', // 0 = Disabled, 1 = Enabled
        { 0: 'Disabled', 1: 'Enabled' }
      ],
      25: [
        1,
        'Remote Desktop (KVM) State', // 0 = Disabled, 1 = Enabled
        { 0: 'Disabled', 1: 'Enabled' }
      ],
      26: [
        1,
        'Opt-in User Consent Option', // 0 = Disabled, 1 = KVM, 0xFF = ALL
        { 0: 'Disabled', 1: 'KVM', 255: 'All' }
      ],
      27: [
        1,
        'Opt-in Remote IT Consent Policy', // 0 = Disabled, 1 = Enabled. Allows user consent to be configured remotely.
        { 0: 'Disabled', 1: 'Enabled' }
      ],
      28: [
        1,
        'ME Provision Halt/Active', // 0 = Stop, 1 = Start. The "ME provisioning Halt/Activate" command must appear in the file only after "PKIDNSSuffix", "ConfigServerFQDN" and "Provisioning Server Address"
        { 0: 'Stop', 1: 'Start' }
      ],
      29: [
        1,
        'Manual Setup and Configuration', // 0 = Automated, 1 = Manual
  ...
  o.AmtSetupBinDecode = function (file) {
    ...
    var obj = {},
      UUID = file.substring(0, 16);
    obj.fileType = 0;
    for (var i in AmtSetupBinSetupGuids) {
      if (UUID == AmtSetupBinSetupGuids[i]) obj.fileType = +i + 1;
    }
    ...
    while (ptr + 512 <= file.length) {
      ...
      var ptr2 = 0,
        recbin = file.substring(ptr + 24, ptr + 512);
      if ((r.flags & 2) != 0) {
        recbin = AmtSetupBinDescrambleRecordData(recbin);
      } // De-Scramble the record
      while (1) {
        ...
        if (AmtSetupBinVarIds[v.moduleid][v.varid]) {
          ...
          if (v.type == 1 && v.length == 1)
            v.value = v.value.charCodeAt(0); // 1 byte number
          else if (v.type == 2 && v.length == 2)
            v.value = ReadShortX(v.value, 0); // 2 byte number
          else if (v.type == 3 && v.length == 4)
            v.value = ReadIntX(v.value, 0); // 4 byte number
  ...
  o.AmtSetupBinEncode = function (obj) {
    ...
    var out = [],
      r = AmtSetupBinSetupGuids[obj.fileType - 1],
      reccount = 0;
    ...
    for (var i in modulesInUse) {
      r += ShortToStrX(modulesInUse[i]);
    } // Write each module in use. Needs to be null terminated, but the padding that follows will do that.
    while (r.length < 512) {
      r += '\0';
    } // Pad the header
    out.push(r);
    ...
    for (var i in obj.records) {
      ...
      for (var j in rec.variables) {
        ...
        if (v.type > 0 && v.type < 4) {
          // If this is a numeric value, encode it correctly
          data = parseInt(data);
        ...
        if (v.type == 4) {
          data = hex2rstr(guidToStr(data.split('-').join('')).split('-').join(''));
        }
      ...
      if ((rec.flags & 2) != 0) {
        r2 = r2.substring(0, 24) + AmtSetupBinScrambleRecordData(r2.substring(24));
      } // Scramble the record starting at byte 24, after the header
      out.push(r2);
    }
    return out.join('');
  };
  ...
  function AmtSetupBinScrambleRecordData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      out += String.fromCharCode((data.charCodeAt(i) + 17) & 0xff);
    }
    return out;
  }
  function AmtSetupBinDescrambleRecordData(data) {
    var out = '';
    for (var i = 0; i < data.length; i++) {
      out += String.fromCharCode((data.charCodeAt(i) + 0xef) & 0xff);
    }
    return out;
  }
