...
if (process.argv[2] == '--launch') {
  try {
    require('appmetrics-dash').monitor({ url: '/', title: 'MeshCentral', port: 88, host: '127.0.0.1' });
  } catch (ex) {}
}
...
function CreateMeshCentralServer(config, args) {
  ...
  function getCurrentVersion() {
    try {
      obj.currentVer = JSON.parse(obj.fs.readFileSync(obj.path.join(__dirname, 'package.json'), 'utf8')).version;
    } catch (ex) {}
    return obj.currentVer;
  } // Fetch server version
  getCurrentVersion();

  // Setup the default configuration and files paths
  if (
    __dirname.endsWith('/node_modules/meshcentral') ||
    ...
    if (obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web/views'))) {
      obj.webViewsOverridePath = obj.path.join(__dirname, '../../meshcentral-web/views');
    }
    if (obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web/public'))) {
      obj.webPublicOverridePath = obj.path.join(__dirname, '../../meshcentral-web/public');
    }
    if (obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web/emails'))) {
      obj.webEmailsOverridePath = obj.path.join(__dirname, '../../meshcentral-web/emails');
    }
  } else {
  ...
  const dir = obj.fs.readdir(obj.path.join(obj.filespath, 'tmp'), function (err, files) {
    ...
    for (var i in files) {
      try {
        const filepath = obj.path.join(obj.filespath, 'tmp', files[i]);
        if (obj.fs.statSync(filepath).mtime.getTime() < removeTime) {
          obj.fs.unlink(filepath, function () {});
        }
      } catch (ex) {}
    }
  });
  ...
  if (obj.config.settings && typeof obj.config.settings.datapath == 'string') {
    obj.datapath = obj.config.settings.datapath;
  }
  if (obj.config.settings && typeof obj.config.settings.filespath == 'string') {
    obj.filespath = obj.config.settings.filespath;
  }

  // Create data and files folders if needed
  try {
    obj.fs.mkdirSync(obj.datapath);
  } catch (ex) {}
  try {
    obj.fs.mkdirSync(obj.filespath);
  } catch (ex) {}
  ...
  obj.Start = function () {
    var i;
    try {
      require('./pass').hash('test', function () {}, 0);
    } catch (ex) {
      console.log('Old version of node, must upgrade.');
      return;
    } // TODO: Not sure if this test works or not.
    ...
    for (var arg in obj.args) {
      obj.args[arg.toLocaleLowerCase()] = obj.args[arg];
      if (validArguments.indexOf(arg.toLocaleLowerCase()) == -1) {
        console.log('Invalid argument "' + arg + '", use --help.');
        return;
      }
    }
    ...
    if (obj.args.mongodb == true) {
      console.log(
        'Must specify: --mongodb [connectionstring] \r\nSee https://docs.mongodb.com/manual/reference/connection-string/ for MongoDB connection string.'
      );
      return;
    }
    if (obj.args.mysql == true) {
      console.log('Must specify: --mysql [connectionstring] \r\nExample mysql://user:password@127.0.0.1:3306/database');
      return;
    }
    if (obj.args.mariadb == true) {
      console.log(
        'Must specify: --mariadb [connectionstring] \r\nExample mariadb://user:password@127.0.0.1:3306/database'
      );
      return;
    }
    for (i in obj.config.settings) {
      obj.args[i] = obj.config.settings[i];
    } // Place all settings into arguments, arguments have already been placed into settings so arguments take precedence.
    ...
    if (obj.args.dbfix) {
      var lines = null,
        ...
        fixedDb = [];
      try {
        lines = obj.fs.readFileSync(obj.getConfigFilePath(obj.args.dbfix), { encoding: 'utf8' }).split('\n');
      } catch (ex) {
        console.log('Invalid file: ' + obj.args.dbfix + ': ' + ex);
        process.exit();
      }
      for (var i = 0; i < lines.length; i++) {
        ...
        if (x != null) {
          fixedDb.push(lines[i]);
          for (var j in x) {
            if (fieldNames.indexOf(j) == -1) {
              fieldNames.push(j);
            }
          }
        }
    ...
    if (
      obj.args.cert != null &&
      (typeof obj.args.cert != 'string' ||
        obj.args.cert.indexOf('@') >= 0 ||
        obj.args.cert.indexOf('/') >= 0 ||
        obj.args.cert.indexOf(':') >= 0)
    ) {
      console.log('Invalid certificate name');
      process.exit();
      return;
    }

    // Perform a password hash
    if (obj.args.hashpassword) {
      require('./pass').hash(obj.args.hashpassword, function (err, salt, hash, tag) {
    ...
    // Dump to mesh cores
    if (obj.args.dumpcores) {
      obj.updateMeshCore(function () {
        console.log('Done.');
      }, true);
      return;
    }
    ...
    // Setup Telegram
    if (obj.args.setuptelegram) {
      require('./meshmessaging.js').SetupTelegram(obj);
      return;
    }
    ...
    if (obj.args.translate) {
      ...
      if (NodeJSVer < 8) {
        console.log('Translation feature requires Node v8 or above, current version is ' + process.version + '.');
        process.exit();
        return;
      }

      // Check if translate.json is in the "meshcentral-data" folder, if so use that and translate default pages.
      var translationFile = null,
        customTranslation = false;
      if (require('fs').existsSync(obj.path.join(obj.datapath, 'translate.json'))) {
        translationFile = obj.path.join(obj.datapath, 'translate.json');
        console.log('Using translate.json in meshcentral-data.');
        customTranslation = true;
      }
      if (translationFile == null) {
        if (require('fs').existsSync(obj.path.join(__dirname, 'translate', 'translate.json'))) {
          translationFile = obj.path.join(__dirname, 'translate', 'translate.json');
          console.log('Using default translate.json.');
        }
      }
      if (translationFile == null) {
    ...
    if (obj.args.npmpath == null) {
      try {
        ...
        if (obj.fs.existsSync(nodepath) && obj.fs.existsSync(npmpath)) {
          ...
          if (npmpath.indexOf(' ') >= 0) {
            npmpath = '"' + npmpath + '"';
          }
          if (obj.platform == 'win32') {
            obj.args.npmpath = npmpath;
          } else {
            obj.args.npmpath = nodepath + ' ' + npmpath;
          }
        }
      } catch (ex) {}
    }
    ...
    if (obj.platform == 'linux') {
      if (obj.args.install == true) {
        ...
        if (require('fs').existsSync('/etc/systemd/system')) {
          systemdConf = '/etc/systemd/system/meshcentral.service';
        } else if (require('fs').existsSync('/lib/systemd/system')) {
          systemdConf = '/lib/systemd/system/meshcentral.service';
        } else if (require('fs').existsSync('/usr/lib/systemd/system')) {
          systemdConf = '/usr/lib/systemd/system/meshcentral.service';
        } else {
          console.log('Unable to find systemd configuration folder.');
          process.exit();
          return;
        }
        ...
        require('child_process').exec('which node', {}, function (error, stdout, stderr) {
          if (error != null || stdout.indexOf('\n') == -1) {
            console.log('ERROR: Unable to get node location: ' + error);
            process.exit();
            return;
          }
          ...
          require('child_process').exec(
            'echo \"' + config + '\" | sudo tee ' + systemdConf,
            {},
            function (error, stdout, stderr) {
              if (error != null && error != '') {
                console.log('ERROR: Unable to write config file: ' + error);
                process.exit();
                return;
              }
              ...
              require('child_process').exec(
                ...
                function (error, stdout, stderr) {
                  if (error != null && error != '') {
                    console.log('ERROR: Unable to enable MeshCentral as a service: ' + error);
                    process.exit();
                    return;
                  }
                  if (stdout.length > 0) {
                  ...
                  require('child_process').exec(
                    ...
                    function (error, stdout, stderr) {
                      if (error != null && error != '') {
                        console.log('ERROR: Unable to start MeshCentral as a service: ' + error);
                        process.exit();
                        return;
                      }
                      if (stdout.length > 0) {
        ...
        require('child_process').exec('sudo systemctl stop meshcentral.service', {}, function (err, stdout, stderr) {
          if (err != null && err != '') {
            console.log('ERROR: Unable to stop MeshCentral as a service: ' + err);
          }
          if (stdout.length > 0) {
            console.log(stdout);
          }
          ...
          require('child_process').exec(
            ...
            function (err, stdout, stderr) {
              if (err != null && err != '') {
                console.log('ERROR: Unable to disable MeshCentral as a service: ' + err);
              }
              if (stdout.length > 0) {
                console.log(stdout);
              }
              ...
              require('child_process').exec('sudo rm ' + systemdConf, {}, function (err, stdout, stderr) {
                if (err != null && err != '') {
                  console.log('ERROR: Unable to delete MeshCentral config file: ' + err);
                }
                console.log('Done.');
              });
            }
        ...
        require('child_process').exec('sudo systemctl start meshcentral.service', {}, function (err, stdout, stderr) {
          if (err != null && err != '') {
            console.log('ERROR: Unable to start MeshCentral: ' + err);
            process.exit();
            return;
          }
          console.log('Done.');
        ...
        require('child_process').exec('sudo systemctl stop meshcentral.service', {}, function (err, stdout, stderr) {
          if (err != null && err != '') {
            console.log('ERROR: Unable to stop MeshCentral: ' + err);
            process.exit();
            return;
          }
          console.log('Done.');
        ...
        require('child_process').exec('sudo systemctl restart meshcentral.service', {}, function (err, stdout, stderr) {
          if (err != null && err != '') {
            console.log('ERROR: Unable to restart MeshCentral: ' + err);
            process.exit();
            return;
          }
          console.log('Done.');
      ...
      if (obj.args.install == true) {
        ...
        require('child_process').exec('which node', {}, function (error, stdout, stderr) {
          ...
          const config =
            ...
            obj.parentpath +
            '\\"\ncommand_args=\\"-r -u \\${user} -P \\${pidfile} -S -T meshcentral -m 3 ' +
            nodePath +
            ' ' +
            __dirname +
            ' \\${meshcentral_args}\\"\nload_rc_config \\$name\nrun_rc_command \\"\\$1\\"\n';
          require('child_process').exec(
            'echo \"' + config + '\" | tee ' + systemdConf + ' && chmod +x ' + systemdConf,
            {},
            function (error, stdout, stderr) {
              if (error != null && error != '') {
                console.log('ERROR: Unable to write config file: ' + error);
        ...
        service.on('exit', function (code) {
          if (code !== 0) {
            console.log('ERROR: Unable to stop MeshCentral as a service');
          }
          console.log('Disabling service...');
    ...
    if (obj.platform == 'win32' && obj.service != null) {
      ...
      if (mcpath.endsWith('\\node_modules\\meshcentral') || mcpath.endsWith('/node_modules/meshcentral')) {
        mcpath = require('path').join(mcpath, '..', '..');
      }
      ...
      if (
        obj.args.xinstall == true ||
        obj.args.xuninstall == true ||
        obj.args.start == true ||
        obj.args.stop == true ||
        obj.args.restart == true
      ) {
        var env = [],
          xenv = [
            'user',
        ...
        for (i in xenv) {
          if (obj.args[xenv[i]] != null) {
            env.push({ name: 'mesh' + xenv[i], value: obj.args[xenv[i]] });
          }
        } // Set some args as service environment variables.

        var serviceFilePath = null;
        if (obj.fs.existsSync(obj.path.join(servicepath, 'winservice.js'))) {
          serviceFilePath = obj.path.join(servicepath, 'winservice.js');
        } else if (obj.fs.existsSync(obj.path.join(__dirname, '../WinService/winservice.js'))) {
          serviceFilePath = obj.path.join(__dirname, '../WinService/winservice.js');
        } else if (obj.fs.existsSync(obj.path.join(__dirname, 'winservice.js'))) {
          serviceFilePath = obj.path.join(__dirname, 'winservice.js');
        }
        if (serviceFilePath == null) {
          console.log('Unable to find winservice.js');
          return;
        }

        const svc = new obj.service({
          name: 'MeshCentral',
        ...
        svc.on('install', function () {
          console.log('MeshCentral service installed.');
          svc.start();
        });
        svc.on('uninstall', function () {
          console.log('MeshCentral service uninstalled.');
          process.exit();
        });
        ...
        svc.on('stop', function () {
          console.log('MeshCentral service stopped.');
          if (obj.args.stop) {
            process.exit();
          }
          if (obj.args.restart) {
            console.log('Holding 5 seconds...');
            setTimeout(function () {
              svc.start();
            }, 5000);
          }
        ...
        svc.on('alreadyinstalled', function () {
          console.log('MeshCentral service already installed.');
          process.exit();
        });
        svc.on('invalidinstallation', function () {
          console.log('Invalid MeshCentral service installation.');
          process.exit();
        });

        if (obj.args.xinstall == true) {
          try {
            svc.install();
          } catch (ex) {
            logException(ex);
        ...
        if (obj.args.stop == true || obj.args.restart == true) {
          try {
            svc.stop();
          } catch (ex) {
            logException(ex);
      ...
      if (obj.args.install == true) {
        ...
        if (obj.fs.existsSync(servicepath) == false) {
          try {
            obj.fs.mkdirSync(servicepath);
          } catch (ex) {
            console.log('ERROR: Unable to create WinService folder: ' + ex);
            process.exit();
            return;
          }
        }
        try {
          obj.fs
            .createReadStream(obj.path.join(__dirname, 'winservice.js'))
            .pipe(obj.fs.createWriteStream(obj.path.join(servicepath, 'winservice.js')));
        } catch (ex) {
          console.log('ERROR: Unable to copy winservice.js: ' + ex);
          process.exit();
          return;
        }
        require('child_process').exec(
          'node winservice.js --install',
          { maxBuffer: 512000, timeout: 120000, cwd: servicepath },
          function (error, stdout, stderr) {
            if (error != null && error != '') {
              console.log('ERROR: Unable to install MeshCentral as a service: ' + error);
              process.exit();
              return;
            }
            console.log(stdout);
        ...
        if (obj.fs.existsSync(servicepath) == true) {
          require('child_process').exec(
            ...
            function (error, stdout, stderr) {
              if (error != null && error != '') {
                console.log('ERROR: Unable to uninstall MeshCentral service: ' + error);
                process.exit();
                return;
              }
              console.log(stdout);
              try {
                obj.fs.unlinkSync(obj.path.join(servicepath, 'winservice.js'));
              } catch (ex) {}
              try {
                obj.fs.rmdirSync(servicepath);
              } catch (ex) {}
            }
          );
          ...
          require('child_process').exec(
            'node winservice.js --uninstall',
            { maxBuffer: 512000, timeout: 120000, cwd: obj.path.join(__dirname, '../WinService') },
            function (error, stdout, stderr) {
              if (error != null && error != '') {
                console.log('ERROR: Unable to uninstall MeshCentral service: ' + error);
                process.exit();
              ...
              console.log(stdout);
              try {
                obj.fs.unlinkSync(obj.path.join(__dirname, '../WinService/winservice.js'));
              } catch (ex) {}
              try {
                obj.fs.rmdirSync(obj.path.join(__dirname, '../WinService'));
              } catch (ex) {}
            }
          );
        } else {
          require('child_process').exec(
            'node winservice.js --uninstall',
            { maxBuffer: 512000, timeout: 120000, cwd: __dirname },
            function (error, stdout, stderr) {
              if (error != null && error != '') {
                console.log('ERROR: Unable to uninstall MeshCentral service: ' + error);
                process.exit();
  ...
  obj.launchChildServer = function (startArgs) {
    ...
    const isInspectorAttached = (() => {
      try {
        return require('node:inspector').url() !== undefined;
      } catch (_) {
        return false;
      }
    }).call();
    const logFromChildProcess = isInspectorAttached ? () => {} : console.log.bind(console);
    if (!startArgs.includes('--disable-proto=delete')) {
      startArgs.unshift('--disable-proto=delete');
    }
    childProcess = child_process.execFile(
      process.argv[0],
      startArgs,
      { maxBuffer: Infinity, cwd: obj.parentpath },
      function (error, stdout, stderr) {
        if (childProcess.xrestart == 1) {
          setTimeout(function () {
            obj.launchChildServer(startArgs);
          }, 500); // This is an expected restart.
          ...
          if (typeof obj.args.selfupdate == 'string') {
            version = '@' + obj.args.selfupdate;
          } else if (typeof obj.args.specificupdate == 'string') {
            version = '@' + obj.args.specificupdate;
            delete obj.args.specificupdate;
          }
          const child_process = require('child_process');
          ...
          if (typeof obj.args.npmproxy == 'string') {
            env['HTTP_PROXY'] = env['HTTPS_PROXY'] = env['http_proxy'] = env['https_proxy'] = obj.args.npmproxy;
          }
          // always use --save-exact - https://stackoverflow.com/a/64507176/1210734
          const xxprocess = child_process.exec(
            npmpath + ' install --save-exact --no-audit meshcentral' + version + npmproxy,
            { maxBuffer: Infinity, cwd: obj.parentpath, env: env },
            function (error, stdout, stderr) {
              if (error != null && error != '') {
          ...
          xxprocess.data = '';
          xxprocess.stdout.on('data', function (data) {
            xxprocess.data += data;
          });
          ...
          xxprocess.on('close', function (code) {
            ...
            if (typeof obj.config.settings.runonserverupdated == 'string') {
              ...
              if (
                ...
                parentpath = require('path').join(__dirname, '../..');
              }
              child_process.exec(
                obj.config.settings.runonserverupdated + ' ' + getCurrentVersion(),
                { maxBuffer: 512000, timeout: 120000, cwd: parentpath },
                function (error, stdout, stderr) {}
              );
            }
            ...
            if (obj.args.cleannpmcacheonupdate === true) {
              ...
              const xxxprocess = child_process.exec(
                npmpath + ' cache clean --force',
                { maxBuffer: Infinity, cwd: obj.parentpath, env: env },
                function (error, stdout, stderr) {}
              );
              xxxprocess.on('close', function (code) {
                setTimeout(function () {
                  obj.launchChildServer(startArgs);
                }, 1000);
              });
          ...
          if (error != null) {
            ...
            if (typeof obj.config.settings.runonservererror == 'string') {
              ...
              child_process.exec(
                obj.config.settings.runonservererror + ' ' + getCurrentVersion(),
                { maxBuffer: 512000, timeout: 120000, cwd: parentpath },
                function (error, stdout, stderr) {}
              );
            }
    ...
    childProcess.stdout.on('data', function (data) {
      ...
      if (data.indexOf('Updating settings folder...') >= 0) {
        childProcess.xrestart = 1;
      } else if (data.indexOf('Updating server certificates...') >= 0) {
        childProcess.xrestart = 1;
      } else if (data.indexOf('Server Ctrl-C exit...') >= 0) {
        childProcess.xrestart = 2;
      } else if (data.indexOf('Starting self upgrade...') >= 0) {
        childProcess.xrestart = 3;
      } else if (data.indexOf('Server restart...') >= 0) {
        childProcess.xrestart = 1;
      } else if (data.indexOf('Starting self upgrade to: ') >= 0) {
        obj.args.specificupdate = data.substring(26).split('\r')[0].split('\n')[0];
        childProcess.xrestart = 3;
      }
      var datastr = data;
    ...
    childProcess.stderr.on('data', function (data) {
      ...
      if (data.startsWith('le.challenges[tls-sni-01].loopback')) {
        return;
      } // Ignore this error output from GreenLock
      if (data[data.length - 1] == '\n') {
        data = data.substring(0, data.length - 1);
      }
      obj.logError(data);
  ...
  obj.logError = function (err) {
    try {
      var errlogpath = null;
      if (typeof obj.args.mesherrorlogpath == 'string') {
        errlogpath = obj.path.join(obj.args.mesherrorlogpath, 'mesherrors.txt');
      } else {
        errlogpath = obj.getConfigFilePath('mesherrors.txt');
      }
      obj.fs.appendFileSync(
        errlogpath,
        '-------- ' +
  ...
  obj.getLatestServerVersion = function (callback) {
    ...
    try {
      if (typeof obj.args.selfupdate == 'string') {
        callback(getCurrentVersion(), obj.args.selfupdate);
        return;
      } // If we are targetting a specific version, return that one as current.
      const child_process = require('child_process');
      const npmpath = typeof obj.args.npmpath == 'string' ? obj.args.npmpath : 'npm';
      const npmproxy = typeof obj.args.npmproxy == 'string' ? ' --proxy ' + obj.args.npmproxy : '';
      const env = Object.assign({}, process.env); // Shallow clone
      ...
      const xxprocess = child_process.exec(
        npmpath + npmproxy + ' view meshcentral dist-tags.latest',
        { maxBuffer: 512000, cwd: obj.parentpath, env: env },
        function (error, stdout, stderr) {}
      );
      xxprocess.data = '';
      xxprocess.stdout.on('data', function (data) {
      ...
      xxprocess.on('close', function (code) {
        ...
        if (code == 0) {
          try {
            latestVer = xxprocess.data.split(' ').join('').split('\r').join('').split('\n').join('');
          } catch (ex) {}
        }
        callback(getCurrentVersion(), latestVer);
      });
    } catch (ex) {
      callback(getCurrentVersion(), null, ex);
    } // If the system is running out of memory, an exception here can easily happen.
  };
  ...
  obj.getServerTags = function (callback) {
    ...
    try {
      if (typeof obj.args.selfupdate == 'string') {
        callback({ current: getCurrentVersion(), latest: obj.args.selfupdate });
        return;
      } // If we are targetting a specific version, return that one as current.
      const child_process = require('child_process');
      const npmpath = typeof obj.args.npmpath == 'string' ? obj.args.npmpath : 'npm';
      ...
      const xxprocess = child_process.exec(
        npmpath + npmproxy + ' dist-tag ls meshcentral',
        { maxBuffer: 512000, cwd: obj.parentpath, env: env },
        function (error, stdout, stderr) {}
      );
      xxprocess.data = '';
      ...
      xxprocess.on('close', function (code) {
        ...
        if (code == 0) {
          try {
            ...
            for (var i in lines) {
              var s = lines[i].split(': ');
              if ((s.length == 2 && obj.args.npmtag == null) || obj.args.npmtag == s[0]) {
                tags[s[0]] = s[1];
              }
            }
          } catch (ex) {}
      ...
      callback({ current: getCurrentVersion() }, ex);
    } // If the system is running out of memory, an exception here can easily happen.
  };
  ...
  obj.getServerVersions = function (callback) {
    try {
      ...
      const xxprocess = child_process.exec(
        npmpath + npmproxy + ' view meshcentral versions --json',
        { maxBuffer: 512000, cwd: obj.parentpath, env: env },
        function (error, stdout, stderr) {}
      );
      xxprocess.data = '';
  ...
  obj.performServerUpdate = function (version) {
    ...
    if (version == null || version == '' || typeof version != 'string') {
      console.log('Starting self upgrade...');
    } else {
      console.log('Starting self upgrade to: ' + version);
    }
    process.exit(200);
    return true;
  };
  ...
  // Initiate server self-update
  obj.performServerCertUpdate = function () {
    console.log('Updating server certificates...');
    process.exit(200);
  };
  ...
  obj.StartVault = function () {
    ...
    if (obj.args.vault != null && obj.args.loadconfigfromdb != null) {
      console.log("Can't load configuration from both database and Vault.");
      process.exit();
      return;
    }
    ...
    if (typeof obj.args.vault == 'string') {
      ...
      if (typeof obj.args.token == 'string') {
        obj.args.vault.token = obj.args.token;
      }
      if (typeof obj.args.unsealkey == 'string') {
        obj.args.vault.unsealkey = obj.args.unsealkey;
      }
      if (typeof obj.args.name == 'string') {
        obj.args.vault.name = obj.args.name;
      }
    }

    // Load configuration for HashiCorp's Vault if needed
    if (obj.args.vault) {
      if (obj.args.vault.endpoint == null) {
        console.log('Missing Vault endpoint.');
        process.exit();
        return;
      }
      if (obj.args.vault.token == null) {
        console.log('Missing Vault token.');
        process.exit();
        return;
      }
      if (obj.args.vault.unsealkey == null) {
        console.log('Missing Vault unsealkey.');
        process.exit();
        return;
      }
      if (obj.args.vault.name == null) {
        obj.args.vault.name = 'meshcentral';
      }

      // Get new instance of the client
      const vault = require('node-vault')({ endpoint: obj.args.vault.endpoint, token: obj.args.vault.token });
      vault
        .unseal({ key: obj.args.vault.unsealkey })
        .then(function () {
          if (obj.args.vaultdeleteconfigfiles) {
            vault
              .delete('secret/data/' + obj.args.vault.name)
              .then(function (r) {
                console.log('Done.');
                process.exit();
              })
            ...
            if (obj.args.vaultpushconfigfiles == '*' || obj.args.vaultpushconfigfiles === true) {
              obj.args.vaultpushconfigfiles = obj.datapath;
            }
            obj.fs.readdir(obj.args.vaultpushconfigfiles, function (err, files) {
              if (err != null) {
                console.log('ERROR: Unable to read from folder ' + obj.args.vaultpushconfigfiles);
                process.exit();
                return;
              }
              ...
              if (configFound == false) {
                console.log('ERROR: No config.json in folder ' + obj.args.vaultpushconfigfiles);
                process.exit();
                return;
              }
              var configFiles = {};
              for (var i in files) {
                ...
                if (
                  ...
                  const path = obj.path.join(obj.args.vaultpushconfigfiles, files[i]),
                    binary = Buffer.from(obj.fs.readFileSync(path, { encoding: 'binary' }), 'binary');
                  console.log('Pushing ' + file + ', ' + binary.length + ' bytes.');
                  if (file.endsWith('.json') || file.endsWith('.key') || file.endsWith('.crt')) {
                    configFiles[file] = binary.toString();
                  } else {
                    configFiles[file] = binary.toString('base64');
                  }
                }
              }
              vault
                .write('secret/data/' + obj.args.vault.name, { data: configFiles })
                .then(function (r) {
                  console.log('Done.');
                  process.exit();
                })
            ...
            vault
              .read('secret/data/' + obj.args.vault.name)
              .then(function (r) {
                if (r == null || r.data == null || r.data.data == null) {
                  console.log('Unable to read configuration from Vault.');
                  process.exit();
                  return;
                }
                ...
                for (var file in configFiles) {
                  if (!file.endsWith('.json') && !file.endsWith('.key') && !file.endsWith('.crt')) {
                    configFiles[file] = Buffer.from(configFiles[file], 'base64');
                  }
                }
                ...
                if (obj.args.vaultpullconfigfiles) {
                  for (var i in configFiles) {
                    ...
                    try {
                      obj.fs.writeFileSync(fullFileName, configFiles[i]);
                    } catch (ex) {
                      console.log('Unable to write to ' + fullFileName);
                      process.exit();
                      return;
                    }
                ...
                var config2 = null;
                try {
                  config2 = JSON.parse(configFiles['config.json']);
                } catch (ex) {
                  console.log('Error, unable to parse config.json from Vault.');
                  process.exit();
                  return;
                }
                ...
                if (config.settings.vault != null && config2.settings != null) {
                  config2.settings.vault = config.settings.vault;
                }
  ...
  obj.StartEx = async function () {
    ...
    if (
      ...
      (config.settings.syslog != null || config.settings.syslogjson != null || config.settings.syslogauth != null)
    ) {
      if (config.settings.syslog === true) {
        config.settings.syslog = 'meshcentral';
      }
      if (config.settings.syslogjson === true) {
        config.settings.syslogjson = 'meshcentral-json';
      }
      if (config.settings.syslogauth === true) {
        config.settings.syslogauth = 'meshcentral-auth';
      }
      ...
      if (typeof config.settings.syslogjson == 'string') {
        ...
        obj.syslogjson.init(
          config.settings.syslogjson,
          obj.syslogjson.LOG_PID | obj.syslogjson.LOG_ODELAY,
          obj.syslogjson.LOG_LOCAL0
        );
      ...
      if (typeof config.settings.syslogauth == 'string') {
        ...
        obj.syslogauth.init(
          config.settings.syslogauth,
          obj.syslogauth.LOG_PID | obj.syslogauth.LOG_ODELAY,
          obj.syslogauth.LOG_LOCAL0
        );
    ...
    if (config) {
      for (var i in config) {
        if (
          ...
          addServerWarning('Unrecognized configuration option \"' + i + '\".', 3, [i]);
        }
      }
    }
    ...
    if (typeof obj.args.userallowedip == 'string') {
      if (obj.args.userallowedip == '') {
        config.settings.userallowedip = obj.args.userallowedip = null;
      } else {
        config.settings.userallowedip = obj.args.userallowedip = obj.args.userallowedip.split(' ').join('').split(',');
      }
    }
    if (typeof obj.args.userblockedip == 'string') {
      if (obj.args.userblockedip == '') {
        config.settings.userblockedip = obj.args.userblockedip = null;
      } else {
        config.settings.userblockedip = obj.args.userblockedip = obj.args.userblockedip.split(' ').join('').split(',');
      }
    }
    if (typeof obj.args.agentallowedip == 'string') {
      if (obj.args.agentallowedip == '') {
        config.settings.agentallowedip = obj.args.agentallowedip = null;
      } else {
        config.settings.agentallowedip = obj.args.agentallowedip = obj.args.agentallowedip
          .split(' ')
          .join('')
          .split(',');
      }
    }
    if (typeof obj.args.agentblockedip == 'string') {
      if (obj.args.agentblockedip == '') {
        config.settings.agentblockedip = obj.args.agentblockedip = null;
      } else {
        config.settings.agentblockedip = obj.args.agentblockedip = obj.args.agentblockedip
          .split(' ')
          .join('')
          .split(',');
      }
    }
    if (typeof obj.args.swarmallowedip == 'string') {
      if (obj.args.swarmallowedip == '') {
        obj.args.swarmallowedip = null;
      } else {
        obj.args.swarmallowedip = obj.args.swarmallowedip.split(' ').join('').split(',');
      }
    }
    if (
      typeof obj.args.agentupdateblocksize == 'number' &&
      obj.args.agentupdateblocksize >= 1024 &&
      obj.args.agentupdateblocksize <= 65531
    ) {
      obj.agentUpdateBlockSize = obj.args.agentupdateblocksize;
    }
    if (typeof obj.args.trustedproxy == 'string') {
      obj.args.trustedproxy = obj.args.trustedproxy.split(' ').join('').split(',');
    }
    if (typeof obj.args.tlsoffload == 'string') {
      obj.args.tlsoffload = obj.args.tlsoffload.split(' ').join('').split(',');
    }
    ...
    if (obj.args.cookieipcheck === false || obj.args.cookieipcheck == 'none') {
      obj.args.cookieipcheck = 'none';
    } else if (typeof obj.args.cookieipcheck != 'string' || obj.args.cookieipcheck.toLowerCase() != 'strict') {
      obj.args.cookieipcheck = 'lax';
    } else {
      obj.args.cookieipcheck = 'strict';
    }

    // Check the "cookieSameSite" value
    if (typeof obj.args.cookiesamesite != 'string') {
      delete obj.args.cookiesamesite;
    } else if (['none', 'lax', 'strict'].indexOf(obj.args.cookiesamesite.toLowerCase()) == -1) {
      delete obj.args.cookiesamesite;
    } else {
      obj.args.cookiesamesite = obj.args.cookiesamesite.toLowerCase();
    }
    ...
    if ((ver >= 11.11 && ver <= 12.15) || ver == 13.2) {
      if (obj.args.wscompression === true || obj.args.agentwscompression === true) {
        addServerWarning(
          'WebSocket compression is disabled, this feature is broken in NodeJS v11.11 to v12.15 and v13.2',
          4
        );
      }
    ...
    if (typeof obj.args.debug == 'string') {
      obj.debugSources = obj.args.debug.toLowerCase().split(',');
    } else if (typeof obj.args.debug == 'object') {
      obj.debugSources = obj.args.debug;
    } else if (obj.args.debug === true) {
      obj.debugSources = '*';
    }

    require('./db.js').CreateDB(obj, function (db) {
      obj.db = db;
      obj.db.SetupDatabase(function (dbversion) {
        // See if any database operations needs to be completed
        if (obj.args.deletedomain) {
          obj.db.DeleteDomain(obj.args.deletedomain, function () {
            console.log('Deleted domain ' + obj.args.deletedomain + '.');
            process.exit();
          });
          return;
        }
        if (obj.args.deletedefaultdomain) {
          obj.db.DeleteDomain('', function () {
            console.log('Deleted default domain.');
            process.exit();
          });
          return;
        }
        ...
        if (obj.args.showitem) {
          obj.db.Get(obj.args.showitem, function (err, docs) {
            console.log(JSON.stringify(docs, null, 2));
            process.exit();
          });
          return;
        }
        if (obj.args.listuserids) {
          obj.db.GetAllType('user', function (err, docs) {
            for (var i in docs) {
        ...
        if (obj.args.showmeshes) {
          obj.db.GetAllType('mesh', function (err, docs) {
            var x = [];
            for (var i in docs) {
              if (docs[i].deleted == null) {
                x.push(docs[i]);
              }
            }
        ...
        if (obj.args.showevents) {
          obj.db.GetAllEvents(function (err, docs) {
            console.log(JSON.stringify(docs, null, 2));
            process.exit();
          });
          return;
        }
        if (obj.args.showsmbios) {
          obj.db.GetAllSMBIOS(function (err, docs) {
            console.log(JSON.stringify(docs, null, 2));
            process.exit();
          });
          return;
        }
        if (obj.args.showpower) {
          obj.db.getAllPower(function (err, docs) {
            console.log(JSON.stringify(docs, null, 2));
            process.exit();
          });
          return;
        }
        if (obj.args.clearpower) {
          obj.db.removeAllPowerEvents(function () {
            process.exit();
          });
          return;
        }
        if (obj.args.showiplocations) {
          obj.db.GetAllType('iploc', function (err, docs) {
            console.log(docs);
            process.exit();
          });
          return;
        }
        if (obj.args.logintoken) {
          obj.getLoginToken(obj.args.logintoken, function (r) {
            console.log(r);
            process.exit();
          });
          return;
        }
        if (obj.args.logintokenkey) {
          obj.showLoginTokenKey(function (r) {
            console.log(r);
            process.exit();
          });
          return;
        ...
        if (obj.args.recordencryptionrecode) {
          obj.db.performRecordEncryptionRecode(function (count) {
            console.log('Re-encoded ' + count + ' record(s).');
            process.exit();
          });
          return;
        }
        if (obj.args.dbstats) {
          obj.db.getDbStats(function (stats) {
            console.log(stats);
            process.exit();
          });
          return;
        }
        if (obj.args.createaccount) {
          // Create a new user account
          if (
            typeof obj.args.createaccount != 'string' ||
            (obj.args.pass == null && obj.args.hashpass == null) ||
            obj.args.pass == '' ||
            obj.args.hashpass == '' ||
            obj.args.createaccount.indexOf(' ') >= 0
          ) {
            console.log(
              'Usage: --createaccount [userid] --pass [password] --domain (domain) --email (email) --name (name).'
            );
            process.exit();
            return;
          }
          var userid = 'user/' + (obj.args.domain ? obj.args.domain : '') + '/' + obj.args.createaccount.toLowerCase(),
            domainid = obj.args.domain ? obj.args.domain : '';
          if (obj.args.createaccount.startsWith('user/')) {
            userid = obj.args.createaccount;
            domainid = obj.args.createaccount.split('/')[1];
          }
          ...
          obj.db.Get(userid, function (err, docs) {
            ...
            if (domainid != '' && (config.domains == null || config.domains[domainid] == null)) {
            ...
            const user = {
              _id: userid,
              type: 'user',
              name: typeof obj.args.name == 'string' ? obj.args.name : userid.split('/')[2],
              domain: domainid,
              creation: Math.floor(Date.now() / 1000),
              links: {}
            };
            if (typeof obj.args.email == 'string') {
              user.email = obj.args.email;
            ...
            if (obj.args.hashpass) {
              ...
              if (hashpasssplit.length != 2) {
                console.log('Invalid hashed password.');
                process.exit();
                return;
              }
              user.salt = hashpasssplit[0];
              user.hash = hashpasssplit[1];
              obj.db.Set(user, function () {
                console.log('Done. This command will only work if MeshCentral is stopped.');
                process.exit();
                return;
              });
            } else {
              ...
              require('./pass').hash(
                ...
                function (err, salt, hash, tag) {
                  if (err) {
                    console.log('Unable create account password: ' + err);
                    process.exit();
                    return;
                  }
                  user.salt = salt;
        ...
        if (obj.args.resetaccount) {
          // Unlock a user account, set a new password and remove 2FA
          if (typeof obj.args.resetaccount != 'string' || obj.args.resetaccount.indexOf(' ') >= 0) {
            console.log('Usage: --resetaccount [userid] --domain (domain) --pass [password].');
            process.exit();
            return;
          }
          var userid = 'user/' + (obj.args.domain ? obj.args.domain : '') + '/' + obj.args.resetaccount.toLowerCase();
          if (obj.args.resetaccount.startsWith('user/')) {
            userid = obj.args.resetaccount;
          }
          if (userid.split('/').length != 3) {
            console.log('Invalid userid.');
            process.exit();
          ...
          obj.db.Get(userid, function (err, docs) {
            ...
            if (docs == null || docs.length == 0) {
              console.log('Unknown userid, usage: --resetaccount [userid] --domain (domain) --pass [password].');
              process.exit();
              return;
            }
            const user = docs[0];
            ...
            delete user.otpkeys;
            delete user.otphkeys;
            delete user.otpdev;
            delete user.otpsms;
            delete user.otpmsg;
            user.otpduo; // Disable 2FA
            ...
            if (config.domains[user.domain].auth || config.domains[user.domain].authstrategies) {
              console.log(
                'This users domain has external authentication methods enabled so the password will not be changed if you set one'
              );
              obj.db.Set(user, function () {
                console.log('Done.');
                process.exit();
        ...
        if (obj.args.adminaccount) {
          // Set a user account to server administrator
          if (typeof obj.args.adminaccount != 'string' || obj.args.adminaccount.indexOf(' ') >= 0) {
            console.log('Invalid userid, usage: --adminaccount [username] --domain (domain)');
            process.exit();
            return;
          }
          var userid = 'user/' + (obj.args.domain ? obj.args.domain : '') + '/' + obj.args.adminaccount.toLowerCase();
          if (obj.args.adminaccount.startsWith('user/')) {
            userid = obj.args.adminaccount;
          }
          if (userid.split('/').length != 3) {
            console.log('Invalid userid.');
            process.exit();
          ...
          obj.db.Get(userid, function (err, docs) {
            ...
            if (docs == null || docs.length == 0) {
              console.log('Unknown userid, usage: --adminaccount [userid] --domain (domain).');
              process.exit();
              return;
            }
            docs[0].siteadmin = 0xffffffff; // Set user as site administrator
            obj.db.Set(docs[0], function () {
              console.log('Done. This command will only work if MeshCentral is stopped.');
              process.exit();
              return;
            });
        ...
        if (obj.args.removesubdomain) {
          // Remove all references to a sub domain from the database
          if (typeof obj.args.removesubdomain != 'string' || obj.args.removesubdomain.indexOf(' ') >= 0) {
            console.log('Invalid sub domain, usage: --removesubdomain [domain]');
            process.exit();
            return;
          }
          obj.db.removeDomain(obj.args.removesubdomain, function () {
            console.log('Done.');
            process.exit();
            return;
          });
        ...
        if (obj.args.removetestagents) {
          // Remove all test agents from the database
        ...
        if (obj.args.nedbtodb) {
          if (db.databaseType == 1) {
            console.log("NeDB is current database, can't perform transfer.");
            process.exit();
            return;
          }
          console.log('Transfering NeDB data into database...');
          db.nedbtodb(function (msg) {
            console.log(msg);
            process.exit();
          });
          return;
                  ...
                  if (typeof obj.args.dblistconfigfiles == 'string') {
                    ...
                    if (data == null) {
                      console.log(
                        docs[i]._id.split('/')[1] +
                          ', ' +
                          Buffer.from(docs[i].data, 'base64').length +
                          ' encrypted bytes - Unable to decrypt.'
                      );
                    } else {
        ...
        if (obj.args.dbshowconfigfile) {
          if (typeof obj.args.configkey != 'string') {
            console.log('Error, --configkey is required.');
            process.exit();
            return;
          }
        ...
        if (obj.args.dbdeleteconfigfiles) {
          console.log('Deleting all configuration files from the database...');
        ...
        if (obj.args.dbpushconfigfiles) {
          ...
          if (obj.args.dbpushconfigfiles !== true && typeof obj.args.dbpushconfigfiles != 'string') {
            ...
            console.log(
              '       --dbpulldatafiles            This will import files from meshcentral-data into the db.'
            );
            process.exit();
          } else {
            if (obj.args.dbpushconfigfiles == '*' || obj.args.dbpushconfigfiles === true) {
              obj.args.dbpushconfigfiles = obj.datapath;
            }
            obj.fs.readdir(obj.args.dbpushconfigfiles, function (err, files) {
              if (err != null) {
                console.log('ERROR: Unable to read from folder ' + obj.args.dbpushconfigfiles);
                process.exit();
                return;
              }
              var configFound = false;
              ...
              if (configFound == false) {
                console.log('ERROR: No config.json in folder ' + obj.args.dbpushconfigfiles);
                process.exit();
                return;
              }
              obj.db.RemoveAllOfType('cfile', function () {
                obj.fs.readdir(obj.args.dbpushconfigfiles, function (err, files) {
                  ...
                  for (var i in files) {
                    ...
                    if (
                      ...
                      const path = obj.path.join(obj.args.dbpushconfigfiles, files[i]),
                        binary = Buffer.from(obj.fs.readFileSync(path, { encoding: 'binary' }), 'binary');
                      console.log('Pushing ' + file + ', ' + binary.length + ' bytes.');
                      lockCount++;
                      if (obj.args.oldencrypt) {
                        obj.db.setConfigFile(file, obj.db.oldEncryptData(obj.args.configkey, binary), function () {
                          if (--lockCount == 0) {
                            console.log('Done.');
                            process.exit();
                          }
                        ...
                        obj.db.setConfigFile(file, obj.db.encryptData(obj.args.configkey, binary), function () {
                          if (--lockCount == 0) {
                            console.log('Done.');
                            process.exit();
                          }
                ...
                if (docs.length == 0) {
                  ...
                  for (var i in docs) {
                    const file = docs[i]._id.split('/')[1],
                      binary = obj.db.decryptData(obj.args.configkey, docs[i].data);
                    if (binary == null) {
                      console.log('Invalid config key.');
                    } else {
                      const fullFileName = obj.path.join(obj.args.dbpullconfigfiles, file);
                      try {
                        obj.fs.writeFileSync(fullFileName, binary);
                      } catch (ex) {
                        console.log('Unable to write to ' + fullFileName);
                        process.exit();
                        return;
        ...
        if (obj.args.dbexport) {
          ...
          if (obj.args.dbexport == true) {
            obj.args.dbexport = obj.getConfigFilePath('meshcentral.db.json');
          }
          obj.db.GetAll(function (err, docs) {
            obj.fs.writeFileSync(obj.args.dbexport, JSON.stringify(docs));
            console.log('Exported ' + docs.length + ' objects(s) to ' + obj.args.dbexport + '.');
            process.exit();
          });
          return;
        }
        if (obj.args.dbexportmin) {
          ...
          if (obj.args.dbexportmin == true) {
            obj.args.dbexportmin = obj.getConfigFilePath('meshcentral.db.json');
          }
          obj.db.GetAllType({ $in: ['user', 'node', 'mesh'] }, function (err, docs) {
            obj.fs.writeFileSync(obj.args.dbexportmin, JSON.stringify(docs));
            console.log('Exported ' + docs.length + ' objects(s) to ' + obj.args.dbexportmin + '.');
            process.exit();
          });
          return;
        }
        if (obj.args.dbimport) {
          ...
          if (obj.args.dbimport == true) {
            obj.args.dbimport = obj.getConfigFilePath('meshcentral.db.json');
          }
          var json = null,
            json2 = '',
            badCharCount = 0;
          try {
            json = obj.fs.readFileSync(obj.args.dbimport, { encoding: 'utf8' });
          } catch (ex) {
            console.log('Invalid JSON file: ' + obj.args.dbimport + ': ' + ex);
            process.exit();
          }
          for (i = 0; i < json.length; i++) {
            if (json.charCodeAt(i) >= 32) {
              json2 += json[i];
            } else {
              var tt = json.charCodeAt(i);
              if (tt != 10 && tt != 13) {
                badCharCount++;
              }
            }
          ...
          if (badCharCount > 0) {
            console.log(badCharCount + ' invalid character(s) where removed.');
          }
          try {
            json = JSON.parse(json2);
          } catch (ex) {
            console.log('Invalid JSON format: ' + obj.args.dbimport + ': ' + e);
            process.exit();
          }
          if (json == null || typeof json.length != 'number' || json.length < 1) {
            console.log('Invalid JSON format: ' + obj.args.dbimport + '.');
          }
          ...
          for (i in json) {
            ...
            for (var j in doc) {
              if (j.indexOf('.') >= 0) {
                console.log('Invalid field name (' + j + ') in document: ' + json[i]);
                return;
              }
            }
          ...
          setTimeout(function () {
            // If the Mongo database is being created for the first time, there is a race condition here. This will get around it.
            obj.db.RemoveAll(function () {
              obj.db.InsertMany(json, function (err) {
                if (err != null) {
                  console.log(err);
                } else {
                  console.log('Imported ' + json.length + ' objects(s) from ' + obj.args.dbimport + '.');
                }
                process.exit();
              });
            });
        ...
        if (obj.args.dbmerge) {
          // Import the entire database from a JSON file
          if (obj.args.dbmerge == true) {
            obj.args.dbmerge = obj.getConfigFilePath('meshcentral.db.json');
          }
          var json = null,
            json2 = '',
            badCharCount = 0;
          try {
            json = obj.fs.readFileSync(obj.args.dbmerge, { encoding: 'utf8' });
          } catch (ex) {
            console.log('Invalid JSON file: ' + obj.args.dbmerge + ': ' + ex);
            process.exit();
          }
          for (i = 0; i < json.length; i++) {
            if (json.charCodeAt(i) >= 32) {
          ...
          try {
            ...
            console.log('Invalid JSON format: ' + obj.args.dbmerge + ': ' + ex);
            process.exit();
          }
          if (json == null || typeof json.length != 'number' || json.length < 1) {
            console.log('Invalid JSON format: ' + obj.args.dbimport + '.');
          ...
          obj.db.GetAllType('user', function (err, docs) {
            ...
            obj.db.GetAllType('mesh', function (err, docs) {
              ...
              for (var i in objectToAdd) {
                pendingCalls++;
                obj.db.Set(objectToAdd[i], function (err) {
                  if (err != null) {
                    console.log(err);
                  } else {
                    if (--pendingCalls == 0) {
        ...
        if (obj.args.loadconfigfromdb) {
          var key = null;
          if (typeof obj.args.configkey == 'string') {
            key = obj.args.configkey;
          } else if (typeof obj.args.loadconfigfromdb == 'string') {
            key = obj.args.loadconfigfromdb;
          }
          if (key == null) {
            console.log('Error, --configkey is required.');
            process.exit();
          ...
          obj.db.getAllConfigFiles(key, function (configFiles) {
            if (configFiles == null) {
              console.log('Error, no configuration files found or invalid configkey.');
              process.exit();
              return;
            }
            if (!configFiles['config.json']) {
              console.log('Error, could not file config.json from database.');
              process.exit();
              return;
            }
            if (typeof configFiles['config.json'] == 'object') {
              configFiles['config.json'] = configFiles['config.json'].toString();
            }
            if (configFiles['config.json'].charCodeAt(0) == 65279) {
              configFiles['config.json'] = configFiles['config.json'].substring(1);
            }
            ...
            try {
              ...
              console.log('Error, unable to parse config.json from database.', ex);
              process.exit();
              return;
            }
  ...
  obj.StartEx1b = async function () {
    ...
    if (Number(process.version.match(/^v(\d+\.\d+)/)[1]) < 20) {
      addServerWarning(
        'MeshCentral will require Node v20 or above in the future, your current version is ' + process.version + '.'
      );
    }
    ...
    if (obj.config.settings.authlog != null) {
      obj.fs.open(obj.config.settings.authlog, 'a', function (err, fd) {
        if (err == null) {
          ...
          console.log('ERROR: Unable to open: ' + obj.config.settings.authlog);
        }
      });
    }
    ...
    if (typeof obj.args.crowdsec == 'object') {
      obj.crowdSecBounser = require('./crowdsec.js').CreateCrowdSecBouncer(obj, obj.args.crowdsec);
    }

    // Check if self update is allowed. If running as a Windows service, self-update is not possible.
    if (obj.fs.existsSync(obj.path.join(__dirname, 'daemon'))) {
    ...
    if (obj.serverSelfWriteAllowed == true && typeof obj.args.selfupdate == 'string') {
      ...
      if (getCurrentVersion() !== obj.args.selfupdate) {
        obj.performServerUpdate();
        return;
      } // We are targetting a specific version, run self update now.
    }

    // Write the server state
    obj.updateServerState('state', 'starting');
    if (process.pid) {
      obj.updateServerState('server-pid', process.pid);
    }
    if (process.ppid) {
      obj.updateServerState('server-parent-pid', process.ppid);
    }
    ...
    for (i in xenv) {
      if (obj.args[xenv[i]] == null && process.env['mesh' + xenv[i]]) {
        obj.args[xenv[i]] = obj.common.toNumber(process.env['mesh' + xenv[i]]);
      }
    }

    // Validate the domains, this is used for multi-hosting
    if (obj.config.domains == null) {
      obj.config.domains = {};
    }
    if (obj.config.domains[''] == null) {
      obj.config.domains[''] = {};
    }
    if (obj.config.domains[''].dns != null) {
      console.log("ERROR: Default domain can't have a DNS name.");
      return;
    }
    var xdomains = {};
    for (i in obj.config.domains) {
      xdomains[i.toLowerCase()] = obj.config.domains[i];
    }
    obj.config.domains = xdomains;
    var bannedDomains = ['public', 'private', 'images', 'scripts', 'styles', 'views']; // List of banned domains
    for (i in obj.config.domains) {
      for (var j in bannedDomains) {
        if (i == bannedDomains[j]) {
          console.log("ERROR: Domain '" + i + "' is not allowed domain name in config.json.");
          delete obj.config.domains[i];
        }
      }
    }
    for (i in obj.config.domains) {
      if (i.length > 64 || Buffer.from(i).length > 64) {
        console.log("ERROR: Domain '" + i + "' is longer that 64 bytes, this is not allowed.");
        delete obj.config.domains[i];
      }
    }
    for (i in obj.config.domains) {
      ...
      if (typeof obj.config.domaindefaults == 'object') {
        for (var j in obj.config.domaindefaults) {
          if (obj.config.domains[i][j] == null) {
            obj.config.domains[i][j] = obj.config.domaindefaults[j];
          }
        }
      }

      // Perform domain setup
      if (typeof obj.config.domains[i] != 'object') {
        console.log('ERROR: Invalid domain configuration in config.json.');
        process.exit();
        return;
      }
      if (i.length > 0 && i[0] == '_') {
        delete obj.config.domains[i];
        continue;
      } // Remove any domains with names that start with _
      if (typeof config.domains[i].auth == 'string') {
        config.domains[i].auth = config.domains[i].auth.toLowerCase();
      }
      if (obj.config.domains[i].limits == null) {
        obj.config.domains[i].limits = {};
      }
      if (obj.config.domains[i].dns == null) {
        obj.config.domains[i].url = i == '' ? '/' : '/' + i + '/';
      } else {
        obj.config.domains[i].url = '/';
      }
      obj.config.domains[i].id = i;
      if (typeof obj.config.domains[i].maxdeviceview != 'number' || obj.config.domains[i].maxdeviceview < 1) {
        delete obj.config.domains[i].maxdeviceview;
      }
      if (typeof obj.config.domains[i].loginkey == 'string') {
        obj.config.domains[i].loginkey = [obj.config.domains[i].loginkey];
      }
      if (
        obj.config.domains[i].loginkey != null &&
        obj.common.validateAlphaNumericArray(obj.config.domains[i].loginkey, 1, 128) == false
      ) {
        console.log('ERROR: Invalid login key, must be alpha-numeric string with no spaces.');
        process.exit();
        return;
      }
      if (typeof obj.config.domains[i].agentkey == 'string') {
        obj.config.domains[i].agentkey = [obj.config.domains[i].agentkey];
      }
      if (
        obj.config.domains[i].agentkey != null &&
        obj.common.validateAlphaNumericArray(obj.config.domains[i].agentkey, 1, 128) == false
      ) {
        console.log('ERROR: Invalid agent key, must be alpha-numeric string with no spaces.');
        process.exit();
        return;
      }
      obj.config.domains[i].userallowedip = obj.config.domains[i].userallowedip = readIpListFromFile(
        obj.config.domains[i].userallowedip
      );
      obj.config.domains[i].userblockedip = obj.config.domains[i].userblockedip = readIpListFromFile(
        obj.config.domains[i].userblockedip
      );
      obj.config.domains[i].agentallowedip = obj.config.domains[i].agentallowedip = readIpListFromFile(
        obj.config.domains[i].agentallowedip
      );
      obj.config.domains[i].agentblockedip = obj.config.domains[i].agentblockedip = readIpListFromFile(
        obj.config.domains[i].agentblockedip
      );
      if (typeof obj.config.domains[i].userallowedip == 'string') {
        if (obj.config.domains[i].userallowedip == '') {
          delete obj.config.domains[i].userallowedip;
        } else {
          obj.config.domains[i].userallowedip = obj.config.domains[i].userallowedip.split(' ').join('').split(',');
        }
      }
      if (typeof obj.config.domains[i].userblockedip == 'string') {
        if (obj.config.domains[i].userblockedip == '') {
          delete obj.config.domains[i].userblockedip;
        } else {
          obj.config.domains[i].userblockedip = obj.config.domains[i].userblockedip.split(' ').join('').split(',');
        }
      }
      if (typeof obj.config.domains[i].agentallowedip == 'string') {
        if (obj.config.domains[i].agentallowedip == '') {
          delete obj.config.domains[i].agentallowedip;
        } else {
          obj.config.domains[i].agentallowedip = obj.config.domains[i].agentallowedip.split(' ').join('').split(',');
        }
      }
      if (typeof obj.config.domains[i].agentblockedip == 'string') {
        if (obj.config.domains[i].agentblockedip == '') {
          delete obj.config.domains[i].agentblockedip;
        } else {
          obj.config.domains[i].agentblockedip = obj.config.domains[i].agentblockedip.split(' ').join('').split(',');
        }
      }
      // Check IP lists and ranges and if DNS return IP addresses
      ...
      if (typeof obj.config.domains[i].ignoreagenthashcheck == 'string') {
        if (obj.config.domains[i].ignoreagenthashcheck == '') {
          delete obj.config.domains[i].ignoreagenthashcheck;
        } else {
          obj.config.domains[i].ignoreagenthashcheck = obj.config.domains[i].ignoreagenthashcheck.split(',');
        }
      }
      if (typeof obj.config.domains[i].allowedorigin == 'string') {
        if (obj.config.domains[i].allowedorigin == '') {
          delete obj.config.domains[i].allowedorigin;
        } else {
          obj.config.domains[i].allowedorigin = obj.config.domains[i].allowedorigin.split(',');
        }
      }
      if (
        obj.config.domains[i].passwordrequirements != null &&
        typeof obj.config.domains[i].passwordrequirements == 'object'
      ) {
        if (typeof obj.config.domains[i].passwordrequirements.skip2factor == 'string') {
          obj.config.domains[i].passwordrequirements.skip2factor =
            obj.config.domains[i].passwordrequirements.skip2factor.split(',');
        } else {
        ...
        if (Array.isArray(obj.config.domains[i].passwordrequirements.logintokens)) {
          ...
          for (var j in obj.config.domains[i].passwordrequirements.logintokens) {
            ...
            if (splitVal.length == 1) {
              newValues.push('user/' + i + '/' + splitVal[0]);
            }
            if (splitVal.length == 2) {
              newValues.push('user/' + splitVal[0] + '/' + splitVal[1]);
            }
            if (splitVal.length == 3) {
              newValues.push(splitVal[0] + '/' + splitVal[1] + '/' + splitVal[2]);
            }
          }
      ...
      if (obj.config.domains[i].auth == 'ldap' && typeof obj.config.domains[i].ldapoptions != 'object') {
        if (i == '') {
          console.log('ERROR: Default domain is LDAP, but is missing LDAPOptions.');
        } else {
          console.log("ERROR: Domain '" + i + "' is LDAP, but is missing LDAPOptions.");
        }
        process.exit();
        return;
      }
      if (obj.config.domains[i].auth == 'ldap' || obj.config.domains[i].auth == 'sspi') {
        obj.config.domains[i].newaccounts = 0;
      } // No new accounts allowed in SSPI/LDAP authentication modes.
      if (obj.config.domains[i].sitestyle == null) {
        obj.config.domains[i].sitestyle = 2;
      } // Default to site style #2

      // Convert newAccountsRights from a array of strings to flags number.
      obj.config.domains[i].newaccountsrights = obj.common.meshServerRightsArrayToNumber(
        obj.config.domains[i].newaccountsrights
      );
      if (typeof obj.config.domains[i].newaccountsrights != 'number') {
        delete obj.config.domains[i].newaccountsrights;
      }
      ...
      if (
        ...
        if (
          obj.config.domains[i].webviewspath == null &&
          obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web-' + i + '/views'))
        ) {
          obj.config.domains[i].webviewspath = obj.path.join(__dirname, '../../meshcentral-web-' + i + '/views');
        }
        if (
          obj.config.domains[i].webpublicpath == null &&
          obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web-' + i + '/public'))
        ) {
          obj.config.domains[i].webpublicpath = obj.path.join(__dirname, '../../meshcentral-web-' + i + '/public');
        }
        if (
          obj.config.domains[i].webemailspath == null &&
          obj.fs.existsSync(obj.path.join(__dirname, '../../meshcentral-web-' + i + '/emails'))
        ) {
          obj.config.domains[i].webemailspath = obj.path.join(__dirname, '../../meshcentral-web-' + i + '/emails');
        }
      } else {
        if (
          obj.config.domains[i].webviewspath == null &&
      ...
      if (typeof obj.config.domains[i].agentcustomization == 'object') {
        if (typeof obj.config.domains[i].agentcustomization.displayname != 'string') {
          delete obj.config.domains[i].agentcustomization.displayname;
        } else {
          obj.config.domains[i].agentcustomization.displayname = obj.config.domains[i].agentcustomization.displayname
            .split('\r')
            .join('')
            .split('\n')
            .join('');
        }
        if (typeof obj.config.domains[i].agentcustomization.description != 'string') {
          delete obj.config.domains[i].agentcustomization.description;
        } else {
          obj.config.domains[i].agentcustomization.description = obj.config.domains[i].agentcustomization.description
            .split('\r')
            .join('')
            .split('\n')
            .join('');
        }
        if (typeof obj.config.domains[i].agentcustomization.companyname != 'string') {
          delete obj.config.domains[i].agentcustomization.companyname;
        } else {
          obj.config.domains[i].agentcustomization.companyname = obj.config.domains[i].agentcustomization.companyname
            .split('\r')
            .join('')
            .split('\n')
            .join('');
        }
        if (typeof obj.config.domains[i].agentcustomization.servicename != 'string') {
          delete obj.config.domains[i].agentcustomization.servicename;
        } else {
          obj.config.domains[i].agentcustomization.servicename = obj.config.domains[i].agentcustomization.servicename
            .split('\r')
            .join('')
            .split('\n')
            .join('')
        ...
        if (typeof obj.config.domains[i].agentcustomization.image != 'string') {
          delete obj.config.domains[i].agentcustomization.image;
        } else {
          try {
            obj.config.domains[i].agentcustomization.image =
              'data:image/png;base64,' +
              Buffer.from(
                obj.fs.readFileSync(obj.getConfigFilePath(obj.config.domains[i].agentcustomization.image)),
                'binary'
              ).toString('base64');
          } catch (ex) {
            console.log(ex);
      ...
      if (typeof obj.config.domains[i].userconsentflags == 'object') {
        var flags = 0;
        if (obj.config.domains[i].userconsentflags.desktopnotify == true) {
          flags |= 1;
        }
        if (obj.config.domains[i].userconsentflags.terminalnotify == true) {
          flags |= 2;
        }
        if (obj.config.domains[i].userconsentflags.filenotify == true) {
          flags |= 4;
        }
        if (obj.config.domains[i].userconsentflags.desktopprompt == true) {
          flags |= 8;
        }
        if (obj.config.domains[i].userconsentflags.terminalprompt == true) {
          flags |= 16;
        }
        if (obj.config.domains[i].userconsentflags.fileprompt == true) {
          flags |= 32;
        }
        if (obj.config.domains[i].userconsentflags.desktopprivacybar == true) {
          flags |= 64;
        }
      ...
      if (typeof obj.config.domains[i].amtmanager == 'object') {
        if (typeof obj.config.domains[i].amtmanager.tlsrootcert == 'object') {
          obj.config.domains[i].amtmanager.tlsrootcert2 = obj.certificateOperations.loadGenericCertAndKey(
            obj.config.domains[i].amtmanager.tlsrootcert
          );
          if (obj.config.domains[i].amtmanager.tlsrootcert2 == null) {
      ...
      if (typeof obj.config.domains[i].agentfileinfo == 'object') {
        if (
          obj.config.domains[i].agentfileinfo.fileversionnumber != null &&
          obj.common.parseVersion(obj.config.domains[i].agentfileinfo.fileversionnumber) == null
        ) {
          delete obj.config.domains[i].agentfileinfo.fileversionnumber;
        }
        if (
          obj.config.domains[i].agentfileinfo.productversionnumber != null &&
          obj.common.parseVersion(obj.config.domains[i].agentfileinfo.productversionnumber) == null
        ) {
          delete obj.config.domains[i].agentfileinfo.productversionnumber;
        }
        if (
          obj.config.domains[i].agentfileinfo.fileversionnumber == null &&
          typeof obj.config.domains[i].agentfileinfo.fileversion == 'string' &&
          obj.common.parseVersion(obj.config.domains[i].agentfileinfo.fileversion) != null
        ) {
          obj.config.domains[i].agentfileinfo.fileversionnumber = obj.config.domains[i].agentfileinfo.fileversion;
        }
        if (typeof obj.config.domains[i].agentfileinfo.icon == 'string') {
          // Load the agent .ico file
          var icon = null;
          try {
            icon = require('./authenticode.js').loadIcon(
              obj.path.join(obj.datapath, obj.config.domains[i].agentfileinfo.icon)
            );
          } catch (ex) {}
          if (icon != null) {
            ...
            addServerWarning('Unable to load agent icon file: ' + obj.config.domains[i].agentfileinfo.icon + '.', 23, [
              obj.config.domains[i].agentfileinfo.icon
            ]);
        ...
        if (typeof obj.config.domains[i].agentfileinfo.logo == 'string') {
          ...
          var logo = null;
          try {
            logo = require('./authenticode.js').loadBitmap(
              obj.path.join(obj.datapath, obj.config.domains[i].agentfileinfo.logo)
            );
          } catch (ex) {}
          if (logo != null) {
            ...
            // Failed to load the icon file, display a server warning
            addServerWarning('Unable to load agent logo file: ' + obj.config.domains[i].agentfileinfo.logo + '.', 24, [
              obj.config.domains[i].agentfileinfo.logo
            ]);
    ...
    if (obj.args.user != null && typeof obj.args.user != 'string') {
      delete obj.args.user;
    }
    if (obj.args.ciralocalfqdn != null && (obj.args.lanonly == true || obj.args.wanonly == true)) {
      addServerWarning("CIRA local FQDN's ignored when server in LAN-only or WAN-only mode.", 7);
    }
    if (obj.args.ciralocalfqdn != null && obj.args.ciralocalfqdn.split(',').length > 4) {
      addServerWarning("Can't have more than 4 CIRA local FQDN's. Ignoring value.", 8);
      obj.args.ciralocalfqdn = null;
    }
    if (obj.args.ignoreagenthashcheck === true) {
      addServerWarning('Agent hash checking is being skipped, this is unsafe.', 9);
    }
    if (obj.args.port == null || typeof obj.args.port != 'number') {
      obj.args.port = 443;
    }
    ...
    if (typeof obj.args.agentidletimeout != 'number') {
      obj.args.agentidletimeout = 150000;
    } else {
      obj.args.agentidletimeout *= 1000;
    } // Default agent idle timeout is 2m, 30sec.
    if (obj.args.lanonly != true && typeof obj.args.webrtconfig == 'object') {
      // fix incase you are using an old mis-spelt webrtconfig
      obj.args.webrtcconfig = obj.args.webrtconfig;
      delete obj.args.webrtconfig;
    }
    if (obj.args.lanonly != true && obj.args.webrtcconfig == null) {
      obj.args.webrtcconfig = {
        iceServers: [{ urls: 'stun:stun.l.google.com:19302' }, { urls: 'stun:stun.cloudflare.com:3478' }]
      };
    } // Setup default WebRTC STUN servers
    else if (obj.args.lanonly != true && typeof obj.args.webrtcconfig == 'object') {
      if (obj.args.webrtcconfig.iceservers) {
        // webrtc is case-sensitive, so must rename iceservers to iceServers!
        obj.args.webrtcconfig.iceServers = obj.args.webrtcconfig.iceservers;
        delete obj.args.webrtcconfig.iceservers;
      }
    }
    if (typeof obj.args.ignoreagenthashcheck == 'string') {
      if (obj.args.ignoreagenthashcheck == '') {
        delete obj.args.ignoreagenthashcheck;
      } else {
        obj.args.ignoreagenthashcheck = obj.args.ignoreagenthashcheck.split(',');
      }
    }
    ...
    if (obj.args.admin && typeof obj.args.admin == 'string') {
      ...
      if (adminname.length == 1) {
        adminname = 'user//' + adminname[0];
      } else if (adminname.length == 2) {
        adminname = 'user/' + adminname[0] + '/' + adminname[1];
      } else {
        console.log('Invalid administrator name.');
        process.exit();
        return;
      }
      obj.db.Get(adminname, function (err, user) {
        ...
        obj.db.Set(user[0], function () {
          if (user[0].domain == '') {
            console.log('User ' + user[0].name + ' set to site administrator.');
          } else {
            console.log('User ' + user[0].name + ' of domain ' + user[0].domain + ' set to site administrator.');
          }
          process.exit();
          return;
        });
    ...
    if (obj.args.unadmin && typeof obj.args.unadmin == 'string') {
      ...
      obj.db.Get(adminname, function (err, user) {
        ...
        if (user[0].siteadmin) {
          delete user[0].siteadmin;
        }
        obj.db.Set(user[0], function () {
          if (user[0].domain == '') {
            console.log('User ' + user[0].name + ' is not a site administrator.');
          } else {
            console.log('User ' + user[0].name + ' of domain ' + user[0].domain + ' is not a site administrator.');
          }
          process.exit();
          return;
        });
    ...
    if (obj.config && obj.config.settings && obj.config.settings.agentlogdump) {
      obj.fs.open(obj.path.join(obj.datapath, 'agenterrorlogs.txt'), 'a', function (err, fd) {
        obj.agentErrorLog = fd;
      });
    }
    ...
    obj.db.Get('dbconfig', function (err, dbconfig) {
      if (dbconfig != null && dbconfig.length == 1) {
        obj.dbconfig = dbconfig[0];
      } else {
        obj.dbconfig = { _id: 'dbconfig', version: 1 };
      }
      if (obj.dbconfig.amtWsEventSecret == null) {
        obj.crypto.randomBytes(32, function (err, buf) {
          obj.dbconfig.amtWsEventSecret = buf.toString('hex');
          obj.db.Set(obj.dbconfig);
        });
      }
      ...
      if (obj.args.getwspass) {
        if (obj.args.getwspass.length == 64) {
          obj.crypto.randomBytes(6, function (err, buf) {
            while (obj.dbconfig.amtWsEventSecret == null) {
              process.nextTick();
            }
            const username = buf.toString('hex');
            const nodeid = obj.args.getwspass;
            const pass = obj.crypto
              .createHash('sha384')
              .update(username.toLowerCase() + ':' + nodeid + ':' + obj.dbconfig.amtWsEventSecret)
              .digest('base64')
              .substring(0, 12)
              .split('/')
              .join('x')
      ...
      if (
        obj.config &&
        obj.config.settings &&
        obj.config.settings.plugins != null &&
        obj.config.settings.plugins != false &&
        (typeof obj.config.settings.plugins != 'object' || obj.config.settings.plugins.enabled != false)
      ) {
  ...
  obj.StartEx2 = function () {
    ...
    obj.certificateOperations.GetMeshServerCertificate(obj.args, obj.config, function (certs) {
      ...
      if (obj.config.letsencrypt == null || obj.redirserver == null) {
        ...
        var leok = true;
        if (typeof obj.config.letsencrypt.names != 'string' && typeof obj.config.settings.cert == 'string') {
          obj.config.letsencrypt.names = obj.config.settings.cert;
        }
        if (typeof obj.config.letsencrypt.email != 'string') {
          leok = false;
          addServerWarning("Missing Let's Encrypt email address.", 10);
        } else if (typeof obj.config.letsencrypt.names != 'string') {
          leok = false;
          addServerWarning("Invalid Let's Encrypt host names.", 11);
        } else if (obj.config.letsencrypt.names.indexOf('*') >= 0) {
          leok = false;
          addServerWarning("Invalid Let's Encrypt names, can't contain a *.", 12);
        } else if (obj.config.letsencrypt.email.split('@').length != 2) {
          leok = false;
          addServerWarning("Invalid Let's Encrypt email address.", 10);
        } else if (obj.config.letsencrypt.email.trim() !== obj.config.letsencrypt.email) {
          leok = false;
          addServerWarning("Invalid Let's Encrypt email address.", 10);
        } else {
          ...
          if (obj.letsencrypt == null) {
            addServerWarning("Unable to setup Let's Encrypt module.", 13);
            leok = false;
          }
        }
        if (leok == true) {
            ...
            if (err == null) {
              ...
              checkResolveAll(obj.config.letsencrypt.names.split(','), function (err) {
                if (err == null) {
                  ...
                  for (var i in err) {
                    addServerWarning("Invalid Let's Encrypt names, unable to resolve: " + err[i], 14, [err[i]]);
                  }
              ...
              addServerWarning(
                "Invalid Let's Encrypt email address, unable to resolve: " + obj.config.letsencrypt.email.split('@')[1],
                15,
                [obj.config.letsencrypt.email.split('@')[1]]
              );
              obj.StartEx3(certs); // Let's Encrypt did not load, just use the configured certificates
            }
          });
  ...
  obj.StartEx3 = function (certs) {
    ...
    for (var i in obj.config.domains) {
      ...
      if (obj.config.domains[i].amtacmactivation == null) {
        obj.config.domains[i].amtacmactivation = {};
      }
      obj.certificateOperations.loadIntelAmtAcmCerts(obj.config.domains[i].amtacmactivation);
      if (obj.config.domains[i].amtacmactivation.acmCertErrors != null) {
        for (var j in obj.config.domains[i].amtacmactivation.acmCertErrors) {
          obj.addServerWarning(obj.config.domains[i].amtacmactivation.acmCertErrors[j]);
        }
      }
      if (typeof obj.config.domains[i].certurl == 'string') {
        ...
        if (obj.config.domains[i].certurl.indexOf('://') < 0) {
          obj.config.domains[i].certurl = 'https://' + obj.config.domains[i].certurl;
        }
      }
    }
    ...
    if (
      obj.config.settings.trustedproxy != null &&
      typeof obj.config.settings.trustedproxy == 'string' &&
      obj.config.settings.trustedproxy.toLowerCase() == 'cloudflare'
    ) {
      ...
      obj.certificateOperations.loadTextFile('https://www.cloudflare.com/ips-v4', null, function (url, data, tag) {
        if (data != null) {
          if (Array.isArray(obj.args.trustedproxy) == false) {
            obj.args.trustedproxy = [];
          }
          const ipranges = data.split('\n');
          for (var i in ipranges) {
            if (ipranges[i] != '') {
              obj.args.trustedproxy.push(ipranges[i]);
            }
          }
  ...
  obj.StartEx4 = function () {
    ...
    if (obj.certificates.CommonName.indexOf('.') == -1) {
      /*console.log('Server name not configured, running in LAN-only mode.');*/ obj.args.lanonly = true;
    }
    ...
    console.log(
      'MeshCentral v' +
        getCurrentVersion() +
        ', ' +
        ['Hybrid (LAN + WAN) mode', 'WAN mode', 'LAN mode'][runmode] +
    ...
    for (i in obj.config.domains) {
      if (
        obj.config.domains[i].dns != null &&
        obj.certificates.CommonName.toLowerCase() === obj.config.domains[i].dns.toLowerCase()
      ) {
        console.log("ERROR: Server sub-domain can't have same DNS name as the parent.");
        process.exit(0);
        return;
      }
    }
    ...
    try {
      ...
      if (obj.fs.existsSync(translationpath2)) {
        translationpath = translationpath2;
      } // If the agent is present in "meshcentral-data/agents", use that one instead.
      var translations = JSON.parse(obj.fs.readFileSync(translationpath).toString());
      if (translations['zh-chs']) {
        translations['zh-hans'] = translations['zh-chs'];
        delete translations['zh-chs'];
      }
      ...
      for (var i in obj.config.domains) {
        ...
        if (
          typeof obj.config.domains[i].agentcustomization == 'object' &&
          typeof obj.config.domains[i].agentcustomization.installtext == 'string'
        ) {
          domainTranslations = Object.assign({}, domainTranslations); // Shallow clone
          for (var j in domainTranslations) {
            delete domainTranslations[j].description;
          }
    ...
    for (var i in obj.config.domains) {
      if (i != '' && obj.config.domains[i].share == null) {
        obj.updateMeshAgentsTable(obj.config.domains[i], function () {});
      }
    }

    // Load the list of mesh agents and install scripts
    if (obj.args.noagentupdate == 1 || obj.args.noagentupdate == true) {
      for (i in obj.meshAgentsArchitectureNumbers) {
        obj.meshAgentsArchitectureNumbers[i].update = false;
      }
    }
      ...
      obj.updateMeshAgentsTable(obj.config.domains[''], function () {
        ...
        obj.crypto.randomBytes(48, function (err, buf) {
          ...
          if (obj.multiServer != null) {
            if (obj.db.databaseType != 3 || obj.db.changeStream != true) {
              console.log(
                'ERROR: Multi-server support requires use of MongoDB with ReplicaSet and ChangeStream enabled.'
              );
              process.exit(0);
              return;
            }
            if (typeof obj.args.sessionkey != 'string') {
              console.log(
                'ERROR: Multi-server support requires "SessionKey" be set in the settings section of config.json, same key for all servers.'
              );
              process.exit(0);
              return;
            }
            obj.serverId = obj.multiServer.serverid;
            for (var serverid in obj.config.peers.servers) {
              obj.peerConnectivityByNode[serverid] = {};
            }
          }

          // If the server is set to "nousers", allow only loopback unless IP filter is set
          if (obj.args.nousers == true && obj.args.userallowedip == null) {
            obj.args.userallowedip = '::1,127.0.0.1';
          }

          // Set the session length to 60 minutes if not set and set a random key if needed
          if (obj.args.sessiontime != null && (typeof obj.args.sessiontime != 'number' || obj.args.sessiontime < 1)) {
            delete obj.args.sessiontime;
          }
          if (typeof obj.args.sessionkey != 'string') {
            obj.args.sessionkey = buf.toString('hex').toUpperCase();
          }

          // Create MQTT Broker to hook into webserver and mpsserver
          if (
            typeof obj.config.settings.mqtt == 'object' &&
            typeof obj.config.settings.mqtt.auth == 'object' &&
            typeof obj.config.settings.mqtt.auth.keyid == 'string' &&
            typeof obj.config.settings.mqtt.auth.key == 'string'
          ) {
          ...
          obj.webserver = require('./webserver.js').CreateWebServer(
            obj,
            obj.db,
            obj.args,
            obj.certificates,
          ...
          if (obj.redirserver != null) {
            obj.redirserver.hookMainWebServer(obj.certificates);
          }

          // Change RelayDNS to a array of strings
          if (typeof obj.args.relaydns == 'string') {
            obj.args.relaydns = [obj.args.relaydns];
          }
          if (obj.common.validateStrArray(obj.args.relaydns, 1) == false) {
            delete obj.args.relaydns;
          }

          // Start the HTTP relay web server if needed
          if (obj.args.relaydns == null && typeof obj.args.relayport == 'number' && obj.args.relayport != 0) {
            obj.webrelayserver = require('./webrelayserver.js').CreateWebRelayServer(
              obj,
              obj.db,
              obj.args,
              obj.certificates,
          ...
          if (obj.supportsProxyCertificatesRequest == true) {
            obj.updateProxyCertificates(true);
          }
          ...
          if (obj.config.sendgrid != null) {
            ...
            if (obj.args.lanonly == true) {
              addServerWarning('SendGrid server has limited use in LAN mode.', 17);
            }
            ...
            obj.mailserver.verify();
            if (obj.args.lanonly == true) {
              addServerWarning('SMTP server has limited use in LAN mode.', 18);
            }
          ...
          for (i in obj.config.domains) {
            if (obj.config.domains[i].sendgrid != null) {
              ...
              obj.config.domains[i].smtp != null &&
              obj.config.domains[i].smtp.host != null &&
              obj.config.domains[i].smtp.from != null
            ) {
              // SMTP server
              obj.config.domains[i].mailserver = require('./meshmail.js').CreateMeshMail(obj, obj.config.domains[i]);
              obj.config.domains[i].mailserver.verify();
              ...
              if (obj.mailserver != null) {
                obj.config.domains[i].mailserver = obj.mailserver;
              }
            }
          }
          ...
          if (config.sms != null) {
            ...
            if (obj.smsserver != null && obj.args.lanonly == true) {
              addServerWarning('SMS gateway has limited use in LAN mode.', 19);
            }
          }
          ...
          if (typeof config.settings.webpush == 'object' && typeof config.settings.webpush.email == 'string') {
            ...
            var vapidKeys = null;
            try {
              vapidKeys = JSON.parse(obj.fs.readFileSync(obj.path.join(obj.datapath, 'vapid.json')).toString());
            } catch (ex) {}
            if (
              vapidKeys == null ||
            ...
            obj.webpush.setVapidDetails(
              'mailto:' + config.settings.webpush.email,
              vapidKeys.publicKey,
              vapidKeys.privateKey
            );
            if (typeof config.settings.webpush.gcmapi == 'string') {
              webpush.setGCMAPIKey(config.settings.webpush.gcmapi);
            }
          }
          ...
          if (
            ...
            typeof config.firebase.senderid == 'string' &&
            typeof config.firebase.serverkey == 'string'
          ) {
            ...
            try {
              serviceAccount = JSON.parse(
                obj.fs.readFileSync(obj.path.join(obj.datapath, config.firebase.serviceaccountfile)).toString()
              );
            } catch (ex) {
              console.log(ex);
            }
            if (serviceAccount != null) {
              obj.firebase = require('./firebase').CreateFirebase(obj, serviceAccount);
            }
          } else if (typeof config.firebaserelay == 'object' && typeof config.firebaserelay.url == 'string') {
            // Setup the push messaging relay
            obj.firebase = require('./firebase').CreateFirebaseRelay(
              obj,
              config.firebaserelay.url,
              config.firebaserelay.key
            );
            ...
            obj.firebase = require('./firebase').CreateFirebaseRelay(
              obj,
              'https://alt.meshcentral.com/firebaserelay.aspx'
            );
          }

          // Setup monitoring
          if (obj.config.settings.prometheus != null) {
          ...
          if (obj.config && obj.config.settings && typeof obj.config.settings.logincookieencryptionkey == 'string') {
            ...
            try {
              obj.loginCookieEncryptionKey = Buffer.from(obj.config.settings.logincookieencryptionkey, 'hex');
            } catch (ex) {}
            if (obj.loginCookieEncryptionKey == null || obj.loginCookieEncryptionKey.length != 80) {
              addServerWarning('Invalid "LoginCookieEncryptionKey" in config.json.', 20);
              obj.loginCookieEncryptionKey = null;
            }
          }
          ...
          if (obj.loginCookieEncryptionKey == null) {
            obj.db.Get('LoginCookieEncryptionKey', function (err, docs) {
              if (
                ...
                docs.length > 0 &&
                docs[0].key != null &&
                obj.args.logintokengen == null &&
                docs[0].key.length >= 160
              ) {
                obj.loginCookieEncryptionKey = Buffer.from(docs[0].key, 'hex');
              } else {
                obj.loginCookieEncryptionKey = obj.generateCookieKey();
                obj.db.Set({
                  _id: 'LoginCookieEncryptionKey',
                  key: obj.loginCookieEncryptionKey.toString('hex'),
                  time: Date.now()
                });
              }
            });
          ...
          obj.db.Get('InvitationLinkEncryptionKey', function (err, docs) {
            if (docs != null && docs.length > 0 && docs[0].key != null && docs[0].key.length >= 160) {
              ...
              obj.invitationLinkEncryptionKey = obj.generateCookieKey();
              obj.db.Set({
                _id: 'InvitationLinkEncryptionKey',
                key: obj.invitationLinkEncryptionKey.toString('hex'),
                time: Date.now()
              });
            }
          });
          ...
          if (
            typeof config.settings.amtprovisioningserver == 'object' &&
            typeof config.settings.amtprovisioningserver.devicegroup == 'string' &&
            typeof config.settings.amtprovisioningserver.newmebxpassword == 'string' &&
            typeof config.settings.amtprovisioningserver.trustedfqdn == 'string' &&
            typeof config.settings.amtprovisioningserver.ip == 'string'
          ) {
            obj.amtProvisioningServer = require('./amtprovisioningserver').CreateAmtProvisioningServer(
              obj,
              config.settings.amtprovisioningserver
            );
          }
          ...
          setInterval(function () {
            ...
            if (obj.serverStatsCounter % 2 == 1) {
              hours = 3;
            } // Half of the event get removed after 3 hours.
            else if (Math.floor(obj.serverStatsCounter / 2) % 2 == 1) {
              hours = 8;
            } // Another half of the event get removed after 8 hours.
            else if (Math.floor(obj.serverStatsCounter / 4) % 2 == 1) {
              hours = 24;
            } // Another half of the event get removed after 24 hours.
            else if (Math.floor(obj.serverStatsCounter / 8) % 2 == 1) {
              hours = 48;
            } // Another half of the event get removed after 48 hours.
            else if (Math.floor(obj.serverStatsCounter / 16) % 2 == 1) {
              hours = 72;
            } // Another half of the event get removed after 72 hours.
            ...
            try {
              data.cpu = require('os').loadavg();
            } catch (ex) {}
            if (obj.mpsserver != null) {
              data.conn.amc = 0;
              for (var i in obj.mpsserver.ciraConnections) {
                data.conn.amc += obj.mpsserver.ciraConnections[i].length;
              }
            }
            ...
            if (obj.firstStats === true) {
              delete obj.firstStats;
              data.first = true;
            }
            if (obj.multiServer != null) {
              data.s = obj.multiServer.serverid;
            }
          ...
          if (obj.args.nousers == true) {
            obj.updateServerState('nousers', '1');
          }
          obj.updateServerState('state', 'running');

          // Setup auto-backup defaults. Unless autobackup is set to false try to make a backup.
          if (obj.config.settings.autobackup == false || obj.config.settings.autobackup == 'false') {
            obj.config.settings.autobackup = { backupintervalhours: -1 };
          } //block all autobackup functions
          else {
            if (typeof obj.config.settings.autobackup != 'object') {
              obj.config.settings.autobackup = {};
            }
            if (typeof obj.config.settings.autobackup.backupintervalhours != 'number') {
              obj.config.settings.autobackup.backupintervalhours = 24;
            }
            if (typeof obj.config.settings.autobackup.keeplastdaysbackup != 'number') {
              obj.config.settings.autobackup.keeplastdaysbackup = 10;
            }
            if (obj.config.settings.autobackup.backuphour != null) {
              obj.config.settings.autobackup.backupintervalhours = 24;
              if (
                typeof obj.config.settings.autobackup.backuphour != 'number' ||
                obj.config.settings.autobackup.backuphour > 23 ||
                obj.config.settings.autobackup.backuphour < 0
              ) {
                obj.config.settings.autobackup.backuphour = 0;
              }
            } else {
              obj.config.settings.autobackup.backuphour = -1;
            }
            ...
            if (!obj.config.settings.autobackup.backupignorefilesglob) {
              obj.config.settings.autobackup.backupignorefilesglob = [];
            } else if (typeof obj.config.settings.autobackup.backupignorefilesglob == 'string') {
              obj.config.settings.autobackup.backupignorefilesglob =
                obj.config.settings.autobackup.backupignorefilesglob.replaceAll(', ', ',').split(',');
            }
            if (!obj.config.settings.autobackup.backupskipfoldersglob) {
              obj.config.settings.autobackup.backupskipfoldersglob = [];
            } else if (typeof obj.config.settings.autobackup.backupskipfoldersglob == 'string') {
              obj.config.settings.autobackup.backupskipfoldersglob =
                obj.config.settings.autobackup.backupskipfoldersglob.replaceAll(', ', ',').split(',');
            }
            if (typeof obj.config.settings.autobackup.backuppath == 'string') {
              obj.backuppath = obj.config.settings.autobackup.backuppath = obj.path.resolve(
                obj.config.settings.autobackup.backuppath
              );
            } else {
              obj.config.settings.autobackup.backuppath = obj.backuppath;
            }
            if (typeof obj.config.settings.autobackup.backupname != 'string') {
              obj.config.settings.autobackup.backupname = 'meshcentral-autobackup-';
            }
            if (typeof obj.config.settings.autobackup.webdav == 'object') {
              ...
              if (typeof obj.config.settings.autobackup.webdav.foldername != 'string') {
                obj.config.settings.autobackup.webdav.foldername = '/MeshCentral-Backups/';
              } else {
                obj.config.settings.autobackup.webdav.foldername = (
                  '/' +
                  obj.config.settings.autobackup.webdav.foldername +
                  '/'
                )
          ...
          // Check if the database is capable of performing a backup
          obj.db.checkBackupCapability(function (err, msg) {
            if (msg != null) {
              obj.addServerWarning(msg, true);
            }
          });
          ...
          if (typeof obj.config.settings.managealldevicegroups == 'string') {
            obj.config.settings.managealldevicegroups = obj.config.settings.managealldevicegroups.split(',');
          } else if (Array.isArray(obj.config.settings.managealldevicegroups) == false) {
            ...
            if (Array.isArray(obj.config.domains[i].managealldevicegroups)) {
              for (var j in obj.config.domains[i].managealldevicegroups) {
                if (typeof obj.config.domains[i].managealldevicegroups[j] == 'string') {
                  ...
                  if (obj.config.settings.managealldevicegroups.indexOf(u) == -1) {
                    obj.config.settings.managealldevicegroups.push(u);
                  }
                }
              }
            }
          ...
          if (
            typeof config.settings.watchdog == 'object' &&
            typeof config.settings.watchdog.interval == 'number' &&
            typeof config.settings.watchdog.timeout == 'number' &&
            config.settings.watchdog.interval >= 50 &&
            config.settings.watchdog.timeout >= 50
          ) {
            ...
            obj.watchdog = setInterval(function () {
              const now = Date.now(),
                delta = now - obj.watchdogtime - config.settings.watchdog.interval;
              if (delta > obj.watchdogmax) {
                obj.watchdogmax = delta;
                obj.watchdogmaxtime = new Date().toLocaleString();
              }
              if (delta > config.settings.watchdog.timeout) {
                ...
                while (obj.watchdogtable.length > 10) {
                  obj.watchdogtable.shift();
                }
                obj.debug('main', msg);
                try {
                  var errlogpath = null;
  ...
  obj.StartEx5 = function () {
    ...
    for (var i in obj.config.domains) {
      if (obj.config.domains[i].ipkvm == true) {
        ipKvmSupport = true;
      }
    }
    if (ipKvmSupport) {
      obj.ipKvmManager = require('./meshipkvm').CreateIPKVMManager(obj);
    }
    ...
    if (typeof obj.config.settings.runonserverstarted == 'string') {
      ...
      child_process.exec(
        obj.config.settings.runonserverstarted + ' ' + getCurrentVersion(),
        { maxBuffer: 512000, timeout: 120000, cwd: parentpath },
        function (error, stdout, stderr) {}
      );
    }
  ...
  obj.updateProxyCertificates = function (force) {
    ...
    for (var i in obj.config.domains) {
      if (obj.config.domains[i].certurl != null) {
        ...
        if (dnsname == null && obj.config.settings.cert != null) {
          dnsname = obj.config.settings.cert;
        }
        obj.certificateOperations.loadCertificate(
          obj.config.domains[i].certurl,
          dnsname,
          obj.config.domains[i],
          function (url, cert, xhostname, xdomain) {
            obj.pendingProxyCertificatesRequests--;
            if (cert != null) {
              ...
              if (xdomain.certhash != hash) {
                ...
                try {
                  ...
                  const forgeCert = obj.certificateOperations.forge.pki.certificateFromAsn1(
                    obj.certificateOperations.forge.asn1.fromDer(cert)
                  );
                  xdomain.certkeyhash = obj.certificateOperations.forge.pki.getPublicKeyFingerprint(
                    forgeCert.publicKey,
                    { md: obj.certificateOperations.forge.md.sha384.create(), encoding: 'hex' }
                  );
                ...
                if (obj.webserver) {
                  obj.webserver.webCertificateHashs[xdomain.id] = obj.webserver.webCertificateFullHashs[xdomain.id] =
                    Buffer.from(hash, 'hex').toString('binary');
                  if (xdomain.certkeyhash != null) {
                    obj.webserver.webCertificateHashs[xdomain.id] = Buffer.from(xdomain.certkeyhash, 'hex').toString(
                      'binary'
                    );
                  }

                  // Disconnect all agents with bad web certificates
                  for (var i in obj.webserver.wsagentsWithBadWebCerts) {
                    obj.webserver.wsagentsWithBadWebCerts[i].close(1);
                  }
                }
                ...
                if (xdomain.certkeyhash != null && xdomain.certhash != xdomain.certkeyhash) {
                  console.log(obj.common.format('  SHA384 key hash: {0}', xdomain.certkeyhash));
                }
              }
            } else {
  ...
  obj.maintenanceActions = function () {
    ...
    if (typeof obj.args.selfupdate == 'string' && getCurrentVersion() === obj.args.selfupdate) {
      obj.args.selfupdate = false;
    }
    ...
    if (obj.args.selfupdate && obj.serverSelfWriteAllowed == true) {
      obj.db.getValueOfTheDay('performSelfUpdate', 1, function (performSelfUpdate) {
        if (performSelfUpdate.value > 0) {
          ...
          obj.getLatestServerVersion(function (currentVer, latestVer) {
  ...
  function checkAutobackup() {
    if (obj.config.settings.autobackup.backupintervalhours >= 1) {
      obj.db.Get('LastAutoBackupTime', function (err, docs) {
        if (err != null) {
          console.error('checkAutobackup: Error getting LastBackupTime from DB');
          return;
        }
        var lastBackup = 0;
        ...
        if (docs.length == 1) {
          lastBackup = docs[0].value;
        }
        const delta = now - lastBackup;
        //const delta = 9999999999; // DEBUG: backup always
        obj.debug(
          'backup',
          'Entering checkAutobackup, lastAutoBackupTime: ' +
            new Date(lastBackup).toLocaleString('default', { dateStyle: 'medium', timeStyle: 'short' }) +
            ', delta: ' +
            (delta / (1000 * 60 * 60)).toFixed(2) +
            ' hours'
        );
        ...
        if (
          delta > obj.config.settings.autobackup.backupintervalhours * 60 * 60 * 1000 ||
          (currentHour == obj.config.settings.autobackup.backuphour && delta >= 2 * 60 * 60 * 1000)
        ) {
  ...
  obj.Stop = function (restoreFile) {
    ...
    // Set all nodes to power state of unknown (0)
    obj.db.storePowerEvent({ time: new Date(), nodeid: '*', power: 0, s: 2 }, obj.multiServer, function () {
      // s:2 indicates that the server is shutting down.
      if (restoreFile) {
            ...
            if (/\/$/.test(entry.fileName)) {
              ...
              zipfile.openReadStream(entry, function (err, readStream) {
                ...
                if (directory != '.') {
                  ...
                  if (obj.fs.existsSync(directory) == false) {
                    obj.fs.mkdirSync(directory);
                  }
                }
  ...
  obj.AddEventDispatch = function (ids, target) {
    ...
    for (var i in ids) {
      ...
      if (!obj.eventsDispatch[id]) {
        obj.eventsDispatch[id] = [target];
      } else {
        obj.eventsDispatch[id].push(target);
      }
    }
  };
      ...
      if (obj.eventsDispatch[id]) {
        ...
        if (j >= 0) {
          if (obj.eventsDispatch[id].length == 1) {
            ...
            for (var k in obj.eventsDispatch[i]) {
              if (obj.eventsDispatch[i][k] != target) {
                newList.push(obj.eventsDispatch[i][k]);
              }
            }
  ...
  obj.DispatchEvent = function (ids, source, event, fromPeerServer) {
    ...
    if (obj.syslog && event.msg) {
      obj.syslog.log(obj.syslog.LOG_INFO, event.msg);
    }
    if (obj.syslogjson) {
      obj.syslogjson.log(obj.syslogjson.LOG_INFO, JSON.stringify(event));
    }
    if (obj.syslogtcp && event.msg) {
      obj.syslogtcp.log(event.msg, obj.syslogtcp.LOG_INFO);
    }
    ...
    if (typeof event == 'object' && !event.nolog) {
      ...
      if (storeEvent.node) {
        delete storeEvent.node;
      } // Skip the "node" field. May skip more in the future.
      if (storeEvent.links) {
        // Escape "links" names that may have "." and/or "$"
        storeEvent.links = Object.assign({}, storeEvent.links);
        for (var i in storeEvent.links) {
          var ue = obj.common.escapeFieldName(i);
          if (ue !== i) {
            storeEvent.links[ue] = storeEvent.links[i];
            delete storeEvent.links[i];
          }
        }
      }
      if (storeEvent.mesh) {
    ...
    for (var j in ids) {
      ...
      if (eventsDispatch) {
        for (var i in eventsDispatch) {
          if (targets.indexOf(eventsDispatch[i]) == -1) {
            // Check if we already displatched to this target
            targets.push(eventsDispatch[i]);
            try {
              eventsDispatch[i].HandleEvent(source, event, ids, id);
            } catch (ex) {
              console.log(ex, eventsDispatch[i]);
            }
          }
        }
      }
    }
    if (fromPeerServer == null && obj.multiServer != null && (typeof event != 'object' || event.nopeers != 1)) {
      obj.multiServer.DispatchEvent(ids, source, event);
    }
  };

  // Get the connection state of a node
  obj.GetConnectivityState = function (nodeid) {
    return obj.connectivityByNode[nodeid];
  };
  ...
  obj.GetRoutingServerIdNotSelf = function (nodeid, connectType) {
    ...
    for (var serverid in obj.peerConnectivityByNode) {
      ...
      if (state != null && (state.connectivity & connectType) != 0) {
        return { serverid: serverid, meshid: state.meshid };
      }
    }
    return null;
  };
  ...
  obj.GetRoutingServerId = function (nodeid, connectType) {
    ...
    if (connections != null) {
      ...
      if (state != null && (state.connectivity & connectType) != 0) {
        return { serverid: obj.serverId, meshid: state.meshid };
      }
    }
  ...
  obj.UpdateConnectivityState = function (nodeids) {
    for (var nodeid in nodeids) {
      ...
      if (oldState != null) {
        meshid = oldState.meshid;
        oldConnectivity = oldState.connectivity;
        oldPowerState = oldState.powerState;
      }
      for (var serverid in obj.peerConnectivityByNode) {
        ...
        if (peerState != null) {
          if (state == null) {
            ...
            if (peerState.powerState != 0 && (state.powerState == 0 || peerState.powerState < state.powerState)) {
              newPowerState = state.powerState = peerState.powerState;
            }
            meshid = state.meshid = peerState.meshid;
            //if (peerState.agentPower) { state.agentPower = peerState.agentPower; }
      ...
      if (newConnectivity != oldPowerState || newPowerState != oldPowerState) {
        obj.DispatchEvent(obj.webserver.CreateNodeDispatchTargets(meshid, nodeid), obj, {
          action: 'nodeconnect',
          meshid: meshid,
          nodeid: nodeid,
          domain: nodeid.split('/')[1],
  ...
  obj.NotifyUserOfDeviceStateChange = function (
    meshid,
    nodeid,
    connectTime,
    connectType,
    ...
    for (var i in mesh.links) {
      if (i.startsWith('user/') && users.indexOf(i) < 0) {
        users.push(i);
      }
      if (i.startsWith('ugrp/')) {
        var usergrp = obj.webserver.userGroups[i];
        if (usergrp.links != null) {
          for (var j in usergrp.links) {
            if (j.startsWith('user/') && users.indexOf(j) < 0) {
              users.push(j);
            }
          }
        }
    ...
    for (var i in users) {
      ...
      if (user != null) {
        ...
        if (meshLinks != null && meshLinks.notify != null) {
          notify |= meshLinks.notify;
        }

        // User notifications
        if (user.notify != null) {
          if (user.notify[meshid] != null) {
            notify |= user.notify[meshid];
          }
          if (user.notify[nodeid] != null) {
            notify |= user.notify[nodeid];
          }
        }
        ...
        if (user.email != null && user.emailVerified == true && mailserver != null && (notify & 48) != 0) {
          if (stateSet == true) {
            if ((notify & 16) != 0) {
              mailserver.notifyDeviceConnect(
                user,
                meshid,
                nodeid,
                connectTime,
              ...
              mailserver.cancelNotifyDeviceDisconnect(
                user,
                meshid,
                nodeid,
                connectTime,
            ...
            if ((notify & 32) != 0) {
              mailserver.notifyDeviceDisconnect(
                user,
                meshid,
                nodeid,
                connectTime,
              ...
              mailserver.cancelNotifyDeviceConnect(
                user,
                meshid,
                nodeid,
                connectTime,
        ...
        if (obj.msgserver != null && (notify & 384) != 0) {
          if (stateSet == true) {
            if ((notify & 128) != 0) {
              obj.msgserver.notifyDeviceConnect(
                user,
                meshid,
                nodeid,
                connectTime,
              ...
              obj.msgserver.cancelNotifyDeviceDisconnect(
                user,
                meshid,
                nodeid,
                connectTime,
            ...
            if ((notify & 256) != 0) {
              obj.msgserver.notifyDeviceDisconnect(
                user,
                meshid,
                nodeid,
                connectTime,
              ...
              obj.msgserver.cancelNotifyDeviceConnect(
                user,
                meshid,
                nodeid,
                connectTime,
  ...
  obj.NotifyUserOfDeviceHelpRequest = function (domain, meshid, nodeid, devicename, helpusername, helprequest) {
    ...
    for (var i in users) {
      ...
      if (user != null) {
        ...
        // Mail help request
        if (user.email != null && user.emailVerified == true && (notify & 64) != 0) {
          mailserver.sendDeviceHelpMail(
            domain,
            user.name,
            user.email,
            devicename,
        ...
        if (user.msghandle != null && (notify & 512) != 0) {
          obj.msgserver.sendDeviceHelpRequest(
            domain,
            user.name,
            user.msghandle,
            devicename,
  ...
  obj.SetConnectivityState = function (meshid, nodeid, connectTime, connectType, powerState, serverid, extraInfo) {
    ...
    if (serverid == null && obj.multiServer != null) {
      obj.multiServer.DispatchMessage({
        action: 'SetConnectivityState',
        meshid: meshid,
        nodeid: nodeid,
    ...
    if (obj.multiServer == null) {
      ...
      if (connectType == 1) {
        state.agentPower = powerState;
      } else if (connectType == 2) {
        state.ciraPower = powerState;
      } else if (connectType == 4) {
        state.amtPower = powerState;
      }
      var powerState = 0,
        oldPowerState = state.powerState;
      if ((state.connectivity & 1) != 0) {
        powerState = state.agentPower;
      } else if ((state.connectivity & 2) != 0) {
        powerState = state.ciraPower;
      } else if ((state.connectivity & 4) != 0) {
        powerState = state.amtPower;
      }
      ...
      if (eventConnectChange == 1) {
        ...
        if (extraInfo && extraInfo.remoteaddrport) {
          lc.addr = extraInfo.remoteaddrport;
        }
        obj.db.Set(lc);

        // Notify any users of device connection
        obj.NotifyUserOfDeviceStateChange(
          meshid,
          nodeid,
          connectTime,
          connectType,
      ...
      if (state.powerState == null || state.powerState == undefined || state.powerState != powerState) {
        ...
        // Set new power state in database
        var record = {
          time: new Date(connectTime),
          nodeid: nodeid,
          power: powerState,
          server: obj.multiServer.serverid
        };
        if (oldPowerState != null) {
          record.oldPower = oldPowerState;
        }
  ...
  obj.ClearConnectivityState = function (meshid, nodeid, connectType, serverid, extraInfo) {
    ...
    if (obj.multiServer == null) {
      ...
      if (state.powerState == null || state.powerState != powerState) {
        ...
        // Set new power state in database
        obj.db.storePowerEvent(
          { time: new Date(), nodeid: nodeid, power: powerState, oldPower: oldPowerState },
          obj.multiServer
        );
      }
      ...
      if ((state.connectivity & connectType) != 0) {
        ...
        // If the node is completely disconnected, clean it up completely
        if (state.connectivity == 0) {
          delete obj.peerConnectivityByNode[serverid][nodeid];
          state.powerState = 0;
        }
  ...
  obj.updateMeshCore = function (func, dumpToFile) {
    ...
    if (obj.fs.existsSync(obj.path.join(meshcorePath, 'meshcore.js')) == false) {
      ...
      if (obj.fs.existsSync(obj.path.join(meshcorePath, 'meshcore.js')) == false) {
        obj.defaultMeshCores = obj.defaultMeshCoresHash = {};
        if (func != null) {
          func(false);
        } // meshcore.js not found
      }
    }
    ...
    if (obj.fs.existsSync(obj.path.join(__dirname, 'agents', 'recoverycore.js')) == true) {
      try {
        meshRecoveryCore = obj.fs.readFileSync(obj.path.join(__dirname, 'agents', 'recoverycore.js')).toString();
      } catch (ex) {}
    ...
    if (obj.fs.existsSync(obj.path.join(__dirname, 'agents', 'meshcore_diagnostic.js')) == true) {
      try {
        meshAgentRecoveryCore = obj.fs
          .readFileSync(obj.path.join(__dirname, 'agents', 'meshcore_diagnostic.js'))
          .toString();
      } catch (ex) {}
    ...
    if (obj.fs.existsSync(obj.path.join(__dirname, 'agents', 'tinycore.js')) == true) {
      try {
        meshTinyCore = obj.fs.readFileSync(obj.path.join(__dirname, 'agents', 'tinycore.js')).toString();
      } catch (ex) {}
    ...
    if (obj.args.minifycore !== false) {
      try {
        meshCore = obj.fs.readFileSync(obj.path.join(meshcorePath, 'meshcore.min.js')).toString();
      } catch (ex) {}
    } // Favor minified meshcore if present.
    if (meshCore == null) {
      try {
        meshCore = obj.fs.readFileSync(obj.path.join(meshcorePath, 'meshcore.js')).toString();
      } catch (ex) {}
    } // Use non-minified meshcore.
    if (meshCore != null) {
      var moduleDirPath = null;
      if (obj.args.minifycore !== false) {
        try {
          moduleDirPath = obj.path.join(meshcorePath, 'modules_meshcore_min');
          modulesDir = obj.fs.readdirSync(moduleDirPath);
        } catch (ex) {}
      } // Favor minified modules if present.
      if (modulesDir == null) {
        try {
          moduleDirPath = obj.path.join(meshcorePath, 'modules_meshcore');
          modulesDir = obj.fs.readdirSync(moduleDirPath);
        } catch (ex) {}
      } // Use non-minified mofules.
      if (modulesDir != null) {
        for (var i in modulesDir) {
          if (modulesDir[i].toLowerCase().endsWith('.json')) {
            ...
            } // Remove the ".min" for ".min.json" files.
            const jsonData = obj.escapeCodeString(
              obj.fs.readFileSync(obj.path.join(moduleDirPath, modulesDir[i])).toString('utf8'),
              true
            );
          ...
          if (modulesDir[i].toLowerCase().endsWith('.js')) {
            ...
            } // Remove the ".min" for ".min.js" files.
            const moduleData = [
              'try { addModule("',
              moduleName,
              '", "',
              obj.escapeCodeString(obj.fs.readFileSync(obj.path.join(moduleDirPath, modulesDir[i])).toString('binary')),
              '"); addedModules.push("',
              moduleName,
              '"); } catch (ex) { }\r\n'
            ];
            ...
            if (modulesAdd['windows-recovery'] != null) {
              if (
                moduleName == 'win-console' ||
                moduleName == 'win-message-pump' ||
                moduleName == 'win-terminal' ||
                moduleName == 'win-virtual-terminal'
              ) {
      ...
      for (var i in modulesAdd) {
        ...
        if (dumpToFile) {
          console.log(
            'Core module ' + i + ' is ' + obj.defaultMeshCores[i].length + ' bytes, saving to meshcores/' + i + '.js.'
          ); // Print the core size and filename
        ...
        const callback = function MeshCoreDeflateCb(err, buffer) {
          if (err == null) {
            obj.defaultMeshCoresDeflate[MeshCoreDeflateCb.i] = buffer;
          }
        };
        callback.i = i;
  ...
  obj.updateMeshCmd = function (func) {
    ...
    var meshCmd = null,
      ...
      moduleDirPath,
      modulesDir = null;
    if (obj.args.minifycore !== false && obj.fs.existsSync(obj.path.join(obj.datapath, 'meshcmd.min.js'))) {
      meshcmdPath = obj.path.join(obj.datapath, 'meshcmd.min.js');
      meshCmd = obj.fs.readFileSync(meshcmdPath).toString();
    } else if (obj.fs.existsSync(obj.path.join(obj.datapath, 'meshcmd.js'))) {
      meshcmdPath = obj.path.join(obj.datapath, 'meshcmd.js');
      meshCmd = obj.fs.readFileSync(meshcmdPath).toString();
    } else if (
      obj.args.minifycore !== false &&
      obj.fs.existsSync(obj.path.join(__dirname, 'agents', 'meshcmd.min.js'))
    ) {
      meshcmdPath = obj.path.join(__dirname, 'agents', 'meshcmd.min.js');
      meshCmd = obj.fs.readFileSync(meshcmdPath).toString();
    } else if (obj.fs.existsSync(obj.path.join(__dirname, 'agents', 'meshcmd.js'))) {
      meshcmdPath = obj.path.join(__dirname, 'agents', 'meshcmd.js');
      meshCmd = obj.fs.readFileSync(meshcmdPath).toString();
    } else {
      obj.defaultMeshCmd = null;
      if (func != null) {
      ...
      return;
    } // meshcmd.js not found
    meshCmd = meshCmd.replace("'***Mesh*Cmd*Version***'", "'" + getCurrentVersion() + "'");

    // Figure out where the modules_meshcmd folder is.
    if (obj.args.minifycore !== false) {
      try {
        moduleDirPath = obj.path.join(meshcmdPath, 'modules_meshcmd_min');
        modulesDir = obj.fs.readdirSync(moduleDirPath);
      } catch (ex) {}
    } // Favor minified modules if present.
    if (modulesDir == null) {
      try {
        moduleDirPath = obj.path.join(meshcmdPath, 'modules_meshcmd');
        modulesDir = obj.fs.readdirSync(moduleDirPath);
      } catch (ex) {}
    } // Use non-minified mofules.
    if (obj.args.minifycore !== false) {
      if (modulesDir == null) {
        try {
          moduleDirPath = obj.path.join(__dirname, 'agents', 'modules_meshcmd_min');
          modulesDir = obj.fs.readdirSync(moduleDirPath);
        } catch (ex) {}
      }
    } // Favor minified modules if present.
    if (modulesDir == null) {
      try {
        moduleDirPath = obj.path.join(__dirname, 'agents', 'modules_meshcmd');
        modulesDir = obj.fs.readdirSync(moduleDirPath);
      } catch (ex) {}
    } // Use non-minified mofules.
    ...
    if (obj.updateMeshCmdTimer === 'notset') {
      ...
      obj.fs.watch(meshcmdPath, function (eventType, filename) {
        if (obj.updateMeshCmdTimer != null) {
          clearTimeout(obj.updateMeshCmdTimer);
          obj.updateMeshCmdTimer = null;
        }
        obj.updateMeshCmdTimer = setTimeout(function () {
          obj.updateMeshCmd();
        }, 5000);
      });
    }
  ...
  obj.updateMeshTools = function () {
    for (var toolname in meshToolsList) {
      if (meshToolsList[toolname].winhash === true) {
        ...
        if (obj.fs.existsSync(toolpath2)) {
          toolpath = toolpath2;
        } // If the tool is present in "meshcentral-data/agents", use that one instead.
        ...
        hashStream.on('data', function (data) {
          ...
          obj.meshToolsBinaries[this.toolname].url =
            'https://' +
            obj.certificates.CommonName +
            ':' +
            (typeof obj.args.aliasport == 'number' ? obj.args.aliasport : obj.args.port) +
            '/meshagents?meshaction=' +
            this.dlname;
          var stats = null;
          try {
            stats = obj.fs.statSync(this.toolpath);
          } catch (ex) {}
          if (stats != null) {
            obj.meshToolsBinaries[this.toolname].size = stats.size;
          }
        });
        ...
        try {
          ...
          stream.on('data', function (data) {
            ...
            this.hashx += data.length;
          });
          stream.on('error', function (data) {
            // If there is an error reading this file, make sure this agent is not in the agent table
            if (obj.meshToolsBinaries[this.toolname] != null) {
              delete obj.meshToolsBinaries[this.toolname];
            }
          });
          stream.on('end', function () {
            ...
            var stats = null;
            try {
              stats = obj.fs.statSync(this.agentpath);
            } catch (ex) {}
            if (stats != null) {
              obj.meshToolsBinaries[this.toolname].size = stats.size;
            }
  ...
  obj.updateMeshAgentInstallScripts = function () {
    for (var scriptid in meshAgentsInstallScriptList) {
      ...
      try {
        ...
        stream.on('error', function (data) {
          // If there is an error reading this file, make sure this agent is not in the agent table
          if (obj.meshAgentInstallScripts[this.info.id] != null) {
            delete obj.meshAgentInstallScripts[this.info.id];
          }
        });
        stream.on('end', function () {
          // Add the agent to the agent table with all information and the hash
          ...
          obj.meshAgentInstallScripts[this.info.id].url =
            'https://' +
            obj.certificates.CommonName +
            ':' +
            (typeof obj.args.aliasport == 'number' ? obj.args.aliasport : obj.args.port) +
          ...
          if (stats != null) {
            obj.meshAgentInstallScripts[this.info.id].size = stats.size;
          }

          // Place Unit line breaks on Linux scripts if not already present.
          if (obj.meshAgentInstallScripts[this.info.id].linux === true) {
            obj.meshAgentInstallScripts[this.info.id].data = obj.meshAgentInstallScripts[this.info.id].data
              .split('\r\n')
              .join('\n');
          }
        });
  ...
  obj.meshAgentsArchitectureNumbers = {
    0: {
      id: 0,
      localname: 'Unknown',
      rname: 'meshconsole.exe',
      desc: 'Unknown agent',
      update: false,
      amt: true,
      platform: 'unknown',
      core: 'linux-noamt',
      rcore: 'linux-recovery',
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    },
    1: {
      id: 1,
      localname: 'MeshConsole.exe',
      rname: 'meshconsole32.exe',
      desc: 'Windows x86-32 console',
      update: true,
      amt: true,
      platform: 'win32',
      core: 'windows-amt',
      rcore: 'windows-recovery',
      arcore: 'windows-agentrecovery',
      tcore: 'windows-tiny'
    },
    2: {
      id: 2,
      localname: 'MeshConsole64.exe',
      rname: 'meshconsole64.exe',
      desc: 'Windows x86-64 console',
      update: true,
      amt: true,
      platform: 'win32',
      core: 'windows-amt',
    ...
    8: {
      id: 8,
      localname: 'MeshAgent-Linux-XEN-x86-32',
      rname: 'meshagent',
      desc: 'XEN x86-64',
      update: true,
      amt: false,
    ...
    10: {
      id: 10,
      localname: 'MeshAgent-Linux-ARM-PlugPC',
      rname: 'meshagent',
      desc: 'Linux ARM PlugPC',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    11: {
      id: 11,
      localname: 'meshagent_osx-x86-32',
      rname: 'meshosx',
      desc: 'Apple macOS x86-32',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Apple x86-32 binary, no longer supported.
    12: {
      id: 12,
      localname: 'MeshAgent-Android-x86',
      rname: 'meshandroid',
      desc: 'Android x86-32',
      update: true,
      amt: false,
    ...
    13: {
      id: 13,
      localname: 'meshagent_pogo',
      rname: 'meshagent',
      desc: 'Linux ARM PogoPlug',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    14: {
      id: 14,
      localname: 'meshagent_android.apk',
      rname: 'meshandroid.apk',
      desc: 'Android',
      update: false,
      amt: false,
      platform: 'android',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Get this one from Google Play
    15: {
      id: 15,
      localname: 'meshagent_poky',
      rname: 'meshagent',
      desc: 'Linux Poky x86-32',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    16: {
      id: 16,
      localname: 'meshagent_osx-x86-64',
      rname: 'meshagent',
      desc: 'Apple macOS x86-64',
      update: true,
      amt: false,
      platform: 'osx',
      core: 'linux-noamt',
    ...
    17: {
      id: 17,
      localname: 'MeshAgent-ChromeOS',
      rname: 'meshagent',
      desc: 'Google ChromeOS',
      update: false,
      amt: false,
      platform: 'chromeos',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Get this one from Chrome store
    18: {
      id: 18,
      localname: 'meshagent_poky64',
      rname: 'meshagent',
      desc: 'Linux Poky x86-64',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    19: {
      id: 19,
      localname: 'meshagent_x86_nokvm',
      rname: 'meshagent',
      desc: 'Linux x86-32 NoKVM',
      update: true,
      amt: true,
      platform: 'linux',
      core: 'linux-amt',
    ...
    20: {
      id: 20,
      localname: 'meshagent_x86-64_nokvm',
      rname: 'meshagent',
      desc: 'Linux x86-64 NoKVM',
      update: true,
      amt: true,
      platform: 'linux',
      core: 'linux-amt',
    ...
    21: {
      id: 21,
      localname: 'MeshAgent-WinMinCore-Console-x86-32.exe',
      rname: 'meshagent.exe',
      desc: 'Windows MinCore Console x86-32',
      update: true,
      amt: false,
      platform: 'win32',
      core: 'windows-amt',
    ...
    22: {
      id: 22,
      localname: 'MeshAgent-WinMinCore-Service-x86-64.exe',
      rname: 'meshagent.exe',
      desc: 'Windows MinCore Service x86-32',
      update: true,
      amt: false,
      platform: 'win32',
      core: 'windows-amt',
    ...
    24: {
      id: 24,
      localname: 'meshagent_arm-linaro',
      rname: 'meshagent',
      desc: 'Linux ARM Linaro',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    25: {
      id: 25,
      localname: 'meshagent_armhf',
      rname: 'meshagent',
      desc: 'Linux ARM - HardFloat',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    26: {
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // This is replaced by ARCHID 32
    27: {
      id: 27,
      localname: 'meshagent_armhf2',
      rname: 'meshagent',
      desc: 'Linux ARM - HardFloat',
      update: true,
      amt: false,
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Raspbian 7 2015-02-02 for old Raspberry Pi.
    28: {
      id: 28,
      localname: 'meshagent_mips24kc',
      rname: 'meshagent',
      desc: 'Linux MIPS24KC/MUSL (OpenWRT)',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // MIPS Router with OpenWRT
    29: {
      id: 29,
      localname: 'meshagent_osx-arm-64',
      rname: 'meshagent',
      desc: 'Apple macOS ARM-64',
      update: true,
      amt: false,
      platform: 'osx',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Apple Silicon ARM 64bit
    30: {
      id: 30,
      localname: 'meshagent_freebsd_x86-64',
      rname: 'meshagent',
      desc: 'FreeBSD x86-64',
      update: true,
      amt: false,
    ...
    33: {
      id: 33,
      localname: 'meshagent_openwrt_x86_64',
      rname: 'meshagent',
      desc: 'OpenWRT x86-64',
      update: true,
      amt: false,
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // This is replaced with ARCHID 36.
    34: {
      id: 34,
      localname: 'assistant_windows',
      rname: 'meshassistant',
      desc: 'MeshCentral Assistant (Windows)',
      update: false,
      amt: false,
      platform: 'assistant',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // MeshCentral Assistant for Windows
    35: {
      id: 35,
      localname: 'meshagent_linux-armada370-hf',
      rname: 'meshagent',
      desc: 'Armada370 - ARM32/HF (libc/2.26)',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    37: {
      id: 37,
      localname: 'meshagent_openbsd_x86-64',
      rname: 'meshagent',
      desc: 'OpenBSD x86-64',
      update: true,
      amt: false,
    ...
    40: {
      id: 40,
      localname: 'meshagent_mipsel24kc',
      rname: 'meshagent',
      desc: 'Linux MIPSEL24KC (OpenWRT)',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    41: {
      id: 41,
      localname: 'meshagent_aarch64-cortex-a53',
      rname: 'meshagent',
      desc: 'ARMADA/CORTEX-A53/MUSL (OpenWRT)',
      update: true,
      amt: false,
      platform: 'linux',
      core: 'linux-noamt',
    ...
    42: {
      id: 42,
      localname: 'MeshConsoleARM64.exe',
      rname: 'meshconsolearm64.exe',
      desc: 'Windows ARM-64 console',
      update: true,
      amt: true,
      platform: 'win32',
      core: 'windows-amt',
    ...
    43: {
      id: 43,
      localname: 'MeshServiceARM64.exe',
      rname: 'meshagentarm64.exe',
      desc: 'Windows ARM-64 service',
      update: true,
      amt: true,
      platform: 'win32',
      core: 'windows-amt',
    ...
    10005: {
      id: 10005,
      localname: 'meshagent_osx-universal-64',
      rname: 'meshagent',
      desc: 'Apple macOS Universal Binary',
      update: true,
      amt: false,
      platform: 'osx',
      core: 'linux-noamt',
      ...
      arcore: 'linux-agentrecovery',
      tcore: 'linux-tiny'
    }, // Apple Silicon + x86 universal binary
    10006: {
      id: 10006,
      localname: 'MeshCentralAssistant.exe',
      rname: 'MeshCentralAssistant.exe',
      desc: 'MeshCentral Assistant for Windows',
      update: false,
      amt: false,
      platform: 'win32'
    ...
    11000: {
      id: 11000,
      localname: 'MeshCmd.exe',
      rname: 'MeshCmd.exe',
      desc: 'Windows x86-32 meshcmd',
      update: false,
      amt: true,
      platform: 'win32',
      codesign: true
    }, // MeshCMD for Windows x86 32-bit
    11001: {
      id: 11001,
      localname: 'MeshCmd64.exe',
      rname: 'MeshCmd64.exe',
      desc: 'Windows x86-64 meshcmd',
      update: false,
      amt: true,
      platform: 'win32',
      codesign: true
    }, // MeshCMD for Windows x86 64-bit
    11002: {
      id: 11002,
      localname: 'MeshCmdARM64.exe',
      rname: 'MeshCmdARM64.exe',
      desc: 'Windows ARM-64 meshcmd',
      update: false,
      amt: true,
      platform: 'win32',
      codesign: true
    } // MeshCMD for Windows ARM 64-bit
  };
  ...
  obj.signMeshAgents = function (domain, func) {
    ...
    if (agentSignCertInfo == null) {
      func();
      return;
    } // No code signing certificate, nothing to do.
    ...
    if (obj.args.agentaliasdns != null) {
      httpsHost = obj.args.agentaliasdns;
    }
    ...
    if (obj.config.settings.agentsignlock) {
      signUrl += '?ServerID=' + obj.certificateOperations.getPublicKeyHash(obj.certificates.agent.cert).toUpperCase();
    }

    // Setup the time server
    var timeStampUrl = 'http://timestamp.comodoca.com/authenticode';
    if (obj.args.agenttimestampserver === false) {
      timeStampUrl = null;
    } else if (typeof obj.args.agenttimestampserver == 'string') {
      timeStampUrl = obj.args.agenttimestampserver;
    }

    // Setup the time server proxy
    var timeStampProxy = null;
    if (typeof obj.args.agenttimestampproxy == 'string') {
      timeStampProxy = obj.args.agenttimestampproxy;
    } else if (obj.args.agenttimestampproxy !== false && typeof obj.args.npmproxy == 'string') {
      timeStampProxy = obj.args.npmproxy;
    }
    ...
    for (var archid in obj.meshAgentsArchitectureNumbers) {
      ...
      if (domain.id == '') {
        ...
        if (obj.fs.existsSync(agentpath2)) {
          ...
          delete obj.meshAgentsArchitectureNumbers[archid].codesign;
        } // If the agent is present in "meshcentral-data/agents", use that one instead.
      } else {
        ...
        if (obj.fs.existsSync(agentpath)) {
          delete obj.meshAgentsArchitectureNumbers[archid].codesign;
        } else {
          continue;
        } // If the agent is not present in "meshcentral-data/agents" skip.
      }

      // Open the original agent with authenticode
      const signeedagentpath = obj.path.join(
        serverSignedAgentsPath,
        obj.meshAgentsArchitectureNumbers[archid].localname
      );
      ...
      if (originalAgent != null) {
        ...
        if (destinationAgent != null) {
          ...
          if (destinationAgentOk == true) {
            ...
            for (var i in versionProperties) {
              const prop = versionProperties[i],
                propl = prop.toLowerCase();
              if (
                domain.agentfileinfo != null &&
                ...
                if (domain.agentfileinfo[propl] != versionStrings[prop]) {
                  ...
                  break;
                } // If the resource we want is not the same as the destination executable, we need to re-sign the agent.
              } else {
                if (orgVersionStrings[prop] != versionStrings[prop]) {
                  destinationAgentOk = false;
                  break;
                } // if the resource of the orginal agent not the same as the destination executable, we need to re-sign the agent.
              }
            }
            ...
            if (destinationAgentOk == true) {
              if (
                domain.agentfileinfo != null &&
                typeof domain.agentfileinfo == 'object' &&
                typeof domain.agentfileinfo['fileversionnumber'] == 'string'
              ) {
                if (domain.agentfileinfo['fileversionnumber'] != versionStrings['~FileVersion']) {
                  destinationAgentOk = false;
                } // If the resource we want is not the same as the destination executable, we need to re-sign the agent.
              } else {
                if (orgVersionStrings['~FileVersion'] != versionStrings['~FileVersion']) {
                  destinationAgentOk = false;
                } // if the resource of the orginal agent not the same as the destination executable, we need to re-sign the agent.
              }
            }
            ...
            if (destinationAgentOk == true) {
              if (
                domain.agentfileinfo != null &&
                typeof domain.agentfileinfo == 'object' &&
                typeof domain.agentfileinfo['productversionnumber'] == 'string'
              ) {
                if (domain.agentfileinfo['productversionnumber'] != versionStrings['~ProductVersion']) {
                  destinationAgentOk = false;
                } // If the resource we want is not the same as the destination executable, we need to re-sign the agent.
              } else {
                if (orgVersionStrings['~ProductVersion'] != versionStrings['~ProductVersion']) {
                  destinationAgentOk = false;
                } // if the resource of the orginal agent not the same as the destination executable, we need to re-sign the agent.
              }
            }
                  ...
                  if (agentIconGroupNames.length > 0) {
                    ...
                    if (agentMainIconGroup.resCount != domain.agentfileinfo.icon.resCount) {
                      ...
                      if (agentMainIconGroupHash != iconHash) {
                        destinationAgentOk = false;
                      } // If the existing agent icon does not match the desired icon, we need to re-sign the agent.
                    }
                  }
                }
              } else {
                ...
                if (agentBitmaps != null) {
                  ...
                  if (agentBitmapNames.length > 0) {
                    ...
                    if (agentMainBitmapHash != bitmapHash) {
                      destinationAgentOk = false;
                    } // If the existing agent logo does not match the desired logo, we need to re-sign the agent.
                  }
                }
              } else {
                // Check if the destination agent has the default icon
          ...
          if (
            destinationAgentOk == true &&
            originalAgent
              .getHashOfSection('sha384', '.text')
              .compare(destinationAgent.getHashOfSection('sha384', '.text')) != 0
          ) {
            destinationAgentOk = false;
          }
          if (
            destinationAgentOk == true &&
            originalAgent
              .getHashOfSection('sha384', '.data')
              .compare(destinationAgent.getHashOfSection('sha384', '.data')) != 0
          ) {
            destinationAgentOk = false;
          }
          if (
            destinationAgentOk == true &&
            originalAgent
              .getHashOfSection('sha384', '.rdata')
              .compare(destinationAgent.getHashOfSection('sha384', '.rdata')) != 0
          ) {
            destinationAgentOk = false;
          }
        ...
        if (destinationAgentOk == false) {
          ...
          try {
            obj.fs.mkdirSync(serverSignedAgentsPath);
          } catch (ex) {}
          const xagentSignedFunc = function agentSignedFunc(err, size) {
            if (err == null) {
              ...
              console.log(
                obj.common.format(
                  'Code signed {0}.',
                  agentSignedFunc.objx.meshAgentsArchitectureNumbers[agentSignedFunc.archid].localname
                )
              );
            } else {
          ...
          if (domain.agentfileinfo != null && typeof domain.agentfileinfo == 'object') {
            ...
            for (var i in versionProperties) {
              const prop = versionProperties[i],
                propl = prop.toLowerCase();
              if (domain.agentfileinfo[propl] && domain.agentfileinfo[propl] != versionStrings[prop]) {
            ...
            if (
              domain.agentfileinfo['fileversionnumber'] &&
            ...
            if (
              domain.agentfileinfo['productversionnumber'] &&
  ...
  obj.updateMeshAgentsTable = function (domain, func) {
    ...
    const agentInfo = [];
    try {
      agentInfo = JSON.parse(obj.fs.readFileSync(obj.path.join(__dirname, 'agents', 'hashagents.json'), 'utf8'));
    } catch (ex) {}
    ...
    for (var archid in obj.meshAgentsArchitectureNumbers) {
      ...
      if (domain.id == '') {
        ...
        if (obj.meshAgentsArchitectureNumbers[archid].unsigned !== true) {
          const agentpath2 = obj.path.join(
            obj.datapath,
            'signedagents' + suffix,
            obj.meshAgentsArchitectureNumbers[archid].localname
          );
          if (obj.fs.existsSync(agentpath2)) {
            agentpath = agentpath2;
          } // If the agent is present in "meshcentral-data/signedagents", use that one instead.
          const agentpath3 = obj.path.join(
            obj.datapath,
            'agents' + suffix,
            obj.meshAgentsArchitectureNumbers[archid].localname
          );
          if (obj.fs.existsSync(agentpath3)) {
            agentpath = agentpath3;
          } // If the agent is present in "meshcentral-data/agents", use that one instead.
        }
      } else {
        // When processing an extra domain, only load agents that are specific to that domain
      ...
      var stats = null;
      try {
        stats = obj.fs.statSync(agentpath);
      } catch (ex) {}
      ...
      objx.meshAgentBinaries[archid].url =
        'http://' +
        obj.certificates.CommonName +
        ':' +
        (typeof obj.args.aliasport == 'number' ? obj.args.aliasport : obj.args.port) +
      ...
      if (agentInfo[archid] != null && agentInfo[archid].mtime != null) {
        objx.meshAgentBinaries[archid].mtime = new Date(agentInfo[archid].mtime);
      } // Set agent time if available
      ...
      if (
        obj.args.agentsinram === true ||
        ((archid == 3 || archid == 4 || archid == 43) && obj.args.agentsinram !== false)
      ) {
        if (archid == 3 || archid == 4 || archid == 43) {
          ...
          outStream.on('finish', function () {
            ...
            const onZipData = function onZipData(buffer) {
              onZipData.x.zacc.push(buffer);
            };
            ...
            const onZipError = function onZipError() {
              delete onZipData.x.zacc;
            };
      ...
      const options = {
        ...
        platform: obj.meshAgentsArchitectureNumbers[archid].platform
      };
      if (objx.meshAgentBinaries[archid].pe != null) {
        options.peinfo = objx.meshAgentBinaries[archid].pe;
      }
      obj.exeHandler.hashExecutableFile(options);
      ...
      if (obj.args.agentsinram === false && (archid == 3 || archid == 4 || archid == 43)) {
        ...
        objx.meshAgentBinaries[archid].fileHashHex = Buffer.from(
          objx.meshAgentBinaries[archid].fileHash,
          'binary'
        ).toString('hex');
      }
    }
  ...
  obj.getLoginToken = function (userid, func) {
    ...
    if (x == null || x.length != 3 || x[0] != 'user') {
      func('Invalid userid.');
      return;
    }
    obj.db.Get(userid, function (err, docs) {
      if (err != null || docs == null || docs.length == 0) {
        func('User ' + userid + ' not found.');
        return;
      } else {
        ...
        obj.db.Get('LoginCookieEncryptionKey', function (err, docs) {
          if (docs.length > 0 && docs[0].key != null && obj.args.logintokengen == null && docs[0].key.length >= 160) {
            ...
            obj.loginCookieEncryptionKey = obj.generateCookieKey();
            obj.db.Set(
              { _id: 'LoginCookieEncryptionKey', key: obj.loginCookieEncryptionKey.toString('hex'), time: Date.now() },
              function () {
                func(obj.encodeCookie({ u: userid, a: 3 }, obj.loginCookieEncryptionKey));
              }
            );
    ...
    obj.db.Get('LoginCookieEncryptionKey', function (err, docs) {
      if (docs.length > 0 && docs[0].key != null && obj.args.logintokengen == null && docs[0].key.length >= 160) {
        ...
        obj.db.Set(
          { _id: 'LoginCookieEncryptionKey', key: obj.loginCookieEncryptionKey.toString('hex'), time: Date.now() },
          function () {
            func(obj.loginCookieEncryptionKey.toString('hex'));
          }
        );
      }
    });
  ...
  obj.encryptSessionData = function (data, key) {
    ...
    if (key == null) {
      key = obj.loginCookieEncryptionKey;
    }
    try {
      const iv = Buffer.from(obj.crypto.randomBytes(12), 'binary'),
        cipher = obj.crypto.createCipheriv('aes-256-gcm', key.slice(0, 32), iv);
      ...
      return Buffer.concat([iv, cipher.getAuthTag(), crypted])
        .toString(obj.args.cookieencoding ? obj.args.cookieencoding : 'base64')
        .replace(/\+/g, '@')
        .replace(/\//g, '$');
    } catch (ex) {
      return null;
  ...
  obj.decryptSessionData = function (data, key) {
    ...
    try {
      const buf = Buffer.from(
        ...
        obj.args.cookieencoding ? obj.args.cookieencoding : 'base64'
      );
  ...
  obj.encodeCookie = function (o, key) {
    try {
      ...
      const r = Buffer.concat([iv, cipher.getAuthTag(), crypted])
        .toString(obj.args.cookieencoding ? obj.args.cookieencoding : 'base64')
        .replace(/\+/g, '@')
        .replace(/\//g, '$');
      ...
      obj.debug('cookie', 'ERR: Failed to encode AESGCM cookie due to exception: ' + ex);
      return null;
    }
  };
  ...
  obj.decodeCookie = function (cookie, key, timeout) {
    ...
    if (r === -1) {
      r = obj.decodeCookieAESSHA(cookie, key, timeout);
    } // If decodeCookieAESGCM() failed to decode, try decodeCookieAESSHA()
    if (
      r == null &&
      obj.args.cookieencoding == null &&
      cookie.length != 64 &&
      (cookie == cookie.toLowerCase() || cookie == cookie.toUpperCase())
    ) {
    ...
    if (r != null && typeof r.once == 'string' && r.once.length > 0) {
      ...
      if (obj.cookieUseOnceTable[r.once] == null) {
        const ctimeout =
          r.expire == null || typeof r.expire != 'number'
            ? r.time + (timeout + 3) * 60000
            : r.time + (r.expire + 3) * 60000;
        ...
        if (++obj.cookieUseOnceTableCleanCounter > 20) {
          const now = Date.now();
          for (var i in obj.cookieUseOnceTable) {
            if (obj.cookieUseOnceTable[i] < now) {
              delete obj.cookieUseOnceTable[i];
            }
          }
  ...
  obj.decodeCookieAESGCM = function (cookie, key, timeout) {
    try {
      ...
      if (o.time == null || o.time == null || typeof o.time != 'number') {
        obj.debug('cookie', 'ERR: Bad cookie due to invalid time');
        return null;
      }
      ...
      if (o.expire == null || typeof o.expire != 'number') {
        ...
        if (o.dtime > timeout * 60000 || o.dtime < -30000) {
          obj.debug('cookie', 'ERR: Bad cookie due to timeout');
          return null;
        } // The cookie is only valid 120 seconds, or 30 seconds back in time (in case other server's clock is not quite right)
      } else {
        // An expire time is included in the cookie (in minutes), use this.
        if (o.expire !== 0 && (o.dtime > o.expire * 60000 || o.dtime < -30000)) {
          obj.debug('cookie', 'ERR: Bad cookie due to timeout');
          return null;
        } // The cookie is only valid 120 seconds, or 30 seconds back in time (in case other server's clock is not quite right)
      }
      ...
      obj.debug('cookie', 'ERR: Bad AESGCM cookie due to exception: ' + ex);
      return -1;
    }
  };
  ...
  obj.decodeCookieAESSHA = function (cookie, key, timeout) {
    try {
      ...
      if (Buffer.compare(hmac.digest(), Buffer.from(rawmsg.slice(0, 48))) == false) {
        return null;
      }
      ...
      obj.debug('cookie', 'ERR: Bad AESSHA cookie due to exception: ' + ex);
      return null;
    }
  };
  ...
  obj.debug = function (source, ...args) {
    ...
    if (obj.debugSources != null && (obj.debugSources == '*' || obj.debugSources.indexOf(source) >= 0)) {
      console.log(source.toUpperCase() + ':', ...args);
    }

    // Send event to log file
    if (obj.config.settings && obj.config.settings.log) {
      if (typeof obj.args.log == 'string') {
        obj.args.log = obj.args.log.split(',');
      }
      if (obj.args.log.indexOf(source) >= 0 || obj.args.log[0] == '*') {
        ...
        if (obj.xxLogFile != null) {
          try {
            if (obj.xxLogDateStr != d.toLocaleDateString()) {
              obj.xxLogDateStr = d.toLocaleDateString();
              obj.fs.writeSync(obj.xxLogFile, '---- ' + d.toLocaleDateString() + ' ----\r\n');
            }
            const formattedArgs = args.map(function (arg) {
              return typeof arg === 'object' && arg !== null ? JSON.stringify(arg) : arg;
            });
            obj.fs.writeSync(
              obj.xxLogFile,
              new Date().toLocaleTimeString() + ' - ' + source + ': ' + formattedArgs.join(', ') + '\r\n'
            );
          } catch (ex) {}
        }
      }
    ...
    if (
      obj.debugRemoteSources != null &&
      (obj.debugRemoteSources == '*' || obj.debugRemoteSources.indexOf(source) >= 0)
    ) {
      var sendcount = 0;
      for (var sessionid in obj.webserver.wssessions2) {
        ...
        if (ws != null && ws.userid != null) {
          ...
          if (user != null && user.siteadmin == 4294967295) {
            try {
              ws.send(JSON.stringify({ action: 'trace', source: source, args: args, time: Date.now() }));
              sendcount++;
            } catch (ex) {}
          }
        }
      ...
      if (sendcount == 0) {
        obj.debugRemoteSources = null;
      } // If there are no listeners, remove debug sources.
    }
  };
  ...
  obj.updateServerState = function (name, val) {
    ...
    try {
      if (name != null && val != null) {
        var changed = false;
        if (name != null && meshServerState[name] != val) {
          if (val == null && meshServerState[name] != null) {
            delete meshServerState[name];
            changed = true;
          } else {
            if (meshServerState[name] != val) {
              meshServerState[name] = val;
              changed = true;
            }
          }
        }
      ...
      var r = 'time=' + Date.now() + '\r\n';
      for (var i in meshServerState) {
        r += i + '=' + meshServerState[i] + '\r\n';
      }
      try {
  ...
  function readIpListFromFile(arg) {
    ...
    try {
      lines = obj.fs
        .readFileSync(obj.path.join(obj.datapath, arg.substring(5)))
        .toString()
        .split(/\r?\n/)
        .join('\r')
        .split('\r');
    ...
    for (var i in lines) {
      ...
      if (
        candidate.length > 0 &&
        candidate.indexOf('@') === -1 &&
        (candidate.indexOf('.') > -1 || candidate.charAt(0) === ':')
      ) {
  ...
  function logInfoEvent(msg) {
    if (obj.servicelog != null) {
      obj.servicelog.info(msg);
    }
    console.log(msg);
  }
  function logWarnEvent(msg) {
    if (obj.servicelog != null) {
      obj.servicelog.warn(msg);
    }
    console.log(msg);
  }
  function logErrorEvent(msg) {
    if (obj.servicelog != null) {
      obj.servicelog.error(msg);
    }
    console.error(msg);
  }
  obj.getServerWarnings = function () {
    return serverWarnings;
  };
  // TODO: migrate from other addServerWarning function and add timestamp
  obj.addServerWarning = function (msg, id, args, print) {
    serverWarnings.push({ msg: msg, id: id, args: args });
    if (print !== false) {
  ...
  obj.authLog = function (server, msg, args) {
    ...
    if (args != null) {
      ...
      if (typeof args.useragent == 'string') {
        const userAgentInfo = obj.webserver.getUserAgentInfo(args.useragent);
        str += ', Browser: ' + userAgentInfo.browserStr + ', OS: ' + userAgentInfo.osStr;
      }
    }
    obj.debug('authlog', str);
    if (obj.syslogauth != null) {
      try {
        obj.syslogauth.log(obj.syslogauth.LOG_INFO, str);
      } catch (ex) {}
    }
    if (obj.authlogfile != null) {
      // Write authlog to file
      try {
        const d = new Date(),
        ...
        str =
          ...
          d.getDate() +
          ' ' +
          obj.common.zeroPad(d.getHours(), 2) +
          ':' +
          obj.common.zeroPad(d.getMinutes(), 2) +
          ':' +
          d.getSeconds() +
          ' meshcentral ' +
          server +
  ...
  obj.getConfigFilePath = function (filename) {
    if (
      obj.config != null &&
      obj.config.configfiles != null &&
      obj.config.configfiles[filename] != null &&
      typeof obj.config.configfiles[filename] == 'string'
    ) {
...
function checkResolveAll(names, func) {
  const dns = require('dns'),
    state = { func: func, count: names.length, err: null };
  for (var i in names) {
    dns.lookup(
      names[i],
      { all: true },
      function (err, records) {
        if (err != null) {
          if (this.state.err == null) {
            this.state.err = [this.name];
          } else {
            this.state.err.push(this.name);
          }
        }
        if (--this.state.count == 0) {
          this.state.func(this.state.err);
        }
...
function getConfig(createSampleConfig) {
  ...
  if (fs.existsSync(configFilePath)) {
    ...
    try {
      ...
      console.log('ERROR: Unable to parse ' + configFilePath + '.');
      return null;
    }
    ...
    for (i in config.domains) {
      if (i.split('/').length > 1 || i.split(' ').length > 1) {
        console.log("ERROR: Error in config.json, domain names can't have spaces or /.");
        return null;
      }
    }
  } else {
    if (createSampleConfig === true) {
      ...
      if (fs.existsSync(sampleConfigPath)) {
        fs.createReadStream(sampleConfigPath).pipe(fs.createWriteStream(configFilePath));
      }
    }
  }
  ...
  if (modules.length > 0) {
    ...
    for (var i in modules) {
      ...
      if (moduleInfo.length == 1) {
        // normal package without version
        moduleName = moduleInfo[0];
      } else if (moduleInfo.length == 2) {
        // normal package with a version OR custom repo package with no version
        moduleName = moduleInfo[0] === '' ? moduleNameAndVersion : moduleInfo[0];
        moduleVersion = moduleInfo[0] === '' ? null : moduleInfo[1];
      } else if (moduleInfo.length == 3) {
        // custom repo package and package with a version
      ...
      try {
        ...
        if (moduleVersion) {
          ...
          try {
            versionMatch = require(`${moduleName}/package.json`).version == moduleVersion;
          } catch (ex) {
            if (ex.code == 'ERR_PACKAGE_PATH_NOT_EXPORTED') {
              modulePath = ('' + ex).split(' ').at(-1);
            } else {
              throw new Error();
            }
          }
          ...
          if (versionMatch == false && modulePath != null) {
            if (JSON.parse(require('fs').readFileSync(modulePath, 'utf8')).version != moduleVersion) {
              throw new Error();
            }
          ...
          if (typeof dependencies[moduleName] != null) {
    ...
    if (missingModules.length > 0) {
      ...
      InstallModuleEx(missingModules, args, func);
    } else {
      func();
    }
  }
...
function InstallModuleEx(modulenames, args, func) {
  ...
  function getCurrentVersion() {
    try {
      return JSON.parse(require('fs').readFileSync(require('path').join(__dirname, 'package.json'), 'utf8')).version;
    } catch (ex) {}
    return null;
  } // Fetch server version
  ...
  if (args.debug) {
    console.log('NPM Command Line: ' + npmpath + ` install --save-exact --no-audit --omit=optional --no-fund ${names}`);
  }
  // always use --save-exact - https://stackoverflow.com/a/64507176/1210734
  child_process.exec(
    npmpath + ` install --save-exact --no-audit --no-optional --omit=optional ${names}`,
    { maxBuffer: 512000, timeout: 300000, cwd: parentpath },
    function (error, stdout, stderr) {
      if (error != null && error != '') {
        var mcpath = __dirname;
        if (mcpath.endsWith('\\node_modules\\meshcentral') || mcpath.endsWith('/node_modules/meshcentral')) {
        ...
        console.log(
          'ERROR: Unable to install required modules. MeshCentral may not have access to npm, or npm may not have suffisent rights to load the new module. To manualy install this module try:\r\n\r\n   cd "' +
            mcpath +
            '"\r\n   npm install --no-audit --no-optional --omit=optional ' +
            names +
            '\r\n   node node_modules' +
            (require('os').platform() == 'win32' ? '\\' : '/') +
            'meshcentral'
...
process.on('SIGINT', function () {
  ...
  console.log('Server Ctrl-C exit...');
  process.exit();
});
...
function addServerWarning(msg, id, args, print) {
  serverWarnings.push({ msg: msg, id: id, args: args });
  if (print !== false) {
    console.log('WARNING: ' + msg);
  }
...
function mainStart() {
  ...
  if (Number(process.version.match(/^v(\d+\.\d+)/)[1]) < 16) {
    console.log('MeshCentral requires Node v16 or above, current version is ' + process.version + '.');
    return;
  }

  // If running within the node_modules folder, move working directory to the parent of the node_modules folder.
  if (__dirname.endsWith('\\node_modules\\meshcentral') || __dirname.endsWith('/node_modules/meshcentral')) {
  ...
  InstallModules(['minimist'], {}, function () {
    ...
    if (args.npmpath == null) {
      try {
        ...
        if (require('fs').existsSync(xnodepath) && require('fs').existsSync(xnpmpath)) {
          if (xnodepath.indexOf(' ') >= 0) {
            xnodepath = '"' + xnodepath + '"';
          }
          if (xnpmpath.indexOf(' ') >= 0) {
            xnpmpath = '"' + xnpmpath + '"';
          }
          if (require('os').platform() == 'win32') {
            npmpath = xnpmpath;
          } else {
            npmpath = xnodepath + ' ' + xnpmpath;
          }
        }
      } catch (ex) {
        console.log(ex);
    ...
    for (var i in config.domains) {
      ...
      if (
        (config.domains[i].smtp != null && config.domains[i].smtp.name != 'console') ||
        config.domains[i].sendmail != null
      ) {
        nodemailer = true;
      }
      if (config.domains[i].sendgrid != null) {
        sendgrid = true;
      }
      if (config.domains[i].yubikey != null) {
        yubikey = true;
      }
      if (config.domains[i].auth == 'ldap') {
        ldap = true;
      }
      if (mstsc == false) {
        config.domains[i].mstsc = false;
      }
      if (config.domains[i].ssh == true) {
        ssh = true;
      }
      if (typeof config.domains[i].authstrategies == 'object') {
        if (passport.indexOf('passport') == -1) {
          passport.push('passport@0.7.0', 'connect-flash@0.1.1');
        } // Passport v0.6.0 requires a patch, see https://github.com/jaredhanson/passport/issues/904 and include connect-flash here to display errors
        if (
          typeof config.domains[i].authstrategies.twitter == 'object' &&
          typeof config.domains[i].authstrategies.twitter.clientid == 'string' &&
          typeof config.domains[i].authstrategies.twitter.clientsecret == 'string' &&
          passport.indexOf('passport-twitter') == -1
        ) {
          passport.push('passport-twitter@1.0.4');
        }
        if (
          typeof config.domains[i].authstrategies.google == 'object' &&
          typeof config.domains[i].authstrategies.google.clientid == 'string' &&
          typeof config.domains[i].authstrategies.google.clientsecret == 'string' &&
          passport.indexOf('passport-google-oauth20') == -1
        ) {
          passport.push('passport-google-oauth20@2.0.0');
        }
        if (
          typeof config.domains[i].authstrategies.github == 'object' &&
          typeof config.domains[i].authstrategies.github.clientid == 'string' &&
          typeof config.domains[i].authstrategies.github.clientsecret == 'string' &&
          passport.indexOf('passport-github2') == -1
        ) {
          passport.push('passport-github2@0.1.12');
        }
        if (
          typeof config.domains[i].authstrategies.azure == 'object' &&
          typeof config.domains[i].authstrategies.azure.clientid == 'string' &&
          typeof config.domains[i].authstrategies.azure.clientsecret == 'string' &&
          typeof config.domains[i].authstrategies.azure.tenantid == 'string' &&
          passport.indexOf('passport-azure-oauth2') == -1
        ) {
          passport.push('passport-azure-oauth2@0.1.0');
          passport.push('jwt-simple@0.5.6');
        }
        ...
        if (
          typeof config.domains[i].authstrategies.saml == 'object' ||
          typeof config.domains[i].authstrategies.jumpcloud == 'object'
        ) {
          passport.push('passport-saml');
        }
      }
      if (config.domains[i].sessionrecording != null) {
        sessionRecording = true;
      }
      if (
        config.domains[i].passwordrequirements != null &&
        config.domains[i].passwordrequirements.bancommonpasswords == true
      ) {
        wildleek = true;
      }
      if (config.domains[i].newaccountscaptcha != null && config.domains[i].newaccountscaptcha !== false) {
        captcha = true;
      }
      if (typeof config.domains[i].duo2factor == 'object' && passport.indexOf('@duosecurity/duo_universal') == -1) {
        passport.push('@duosecurity/duo_universal@2.1.0');
      }
    }
    ...
    if (require('os').platform() == 'win32') {
      modules.push('node-windows@0.1.14');
      modules.push('loadavg-windows@1.1.1');
      if (sspi == true) {
        modules.push('node-sspi@0.2.10');
      }
    } // Add Windows modules
    if (ldap == true) {
      modules.push('ldapauth-fork@5.0.5');
    }
    if (ssh == true) {
      modules.push('ssh2@1.17.0');
    }
    if (passport != null) {
      modules.push(...passport);
    }
    if (captcha == true) {
      modules.push('svg-captcha@1.4.0');
    }

    if (sessionRecording == true) {
      modules.push('image-size@2.0.2');
    } // Need to get the remote desktop JPEG sizes to index the recording file.
    ...
    if (config.settings.mqtt != null) {
      modules.push('aedes@0.51.3');
    } // Add MQTT Modules
    if (config.settings.mysql != null) {
      modules.push('mysql2@3.15.1');
    } // Add MySQL.
    ...
    if (config.settings.mongodb != null) {
      modules.push('mongodb@4.17.2');
      modules.push('@mongodb-js/saslprep@1.3.1');
    } // Add MongoDB, official driver.
    if (config.settings.postgres != null) {
      modules.push('pg@8.16.3');
    } // Add Postgres, official driver.
    if (config.settings.mariadb != null) {
      modules.push('mariadb@3.4.5');
    } // Add MariaDB, official driver.
    if (config.settings.acebase != null) {
      modules.push('acebase@1.29.5');
    } // Add AceBase, official driver.
    if (config.settings.sqlite3 != null) {
      modules.push('sqlite3@5.1.7');
    } // Add sqlite3, official driver.
    if (config.settings.vault != null) {
      modules.push('node-vault@0.10.5');
    } // Add official HashiCorp's Vault module.
    const hasExistingProxy =
      process.env['HTTP_PROXY'] ||
      process.env['HTTPS_PROXY'] ||
      process.env['http_proxy'] ||
      process.env['https_proxy'];
    if ((config.settings.plugins != null && config.settings.plugins.proxy != null) || hasExistingProxy) {
      modules.push('https-proxy-agent@7.0.6');
    } // Required for HTTP/HTTPS proxy support
    else if (config.settings.xmongodb != null) {
      modules.push('mongojs@3.1.0');
    } // Add MongoJS, old driver.
    if (nodemailer || (config.smtp != null && config.smtp.name != 'console') || config.sendmail != null) {
      modules.push('nodemailer@6.10.1');
    } // Add SMTP support
    if (sendgrid || config.sendgrid != null) {
      modules.push('@sendgrid/mail@8.1.6');
    } // Add SendGrid support
    if ((args.translate || args.dev) && Number(process.version.match(/^v(\d+\.\d+)/)[1]) >= 16) {
      modules.push('jsdom@22.1.0');
      modules.push('esprima@4.0.1');
      modules.push('html-minifier-terser@7.2.0');
    } // Translation support
    if (typeof config.settings.crowdsec == 'object') {
      modules.push('@crowdsec/express-bouncer@0.1.0');
    } // Add CrowdSec bounser module (https://www.npmjs.com/package/@crowdsec/express-bouncer)
    if (config.settings.prometheus != null) {
      modules.push('prom-client@15.1.3');
    } // Add Prometheus Metrics support

    if (typeof config.settings.autobackup == 'object') {
      // Setup encrypted zip support if needed
      if (config.settings.autobackup.zippassword) {
        modules.push('archiver-zip-encrypted@2.0.0');
      }
      // Enable Google Drive Support
      if (typeof config.settings.autobackup.googledrive == 'object') {
        modules.push('googleapis@128.0.0');
      }
      // Enable WebDAV Support
      if (typeof config.settings.autobackup.webdav == 'object') {
        if (
          typeof config.settings.autobackup.webdav.url != 'string' ||
          typeof config.settings.autobackup.webdav.username != 'string' ||
          typeof config.settings.autobackup.webdav.password != 'string'
        ) {
          addServerWarning('Missing WebDAV parameters.', 2, null, !args.launch);
        } else {
          modules.push('webdav@5.9.0');
        }
      }
      // Enable S3 Support
      if (typeof config.settings.autobackup.s3 == 'object') {
        modules.push('minio@8.0.6');
      }
    }

    // Setup common password blocking
    if (wildleek == true) {
      modules.push('wildleek@2.0.0');
    }
    ...
    if (config.settings.no2factorauth !== true) {
      ...
      if (yubikey == true) {
        modules.push('yub@0.11.1');
      } // Add YubiKey OTP support (replaced yubikeyotp due to form-data issues)
      if (allsspi == false) {
        modules.push('otplib@12.0.1');
      } // Google Authenticator support (v10 supports older NodeJS versions).
    }

    // Desktop multiplexor support
    if (config.settings.desktopmultiplex === true) {
      modules.push('image-size@2.0.2');
    }
    ...
    if (config.sms != null) {
      if (config.sms.provider == 'twilio') {
        modules.push('twilio@4.23.0');
      }
      if (config.sms.provider == 'plivo') {
        modules.push('plivo@4.75.1');
      }
      if (config.sms.provider == 'telnyx') {
        modules.push('telnyx@1.25.5');
      }
    }

    // Messaging support
    if (config.messaging != null) {
      if (config.messaging.telegram != null) {
        modules.push('telegram@2.26.22');
        modules.push('input@1.0.1');
      }
      if (config.messaging.discord != null) {
        if (nodeVersion >= 17) {
          modules.push('discord.js@14.6.0');
        } else {
          delete config.messaging.discord;
          addServerWarning('This NodeJS version does not support Discord.js.', 26);
        }
      }
      if (config.messaging.xmpp != null) {
        modules.push('@xmpp/client@0.13.6');
      }
      if (config.messaging.pushover != null) {
        modules.push('node-pushover@1.0.0');
      }
      if (config.messaging.zulip != null) {
        modules.push('zulip@0.1.0');
      }
    }

    // Setup web based push notifications
    if (typeof config.settings.webpush == 'object' && typeof config.settings.webpush.email == 'string') {
      modules.push('web-push@3.6.7');
    }

    // Firebase Support
    if (config.firebase != null && typeof config.firebase.serviceaccountfile == 'string') {
      modules.push('firebase-admin@12.7.0');
    }

    // Syslog support
    if (require('os').platform() != 'win32' && (config.settings.syslog || config.settings.syslogjson)) {
      modules.push('modern-syslog@1.2.0');
    }
    if (config.settings.syslogtcp) {
      modules.push('syslog@0.1.1-1');
    }
    ...
    if (config.settings.heapdump === true) {
      modules.push('heapdump@0.3.15');
    }
    ...
    InstallModules(modules, args, function () {
      if (require('os').platform() == 'win32') {
        try {
          ...
          console.log("Module node-windows can't be loaded. Restart MeshCentral.");
          process.exit();
          return;
        }
      }
