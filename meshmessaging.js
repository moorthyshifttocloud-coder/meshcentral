...
module.exports.CreateServer = function (parent) {
  ...
  if (parent.config.messaging.telegram) {
    ...
    var telegramOK = true;
    if (typeof parent.config.messaging.telegram.apiid != 'number') {
      console.log('Invalid or missing Telegram apiid.');
      telegramOK = false;
    }
    if (typeof parent.config.messaging.telegram.apihash != 'string') {
      console.log('Invalid or missing Telegram apihash.');
      telegramOK = false;
    }
    if (
      typeof parent.config.messaging.telegram.session != 'string' &&
      typeof parent.config.messaging.telegram.bottoken != 'string'
    ) {
      console.log('Invalid or missing Telegram session or bottoken.');
      telegramOK = false;
    }
    ...
    if (telegramOK) {
      ...
      async function setupTelegram() {
        ...
        if (parent.config.messaging.telegram.usewss == false) {
          options.useWSS = false;
        }
        if (typeof parent.config.messaging.telegram.connectionretries == 'number') {
          options.connectionRetries = parent.config.messaging.telegram.connectionretries;
        }
        if (typeof parent.config.messaging.telegram.proxy == 'object') {
          options.proxy = parent.config.messaging.telegram.proxy;
        }
        if (parent.config.messaging.telegram.bottoken == null) {
          ...
          const client = new TelegramClient(
            stringSession,
            parent.config.messaging.telegram.apiid,
            parent.config.messaging.telegram.apihash,
            options
          );
          await client.start({
          ...
          await client.start({
            botAuthToken: parent.config.messaging.telegram.bottoken,
            onError: function (err) {
              console.log('Telegram error', err);
            }
          });
  ...
  if (parent.config.messaging.discord) {
    ...
    var discordOK = true;
    if (typeof parent.config.messaging.discord.token != 'string') {
      console.log('Invalid or missing Discord token.');
      discordOK = false;
    }
  ...
  if (parent.config.messaging.xmpp) {
    ...
    var xmppOK = true;
    if (typeof parent.config.messaging.xmpp.service != 'string') {
      console.log('Invalid or missing XMPP service.');
      xmppOK = false;
    }
    ...
    if (xmppOK) {
      ...
      xmpp.on('error', function (err) {
        parent.debug('email', 'XMPP error: ' + err);
        console.error('XMPP error', err);
      });
      xmpp.on('offline', function () {
        parent.debug('email', 'XMPP client is offline.');
  ...
  if (typeof parent.config.messaging.pushover == 'object') {
    ...
    var pushoverOK = true;
    if (typeof parent.config.messaging.pushover.token != 'string') {
      console.log('Invalid or missing Pushover token.');
      pushoverOK = false;
    }
  ...
  async function discordSendMsg(userId, message) {
    const user = await obj.discordClient.users.fetch(userId).catch(function () {
      return null;
    });
    if (!user) return;
    await user.send(message).catch(function (ex) {
      console.log('Discord Error', ex);
    });
  }
  ...
  async function discordFindUserByTag(userTag, func) {
    ...
    guilds.forEach(async function (value, key) {
      ...
      guildMembers.forEach(async function (value, key) {
        if (value.user.username + (value.user.discriminator != '0' ? '#' + value.user.discriminator : '') == userTag) {
          func(key);
          return;
        }
      });
  ...
  obj.sendMessage = function (to, msg, domain, func) {
    if (to.startsWith('telegram:') && obj.telegramClient != null) {
      ...
      async function sendTelegramMessage(to, msg, func) {
        ...
        try {
          await obj.telegramClient.sendMessage(to.substring(9), { message: msg });
          if (func != null) {
            func(true);
          }
        } catch (ex) {
    ...
    } else if (to.startsWith('discord:') && obj.discordClient != null) {
      // Discord
      discordFindUserByTag(to.substring(8), function (userid) {
        ...
        discordSendMsg(userid, msg);
        if (func != null) {
          func(true);
        }
      });
    } else if (to.startsWith('xmpp:') && obj.xmppClient != null) {
      // XMPP
      parent.debug('email', 'Sending XMPP message to: ' + to.substring(5) + ': ' + msg);
      sendXmppMessage(to, msg, func);
    } else if (to.startsWith('callmebot:') && obj.callMeBotClient != null) {
      // CallMeBot
      ...
      if (toData[0] == 'signal' && toData.length == 3) {
        var url =
          ...
          encodeURIComponent(toData[1]) +
          '&apikey=' +
          encodeURIComponent(toData[2]) +
          '&text=' +
          encodeURIComponent(msg);
        require('https').get(url, function (r) {
          if (func != null) {
            func(r.statusCode == 200);
          }
        });
        ...
        var url =
          'https://api.callmebot.com/facebook/send.php?apikey=' +
          encodeURIComponent(toData[1]) +
          '&text=' +
          encodeURIComponent(msg);
        require('https').get(url, function (r) {
        ...
        var url =
          'https://api.callmebot.com/text.php?user=' +
          encodeURIComponent(toData[1]) +
          '&text=' +
          encodeURIComponent(msg);
        require('https').get(url, function (r) {
    ...
    } else if (to.startsWith('pushover:') && obj.pushoverClient != null) {
      // Pushover
      const Pushover = require('node-pushover');
      const push = new Pushover({ token: parent.config.messaging.pushover.token, user: to.substring(9) });
      push.send(domain.title ? domain.title : 'MeshCentral', msg, function (err, res) {
        if (func != null) {
          func(err == null);
        }
      });
    } else if (to.startsWith('ntfy:') && obj.ntfyClient != null) {
      // ntfy
      const url =
        'https://' +
        (typeof parent.config.messaging.ntfy == 'object' && typeof parent.config.messaging.ntfy.host == 'string'
          ? parent.config.messaging.ntfy.host
          : 'ntfy.sh') +
        '/' +
        encodeURIComponent(to.substring(5));
      const headers = { 'User-Agent': 'MeshCentral v' + parent.currentVer };
      if (typeof parent.config.messaging.ntfy.authorization == 'string') {
        headers['Authorization'] = parent.config.messaging.ntfy.authorization;
      }
      const req = require('https').request(new URL(url), { method: 'POST', headers: headers }, function (res) {
        if (func != null) {
          func(res.statusCode == 200);
        }
      });
      req.end(msg);
    } else if (to.startsWith('zulip:') && obj.zulipClient != null) {
      // zulip
    ...
    } else if (to.startsWith('slack:') && obj.slackClient != null) {
      //slack
      const req = require('https').request(new URL(to.substring(6)), { method: 'POST' }, function (res) {
        if (func != null) {
          func(true);
        }
      });
  ...
  obj.callmebotUrlToHandle = function (xurl) {
    var url = null;
    try {
      url = require('url').parse(xurl);
    } catch (ex) {
      return;
    }
    if (url == null || url.host != 'api.callmebot.com' || url.query == null) return;
    var urlArgs = {},
      urlArgs2 = url.query.split('&');
    for (var i in urlArgs2) {
      var j = urlArgs2[i].indexOf('=');
      if (j > 0) {
        urlArgs[urlArgs2[i].substring(0, j)] = urlArgs2[i].substring(j + 1);
      }
    }
    ...
    if (url.path.startsWith('/signal') && urlArgs['phone'] != null && urlArgs['apikey'] != null) {
      return 'callmebot:signal|' + urlArgs['phone'] + '|' + urlArgs['apikey'];
    }
    if (url.path.startsWith('/whatsapp') && urlArgs['phone'] != null && urlArgs['apikey'] != null) {
      return 'callmebot:whatsapp|' + urlArgs['phone'] + '|' + urlArgs['apikey'];
    }
    if (url.path.startsWith('/facebook') && urlArgs['apikey'] != null) {
      return 'callmebot:facebook|' + urlArgs['apikey'];
    }
    if (url.path.startsWith('/text') && urlArgs['user'] != null) {
      return 'callmebot:telegram|' + urlArgs['user'];
    }
    return null;
  };
  ...
  function getTemplate(templateNumber, domain, lang) {
    ...
    if (Array.isArray(lang)) {
      lang = lang[0];
    } // TODO: For now, we only use the first language given.
    if (lang != null) {
      lang = lang.split('-')[0];
    } // Take the first part of the language, "xx-xx"

    var r = {},
      emailsPath = null;
    if (domain != null && domain.webemailspath != null) {
      emailsPath = domain.webemailspath;
    } else if (obj.parent.webEmailsOverridePath != null) {
      emailsPath = obj.parent.webEmailsOverridePath;
    } else if (obj.parent.webEmailsPath != null) {
      emailsPath = obj.parent.webEmailsPath;
    }
    if (emailsPath == null || obj.parent.fs.existsSync(emailsPath) == false) {
      return null;
    }
    ...
    if (txtfile == null && lang != null && lang != 'en') {
      ...
      var translationsPathTxt = obj.parent.path.join(
        obj.parent.webEmailsPath,
        'translations',
        'sms-messages_' + lang + '.txt'
      );
      if (obj.parent.fs.existsSync(translationsPath) && obj.parent.fs.existsSync(translationsPathTxt)) {
        txtfile = obj.parent.fs.readFileSync(translationsPathTxt).toString();
      }
  ...
  obj.sendMessagingCheck = function (domain, to, verificationCode, language, func) {
    ...
    if (sms == null) {
      parent.debug('email', 'Error: Failed to get SMS template');
      return;
  ...
  obj.sendDeviceNotify = function (domain, username, to, connections, disconnections, lang) {
    ...
    if (connections.length > 0) {
      sms.push('Connections: ' + connections.join(', '));
    } // TODO: Translate 'Connections: '
    if (disconnections.length > 0) {
      sms.push('Disconnections: ' + disconnections.join(', '));
    } // TODO: Translate 'Disconnections: '
    if (sms.length == 0) return;
    sms = sms.join(' - ');
    if (sms.length > 1000) {
      sms = sms.substring(0, 997) + '...';
  ...
  obj.notifyDeviceConnect = function (user, meshid, nodeid, connectTime, connectType, powerState, serverid, extraInfo) {
    ...
    if (obj.deviceNotifications[user._id] == null) {
      ...
      obj.deviceNotifications[user._id].timer = setTimeout(
        function () {
          sendDeviceNotifications(user._id);
        },
        1 * 60 * 1000
      ); // 1 minute before message is sent
    }
    ...
    if (extraInfo != null && extraInfo.name != null) {
      obj.deviceNotifications[user._id].nodes[nodeid].nn = extraInfo.name;
    }
    obj.deviceNotifications[user._id].nodes[nodeid].mn = mesh.name;
  };

  // Cancel a device disconnect notification
  obj.cancelNotifyDeviceDisconnect = function (
    user,
    meshid,
    nodeid,
    connectTime,
  ...
  obj.notifyDeviceDisconnect = function (
    user,
    meshid,
    nodeid,
    connectTime,
  ...
  obj.cancelNotifyDeviceConnect = function (
    user,
    meshid,
    nodeid,
    connectTime,
...
module.exports.SetupTelegram = async function (parent) {
  ...
  if (
    typeof parent.config.messaging != 'object' ||
    typeof parent.config.messaging.telegram != 'object' ||
    typeof parent.config.messaging.telegram.apiid != 'number' ||
    typeof parent.config.messaging.telegram.apihash != 'string'
  ) {
    console.log('Login to your Telegram account at this URL: https://my.telegram.org/.');
    console.log(
      'Click "API development tools" and fill your application details (only app title and short name required).'
    );
  ...
  if (
    (parent.config.messaging.telegram.session == null ||
      parent.config.messaging.telegram.session == '' ||
      typeof parent.config.messaging.telegram.session != 'string') &&
    (parent.config.messaging.telegram.bottoken == null ||
      parent.config.messaging.telegram.bottoken == '' ||
      typeof parent.config.messaging.telegram.bottoken != 'string')
  ) {
    if (parent.args.setuptelegram == 'user') {
      ...
      const client = new TelegramClient(
        ...
        parent.config.messaging.telegram.apiid,
        parent.config.messaging.telegram.apihash,
        { connectionRetries: 5, baseLogger: logger }
      );
      await client.start({
        phoneNumber: async function () {
          return await input.text('Please enter your number (+1-111-222-3333): ');
        },
