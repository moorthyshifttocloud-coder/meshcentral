...
module.exports.CreateFirebase = function (parent, serviceAccount) {
  ...
  if (parent.config.firebase && parent.config.firebase.log === true) {
    ...
    obj.log = function (msg) {
      try {
        parent.fs.appendFileSync(obj.logpath, new Date().toLocaleString() + ': ' + msg + '\r\n');
      } catch (ex) {
        console.log('ERROR: Unable to write to firebase.txt.');
      }
    };
  } else {
    obj.log = function () {};
  ...
  if (parent.config.firebaserelay && parent.config.firebaserelay.log === true) {
    ...
    obj.log = function (msg) {
      try {
        ...
        console.log('ERROR: Unable to write to firebaserelay.txt.');
      }
    };
  } else {
    obj.log = function () {};
  ...
  if (relayUrl.protocol == 'wss:') {
    ...
    obj.connectWebSocket = function () {
      ...
      obj.wsclient.on('close', function (a, b, c) {
        ...
        if (obj.reconnectTimer == null) {
          if (obj.lastConnect != null && Date.now() - obj.lastConnect > 10000) {
            obj.backoffTimer = 0;
          }
          obj.backoffTimer += 1000;
          obj.backoffTimer = obj.backoffTimer * 2;
          if (obj.backoffTimer > 1200000) {
            obj.backoffTimer = 600000;
          } // Maximum 10 minutes backoff.
    ...
    function processMessage(messageId, from, data, category) {
      ...
      if (ninfo == null) {
        obj.stats.receivedNoRoute++;
        return;
      }

      if (data != null && data.con != null && data.s != null) {
        // Console command
        obj.stats.received++;
        parent.webserver.routeAgentCommand(
          { action: 'msg', type: 'console', value: data.con, sessionid: data.s },
          ninfo.did,
          ninfo.nid,
          ninfo.mid
        );
    ...
    obj.sendToDeviceEx = function (node, payload, options, func) {
      ...
      if (node == null || typeof node.pmt != 'string') {
        func(0, 'error');
        return;
      }
      obj.log(
      ...
      // Fill in our lookup table
      if (node._id != null) {
        obj.tokenToNodeMap[node.pmt] = { nid: node._id, mid: node.meshid, did: node.domain };
      }
      ...
      if (payload.data.shash == null) {
        payload.data.shash = parent.webserver.agentCertificateHashBase64;
      } // Add the server agent hash, new Android agents will reject notifications that don't have this.

      // If the web socket is open, send now
      if (obj.wsopen == true) {
        try {
          obj.wsclient.send(JSON.stringify({ pmt: node.pmt, payload: payload, options: options }));
        } catch (ex) {
          func(0, 'error');
          obj.stats.sendError++;
          return;
    ...
    obj.sendToDeviceEx = function (node, payload, options, func) {
      ...
      const querydata = querystring.stringify({
        msg: JSON.stringify({ pmt: node.pmt, payload: payload, options: options })
      });
      ...
      const req = https.request(httpOptions, function (res) {
        ...
        if (func != null) {
          func(++obj.messageId, res.statusCode == 200 ? null : 'error');
        }
      });
      parent.debug('email', 'Firebase-sending');
      req.on('error', function (error) {
        obj.stats.sent++;
        func(++obj.messageId, 'error');
      });
      req.write(querydata);
      req.end();
    };
