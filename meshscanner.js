...
module.exports.CreateMeshScanner = function (parent) {
  ...
  obj.agentCertificateHashHex = parent.certificateOperations.forge.pki
    .getPublicKeyFingerprint(
      parent.certificateOperations.forge.pki.certificateFromPem(parent.certificates.agent.cert).publicKey,
      { md: parent.certificateOperations.forge.md.sha384.create(), encoding: 'hex' }
    )
    .toUpperCase();
  obj.error = 0;
  ...
  if (
    typeof obj.parent.args.localdiscovery == 'object' &&
    typeof obj.parent.args.localdiscovery.key == 'string' &&
    obj.parent.args.localdiscovery.key.length > 0
  ) {
    obj.multicastKey = parent.crypto
      .createHash('sha384')
      .update(obj.parent.args.localdiscovery.key)
      .digest('raw')
      .slice(0, 32);
  }
  ...
  function encryptPacket(plainPacket) {
    if (obj.multicastKey == null) {
      return plainPacket;
    }
    const iv = parent.crypto.randomBytes(16),
      aes = parent.crypto.createCipheriv('aes-256-cbc', obj.multicastKey, iv);
  ...
  function getInterfaceList() {
    var i;
    var ipv4 = ['*'],
      ipv6 = ['*']; // Bind to IN_ADDR_ANY always
    ...
    for (i in interfaces) {
      ...
      for (var j in xinterface) {
        ...
        if (interface2.mac != '00:00:00:00:00:00' && interface2.internal == false) {
          if (interface2.family == 'IPv4') {
            ipv4.push(interface2.address);
          }
          if (interface2.family == 'IPv6') {
            ipv6.push(interface2.address + '%' + i);
          }
        }
      }
    }
  ...
  function setupServers() {
    var addresses = getInterfaceList(),
      ...
      localAddress,
      bindOptions;
    for (i in obj.servers4) {
      obj.servers4[i].xxclear = true;
    }
    for (i in obj.servers6) {
      obj.servers6[i].xxclear = true;
    }
    for (i in addresses.ipv4) {
      ...
      if (obj.servers4[localAddress] != null) {
        ...
        try {
          ...
          server4.on('error', function (err) {
            /*if (this.xxlocal == '*') { console.log("ERROR: Server port 16989 not available, check if server is running twice."); } this.close(); delete obj.servers6[this.xxlocal];*/
          });
          bindOptions = { port: 16989, exclusive: true };
          if (server4.xxlocal != '*') {
            bindOptions.address = server4.xxlocal;
          }
          server4.bind(bindOptions, function () {
            try {
              ...
              try {
                ...
                this.addMembership(membershipIPv4, this.xxlocal);
              } catch (e) {
                doscan = false;
              }
              ...
              this.on('message', function (msg, info) {
                onUdpPacket(msg, info, this);
              });
              if (doscan == true) {
    ...
    for (i in addresses.ipv6) {
      ...
      if (obj.servers6[localAddress] != null) {
        ...
        try {
          ...
          server6.on('error', function (err) {
            /*this.close(); delete obj.servers6[this.xxlocal];*/
          });
          bindOptions = { port: 16989, exclusive: true };
          if (server6.xxlocal != '*') {
            bindOptions.address = server6.xxlocal;
          }
          server6.bind(bindOptions, function () {
            try {
              ...
              try {
                this.setBroadcast(true);
                this.setMulticastTTL(128);
                this.addMembership(membershipIPv6, this.xxlocal);
              } catch (e) {
                doscan = false;
              }
              this.on('error', function (error) {
    ...
    for (i in obj.servers4) {
      if (obj.servers4[i].xxclear == true) {
        obj.servers4[i].close();
        delete obj.servers4[i];
      }
    }
    for (i in obj.servers6) {
      if (obj.servers6[i].xxclear == true) {
        obj.servers6[i].close();
        delete obj.servers6[i];
      }
    }
  }
  ...
  obj.start = function () {
    ...
    try {
      if (typeof obj.parent.config.domains[''].title == 'string' && obj.parent.config.domains[''].title.length > 0) {
        name = obj.parent.config.domains[''].title;
        info = '';
        try {
          if (
            typeof obj.parent.config.domains[''].title2 == 'string' &&
            obj.parent.config.domains[''].title2.length > 0
          ) {
    ...
    try {
      if (typeof obj.parent.args.localdiscovery.name == 'string' && obj.parent.args.localdiscovery.name.length > 0) {
        name = obj.parent.args.localdiscovery.name;
        info = '';
        try {
          if (
            typeof obj.parent.args.localdiscovery.info == 'string' &&
            obj.parent.args.localdiscovery.info.length > 0
          ) {
            info = obj.parent.args.localdiscovery.info;
          }
        } catch (ex) {}
      }
    } catch (ex) {}
    if (info == '') {
      info = parent.certificates.CommonName;
    }
    ...
    if (parent.certificates.CommonName.indexOf('.') != -1) {
      url = 'wss://' + parent.certificates.CommonName + ':' + port + '/agent.ashx';
    }
    obj.multicastPacket4x = Buffer.from(
      'MeshCentral2|' + obj.agentCertificateHashHex + '|' + url + '|' + name + '|' + info,
      'ascii'
    );
    ...
    obj.multicastPacket6x = Buffer.from(
      'MeshCentral2|' + obj.agentCertificateHashHex + '|' + url + '|' + name + '|' + info,
      'ascii'
    );
  ...
  obj.performScan = function (server) {
    ...
    if (server != null) {
      if (server.xxtype == 4) {
        var p = encryptPacket(obj.multicastPacket4);
        try {
          server.send(p, 0, p.length, 16990, membershipIPv4);
        } catch (e) {}
      }
      if (server.xxtype == 6) {
        var p = encryptPacket(obj.multicastPacket6);
        try {
          server.send(p, 0, p.length, 16990, membershipIPv6);
        } catch (e) {}
      }
      if (server.xxtype == 4 && server.xxlocal == '*') {
        var p = encryptPacket(obj.multicastPacket4);
        try {
          server.send(p, 0, p.length, 16990, '127.0.0.1');
        } catch (e) {}
        try {
          server.send(p, 0, p.length, 16990, '255.255.255.255');
        } catch (e) {}
      }
      if (server.xxtype == 6 && server.xxlocal == '*') {
        var p = encryptPacket(obj.multicastPacket6);
        try {
          server.send(p, 0, p.length, 16990, '::1');
        } catch (e) {}
      }
    } else {
      for (i in obj.servers4) {
        var p = encryptPacket(obj.multicastPacket4);
        try {
          obj.servers4[i].send(p, 0, p.length, 16990, membershipIPv4);
        } catch (e) {}
      }
      for (i in obj.servers6) {
        var p = encryptPacket(obj.multicastPacket6);
        try {
          obj.servers6[i].send(p, 0, p.length, 16990, membershipIPv6);
        } catch (e) {}
      }
  ...
  function onUdpPacket(msg, info, server) {
    ...
    if (msg.length == 96 && msg.toString('ascii') == obj.agentCertificateHashHex) {
      if (server.xxtype == 4) {
        var p = encryptPacket(obj.multicastPacket4);
        try {
          server.send(p, 0, p.length, info.port, info.address);
        } catch (e) {}
      }
      if (server.xxtype == 6) {
        var p = encryptPacket(obj.multicastPacket6);
      ...
      if (server.xxtype == 4) {
        var p = encryptPacket(obj.multicastPacket4x);
        try {
          server.send(p, 0, p.length, info.port, info.address);
        } catch (e) {}
      }
      if (server.xxtype == 6) {
        var p = encryptPacket(obj.multicastPacket6x);
        try {
          server.send(p, 0, p.length, info.port, info.address);
        } catch (e) {}
      }
  ...
  function sendPendingPacket() {
    if (obj.pendingOutboundPackets.length == 0) {
      if (obj.pendingOutboundTimer != null) {
        clearInterval(obj.pendingOutboundTimer);
        obj.pendingOutboundTimer = null;
      }
      return;
    }
    ...
    if (packet != null) {
      packet[0].send(packet[1], 0, packet[1].length, packet[2], packet[3]);
    }
  }
    ...
    for (i in macs) {
      ...
      for (var k = 0; k < 5; k++) {
        for (j in obj.servers4) {
          ...
          if (host != null) {
            obj.pendingOutboundPackets.push([obj.servers4[j], wakepacket, 7, host]);
          } // IPv4 Directed
        }
      ...
      if (obj.pendingOutboundTimer == null) {
        obj.pendingOutboundTimer = setInterval(sendPendingPacket, 10);
      }
    }
  };
