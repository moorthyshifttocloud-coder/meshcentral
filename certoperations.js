...
module.exports.CertificateOperations = function (parent) {
  ...
  obj.dirExists = function (filePath) {
    try {
      return obj.fs.statSync(filePath).isDirectory();
    } catch (err) {
      return false;
    }
  };
  obj.getFilesizeInBytes = function (filename) {
    try {
      return obj.fs.statSync(filename).size;
    } catch (err) {
      return -1;
    }
  };
  ...
  obj.getAcmCertChain = function (domain, fqdn, hash) {
    if (
      ...
      return { action: 'acmactivate', error: 1, errorText: 'Invalid arguments' };
    if (parent.common.validateString(fqdn, 4, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid FQDN argument.' };
    if (parent.common.validateString(hash, 16, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid hash argument.' };
    ...
    for (var i in domain.amtacmactivation.certs) {
      ...
      if (certEntry.sha256 == hash && (certEntry.cn == '*' || checkAcmActivationCertName(certEntry.cn, fqdn))) {
        hashAlgo = 'sha256';
        signkey = certEntry.key;
        certChain = certEntry.certs;
        certIndex = i;
        break;
      }
      if (certEntry.sha1 == hash && (certEntry.cn == '*' || checkAcmActivationCertName(certEntry.cn, fqdn))) {
        hashAlgo = 'sha1';
        signkey = certEntry.key;
        certChain = certEntry.certs;
        certIndex = i;
    ...
    if (signkey == null)
      return {
        action: 'acmactivate',
        error: 2,
        errorText: "Can't create ACM cert chain, no signing certificate found."
    ...
    if (domain.amtacmactivation.certs[certIndex].cn == '*') {
      ...
      var rootcert = {
        cert: domain.amtacmactivation.certs[certIndex].rootcert,
        key: obj.pki.privateKeyFromPem(domain.amtacmactivation.certs[certIndex].key)
      };
      var leafcert = obj.IssueWebServerCertificate(
        rootcert,
        false,
        fqdn,
        'mc',
        'Intel(R) Client Setup Certificate',
        { serverAuth: true, '2.16.840.1.113741.1.2.3': true },
        false
      );

      // Setup the certificate chain and key
      certChain = [
        obj.pki.certificateToPem(leafcert.cert),
        obj.pki.certificateToPem(domain.amtacmactivation.certs[certIndex].rootcert)
      ];
      signkey = obj.pki.privateKeyToPem(leafcert.key);
    } else {
      // Make sure the cert chain is in PEM format
      var certChain2 = [];
      for (var i in certChain) {
        certChain2.push('-----BEGIN CERTIFICATE-----\r\n' + certChain[i] + '\r\n-----END CERTIFICATE-----\r\n');
      }
    ...
    return {
      ...
      certs: certChain,
      signkey: signkey,
      hash384: obj.getCertHash(certChain[0]),
      hash256: obj.getCertHashSha256(certChain[0])
    };
  };
  ...
  obj.signAcmRequest = function (domain, request, user, pass, ipport, nodeid, meshid, computerName, agentId) {
    if (
      ...
      return { action: 'acmactivate', error: 1, errorText: 'Invalid arguments' };
    if (parent.common.validateString(request.nonce, 16, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid nonce argument.' };
    if (parent.common.validateString(request.realm, 16, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid realm argument.' };
    if (parent.common.validateString(request.fqdn, 4, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid FQDN argument.' };
    if (parent.common.validateString(request.hash, 16, 256) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid hash argument.' };
    if (parent.common.validateString(request.uuid, 36, 36) == false)
      return { action: 'acmactivate', error: 1, errorText: 'Invalid UUID argument.' };
    ...
    for (var i in domain.amtacmactivation.certs) {
      const certEntry = domain.amtacmactivation.certs[i];
      if (
        certEntry.sha256 == request.hash &&
        (certEntry.cn == '*' || checkAcmActivationCertName(certEntry.cn, request.fqdn))
      ) {
        hashAlgo = 'sha256';
        signkey = certEntry.key;
        certChain = certEntry.certs;
    ...
    if (signkey == null)
      return { action: 'acmactivate', error: 2, errorText: "Can't sign ACM request, no signing certificate found." }; // Did not find a match.
    ...
    if (domain.amtacmactivation.certs[certIndex].cn == '*') {
      ...
      // Setup the certificate chain and key
      certChain = [
        pemToBase64(obj.pki.certificateToPem(leafcert.cert)),
        pemToBase64(obj.pki.certificateToPem(domain.amtacmactivation.certs[certIndex].rootcert))
      ];
      signkey = obj.pki.privateKeyToPem(leafcert.key);
    }
    ...
    if (
      obj.logAmtActivation(domain, {
        ...
        action: 'acmactivate',
        domain: domain.id,
        amtUuid: request.uuid,
        certHash: request.hash,
        hashType: hashAlgo,
        amtRealm: request.realm,
        amtFqdn: request.fqdn,
        user: user,
        password: pass,
        ipport: ipport,
        nodeid: nodeid,
      ...
      return { action: 'acmactivate', error: 5, errorText: 'Unable to log operation.' };
    ...
    return {
      ...
      password: obj.crypto
        .createHash('md5')
        .update(user + ':' + request.realm + ':' + pass)
        .digest('hex'),
      nonce: mcNonce.toString('base64'),
      certs: certChain
    };
  };
  ...
  obj.logAmtActivation = function (domain, x) {
    ...
    if (typeof x.amtUuid == 'string' && typeof x.password == 'string') {
      ...
      if (parent.amtPasswords[x.amtUuid] == null) {
        ...
        if (parent.amtPasswords[x.amtUuid].indexOf(x.password) == -1) {
          ...
          while (parent.amtPasswords[x.amtUuid].length > 3) {
            parent.amtPasswords[x.amtUuid].pop();
          } // Only keep the 3 last passwords for any given device
        }
      }
    }
    ...
    if (
      domain.amtacmactivation == null ||
      domain.amtacmactivation.log == null ||
      typeof domain.amtacmactivation.log != 'string'
    ) {
      if (domain.id == '') {
        logpath = parent.path.join(obj.parent.datapath, 'amtactivation.log');
      } else {
        logpath = parent.path.join(obj.parent.datapath, 'amtactivation-' + domain.id + '.log');
      }
    } else {
      logpath = parent.common.joinPath(obj.parent.datapath, domain.amtacmactivation.log);
    }
    try {
      obj.fs.appendFileSync(logpath, JSON.stringify(x) + '\r\n');
    } catch (ex) {
      console.log(ex);
      return false;
    }
  ...
  obj.loadIntelAmtAcmCerts = function (amtacmactivation) {
    ...
    if (amtacmactivation.certs != null) {
      for (var j in amtacmactivation.certs) {
        ...
        if (typeof acmconfig.certpfx == 'string' && typeof acmconfig.certpfxpass == 'string') {
          ...
          try {
            r = obj.loadPfxCertificate(certFilePath, acmconfig.certpfxpass);
          } catch (ex) {
            console.log(ex);
          }
          if (r == null || r.certs == null || r.keys == null) {
            amtacmactivation.acmCertErrors.push('Unable to load certificate file: ' + certFilePath + '.');
            continue;
          }
          if (r.certs.length < 2) {
            amtacmactivation.acmCertErrors.push(
              'Certificate file contains less then 2 certificates: ' + certFilePath + '.'
            );
            continue;
          }
          if (r.keys.length != 1) {
            amtacmactivation.acmCertErrors.push(
              'Certificate file must contain exactly one private key: ' + certFilePath + '.'
            );
            continue;
          }
          ...
          for (var k in acmconfig.certfiles) {
            ...
            try {
              r.certs.push(obj.pki.certificateFromPem(obj.fs.readFileSync(certFilePath)));
            } catch (ex) {
              amtacmactivation.acmCertErrors.push('Unable to load certificate file: ' + certFilePath + '.');
            }
          }
          r.keys.push(
            obj.pki.privateKeyFromPem(
              obj.fs.readFileSync(parent.common.joinPath(obj.parent.datapath, acmconfig.keyfile))
            )
          );
          if (r.certs.length < 2) {
            amtacmactivation.acmCertErrors.push(
        ...
        while (orderingError == false && orderedCerts.length < r.certs.length) {
          ...
          for (var k in r.certs) {
            if (
              (currenthash == null && r.certs[k].subject.hash == r.certs[k].issuer.hash) ||
              (r.certs[k].issuer.hash == currenthash && r.certs[k].subject.hash != r.certs[k].issuer.hash)
            ) {
              currenthash = r.certs[k].subject.hash;
              orderedCerts.unshift(
                Buffer.from(obj.forge.asn1.toDer(obj.pki.certificateToAsn1(r.certs[k])).data, 'binary').toString(
                  'base64'
                )
              );
        ...
        if (orderingError == true) {
          amtacmactivation.acmCertErrors.push(
            'Unable to order Intel AMT ACM activation certificates to create a full chain.'
          );
          continue;
        }
        r.certs = or;
        ...
        if (
          compareArrays(r.certs[0].publicKey.n.data, r.keys[0].n.data) == false ||
          compareArrays(r.certs[0].publicKey.e.data, r.keys[0].e.data) == false
        ) {
          amtacmactivation.acmCertErrors.push(
            'Intel AMT activation certificate provided with a mismatching private key.'
          );
          continue;
        }
        ...
        for (var k in r.certs[0].extensions) {
          if (r.certs[0].extensions[k]['2.16.840.1.113741.1.2.3'] == true) {
        ...
        if (validActivationCert == false) {
          amtacmactivation.acmCertErrors.push(
            'Intel AMT activation certificate must have usage OID "2.16.840.1.113741.1.2.3" or organization name "Intel(R) Client Setup Certificate".'
          );
          continue;
        }
        ...
        if (acmconfig.sha1 == null || acmconfig.sha256 == null) {
          amtacmactivation.acmCertErrors.push(
            'Unable to compute Intel AMT activation certificate SHA1 and SHA256 hashes.'
          );
          continue;
        }
        ...
        if (certCommonName == null) {
          amtacmactivation.acmCertErrors.push('Unable to get Intel AMT activation certificate common name.');
          continue;
        }
        if (amtacmactivation.strictcommonname == true) {
          ...
          if (topLevelNum != null) {
            while (certCommonNameSplit.length > topLevelNum) {
              certCommonNameSplit.shift();
            }
    ...
    if (obj.parent.certificates.root) {
      var x1 = obj.parent.certificates.root.cert.indexOf('-----BEGIN CERTIFICATE-----'),
        x2 = obj.parent.certificates.root.cert.indexOf('-----END CERTIFICATE-----');
      if (x1 >= 0 && x2 > x1) {
        var sha256 = obj.crypto
          .createHash('sha256')
          .update(Buffer.from(obj.parent.certificates.root.cert.substring(x1 + 27, x2), 'base64'))
          .digest('hex');
        var sha1 = obj.crypto
          .createHash('sha1')
          .update(Buffer.from(obj.parent.certificates.root.cert.substring(x1 + 27, x2), 'base64'))
          .digest('hex');
        amtacmactivation.certs.push({
          sha256: sha256,
          sha1: sha1,
          cn: '*',
          rootcert: obj.pki.certificateFromPem(obj.parent.certificates.root.cert),
          key: obj.parent.certificates.root.key
        });
  ...
  obj.loadGenericCertAndKey = function (config) {
    if (typeof config.certpfx == 'string' || typeof config.certpfxpass == 'string') {
      ...
      var r = null;
      try {
        r = obj.loadPfxCertificate(parent.getConfigFilePath(config.certpfx), config.certpfxpass);
      } catch (ex) {
        console.log(ex);
      }
      if (r != null && r.keys.length > 0 && r.certs.length > 0) {
        var attributes = {};
        for (var j in r.certs[0].subject.attributes) {
          attributes[r.certs[0].subject.attributes[j].shortName] = r.certs[0].subject.attributes[j].value;
        }
        return {
          cert: obj.pki.certificateToPem(r.certs[0]),
          key: obj.pki.privateKeyToPem(r.keys[0]),
    ...
    if (typeof config.certfile == 'string' || typeof config.keyfile == 'string') {
      ...
      r.attributes = {};
      for (var j in cert.subject.attributes) {
        r.attributes[cert.subject.attributes[j].shortName] = cert.subject.attributes[j].value;
      }
      return r;
    }
    return null;
  ...
  obj.GetSetupBinFile = function (amtacmactivation, oldmebxpass, newmebxpass, domain, user) {
    ...
    for (var i = 0; i < amtacmactivation.acmmatch.length; i++) {
      if (amtacmactivation.acmmatch[i].cn == '*') {
        wildcardCertSha256 = amtacmactivation.acmmatch[i].sha256;
      }
    }
    ...
    v.value =
      ...
      Buffer.from(wildcardCertSha256, 'hex').toString('binary') +
      String.fromCharCode(certRootName.length) +
      certRootName; // 2 = SHA256 hash type
    setupbin.records[0].variables.push(v);
  ...
  obj.GetBareMetalSetupBinFile = function (amtacmactivation, oldmebxpass, newmebxpass, domain, user) {
    ...
    var port = 9971;
    if (typeof parent.config.settings.amtprovisioningserver.port == 'number') {
      port = parent.config.settings.amtprovisioningserver.port;
    }
    ...
    var ipaddrStr =
      String.fromCharCode(parseInt(ipaddrSplit[3])) +
      String.fromCharCode(parseInt(ipaddrSplit[2])) +
      String.fromCharCode(parseInt(ipaddrSplit[1])) +
      String.fromCharCode(parseInt(ipaddrSplit[0]));
  ...
  obj.loadPfxCertificate = function (filename, password) {
    ...
    for (var i = 0; i < bags[obj.forge.pki.oids.certBag].length; i++) {
      r.certs.push(bags[obj.forge.pki.oids.certBag][i].cert);
    }

    // Get shrouded key from key bags
    bags = pfx.getBags({ bagType: obj.forge.pki.oids.pkcs8ShroudedKeyBag });
    for (var i = 0; i < bags[obj.forge.pki.oids.pkcs8ShroudedKeyBag].length; i++) {
      r.keys.push(bags[obj.forge.pki.oids.pkcs8ShroudedKeyBag][i].key);
    }
    return r;
  };
  ...
  obj.loadCertificate = function (url, hostname, tag, func) {
    ...
    if (u.protocol == 'https:') {
      ...
      parent.debug(
        'cert',
        'loadCertificate() - Loading certificate from ' +
          u.hostname +
          ':' +
          (u.port ? u.port : 443) +
          ', Hostname: ' +
      ...
      const tlssocket = obj.tls.connect(
        u.port ? u.port : 443,
        u.hostname,
        ...
        function () {
          ...
          parent.debug(
            'cert',
            'loadCertificate() - TLS connected, ' + (this.xxcert != null ? 'got certificate.' : 'no certificate.')
          );
          try {
            this.destroy();
          } catch (ex) {}
          this.xxfunc(
            this.xxurl,
            this.xxcert == null ? null : this.xxcert.raw.toString('binary'),
            hostname,
            this.xxtag
          );
        }
      ...
      tlssocket.on('error', function (error) {
        ...
        parent.debug('cert', 'loadCertificate() - TLS error: ' + error);
        this.xxfunc(this.xxurl, null, hostname, this.xxtag);
      });
    } else if (u.protocol == 'file:') {
      // Read the certificate from a file
      obj.fs.readFile(url.substring(7), 'utf8', function (err, data) {
        if (err) {
          func(url, null, hostname, tag);
          return;
        }
        var x1 = data.indexOf('-----BEGIN CERTIFICATE-----'),
          x2 = data.indexOf('-----END CERTIFICATE-----');
        if (x1 >= 0 && x2 > x1) {
  ...
  obj.fileExists = function (filename) {
    ...
    try {
      return obj.fs.statSync(filePath).isFile();
    } catch (err) {
      return false;
    }
  };
  ...
  obj.fileLoad = function (filename, encoding) {
    if (parent.configurationFiles != null && parent.configurationFiles[filename] != null) {
      if (typeof parent.configurationFiles[filename] == 'string') {
  ...
  obj.getCertHashSha1 = function (cert) {
    try {
      ...
      var x1 = cert.indexOf('-----BEGIN CERTIFICATE-----'),
        x2 = cert.indexOf('-----END CERTIFICATE-----');
      if (x1 >= 0 && x2 > x1) {
        return obj.crypto
          .createHash('sha1')
          .update(Buffer.from(cert.substring(x1 + 27, x2), 'base64'))
          .digest('hex');
      } else {
  ...
  obj.getPublicKeyHashBinary = function (pem) {
    ...
    if (X509Certificate == null) {
      ...
      return obj.pki.getPublicKeyFingerprint(obj.pki.certificateFromPem(pem).publicKey, {
        encoding: 'binary',
        md: obj.forge.md.sha384.create()
      });
    } else {
      ...
      return obj.crypto
        .createHash('sha384')
        .update(
          cert.publicKey.export({ type: cert.publicKey.asymmetricKeyType == 'rsa' ? 'pkcs1' : 'spki', format: 'der' })
        )
        .digest('binary');
    }
  };
  ...
  obj.GenerateRootCertificate = function (addThumbPrintToName, commonName, country, organization, strong) {
    ...
    if (addThumbPrintToName === true) {
      commonName += '-' + obj.pki.getPublicKeyFingerprint(cert.publicKey, { encoding: 'hex' }).substring(0, 6);
    }
    if (country == null) {
      country = 'unknown';
    }
  ...
  obj.IssueWebServerCertificate = function (
    rootcert,
    addThumbPrintToName,
    commonName,
    country,
    ...
    var extensions = [
      { name: 'basicConstraints', cA: false },
      {
        ...
        dataEncipherment: extKeyUsage.serverAuth !== true
      },
      extKeyUsage,
      { name: 'subjectKeyIdentifier' }
    ];
  ...
  function fixEndOfLines(str) {
    ...
    if (i > 0) {
      str = str.substring(i);
    } // this solves problems with editors that save text file type indicators ahead of the text.
  ...
  obj.compareCertificateNames = function (certNames, name) {
    ...
    for (var i in certNames) {
      xcertNames.push(certNames[i].toLowerCase());
    }
    if (xcertNames.indexOf(name) >= 0) return true;
    for (var i in xcertNames) {
      if (xcertNames[i].startsWith('*.') == true && name.endsWith(xcertNames[i].substring(1)) == true) {
        return true;
      }
      if (xcertNames[i].startsWith('http://*.') == true) {
        if (name.endsWith(xcertNames[i].substring(8)) == true) {
          return true;
        }
        if (
          xcertNames[i].endsWith('/') == true &&
          name.endsWith(xcertNames[i].substring(8, xcertNames[i].length - 1)) == true
        ) {
          return true;
        }
      }
  ...
  obj.checkCertificate = function (pem, key) {
    ...
    if (X509Certificate == null) {
      ...
      var cert = null;
      try {
        cert = obj.pki.certificateFromPem(pem);
      } catch (ex) {
        return false;
  ...
  obj.getCertificateCommonName = function (pem, field) {
    ...
    if (X509Certificate == null) {
      ...
      for (var i in subjects) {
        if (subjects[i].startsWith(field + '=')) {
          return subjects[i].substring(field.length + 1);
        }
      }
    }
    return null;
  ...
  obj.getCertificateAltNames = function (pem) {
    ...
    if (X509Certificate == null) {
      // This version of NodeJS (<v15.6.0) does not support X509 certs, use Node-Forge instead which only supports RSA certs.
      var cert = obj.pki.certificateFromPem(pem);
      if (cert.subject.getField('CN') != null) {
        altNamesResults.push(cert.subject.getField('CN').value);
      }
      var altNames = cert.getExtension('subjectAltName');
      if (altNames) {
        for (i = 0; i < altNames.altNames.length; i++) {
          if (
            altNames.altNames[i] != null &&
            altNames.altNames[i].type === 2 &&
            typeof altNames.altNames[i].value === 'string'
          ) {
            var acn = altNames.altNames[i].value.toLowerCase();
            if (altNamesResults.indexOf(acn) == -1) {
              altNamesResults.push(acn);
            }
          }
        }
      }
      ...
      for (var i in subjects) {
        if (subjects[i].startsWith('CN=')) {
          altNamesResults.push(subjects[i].substring(3));
        }
      }
  ...
  obj.getCertificateExpire = function (pem) {
    ...
    if (X509Certificate == null) {
      // This version of NodeJS (<v15.6.0) does not support X509 certs, use Node-Forge instead which only supports RSA certs.
      return Date.parse(
        parent.certificateOperations.forge.pki.certificateFromPem(parent.certificates.web.cert).validity.notAfter
      );
    } else {
      // This version of NodeJS supports x509 certificates
  ...
  obj.decryptPrivateKey = function (key) {
    ...
    if (i >= 0 && j > i) {
      ...
      if (parent.config.settings.certificateprivatekeypassword == null) {
        passwords = [];
      } else if (typeof parent.config.settings.certificateprivatekeypassword == 'string') {
        passwords = [parent.config.settings.certificateprivatekeypassword];
      }
      var privateKey = null;
      for (var k in passwords) {
        if (privateKey == null) {
          try {
            privateKey = obj.pki.decryptRsaPrivateKey(key, passwords[k]);
          } catch (ex) {}
        }
      }
      if (privateKey == null) {
        ...
        console.log(
          'Add the password to the "certificatePrivateKeyPassword" value in the Settings section of the config.json.'
        );
  ...
  obj.GetMeshServerCertificate = function (args, config, func) {
    ...
    if (!obj.dirExists(parent.datapath)) {
      obj.fs.mkdirSync(parent.datapath);
    }
    var r = {};
    var rcount = 0;
    ...
    if (obj.fileExists('root-cert-public.crt') && obj.fileExists('root-cert-private.key')) {
      ...
      if (xext == null || xext.keyCertSign !== true || xroot.serialNumber == '') {
        ...
        if (xroot.serialNumber == '') {
          console.log('Fixing root certificate to add serial number...');
          xroot.serialNumber = '' + require('crypto').randomBytes(4).readUInt32BE(0);
        }
        ...
        try {
          obj.fs.writeFileSync(parent.getConfigFilePath('root-cert-public.crt'), r.root.cert);
        } catch (ex) {}
      }
    }
    ...
    if (obj.fileExists('webserver-cert-public.crt') && obj.fileExists('webserver-cert-private.key')) {
      r.webdefault = {
        cert: obj.fileLoad('webserver-cert-public.crt', 'utf8'),
        key: obj.decryptPrivateKey(obj.fileLoad('webserver-cert-private.key', 'utf8'))
      };
      if (obj.checkCertificate(r.webdefault.cert, r.webdefault.key) == false) {
        delete r.webdefault;
      }
    }
    ...
    if (args.tlsoffload) {
      ...
      if (obj.fileExists('webserver-cert-public.crt')) {
        ...
        if (obj.checkCertificate(r.web.cert, null) == false) {
          delete r.web;
        } else {
          rcount++;
        }
      ...
      if (
        obj.fileExists('webserver-cert-public.crt') &&
        obj.decryptPrivateKey(obj.fileExists('webserver-cert-private.key'))
      ) {
        r.web = {
          cert: obj.fileLoad('webserver-cert-public.crt', 'utf8'),
          key: obj.decryptPrivateKey(obj.fileLoad('webserver-cert-private.key', 'utf8'))
        };
        if (obj.checkCertificate(r.web.cert, r.web.key) == false) {
          delete r.web;
        } else {
          rcount++;
        }
    ...
    if (obj.fileExists('mpsserver-cert-public.crt') && obj.fileExists('mpsserver-cert-private.key')) {
      r.mps = {
        cert: obj.fileLoad('mpsserver-cert-public.crt', 'utf8'),
        key: obj.decryptPrivateKey(obj.fileLoad('mpsserver-cert-private.key', 'utf8'))
      };
      if (obj.checkCertificate(r.mps.cert, r.mps.key) == false) {
        delete r.mps;
      } else {
        rcount++;
      }
    ...
    if (obj.fileExists('agentserver-cert-public.crt') && obj.fileExists('agentserver-cert-private.key')) {
      r.agent = {
        cert: obj.fileLoad('agentserver-cert-public.crt', 'utf8'),
        key: obj.decryptPrivateKey(obj.fileLoad('agentserver-cert-private.key', 'utf8'))
      };
      if (obj.checkCertificate(r.agent.cert, r.agent.key) == false) {
        delete r.agent;
      } else {
        rcount++;
      }
    ...
    if (obj.fileExists('codesign-cert-public.crt') && obj.fileExists('codesign-cert-private.key')) {
      r.codesign = {
        cert: obj.fileLoad('codesign-cert-public.crt', 'utf8'),
        key: obj.decryptPrivateKey(obj.fileLoad('codesign-cert-private.key', 'utf8'))
      };
      if (obj.checkCertificate(r.codesign.cert, r.codesign.key) == false) {
        delete r.codesign;
      } else {
        rcount++;
      }
    ...
    if (obj.fileExists('swarmserver-cert-public.crt') && obj.fileExists('swarmserver-cert-private.key')) {
      r.swarmserver = {
        cert: obj.fileLoad('swarmserver-cert-public.crt', 'utf8'),
        key: obj.decryptPrivateKey(obj.fileLoad('swarmserver-cert-private.key', 'utf8'))
      };
      if (obj.checkCertificate(r.swarmserver.cert, r.swarmserver.key) == false) {
        delete r.swarmserver;
      }
    }
    ...
    if (obj.fileExists('swarmserverroot-cert-public.crt')) {
      ...
      if (obj.checkCertificate(r.swarmserverroot.cert, null) == false) {
        delete r.swarmserverroot;
      }
    }
    ...
    if (certargs != undefined) {
      ...
      if (xargs.length > 2) {
        organization = xargs[2];
      }
    }

    // Decode MPS certificate arguments, this is for the Intel AMT CIRA server
    var mpsCommonName =
      config.settings != null && typeof config.settings.mpsaliashost == 'string'
        ? config.settings.mpsaliashost
        : commonName;
    ...
    if (mpscertargs !== undefined) {
      ...
      if (xxargs.length > 0) {
        mpsCommonName = xxargs[0];
      }
      if (xxargs.length > 1) {
        mpsCountry = xxargs[1];
      }
      if (xxargs.length > 2) {
        mpsOrganization = xxargs[2];
      }
    }
    ...
    if (rcount === rcountmax) {
      ...
      if (commonName == 'un-configured') {
        if (r.CommonName.startsWith('*.')) {
          console.log(
            'ERROR: Must specify a server full domain name in Config.json->Settings->Cert when using a wildcard certificate.'
          );
          process.exit(0);
          return;
        }
        ...
        if (obj.compareCertificateNames(r.CommonNames, dnsname) == true) {
          ...
          if (args.tlsoffload) {
            ...
            if (obj.fileExists('webserver-' + i + '-cert-public.crt')) {
              ...
              console.log(
                'WARNING: File "webserver-' +
                  i +
                  '-cert-public.crt" missing, domain "' +
                  i +
            ...
            // If the web certificate already exist, load it. Load both certificate and private key
            if (
              obj.fileExists('webserver-' + i + '-cert-public.crt') &&
              obj.fileExists('webserver-' + i + '-cert-private.key')
            ) {
              r.dns[i] = {
                cert: obj.fileLoad('webserver-' + i + '-cert-public.crt', 'utf8'),
                key: obj.decryptPrivateKey(obj.fileLoad('webserver-' + i + '-cert-private.key', 'utf8'))
              };
              config.domains[i].certs = r.dns[i];
              // If CA certificates are present, load them
              caindex = 1;
    ...
    if (rcount === rcountmax) {
      if (certargs == null && mpscertargs == null) {
        if (func != undefined) {
          func(r);
        }
        return r;
      } // If no certificate arguments are given, keep the certificate
      ...
      if (obj.compareCertificateNames(r.CommonNames, commonName) == false) {
        console.log('Error: ' + commonName + ' does not match name in TLS certificate: ' + r.CommonNames.join(', '));
        forceWebCertGen = 1;
      } else {
        r.CommonName = commonName;
      }
      if (r.AmtMpsName != mpsCommonName) {
        forceMpsCertGen = 1;
      }
      if (r.CodeCertName.startsWith(commonName) === false) {
        forceCodeCertGen = 1;
      }
      if (args.keepcerts == true) {
        forceWebCertGen = 0;
        forceMpsCertGen = 0;
        forceCodeCertGen = 0;
        r.CommonName = commonName;
    ...
    if (parent.configurationFiles != null) {
      ...
      if (r.root == null) {
        console.log('  Code signing certificate is missing.');
      }
      if (r.web == null) {
        console.log('  HTTPS web certificate is missing.');
      }
      if (r.mps == null) {
        console.log('  Intel AMT MPS certificate is missing.');
      }
      if (r.agent == null) {
        console.log('  Server agent authentication certificate is missing.');
      }
      if (r.codesign == null) {
        console.log('  Agent code signing certificate is missing.');
      }
      process.exit(0);
      return null;
    }
    ...
    if (certargs == null && r.web != null) {
      ...
      if (xcountryField != null) {
        country = xcountryField.value;
      }
      var xorganizationField = webCertificate.subject.getField('O');
      if (xorganizationField != null) {
        organization = xorganizationField.value;
      }
    }
    ...
    if (r.root == null) {
      ...
      parent.common.moveOldFiles([
        parent.getConfigFilePath('root-cert-public.crt'),
        parent.getConfigFilePath('root-cert-private.key')
      ]);
    ...
    if (r.web == null || forceWebCertGen === 1) {
      ...
      webCertAndKey = obj.IssueWebServerCertificate(
        rootCertAndKey,
        false,
        commonName,
        country,
      ...
      parent.common.moveOldFiles([
        parent.getConfigFilePath('webserver-cert-public.crt'),
        parent.getConfigFilePath('webserver-cert-private.key')
      ]);
    ...
    var webIssuer = null;
    if (webCertAndKey.cert.issuer.getField('CN') != null) {
      webIssuer = webCertAndKey.cert.issuer.getField('CN').value;
    }
    ...
    if (r.agent == null) {
      ...
      agentCertAndKey = obj.IssueWebServerCertificate(
        rootCertAndKey,
        true,
        'MeshCentralAgentServer',
        country,
        organization,
        {},
        strongCertificate
      ...
      parent.common.moveOldFiles([
        parent.getConfigFilePath('agentserver-cert-public.crt'),
        parent.getConfigFilePath('agentserver-cert-private.key')
      ]);
    ...
    if (r.codesign == null || forceCodeCertGen === 1) {
      ...
      codesignCertAndKey = obj.IssueWebServerCertificate(
        rootCertAndKey,
        true,
        commonName,
        country,
      ...
      parent.common.moveOldFiles([
        parent.getConfigFilePath('codesign-cert-public.crt'),
        parent.getConfigFilePath('codesign-cert-private.key')
      ]);
      ...
      codesignCertAndKey = {
        cert: obj.pki.certificateFromPem(r.codesign.cert),
        key: obj.pki.privateKeyFromPem(r.codesign.key)
      };
    ...
    if (r.mps == null || forceMpsCertGen === 1) {
      ...
      mpsCertAndKey = obj.IssueWebServerCertificate(
        rootCertAndKey,
        false,
        mpsCommonName,
        mpsCountry,
      ...
      parent.common.moveOldFiles([
        parent.getConfigFilePath('mpsserver-cert-public.crt'),
        parent.getConfigFilePath('mpsserver-cert-private.key')
      ]);
    ...
    r = {
      root: { cert: rootCertificate, key: rootPrivateKey },
      web: { cert: webCertificate, key: webPrivateKey, ca: [] },
      webdefault: { cert: webCertificate, key: webPrivateKey, ca: [] },
      mps: { cert: mpsCertificate, key: mpsPrivateKey },
      agent: { cert: agentCertificate, key: agentPrivateKey },
      codesign: { cert: codesignCertificate, key: codesignPrivateKey },
      ca: calist,
      CommonName: commonName,
      RootName: rootName,
      AmtMpsName: mpsCommonName,
      dns: {},
      WebIssuer: webIssuer
    };
    ...
    // Fetch the certificates names for the main certificate
    var webCertificate = obj.pki.certificateFromPem(r.web.cert);
    if (webCertificate.issuer.getField('CN') != null) {
    ...
    if (r.CommonName.startsWith('*.')) {
      ...
      if (commonName.startsWith('*.')) {
        console.log("ERROR: Server can't use a wildcard name: " + commonName);
        process.exit(0);
        return;
      }
      r.CommonName = commonName;
    ...
    if (altNames) {
      for (i = 0; i < altNames.altNames.length; i++) {
        if (
          ...
          var acn = altNames.altNames[i].value.toLowerCase();
          if (r.CommonNames.indexOf(acn) == -1) {
            r.CommonNames.push(acn);
          }
        }
      }
    }
      ...
      if (i != '' && config.domains[i] != null && config.domains[i].dns != null) {
        ...
        if (obj.compareCertificateNames(r.CommonNames, dnsname) == true) {
          ...
          if (!args.tlsoffload) {
            ...
            if (
              obj.fileExists('webserver-' + i + '-cert-public.crt') === false ||
              obj.fileExists('webserver-' + i + '-cert-private.key') === false
            ) {
              console.log('Generating HTTPS certificate for ' + i + '...');
              var xwebCertAndKey = obj.IssueWebServerCertificate(
                rootCertAndKey,
                false,
                dnsname,
                country,
              ...
              parent.common.moveOldFiles([
                parent.getConfigFilePath('webserver-' + i + '-cert-public.crt'),
                parent.getConfigFilePath('webserver-' + i + '-cert-private.key')
              ]);
              ...
              do {
                ...
                if (obj.fileExists('webserver-' + i + '-cert-chain' + caindex + '.crt')) {
                  r.dns[i].ca.push(
                    fixEndOfLines(
                      obj.fs.readFileSync(
                        parent.getConfigFilePath('webserver-' + i + '-cert-chain' + caindex + '.crt'),
                        'utf8'
                      )
                    )
                  );
    ...
    if (obj.fileExists('swarmserver-cert-public.crt') && obj.fileExists('swarmserver-cert-private.key')) {
      r.swarmserver = {
        cert: fixEndOfLines(obj.fs.readFileSync(parent.getConfigFilePath('swarmserver-cert-public.crt'), 'utf8')),
        key: fixEndOfLines(obj.fs.readFileSync(parent.getConfigFilePath('swarmserver-cert-private.key'), 'utf8'))
      };
    }
    ...
    if (obj.fileExists('swarmserverroot-cert-public.crt')) {
      r.swarmserverroot = {
        cert: fixEndOfLines(obj.fs.readFileSync(parent.getConfigFilePath('swarmserverroot-cert-public.crt'), 'utf8'))
      };
    }
    ...
    if (r.web != null) {
      ...
      do {
        ...
        if (obj.fileExists('webserver-cert-chain' + caindex + '.crt')) {
          r.web.ca.push(
            fixEndOfLines(
              obj.fs.readFileSync(parent.getConfigFilePath('webserver-cert-chain' + caindex + '.crt'), 'utf8')
            )
          );
          caok = true;
        }
  ...
  obj.getAccelerator = function () {
    ...
    if (obj.acceleratorCertStore == null) {
      return null;
    }
    if (freeAccelerators.length > 0) {
      return freeAccelerators.pop();
    }
    if (acceleratorCreateCount > 0) {
      ...
      accelerator.on('message', function (message) {
        ...
        if (pendingAccelerator.length > 0) {
          this.send((this.x = pendingAccelerator.shift()));
        } else {
          freeAccelerators.push(this);
        }
      });
      accelerator.on('exit', function (code) {
        if (this.x) {
          pendingAccelerator.push(this.x);
          delete this.x;
        }
        acceleratorCreateCount++;
        if (pendingAccelerator.length > 0) {
          var acc = obj.getAccelerator();
          acc.send((acc.x = pendingAccelerator.shift()));
        }
      });
  ...
  obj.acceleratorStart = function (certificates) {
    ...
    if (obj.acceleratorCertStore != null) {
      console.error('ERROR: Accelerators can only be started once.');
      return;
    }
    obj.acceleratorCertStore = [{ cert: certificates.agent.cert, key: certificates.agent.key }];
    if (certificates.swarmserver != null) {
      obj.acceleratorCertStore.push({ cert: certificates.swarmserver.cert, key: certificates.swarmserver.key });
    }
  };
  ...
  obj.acceleratorPerformSignature = function (privatekey, data, tag, func) {
    ...
    if (acceleratorTotalCount <= 1) {
      ...
      if (typeof privatekey == 'number') {
        privatekey = obj.acceleratorCertStore[privatekey].key;
      }
      ...
      try {
        ...
        acceleratorMessageException++;
        acceleratorMessageLastException = ex;
      }
    } else {
      var acc = obj.getAccelerator();
      if (acc == null) {
