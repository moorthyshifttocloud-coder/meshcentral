...
function CreateIPKVMManager(parent) {
  ...
  for (var i in parent.webserver.meshes) {
    ...
    if (mesh.mtype == 4 && mesh.deleted == null) {
      startManagement(mesh);
    }
  }
  ...
  function startManagement(mesh) {
    if (
      ...
      mesh.kvm == null ||
      mesh.deleted != null ||
      obj.managedGroups[mesh._id] != null
    )
      return;
    var port = 443,
      hostSplit = mesh.kvm.host.split(':'),
      host = hostSplit[0];
  ...
  function changeManagementRelayId(meshid, relayid) {
    const manager = obj.managedGroups[meshid];
    if (manager != null && manager.relayid != null && manager.relayid != relayid) {
      manager.updateRelayId(relayid);
    }
  }
    ...
    for (var i = 0; i < updatedPorts.length; i++) {
      ...
      if (port.Status == 1 && port.Class == 'PDU') {
        ...
        if (obj.managedPorts[nodeid] == null || obj.managedPorts[nodeid].name != port.Name) {
          parent.db.Get(nodeid, function (err, nodes) {
            ...
            if (nodes.length == 0) {
              ...
              // Event the new node
              parent.DispatchEvent(parent.webserver.CreateMeshDispatchTargets(sender.meshid, [nodeid]), obj, {
                etype: 'node',
                action: 'addnode',
                nodeid: nodeid,
                node: device,
                msgid: 57,
                msgArgs: [port.Name, mesh.name],
                msg: 'Added device ' + port.Name + ' to device group ' + mesh.name,
                domain: sender.domainid
              });
            } else {
              ...
              if (device.rname != port.Name) {
                device.rname = port.Name;
                changed = true;
              } // Update the device port name
              if (mesh.flags && mesh.flags & 2 && device.name != port.Name) {
                device.name = port.Name;
                changed = true;
              } // Sync device name to port name
              if (changed) {
            ...
            if (obj.managedPorts[nodeid] == null) {
              ...
              obj.managedPorts[nodeid] = {
                name: port.Name,
                meshid: sender.meshid,
                portid: port.PortId,
                portType: port.PortType,
                portNo: port.PortIndex
              };
            }
          });
        } else {
        ...
        if (obj.managedPorts[nodeid] == null || obj.managedPorts[nodeid].name != port.Name) {
          parent.db.Get(nodeid, function (err, nodes) {
            ...
            if ((portInfo.sessions != null) != (port.StatAvailable != 0)) {
              if (port.StatAvailable != 0) {
                portInfo.sessions = { kvm: { busy: 1 } };
              } else {
                delete portInfo.sessions;
              }
        ...
        if (obj.managedPorts[nodeid] != null) {
          ...
          if (mesh != null && mesh.flags && mesh.flags & 1) {
            // Auto-remove devices
            ...
            // Event node deletion
            parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(sender.meshid, [nodeid]), obj, {
              etype: 'node',
              action: 'removenode',
              nodeid: nodeid,
              domain: domain.id,
  ...
  function generateIpKvmNodeId(meshid, portid, domainid) {
    return (
      ...
      parent.crypto
        .createHash('sha384')
        .update(Buffer.from(meshid + '/' + portid))
        .digest()
        .toString('base64')
        .replace(/\+/g, '@')
        .replace(/\//g, '$')
  ...
  function parseIpKvmUrl(domain, url) {
    ...
    if (relurl.endsWith('/.websocket')) {
      relurl = relurl.substring(0, relurl.length - 11);
    }
    return {
      domain: domain.id,
      relurl: relurl,
      preurl: q.path.substring(0, i + 76),
      nodeid: nodeid,
      nid: nid,
...
function CreateRaritanKX3Manager(parent, hostname, port, username, password) {
  ...
  function setState(newState) {
    ...
    if (obj.onStateChanged != null) {
      obj.onStateChanged(obj, newState);
    }
    if (newState == 2 && updateTimer == null) {
      updateTimer = setInterval(obj.update, 10000);
    }
    if (newState != 2 && updateTimer != null) {
      clearInterval(updateTimer);
      updateTimer = null;
    }
    if (newState == 0 && obj.started == true && retryTimer == null) {
      retryTimer = setTimeout(connect, 20000);
    }
    if (newState == 0) {
      obj.ports = [];
      obj.portCount = 0;
  ...
  function connect() {
    ...
    const data = Buffer.from(
      'is_dotnet=0&is_javafree=0&is_standalone_client=0&is_javascript_kvm_client=1&is_javascript_rsc_client=1&login=' +
    ...
    const req = https.request(options, function (res) {
      if (obj.state == 0) return;
      if (res.statusCode != 302 || res.headers['set-cookie'] == null || res.headers['location'] == null) {
        setState(0);
        return;
      }
      for (var i in res.headers['set-cookie']) {
        if (res.headers['set-cookie'][i].startsWith('pp_session_id=')) {
          obj.authCookie = res.headers['set-cookie'][i].substring(14).split(';')[0];
        }
      }
      if (obj.authCookie == null) {
        setState(0);
        return;
      }
  ...
  function fetchInitialInformation() {
    obj.fetch(
      '/webs_cron.asp?_portsstatushash=&_devicesstatushash=&webs_job=sidebarupdates',
      null,
      null,
      function (server, tag, data) {
        data = data.toString();
        ...
        obj.fetch('/sidebar.asp', null, null, function (server, tag, data) {
          ...
          setState(2);
          if (obj.onPortsChanged != null) {
            obj.onPortsChanged(obj, updatedPorts);
          }
        });
      }
    );
  ...
  obj.update = function () {
    obj.fetch(
      '/webs_cron.asp?_portsstatushash=' + obj.portHash + '&_devicesstatushash=' + obj.deviceHash,
      null,
      null,
      function (server, tag, data) {
        data = data.toString();
        ...
        if (parsed['updatePort']) {
          ...
          if (updatedPorts.length > 0 && obj.onPortsChanged != null) {
            obj.onPortsChanged(obj, updatedPorts);
          }
        }
      }
  ...
  obj.handleIpKvmGet = function (domain, reqinfo, req, res, next) {
    if (reqinfo.relurl == '/') {
      res.redirect(reqinfo.preurl + '/jsclient/Client.asp');
      return;
    }
    ...
    obj.fetch(reqinfo.relurl, null, [res, reqinfo], function (server, args, data, rres) {
      const resx = args[0],
        xreqinfo = args[1];
      if (rres.headers['content-type']) {
        resx.set('content-type', rres.headers['content-type']);
      }
      if (xreqinfo.relurl.startsWith('/js/js_kvm_client.')) {
        ...
        data = data.replace(
          "'use strict';",
          '\'use strict\';sessionStorage.setItem("portPermission","CCC");sessionStorage.setItem("appId","1638838693725_3965868704642470");sessionStorage.setItem("portId","' +
            xreqinfo.kvmport.portid +
            '");sessionStorage.setItem("channelName","' +
            xreqinfo.kvmport.name +
            '");sessionStorage.setItem("portType","' +
            xreqinfo.kvmport.portType +
            '");sessionStorage.setItem("portNo","' +
            xreqinfo.kvmport.portNo +
            '");'
        );
        // Replace the WebSocket code in one of the files to make it work with our server.
        data = data.replace(
          'b=new WebSocket(e+"//"+c+"/"+g);',
          'b=new WebSocket(e+"//"+c+"/ipkvm.ashx/' + xreqinfo.nid + '/"+g);'
        );
      }
      resx.end(data);
    });
  ...
  function logConnection(wsClient) {
    ...
    var event = {
      ...
      msgid: 15,
      msgArgs: [kvmport.portid, reqinfo.clientIp, kvmport.portNo],
      msg: 'Started desktop session' + ' \"' + kvmport.portid + '\" from ' + reqinfo.clientIp + ' to ' + kvmport.portNo,
      protocol: 2,
  ...
  function logDisconnection(wsClient) {
    ...
    var event = {
      ...
      msgArgs: [
        ...
        reqinfo.clientIp,
        kvmport.portNo,
        Math.floor((Date.now() - kvmport.connectionStart) / 1000)
      ],
      msg:
        'Ended desktop session' +
        ' \"' +
        kvmport.portid +
        '\" from ' +
        reqinfo.clientIp +
      ...
      protocol: 2,
      nodeid: reqinfo.nodeid,
      bytesin: kvmport.bytesIn,
      bytesout: kvmport.bytesOut
    };
    parent.parent.DispatchEvent(['*', reqinfo.userid, reqinfo.nodeid, kvmport.meshid], obj, event);
  ...
  obj.handleIpKvmWebSocket = function (domain, reqinfo, ws, req) {
    ...
    if (reqinfo.kvmport.wsClient != null) {
      ...
      try {
          ...
          this.wsBrowser.on('message', function (data) {
            ...
            if (
              ...
              const hash = Buffer.from(
                require('crypto')
                  .createHash('sha256')
                  .update(this.xAuthNonce + obj.authCookie)
                  .digest()
                  .toString('hex')
              );
            ...
            try {
              this.wsClient.kvmport.bytesOut += data.length;
            } catch (ex) {}
            this._socket.pause();
            try {
        ...
        reqinfo.kvmport.wsClient.on('message', function (data) {
          ...
          if (
            this.wsBrowser.xAuthNonce == null &&
            data.length == 67 &&
            data[0] == 0x21 &&
            data[1] == 0x41 &&
            data[2] == 0x7b &&
          ...
          try {
            this.wsBrowser.wsClient.kvmport.bytesIn += data.length;
          } catch (ex) {}
          this._socket.pause();
          try {
        ...
        reqinfo.kvmport.wsClient.on('close', function () {
          ...
          try {
            if (this.wsBrowser) {
              ...
              delete this.wsBrowser.wsClient;
              delete this.wsBrowser;
            }
            if (this.kvmport) {
              delete this.kvmport.wsClient;
              delete this.kvmport;
            }
          } catch (ex) {
            console.log(ex);
...
function CreateWebPowerSwitch(parent, hostname, port, username, password) {
  ...
  obj.update = function () {
    obj.fetch(
      '/restapi/relay/outlets/all;/=name,physical_state/',
      'GET',
      null,
      null,
      function (sender, tag, rdata, res) {
        if (res.statusCode == 207) {
          var rdata2 = null;
          if (rdata != null) {
            try {
              rdata2 = JSON.parse(rdata);
            } catch (ex) {}
          }
  ...
  obj.powerOperation = function (event) {
    ...
    if (event.action == 'turnon') {
      setPowerState(event.portnum - 1, true);
    } else if (event.action == 'turnoff') {
      setPowerState(event.portnum - 1, false);
    }
  };

  function setPowerState(port, state, func) {
    obj.fetch(
      '/restapi/relay/outlets/' + port + '/state/',
      'PUT',
      'value=' + state,
      null,
      function (sender, tag, rdata, res) {
  ...
  obj.fetch = function (url, method, data, tag, func) {
    ...
    if (challenge != null) {
      ...
      options.headers.Authorization =
        renderDigest(requestParams) + ', algorithm=MD5, nc=' + challenge.nc + ', qop=' + challenge.qop;
    }
...
function CreateMiniRouter(parent, nodeid, targetHost, targetPort) {
  ...
  parent.parent.debug(
    'relay',
    'MiniRouter: Request relay for ' + obj.targetHost + ':' + obj.targetPort + ' thru ' + nodeid + '.'
  );
  ...
  function closeTcpSocket(tcpSocket) {
    if (tcpSockets[tcpSocket]) {
      ...
      if (tcpSocket.relaySocket) {
        try {
          tcpSocket.relaySocket.close();
        } catch (ex) {
          console.log(ex);
        }
      }
  ...
  obj.start = function (onReadyFunc) {
    ...
    obj.tcpServer.on('connection', function (socket) {
      ...
      socket.on('data', function (chunk) {
        // Make sure to handle flow control.
        const f = function sendDone() {
          sendDone.tcpSocket.resume();
        };
        f.tcpSocket = this;
        if (this.relaySocket && this.relaySocket.active) {
          this.pause();
          this.relaySocket.send(chunk, f);
        }
      });
      ...
      const cookie = parent.parent.encodeCookie(
        { nouser: 1, domainid: domainid, nodeid: obj.nodeid, tcpaddr: obj.targetHost, tcpport: obj.targetPort },
      ...
      const url =
        protocol +
        '://localhost:' +
        parent.parent.args.port +
        '/' +
        domainadd +
        'meshrelay.ashx?noping=1&hd=1&auth=' +
        cookie; // TODO: &p=10, Protocol 10 is Web-RDP, Specify TCP routing protocol?
      ...
      socket.relaySocket.on('open', function () {
        parent.parent.debug('relay', 'MiniRouter: Relay websocket open');
      });
      socket.relaySocket.on('message', function (data) {
        // Make sure to handle flow control.
        if (!this.active) {
          if (data == 'c') {
          ...
          this._socket.pause();
          const f = function sendDone() {
            sendDone.webSocket._socket.resume();
          };
          f.webSocket = this;
          this.tcpSocket.write(data, f);
        }
      });
      socket.relaySocket.on('close', function (reasonCode, description) {
        parent.parent.debug('relay', 'MiniRouter: Relay websocket closed');
        closeWebSocket(this);
      });
      socket.relaySocket.on('error', function (err) {
        parent.parent.debug('relay', 'MiniRouter: Relay websocket error: ' + err);
        closeWebSocket(this);
      });
    });
  };
  ...
  obj.stop = function () {
    for (var tcpSocket in tcpSockets) {
      closeTcpSocket(tcpSocket);
    }
    obj.tcpServer.close();
    obj.tcpServer = null;
