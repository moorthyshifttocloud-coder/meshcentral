...
module.exports.CreateMeshDeviceFile = function (parent, ws, res, req, domain, user, meshid, nodeid) {
  ...
  obj.close = function (arg) {
    if (obj.ws != null) {
      if (arg == 1 || arg == null) {
        try {
          obj.ws.close();
          parent.parent.debug('relay', 'FileRelay: Soft disconnect (' + obj.req.clientIp + ')');
        } catch (ex) {
          console.log(e);
        }
      ...
      if (arg == 2) {
        try {
          ...
          parent.parent.debug('relay', 'FileRelay: Hard disconnect (' + obj.req.clientIp + ')');
        } catch (ex) {
          console.log(e);
        }
  ...
  if (obj.id == null || (obj.user == null && obj.ruserid == null)) {
    try {
      obj.close();
      parent.parent.debug('relay', 'FileRelay: Connection with no authentication (' + obj.req.clientIp + ')');
    } catch (e) {
      console.log(e);
    }
    return;
  ...
  function performRelay() {
    ...
    if (obj.ws != null) {
      obj.ws._socket.setKeepAlive(true, 240000);
    } // Set TCP keep alive
    ...
    // Check the peer connection status
    {
      ...
      if (relayinfo) {
        if (relayinfo.state == 1) {
          ...
          var agentws = null,
            file = null;
          if (relayinfo.peer1.ws) {
            relayinfo.peer1.ws.res = relayinfo.peer2.res;
            relayinfo.peer1.ws.res = relayinfo.peer2.res;
            relayinfo.peer1.ws.file = relayinfo.peer2.file;
            agentws = relayinfo.peer1.ws;
            file = relayinfo.peer2.file;
          }
          if (relayinfo.peer2.ws) {
            relayinfo.peer2.ws.res = relayinfo.peer1.res;
            relayinfo.peer2.ws.res = relayinfo.peer1.res;
            relayinfo.peer2.ws.file = relayinfo.peer1.file;
            agentws = relayinfo.peer2.ws;
            file = relayinfo.peer1.file;
          }
          agentws._socket.resume(); // Release the traffic
          try {
            agentws.send('c');
          } catch (ex) {} // Send connect to agent
          try {
            agentws.send(JSON.stringify({ type: 'options', file: file }));
          } catch (ex) {} // Send options to agent
          try {
            agentws.send('10');
          } catch (ex) {} // Send file transfer protocol to agent

          parent.parent.debug(
            'relay',
            'FileRelay connected: ' + obj.id + ' (' + obj.req.clientIp + ' --> ' + obj.peer.req.clientIp + ')'
          );
        ...
        parent.parent.debug(
          'relay',
          'FileRelay holding: ' + obj.id + ' (' + obj.req.clientIp + ') ' + (obj.authenticated ? 'Authenticated' : '')
        );
        if (obj.ws != null) {
          ...
          if (parent.parent.multiServer != null) {
            ...
            if (rsession != null && rsession.serverId > parent.parent.serverId) {
              ...
              parent.parent.debug(
                'relay',
                'FileRelay unexpected connection: ' + obj.id + ' (' + obj.req.clientIp + ')'
              );
              delete obj.id;
              delete obj.ws;
              delete obj.peer;
    ...
    if (obj.id != null) {
      ...
      if (relayinfo != null) {
        if (relayinfo.state == 2) {
          ...
          parent.parent.debug(
            'relay',
            'FileRelay disconnect: ' + obj.id + ' (' + obj.req.clientIp + ' --> ' + peer.req.clientIp + ')'
          );
          if (peer.ws) {
            try {
              peer.ws.close();
  ...
  if (obj.authenticated) {
    ...
    const command = {
      ...
      type: 'tunnel',
      userid: user._id,
      value: '*/devicefile.ashx?id=' + obj.id + '&rauth=' + rcookie,
      soptions: {}
    };
    parent.parent.debug('relay', 'FileRelay: Sending agent tunnel command: ' + JSON.stringify(command));
    if (obj.sendAgentMessage(command, user, domain.id) == false) {
      delete obj.id;
      parent.parent.debug('relay', 'FileRelay: Unable to contact this agent (' + obj.req.clientIp + ')');
    }
  }
