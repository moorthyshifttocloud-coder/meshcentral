...
module.exports.CreateMQTTBroker = function (parent, db, args) {
  ...
  obj.generateLogin = function (meshid, nodeid) {
    const meshidsplit = meshid.split('/'),
      nodeidsplit = nodeid.split('/');
    const xmeshid = meshidsplit[2],
      xnodeid = nodeidsplit[2],
      xdomainid = meshidsplit[1];
    ...
    return {
      ...
      user: username,
      pass:
        parent.config.settings.mqtt.auth.keyid +
        ':' +
        nonce +
        ':' +
        parent.crypto
          .createHash('sha384')
          .update(username + ':' + nonce + ':' + parent.config.settings.mqtt.auth.key)
          .digest('base64')
    };
  };
  ...
  aedes.authenticate = function (client, username, password, callback) {
    obj.parent.debug(
      ...
      'Authentication User:' +
        ...
        client.id +
        ', ' +
        client.conn.xtransport +
        '://' +
        cleanRemoteAddr(client.conn.xip)
    );

    // Parse the username and password
    var usersplit = username.split(':');
    var passsplit = password.toString().split(':');
    if (usersplit.length !== 4 || passsplit.length !== 3) {
      obj.parent.debug(
        'mqtt',
        'Invalid user/pass format, ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
      );
      callback(authError, null);
      return;
    }
    if (usersplit[0] !== 'MCAuth1') {
      obj.parent.debug(
        'mqtt',
        'Invalid auth method, ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
      );
      callback(authError, null);
      return;
    }
    ...
    if (passsplit[0] !== parent.config.settings.mqtt.auth.keyid) {
      obj.parent.debug(
        'mqtt',
        'Invalid auth keyid, ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
      );
      callback(authError, null);
      return;
    }
    if (
      parent.crypto
        .createHash('sha384')
        .update(username + ':' + passsplit[1] + ':' + parent.config.settings.mqtt.auth.key)
        .digest('base64') !== passsplit[2]
    ) {
      obj.parent.debug(
        'mqtt',
        'Invalid password, ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
      );
      callback(authError, null);
      return;
    }
    ...
    // Check the domain
    if (typeof client.conn.xdomain == 'object' && xdomainid != client.conn.xdomain.id) {
      obj.parent.debug(
        'mqtt',
        'Invalid domain connection, ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
      );
      callback(null, false);
      return;
    }
    ...
    if (xmeshid.length === 96) {
      xmeshid = Buffer.from(xmeshid, 'hex').toString('base64');
    }
    if (xmeshid.length !== 64 || xnodeid.length != 64) {
      callback(authError, null);
      return;
    }
    ...
    db.Get(client.xdbNodeKey, function (err, nodes) {
      ...
      if (obj.connections[client.xdbNodeKey] == null) {
        ...
        parent.SetConnectivityState(client.xdbMeshKey, client.xdbNodeKey, Date.now(), 16, 7, null, {
          name: nodes[0].name
        }); // Indicate this node has a MQTT connection, 7 = Present state
      } else {
      ...
      client.conn.on('end', function () {
        ...
        obj.parent.debug(
          'mqtt',
          'Connection closed, ' + this.parent.conn.xtransport + '://' + cleanRemoteAddr(this.parent.conn.xip)
        );

        // Remove this client from the connections list
        if (this.parent.xdbNodeKey != null && obj.connections[this.parent.xdbNodeKey] != null) {
          var clients = obj.connections[this.parent.xdbNodeKey],
            i = clients.indexOf(client);
          if (i >= 0) {
            if (clients.length == 1) {
              delete obj.connections[this.parent.xdbNodeKey];
              parent.ClearConnectivityState(this.parent.xdbMeshKey, this.parent.xdbNodeKey, 16, null, {
                name: nodes[0].name
              }); // Remove the MQTT connection for this node
            } else {
              clients.splice(i, 1);
            }
          }
  ...
  aedes.authorizeSubscribe = function (client, sub, callback) {
    ...
    obj.parent.debug(
      'mqtt',
      'AuthorizeSubscribe "' + sub.topic + '", ' + client.conn.xtransport + '://' + cleanRemoteAddr(client.conn.xip)
    );
    if (allowedSubscriptionTopics.indexOf(sub.topic) === -1) {
      sub = null;
    } // If not a supported subscription, deny it.
  ...
  obj.publish = function (nodeid, topic, message) {
    ...
    if (parent.multiServer != null) {
      parent.multiServer.DispatchMessage(
        JSON.stringify({ action: 'mqtt', nodeid: nodeid, topic: topic, message: message })
      );
    }
  ...
  function handleMessage(nodeid, meshid, domainid, topic, message) {
    ...
    if (topic == 'console') {
      parent.webserver.routeAgentCommand(
        { action: 'msg', type: 'console', value: message.toString(), source: 'MQTT' },
        domainid,
        nodeid,
        meshid
      );
