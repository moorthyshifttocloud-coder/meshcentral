...
module.exports.CreateAmtScanner = function (parent) {
  ...
  obj.stop = function () {
    ...
    obj.servers = {};
    if (obj.mainTimer != null) {
      clearInterval(obj.mainTimer);
      obj.mainTimer = null;
    }
  };
  ...
  obj.performRangeScan = function (userid, rangestr) {
    if (obj.rpacket == null) {
      obj.rpacket = obj.buildRmcpPing(0);
    }
    ...
    rangeinfo.server.on('error', (err) => {
      console.log(err);
    });
    rangeinfo.server.on('message', function (data, rinfo) {
      obj.parseRmcpPacket(data, rinfo, 0, obj.reportMachineState, rangeinfo);
    });
    rangeinfo.server.on('listening', function () {
      for (var i = rangeinfo.min; i <= rangeinfo.max; i++) {
        rangeinfo.server.send(obj.rpacket, 623, obj.IPv4NumToStr(i));
      }
    });
    rangeinfo.timer = setTimeout(function () {
      // ************************* USE OF OUTER VARS!!!!!!!!!!!!!!!
      obj.parent.DispatchEvent(['*', userid], obj, {
        action: 'scanamtdevice',
        range: rangeinfo.range,
        results: rangeinfo.results,
        nolog: 1
      });
  ...
  obj.parseIpv4Range = function (range) {
    ...
    var x = range.split('-');
    if (x.length == 2) {
      return { min: obj.parseIpv4Addr(x[0]), max: obj.parseIpv4Addr(x[1]) };
    }
    x = range.split('/');
    if (x.length == 2) {
      var ip = obj.parseIpv4Addr(x[0]),
        masknum = parseInt(x[1]),
        mask = 0;
  ...
  obj.parseIpv4Addr = function (addr) {
    var x = addr.split('.');
    if (x.length == 4) {
      return (parseInt(x[0]) << 24) + (parseInt(x[1]) << 16) + (parseInt(x[2]) << 8) + (parseInt(x[3]) << 0);
    }
    return null;
  };
  ...
  obj.ResolveName = function (hostname, func) {
    if (hostname == '127.0.0.1' || hostname == '::1' || hostname == 'localhost') {
      func(hostname, null);
    } // Don't scan localhost
    if (obj.net.isIP(hostname) > 0) {
      func(hostname, hostname);
      return;
    } // This is an IP address, already resolved.
    obj.dns.lookup(hostname, function (err, address, family) {
      if (err == null) {
        func(hostname, address);
      } else {
        func(hostname, null);
      }
    });
  ...
  obj.performScan = function () {
    ...
    obj.parent.db.getLocalAmtNodes(function (err, docs) {
      // TODO: handler more than 10 computer scan at the same time. DNS resolved may need to be a seperate module.
      for (var i in obj.scanTable) {
        obj.scanTable[i].present = false;
      }
      if (err == null && docs.length > 0) {
        for (var i in docs) {
          var doc = docs[i],
            host = doc.host.toLowerCase();
          const ciraConnections = obj.parent.mpsserver
            ? obj.parent.mpsserver.GetConnectionToNode(doc._id, null, true)
            : null; // See if any OOB connections are present
          if (host != '127.0.0.1' && host != '::1' && host.toLowerCase() != 'localhost' && ciraConnections == null) {
            var scaninfo = obj.scanTable[doc._id];
            if (scaninfo == null) {
              var tag = obj.nextTag++;
              obj.scanTableTags[tag] =
                obj.scanTable[doc._id] =
                scaninfo =
                  { nodeinfo: doc, present: true, tag: tag, state: 0 };
              ...
              if (scaninfo.state == 1 && delta >= PeriodicScanTimeout) {
                ...
                scaninfo.state = 0;
                obj.parent.ClearConnectivityState(scaninfo.nodeinfo.meshid, scaninfo.nodeinfo._id, 4, null, {
                  name: doc.name
                }); // Clear connectivity state
                if (obj.parent.amtManager != null) {
                  obj.parent.amtManager.stopAmtManagement(scaninfo.nodeinfo._id, 3, scaninfo.nodeinfo.host);
                }
                ...
                obj.checkTcpPresence(
                  host,
                  doc.intelamt.tls == 1 ? 16993 : 16992,
                  scaninfo,
                  function (tag, result, version) {
                    // TODO: It is bad that "obj" is being accessed within this function.
                    if (result == false) return;
                    tag.lastpong = Date.now();
                    if (tag.state == 0) {
                      tag.state = 1;
                      obj.parent.SetConnectivityState(tag.nodeinfo.meshid, tag.nodeinfo._id, tag.lastpong, 4, 7, null, {
                        name: doc.name
                      }); // Report power state as "present" (7).
                      if (version != null) {
                        obj.changeAmtState(tag.nodeinfo._id, version, 2, tag.nodeinfo.intelamt.tls);
                      }
                      if (obj.parent.amtManager != null) {
                        obj.parent.amtManager.startAmtManagement(tag.nodeinfo._id, 3, tag.nodeinfo.host);
                      }
                    }
                  }
                );
  ...
  obj.performSpecificScan = function (node) {
    ...
    const ciraConnections = obj.parent.mpsserver
      ? obj.parent.mpsserver.GetConnectionToNode(node._id, null, true)
      : null; // See if any OOB connections are present
    if (host != '127.0.0.1' && host != '::1' && host.toLowerCase() != 'localhost' && ciraConnections == null) {
      obj.checkTcpPresence(
        host,
        node.intelamt.tls == 1 ? 16993 : 16992,
        { nodeinfo: node },
        function (tag, result, version) {
          if (result == true && obj.parent.amtManager != null) {
            obj.parent.amtManager.startAmtManagement(tag.nodeinfo._id, 3, tag.nodeinfo.host);
          }
        }
      );
  ...
  obj.checkAmtPresence = function (host, tag) {
    obj.ResolveName(host, function (hostname, ip) {
      obj.checkAmtPresenceEx(ip, tag);
    });
  };
  ...
  obj.checkAmtPresenceEx = function (host, tag) {
    ...
    if (server == undefined) {
      ...
      server.on('message', (data, rinfo) => {
        obj.parseRmcpPacket(data, rinfo, serverid, obj.changeConnectState, null);
      });
      server.on('listening', () => {
        obj.pendingSends.push([server, packet, host]);
        if (obj.pendingSendTimer == null) {
          obj.pendingSendTimer = setInterval(obj.sendPendingPacket, 10);
        }
      });
      server.bind(0);
  ...
  obj.parseRmcpPacket = function (data, rinfo, serverid, func, user) {
    ...
    if ((data[12] == 0 || data[13] != 0 || data[14] != 1 || data[15] != 0x57) && data[21] & 32) {
      ...
      if (dualPorts == true) {
        openPorts = [16992, 16993];
      }
      if (provisioningState <= 2) {
        func(tag, minorVersion, majorVersion, provisioningState, openPort, dualPorts, rinfo, user);
      }
    }
  };

  // Use the RMCP packet to change the computer state
  obj.changeConnectState = function (
    tag,
    minorVersion,
    majorVersion,
    provisioningState,
    ...
    if (scaninfo != undefined) {
      ...
      if (scaninfo.state == 0) {
        scaninfo.state = 1;
        if (openPort == 16993 || dualPorts == true) {
          scaninfo.nodeinfo.intelamt.tls = 1;
        } else if (openPort == 16992) {
          scaninfo.nodeinfo.intelamt.tls = 0;
        }
        if (majorVersion > 0) {
          // Older versions of Intel AMT report the AMT version.
        ...
        obj.parent.SetConnectivityState(
          scaninfo.nodeinfo.meshid,
          scaninfo.nodeinfo._id,
          scaninfo.lastpong,
          4,
          7,
          null,
          { name: scaninfo.nodeinfo.name }
        ); // Report power state as "present" (7).
        obj.changeAmtState(
          scaninfo.nodeinfo._id,
          scaninfo.nodeinfo.intelamt.ver,
          provisioningState,
          scaninfo.nodeinfo.intelamt.tls
        );
        if (obj.parent.amtManager != null) {
          obj.parent.amtManager.startAmtManagement(scaninfo.nodeinfo._id, 3, scaninfo.nodeinfo.host);
        }
      }
    }
  };
  ...
  // Use the RMCP packet to change the computer state
  obj.reportMachineState = function (
    tag,
    minorVersion,
    majorVersion,
    provisioningState,
    ...
    obj.dns.reverse(rinfo.address, function (err, hostnames) {
      if (err == null && hostnames != null && hostnames.length > 0) {
        user.results[rinfo.address] = {
          ver: majorVersion + '.' + minorVersion,
          tls: openPort == 16993 || dualPorts == true ? 1 : 0,
  ...
  obj.changeAmtState = function (nodeid, version, provisioningState, tls) {
    ...
    obj.parent.db.Get(nodeid, function (err, nodes) {
      ...
      obj.parent.db.Get(node.meshid, function (err, meshes) {
        ...
        var changes = [],
          event = { etype: 'node', action: 'changenode', nodeid: node._id };
        event.msg = +': ';
        ...
        if (node.intelamt.tls != tls) {
          node.intelamt.tls = tls;
          change = true;
          changes.push(tls == 1 ? 'TLS' : 'NoTLS');
        }
        if (obj.compareAmtVersionStr(node.intelamt.ver, version)) {
          node.intelamt.ver = version;
          change = true;
          changes.push('AMT Version ' + version);
        }
        if (node.intelamt.state != provisioningState) {
          node.intelamt.state = provisioningState;
          change = true;
        ...
        if (change == true) {
          ...
          event.msg =
            'Intel&reg; AMT changed device ' + node.name + ' from mesh ' + mesh.name + ': ' + changes.join(', ');
          event.node = obj.parent.webserver.CloneSafeNode(node);
          if (obj.parent.db.changeStream) {
            event.noact = 1;
  ...
  obj.checkTcpPresence = function (host, port, scaninfo, func) {
    obj.ResolveName(host, function (hostname, ip) {
      obj.checkTcpPresenceEx(ip, port, scaninfo, func);
    });
  };
  ...
  obj.checkTcpPresenceEx = function (host, port, scaninfo, func) {
    ...
    try {
      ...
      if (port == 16992) {
        ...
        if (scaninfo.tlsoption == null) {
          scaninfo.tlsoption = 0;
        }
        const tlsOptions = {
        ...
        client = obj.tls.connect(port, host, tlsOptions, function () {
          this.write('GET / HTTP/1.1\r\nhost: ' + host + '\r\n\r\n');
        });
      }
      ...
      client.on('data', function (data) {
        var version = obj.getIntelAmtVersionFromHeaders(data.toString());
        if (this.scaninfo.tcp != null) {
          delete this.scaninfo.tcp;
          try {
            this.destroy();
          } catch (ex) {}
          this.func(this.scaninfo, version != null, version);
        }
      });
      client.on('error', function () {
        if (this.scaninfo.tlsoption == 0) {
          this.scaninfo.tlsoption = 1;
        } else if (this.scaninfo.tlsoption == 1) {
          this.scaninfo.tlsoption = 0;
        }
        if (this.scaninfo.tcp != null) {
          delete this.scaninfo.tcp;
          try {
  ...
  obj.getIntelAmtVersionFromHeaders = function (headers) {
    ...
    for (var i in lines) {
      ...
      if (lines[i].substring(0, 46) == 'Server: Intel(R) Active Management Technology ') {
        ...
        var ver = lines[i].substring(46),
          splitver = ver.split('.');
        if (
          (splitver.length == 3 || splitver.length == 4) &&
          '' + parseInt(splitver[0]) === splitver[0] &&
          '' + parseInt(splitver[1]) === splitver[1] &&
          '' + parseInt(splitver[2]) === splitver[2]
        ) {
          return splitver[0] + '.' + splitver[1] + '.' + splitver[2];
        }
      }
    }
    return null;
