...
class ModernModal {
  ...
  show(title, content, okCallback = null, okButtonText = 'OK') {
    const sizeClass =
      this.options.size === 'large' ? 'modal-lg' : this.options.size === 'extra-large' ? 'modal-xl' : '';
...
class IconUploadComponent {
  ...
  getPreviewSrc(value) {
    ...
    if (typeof this.options.normalizePreviewUrl !== 'function') {
      return value;
    }
    try {
      return this.options.normalizePreviewUrl(value);
    } catch (ex) {
      return value;
    }
  }
  ...
  handleUrlInput(input) {
    ...
    if (value.length > 0) {
      ...
      if (previewIcon.tagName.toLowerCase() === 'img') {
        previewIcon.src = this.getPreviewSrc(value);
      } else {
        previewIcon.style.backgroundImage = `url('${value}')`;
      }
    } else {
    ...
    try {
      if (this.options.onUpload) {
        ...
        if (result && result.path) {
          previewContainer.classList.remove('d-none');
          if (previewIcon.tagName.toLowerCase() === 'img') {
            previewIcon.src = this.getPreviewSrc(result.path);
          } else {
            previewIcon.style.backgroundImage = `url('${result.path}')`;
          }
