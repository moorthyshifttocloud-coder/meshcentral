    ...
    install: function () {
      ...
      this.canvas.addEventListener('mousemove', function (e) {
        ...
        if (self.mouseNagleTimer == null) {
          ...
          self.mouseNagleTimer = setTimeout(function () {
            self.socket.send(JSON.stringify(self.mouseNagleData));
      ...
      this.canvas.addEventListener('mousedown', function (e) {
        if (!self.socket || !self.activeSession) return;
        if (self.mouseNagleTimer != null) {
          clearTimeout(self.mouseNagleTimer);
          self.mouseNagleTimer = null;
        }
        var rect = e.target.getBoundingClientRect();
        self.socket.send(
          JSON.stringify(['mouse', e.clientX - rect.left, e.clientY - rect.top, mouseButtonMap(e.button), true])
        );
        e.preventDefault();
        return false;
      });
      this.canvas.addEventListener('mouseup', function (e) {
        ...
        var rect = e.target.getBoundingClientRect();
        self.socket.send(
          JSON.stringify(['mouse', e.clientX - rect.left, e.clientY - rect.top, mouseButtonMap(e.button), false])
        );
        e.preventDefault();
        return false;
      });
      ...
      this.canvas.addEventListener('DOMMouseScroll', function (e) {
        ...
        var rect = e.target.getBoundingClientRect();
        self.socket.send(
          JSON.stringify(['wheel', e.clientX - rect.left, e.clientY - rect.top, step, delta > 0, isHorizontal])
        );
        e.preventDefault();
        return false;
      });
            ...
            navigator.clipboard
              ...
              .then(function (data) {
                if (data != self.prevClipboard) {
                  ...
                  if (self.socket) {
                    self.socket.send(JSON.stringify(['clipboard', data]));
                  }
                }
              })
              .catch(function () {});
          ...
          switch (msg[0]) {
            ...
            case 'rdp-clipboard': {
              if (msg[1] != null && navigator.clipboard.writeText != null) {
                navigator.clipboard
                  .writeText(msg[1]) // Put remote clipboard data into our clipboard
                  .then(function () {})
                  .catch(function (err) {
                    console.log('clipboard.writeText Error', err);
                  });
              }
              break;
            }
