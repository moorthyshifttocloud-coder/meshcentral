...
var CreateWsmanComm = function (host, port, user, pass, tls) {
  ...
  obj.PerformAjaxExNodeJS = function (postdata, callback, tag, url, action) {
    obj.PerformAjaxExNodeJS2(postdata, callback, tag, url, action, 3);
  };
  ...
  obj.xxConnectHttpSocket = function () {
    ...
    obj.socket = new WebSocket(
      window.location.protocol.replace('http', 'ws') +
        ...
        obj.tlsv1only +
        (user == '*' ? '&serverauth=1' : '') +
        (typeof pass === 'undefined' ? '&serverauth=1&user=' + user : '')
    ); // The "p=1" indicates to the relay that this is a WSMAN session
      ...
      if (obj.socketParseState == 1) {
        ...
        if (
          ...
          if (isNaN(csize)) {
            ...
            return;
          } // Critical error, close the socket and exit.
