...
var CreateAmtRemoteDesktop = function (divid, scrolldiv) {
  ...
  obj.ProcessBinaryData = function (data) {
    ...
    if (obj.recordedData != null && obj.recordedHolding !== true) {
      obj.recordedData.push(recordingEntry(2, 1, String.fromCharCode.apply(null, new Uint8Array(data))));
    }
    // ###END###{DesktopRecorder}
    ...
    while (obj.acc != null && obj.acc.byteLength > 0) {
      ...
      var cmdsize = 0,
        accview = new DataView(obj.acc.buffer);
      if (obj.state == 0 && obj.acc.byteLength >= 12) {
        ...
        obj.state = 3;
        if (obj.parent) {
          obj.parent.disconnectCode = 50000;
        } // If Intel AMT disconnects at exactly this moment, indicates we need RLE8 or unsupported GPU.
        ...
        // ###BEGIN###{DesktopRecorder}
        obj.DeskRecordServerInit = String.fromCharCode.apply(
          null,
          new Uint8Array(obj.acc.buffer.slice(0, 24 + namelen))
        );
        // ###END###{DesktopRecorder}
        ...
        obj.send(
          String.fromCharCode(2, 0) +
            ShortToStr(supportedEncodings.length / 4 + 1) +
            supportedEncodings +
            IntToStr(-223)
        ...
        if (obj.graymode == false) {
          ...
          if (obj.lowcolor == false) {
            ...
            ); // Setup 8 bit black and white RGB800
          } else {
            obj.send(
              String.fromCharCode(0, 0, 0, 0, 8, 4, 0, 1) +
                ...
                ShortToStr(0) +
                String.fromCharCode(0, 0, 0, 0, 0, 0)
            ); // Setup 4 bit black and white RGB400
          }
        }
        ...
        if (obj.kvmExtChanged != null) {
          if (obj.decimationMode > 0) {
            obj.sendKvmExtCmd(2, obj.decimationMode);
          } // Set Decimation Mode (0 = Do not set, 1 = Disable, 2 = Auto, 3 = Enable)
        ...
        if (obj.parent) {
          ...
          if (obj.bpp * obj.width * obj.height > 9216000) {
            obj.parent.disconnectCode = 50002;
          } // Display buffer too large.
        }
      } else if (obj.state == 4) {
        switch (obj.acc[0]) {
          case 0: // FramebufferUpdate
            ...
            if (obj.recordedHolding === true) {
              delete obj.recordedHolding;
              obj.recordedData.push(recordingEntry(2, 1, String.fromCharCode.apply(null, obj.acc)));
            }
            // ###END###{DesktopRecorder}
        ...
        if (encoding == 0xffffff21) {
          ...
          // Check if the screen size is larger than Intel AMT should be able to handle
          //console.log('KVM Buffer Size: ' + (obj.bpp * obj.width * obj.height));
          if (obj.parent && obj.bpp * obj.width * obj.height > 9216000) {
            obj.parent.disconnectCode = 50002;
          } // Display buffer too large.
        } else if (encoding == 0) {
          // RAW encoding
          ...
          if (obj.bpp == 2) {
            for (var i = 0; i < s; i++) {
              _setPixel16(accview.getUint16(ptr, true), i);
              ptr += 2;
            }
          } else {
            for (var i = 0; i < s; i++) {
              _setPixel8(obj.acc[ptr++], i);
            }
          }
          ...
          // ###BEGIN###{Inflate}
            ...
            if (str.length > 0) {
              _decodeLRE(strToArr(str), 0, x, y, width, height, s, str.length);
            } else {
      ...
      if (cmdsize == 0) return;
      if (cmdsize != obj.acc.byteLength) {
        obj.acc = new Uint8Array(obj.acc.buffer.slice(cmdsize));
      } else {
        obj.acc = null;
      }
    }
  ...
  function _decodeLRE(data, ptr, x, y, width, height, s, datalen) {
    ...
    if (subencoding == 0) {
      ...
      if (obj.bpp == 2) {
        for (i = 0; i < s; i++) {
          _setPixel16(data[ptr++] + (data[ptr++] << 8), i);
        }
      } else {
        for (i = 0; i < s; i++) {
          _setPixel8(data[ptr++], i);
        }
      }
      _putImage(obj.spare, x, y);
      ...
      if (obj.graymode) {
        ...
        obj.canvas.fillStyle =
          'rgb(' +
          (obj.bpp == 1
            ? (v & 224) + ',' + ((v & 28) << 3) + ',' + _fixColor((v & 3) << 6)
      ...
      if (obj.bpp == 2) {
        while (rlecount < s && ptr < data.byteLength) {
          ...
          runlength = 1;
          do {
            runlength += runlengthdecode = data[ptr++];
    ...
    if (obj.focusmode > 0) {
      ...
      obj.send(
        String.fromCharCode(3, 1) +
          ShortToStr(Math.max(Math.min(obj.ox, obj.mx) - obj.focusmode, 0)) +
          ShortToStr(Math.max(Math.min(obj.oy, obj.my) - obj.focusmode, 0)) +
          ShortToStr(df + Math.abs(obj.ox - obj.mx)) +
          ShortToStr(df + Math.abs(obj.oy - obj.my))
  ...
  obj.sendkey = function (k, d) {
    if (typeof k == 'object') {
      var buf = '';
      for (var i in k) {
        buf += String.fromCharCode(4, k[i][1], 0, 0) + IntToStr(k[i][0]);
      }
      obj.send(buf);
    } else {
    ...
    if (obj.onKvmData != null) {
      ...
      if (d.length >= 16 && d.substring(0, 15) == '\0KvmDataChannel') {
        if (obj.kvmDataSupported == false) {
          obj.kvmDataSupported = true; /*console.log('KVM Data Channel Supported.');*/
        }
        if ((obj.onKvmDataAck == -1 && d.length == 16) || d.charCodeAt(15) != 0) {
          obj.onKvmDataAck = true;
        }
        try {
          if (urlvars && urlvars['kvmdatatrace']) {
            console.log('KVM-DataChannel-Recv(' + (d.length - 16) + '): ' + d.substring(16));
          }
        } catch (ex) {}
        if (d.length >= 16) {
        ...
        var cmd = d.charCodeAt(11),
          val = d.charCodeAt(12);
        //console.log('Received KvmExtCmd', cmd, val, d.length);
        if (cmd == 1) {
          obj.kvmExt.decimationMode = val;
          if (d.length > 13) {
            obj.kvmExt.decimationState = d.charCodeAt(13);
          }
          if (obj.kvmExtChanged != null) {
            obj.kvmExtChanged(1, obj.kvmExt, obj.kvmExt);
          }
        }
        if (cmd == 2) {
          obj.sendKvmExtCmd(1);
        ...
        if (cmd == 3) {
          obj.kvmExt.compression = val;
          if (obj.kvmExtChanged != null) {
            obj.kvmExtChanged(3, obj.kvmExt);
          }
        }
        if (cmd == 4) {
          obj.sendKvmExtCmd(3);
        ...
        try {
          if (urlvars && urlvars['kvmdatatrace']) {
            console.log('KVM-ClipBoard-Recv(' + d.length + '): ' + rstr2hex(d) + ', ' + d);
          }
        } catch (ex) {}
      }
    }
  ...
  obj.sendKvmData = function (x) {
    if (obj.onKvmDataAck !== true) {
      ...
      try {
        if (urlvars && urlvars['kvmdatatrace']) {
          console.log('KVM-DataChannel-Send(' + x.length + '): ' + x);
        }
      } catch (ex) {}
  ...
  obj.sendKeepAlive = function () {
    if (obj.lastKeepAlive < Date.now() - 5000) {
      obj.lastKeepAlive = Date.now();
      obj.send(String.fromCharCode(6, 0, 0, 0) + IntToStr(16) + '\0KvmDataChannel\0');
    }
  };
  // ###END###{DesktopInband}
  ...
  obj.sendClipboardData = function (x) {
    try {
      if (urlvars && urlvars['kvmdatatrace']) {
        console.log('KVM-ClipBoard-Send(' + x.length + '): ' + rstr2hex(x) + ', ' + x);
      }
    } catch (ex) {}
    obj.send(String.fromCharCode(6, 0, 0, 0) + IntToStr(x.length) + x);
  };
    ...
    obj.recordedData.push(
      recordingEntry(
        ...
        JSON.stringify({
          ...
          protocol: 102,
          bpp: obj.bpp,
          graymode: obj.graymode,
          lowcolor: obj.lowcolor,
          screenSize: [obj.width, obj.height]
        })
      )
    ); // Metadata, 102 = Midstream Intel AMT KVM
    obj.DeskRecordServerInit =
      String.fromCharCode(obj.width >> 8, obj.width & 0xff, obj.height >> 8, obj.height & 0xff) +
      obj.DeskRecordServerInit.substring(4);
  ...
  function recordingEntry(type, flags, data) {
    ...
    if (typeof data == 'number') {
      ...
      return (
        ShortToStr(type) + ShortToStr(flags) + IntToStr(data.length) + IntToStr(now >> 32) + IntToStr(now & 32) + data
      );
    }
  }
  // ###END###{DesktopRecorder}
