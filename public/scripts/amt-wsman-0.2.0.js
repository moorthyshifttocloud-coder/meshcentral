...
var WsmanStackCreateService = function (host, port, user, pass, tls, extra) {
  ...
  obj.PerformAjax = function (postdata, callback, tag, pri, namespaces) {
    ...
    obj.comm.PerformAjax(
      '<?xml version=\"1.0\" encoding=\"utf-8\"?><Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:a="http://schemas.xmlsoap.org/ws/2004/08/addressing" xmlns:w="http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd" xmlns=\"http://www.w3.org/2003/05/soap-envelope\" ' +
        ...
        '><Header><a:Action>' +
        postdata,
      function (data, status, tag) {
        if (status != 200) {
          callback(obj, null, { Header: { HttpError: status } }, status, tag);
          return;
        }
        ...
        if (!wsresponse || wsresponse == null) {
          ...
          callback(obj, wsresponse.Header['ResourceURI'], wsresponse, 200, tag);
        }
      },
      tag,
      pri
  ...
  obj.ExecSubscribe = function (resuri, delivery, url, callback, tag, pri, selectors, opaque, user, pass) {
    ...
    if (user != undefined && pass != undefined) {
      ...
      digest2 = '<Auth Profile="http://schemas.xmlsoap.org/ws/2004/08/eventing/DeliveryModes/secprofile/http/digest"/>';
    }
    if (opaque != undefined && opaque != null) {
      opaque = '<a:ReferenceParameters>' + opaque + '</a:ReferenceParameters>';
    } else {
      opaque = '';
    }
    var data =
      ...
      _PutObjToSelectorsXml(selectors) +
      digest +
    ...
    for (var i in args) {
      if (args[i] != null) {
        if (Array.isArray(args[i])) {
          for (var x in args[i]) {
            argsxml += '<r:' + i + '>' + args[i][x] + '</r:' + i + '>';
          }
        } else {
          argsxml += '<r:' + i + '>' + args[i] + '</r:' + i + '>';
        }
      }
    }
  ...
  function _ParseWsmanRec(node) {
    ...
    for (var i = 0; i < node.childNodes.length; i++) {
      ...
      if (r[child.localName] instanceof Array) {
        r[child.localName].push(childObj);
      } else if (r[child.localName] == undefined) {
        r[child.localName] = childObj;
      } else {
        r[child.localName] = [r[child.localName], childObj];
      }
    }
    return r;
  }
    ...
    for (var prop in putObj) {
      ...
      if (typeof putObj[prop] === 'object' && putObj[prop]['ReferenceParameters']) {
        result +=
          ...
          putObj[prop]['ReferenceParameters']['ResourceURI'] +
        ...
        if (Array.isArray(selectorArray)) {
          for (var i = 0; i < selectorArray.length; i++) {
            result +=
              '<w:Selector' +
              _ObjectToXmlAttributes(selectorArray[i]) +
              '>' +
              selectorArray[i]['Value'] +
              '</w:Selector>';
          }
        } else {
          result +=
            '<w:Selector' + _ObjectToXmlAttributes(selectorArray) + '>' + selectorArray['Value'] + '</w:Selector>';
        }
  ...
  function _PutObjToSelectorsXml(selectorSet) {
    ...
    if (selectorSet['InstanceID'])
      return (
        '<w:SelectorSet><w:Selector Name="InstanceID">' + selectorSet['InstanceID'] + '</w:Selector></w:SelectorSet>'
      );
