...
script_functionTableX3 = [
  ...
  function (runner, x) {
    return GetSidString(x);
  },
  function (runner, x) {
    return GetSidByteArray(x);
  }
];
...
function script_setup(binary, startvars) {
  ...
  if (binary.length < 6) {
    console.error('Invalid script length');
    return null;
  } // Script must have at least 6 byte header
  if (ReadInt(binary, 0) != 0x247d2945) {
    console.error('Invalid binary script');
    return null;
  } // Check the script magic header
  if (ReadShort(binary, 4) > 1) {
    console.error('Unsupported script version');
    return null;
  ...
  obj.getVar = function (name) {
    if (name == undefined) return undefined;
    return obj.getVarEx(name.split('.'), obj.variables);
  };
  obj.getVarEx = function (name, val) {
    try {
      if (name == undefined) return undefined;
      if (name.length == 0) return val;
      return obj.getVarEx(name.slice(1), val[name[0]]);
    } catch (e) {
      return null;
    }
  };
  obj.setVar = function (name, val) {
    obj.setVarEx(name.split('.'), obj.variables, val);
  };
  obj.setVarEx = function (name, vars, val) {
    if (name.length == 1) {
      vars[name[0]] = val;
    } else {
      obj.setVarEx(name.slice(1), vars[name[0]], val);
    }
  };
    ...
    if (obj.ip < obj.script.length) {
      ...
      for (var i = 0; i < argcount; i++) {
        ...
        if (argtyp < 2) {
          ...
          while (argval.split('{').length > 1) {
            var t = argval.split('{').pop().split('}').shift();
            argval = argval.replace('{' + t + '}', obj.getVar(t));
          }
          if (argtyp == 1) {
            obj.variables['__' + i] = decodeURI(argval);
            argval = '__' + i;
          } // If argtyp is 1, this is a literal. Store in temp variable.
          args.push(argval);
        }
      ...
      var argsval = [];
      for (var i = 0; i < 10; i++) {
        argsval.push(obj.getVar(args[i]));
      }
      var storeInArg0;
        ...
        if (cmdid < 10000) {
          ...
          switch (cmdid) {
            ...
            case 3: // print(message)
              if (obj.onConsole) {
                obj.onConsole(obj.toString(argsval[0]), obj);
              } else {
                console.log(obj.toString(argsval[0]));
              }
            ...
            case 5: // getitem(a, b, c)
              for (var i in argsval[1]) {
                if (argsval[1][i][argsval[2]] == argsval[3]) {
                  storeInArg0 = i;
                }
              }
              break;
          ...
          if (cmdid < 20000) {
            // functions of type ARG1 = func(ARG2, ARG3, ARG4, ARG5, ARG6)
            storeInArg0 = script_functionTableX2[cmdid - 10000](
              argsval[1],
              argsval[2],
              argsval[3],
              argsval[4],
            ...
            if (script_functionTableX3 && script_functionTableX3[cmdid - 20000]) {
              storeInArg0 = script_functionTableX3[cmdid - 20000](
                obj,
                argsval[1],
                argsval[2],
                argsval[3],
                ...
                argsval[5],
                argsval[6]
              ); // Note that optional calls start with "obj" as first argument.
            }
          }
        }
  ...
  obj.xxWsmanReturnFix = function (x) {
    ...
    if (x.ReturnValueStr) {
      x['ReturnValueStr'] = x.ReturnValueStr;
      delete x.ReturnValueStr;
    }
  };
    ...
    if (responses) {
      ...
      for (var i in responses) {
        ...
        for (var j in responses[i]) {
          obj.xxWsmanReturnFix(responses[i][j]);
        }
      }
    }
    // ###END###{**ClosureAdvancedMode}
...
function script_compile(script, onmsg) {
  ...
  for (var i in scriptlines) {
    ...
    if (scriptline.startsWith('##SWAP ')) {
      var x = scriptline.split(' ');
      if (x.length == 3) {
        swaps[x[1]] = x[2];
      }
    } // Add a swap instance
    ...
    for (var x in swaps) {
      scriptline = scriptline.split(x).join(swaps[x]);
    } // Apply all swaps
    ...
    if (funcIndex == -1) {
      funcIndex = script_functionTable2.indexOf(keywords[0].toLowerCase());
      if (funcIndex >= 0) funcIndex += 10000;
    }
    if (funcIndex == -1) {
      funcIndex = script_functionTable3.indexOf(keywords[0].toLowerCase());
      if (funcIndex >= 0) funcIndex += 20000;
    } // Optional methods
    if (funcIndex == -1) {
      if (onmsg) {
        onmsg('Unabled to compile, unknown command: ' + keywords[0]);
      }
      return '';
    }
      ...
      if (keywords[j][0] == ':') {
        ...
        if (argint == keywords[j]) {
          ...
          if (keywords[j][0] == '"' && keywords[j][keywords[j].length - 1] == '"') {
            cmd +=
              ShortToStr(keywords[j].length - 1) +
              String.fromCharCode(1) +
              keywords[j].substring(1, keywords[j].length - 1);
          } else {
  ...
  for (i in labelswap) {
    var label = labelswap[i][0].toUpperCase(),
      position = labelswap[i][1],
      target = labels[label];
    if (target == undefined) {
      if (onmsg) {
        onmsg('Unabled to compile, unknown label: ' + label);
      }
      return '';
    }
...
function script_decompile(binary, onecmd) {
  ...
  if (onecmd >= 0) {
    ...
    if (magic != 0x247d2945) {
      return '# Invalid binary script: ' + magic;
    }
    if (version != 1) {
  ...
  while (ptr < binary.length) {
    ...
    for (var i = 0; i < argcount; i++) {
      ...
      var argtyp = argval.charCodeAt(0);
      if (argtyp == 0) {
        argstr += ' ' + argval.substring(1);
      } // Variable
      else if (argtyp == 1) {
        argstr += ' \"' + argval.substring(1) + '\"';
      } // String
      else if (argtyp == 2) {
        argstr += ' ' + ReadInt(argval, 1);
      } // Integer
      else if (argtyp == 3) {
        // Label
