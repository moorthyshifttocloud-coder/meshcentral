...
var CreateAgentRedirect = function (meshserver, module, serverPublicNamePort, authCookie, rauthCookie, domainUrl) {
    ...
    if (controlMsg.type == 'console') {
      ...
      if (controlMsg.type == 'answer') {
        obj.webrtc.setRemoteDescription(
          new RTCSessionDescription(controlMsg),
          function () {
        ...
        if (obj.attemptWebRTC == true) {
          ...
          if (obj.webrtc != null && obj.webrtc.createDataChannel) {
            ...
            obj.webchannel.onclose = function (event) {
              if (obj.webRtcActive) {
                console.log('Session Disconnected: WebRTC DataChannel closed (Tab inactive or network dropped)');
                obj.Stop();
              }
            };
            obj.webrtc.onicecandidate = function (e) {
              if (e.candidate == null) {
                try {
                  obj.sendCtrlMsg(JSON.stringify(obj.webrtcoffer));
            ...
            obj.webrtc.oniceconnectionstatechange = function () {
              if (obj.webrtc != null) {
                if (obj.webrtc.iceConnectionState == 'disconnected') {
                  console.log(
                    'Session Disconnected: WebRTC ICE Connection State is "disconnected" (often caused by tab sleeping)'
                  );
      ...
      if (obj.m.ProcessBinaryCommand) {
        ...
        if (cmdAccLen != 0) {
          ...
          if (cmdsize != view.byteLength) {
            ...
            ((cmdAccLen = view.byteLength), (cmdAcc = [view]));
          } else {
  ...
  obj.xxOnSocketClosed = function () {
    ...
    console.log(
      'Session Disconnected: WebSocket connection closed by server/network (Active Tab: ' + !document.hidden + ')'
    );
    obj.Stop(1);
  };
  ...
  obj.Stop = function (x) {
    ...
    if (obj.socket != null) {
      ...
      } catch (ex) {} // If connected, send the close command
      try {
        if (obj.socket.readyState <= 1) {
          obj.socket.close();
        }
      } catch (ex) {} // If connecting or connected, close the websocket
      obj.socket = null;
    }
    obj.xxStateChange(0);
  };
