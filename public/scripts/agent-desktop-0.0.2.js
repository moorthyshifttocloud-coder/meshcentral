...
var CreateAgentRemoteDesktop = function (canvasid, scrolldiv) {
  ...
  obj.mouseCursorActive = function (x) {
    if (xMouseCursorActive == x) return;
    xMouseCursorActive = x;
    obj.CanvasId.style.cursor = x == true ? xMouseCursorCurrent : 'default';
  };
  var mouseCursors = [
    'default',
    'progress',
  ...
  obj.SendCompressionLevel = function (type, level, scaling, frametimer) {
    // Type: 1 = JPEG, 2 = PNG, 3 = TIFF, 4 = WebP
    obj.ImageType = type;
    if (level) {
    ...
    obj.send(
      String.fromCharCode(0x00, 0x05, 0x00, 0x0a, type, obj.CompressionLevel) +
        obj.shortToStr(obj.ScalingLevel) +
        obj.shortToStr(obj.FrameRateTimer)
    );
  };
  ...
  obj.ProcessBinaryCommand = function (cmd, cmdsize, view) {
    ...
    for (let i = 0; i < view.length; i += chunkSize) {
      result += String.fromCharCode.apply(null, view.slice(i, i + chunkSize));
    }
    // Record the command if needed
    if (obj.recordedData != null) {
      if (cmdsize > 65000) {
        obj.recordedData.push(
          recordingEntry(
            2,
            1,
            obj.shortToStr(27) +
              obj.shortToStr(8) +
              obj.intToStr(cmdsize) +
              obj.shortToStr(cmd) +
              obj.shortToStr(0) +
              obj.shortToStr(0) +
              obj.shortToStr(0) +
    ...
    switch (cmd) {
      ...
      case 11: // GetDisplays (TODO)
        ...
        obj.selectedDisplay = selectedDisplay;
      ...
      case 18: // MNG_KVM_KEYSTATE
        ...
        if (obj.onKeyboardStateChanged) {
          obj.onKeyboardStateChanged(obj, obj.KeyboardState);
        }
        console.log(
          'MNG_KVM_KEYSTATE:' +
            (obj.KeyboardState & 1 ? ' NumLock' : '') +
            (obj.KeyboardState & 2 ? ' ScrollLock' : '') +
            (obj.KeyboardState & 4 ? ' CapsLock' : '')
        );
        break;
      case 65: // Alert
        var str = String.fromCharCode.apply(null, view.slice(4));
        if (str[0] != '.') {
          ...
          if (obj.parent && obj.parent.setConsoleMessage) {
            obj.parent.setConsoleMessage(str);
          }
        } else {
      ...
      case 82: // DISPLAY LOCATION & SIZE
        ...
        var screenCount = (cmdsize - 4) / 10,
          screenInfo = {},
          ptr = 4;
        for (var i = 0; i < screenCount; i++) {
          screenInfo[(view[ptr + 0] << 8) + view[ptr + 1]] = {
      ...
      case 87: // MNG_KVM_INPUT_LOCK
        ...
        if (obj.RemoteInputLock == null || obj.RemoteInputLock !== (view[4] != 0)) {
          ...
          if (obj.onRemoteInputLockChanged) {
            obj.onRemoteInputLockChanged(obj, obj.RemoteInputLock);
          }
        }
        break;
      case 88: // MNG_KVM_MOUSE_CURSOR
        ...
        if (xMouseCursorActive) {
          obj.CanvasId.style.cursor = xMouseCursorCurrent;
        }
        break;
      default:
  ...
  obj.SendKeyMsg = function (action, event) {
    ...
    if (
      (obj.UseExtendedKeyFlag || urlargs.extkeys == 1) &&
      typeof event.code == 'string' &&
      (event.code.startsWith('Arrow') || extendedKeyTable.indexOf(event.code) >= 0)
    ) {
      extendedKey = true;
    }
  ...
  obj._handleAltGrTimeout = function () {
    //Windows and no Ctrl+Alt -> send only Ctrl.
    obj._altGrArmed = false;
    clearTimeout(obj._altGrTimeout);
  ...
  obj.SendRemoteInputLock = function (code) {
    obj.send(String.fromCharCode(0x00, 87, 0x00, 0x05, code));
  };
  ...
  obj.SendKeyMsgKC = function (action, kc, extendedKey) {
    ...
    if (typeof action == 'object') {
      for (var i in action) {
        obj.SendKeyMsgKC(action[i][0], action[i][1], action[i][2]);
      }
    } else {
      if (action == 1) {
        // Key Down
        if (obj.pressedKeys.indexOf(kc) == -1) {
          obj.pressedKeys.unshift(kc);
        } // Add key press to start of array
      } else if (action == 2) {
        // Key Up
        var i = obj.pressedKeys.indexOf(kc);
        if (i != -1) {
          obj.pressedKeys.splice(i, 1);
        } // Remove the key press from the pressed array
      }
      if (obj.debugmode > 0) {
        console.log('Sending Key ' + kc + ', action ' + action);
      }
  ...
  obj.SendKeyUnicode = function (action, val) {
    if (obj.State != 3) return;
    if (obj.debugmode > 0) {
      console.log('Sending UnicodeKey ' + val + ', action ' + action);
    }
  ...
  obj.SendTouchMsg1 = function (id, flags, x, y) {
    if (obj.State == 3)
      obj.send(
        String.fromCharCode(0x00, obj.InputType.TOUCH) +
          obj.shortToStr(14) +
          String.fromCharCode(0x01, id) +
          obj.intToStr(flags) +
          obj.shortToStr(x) +
          obj.shortToStr(y)
      );
  ...
  obj.SendTouchMsg2 = function (id, flags) {
    ...
    for (var k in obj.TouchArray) {
      ...
      msg +=
        ...
        obj.intToStr(flags2) +
        obj.shortToStr(obj.TouchArray[k].x) +
        obj.shortToStr(obj.TouchArray[k].y);
      if (obj.TouchArray[k].f == 2) delete obj.TouchArray[k];
    }
    if (obj.State == 3)
      obj.send(
        String.fromCharCode(0x00, obj.InputType.TOUCH) +
          obj.shortToStr(5 + msg.length) +
    ...
    if (Action != null && obj.Canvas != null) {
      ...
      if (X >= 0 && X <= obj.Canvas.canvas.width && Y >= 0 && Y <= obj.Canvas.canvas.height) {
        ...
        if (Action == obj.KeyAction.UP || Action == obj.KeyAction.DOWN) {
          if (event.which) {
            event.which == 1
              ? (Button = obj.MouseButton.LEFT)
              : event.which == 2
                ? (Button = obj.MouseButton.MIDDLE)
                : (Button = obj.MouseButton.RIGHT);
        ...
        if (obj.SwapMouse === true) {
          if (Button == obj.MouseButton.LEFT) {
            Button = obj.MouseButton.RIGHT;
          } else if (Button == obj.MouseButton.RIGHT) {
            Button = obj.MouseButton.LEFT;
          }
        }

        // Reverse mouse wheel if needed
        if (obj.ReverseMouseWheel) {
          Delta = -1 * Delta;
        }

        var MouseMsg = '';
        if (Action == obj.KeyAction.DBLCLICK) {
          MouseMsg = String.fromCharCode(
            0x00,
            obj.InputType.MOUSE,
            0x00,
            0x0a,
          ...
          MouseMsg = String.fromCharCode(
            ...
            0x0a,
            0x00,
            Action == obj.KeyAction.DOWN ? Button : (Button * 2) & 0xff,
            (X / 256) & 0xff,
            X & 0xff,
            (Y / 256) & 0xff,
            Y & 0xff
  ...
  obj.SetDisplay = function (number) {
    /*console.log('Set display', number);*/ obj.send(
      String.fromCharCode(0x00, 0x0c, 0x00, 0x06, number >> 8, number & 0xff)
    );
  ...
  obj.onResize = function () {
    ...
    obj.FirstDraw = false;
    if (obj.debugmode > 1) {
      console.log('onResize: ' + obj.ScreenWidth + ' x ' + obj.ScreenHeight);
    }
  };
  ...
  obj.handleReleaseKeys = function () {
    ...
    for (var i in p) {
      obj.SendKeyMsgKC(obj.KeyAction.UP, p[i]);
    } // Release all keys
  };
  ...
  obj.xxMsTouchEvent = function (evt) {
    ...
    if (evt.type == 'MSPointerDown' || evt.type == 'MSPointerMove' || evt.type == 'MSPointerUp') {
      ...
      if (evt.type == 'MSPointerDown')
        flags = 0x00010000 | 0x00000002 | 0x00000004; // POINTER_FLAG_DOWN
    ...
    if (obj.touchenabled == 0 || obj.touchenabled == 1) {
      ...
      for (var i in e.originalEvent.changedTouches) {
        ...
        if (!obj.TouchArray[id]) {
          obj.TouchArray[id] = {
            x: (e.originalEvent.touches[i].pageX - Offsets[0]) * (Canvas.canvas.width / obj.CanvasId.clientWidth),
            y: (e.originalEvent.touches[i].pageY - Offsets[1]) * (Canvas.canvas.height / obj.CanvasId.clientHeight),
            f: 1
          };
        }
      }
      if (Object.keys(obj.TouchArray).length > 0 && touchtimer == null) {
  ...
  obj.xxTouchMove = function (e) {
    ...
    if (obj.touchenabled == 0 || obj.touchenabled == 1) {
      ...
      for (var i in e.originalEvent.changedTouches) {
        ...
        if (obj.TouchArray[id]) {
          obj.TouchArray[id].x =
            (e.originalEvent.touches[i].pageX - Offsets[0]) * (obj.Canvas.canvas.width / obj.CanvasId.clientWidth);
          obj.TouchArray[id].y =
            (e.originalEvent.touches[i].pageY - Offsets[1]) * (obj.Canvas.canvas.height / obj.CanvasId.clientHeight);
        }
      }
    }
  };
  ...
  obj.setRotation = function (x) {
    ...
    if (obj.onScreenSizeChange != null) {
      console.log('s4', obj.ScreenWidth, obj.ScreenHeight);
      obj.onScreenSizeChange(obj, obj.ScreenWidth, obj.ScreenHeight, obj.CanvasId);
    }
    return true;
  };
      ...
      fileReader.onload = function (event) {
        ...
        obj.recordedData.push(
          recordingEntry(
            1,
            0,
            JSON.stringify({ magic: 'MeshCentralRelaySession', ver: 1, time: new Date().toLocaleString(), protocol: 2 })
          )
        ); // Metadata (nodeid: obj.nodeid)
        obj.recordedData.push(
          recordingEntry(
            2,
            1,
            obj.shortToStr(7) + obj.shortToStr(8) + obj.shortToStr(obj.ScreenWidth) + obj.shortToStr(obj.ScreenHeight)
          )
        ...
        if (cmdlen > 65000) {
          ...
          obj.recordedData.push(
            recordingEntry(
              2,
              1,
              obj.shortToStr(3) + obj.shortToStr(cmdlen) + obj.shortToStr(0) + obj.shortToStr(0) + binary
            )
          );
        }
      };
