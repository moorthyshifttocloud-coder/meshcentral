...
var CreateRDPDesktop = function (canvasid, domainUrl) {
  ...
  obj.Start = function (nodeid, port, credentials) {
    ...
    var options = {
      savepass: credentials.savecred,
      useServerCreds: credentials.servercred,
      width: credentials.width,
      height: credentials.height,
      flags: credentials.flags,
      workingDir: credentials.workdir,
      alternateShell: credentials.altshell
    };
      ...
      if (typeof evt.data == 'string') {
        ...
        switch (msg[0]) {
          case 'rdp-connect': {
            ...
            if (obj.m.onScreenSizeChange != null) {
              obj.m.onScreenSizeChange(obj, obj.ScreenWidth, obj.ScreenHeight, obj.CanvasId);
            }
            break;
          }
          case 'rdp-bitmap': {
          ...
          case 'rdp-pointer': {
            ...
            if (xMouseCursorActive) {
              obj.CanvasId.style.cursor = pointer;
            }
            break;
          }
          case 'rdp-close': {
          ...
          case 'rdp-error': {
            ...
            if (msg.length > 2) {
              obj.consoleMessageArgs = [msg[2]];
            }
            switch (msg[1]) {
              case 'NODE_RDP_PROTOCOL_X224_NEG_FAILURE':
                if (msg[2] == 1) {
                  obj.consoleMessageId = 9;
                } // "SSL required by server";
                else if (msg[2] == 2) {
                  obj.consoleMessageId = 10;
                } // "SSL not allowed by server";
                else if (msg[2] == 3) {
                  obj.consoleMessageId = 11;
                } // "SSL certificate not on server";
                else if (msg[2] == 4) {
                  obj.consoleMessageId = 12;
                } // "Inconsistent flags";
                else if (msg[2] == 5) {
                  obj.consoleMessageId = 13;
                } // "Hybrid required by server";
                else if (msg[2] == 6) {
                  obj.consoleMessageId = 14;
          ...
          case 'rdp-clipboard': {
            obj.lastClipboardContent = msg[1];
            if (obj.m.onClipboardChanged) {
              obj.m.onClipboardChanged(msg[1]);
            }
            break;
          }
          case 'ping': {
            obj.socket.send('["pong"]');
            break;
          }
          case 'pong': {
            break;
  ...
  obj.m.getClipboard = function () {
    return obj.lastClipboardContent;
  };
  ...
  obj.m.mousemove = function (e) {
    ...
    if (obj.mouseNagleTimer == null) {
      obj.mouseNagleTimer = setTimeout(function () {
        obj.socket.send(JSON.stringify(obj.mouseNagleData));
        obj.mouseNagleTimer = null;
      }, 50);
    }
    e.preventDefault();
    return false;
  ...
  obj.m.mouseup = function (e) {
    ...
    if (m.x < 0 || m.y < 0 || m.x > obj.ScreenWidth || m.y > obj.ScreenHeight) return;
    if (obj.mouseNagleTimer != null) {
      clearTimeout(obj.mouseNagleTimer);
      obj.mouseNagleTimer = null;
    }
  ...
  obj.m.mousewheel = function (e) {
    ...
    if (obj.m.ReverseMouseWheel) {
      delta = -1 * delta;
    } // Reverse the mouse wheel
    if (delta != 0) {
      obj.socket.send(JSON.stringify(['wheel', m.x, m.y, delta, false, false]));
    }
    e.preventDefault();
    return false;
  };
  ...
  obj.m.SendKeyMsgKC = function (action, kc, extendedKey) {
    ...
    if (typeof action == 'object') {
      for (var i in action) {
        obj.m.SendKeyMsgKC(action[i][0], action[i][1], action[i][2]);
      }
    } else {
