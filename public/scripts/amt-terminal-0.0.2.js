...
var CreateAmtRemoteTerminal = function (divid, options) {
  ...
  obj.protocol = 1; // SOL
  if (options.protocol) {
    obj.protocol = options.protocol;
  } // 1 = Normal, 6 = PowerShell
  ...
  obj.ProcessData = function (str) {
    if (obj.debugmode == 2) {
      console.log('TRecv(' + str.length + '): ' + rstr2hex(str));
    }
    // ###BEGIN###{Terminal-Enumation-UTF8}
    ...
    if (obj.terminalEmulation == 0) {
      try {
        str = decode_utf8(utf8decodeBuffer + str);
      } catch (ex) {
        utf8decodeBuffer += str;
        return;
      }
    } // If we get data in the middle of a UTF-8 code, buffer it for next time.
    utf8decodeBuffer = '';
    // ###END###{Terminal-Enumation-All}
    if (obj.capture != null) obj.capture += str;
    _ProcessVt100EscString(str);
    obj.TermDraw();
  };

  function _ProcessVt100EscString(str) {
    for (var i = 0; i < str.length; i++)
      _ProcessVt100EscChar(String.fromCharCode(str.charCodeAt(i)), str.charCodeAt(i));
  }
        ...
        switch (b) {
          ...
          case 'M':
            ...
            for (var y = _scrollRegion[1]; y >= _scrollRegion[0] + x; y--) {
              for (var z = 0; z < obj.width; z++) {
                _tscreen[y][z] = _tscreen[y - x][z];
                _scratt[y][z] = _scratt[y - x][z];
              }
            }
  ...
  function _ProcessXTermHandler(_escNumber) {
    ...
    if ((cmd == 0 || cmd == 2) && _escNumber.length > 1 && _escNumber[1] != '?') {
      if (obj.onTitleChange) {
        obj.onTitleChange(obj, (obj.title = _escNumber[1]));
      }
    }
  }
    ...
    if (mode == 1) {
      ...
      switch (code) {
        ...
        case 'P': // Delete X Character(s), default 1 char
          ...
          for (i = _termx; i < obj.width - x; i++) {
            _tscreen[_termy][i] = _tscreen[_termy][i + x];
            _scratt[_termy][i] = _scratt[_termy][i + x];
          }
          for (i = obj.width - x; i < obj.width; i++) {
            _tscreen[_termy][i] = ' ';
            _scratt[_termy][i] = 7 << 6;
          }
          break;
        ...
        case 'r': // Set the scroll region
          if (argslen == 2) {
            _scrollRegion = [args[0] - 1, args[1] - 1];
          }
          if (_scrollRegion[0] < 0) {
            _scrollRegion[0] = 0;
          }
          if (_scrollRegion[0] > obj.height - 1) {
            _scrollRegion[0] = obj.height - 1;
          }
          if (_scrollRegion[1] < 0) {
            _scrollRegion[1] = 0;
          }
          if (_scrollRegion[1] > obj.height - 1) {
            _scrollRegion[1] = obj.height - 1;
          }
          if (_scrollRegion[0] > _scrollRegion[1]) {
            _scrollRegion[0] = _scrollRegion[1];
          }
          break;
  ...
  var AsciiToUnicode = [
    ...
    0x00a3, 0x00a5, 0x20a7, 0x0192, 0x00e1, 0x00ed, 0x00f3, 0x00fa, 0x00f1, 0x00d1, 0x00aa, 0x00da, 0x00bf, 0x2310,
    0x00ac, 0x00bd, 0x00bc, 0x00a1, 0x00ab, 0x00bb, 0x2593, 0x2592, 0x2591, 0x2502, 0x2524, 0x2561, 0x2562, 0x2556,
    0x2555, 0x2563, 0x2551, 0x2557, 0x255d, 0x255c, 0x255b, 0x2510, 0x2514, 0x2534, 0x252c, 0x251c, 0x2500, 0x253c,
    0x255e, 0x255f, 0x255a, 0x2554, 0x2569, 0x2566, 0x2560, 0x2550, 0x256c, 0x2567, 0x2568, 0x2564, 0x2565, 0x2568,
    0x2558, 0x2552, 0x2553, 0x256b, 0x256a, 0x2518, 0x250c, 0x2588, 0x2584, 0x258b, 0x2590, 0x2580, 0x03b1, 0x00df,
    0x0393, 0x03c0, 0x03a3, 0x03c3, 0x00b5, 0x03c4, 0x03c6, 0x03b8, 0x2126, 0x03b4, 0x221e, 0x00f8, 0x03b5, 0x220f,
    0x2261, 0x00b1, 0x2265, 0x2266, 0x2320, 0x2321, 0x00f7, 0x2248, 0x00b0, 0x2022, 0x00b7, 0x221a, 0x207f, 0x00b2,
    0x220e, 0x00a0
  ];
  ...
  var AsciiToUnicodeIntel = [
    ...
    0x00c4, 0x00c5, 0x00c9, 0x00e6, 0x00c6, 0x00f4, 0x00f6, 0x00f2, 0x00fb, 0x00f9, 0x00ff, 0x00d6, 0x00dc, 0x00a2,
    0x00a3, 0x00a5, 0x20a7, 0x0192, 0x00e1, 0x00ed, 0x00f3, 0x00fa, 0x00f1, 0x00d1, 0x00aa, 0x00da, 0x00bf, 0x2310,
    0x00ac, 0x00bd, 0x00bc, 0x00a1, 0x00ae, 0x00bb, 0x2593, 0x2592, 0x2591, 0x2502, 0x2524, 0x2561, 0x2562, 0x2556,
    0x2555, 0x2563, 0x2551, 0x2557, 0x255d, 0x255c, 0x255b, 0x2510, 0x2514, 0x2534, 0x252c, 0x251c, 0x2500, 0x253c,
    0x255e, 0x255f, 0x255a, 0x2554, 0x2569, 0x2566, 0x2560, 0x2550, 0x256c, 0x2567, 0x2568, 0x2564, 0x2565, 0x2568,
    0x2558, 0x2552, 0x2553, 0x256b, 0x256a, 0x2518, 0x250c, 0x2588, 0x2584, 0x258b, 0x2590, 0x2580, 0x03b1, 0x00df,
    0x0393, 0x03c0, 0x03a3, 0x03c3, 0x00b5, 0x03c4, 0x03c6, 0x03b8, 0x2126, 0x03b4, 0x221e, 0x00f8, 0x03b5, 0x220f,
  ...
  function _ProcessVt100Char(c) {
    ...
    if (obj.terminalEmulation == 1) {
      ...
      if ((ch & 0x80) != 0) {
        c = String.fromCharCode(AsciiToUnicode[ch & 0x7f]);
      }
    } else if (obj.terminalEmulation == 2) {
      // ANSI - Intel Extended ASCII emulation.
      if ((ch & 0x80) != 0) {
        c = String.fromCharCode(AsciiToUnicodeIntel[ch & 0x7f]);
      }
    }
    // ###END###{Terminal-Enumation-All}
    ...
    switch (ch) {
      case 16: {
        c = ' ';
        break;
      } // This is an odd char that show up on Intel BIOS's.
      case 24: {
        c = '↑';
        break;
      }
  ...
  obj.TermSendKeys = function (keys) {
    if (obj.debugmode == 2) {
      console.log('TSend(' + keys.length + '): ' + rstr2hex(keys), keys);
    }
    obj.parent.send(keys);
  };
  obj.TermSendKey = function (key) {
    if (obj.debugmode == 2) {
      console.log('TSend(1): ' + rstr2hex(String.fromCharCode(key)), key);
    }
    obj.parent.send(String.fromCharCode(key));
  };
  ...
  obj.TermHandleKeyDown = function (e) {
    ...
    if (e.which == 27) {
      obj.TermSendKeys(String.fromCharCode(27));
      return true;
    } // ESC

    if (_altKeypadMode == true) {
      if (e.which == 37) {
        obj.TermSendKeys(String.fromCharCode(27, 79, 68));
        return true;
      } // Left
      if (e.which == 38) {
        obj.TermSendKeys(String.fromCharCode(27, 79, 65));
        return true;
      } // Up
      if (e.which == 39) {
        obj.TermSendKeys(String.fromCharCode(27, 79, 67));
        return true;
      } // Right
      if (e.which == 40) {
        obj.TermSendKeys(String.fromCharCode(27, 79, 66));
        return true;
      } // Down
    } else {
      if (e.which == 37) {
        obj.TermSendKeys(String.fromCharCode(27, 91, 68));
        return true;
      } // Left
      if (e.which == 38) {
        obj.TermSendKeys(String.fromCharCode(27, 91, 65));
        return true;
      } // Up
      if (e.which == 39) {
        obj.TermSendKeys(String.fromCharCode(27, 91, 67));
        return true;
      } // Right
      if (e.which == 40) {
        obj.TermSendKeys(String.fromCharCode(27, 91, 66));
        return true;
      } // Down
    }

    if (e.which == 33) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 53, 126));
      return true;
    } // PageUp
    if (e.which == 34) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 54, 126));
      return true;
    } // PageDown
    if (e.which == 35) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 70));
      return true;
    } // End
    if (e.which == 36) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 72));
      return true;
    } // Home
    if (e.which == 45) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 50, 126));
      return true;
    } // Insert
    if (e.which == 46) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 51, 126));
      return true;
    } // Delete
    ...
    if ((e.which > 111) & (e.which < 124) && e.repeat == false) {
      // F1 to F12 keys
      if (obj.fxEmulation == 0 && e.which < 122) {
        obj.TermSendKeys(String.fromCharCode(27, 91, 79, fx0[e.which - 112]));
        return true;
      } // 'Intel (F10 = ESC+[OM)'
      if (obj.fxEmulation == 1) {
        obj.TermSendKeys(String.fromCharCode(27, fx1[e.which - 112]));
        return true;
      } // 'Alternate (F10 = ESC+0)'
      if (obj.fxEmulation == 2) {
        obj.TermSendKeys(String.fromCharCode(27, 79, fx2[e.which - 112]));
        return true;
      } // 'VT100+ (F10 = ESC+[OY)'
    }
    ...
    var fx0 = [80, 81, 119, 120, 116, 117, 113, 114, 112, 77];
    if ((e.which > 111) & (e.which < 122) && e.repeat == false) {
      obj.TermSendKeys(String.fromCharCode(27, 91, 79, fx0[e.which - 112]));
      return true;
    } // 'Intel (F10 = ESC+[OM)'
  ...
  obj.TermDrawLine = function (buf, y, closetag) {
    ...
    for (var x = 0; x < obj.width; ++x) {
      newat = _scratt[y][x];
      if (_termx == x && _termy == y && _cursorVisible) {
        newat |= _VTREVERSE;
      } // If this is the cursor location, reverse the color.
      if (newat != oldat) {
        buf += closetag;
        closetag = '';
        x1 = 6;
        ...
        buf +=
          '<span style="color:#' +
          _TermColors[(newat >> x1) & 0x3f] +
          ';background-color:#' +
          _TermColors[(newat >> x2) & 0x3f];
  ...
  obj.TermDraw = function () {
    ...
    if (scrollBackBuffer.length > 800) {
      scrollBackBuffer = scrollBackBuffer.slice(scrollBackBuffer.length - 800);
    }
    ...
    if (obj.heightLock == 0) {
      setTimeout(obj.TermLockHeight, 10);
    }
  };
  ...
  obj.TermInit = function () {
    obj.TermResetScreen();
  };

  obj.heightLock = 0;
  obj.DivElement.style['height'] = '';
  if (options != null && options.cols != null && options.rows != null) {
    obj.Init(options.cols, options.rows);
  } else {
    obj.Init();
  }
  return obj;
