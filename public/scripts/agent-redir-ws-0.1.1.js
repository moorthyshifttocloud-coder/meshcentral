...
var CreateAgentRedirect = function (meshserver, module, serverPublicNamePort, authCookie, rauthCookie, domainUrl) {
  ...
  obj.Start = function (nodeid) {
    ...
    if (urlargs != null && urlargs.slowrelay != null) {
      url += '&slowrelay=' + urlargs.slowrelay;
    }
    obj.nodeid = nodeid;
    obj.connectstate = 0;
    console.log('[MeshCentral] [1] MeshCentral Relay  -- Connecting...');
    ...
    if (obj.meshserver != null) {
      var rurl = '*' + domainUrl + 'meshrelay.ashx?p=' + obj.protocol + '&nodeid=' + nodeid + '&id=' + obj.tunnelid;
      if (rauthCookie != null && rauthCookie != '') {
        rurl += '&rauth=' + rauthCookie;
      }
      obj.meshserver.send({ action: 'msg', type: 'tunnel', nodeid: obj.nodeid, value: rurl, usage: obj.protocol });
    }
  };
  ...
  obj.xxOnControlCommand = function (msg) {
    ...
    if (controlMsg.ctrlChannel != '102938') {
      if (obj.m.ProcessData) {
        obj.m.ProcessData(msg);
      } else {
        console.log(msg);
      }
      return;
    ...
    if (typeof args != 'undefined' && args.redirtrace) {
      console.log('RedirRecv', controlMsg);
    }
    if (controlMsg.type == 'console') {
      obj.setConsoleMessage(controlMsg.msg, controlMsg.msgid, controlMsg.msgargs, controlMsg.timeout);
    } else if (controlMsg.type == 'metadata') {
      ...
      obj.latency.current = new Date().getTime() - controlMsg.time;
      if (obj.latency.callback != null) {
        obj.latency.callback(obj.latency.current);
      }
    } else if (obj.webrtc != null) {
      if (controlMsg.type == 'answer') {
        ...
        for (var li = 0; li < rawLines.length; li++) {
          if (rawLines[li]) {
            console.log('[MeshCentral]     SDP[' + li + ']: ' + rawLines[li]);
          }
        }
        // var cleanLines = [];
        ...
        obj.webrtc.setRemoteDescription(
          new RTCSessionDescription(cleanAnswer),
          function () {
        ...
        if (controlMsg.candidate != null) {
          try {
            obj.webrtc.addIceCandidate(new RTCIceCandidate(controlMsg.candidate));
          } catch (ex) {}
        }
      } else if (controlMsg.type == 'webrtc0') {
  ...
  obj.setConsoleMessage = function (str, id, args, timeout) {
    ...
    if (obj.onConsoleMessageChange) {
      obj.onConsoleMessageChange(obj, obj.consoleMessage, obj.consoleMessageId);
    }
  };
  ...
  function buildVanillaIceSDP(baseSdp) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (l.startsWith('m=')) {
        inApp = l.startsWith('m=application');
      }
  ...
  obj.xxOnMessage = function (e) {
    if (obj.State < 3) {
      if (e.data == 'c' || e.data == 'cr') {
        ...
        if (obj.options != null) {
          delete obj.options.action;
          obj.options.type = 'options';
          try {
            obj.sendCtrlMsg(JSON.stringify(obj.options));
          } catch (ex) {}
        }
        try {
        ...
        if (obj.attemptWebRTC == true) {
          ...
          if (obj.webrtc != null && obj.webrtc.createDataChannel) {
                  ...
                  .then(function (stats) {
                    ...
                    stats.forEach(function (report) {
                      if (!logged && report.type === 'candidate-pair' && report.state === 'succeeded') {
                        ...
                        if (localType === 'relay' || remoteType === 'relay') {
                          console.log(
                            '[MeshCentral] [6] TURN Peer-to-Peer      -- Connected (traffic via TURN relay server)'
                          );
                        } else if (
                          localType === 'srflx' ||
                          remoteType === 'srflx' ||
                          ...
                          console.log(
                            '[MeshCentral] [6] STUN Peer-to-Peer      -- Connected (NAT traversal, direct peer)'
                          );
                        } else {
                          console.log('[MeshCentral] [6] Direct Peer-to-Peer    -- Connected (LAN)');
                        }
            ...
            obj.webchannel.onclose = function () {
              if (obj.webRtcActive) {
                console.log('[MeshCentral] Session Disconnected -- WebRTC DataChannel closed');
                obj.Stop();
              }
            };
            ...
            function sendVanillaOffer() {
              ...
              console.log(
                '[MeshCentral] [5b] Sending vanilla-ICE offer — host:' +
                  nHost +
                  ' stun:' +
                  nStun +
                  ' turn:' +
              ...
              if (nTurn === 0) {
                console.log(
                  '[MeshCentral] [!] WARNING: No TURN candidate in final SDP — agent cannot fall back to TURN relay.'
                );
              }

              try {
                obj.sendCtrlMsg(JSON.stringify(offer));
              } catch (ex) {}
            }
              ...
              if (obj.webrtc != null) {
                ...
                if (obj.webrtc.getStats) {
                  obj.webrtc
                    ...
                    .then(function (stats) {
                      stats.forEach(function (report) {
                        if (report.type === 'candidate-pair') {
                          ...
                          var remote = stats.get(report.remoteCandidateId);
                          console.log(
                            '[MeshCentral]     CandidatePair state=' +
                              report.state +
                              ' local=' +
                              (local ? local.candidateType + '/' + local.protocol + '/' + local.address : '?') +
                              ' remote=' +
                              (remote ? remote.candidateType + '/' + remote.protocol + '/' + remote.address : '?') +
                              ' nominated=' +
                              report.nominated +
                              ' bytesSent=' +
                              (report.bytesSent || 0)
                    ...
                    .catch(function (e) {
                      console.log('[MeshCentral]     getStats failed: ' + e);
                    });
                }
                if (s === 'checking') {
                  console.log('[MeshCentral]     STUN/TURN WebRTC    -- Checking candidates...');
                  if (_iceCheckTimer) {
                    clearTimeout(_iceCheckTimer);
                  }
                  _iceCheckTimer = setTimeout(function () {
                    _iceCheckTimer = null;
                    if (obj.webrtc && obj.webrtc.iceConnectionState === 'checking') {
                      console.log(
                        '[MeshCentral] [!] STUN/TURN WebRTC    -- Timed out in "checking", staying on Relay.'
                      );
                      obj.xxCloseWebRTC();
                    }
                  }, 25000);
                  ...
                  if (obj.webRtcActive == true) {
                    console.log(
                      '[MeshCentral]     WebRTC ICE         -- Temporarily disconnected, waiting to recover...'
                    );
                    setTimeout(function () {
                      if (
                        obj.webrtc &&
                        (obj.webrtc.iceConnectionState === 'disconnected' || obj.webrtc.iceConnectionState === 'failed')
                      ) {
                        console.log(
                          '[MeshCentral] [-] WebRTC Disconnected  -- Could not recover, tearing down session.'
                        );
                        obj.Stop();
                      }
                    }, 6000);
            ...
            obj.webrtc.createOffer(
              function (offer) {
                obj.webrtcoffer = offer;
                obj.webrtc.setLocalDescription(
                  offer,
                  function () {
                    ...
                    _gatherTimer = setTimeout(function () {
                      _gatherTimer = null;
                      console.log(
                        '[MeshCentral] [!] ICE Gathering       -- 8s timeout, sending with whatever was gathered...'
                      );
                      sendVanillaOffer();
                    }, 8000);
                  },
    ...
    if (typeof e.data == 'string') {
      ...
      if (obj.m.ProcessBinaryCommand) {
        ...
        if (cmdAccLen != 0) {
          ...
          cmdAccLen += view.byteLength;
          if (cmdAccCmdSize <= cmdAccLen) {
            var tmp = new Uint8Array(cmdAccLen),
              tmpPtr = 0;
            for (var i in cmdAcc) {
              tmp.set(cmdAcc[i], tmpPtr);
              tmpPtr += cmdAcc[i].byteLength;
            }
            obj.m.ProcessBinaryCommand(cmdAccCmd, cmdAccCmdSize, tmp);
            cmdAccCmd = 0;
            cmdAccCmdSize = 0;
        ...
        if (e.data.byteLength < 16000) {
          ...
          var bb = new Blob([new Uint8Array(e.data)]),
            f = new FileReader();
          f.onload = function (e) {
            obj.m.ProcessData(e.target.result);
          };
          f.readAsBinaryString(bb);
        }
      }
  ...
  obj.send = function (x) {
    if (typeof args != 'undefined' && args.redirtrace) {
      console.log('RedirSend', typeof x, x.length, x[0] == '{' ? x : rstr2hex(x).substring(0, 64));
    }
    try {
      if (obj.socket != null && obj.socket.readyState == WebSocket.OPEN) {
        if (typeof x == 'string') {
  ...
  obj.Stop = function (x) {
    ...
    if (obj.State != 0) {
      console.log('Executing obj.Stop(), tearing down session.');
    }
    obj.xxCloseWebRTC();
    obj.latency.current = -1;
    obj.connectstate = -1;
