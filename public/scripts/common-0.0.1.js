...
function QVH(x, y) {
  try {
    y ? Q(x).classList.remove('visually-hidden') : Q(x).classList.add('visually-hidden');
  } catch (x) {}
} // "Q" visibility

// Move cursor to end of input box
function inputBoxFocus(x) {
  Q(x).focus();
  var v = Q(x).value;
  Q(x).value = '';
  Q(x).value = v;
...
function ArrayElementMove(arr, from, to) {
...
function parseUriArgs(decodeUrl) {
  ...
  for (var j in parsedUri) {
    ...
    if (decodeUrl) {
      r[name] = decodeURIComponent(arg.substring(i + 1));
    }
    if (!isSafeString2(r[name])) {
      delete r[name];
    } else {
