...
var CreateAmtRedirect = function (module, authCookie) {
  ...
  function randomHex(length) {
    ...
    for (var i = 0; i < length; i++) {
      r += 'abcdef0123456789'.charAt(Math.floor(Math.random() * 16));
    }
    return r;
  }
  ...
  obj.Start = function (host, port, user, pass, tls) {
    ...
    var url =
      ...
      tls +
      (user == '*' ? '&serverauth=1' : '') +
      (typeof pass === 'undefined' ? '&serverauth=1&user=' + user : ''); // The 'p=2' indicates to the relay that this is a REDIRECTION session
  ...
  obj.xxOnMessage = function (e) {
    ...
    while (obj.acc != null && obj.acc.byteLength >= 1) {
      var cmdsize = 0,
        accArray = new Uint8Array(obj.acc);
      switch (accArray[0]) {
        ...
        case 0x14: // AuthenticateSessionReply (20)
          ...
          var status = accArray[1],
            authType = accArray[4],
            authData = [];
          for (i = 0; i < authDataLen; i++) {
            authData.push(accArray[9 + i]);
          }
              ...
              case 1: {
                ...
                obj.xxSend(
                  String.fromCharCode(0x20, 0x00, 0x00, 0x00) +
                    IntToStrX(obj.amtsequence++) +
                    ShortToStrX(MaxTxBuffer) +
                    ShortToStrX(TxTimeout) +
                    ShortToStrX(TxOverflowTimeout) +
                    ShortToStrX(RxTimeout) +
                    ShortToStrX(RxFlushTimeout) +
        ...
        case 0x2a: // Incoming display data (42)
          ...
          if (obj.m.ProcessBinaryData) {
            obj.m.ProcessBinaryData(new Uint8Array(accArray.buffer.slice(10, cs)));
          } else {
            obj.m.ProcessData(arrToStr(new Uint8Array(accArray.buffer.slice(10, cs))));
          }
          cmdsize = cs;
          break;
        ...
        case 0x41:
          ...
          if (accArray.byteLength > 8) {
            if (obj.m.ProcessBinaryData) {
              obj.m.ProcessBinaryData(new Uint8Array(accArray.buffer.slice(8)));
            } else {
              obj.m.ProcessData(arrToStr(new Uint8Array(accArray.buffer.slice(8))));
            }
          }
  ...
  obj.Send = obj.send = function (x) {
    ...
    if (obj.protocol == 1) {
      obj.xxSend(
        String.fromCharCode(0x28, 0x00, 0x00, 0x00) + IntToStrX(obj.amtsequence++) + ShortToStrX(x.length) + x
      );
    } else {
      obj.xxSend(x);
    }
    ...
    if (obj.inDataCount == 0 && obj.tlsv1only == 0) {
      ...
      obj.socket = new WebSocket(
        window.location.protocol.replace('http', 'ws') +
          ...
          obj.tls +
          '&tls1only=1' +
          (obj.user == '*' ? '&serverauth=1' : '') +
          (typeof pass === 'undefined' ? '&serverauth=1&user=' + obj.user : '')
      ); // The 'p=2' indicates to the relay that this is a REDIRECTION session
      obj.socket.binaryType = 'arraybuffer';
      obj.socket.onopen = obj.xxOnSocketConnected;
      obj.socket.onmessage = obj.xxOnMessage;
      obj.socket.onclose = obj.xxOnSocketClosed;
