...
module.exports.CreateDB = function (parent, func) {
  ...
  obj.pluginsActive =
    ...
    parent.config.settings.plugins != null &&
    parent.config.settings.plugins != false &&
    (typeof parent.config.settings.plugins != 'object' || parent.config.settings.plugins.enabled != false);
  obj.dbCounters = {
    fileSet: 0,
    fileRemove: 0,
    powerSet: 0,
  ...
  obj.SetupDatabase = function (func) {
    ...
    obj.Get('DatabaseIdentifier', function (err, docs) {
      if (err != null) {
        parent.debug('db', 'ERROR (Get DatabaseIdentifier): ' + err);
      }
    ...
    obj.Get('SchemaVersion', function (err, docs) {
      if (err != null) {
        parent.debug('db', 'ERROR (Get SchemaVersion): ' + err);
      }
      var ver = 0;
      ...
      if (ver == 1) {
        console.log('This is an unsupported beta 1 database, delete it to create a new one.');
        process.exit(0);
      }
  ...
  obj.maintenance = function () {
    ...
    if (obj.databaseType == DB_NEDB) {
      // NeDB will not remove expired records unless we try to access them. This will force the removal.
      ...
      obj.serverstatsfile.remove(
        { time: { $lt: new Date(Date.now() - expireServerStatsSeconds * 1000) } },
        { multi: true }
      ); // Force delete older events
    } else if (obj.databaseType == DB_MARIADB || obj.databaseType == DB_MYSQL) {
      // MariaDB or MySQL
      sqlDbQuery(
        'DELETE FROM events WHERE time < ?',
        [new Date(Date.now() - expireEventsSeconds * 1000)],
        function (doc, err) {}
      ); // Delete events older than expireEventsSeconds
      sqlDbQuery(
        'DELETE FROM power WHERE time < ?',
        [new Date(Date.now() - expirePowerEventsSeconds * 1000)],
        function (doc, err) {}
      ); // Delete events older than expirePowerSeconds
      ...
      sqlDbQuery(
        'DELETE FROM power WHERE time < $1',
        [new Date(Date.now() - expirePowerEventsSeconds * 1000)],
        function (doc, err) {}
      ); // Delete events older than expirePowerSeconds
      sqlDbQuery(
        'DELETE FROM serverstats WHERE time < $1',
        [new Date(Date.now() - expireServerStatsSeconds * 1000)],
        function (doc, err) {}
      ); // Delete server stats older than expireServerStatsSeconds
      ...
      obj.file
        .query('events')
        .filter('time', '<', new Date(Date.now() - expireEventsSeconds * 1000))
        .remove()
        .then(function () {
          obj.file
            .query('stats')
            .filter('time', '<', new Date(Date.now() - expireServerStatsSeconds * 1000))
            .remove()
            .then(function () {
              obj.file
                .query('power')
                .filter('time', '<', new Date(Date.now() - expirePowerEventsSeconds * 1000))
                .remove()
                .then(function () {
      ...
      obj.file.serialize(function () {
        ...
        obj.file.exec(obj.sqliteConfig.maintenance, function (err) {
          ...
          if (parent.config.settings.debug) {
            sqliteGetPragmas(
              ['freelist_count', 'page_size', 'page_count', 'cache_size'],
              function (pragma, pragmaValue) {
  ...
  obj.removeInactiveDevices = function (showall, cb) {
    ...
    var removeInactiveDevicesPerDomain = {},
      minRemoveInactiveDevicesPerDomain = {},
      minRemoveInactiveDevice = 9999;
    for (var i in parent.config.domains) {
      if (typeof parent.config.domains[i].autoremoveinactivedevices == 'number') {
        ...
        if (v >= 1 && v <= 2000) {
          if (v < minRemoveInactiveDevice) {
            minRemoveInactiveDevice = v;
          }
    ...
    for (var i in parent.webserver.meshes) {
      if (typeof parent.webserver.meshes[i].expireDevs == 'number') {
        ...
        if (v >= 1 && v <= 2000) {
          ...
          if (
            minRemoveInactiveDevicesPerDomain[parent.webserver.meshes[i].domain] == null ||
    ...
    if (minRemoveInactiveDevice == 9999) {
      if (cb) {
        cb('No device removal policy set, nothing to do.');
      }
      return;
    }
    const now = Date.now();
    ...
    for (var domainid in minRemoveInactiveDevicesPerDomain) {
      obj.GetAllTypeNoTypeField('lastconnect', domainid, function (err, docs) {
        ...
        for (var j in docs) {
          ...
          var expireDays = -1;
          if (removeInactiveDevicesPerDomain[docs[j].domain]) {
            expireDays = removeInactiveDevicesPerDomain[docs[j].domain];
          }
          const mesh = parent.webserver.meshes[docs[j].meshid];
          if (mesh && typeof mesh.expireDevs == 'number') {
            expireDays = mesh.expireDevs;
          }
          var remove = false;
          if (expireDays > 0) {
            if (expireDays < days) {
              ...
              if (conn == null) {
                ...
                obj.Get(nodeid, function (err, docs) {
                  ...
                  if (docs == null || docs.length != 1) {
                    obj.Remove('lc' + nodeid);
                    return;
                  } // Remove last connect time
                  const node = docs[0];
                      ...
                      if (i.startsWith('user/')) {
                        ...
                        if (cuser != null && cuser.links != null && cuser.links[node._id] != null) {
                          ...
                          var event = {
                            ...
                            domain: node.domain,
                            account: parent.webserver.CloneSafeUser(cuser)
                          };
                          if (obj.changeStream) {
                            event.noact = 1;
                  ...
                  if (
                    parent.webserver.meshes[node.meshid] != null &&
                    parent.webserver.meshes[node.meshid].name != null
                  ) {
                    meshname = parent.webserver.meshes[node.meshid].name;
                  }
                  var event = {
                    etype: 'node',
                    action: 'removenode',
                    nodeid: node._id,
                    msgid: 87,
                    msgArgs: [node.name, meshname],
                    msg: 'Removed device ' + node.name + ' from device group ' + meshname,
                    domain: node.domain
                  };
  ...
  obj.removeDomain = function (domainName, func) {
    ...
    if (obj.databaseType == DB_ACEBASE) {
      ...
      pendingCalls = 2;
      sqlDbQuery('DELETE FROM main WHERE domain = $1', [domainName], function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      sqlDbQuery('DELETE FROM events WHERE domain = $1', [domainName], function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      ...
      // MongoDB
      pendingCalls = 3;
      obj.file.deleteMany({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      obj.eventsfile.deleteMany({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      obj.powerfile.deleteMany({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      ...
      // NeDB or MongoJS
      pendingCalls = 3;
      obj.file.remove({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      obj.eventsfile.remove({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
      obj.powerfile.remove({ domain: domainName }, { multi: true }, function () {
        if (--pendingCalls == 0) {
          func();
        }
      });
  ...
  obj.cleanup = function (func) {
    ...
    obj.GetAllType('ugrp', function (err, docs) {
      if (err != null) {
        parent.debug('db', 'ERROR (GetAll user): ' + err);
      }
        ...
        if (err == null && docs.length > 0) {
          for (var i in docs) {
            ...
            if (docs[i].email && docs[i].email != docs[i].email.toLowerCase()) {
              docs[i].email = docs[i].email.toLowerCase();
              fixed = true;
            }

            // Fix account creation
            if (docs[i].creation) {
              if (docs[i].creation > 1300000000000) {
                docs[i].creation = Math.floor(docs[i].creation / 1000);
                fixed = true;
              }
              if (docs[i].creation % 1 != 0) {
                docs[i].creation = Math.floor(docs[i].creation);
                fixed = true;
              }
            }
            ...
            if (docs[i].login) {
              if (docs[i].login > 1300000000000) {
                docs[i].login = Math.floor(docs[i].login / 1000);
                fixed = true;
              }
              if (docs[i].login % 1 != 0) {
                docs[i].login = Math.floor(docs[i].login);
                fixed = true;
              }
            }
            ...
            if (docs[i].passchange) {
              if (docs[i].passchange > 1300000000000) {
                docs[i].passchange = Math.floor(docs[i].passchange / 1000);
                fixed = true;
              }
              if (docs[i].passchange % 1 != 0) {
                docs[i].passchange = Math.floor(docs[i].passchange);
                fixed = true;
              }
            }

            // Fix subscriptions
            if (docs[i].subscriptions != null) {
              delete docs[i].subscriptions;
              fixed = true;
            }
          ...
          obj.GetAllType('mesh', function (err, docs) {
            if (err != null) {
              parent.debug('db', 'ERROR (GetAll mesh): ' + err);
            }
            var meshlist = [];
            if (err == null && docs.length > 0) {
              for (var i in docs) {
                ...
                if (typeof docs[i].mtype == 'string') {
                  docs[i].mtype = parseInt(docs[i].mtype);
                  meshChange = true;
                }

                // If the device group is deleted, remove any invite codes
                if (docs[i].deleted && docs[i].invite) {
                  delete docs[i].invite;
                  meshChange = true;
                }
                ...
                if (docs[i].links != null) {
                  for (var j in docs[i].links) {
                    if (validIdentifiers[j] == null) {
                      ...
                      var xid = j,
                        xid2 = common.unEscapeFieldName(xid);
                      while (xid != xid2 && validIdentifiers[xid2] == null) {
                        xid = xid2;
                        xid2 = common.unEscapeFieldName(xid2);
                      }
            ...
            if (obj.databaseType == DB_SQLITE) {
              ...
              // Postgres
              sqlDbQuery(
                "DELETE FROM Main WHERE ((extra != NULL) AND (extra LIKE ('mesh/%')) AND (extra != ANY ($1)))",
                [meshlist],
              ...
              // MariaDB
              sqlDbQuery(
                'DELETE FROM Main WHERE (extra LIKE ("mesh/%") AND (extra NOT IN ?)',
                [meshlist],
                function (err, response) {}
              );
            } else if (obj.databaseType == DB_MONGODB) {
    ...
    if (obj.databaseType == DB_ACEBASE) {
      ...
      obj.file.aggregate([{ $group: { _id: '$type', count: { $sum: 1 } } }]).toArray(function (err, docs) {
        ...
        if (err == null) {
          for (var i in docs) {
            if (docs[i]._id != null) {
              counters[docs[i]._id] = docs[i].count;
              totalCount += docs[i].count;
            }
          }
        }
        func(counters);
                      ...
                      obj.file.count({ type: 'lastconnect' }, function (err, lastconnectCount) {
                        obj.file.count({}, function (err, totalCount) {
                          func({
                            ...
                            mesh: meshCount,
                            user: userCount,
                            sysinfo: sysinfoCount,
                            iploc: iplocCount,
                            note: noteCount,
                            ifinfo: ifinfoCount,
                            cfile: cfileCount,
                            lastconnect: lastconnectCount,
                            total: totalCount
                          });
                        });
                      });
  ...
  obj.getValueOfTheDay = function (id, startValue, func) {
    obj.Get(id, function (err, docs) {
      var date = new Date(),
        t = date.toLocaleDateString();
      if (err == null && docs.length == 1) {
        var r = docs[0];
        if (r.day == t) {
          func({ _id: id, value: r.value, day: t });
          return;
        }
      }
      func({ _id: id, value: startValue, day: t });
    });
  };
  obj.escapeBase64 = function escapeBase64(val) {
  ...
  obj.performRecordEncryptionRecode = function (func) {
    ...
    obj.GetAllType('user', function (err, docs) {
      if (err != null) {
        parent.debug('db', 'ERROR (performRecordEncryptionRecode): ' + err);
      }
      if (err == null) {
        for (var i in docs) {
          count++;
      ...
      obj.GetAllType('node', function (err, docs) {
        ...
        obj.GetAllType('mesh', function (err, docs) {
          ...
          if (obj.databaseType == DB_NEDB) {
            // If we are using NeDB, compact the database.
            obj.file.compactDatafile();
            obj.file.on('compaction.done', function () {
              func(count);
            }); // It's important to wait for compaction to finish before exit, otherwise NeDB may corrupt.
          } else {
  ...
  function performTypedRecordDecrypt(data) {
    ...
    for (var i in data) {
      ...
      if (data[i].intelamt != null && typeof data[i].intelamt == 'object' && data[i].intelamt._CRYPT) {
        data[i].intelamt = performPartialRecordDecrypt(data[i].intelamt);
      }
      if (data[i].amt != null && typeof data[i].amt == 'object' && data[i].amt._CRYPT) {
        data[i].amt = performPartialRecordDecrypt(data[i].amt);
      }
      if (data[i].kvm != null && typeof data[i].kvm == 'object' && data[i].kvm._CRYPT) {
  ...
  function performTypedRecordEncrypt(data) {
    ...
    if (data.type == 'user') {
      return performPartialRecordEncrypt(Clone(data), [
        'otpkeys',
        'otphkeys',
        'otpsecret',
        'salt',
      ...
      if (data.ssh || data.rdp) {
        xdata = performPartialRecordEncrypt(xdata, ['ssh', 'rdp']);
      }
      if (data.intelamt) {
        xdata.intelamt = performPartialRecordEncrypt(xdata.intelamt, ['pass', 'mpspass']);
      }
      return xdata;
    } else if (data.type == 'mesh' && (data.amt || data.kvm)) {
      var xdata = Clone(data);
      if (data.amt) {
        xdata.amt = performPartialRecordEncrypt(xdata.amt, ['password']);
      }
      if (data.kvm) {
        xdata.kvm = performPartialRecordEncrypt(xdata.kvm, ['pass']);
      }
      return xdata;
    }
    return data;
  ...
  function performPartialRecordEncrypt(plainobj, encryptNames) {
    ...
    if (enclen > 0) {
      plainobj._CRYPT = performRecordEncrypt(enc);
    } else {
  ...
  function performRecordDecrypt(ciphertext) {
    ...
    try {
      ...
      throw 'Incorrect DbRecordsDecryptKey/DbRecordsEncryptKey or invalid database _CRYPT data: ' + e;
    }
    return r;
  }
  ...
  if (typeof parent.args.dbexpire == 'object') {
    if (typeof parent.args.dbexpire.events == 'number') {
      expireEventsSeconds = parent.args.dbexpire.events;
    }
    if (typeof parent.args.dbexpire.powerevents == 'number') {
      expirePowerEventsSeconds = parent.args.dbexpire.powerevents;
    }
    if (typeof parent.args.dbexpire.statsevents == 'number') {
      expireServerStatsSeconds = parent.args.dbexpire.statsevents;
    }
  }
  ...
  if (typeof parent.args.dbrecordsencryptkey == 'string' && parent.args.dbrecordsencryptkey.length != 0) {
    ...
    obj.dbRecordsEncryptKey = obj.dbRecordsDecryptKey = parent.crypto
      .createHash('sha384')
      .update(parent.args.dbrecordsencryptkey)
      .digest('raw')
      .slice(0, 32);
  }
  ...
  if (typeof parent.args.dbrecordsdecryptkey == 'string' && parent.args.dbrecordsdecryptkey.length != 0) {
    // Hash the database password into a AES256 key and setup encryption and decryption.
    obj.dbRecordsDecryptKey = parent.crypto
      .createHash('sha384')
      .update(parent.args.dbrecordsdecryptkey)
      .digest('raw')
      .slice(0, 32);
  }
  ...
  if (parent.args.sqlite3) {
    ...
    if (typeof configParams == 'string') {
      databaseName = configParams;
    } else {
      databaseName = configParams.name ? configParams.name : 'meshcentral';
    }
    ...
    if (!['delete', 'truncate', 'persist', 'memory', 'wal'].includes(obj.sqliteConfig.journalMode)) {
      obj.sqliteConfig.journalMode = 'delete';
    }
    ...
    if (obj.sqliteConfig.journalMode == 'wal') {
      obj.sqliteConfig.maintenance += 'PRAGMA wal_checkpoint(PASSIVE);';
    }
    if (obj.sqliteConfig.autoVacuum == 'incremental') {
      obj.sqliteConfig.maintenance += 'PRAGMA incremental_vacuum(' + obj.sqliteConfig.incrementalVacuum + ');';
    }
    obj.sqliteConfig.maintenance += 'PRAGMA optimize;';

    parent.debug('db', 'SQlite config options: ' + JSON.stringify(obj.sqliteConfig, null, 4));
    if (obj.sqliteConfig.journalMode == 'memory') {
      console.log(
        '[WARNING] journal_mode=memory: this can lead to database corruption if there is a crash during a transaction. See https://www.sqlite.org/pragma.html#pragma_journal_mode'
      );
    }
    //.cached not usefull
    obj.file = new sqlite3.Database(
      path.join(parent.datapath, databaseName + '.sqlite'),
      ...
      function (err) {
        ...
        obj.file.exec(
          ...
          function (err) {
            if (err) {
              console.log('SQLite Error creating pluginpermissions table: ' + err);
            }
            //setupFunctions could be put in the sqliteSetupOptions, but left after it for clarity
            setupFunctions(func);
          }
    ...
    obj.file = new AceBase('meshcentral', {
      sponsor: typeof parent.args.acebase == 'object' && parent.args.acebase.sponsor,
      logLevel: 'error',
      storage: { path: parent.datapath }
    });
    ...
    if (typeof connectinArgs == 'string') {
      ...
      try {
        if (connectinArgs.ssl) {
          if (connectinArgs.ssl.dontcheckserveridentity == true) {
            connectionObject.ssl.checkServerIdentity = function (name, cert) {
              return undefined;
            };
          }
          if (connectinArgs.ssl.cacertpath) {
            connectionObject.ssl.ca = [require('fs').readFileSync(connectinArgs.ssl.cacertpath, 'utf8')];
          }
          if (connectinArgs.ssl.clientcertpath) {
            connectionObject.ssl.cert = [require('fs').readFileSync(connectinArgs.ssl.clientcertpath, 'utf8')];
          }
          if (connectinArgs.ssl.clientkeypath) {
            connectionObject.ssl.key = [require('fs').readFileSync(connectinArgs.ssl.clientkeypath, 'utf8')];
          }
        }
      } catch (ex) {
      ...
      tempDatastore
        ...
        .then(function (conn) {
          conn
            ...
            .catch(function (ex) {
              console.log('Auto-create database failed: ' + ex);
            });
        })
        .catch(function (ex) {
          console.log('Auto-create database failed: ' + ex);
    ...
    if (connectionArgs.createdatabase === false) {
      ...
      DatastoreTest.query(
        ...
        [databaseName],
        function (err, res) {
          // check database exists first before creating
          if (res.rowCount != 0) {
            // database exists now check tables exists
            DatastoreTest.end();
            Datastore.connect();
            Datastore.query('SELECT doc FROM main WHERE id = $1', ['DatabaseIdentifier'], function (err, res) {
              if (err == null) {
                ...
                //42P01 = undefined table, https://www.postgresql.org/docs/current/errcodes-appendix.html
                postgreSqlCreateTables(func);
              } else {
                console.log('Postgresql database exists, other error: ', err.message);
                process.exit(0);
              }
            });
          } else {
            // If not present, create the tables and indexes
      ...
      function (err, client) {
        ...
        db.admin().serverInfo(function (err, info) {
          if (
            err != null ||
            info == null ||
            info.versionArray == null ||
            Array.isArray(info.versionArray) == false ||
            info.versionArray.length < 2 ||
            typeof info.versionArray[0] != 'number' ||
            typeof info.versionArray[1] != 'number'
          ) {
            ...
            if (info.versionArray[0] < 3 || (info.versionArray[0] == 3 && info.versionArray[1] < 6)) {
              ...
              parent.addServerWarning(
                'Current version of MongoDB (' +
                  info.version +
                  ') is too old, please upgrade to MongoDB 3.6 or better.',
                true
              );
            }
          }
        ...
        obj.file.indexes(function (err, indexes) {
          ...
          if (
            indexCount != 5 ||
            indexesByName['TypeDomainMesh1'] == null ||
            indexesByName['Email1'] == null ||
            indexesByName['Mesh1'] == null ||
            indexesByName['AmtUuid1'] == null
          ) {
              ...
              [
                {
                  $match: {
                    $or: [
                      { 'fullDocument.type': { $in: ['node', 'mesh', 'user', 'ugrp'] } },
            ...
            obj.fileChangeStream.on('change', function (change) {
              ...
              if (change.operationType == 'update' || change.operationType == 'replace') {
                ...
                switch (change.fullDocument.type) {
                  case 'node': {
                    dbNodeChange(change, false);
                    break;
                  } // A node has changed
                  case 'mesh': {
                    dbMeshChange(change, false);
                    break;
                  } // A device group has changed
                  case 'user': {
                    dbUserChange(change, false);
                    break;
                  } // A user account has changed
                  case 'ugrp': {
                    dbUGrpChange(change, false);
                    break;
                  } // A user account has changed
                }
                ...
                switch (change.fullDocument.type) {
                  case 'node': {
                    dbNodeChange(change, true);
                    break;
                  } // A node has added
                  case 'mesh': {
                    dbMeshChange(change, true);
                    break;
                  } // A device group has created
                  case 'user': {
                    dbUserChange(change, true);
                    break;
                  } // A user account has created
                  case 'ugrp': {
                    dbUGrpChange(change, true);
                    break;
                  } // A user account has created
                }
                ...
                switch (splitId[0]) {
                  ...
                  case 'mesh': {
                    parent.DispatchEvent(['*', change.documentKey._id], obj, {
                      etype: 'mesh',
                      action: 'deletemesh',
                      meshid: change.documentKey._id,
                      domain: splitId[1]
                    });
                    break;
                  }
        ...
        obj.eventsfile.indexes(function (err, indexes) {
          ...
          if (
            indexCount != 5 ||
            indexesByName['UseridAction1'] == null ||
            indexesByName['DomainNodeTime1'] == null ||
            indexesByName['IdsAndTime1'] == null ||
            indexesByName['ExpireTime1'] == null
          ) {
            // Reset all indexes
        ...
        obj.powerfile.indexes(function (err, indexes) {
          ...
          if (indexCount != 3 || indexesByName['NodeIdAndTime1'] == null || indexesByName['ExpireTime1'] == null) {
            ...
            obj.powerfile.dropIndexes(function (err) {
              ...
              obj.powerfile.createIndex(
                { time: 1 },
                { expireAfterSeconds: expirePowerEventsSeconds, name: 'ExpireTime1' }
              );
            });
        ...
        obj.serverstatsfile.indexes(function (err, indexes) {
          ...
          if (indexCount != 3 || indexesByName['ExpireTime1'] == null) {
            ...
            obj.serverstatsfile.dropIndexes(function (err) {
              // Create all indexes
              obj.serverstatsfile.createIndex(
                { time: 1 },
                { expireAfterSeconds: expireServerStatsSeconds, name: 'ExpireTime1' }
              );
    ...
    if (parent.args.mongodbcol) {
      dbcollection = parent.args.mongodbcol;
    }
    ...
    } catch (ex) {} // This is the NeDB with Node 23 support.
    if (Datastore == null) {
      try {
        Datastore = require('@yetzt/nedb');
      } catch (ex) {} // This is the NeDB with fixed security dependencies.
      if (Datastore == null) {
        Datastore = require('nedb');
      } // So not to break any existing installations, if the old NeDB is present, use it.
    }
    ...
    obj.eventsfile = new Datastore({
      filename: parent.getConfigFilePath('meshcentral-events.db'),
      autoload: true,
    ...
    obj.powerfile = new Datastore({
      filename: parent.getConfigFilePath('meshcentral-power.db'),
      autoload: true,
      corruptAlertThreshold: 1
    });
    ...
    obj.serverstatsfile = new Datastore({
      filename: parent.getConfigFilePath('meshcentral-stats.db'),
      autoload: true,
      corruptAlertThreshold: 1
    });
  ...
  function sqliteSetOptions(func) {
    ...
    obj.file.get('PRAGMA auto_vacuum;', function (err, current) {
      let pragma =
        ...
        ';' +
        'PRAGMA synchronous=' +
        obj.sqliteConfig.synchronous +
        ';' +
        'PRAGMA journal_size_limit=' +
        obj.sqliteConfig.journalSize +
        ';' +
        'PRAGMA auto_vacuum=' +
        obj.sqliteConfig.autoVacuum +
        ';' +
        'PRAGMA incremental_vacuum=' +
        obj.sqliteConfig.incrementalVacuum +
        ';' +
      ...
      if (
        obj.sqliteConfig.startupVacuum ||
      ...
      obj.file.exec(pragma, function (err) {
        if (err) {
          parent.debug('db', 'Config pragma error: ' + err.message);
        }
        sqliteGetPragmas(
          [
            'journal_mode',
    ...
    pragmas.forEach(function (pragma) {
      obj.file.get('PRAGMA ' + pragma + ';', function (err, res) {
        if (pragma == 'auto_vacuum') {
          res[pragma] = SQLITE_AUTOVACUUM[res[pragma]];
        }
        if (pragma == 'synchronous') {
          res[pragma] = SQLITE_SYNCHRONOUS[res[pragma]];
        }
        if (func) {
          func(pragma, res[pragma]);
        }
      });
    });
  }
  ...
  function checkObjectNames(r, tag) {
    ...
    for (var i in r) {
      if (i.indexOf('.') >= 0) {
        throw 'BadDbName (' + tag + '): ' + JSON.stringify(r);
      }
  ...
  function sqlDbQuery(query, args, func, debug) {
    if (obj.databaseType == DB_SQLITE) {
      ...
      obj.file.all(query, args, function (err, docs) {
        if (err != null) {
          console.log(query, args, err, docs);
        }
        if (docs != null) {
          for (var i in docs) {
            if (typeof docs[i].doc == 'string') {
              try {
                docs[i] = JSON.parse(docs[i].doc);
              } catch (ex) {
          ...
          conn
            ...
            .then(function (rows) {
              ...
              for (var i in rows) {
                if (rows[i].doc) {
                  docs.push(
                    performTypedRecordDecrypt(typeof rows[i].doc == 'object' ? rows[i].doc : JSON.parse(rows[i].doc))
                  );
      ...
      Datastore.query(query, args, function (error, results) {
        if (error != null) {
          ...
          var docs = [];
          if (results.command == 'INSERT' && results.rows != null && results.rows.length == 1) {
            docs = results.rows[0];
  ...
  function sqlDbExec(query, args, func) {
    ...
    } else if (obj.databaseType == DB_MYSQL || obj.databaseType == DB_POSTGRESQL) {
      // MySQL or Postgres SQL
      Datastore.query(query, args, function (error, results, fields) {
        if (func)
          try {
            func(error, results ? results[0] : null);
          } catch (ex) {
            console.log(ex);
          }
      });
  ...
  function sqlDbBatchExec(queries, func) {
    if (obj.databaseType == DB_MARIADB) {
        ...
        .then(function (conn) {
          ...
          for (var i in queries) {
            if (typeof queries[i] == 'string') {
              ...
              Promises.push(conn.query(queries[i][0], queries[i][1]));
            }
          }
      ...
      Datastore.getConnection(function (err, connection) {
        ...
        for (var i in queries) {
          if (typeof queries[i] == 'string') {
            Promises.push(connection.promise().query(queries[i]));
          } else {
            Promises.push(connection.promise().query(queries[i][0], queries[i][1]));
          }
        }
        Promise.all(Promises)
          ...
          .catch(function (error, results, fields) {
            connection.release();
            if (func) {
              try {
                func(error);
      ...
      for (var i in queries) {
        if (typeof queries[i] == 'string') {
          Promises.push(Datastore.query(queries[i]));
        } else {
          Promises.push(Datastore.query(queries[i][0], queries[i][1]));
        }
      }
      Promise.all(Promises)
        .then(function (error, results, fields) {
  ...
  function setupFunctions(func) {
    if (obj.databaseType == DB_SQLITE) {
      ...
      obj.Set = function (value, func) {
        ...
        if (value.type == 'node' && value.intelamt != null && value.intelamt.uuid != null) {
          extraex = 'uuid/' + value.intelamt.uuid;
        }
        if (value._id == null) {
          value._id = require('crypto').randomBytes(16).toString('hex');
        }
        sqlDbQuery(
          'INSERT INTO main VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO UPDATE SET type = $2, domain = $3, extra = $4, extraex = $5, doc = $6;',
          [
            value._id,
            value.type ? value.type : null,
            value.domain != null ? value.domain : null,
            extra,
            extraex,
            JSON.stringify(performTypedRecordEncrypt(value))
          ],
          func
        );
      };
      ...
      obj.GetAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, skip, limit, func) {
        if (limit == 0) {
          limit = -1;
        } // In SQLite, no limit is -1
        if (id && id != '') {
          sqlDbQuery(
            'SELECT doc FROM main WHERE (id = $1) AND (type = $2) AND (domain = $3) AND (extra IN (' +
              dbMergeSqlArray(meshes) +
              ')) LIMIT $4 OFFSET $5',
            [id, type, domain, limit, skip],
            function (err, docs) {
              if (docs != null) {
                for (var i in docs) {
                  delete docs[i].type;
          ...
          if (extrasids == null) {
            sqlDbQuery(
              'SELECT doc FROM main WHERE (type = $1) AND (domain = $2) AND (extra IN (' +
                dbMergeSqlArray(meshes) +
                ')) LIMIT $3 OFFSET $4',
              [type, domain, limit, skip],
              function (err, docs) {
                if (docs != null) {
                  for (var i in docs) {
                    delete docs[i].type;
      ...
      obj.CountAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, func) {
        if (id && id != '') {
          sqlDbQuery(
            'SELECT COUNT(doc) FROM main WHERE (id = $1) AND (type = $2) AND (domain = $3) AND (extra IN (' +
              dbMergeSqlArray(meshes) +
              '))',
            [id, type, domain],
            function (err, docs) {
          ...
          if (extrasids == null) {
            sqlDbQuery(
              'SELECT COUNT(doc) FROM main WHERE (type = $1) AND (domain = $2) AND (extra IN (' +
                dbMergeSqlArray(meshes) +
                '))',
              [type, domain],
              function (err, docs) {
      ...
      obj.GetAllTypeNodeFiltered = function (nodes, domain, type, id, func) {
        if (id && id != '') {
          ...
          sqlDbQuery(
            'SELECT doc FROM main WHERE (type = $1) AND (domain = $2) AND (extra IN (' + dbMergeSqlArray(nodes) + '))',
            [type, domain],
            function (err, docs) {
              if (docs != null) {
                for (var i in docs) {
      ...
      obj.GetAllIdsOfType = function (ids, domain, type, func) {
        sqlDbQuery(
          'SELECT doc FROM main WHERE (id IN (' + dbMergeSqlArray(ids) + ')) AND domain = $1 AND type = $2',
          [domain, type],
          function (err, docs) {
            if (docs != null) {
              for (var i in docs) {
      ...
      obj.GetUserWithEmail = function (domain, email, func) {
        sqlDbQuery(
          'SELECT doc FROM main WHERE domain = $1 AND extra = $2',
          [domain, 'email/' + email],
          function (err, docs) {
            if (docs != null) {
              for (var i in docs) {
      ...
      obj.Remove = function (id, func) {
        sqlDbQuery('DELETE FROM main WHERE id = $1', [id], func);
      };
      obj.RemoveAll = function (func) {
        sqlDbQuery('DELETE FROM main', null, func);
      };
      obj.RemoveAllOfType = function (type, func) {
        sqlDbQuery('DELETE FROM main WHERE type = $1', [type], func);
      };
      obj.InsertMany = function (data, func) {
        var pendingOps = 0;
        for (var i in data) {
          pendingOps++;
          obj.SetRaw(data[i], function () {
            if (--pendingOps == 0) {
              func();
            }
          });
        }
      }; // Insert records directly, no link escaping
      obj.RemoveMeshDocuments = function (id, func) {
        sqlDbQuery('DELETE FROM main WHERE extra = $1', [id], function () {
          sqlDbQuery('DELETE FROM main WHERE id = $1', ['nt' + id], func);
        });
      };
      obj.MakeSiteAdmin = function (username, domain) {
        obj.Get('user/' + domain + '/' + username, function (err, docs) {
          if (err == null && docs.length == 1) {
            docs[0].siteadmin = 0xffffffff;
            obj.Set(docs[0]);
          }
        });
      };
      obj.DeleteDomain = function (domain, func) {
        sqlDbQuery('DELETE FROM main WHERE domain = $1', [domain], func);
      };
      ...
      obj.getLocalAmtNodes = function (func) {
        sqlDbQuery("SELECT doc FROM main WHERE (type = 'node') AND (extraex IS NULL)", null, function (err, docs) {
          ...
          if (err == null) {
            for (var i in docs) {
              if (docs[i].host != null && docs[i].intelamt != null) {
                r.push(docs[i]);
              }
            }
          }
      ...
      obj.getAmtUuidMeshNode = function (domainid, mtype, uuid, func) {
        sqlDbQuery(
          'SELECT doc FROM main WHERE domain = $1 AND extraex = $2',
          [domainid, 'uuid/' + uuid],
          function (err, docs) {
            if (docs != null) {
              for (var i in docs) {
      ...
      obj.isMaxType = function (max, type, domainid, func) {
        if (max == null) {
          ...
          sqlDbExec(
            'SELECT COUNT(id) FROM main WHERE domain = $1 AND type = $2',
            [domainid, type],
            function (err, response) {
              func(response['COUNT(id)'] == null || response['COUNT(id)'] > max, response['COUNT(id)']);
            }
          );
        }
      };
      ...
      obj.StoreEvent = function (event, func) {
        ...
        sqlDbQuery(
          'INSERT INTO events VALUES (NULL, $1, $2, $3, $4, $5, $6) RETURNING id',
          [
            event.time,
            typeof event.domain == 'string' ? event.domain : null,
            event.action,
            event.nodeid ? event.nodeid : null,
            event.userid ? event.userid : null,
          ...
          function (err, docs) {
            ...
            if (err == null && docs[0].id) {
              for (var i in event.ids) {
                if (event.ids[i] != '*') {
                  obj.pendingTransfer++;
                  sqlDbQuery('INSERT INTO eventids VALUES ($1, $2)', [docs[0].id, event.ids[i]], function () {
                    if (func) {
                      func();
                    }
                  });
      ...
      obj.GetEvents = function (ids, domain, filter, func) {
        ...
        if (ids.indexOf('*') >= 0) {
          ...
          query =
            query + 'JOIN eventids ON id = fkid WHERE (domain = $1 AND (target IN (' + dbMergeSqlArray(ids) + '))';
          if (filter != null) {
            query = query + ' AND action = $2';
            dataarray.push(filter);
          }
      ...
      obj.GetUserEvents = function (ids, domain, userid, filter, func) {
        ...
        if (ids.indexOf('*') >= 0) {
          ...
          query =
            query +
            'JOIN eventids ON id = fkid WHERE (domain = $1 AND userid = $2 AND (target IN (' +
            dbMergeSqlArray(ids) +
            '))';
          if (filter != null) {
            query = query + ' AND action = $3';
      ...
      obj.GetEventsTimeRange = function (ids, domain, msgids, start, end, func) {
        if (ids.indexOf('*') >= 0) {
          sqlDbQuery(
            'SELECT doc FROM events WHERE ((domain = $1) AND (time BETWEEN $2 AND $3)) ORDER BY time',
            [domain, start, end],
            func
          );
        } else {
          sqlDbQuery(
            'SELECT doc FROM events JOIN eventids ON id = fkid WHERE ((domain = $1) AND (target IN (' +
              dbMergeSqlArray(ids) +
              ')) AND (time BETWEEN $2 AND $3)) GROUP BY id ORDER BY time',
            [domain, start, end],
            func
          );
        }
      ...
      obj.GetNodeEventsSelfWithLimit = function (nodeid, domain, userid, limit, filter, func) {
        var query =
          'SELECT doc FROM events WHERE (nodeid = $1) AND (domain = $2) AND ((userid = $3) OR (userid IS NULL)) ';
      ...
      obj.RemoveAllEvents = function (domain) {
        sqlDbQuery('DELETE FROM events', null, function (err, docs) {});
      };
      obj.RemoveAllNodeEvents = function (domain, nodeid) {
        if (domain == null || nodeid == null) return;
        sqlDbQuery('DELETE FROM events WHERE domain = $1 AND nodeid = $2', [domain, nodeid], function (err, docs) {});
      };
      obj.RemoveAllUserEvents = function (domain, userid) {
        if (domain == null || userid == null) return;
        sqlDbQuery('DELETE FROM events WHERE domain = $1 AND userid = $2', [domain, userid], function (err, docs) {});
      };
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        sqlDbQuery(
          "SELECT COUNT(*) FROM events WHERE action = 'authfail' AND domain = $1 AND userid = $2 AND time > $3",
          [domainid, userid, lastlogin],
          function (err, response) {
            func(err == null ? response[0]['COUNT(*)'] : 0);
          }
        );
      };
      ...
      obj.getAllPower = function (func) {
        sqlDbQuery('SELECT doc FROM power', null, func);
      };
      obj.storePowerEvent = function (event, multiServer, func) {
        obj.dbCounters.powerSet++;
        if (multiServer != null) {
          event.server = multiServer.serverid;
        }
        sqlDbQuery(
          'INSERT INTO power VALUES (NULL, $1, $2, $3)',
          [event.time, event.nodeid ? event.nodeid : null, JSON.stringify(event)],
          func
        );
      };
      obj.getPowerTimeline = function (nodeid, func) {
        sqlDbQuery("SELECT doc FROM power WHERE ((nodeid = $1) OR (nodeid = '*')) ORDER BY time ASC", [nodeid], func);
      };
      obj.removeAllPowerEvents = function () {
        sqlDbQuery('DELETE FROM power', null, function (err, docs) {});
      };
      obj.removeAllPowerEventsForNode = function (nodeid) {
        if (nodeid == null) return;
        sqlDbQuery('DELETE FROM power WHERE nodeid = $1', [nodeid], function (err, docs) {});
      };

      // Database actions on the SMBIOS collection
      obj.GetAllSMBIOS = function (func) {
        sqlDbQuery('SELECT doc FROM smbios', null, func);
      };
      obj.SetSMBIOS = function (smbios, func) {
        var expire = new Date(smbios.time);
        expire.setMonth(expire.getMonth() + 6);
        sqlDbQuery(
          'INSERT INTO smbios VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET time = $2, expire = $3, doc = $4',
          [smbios._id, smbios.time, expire, JSON.stringify(smbios)],
          func
        );
      };
      obj.RemoveSMBIOS = function (id) {
        sqlDbQuery('DELETE FROM smbios WHERE id = $1', [id], function (err, docs) {});
      };
      obj.GetSMBIOS = function (id, func) {
        sqlDbQuery('SELECT doc FROM smbios WHERE id = $1', [id], func);
      };

      // Database actions on the Server Stats collection
      obj.SetServerStats = function (data, func) {
        sqlDbQuery(
          'INSERT INTO serverstats VALUES ($1, $2, $3) ON CONFLICT (time) DO UPDATE SET expire = $2, doc = $3',
          [data.time, data.expire, JSON.stringify(data)],
          func
        );
      };
      obj.GetServerStats = function (hours, func) {
        ...
        sqlDbQuery('SELECT doc FROM serverstats WHERE time > $1', [t], func);
      }; // TODO: Expire old entries

      // Read a configuration file from the database
      obj.getConfigFile = function (path, func) {
        obj.Get('cfile/' + path, func);
      };

      // Write a configuration file to the database
      obj.setConfigFile = function (path, data, func) {
        obj.Set({ _id: 'cfile/' + path, type: 'cfile', data: data.toString('base64') }, func);
      };

      // List all configuration files
      obj.listConfigFiles = function (func) {
        sqlDbQuery('SELECT doc FROM main WHERE type = "cfile" ORDER BY id', func);
      };

      // Get database information (TODO: Complete this)
      obj.getDbStats = function (func) {
        obj.stats = { c: 4 };
        sqlDbQuery('SELECT COUNT(*) FROM main', null, function (err, response) {
          obj.stats.meshcentral = err == null ? response[0]['COUNT(*)'] : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM serverstats', null, function (err, response) {
          obj.stats.serverstats = err == null ? response[0]['COUNT(*)'] : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM power', null, function (err, response) {
          obj.stats.power = err == null ? response[0]['COUNT(*)'] : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM smbios', null, function (err, response) {
          obj.stats.smbios = err == null ? response[0]['COUNT(*)'] : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
      ...
      // Plugin operations
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          sqlDbQuery('INSERT INTO plugin VALUES (NULL, $1)', [JSON.stringify(plugin)], func);
        }; // Add a plugin
        obj.getPlugins = function (func) {
          sqlDbQuery('SELECT JSON_INSERT(doc, "$._id", id) as doc FROM plugin', null, func);
        }; // Get all plugins
        obj.getPlugin = function (id, func) {
          sqlDbQuery('SELECT JSON_INSERT(doc, "$._id", id) as doc FROM plugin WHERE id = $1', [id], func);
        }; // Get plugin
        obj.deletePlugin = function (id, func) {
          sqlDbQuery('DELETE FROM plugin WHERE id = $1', [id], func);
        }; // Delete plugin
        obj.setPluginStatus = function (id, status, func) {
          sqlDbQuery('UPDATE plugin SET doc=JSON_SET(doc,"$.status",$1) WHERE id=$2', [status, id], func);
        };
        obj.updatePlugin = function (id, args, func) {
          delete args._id;
          sqlDbQuery('UPDATE plugin SET doc=json_patch(doc,$1) WHERE id=$2', [JSON.stringify(args), id], func);
        };
        obj.getPluginPermissions = function (pluginName, func) {
          sqlDbQuery(
            'SELECT doc FROM pluginpermissions WHERE id = $1',
            ['pluginpermission//' + pluginName],
            function (err, docs) {
              if (docs && docs.length > 0) {
                func(null, [docs[0].doc]);
              } else {
                func(null, []);
              }
            }
        ...
        obj.setPluginPermissions = function (pluginName, data, func) {
          delete data._id;
          sqlDbQuery(
            'INSERT INTO pluginpermissions VALUES ($1, $2) ON DUPLICATE KEY UPDATE doc = $2',
            ['pluginpermission//' + pluginName, JSON.stringify(data)],
            func
          );
        };
      }
      ...
      obj.Set = function (data, func) {
        ...
        obj.file
          .ref('meshcentral')
          .child(encodeURIComponent(xdata._id))
          .set(common.aceEscapeFieldNames(xdata))
          .then(function (ref) {
            if (func) {
              func();
            }
      ...
      obj.Get = function (id, func) {
        obj.file
          ...
          .get(function (snapshot) {
            if (snapshot.exists()) {
              func(null, performTypedRecordDecrypt([common.aceUnEscapeFieldNames(snapshot.val())]));
            } else {
              func(null, []);
            }
          });
      ...
      obj.GetAll = function (func) {
        obj.file.ref('meshcentral').get(function (snapshot) {
          ...
          const docs = Object.keys(val).map(function (key) {
            return val[key];
          });
      ...
      obj.GetHash = function (id, func) {
        obj.file
          .ref('meshcentral')
          .child(encodeURIComponent(id))
          .get({ include: ['hash'] }, function (snapshot) {
            if (snapshot.exists()) {
              func(null, snapshot.val());
            } else {
              func(null, null);
            }
          });
      ...
      obj.GetAllTypeNoTypeField = function (type, domain, func) {
        obj.file
          ...
          .get({ exclude: ['type'] }, function (snapshots) {
            const docs = [];
            for (var i in snapshots) {
              const x = snapshots[i].val();
              docs.push(x);
            }
            func(null, common.aceUnEscapeAllFieldNames(docs));
          });
      ...
      obj.GetAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, skip, limit, func) {
        ...
        if (extrasids == null) {
          ...
          query.get(function (snapshots) {
            ...
            for (var i in snapshots) {
              const x = snapshots[i].val();
              if (extrasids.indexOf(x._id) >= 0 || meshes.indexOf(x.meshid) >= 0) {
                docs.push(x);
              }
            }
            func(null, performTypedRecordDecrypt(docs));
      ...
      obj.InsertMany = function (data, func) {
        var r = {};
        for (var i in data) {
          const ref = obj.file.ref('meshcentral').child(encodeURIComponent(data[i]._id));
          r[ref.key] = common.aceEscapeFieldNames(data[i]);
        }
        obj.file
          .ref('meshcentral')
          .set(r)
      ...
      obj.RemoveMeshDocuments = function (id) {
        obj.file.query('meshcentral').filter('meshid', '==', id).remove();
        obj.file
          .ref('meshcentral')
          .child(encodeURIComponent('nt' + id))
          .remove();
      };
      obj.MakeSiteAdmin = function (username, domain) {
        obj.Get('user/' + domain + '/' + username, function (err, docs) {
      ...
      obj.isMaxType = function (max, type, domainid, func) {
        if (max == null) {
          ...
          obj.file
            ...
            .filter('type', '==', type)
            .filter('domain', '==', domainid)
            .get({ snapshots: false }, function (snapshots) {
              func(snapshots.length > max, snapshots.length);
            });
        }
      };
      ...
      obj.StoreEvent = function (event, func) {
        if (typeof event.account == 'object') {
          event = Object.assign({}, event);
          event.account = common.aceEscapeFieldNames(event.account);
        }
        obj.dbCounters.eventsSet++;
        obj.file
          .ref('events')
      ...
      obj.GetEvents = function (ids, domain, filter, func) {
        ...
        if (filter != null) {
          obj.file
            ...
            .sort('time', false)
            .get({ exclude: ['_id', 'domain', 'node', 'type'] }, function (snapshots) {
              const docs = [];
              for (var i in snapshots) {
                const doc = snapshots[i].val();
                if (doc.ids == null || !Array.isArray(doc.ids)) continue;
                var found = false;
                for (var j in doc.ids) {
                  if (ids.indexOf(doc.ids[j]) >= 0) {
                    found = true;
                  }
                } // Check if one of the items in both arrays matches
                if (found) {
                  delete doc.ids;
                  if (typeof doc.account == 'object') {
                    doc.account = common.aceUnEscapeFieldNames(doc.account);
                  }
                  docs.push(doc);
                }
              }
      ...
      obj.GetUserEvents = function (ids, domain, userid, filter, func) {
        if (filter != null) {
          obj.file
            ...
            .filter('action', '==', filter)
            .sort('time', false)
            .get({ exclude: ['_id', 'domain', 'node', 'type', 'ids'] }, function (snapshots) {
              const docs = [];
              for (var i in snapshots) {
                docs.push(snapshots[i].val());
              }
      ...
      obj.GetEventsTimeRange = function (ids, domain, msgids, start, end, func) {
        obj.file
          ...
          .filter('domain', '==', domain)
          .filter('ids', 'in', ids)
          .filter('msgid', 'in', msgids)
          .filter('time', 'between', [start, end])
          .sort('time', false)
          .get({ exclude: ['type', '_id', 'domain', 'node'] }, function (snapshots) {
            const docs = [];
            for (var i in snapshots) {
              docs.push(snapshots[i].val());
            }
      ...
      obj.GetUserLoginEvents = function (domain, userid, func) {
        obj.file
          .query('events')
          .filter('domain', '==', domain)
          .filter('action', 'in', ['authfail', 'login'])
          .filter('userid', '==', userid)
          .filter('msgArgs', 'exists')
          .sort('time', false)
          .get({ include: ['action', 'time', 'msgid', 'msgArgs', 'tokenName'] }, function (snapshots) {
            const docs = [];
            for (var i in snapshots) {
              docs.push(snapshots[i].val());
            }
      ...
      obj.GetNodeEventsWithLimit = function (nodeid, domain, limit, filter, func) {
        if (filter != null) {
          obj.file
            ...
            .filter('action', '==', filter)
            .sort('time', false)
            .get({ exclude: ['type', 'etype', '_id', 'domain', 'ids', 'node', 'nodeid'] }, function (snapshots) {
              const docs = [];
              for (var i in snapshots) {
                docs.push(snapshots[i].val());
              }
      ...
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        obj.file
          ...
          .filter('domain', '==', domainid)
          .filter('userid', '==', userid)
          .filter('time', '>', lastlogin)
          .sort('time', false)
          .get({ snapshots: false }, function (snapshots) {
            func(null, snapshots.length);
          });
      };
      ...
      obj.getPowerTimeline = function (nodeid, func) {
        obj.file
          .query('power')
          .filter('nodeid', 'in', ['*', nodeid])
          .sort('time')
          .get({ exclude: ['_id', 'nodeid', 's'] }, function (snapshots) {
            const docs = [];
            for (var i in snapshots) {
              docs.push(snapshots[i].val());
            }
      ...
      if (obj.smbiosfile != null) {
        ...
        obj.SetSMBIOS = function (smbios, func) {
          obj.file
            .ref('meshcentral/' + encodeURIComponent(smbios._id))
            .set(smbios)
            .then(function (ref) {
              if (func) {
                func();
      ...
      obj.GetServerStats = function (hours, func) {
        ...
        obj.file
          .query('stats')
          .filter('time', '>', t)
          .get({ exclude: ['_id', 'cpu'] }, function (snapshots) {
            const docs = [];
            for (var i in snapshots) {
              docs.push(snapshots[i].val());
            }
      ...
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          ...
          obj.file
            .ref('plugin')
            .child(encodeURIComponent(plugin._id))
            .set(plugin)
            .then(function (ref) {
              if (func) {
                func();
        ...
        obj.getPluginPermissions = function (pluginName, func) {
          obj.file
            .ref('pluginpermissions')
            .child('pluginpermission//' + pluginName)
            .get(function (snapshot) {
              if (snapshot.exists()) {
                func(null, [snapshot.val()]);
              } else {
      ...
      obj.Set = function (value, func) {
        ...
        sqlDbQuery(
          ...
          [
            ...
            extra,
            extraex,
            performTypedRecordEncrypt(value)
          ],
          func
        );
      };
      ...
      obj.Get = function (_id, func) {
        sqlDbQuery('SELECT doc FROM main WHERE id = $1', [_id], function (err, docs) {
          if (docs != null && docs.length > 0 && docs[0].links != null) {
            docs[0] = common.unEscapeLinksFieldName(docs[0]);
          }
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      ...
      obj.GetAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, skip, limit, func) {
        ...
        if (id && id != '') {
          sqlDbQuery(
            'SELECT doc FROM main WHERE (id = $1) AND (type = $2) AND (domain = $3) AND (extra = ANY ($4)) LIMIT $5 OFFSET $6',
            [id, type, domain, meshes, limit, skip],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
                  delete docs[i].type;
          ...
          if (extrasids == null) {
            sqlDbQuery(
              'SELECT doc FROM main WHERE (type = $1) AND (domain = $2) AND (extra = ANY ($3)) LIMIT $4 OFFSET $5',
              [type, domain, meshes, limit, skip],
              function (err, docs) {
                if (err == null) {
                  for (var i in docs) {
                    delete docs[i].type;
            ...
            sqlDbQuery(
              'SELECT doc FROM main WHERE (type = $1) AND (domain = $2) AND ((extra = ANY ($3)) OR (id = ANY ($4))) LIMIT $5 OFFSET $6',
              [type, domain, meshes, extrasids, limit, skip],
              function (err, docs) {
                if (err == null) {
                  for (var i in docs) {
                    delete docs[i].type;
      ...
      obj.CountAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, func) {
        if (id && id != '') {
          sqlDbQuery(
            'SELECT COUNT(doc) FROM main WHERE (id = $1) AND (type = $2) AND (domain = $3) AND (extra = ANY ($4))',
            [id, type, domain, meshes],
            function (err, docs) {
              func(err, docs);
            }
          );
          ...
          if (extrasids == null) {
            sqlDbQuery(
              'SELECT COUNT(doc) FROM main WHERE (type = $1) AND (domain = $2) AND (extra = ANY ($3))',
              [type, domain, meshes],
              function (err, docs) {
                func(err, docs);
              },
            ...
            sqlDbQuery(
              'SELECT COUNT(doc) FROM main WHERE (type = $1) AND (domain = $2) AND ((extra = ANY ($3)) OR (id = ANY ($4)))',
              [type, domain, meshes, extrasids],
              function (err, docs) {
                func(err, docs);
              }
            );
      ...
      obj.GetAllTypeNodeFiltered = function (nodes, domain, type, id, func) {
        if (id && id != '') {
          sqlDbQuery(
            'SELECT doc FROM main WHERE (id = $1) AND (type = $2) AND (domain = $3) AND (extra = ANY ($4))',
            [id, type, domain, nodes],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
                  delete docs[i].type;
          ...
          sqlDbQuery(
            'SELECT doc FROM main WHERE (type = $1) AND (domain = $2) AND (extra = ANY ($3))',
            [type, domain, nodes],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
      ...
      obj.GetAllIdsOfType = function (ids, domain, type, func) {
        sqlDbQuery(
          'SELECT doc FROM main WHERE (id = ANY ($1)) AND domain = $2 AND type = $3',
          [ids, domain, type],
          function (err, docs) {
            func(err, performTypedRecordDecrypt(docs));
          }
      ...
      obj.getAmtUuidMeshNode = function (domainid, mtype, uuid, func) {
        sqlDbQuery('SELECT doc FROM main WHERE domain = $1 AND extraex = $2', [domainid, 'uuid/' + uuid], func);
      };
      obj.isMaxType = function (max, type, domainid, func) {
        if (max == null) {
          func(false);
      ...
      obj.StoreEvent = function (event, func) {
        obj.dbCounters.eventsSet++;
        sqlDbQuery(
          'INSERT INTO events VALUES (DEFAULT, $1, $2, $3, $4, $5, $6) RETURNING id',
          [
            event.time,
            typeof event.domain == 'string' ? event.domain : null,
            event.action,
          ...
          function (err, docs) {
            ...
            if (docs.id) {
              for (var i in event.ids) {
                if (event.ids[i] != '*') {
                  obj.pendingTransfer++;
                  sqlDbQuery('INSERT INTO eventids VALUES ($1, $2)', [docs.id, event.ids[i]], function () {
                    if (func) {
                      func();
                    }
                  });
      ...
      obj.GetEventsWithLimit = function (ids, domain, limit, filter, func) {
        ...
        if (ids.indexOf('*') >= 0) {
          ...
          if (ids.length == 0) {
            ids = '';
          } // MySQL can't handle a query with IN() on an empty array, we have to use an empty string instead.
          query = query + 'JOIN eventids ON id = fkid WHERE (domain = $1 AND (target = ANY ($2))';
          dataarray.push(ids);
          if (filter != null) {
            query = query + ' AND action = $3) ORDER BY time DESC LIMIT $4';
      ...
      obj.GetEventsTimeRange = function (ids, domain, msgids, start, end, func) {
        if (ids.indexOf('*') >= 0) {
          ...
          sqlDbQuery(
            'SELECT doc FROM events JOIN eventids ON id = fkid WHERE ((domain = $1) AND (target = ANY ($2)) AND (time BETWEEN $3 AND $4)) GROUP BY id ORDER BY time',
            [domain, ids, start, end],
            func
          );
        }
      };
      ...
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        sqlDbQuery(
          ...
          function (err, response, raw) {
            func(err == null ? parseInt(raw.rows[0].count) : 0);
          }
        );
      };
      ...
      obj.storePowerEvent = function (event, multiServer, func) {
        ...
        sqlDbQuery(
          'INSERT INTO power VALUES (DEFAULT, $1, $2, $3)',
          [event.time, event.nodeid ? event.nodeid : null, event],
          func
        );
      };
      obj.getPowerTimeline = function (nodeid, func) {
      ...
      obj.SetSMBIOS = function (smbios, func) {
        ...
        sqlDbQuery(
          'INSERT INTO smbios VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET time = $2, expire = $3, doc = $4',
          [smbios._id, smbios.time, expire, smbios],
          func
        );
      };
      obj.RemoveSMBIOS = function (id) {
      ...
      obj.getDbStats = function (func) {
        obj.stats = { c: 4 };
        sqlDbQuery('SELECT COUNT(*) FROM main', null, function (err, response, raw) {
          obj.stats.meshcentral = err == null ? parseInt(raw.rows[0].count) : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM serverstats', null, function (err, response, raw) {
          obj.stats.serverstats = err == null ? parseInt(raw.rows[0].count) : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM power', null, function (err, response, raw) {
          obj.stats.power = err == null ? parseInt(raw.rows[0].count) : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbQuery('SELECT COUNT(*) FROM smbios', null, function (err, response, raw) {
          obj.stats.smbios = err == null ? parseInt(raw.rows[0].count) : 0;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
      ...
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          sqlDbQuery('INSERT INTO plugin VALUES (DEFAULT, $1)', [plugin], func);
        }; // Add a plugin
        obj.getPlugins = function (func) {
          sqlDbQuery("SELECT doc::jsonb || ('{\"_id\":' || plugin.id || '}')::jsonb as doc FROM plugin", null, func);
        }; // Get all plugins
        obj.getPlugin = function (id, func) {
          sqlDbQuery(
            "SELECT doc::jsonb || ('{\"_id\":' || plugin.id || '}')::jsonb as  doc FROM plugin WHERE id = $1",
            [id],
            func
          );
        }; // Get plugin
        ...
        obj.setPluginStatus = function (id, status, func) {
          sqlDbQuery("UPDATE plugin SET doc= jsonb_set(doc::jsonb,'{status}',$1) WHERE id=$2", [status, id], func);
        };
        obj.updatePlugin = function (id, args, func) {
          delete args._id;
          sqlDbQuery('UPDATE plugin SET doc= doc::jsonb || ($1) WHERE id=$2', [args, id], func);
        };
        obj.getPluginPermissions = function (pluginName, func) {
          sqlDbQuery(
            'SELECT doc FROM pluginpermissions WHERE id = $1',
            ['pluginpermission//' + pluginName],
            function (err, docs) {
              if (docs && docs.length > 0 && docs[0].doc) {
                func(null, [typeof docs[0].doc === 'string' ? JSON.parse(docs[0].doc) : docs[0].doc]);
              } else {
                func(null, []);
              }
            }
        ...
        obj.setPluginPermissions = function (pluginName, data, func) {
          delete data._id;
          sqlDbQuery(
            'INSERT INTO pluginpermissions VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET doc = $2',
            ['pluginpermission//' + pluginName, JSON.stringify(data)],
            func
          );
        };
      ...
      obj.Get = function (_id, func) {
        sqlDbQuery('SELECT doc FROM main WHERE id = ?', [_id], function (err, docs) {
          if (docs != null && docs.length > 0 && docs[0].links != null) {
            docs[0] = common.unEscapeLinksFieldName(docs[0]);
          }
          func(err, performTypedRecordDecrypt(docs));
      ...
      obj.GetHash = function (id, func) {
        sqlDbQuery('SELECT doc FROM main WHERE id = ?', [id], function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetAllTypeNoTypeField = function (type, domain, func) {
        sqlDbQuery('SELECT doc FROM main WHERE type = ? AND domain = ?', [type, domain], function (err, docs) {
          if (err == null) {
            for (var i in docs) {
              delete docs[i].type;
            }
      ...
      obj.GetAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, skip, limit, func) {
        ...
        if (id && id != '') {
          sqlDbQuery(
            'SELECT doc FROM main WHERE id = ? AND type = ? AND domain = ? AND extra IN (?) LIMIT ? OFFSET ?',
            [id, type, domain, meshes, limit, skip],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
          ...
          sqlDbQuery(
            'SELECT doc FROM main WHERE type = ? AND domain = ? AND (extra IN (?) OR id IN (?)) LIMIT ? OFFSET ?',
            [type, domain, meshes, extrasids, limit, skip],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
      ...
      obj.CountAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, func) {
        ...
        if (id && id != '') {
          sqlDbQuery(
            'SELECT COUNT(doc) FROM main WHERE id = ? AND type = ? AND domain = ? AND extra IN (?)',
            [id, type, domain, meshes],
            function (err, docs) {
              func(err, docs);
            }
          ...
          sqlDbQuery(
            'SELECT COUNT(doc) FROM main WHERE type = ? AND domain = ? AND (extra IN (?) OR id IN (?))',
            [type, domain, meshes, extrasids],
            function (err, docs) {
              func(err, docs);
            }
      ...
      obj.GetAllTypeNodeFiltered = function (nodes, domain, type, id, func) {
        ...
        if (id && id != '') {
          sqlDbQuery(
            'SELECT doc FROM main WHERE id = ? AND type = ? AND domain = ? AND extra IN (?)',
            [id, type, domain, nodes],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
          ...
          sqlDbQuery(
            'SELECT doc FROM main WHERE type = ? AND domain = ? AND extra IN (?)',
            [type, domain, nodes],
            function (err, docs) {
              if (err == null) {
                for (var i in docs) {
      ...
      obj.GetAllType = function (type, func) {
        sqlDbQuery('SELECT doc FROM main WHERE type = ?', [type], function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetAllIdsOfType = function (ids, domain, type, func) {
        ...
        sqlDbQuery(
          'SELECT doc FROM main WHERE id IN (?) AND domain = ? AND type = ?',
          [ids, domain, type],
          function (err, docs) {
            func(err, performTypedRecordDecrypt(docs));
          }
      ...
      obj.GetUserWithEmail = function (domain, email, func) {
        sqlDbQuery(
          'SELECT doc FROM main WHERE domain = ? AND extra = ?',
          [domain, 'email/' + email],
          function (err, docs) {
            func(err, performTypedRecordDecrypt(docs));
          }
      ...
      obj.Remove = function (id, func) {
        sqlDbQuery('DELETE FROM main WHERE id = ?', [id], func);
      };
      obj.RemoveAll = function (func) {
        sqlDbQuery('DELETE FROM main', null, func);
      };
      obj.RemoveAllOfType = function (type, func) {
        sqlDbQuery('DELETE FROM main WHERE type = ?', [type], func);
      };
      obj.InsertMany = function (data, func) {
        var pendingOps = 0;
        for (var i in data) {
      ...
      obj.RemoveMeshDocuments = function (id, func) {
        sqlDbQuery('DELETE FROM main WHERE extra = ?', [id], function () {
          sqlDbQuery('DELETE FROM main WHERE id = ?', ['nt' + id], func);
        });
      };
      obj.MakeSiteAdmin = function (username, domain) {
        obj.Get('user/' + domain + '/' + username, function (err, docs) {
      ...
      obj.DeleteDomain = function (domain, func) {
        sqlDbQuery('DELETE FROM main WHERE domain = ?', [domain], func);
      };
      obj.SetUser = function (user) {
        if (user == null) return;
        if (user.subscriptions != null) {
      ...
      obj.getAmtUuidMeshNode = function (domainid, mtype, uuid, func) {
        sqlDbQuery('SELECT doc FROM main WHERE domain = ? AND extraex = ?', [domainid, 'uuid/' + uuid], func);
      };
      obj.isMaxType = function (max, type, domainid, func) {
        if (max == null) {
          func(false);
          ...
          sqlDbExec(
            'SELECT COUNT(id) FROM main WHERE domain = ? AND type = ?',
            [domainid, type],
            function (err, response) {
              func(response['COUNT(id)'] == null || response['COUNT(id)'] > max, response['COUNT(id)']);
            }
      ...
      obj.StoreEvent = function (event, func) {
        ...
        for (var i in event.ids) {
          if (event.ids[i] != '*') {
            batchQuery.push(['INSERT INTO eventids VALUE (LAST_INSERT_ID(), ?)', [event.ids[i]]]);
          }
        }
      ...
      obj.GetEventsTimeRange = function (ids, domain, msgids, start, end, func) {
        if (ids.indexOf('*') >= 0) {
          sqlDbQuery(
            'SELECT doc FROM events WHERE ((domain = ?) AND (time BETWEEN ? AND ?)) ORDER BY time',
            [domain, start, end],
            func
          );
        } else {
          ...
          sqlDbQuery(
            'SELECT doc FROM events JOIN eventids ON id = fkid WHERE ((domain = ?) AND (target IN (?)) AND (time BETWEEN ? AND ?)) GROUP BY id ORDER BY time',
            [domain, ids, start, end],
            func
          );
        }
      ...
      obj.RemoveAllNodeEvents = function (domain, nodeid) {
        if (domain == null || nodeid == null) return;
        sqlDbQuery('DELETE FROM events WHERE domain = ? AND nodeid = ?', [domain, nodeid], function (err, docs) {});
      };
      obj.RemoveAllUserEvents = function (domain, userid) {
        if (domain == null || userid == null) return;
        sqlDbQuery('DELETE FROM events WHERE domain = ? AND userid = ?', [domain, userid], function (err, docs) {});
      };
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        sqlDbExec(
          'SELECT COUNT(id) FROM events WHERE action = "authfail" AND domain = ? AND userid = ? AND time > ?',
          [domainid, userid, lastlogin],
          function (err, response) {
            func(err == null ? response['COUNT(id)'] : 0);
          }
        );
      };
      ...
      obj.storePowerEvent = function (event, multiServer, func) {
        ...
        sqlDbQuery(
          ...
          [null, event.time, event.nodeid ? event.nodeid : null, JSON.stringify(event)],
          func
        );
      };
      obj.getPowerTimeline = function (nodeid, func) {
        sqlDbQuery('SELECT doc FROM power WHERE ((nodeid = ?) OR (nodeid = "*")) ORDER BY time ASC', [nodeid], func);
      };
      obj.removeAllPowerEvents = function () {
        sqlDbQuery('DELETE FROM power', null, function (err, docs) {});
      };
      obj.removeAllPowerEventsForNode = function (nodeid) {
        if (nodeid == null) return;
        sqlDbQuery('DELETE FROM power WHERE nodeid = ?', [nodeid], function (err, docs) {});
      };
      ...
      obj.SetSMBIOS = function (smbios, func) {
        ...
        expire.setMonth(expire.getMonth() + 6);
        sqlDbQuery(
          'REPLACE INTO smbios VALUE (?, ?, ?, ?)',
          [smbios._id, smbios.time, expire, JSON.stringify(smbios)],
          func
        );
      };
      obj.RemoveSMBIOS = function (id) {
        sqlDbQuery('DELETE FROM smbios WHERE id = ?', [id], function (err, docs) {});
      };
      obj.GetSMBIOS = function (id, func) {
        sqlDbQuery('SELECT doc FROM smbios WHERE id = ?', [id], func);
      };

      // Database actions on the Server Stats collection
      obj.SetServerStats = function (data, func) {
        sqlDbQuery('REPLACE INTO serverstats VALUE (?, ?, ?)', [data.time, data.expire, JSON.stringify(data)], func);
      };
      obj.GetServerStats = function (hours, func) {
        var t = new Date();
        t.setTime(t.getTime() - 60 * 60 * 1000 * hours);
        sqlDbQuery('SELECT doc FROM serverstats WHERE time > ?', [t], func);
      };
      ...
      obj.getDbStats = function (func) {
        obj.stats = { c: 4 };
        sqlDbExec('SELECT COUNT(id) FROM main', null, function (err, response) {
          obj.stats.meshcentral = Number(response['COUNT(id)']);
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbExec('SELECT COUNT(time) FROM serverstats', null, function (err, response) {
          obj.stats.serverstats = Number(response['COUNT(time)']);
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbExec('SELECT COUNT(id) FROM power', null, function (err, response) {
          obj.stats.power = Number(response['COUNT(id)']);
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
        });
        sqlDbExec('SELECT COUNT(id) FROM smbios', null, function (err, response) {
          obj.stats.smbios = Number(response['COUNT(id)']);
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(obj.stats);
          }
      ...
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          sqlDbQuery('INSERT INTO plugin VALUE (?, ?)', [null, JSON.stringify(plugin)], func);
        }; // Add a plugin
        obj.getPlugins = function (func) {
          sqlDbQuery('SELECT JSON_INSERT(doc, "$._id", id) as doc FROM plugin', null, func);
        }; // Get all plugins
        obj.getPlugin = function (id, func) {
          sqlDbQuery('SELECT JSON_INSERT(doc, "$._id", id) as doc FROM plugin WHERE id = ?', [id], func);
        }; // Get plugin
        obj.deletePlugin = function (id, func) {
          sqlDbQuery('DELETE FROM plugin WHERE id = ?', [id], func);
        }; // Delete plugin
        obj.setPluginStatus = function (id, status, func) {
          sqlDbQuery('UPDATE meshcentral.plugin SET doc=JSON_SET(doc,"$.status",?) WHERE id=?', [status, id], func);
        };
        obj.updatePlugin = function (id, args, func) {
          delete args._id;
          sqlDbQuery(
            'UPDATE meshcentral.plugin SET doc=JSON_MERGE_PATCH(doc,?) WHERE id=?',
      ...
      if (parent.config.settings.mongodbbulkoperations) {
        obj.Set = function (data, func) {
          // Fast Set operation using bulkWrite(), this is much faster then using replaceOne()
          if (obj.filePendingSet == false) {
            // Perform the operation now
            obj.dbCounters.fileSet++;
            obj.filePendingSet = true;
            obj.filePendingSets = null;
            if (func != null) {
              obj.filePendingCbs = [func];
            }
            obj.file.bulkWrite(
              [
                {
                  replaceOne: {
                    filter: { _id: data._id },
                    replacement: performTypedRecordEncrypt(common.escapeLinksFieldNameEx(data)),
                    upsert: true
                  }
                }
              ],
              fileBulkWriteCompleted
            );
          } else {
            ...
            if (func != null) {
              if (obj.filePendingCb == null) {
                obj.filePendingCb = [func];
              } else {
                obj.filePendingCb.push(func);
              }
            }
          }
        };
        ...
        obj.Get = function (id, func) {
          // Fast Get operation using a bulk find() to reduce round trips to the database.
          // Encode arguments into return function if any are present.
          var func2 = func;
          if (arguments.length > 2) {
            var parms = [func];
            for (var parmx = 2; parmx < arguments.length; ++parmx) {
              parms.push(arguments[parmx]);
            }
          ...
          if (obj.filePendingGets == null) {
            ...
            if (obj.filePendingGet == null) {
              obj.filePendingGet = {};
            }
            if (obj.filePendingGet[id] == null) {
              obj.filePendingGet[id] = [func2];
            } else {
              obj.filePendingGet[id].push(func2);
            }
          }
        };
      } else {
      ...
      obj.GetAll = function (func) {
        obj.file.find({}).toArray(function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetHash = function (id, func) {
      ...
      obj.CountAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, func) {
        if (extrasids == null) {
          ...
          var f = obj.file.find(x, { type: 0 });
          if (f.countDocuments) {
            f.countDocuments(function (err, count) {
              func(err, count);
            });
          } else {
            f.count(function (err, count) {
              func(err, count);
            });
          }
        } else {
      ...
      obj.GetAllTypeNodeFiltered = function (nodes, domain, type, id, func) {
        ...
        obj.file.find(x, { type: 0 }).toArray(function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetAllType = function (type, func) {
        obj.file.find({ type: type }).toArray(function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetAllIdsOfType = function (ids, domain, type, func) {
        obj.file.find({ type: type, domain: domain, _id: { $in: ids } }).toArray(function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetUserWithEmail = function (domain, email, func) {
        obj.file.find({ type: 'user', domain: domain, email: email }).toArray(function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetUserWithVerifiedEmail = function (domain, email, func) {
        obj.file
          .find({ type: 'user', domain: domain, email: email, emailVerified: true })
          .toArray(function (err, docs) {
            func(err, performTypedRecordDecrypt(docs));
          });
      };
      ...
      if (parent.config.settings.mongodbbulkoperations) {
        obj.Remove = function (id, func) {
          // Fast remove operation using a bulk find() to reduce round trips to the database.
          if (obj.filePendingRemoves == null) {
            ...
            if (obj.filePendingRemove == null) {
              obj.filePendingRemove = {};
            }
            if (obj.filePendingRemove[id] == null) {
              obj.filePendingRemove[id] = [func];
            } else {
              obj.filePendingRemove[id].push(func);
            }
          }
        };
      } else {
        obj.Remove = function (id, func) {
          obj.dbCounters.fileRemove++;
          obj.file.deleteOne({ _id: id }, func);
        };
      }

      obj.RemoveAll = function (func) {
        obj.file.deleteMany({}, { multi: true }, func);
      };
      obj.RemoveAllOfType = function (type, func) {
        obj.file.deleteMany({ type: type }, { multi: true }, func);
      };
      obj.InsertMany = function (data, func) {
        obj.file.insertMany(data, func);
      }; // Insert records directly, no link escaping
      obj.RemoveMeshDocuments = function (id) {
        obj.file.deleteMany({ meshid: id }, { multi: true });
        obj.file.deleteOne({ _id: 'nt' + id });
      };
      obj.MakeSiteAdmin = function (username, domain) {
        obj.Get('user/' + domain + '/' + username, function (err, docs) {
          if (err == null && docs.length == 1) {
      ...
      obj.DeleteDomain = function (domain, func) {
        obj.file.deleteMany({ domain: domain }, { multi: true }, func);
      };
      obj.SetUser = function (user) {
        if (user == null) return;
        if (user.subscriptions != null) {
      ...
      obj.getLocalAmtNodes = function (func) {
        obj.file.find({ type: 'node', host: { $exists: true, $ne: null }, intelamt: { $exists: true } }).toArray(func);
      };
      obj.getAmtUuidMeshNode = function (domainid, mtype, uuid, func) {
        obj.file.find({ type: 'node', domain: domainid, mtype: mtype, 'intelamt.uuid': uuid }).toArray(func);
      };
      ...
      obj.isMaxType = function (max, type, domainid, func) {
        if (obj.file.countDocuments) {
          if (max == null) {
            ...
            obj.file.countDocuments({ type: type, domain: domainid }, function (err, count) {
              func(err != null || count > max, count);
            });
          }
        } else {
          if (max == null) {
            ...
            obj.file.count({ type: type, domain: domainid }, function (err, count) {
              func(err != null || count > max, count);
            });
          }
        }
      ...
      // Database actions on the events collection
      obj.GetAllEvents = function (func) {
        obj.eventsfile.find({}).toArray(func);
      };

      // Bulk operations
      if (parent.config.settings.mongodbbulkoperations) {
        obj.StoreEvent = function (event, func) {
          // Fast MongoDB event store using bulkWrite()
          if (obj.eventsFilePendingSet == false) {
            // Perform the operation now
            obj.dbCounters.eventsSet++;
            obj.eventsFilePendingSet = true;
            obj.eventsFilePendingSets = null;
            if (func != null) {
              obj.eventsFilePendingCbs = [func];
            }
            ...
            if (obj.eventsFilePendingSets == null) {
              obj.eventsFilePendingSets = [];
            }
            obj.eventsFilePendingSets.push(event);
            if (func != null) {
              if (obj.eventsFilePendingCb == null) {
                obj.eventsFilePendingCb = [func];
              } else {
                obj.eventsFilePendingCb.push(func);
              }
            }
          }
        };
        ...
        obj.StoreEvent = function (event, func) {
          obj.dbCounters.eventsSet++;
          obj.eventsfile.insertOne(event, func);
        };
      }
      ...
      obj.GetEvents = function (ids, domain, filter, func) {
        ...
        obj.eventsfile
          .find(finddata)
          .project({ type: 0, _id: 0, domain: 0, ids: 0, node: 0 })
          .sort({ time: -1 })
          .toArray(func);
      };
      obj.GetEventsWithLimit = function (ids, domain, limit, filter, func) {
      ...
      obj.GetEventsTimeRange = function (ids, domain, msgids, start, end, func) {
        obj.eventsfile
          ...
          .project({ type: 0, _id: 0, domain: 0, node: 0 })
          .sort({ time: 1 })
          .toArray(func);
      };
      obj.GetUserLoginEvents = function (domain, userid, func) {
        obj.eventsfile
          .find({ domain: domain, action: { $in: ['authfail', 'login'] }, userid: userid, msgArgs: { $exists: true } })
          .project({ action: 1, time: 1, msgid: 1, msgArgs: 1, tokenName: 1 })
          .sort({ time: -1 })
          .toArray(func);
      };
      obj.GetNodeEventsWithLimit = function (nodeid, domain, limit, filter, func) {
        ...
        obj.eventsfile
          .find(finddata)
          .project({ type: 0, etype: 0, _id: 0, domain: 0, ids: 0, node: 0, nodeid: 0 })
          .sort({ time: -1 })
          .limit(limit)
          .toArray(func);
      };
      ...
      obj.RemoveAllEvents = function (domain) {
        obj.eventsfile.deleteMany({ domain: domain }, { multi: true });
      };
      obj.RemoveAllNodeEvents = function (domain, nodeid) {
        if (domain == null || nodeid == null) return;
        obj.eventsfile.deleteMany({ domain: domain, nodeid: nodeid }, { multi: true });
      };
      obj.RemoveAllUserEvents = function (domain, userid) {
        if (domain == null || userid == null) return;
        obj.eventsfile.deleteMany({ domain: domain, userid: userid }, { multi: true });
      };
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        if (obj.eventsfile.countDocuments) {
          obj.eventsfile.countDocuments(
            { action: 'authfail', userid: userid, domain: domainid, time: { $gte: lastlogin } },
            function (err, count) {
              func(err == null ? count : 0);
            }
          );
        } else {
          obj.eventsfile.count(
            { action: 'authfail', userid: userid, domain: domainid, time: { $gte: lastlogin } },
            function (err, count) {
              func(err == null ? count : 0);
            }
      ...
      // Database actions on the power collection
      obj.getAllPower = function (func) {
        obj.powerfile.find({}).toArray(func);
      };
      ...
      if (parent.config.settings.mongodbbulkoperations) {
        obj.storePowerEvent = function (event, multiServer, func) {
          ...
          if (obj.powerFilePendingSet == false) {
            // Perform the operation now
            obj.dbCounters.powerSet++;
            obj.powerFilePendingSet = true;
            obj.powerFilePendingSets = null;
            if (func != null) {
              obj.powerFilePendingCbs = [func];
            }
            ...
            if (obj.powerFilePendingSets == null) {
              obj.powerFilePendingSets = [];
            }
            obj.powerFilePendingSets.push(event);
            if (func != null) {
              if (obj.powerFilePendingCb == null) {
                obj.powerFilePendingCb = [func];
              } else {
                obj.powerFilePendingCb.push(func);
              }
            }
          }
        };
        ...
        obj.storePowerEvent = function (event, multiServer, func) {
          ...
          obj.powerfile.insertOne(event, func);
        };
      }

      obj.getPowerTimeline = function (nodeid, func) {
        obj.powerfile
          .find({ nodeid: { $in: ['*', nodeid] } })
          .project({ _id: 0, nodeid: 0, s: 0 })
          .sort({ time: 1 })
          .toArray(func);
      };
      obj.removeAllPowerEvents = function () {
        obj.powerfile.deleteMany({}, { multi: true });
      };
      obj.removeAllPowerEventsForNode = function (nodeid) {
        if (nodeid == null) return;
        obj.powerfile.deleteMany({ nodeid: nodeid }, { multi: true });
      };

      // Database actions on the SMBIOS collection
      obj.GetAllSMBIOS = function (func) {
        obj.smbiosfile.find({}).toArray(func);
      };
      obj.SetSMBIOS = function (smbios, func) {
        obj.smbiosfile.updateOne({ _id: smbios._id }, { $set: smbios }, { upsert: true }, func);
      };
      obj.RemoveSMBIOS = function (id) {
        obj.smbiosfile.deleteOne({ _id: id });
      };
      obj.GetSMBIOS = function (id, func) {
        obj.smbiosfile.find({ _id: id }).toArray(func);
      };
      ...
      obj.GetServerStats = function (hours, func) {
        var t = new Date();
        t.setTime(t.getTime() - 60 * 60 * 1000 * hours);
        obj.serverstatsfile.find({ time: { $gt: t } }, { _id: 0, cpu: 0 }).toArray(func);
      };
      ...
      // List all configuration files
      obj.listConfigFiles = function (func) {
        obj.file.find({ type: 'cfile' }).sort({ _id: 1 }).toArray(func);
      };

      // Get database information
      obj.getDbStats = function (func) {
        obj.stats = { c: 6 };
        obj.getStats(function (r) {
          obj.stats.recordTypes = r;
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(getDbStatsEx(obj.stats));
          }
        });
        obj.file.stats().then(
          function (stats) {
        ...
        obj.serverstatsfile.stats().then(
          function (stats) {
            obj.stats[stats.ns] = {
              size: stats.size,
              count: stats.count,
      ...
      function getDbStatsEx(data) {
        ...
        try {
          r.smbios = data['meshcentral.smbios'].count;
        } catch (ex) {}
        try {
          r.power = data['meshcentral.power'].count;
        } catch (ex) {}
        try {
          r.events = data['meshcentral.events'].count;
        } catch (ex) {}
        try {
          r.serverstats = data['meshcentral.serverstats'].count;
        } catch (ex) {}
        return r;
      }
      ...
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          plugin.type = 'plugin';
          obj.pluginsfile.insertOne(plugin, func);
        }; // Add a plugin
        obj.getPlugins = function (func) {
          obj.pluginsfile.find({ type: 'plugin' }).project({ type: 0 }).sort({ name: 1 }).toArray(func);
        }; // Get all plugins
        obj.getPlugin = function (id, func) {
          id = require('mongodb').ObjectId(id);
          obj.pluginsfile.find({ _id: id }).sort({ name: 1 }).toArray(func);
        }; // Get plugin
        obj.deletePlugin = function (id, func) {
          id = require('mongodb').ObjectId(id);
          obj.pluginsfile.deleteOne({ _id: id }, func);
        }; // Delete plugin
        obj.setPluginStatus = function (id, status, func) {
          id = require('mongodb').ObjectId(id);
          obj.pluginsfile.updateOne({ _id: id }, { $set: { status: status } }, func);
        };
        obj.updatePlugin = function (id, args, func) {
          delete args._id;
          id = require('mongodb').ObjectId(id);
          obj.pluginsfile.updateOne({ _id: id }, { $set: args }, func);
        };
        obj.getPluginPermissions = function (pluginName, func) {
          obj.pluginpermissionsfile.findOne({ _id: 'pluginpermission//' + pluginName }, function (err, doc) {
            func(err, doc ? [doc] : []);
          });
        };
        obj.setPluginPermissions = function (pluginName, data, func) {
          delete data._id;
          obj.pluginpermissionsfile.updateOne(
      ...
      obj.GetAll = function (func) {
        obj.file.find({}, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetHash = function (id, func) {
        obj.file.find({ _id: id }, { _id: 0, hash: 1 }, func);
      };
      obj.GetAllTypeNoTypeField = function (type, domain, func) {
        obj.file.find({ type: type, domain: domain }, { type: 0 }, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      ...
      obj.CountAllTypeNoTypeFieldMeshFiltered = function (meshes, extrasids, domain, type, id, func) {
        if (extrasids == null) {
          ...
          obj.file.count(x, function (err, count) {
            func(err, count);
          });
        } else {
          const x = { type: type, domain: domain, $or: [{ meshid: { $in: meshes } }, { _id: { $in: extrasids } }] };
      ...
      obj.GetAllTypeNodeFiltered = function (nodes, domain, type, id, func) {
        ...
        obj.file.find(x, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetAllType = function (type, func) {
        obj.file.find({ type: type }, function (err, docs) {
          func(err, common.unEscapeAllLinksFieldName(performTypedRecordDecrypt(docs)));
        });
      };
      obj.GetAllIdsOfType = function (ids, domain, type, func) {
        obj.file.find({ type: type, domain: domain, _id: { $in: ids } }, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetUserWithEmail = function (domain, email, func) {
        obj.file.find({ type: 'user', domain: domain, email: email }, { type: 0 }, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.GetUserWithVerifiedEmail = function (domain, email, func) {
        obj.file.find({ type: 'user', domain: domain, email: email, emailVerified: true }, function (err, docs) {
          func(err, performTypedRecordDecrypt(docs));
        });
      };
      obj.Remove = function (id, func) {
        obj.file.remove({ _id: id }, func);
      };
      obj.RemoveAll = function (func) {
        obj.file.remove({}, { multi: true }, func);
      };
      obj.RemoveAllOfType = function (type, func) {
        obj.file.remove({ type: type }, { multi: true }, func);
      };
      obj.InsertMany = function (data, func) {
        obj.file.insert(data, func);
      }; // Insert records directly, no link escaping
      obj.RemoveMeshDocuments = function (id) {
        obj.file.remove({ meshid: id }, { multi: true });
        obj.file.remove({ _id: 'nt' + id });
      };
      obj.MakeSiteAdmin = function (username, domain) {
        obj.Get('user/' + domain + '/' + username, function (err, docs) {
          if (err == null && docs.length == 1) {
      ...
      obj.DeleteDomain = function (domain, func) {
        obj.file.remove({ domain: domain }, { multi: true }, func);
      };
      obj.SetUser = function (user) {
        if (user == null) return;
        if (user.subscriptions != null) {
      ...
      obj.getLocalAmtNodes = function (func) {
        obj.file.find({ type: 'node', host: { $exists: true, $ne: null }, intelamt: { $exists: true } }, func);
      };
      obj.getAmtUuidMeshNode = function (domainid, mtype, uuid, func) {
        obj.file.find({ type: 'node', domain: domainid, mtype: mtype, 'intelamt.uuid': uuid }, func);
      };
      obj.isMaxType = function (max, type, domainid, func) {
        if (max == null) {
          func(false);
      ...
      // Database actions on the events collection
      obj.GetAllEvents = function (func) {
        obj.eventsfile.find({}, func);
      };
      obj.StoreEvent = function (event, func) {
        obj.eventsfile.insert(event, func);
      };
      obj.GetEvents = function (ids, domain, filter, func) {
        var finddata = { domain: domain, ids: { $in: ids } };
        if (filter != null) finddata.action = filter;
      ...
      obj.GetEventsWithLimit = function (ids, domain, limit, filter, func) {
        ...
        if (obj.databaseType == DB_NEDB) {
          obj.eventsfile
            .find(finddata, { _id: 0, domain: 0, ids: 0, node: 0 })
            .sort({ time: -1 })
            .limit(limit)
            .exec(func);
        } else {
          obj.eventsfile
            .find(finddata, { type: 0, _id: 0, domain: 0, ids: 0, node: 0 })
            .sort({ time: -1 })
            .limit(limit, func);
        }
      };
      ...
      obj.GetUserLoginEvents = function (domain, userid, func) {
        if (obj.databaseType == DB_NEDB) {
          obj.eventsfile
            .find(
              { domain: domain, action: { $in: ['authfail', 'login'] }, userid: userid, msgArgs: { $exists: true } },
              { action: 1, time: 1, msgid: 1, msgArgs: 1, tokenName: 1 }
            )
            .sort({ time: -1 })
            .exec(func);
        } else {
      ...
      obj.GetNodeEventsWithLimit = function (nodeid, domain, limit, filter, func) {
        ...
        if (obj.databaseType == DB_NEDB) {
          obj.eventsfile
            .find(finddata, { type: 0, etype: 0, _id: 0, domain: 0, ids: 0, node: 0, nodeid: 0 })
            .sort({ time: -1 })
            .limit(limit)
            .exec(func);
        } else {
      ...
      obj.RemoveAllEvents = function (domain) {
        obj.eventsfile.remove({ domain: domain }, { multi: true });
      };
      obj.RemoveAllNodeEvents = function (domain, nodeid) {
        if (domain == null || nodeid == null) return;
        obj.eventsfile.remove({ domain: domain, nodeid: nodeid }, { multi: true });
      };
      obj.RemoveAllUserEvents = function (domain, userid) {
        if (domain == null || userid == null) return;
        obj.eventsfile.remove({ domain: domain, userid: userid }, { multi: true });
      };
      obj.GetFailedLoginCount = function (userid, domainid, lastlogin, func) {
        obj.eventsfile.count(
          { action: 'authfail', userid: userid, domain: domainid, time: { $gte: lastlogin } },
      ...
      // Database actions on the power collection
      obj.getAllPower = function (func) {
        obj.powerfile.find({}, func);
      };
      obj.storePowerEvent = function (event, multiServer, func) {
        if (multiServer != null) {
          event.server = multiServer.serverid;
        }
        obj.powerfile.insert(event, func);
      };
      obj.getPowerTimeline = function (nodeid, func) {
        if (obj.databaseType == DB_NEDB) {
          obj.powerfile
            .find({ nodeid: { $in: ['*', nodeid] } }, { _id: 0, nodeid: 0, s: 0 })
            .sort({ time: 1 })
            .exec(func);
        } else {
          obj.powerfile.find({ nodeid: { $in: ['*', nodeid] } }, { _id: 0, nodeid: 0, s: 0 }).sort({ time: 1 }, func);
        }
      };
      obj.removeAllPowerEvents = function () {
        obj.powerfile.remove({}, { multi: true });
      };
      obj.removeAllPowerEventsForNode = function (nodeid) {
        if (nodeid == null) return;
        obj.powerfile.remove({ nodeid: nodeid }, { multi: true });
      };

      // Database actions on the SMBIOS collection
      if (obj.smbiosfile != null) {
        obj.GetAllSMBIOS = function (func) {
          obj.smbiosfile.find({}, func);
        };
        obj.SetSMBIOS = function (smbios, func) {
          obj.smbiosfile.update({ _id: smbios._id }, smbios, { upsert: true }, func);
        };
        obj.RemoveSMBIOS = function (id) {
          obj.smbiosfile.remove({ _id: id });
        };
        obj.GetSMBIOS = function (id, func) {
          obj.smbiosfile.find({ _id: id }, func);
        };
      }
      ...
      obj.GetServerStats = function (hours, func) {
        var t = new Date();
        t.setTime(t.getTime() - 60 * 60 * 1000 * hours);
        obj.serverstatsfile.find({ time: { $gt: t } }, { _id: 0, cpu: 0 }, func);
      };
      ...
      // List all configuration files
      obj.listConfigFiles = function (func) {
        obj.file.find({ type: 'cfile' }).sort({ _id: 1 }).exec(func);
      };
      ...
      obj.getDbStats = function (func) {
        ...
        obj.file.count({}, function (err, count) {
          obj.stats.meshcentral = { count: count };
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(getDbStatsEx(obj.stats));
          }
        });
        obj.eventsfile.count({}, function (err, count) {
          obj.stats.events = { count: count };
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(getDbStatsEx(obj.stats));
          }
        });
        obj.powerfile.count({}, function (err, count) {
          obj.stats.power = { count: count };
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(getDbStatsEx(obj.stats));
          }
        });
        obj.serverstatsfile.count({}, function (err, count) {
          obj.stats.serverstats = { count: count };
          if (--obj.stats.c == 0) {
            delete obj.stats.c;
            func(getDbStatsEx(obj.stats));
          }
      ...
      function getDbStatsEx(data) {
        ...
        try {
          r.smbios = data['smbios'].count;
        } catch (ex) {}
        try {
          r.power = data['power'].count;
        } catch (ex) {}
        try {
          r.events = data['events'].count;
        } catch (ex) {}
        try {
          r.serverstats = data['serverstats'].count;
        } catch (ex) {}
        return r;
      }
      ...
      if (obj.pluginsActive) {
        obj.addPlugin = function (plugin, func) {
          plugin.type = 'plugin';
          obj.pluginsfile.insert(plugin, func);
        }; // Add a plugin
        obj.getPlugins = function (func) {
          obj.pluginsfile.find({ type: 'plugin' }, { type: 0 }).sort({ name: 1 }).exec(func);
        }; // Get all plugins
        obj.getPlugin = function (id, func) {
          obj.pluginsfile.find({ _id: id }).sort({ name: 1 }).exec(func);
        }; // Get plugin
        obj.deletePlugin = function (id, func) {
          obj.pluginsfile.remove({ _id: id }, func);
        }; // Delete plugin
        obj.setPluginStatus = function (id, status, func) {
          obj.pluginsfile.update({ _id: id }, { $set: { status: status } }, func);
        };
        obj.updatePlugin = function (id, args, func) {
          delete args._id;
          obj.pluginsfile.update({ _id: id }, { $set: args }, func);
        };
        obj.getPluginPermissions = function (pluginName, func) {
          obj.pluginsfile.findOne({ _id: 'pluginpermission//' + pluginName }, function (err, doc) {
            func(err, doc ? [doc] : []);
          });
        };
        obj.setPluginPermissions = function (pluginName, data, func) {
          delete data._id;
          obj.pluginsfile.update({ _id: 'pluginpermission//' + pluginName }, { $set: data }, { upsert: true }, func);
        };
      }
    }
  ...
  obj.getBackupConfig = function () {
    var r = '',
      backupPath = parent.backuppath;

    let dbname = 'meshcentral';
    if (parent.args.mongodbname) {
      dbname = parent.args.mongodbname;
    } else if (typeof parent.args.mariadb == 'object' && typeof parent.args.mariadb.database == 'string') {
      dbname = parent.args.mariadb.database;
    } else if (typeof parent.args.mysql == 'object' && typeof parent.args.mysql.database == 'string') {
      dbname = parent.args.mysql.database;
    } else if (typeof parent.args.postgres == 'object' && typeof parent.args.postgres.database == 'string') {
      dbname = parent.args.postgres.database;
    } else if (typeof parent.config.settings.sqlite3 == 'string') {
      dbname = parent.config.settings.sqlite3 + '.sqlite';
    }
    ...
    const fileSuffix =
      ...
      '-' +
      padNumber(currentDate.getMonth() + 1, 2) +
      '-' +
      padNumber(currentDate.getDate(), 2) +
      '-' +
      padNumber(currentDate.getHours(), 2) +
      '-' +
    ...
    if (parent.config.settings.autobackup.backupintervalhours == -1) {
      ...
      if (parent.config.settings.autobackup.backuphour != null && parent.config.settings.autobackup.backuphour != -1) {
        r +=
          'Backup between: ' +
          parent.config.settings.autobackup.backuphour +
          'H-' +
          (parent.config.settings.autobackup.backuphour + 1) +
          'H\r\n';
      }
      ...
      if (parent.config.settings.autobackup.zippassword != null) {
        r += 'ZIP Password: ';
        if (typeof parent.config.settings.autobackup.zippassword != 'string') {
          r += 'Bad zippassword type, Backups will not be encrypted\r\n';
        } else if (parent.config.settings.autobackup.zippassword == '') {
          r += 'Blank zippassword, Backups will fail\r\n';
        } else {
          r += 'Set\r\n';
        }
      }
      if (parent.config.settings.autobackup.mongodumppath != null) {
        r += 'MongoDump Path: ';
        if (typeof parent.config.settings.autobackup.mongodumppath != 'string') {
          r += 'Bad mongodumppath type\r\n';
        } else {
          r += parent.config.settings.autobackup.mongodumppath + '\r\n';
        }
      }
      if (parent.config.settings.autobackup.mysqldumppath != null) {
        r += 'MySqlDump Path: ';
        if (typeof parent.config.settings.autobackup.mysqldumppath != 'string') {
          r += 'Bad mysqldump type\r\n';
        } else {
          r += parent.config.settings.autobackup.mysqldumppath + '\r\n';
        }
      }
      if (parent.config.settings.autobackup.pgdumppath != null) {
        r += 'pgDump Path: ';
        if (typeof parent.config.settings.autobackup.pgdumppath != 'string') {
          r += 'Bad pgdump type\r\n';
        } else {
          r += parent.config.settings.autobackup.pgdumppath + '\r\n';
        }
      }
      ...
      if (parent.config.settings.autobackup.backupwebfolders) {
        ...
        if (parent.webViewsOverridePath) {
          r += parent.webViewsOverridePath;
        }
        if (parent.webPublicOverridePath) {
          r += ', ' + parent.webPublicOverridePath;
        }
        if (parent.webEmailsOverridePath) {
          r += ',' + parent.webEmailsOverridePath;
        }
        r += '\r\n';
      }
      ...
      if (typeof parent.config.settings.autobackup.webdav == 'object') {
        ...
        r +=
          'WebDAV backup path: ' +
          (typeof parent.config.settings.autobackup.webdav.foldername == 'string'
            ? parent.config.settings.autobackup.webdav.foldername
            : 'MeshCentral-Backups') +
          '\r\n';
        r +=
          'WebDAV maximum files: ' +
          (typeof parent.config.settings.autobackup.webdav.maxfiles == 'number'
            ? parent.config.settings.autobackup.webdav.maxfiles
            : 'no limit') +
          '\r\n';
      }
  ...
  function buildSqlDumpCommand() {
    ...
    cmd +=
      parent.platform == 'win32' ? ' --password=\"' + props.password + '\"' : " --password='" + props.password + "'";
    if (props.host) {
      cmd += ' -h ' + props.host;
    }
    if (props.port) {
    ...
    if (obj.databaseType == DB_MARIADB) {
      if (props.ssl) {
        ...
        if (props.ssl.dontcheckserveridentity != true) {
          sslOptions += ' --ssl-verify-server-cert';
        } else {
          sslOptions += ' --ssl-verify-server-cert=false';
        }
  ...
  function buildMongoDumpCommand() {
    ...
    if (dburl) {
      cmd = '\"' + mongoDumpPath + '\" --uri=\"' + dburl + '\"';
    }
    if (parent.config.settings.autobackup?.mongodumpargs) {
      cmd = '\"' + mongoDumpPath + '\" ' + parent.config.settings.autobackup.mongodumpargs;
      if (!parent.config.settings.autobackup.mongodumpargs.includes('--db=')) {
        cmd += ' --db=' + (parent.config.settings.mongodbname ? parent.config.settings.mongodbname : 'meshcentral');
      }
    }
    return cmd;
  }
  ...
  obj.checkBackupCapability = function (func) {
    ...
    try {
      fs.writeFileSync(testFile, 'DeleteMe');
    } catch (e) {
    ...
    try {
      ...
      parent.debug('backup', 'Backuppath ' + backupPath + ' accesscheck successful');
    } catch (e) {
      console.error(e.message);
    ...
    if (obj.databaseType == DB_MONGOJS || obj.databaseType == DB_MONGODB) {
      ...
      parent.config.settings.autobackup.pgdumppath = path.normalize(
        parent.config.settings.autobackup.pgdumppath ? parent.config.settings.autobackup.pgdumppath : 'pg_dump'
      );
      let cmd =
        '"' +
        parent.config.settings.autobackup.pgdumppath +
        '"' +
        ' --dbname=postgresql://' +
        encodeURIComponent(parent.config.settings.postgres.user) +
        ':' +
        encodeURIComponent(parent.config.settings.postgres.password) +
        '@' +
        parent.config.settings.postgres.host +
        ':' +
        parent.config.settings.postgres.port +
        '/' +
      ...
      for (var i in docs) {
        ...
        if (obj.filePendingGets[id] != null) {
          for (var j in obj.filePendingGets[id]) {
            if (typeof obj.filePendingGets[id][j] == 'function') {
              obj.filePendingGets[id][j](err, performTypedRecordDecrypt([docs[i]]));
            }
          }
  ...
  function fileBulkRemoveCompleted(err) {
    ...
    for (var i in obj.filePendingRemoves) {
      for (var j in obj.filePendingRemoves[i]) {
        if (typeof obj.filePendingRemoves[i][j] == 'function') {
  ...
  function fileBulkWriteCompleted() {
    ...
    if (obj.filePendingCbs != null) {
      for (var i in obj.filePendingCbs) {
        if (typeof obj.filePendingCbs[i] == 'function') {
          obj.filePendingCbs[i]();
        }
      }
      obj.filePendingCbs = null;
    }
    if (obj.filePendingSets != null) {
      ...
      obj.filePendingCb = null;
      for (var i in obj.filePendingSets) {
        ops.push({
          replaceOne: {
            filter: { _id: i },
            replacement: performTypedRecordEncrypt(common.escapeLinksFieldNameEx(obj.filePendingSets[i])),
            upsert: true
          }
        });
      }
  ...
  function eventsFileBulkWriteCompleted() {
    // Callbacks
    if (obj.eventsFilePendingCbs != null) {
      for (var i in obj.eventsFilePendingCbs) {
        obj.eventsFilePendingCbs[i]();
      }
      obj.eventsFilePendingCbs = null;
    }
    if (obj.eventsFilePendingSets != null) {
      ...
      var ops = [];
      for (var i in obj.eventsFilePendingSets) {
        ops.push({ insertOne: { document: obj.eventsFilePendingSets[i] } });
      }
  ...
  function powerFileBulkWriteCompleted() {
    // Callbacks
    if (obj.powerFilePendingCbs != null) {
      for (var i in obj.powerFilePendingCbs) {
        obj.powerFilePendingCbs[i]();
      }
      obj.powerFilePendingCbs = null;
    }
    if (obj.powerFilePendingSets != null) {
      ...
      var ops = [];
      for (var i in obj.powerFilePendingSets) {
        ops.push({ insertOne: { document: obj.powerFilePendingSets[i] } });
      }
  ...
  obj.performBackup = function (func) {
    ...
    try {
      ...
      if (obj.databaseType == DB_MONGOJS || obj.databaseType == DB_MONGODB) {
        ...
        var cmd = buildMongoDumpCommand();
        cmd += dburl
          ? ' --archive=\"' + obj.newDBDumpFile + '\"'
          : ' --db=\"' + dbname + '\" --archive=\"' + obj.newDBDumpFile + '\"';
        parent.debug('backup', 'Mongodump cmd: ' + cmd);
        const child_process = require('child_process');
        const dumpProcess = child_process.exec(cmd, { cwd: parent.parentpath }, (error) => {
          if (error) {
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
            console.error('ERROR: Unable to perform MongoDB backup: ' + error + '\r\n');
        ...
        dumpProcess.on('exit', (code) => {
          if (code != 0) {
            console.log(`Mongodump child process exited with code ${code}`);
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
          }
          obj.createBackupfile(func);
        });
        ...
        const dumpProcess = child_process.exec(cmd, { cwd: parent.parentpath }, (error) => {
          if (error) {
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
            console.error('ERROR: Unable to perform MySQL backup: ' + error + '\r\n');
            obj.createBackupfile(func);
          }
        });
        dumpProcess.on('exit', (code) => {
          if (code != 0) {
            console.error(`MySQLdump child process exited with code ${code}`);
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
          }
          obj.createBackupfile(func);
        });
        ...
        const child_process = require('child_process');
        const dumpProcess = child_process.exec(cmd, { cwd: dataPath }, (error) => {
          if (error) {
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
            console.log('ERROR: Unable to perform PostgreSQL dump: ' + error.message + '\r\n');
            obj.createBackupfile(func);
          }
        });
        dumpProcess.on('exit', (code) => {
          if (code != 0) {
            console.log(`PostgreSQLdump child process exited with code: ` + code);
            obj.backupStatus |= BACKUPFAIL_DBDUMP;
          }
          obj.createBackupfile(func);
        });
      ...
      console.error(ex);
      parent.addServerWarning('Something went wrong during performBackup, check errorlog: ' + ex.message, true);
    }
  ...
  obj.createBackupfile = function (func) {
    ...
    let archive = null;
    let zipLevel = Math.min(
      Math.max(
        Number(parent.config.settings.autobackup.zipcompression ? parent.config.settings.autobackup.zipcompression : 5),
        1
      ),
      9
    );
    ...
    if (parent.config.settings.autobackup && typeof parent.config.settings.autobackup.zippassword == 'string') {
      try {
        ...
        if (archiver.isRegisteredFormat('zip-encrypted') == false) {
          archiver.registerFormat('zip-encrypted', require('archiver-zip-encrypted'));
        }
        archive = archiver.create('zip-encrypted', {
          zlib: { level: zipLevel },
          encryptionMethod: 'aes256',
          password: parent.config.settings.autobackup.zippassword
        });
        if (func) {
          func('Creating encrypted ZIP');
        }
      } catch (ex) {
        // registering encryption failed, do not fall back to non-encrypted, fail backup and skip old backup removal as a precaution to not lose any backups
        obj.backupStatus |= BACKUPFAIL_ZIPMODULE;
        if (func) {
          func('Zipencryptionmodule failed, aborting');
        }
        console.error('Zipencryptionmodule failed, aborting');
      }
    } else {
      if (func) {
        func('Creating a NON-ENCRYPTED ZIP');
      }
    ...
    if (obj.backupStatus == 0) {
      ...
      output.on('close', function () {
        if (obj.backupStatus == 0) {
          let mesg =
            ...
            obj.newAutoBackupFile +
            ', backup-size: ' +
            (archive.pointer() / 1048576).toFixed(2) +
            'Mb';
          console.log(mesg);
          if (func) {
            func(mesg);
          ...
          let mesg =
            'Zipbackup failed (' +
            obj.backupStatus.toString(2).slice(-8) +
            '), deleting incomplete backup: ' +
            obj.newAutoBackupFile;
          if (func) {
            func(mesg);
          } else {
            parent.addServerWarning(mesg, true);
          }
          if (fs.existsSync(obj.newAutoBackupFile)) {
            fs.unlink(obj.newAutoBackupFile, function (err) {
              if (err) {
                console.error('Failed to clean up backupfile: ' + err.message);
              }
            });
          }
        }
        if (obj.databaseType != DB_NEDB) {
          ...
          if (fs.existsSync(obj.newDBDumpFile)) {
            fs.unlink(obj.newDBDumpFile, function (err) {
              if (err) {
                console.error('Failed to clean up dbdump file: ' + err.message);
              }
            });
          }
        }
      ...
      globIgnoreFiles = parent.config.settings.autobackup.backupignorefilesglob
      ...
      } //skip sqlite database file, and temp files with ext -journal, -wal & -shm
      ...
      if (parent.config.settings.autobackup.backupwebfolders) {
        if (parent.webViewsOverridePath) {
          archive.directory(parent.webViewsOverridePath, 'meshcentral-views');
        }
        if (parent.webPublicOverridePath) {
          archive.directory(parent.webPublicOverridePath, 'meshcentral-public');
        }
        if (parent.webEmailsOverridePath) {
          archive.directory(parent.webEmailsOverridePath, 'meshcentral-emails');
        }
      }
      if (parent.config.settings.autobackup.backupotherfolders) {
      ...
      if (func) {
        func('Backup failed (' + obj.backupStatus.toString(2).slice(-8) + ')');
      } else {
    ...
    if (parent.config.settings.autobackup && typeof parent.config.settings.autobackup.keeplastdaysbackup == 'number') {
      ...
      fs.readdir(parent.backuppath, function (err, dir) {
        try {
          if (err == null) {
            if (dir.length > 0) {
                ...
                if (name.startsWith(fileName) && name.endsWith('.zip')) {
                  ...
                  if (timex.length == 5) {
                    ...
                    if (fileDate && cutoffDate > fileDate) {
                      ...
                      fs.unlink(path.join(parent.backuppath, name), function (err) {
                        if (err) {
                          console.error(err.message);
                          if (func) {
                            func('Error removing: ' + err.message);
                          }
                        }
                      });
                      removed++;
              ...
              let mesg =
                ...
                '. Removed ' +
                removed +
                ' expired backupfiles using cutoffDate: ' +
                cutoffDate.toLocaleString('default', { dateStyle: 'short', timeStyle: 'short' });
              parent.debug(mesg);
              if (func) {
                func(mesg);
              }
            } else {
              console.error('No files found in ' + parent.backuppath + '. There should be at least one.');
            }
          } else {
            console.error(err);
            parent.addServerWarning(
              'Reading files in backup directory ' + parent.backuppath + ' failed, check errorlog: ' + err.message,
              true
            );
          }
        } catch (ex) {
          console.error(ex);
          parent.addServerWarning(
            'Something went wrong during removeExpiredBackupfiles, check errorlog: ' + ex.message,
            true
          );
        }
      });
    ...
    try {
      ...
      if ((await client.exists(wdConfig.foldername)) === false) {
        ...
        if (typeof wdConfig.maxfiles == 'number' && wdConfig.maxfiles > 1) {
          ...
          let files = await client.getDirectoryContents(wdConfig.foldername, {
            deep: false,
            glob: '/**/' + fileName + '*.zip'
          });
          const xdateTimeSort = function (a, b) {
            if (a.xdate > b.xdate) return 1;
            if (a.xdate < b.xdate) return -1;
            return 0;
          };
          for (const i in files) {
            files[i].xdate = new Date(files[i].lastmod);
          }
          ...
          while (files.length >= wdConfig.maxfiles) {
            ...
            console.log('WebDAV file deleted: ' + delFile);
            if (func) {
              func('WebDAV file deleted: ' + delFile);
            }
          }
        }
      }
      ...
      await pipeline(
        ...
        client.createWriteStream(wdConfig.foldername + path.basename(filename))
      );
      console.log('WebDAV upload completed: ' + wdConfig.foldername + path.basename(filename));
      if (func) {
        func('WebDAV upload completed: ' + wdConfig.foldername + path.basename(filename));
      }
    } catch (err) {
      console.error('WebDAV error: ' + err.message);
      if (func) {
        func('WebDAV error: ' + err.message);
      }
    }
  }
  ...
  obj.performCloudBackup = function (filename, func) {
    // WebDAV Backup
    if (
      typeof parent.config.settings.autobackup == 'object' &&
      typeof parent.config.settings.autobackup.webdav == 'object'
    ) {
      parent.debug('backup', 'Entering WebDAV backup');
      if (func) {
        func('Entering WebDAV backup.');
      }
    ...
    if (
      typeof parent.config.settings.autobackup == 'object' &&
      typeof parent.config.settings.autobackup.googledrive == 'object'
    ) {
      ...
      obj.Get('GoogleDriveBackup', function (err, docs) {
        ...
        if (func) {
          func('Attempting Google Drive upload...');
        }
        ...
        oAuth2Client.on('tokens', function (tokens) {
          if (tokens.refresh_token) {
            docs[0].token = tokens.refresh_token;
            parent.db.Set(docs[0]);
          }
        }); // Update the token in the database
        oAuth2Client.setCredentials(docs[0].token);
        const drive = google.drive({ version: 'v3', auth: oAuth2Client });
        const createdTimeSort = function (a, b) {
        ...
        var useGoogleDrive = function (folderid) {
            ...
            drive.files.list(
              ...
              function (err, res) {
                if (err) {
                  ...
                  if (func) {
                    func('GoogleDrive (files.list) error: ' + err);
                  }
                  return;
                }
                // Delete any old files if more than 10 files are present in the backup folder.
                res.data.files.sort(createdTimeSort);
                while (res.data.files.length >= parent.config.settings.autobackup.googledrive.maxfiles) {
                  drive.files.delete({ fileId: res.data.files.shift().id }, function (err, res) {});
                }
              }
            );
          }
          ...
          if (func) {
            func('Uploading to Google Drive...');
          }
          ...
          drive.files.create(
            ...
            function (err, res) {
              if (err) {
                ...
                if (func) {
                  func('GoogleDrive (files.create) error: ' + err);
                }
                return;
              }
              //console.log('Upload done.');
              if (func) {
                func('Google Drive upload completed.');
              }
            }
          );
        };
        ...
        if (typeof parent.config.settings.autobackup.googledrive.foldername == 'string') {
          folderName = parent.config.settings.autobackup.googledrive.foldername;
        }
        ...
        drive.files.list(
          ...
          function (err, res) {
            ...
            if (res.data.files.length == 0) {
              ...
              drive.files.create(
                ...
                function (err, file) {
                  if (err) {
                    ...
                    if (func) {
                      func('GoogleDrive (folder.create) error: ' + err);
                    }
                    return;
                  }
              ...
              useGoogleDrive(res.data.files[0].id);
            }
          }
        );
      });
    ...
    if (
      typeof parent.config.settings.autobackup == 'object' &&
      typeof parent.config.settings.autobackup.s3 == 'object'
    ) {
      parent.debug('backup', 'Entering S3 backup');
      var s3folderName = 'MeshCentral-Backups';
      if (typeof parent.config.settings.autobackup.s3.foldername == 'string') {
        s3folderName = parent.config.settings.autobackup.s3.foldername;
      }
      ...
      var accessKey = parent.config.settings.autobackup.s3.accesskey,
        ...
        endpoint = parent.config.settings.autobackup.s3.endpoint
      ...
      listObjectsPromise
        .then(function (objects) {
          ...
          if (fileCount >= threshold) {
            ...
            if (func) {
              func('Deleting ' + filesToDelete + ' older ' + (filesToDelete == 1 ? 'file' : 'files') + ' from S3 ...');
            }
              ...
              return new Promise(function (resolve, reject) {
                minioClient.removeObject(bucketName, fileToDelete.name, function (err) {
                  if (err) {
                    ...
                    if (func) {
                      func('Deleted file: ' + fileToDelete.name + ' from S3');
                    }
                    resolve();
                  }
                });
        ...
        .then(function () {
          ...
          var uploadPromise = new Promise(function (resolve, reject) {
            if (func) {
              func('Uploading file ' + uploadPath + ' to S3');
            }
            minioClient.fPutObject(bucketName, uploadPath, fileToUpload, function (err, etag) {
              if (err) {
                ...
                if (func) {
                  func('Uploaded file: ' + uploadPath + ' to S3');
                }
                resolve(etag);
              }
            });
        ...
        .catch(function (error) {
          if (func) {
            func('Error managing files in S3: ' + error);
          }
        });
    }
  };
  ...
  obj.nedbtodb = function (func) {
    ...
    try {
      nedbDatastore = require('@seald-io/nedb');
    } catch (ex) {} // This is the NeDB with Node 23 support.
    if (nedbDatastore == null) {
      try {
        nedbDatastore = require('@yetzt/nedb');
      } catch (ex) {} // This is the NeDB with fixed security dependencies.
      if (nedbDatastore == null) {
    ...
    var nedbeventsfile = new nedbDatastore({
      filename: parent.getConfigFilePath('meshcentral-events.db'),
      autoload: true,
      corruptAlertThreshold: 1
    });
    var nedbpowerfile = new nedbDatastore({
      filename: parent.getConfigFilePath('meshcentral-power.db'),
      autoload: true,
      corruptAlertThreshold: 1
    });
    var nedbserverstatsfile = new nedbDatastore({
      filename: parent.getConfigFilePath('meshcentral-stats.db'),
      autoload: true,
      corruptAlertThreshold: 1
    });
    ...
    nedbfile.find({}, function (err, docs) {
      if (err == null && docs.length > 0) {
        ...
        for (var i in docs) {
          ...
          obj.Set(common.unEscapeLinksFieldName(docs[i]), function () {
            obj.pendingTransfer--;
          });
        }
      }
      ...
      nedbeventsfile.find({}, function (err, docs) {
        if (err == null && docs.length > 0) {
          for (var i in docs) {
            ...
            obj.StoreEvent(docs[i], function () {
              obj.pendingTransfer--;
            });
          }
        }
        ...
        nedbpowerfile.find({}, function (err, docs) {
          if (err == null && docs.length > 0) {
            for (var i in docs) {
              ...
              obj.storePowerEvent(docs[i], null, function () {
                obj.pendingTransfer--;
              });
            }
          }
          ...
          nedbserverstatsfile.find({}, function (err, docs) {
            if (err == null && docs.length > 0) {
              for (var i in docs) {
                ...
                obj.SetServerStats(docs[i], function () {
                  obj.pendingTransfer--;
                });
              }
            }
  ...
  function dbNodeChange(nodeChange, added) {
    ...
    parent.DispatchEvent(['*', node.meshid], obj, {
      ...
      node: parent.webserver.CloneSafeNode(node),
      nodeid: node._id,
      domain: node.domain,
      nolog: 1
    });
  ...
  function dbMeshChange(meshChange, added) {
    ...
    if (mesh2.deleted) {
      mesh2.action = 'deletemesh';
    } else {
      mesh2.action = added ? 'createmesh' : 'meshchange';
    }
  ...
  function dbUserChange(userChange, added) {
    ...
    parent.DispatchEvent(targets, obj, {
      etype: 'user',
      userid: user._id,
      username: user.name,
      account: parent.webserver.CloneSafeUser(user),
      action: added ? 'accountcreate' : 'accountchange',
      domain: user.domain,
      nolog: 1
    });
  }
  ...
  function dbUGrpChange(ugrpChange, added) {
    ...
    if (uusergroup != null) {
      ...
      for (var i in usergroup) {
        uusergroup[i] = usergroup[i];
      }
      for (var i in uusergroup) {
        if (usergroup[i] == null) {
          delete uusergroup[i];
        }
      }
