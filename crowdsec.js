module.exports.CreateCrowdSecBouncer = function (parent, config) {
  ...
  const {
    ...
    getRemediationForIp
  } = require('@crowdsec/express-bouncer/src/nodejs-bouncer');
  const applyCaptcha = require('@crowdsec/express-bouncer/src/express-crowdsec-middleware/lib/captcha');
  const {
    BYPASS_REMEDIATION,
    CAPTCHA_REMEDIATION,
    BAN_REMEDIATION
  } = require('@crowdsec/express-bouncer/src/nodejs-bouncer/lib/constants'); // "bypass", "captcha", "ban";
  ...
  if (typeof config.useragent != 'string') {
    config.useragent = 'CrowdSec Express-NodeJS bouncer/v0.0.1';
  }
  if (typeof config.timeout != 'number') {
    config.timeout = 2000;
  }
  if (
    typeof config.fallbackremediation != 'string' ||
    ['bypass', 'captcha', 'ban'].indexOf(config.fallbackremediation) == -1
  ) {
    config.fallbackremediation = BAN_REMEDIATION;
  }
  if (typeof config.maxremediation != 'number') {
    config.maxremediation = BAN_REMEDIATION;
  }
  if (typeof config.captchagenerationcacheduration != 'number') {
    config.captchagenerationcacheduration = 60 * 1000;
  } // 60 seconds
  if (typeof config.captcharesolutioncacheduration != 'number') {
    config.captcharesolutioncacheduration = 30 * 60 * 1000;
  } // 30 minutes
  if (typeof config.captchatexts != 'object') {
    config.captchatexts = {};
  } else {
    if (typeof config.captchatexts.tabtitle == 'string') {
      config.captchatexts.tabTitle = config.captchatexts.tabtitle;
      delete config.captchatexts.tabtitle;
    } // Fix "tabTitle" capitalization
  }
  if (typeof config.bantexts != 'object') {
    config.bantexts = {};
  } else {
    if (typeof config.bantexts.tabtitle == 'string') {
      config.bantexts.tabTitle = config.bantexts.tabtitle;
      delete config.bantexts.tabtitle;
    } // Fix "tabTitle" capitalization
  }
  if (typeof config.colors != 'object') {
    ...
    if (typeof config.colors.text == 'object') {
      colors.text = config.colors.text;
    }
    if (typeof config.colors.background == 'object') {
      colors.background = config.colors.background;
    }
    config.colors = colors;
  }
  if (typeof config.hidecrowdsecmentions != 'boolean') {
    config.hidecrowdsecmentions = false;
  }
  ...
  if (typeof config.customlogger != 'object') {
    delete config.customlogger;
  }
  if (typeof config.bypassconnectiontest != 'boolean') {
    config.bypassconnectiontest = false;
  }
  ...
  // Test connectivity
  obj.testConnectivity = async function () {
    return (await testConnectionToCrowdSec())['success'];
  };
  ...
  obj.process = async function (domain, req, res, next) {
    try {
      ...
      try {
        remediation = await getRemediationForIp(req.clientIp);
      } catch (ex) {}
      ...
      switch (remediation) {
        ...
        case CAPTCHA_REMEDIATION:
          if (currentCaptchaIpList[req.clientIp] == null || currentCaptchaIpList[req.clientIp].resolved !== true) {
            var domainCaptchaUrl =
              domain != null && domain.id != '' && domain.dns == null
                ? '/' + domain.id + '/captcha.ashx'
                : '/captcha.ashx';
            if (req.url != domainCaptchaUrl) {
              res.redirect(domainCaptchaUrl);
              return true;
            }
          }
          break;
  ...
  obj.applyCaptcha = async function (req, res, next) {
    await applyCaptchaEx(
      req.clientIp,
      req,
      res,
      next,
      config.captchagenerationcacheduration,
      config.captcharesolutioncacheduration,
      logger
    );
  };

  // Process a captcha request
  async function applyCaptchaEx(
    ip,
    req,
    res,
    next,
