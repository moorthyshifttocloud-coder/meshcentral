...
module.exports.CreateMonitoring = function (parent, args) {
  ...
  obj.counterMetrics = {
    // Counter Metrics always start at 0 and increase but never decrease
  ...
  obj.gaugeMetrics = {
    // Gauge Metrics always start at 0 and can increase and decrease
  ...
  obj.collectors = [];
  if (parent.config.settings.prometheus != null) {
    // Create Prometheus Monitoring Endpoint
    if (
      typeof parent.config.settings.prometheus == 'number' &&
      (parent.config.settings.prometheus < 1 || parent.config.settings.prometheus > 65535)
    ) {
      ...
      for (const key in obj.gaugeMetrics) {
        obj.gaugeMetrics[key].prometheus = new obj.prometheus.Gauge({
          name: 'meshcentral_' + String(key).toLowerCase(),
          help: obj.gaugeMetrics[key].description
        });
      }
      for (const key in obj.counterMetrics) {
        obj.counterMetrics[key].prometheus = new obj.prometheus.Counter({
          name: 'meshcentral_' + String(key).toLowerCase(),
          help: obj.counterMetrics[key].description
        });
      }
      obj.app.get('/', function (req, res) {
        res.send('MeshCentral Prometheus server.');
      });
      ...
      obj.app.get('/metrics', async (req, res) => {
        try {
          ...
          for (var i in parent.webserver.meshes) {
            if (parent.webserver.meshes[i].deleted == null) {
              activeDeviceGroups++;
            }
          ...
          for (var i in parent.connectivityByNode) {
            ...
            if (node && typeof node.connectivity !== 'undefined' && node.connectivity === 4) {
              gauges.ConnectedIntelAMT++;
            }
          }
          for (const key in gauges) {
            obj.gaugeMetrics[key].prometheus.set(gauges[key]);
          }
          ...
          for (const key in counters) {
            obj.counterMetrics[key].prometheus.reset();
            obj.counterMetrics[key].prometheus.inc(counters[key]);
          }
