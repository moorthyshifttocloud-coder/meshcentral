...
module.exports.streamExeWithJavaScript = function (options) {
  ...
  if (!options.sourceFileName) {
    throw 'source file not specified';
  }
  if (!options.js) {
...
module.exports.streamExeWithMeshPolicy = function (options) {
  ...
  if (!options.msh) {
    throw 'msh content not specified';
  }
  ...
  if ((options.platform == 'win32' && options.peinfo.CertificateTableAddress == 0) || options.platform != 'win32') {
    ...
    options.destinationStream.sourceStream.on('end', function () {
      ...
      this.options.destinationStream.write(sz); // Length in small endian
      this.options.destinationStream.end(
        Buffer.from(this.options.randomPolicy === true ? exeNullPolicyGuid : exeMeshPolicyGuid, 'hex')
      ); // Guid
    });
    // Pipe the entire source binary without ending the stream.
    options.destinationStream.sourceStream.pipe(options.destinationStream, { end: false });
    ...
    options.destinationStream.sourceStream = require('fs').createReadStream(options.sourceFileName, {
      flags: 'r',
      start: 0,
      end: options.peinfo.CertificateTableSizePos - 1
    });
    options.destinationStream.sourceStream.mshPadding =
      (8 - ((options.peinfo.certificateDwLength + options.mshbuf.length + 20) % 8)) % 8; // Compute the padding with quad-align
    options.destinationStream.sourceStream.CertificateTableSize =
      options.peinfo.CertificateTableSize +
      options.mshbuf.length +
      20 +
      options.destinationStream.sourceStream.mshPadding; // Add to the certificate table size
    options.destinationStream.sourceStream.certificateDwLength =
      options.peinfo.certificateDwLength +
      options.mshbuf.length +
      20 +
      options.destinationStream.sourceStream.mshPadding; // Add to the certificate size
    options.destinationStream.sourceStream.options = options;
...
module.exports.hashExecutableFile = function (options) {
  if (!options.sourcePath || !options.targetStream) {
    throw 'Please specify sourcePath and targetStream';
  }
  var fs = require('fs');
  ...
  if (options.platform == 'win32') {
    if (options.peinfo.CertificateTableAddress != 0) {
      options.state.endIndex = options.peinfo.CertificateTableAddress;
    }
  ...
  if (options.state.checkSumIndex != 0) {
    // Windows
    options.state.source = fs.createReadStream(options.sourcePath, {
      flags: 'r',
      start: 0,
      end: options.state.checkSumIndex - 1
    });
    options.state.source.on('end', function () {
      options.targetStream.write(Buffer.alloc(4));
      var source = fs.createReadStream(options.sourcePath, {
        flags: 'r',
        start: options.state.checkSumIndex + 4,
        end: options.state.tableIndex - 1
      });
      source.on('end', function () {
        options.targetStream.write(Buffer.alloc(8));
        var source = fs.createReadStream(options.sourcePath, {
          flags: 'r',
          start: options.state.tableIndex + 8,
          end: options.state.endIndex - 1
        });
