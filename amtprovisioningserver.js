...
module.exports.CreateAmtProvisioningServer = function (parent, config) {
  ...
  // Check configuration
  if (checkAmtPassword(config.newmebxpassword) == false) {
    console.log(
      'Invalid MEBx password, must have 1 lower, 1 upper, 1 numeric, 1 non-alpha and be 8 or more in length.'
    );
    return null;
  }
  ...
  obj.server = net.createServer(function (socket) {
    ...
    socket.on('close', function () {
      if (this.data != null) {
        processHelloData(this.data, this.ra);
      }
      delete this.ra;
    ...
    socket.on('data', function (data) {
      ...
      if (str.startsWith('GET ') && str.indexOf('\r\n\r\n') >= 0) {
        this.data = null;
        var content =
          '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Intel&reg; AMT Hello Server</title></head><body>Intel AMT hello server.<br />Intel&reg; AMT devices should send notification to this port for activation.</body></html>';
        try {
          socket.end(
  ...
  function processHelloData(data, addr) {
    ...
    if (dev == null) {
      parent.debug('amtsca', addr, 'Got invalid hello from: ' + addr);
      return;
    } // Invalid Intel AMT hello
    ...
    dev.consoleMsg = function deviceConsoleMsg(msg) {
      parent.debug(
        'amtsca',
        deviceConsoleMsg.dev.aquired.host ? deviceConsoleMsg.dev.aquired.host : deviceConsoleMsg.dev.addr,
        msg
      );
      return;
    };
    ...
    if (mesh == null || mesh.mtype !== 1 || typeof mesh.amt !== 'object' || typeof mesh.amt.type !== 'number') {
      dev.consoleMsg('Invalid device group for Intel AMT activation.');
      return;
    }
    if (mesh.amt.type != 3 && mesh.amt.type != 4) {
      dev.consoleMsg('Device group does not have ACM activation policy.');
      return;
    }
    dev.mesh = mesh;
    dev.meshid = mesh._id;
    ...
    const id = Buffer.from(g + g + g, 'hex')
      .toString('base64')
      .replace(/\+/g, '@')
      .replace(/\//g, '$');
  ...
  function processHelloDataEx1(dev) {
    ...
    if (certinfo == null) {
      dev.consoleMsg('Unable to find a matching ACM activation certificate.');
      destroyDevice(dev);
      return;
    }
    var certchain = parent.certificateOperations.getAcmCertChain(
      parent.config.domains[dev.domainid],
      dev.trustedFqdn,
      certinfo.cert.sha256
    );
    if (certchain == null) {
      dev.consoleMsg('Unable to create TLS certificate chain.');
      destroyDevice(dev);
      return;
    }
    ...
    var comm = CreateWsmanComm(dev.aquired.host, 16993, 'admin', '', 1, {
      cert: dev.certchain.certs.reverse().join(''),
      key: dev.certchain.signkey
  ...
  function processHelloDataEx2(stack, name, responses, status) {
    ...
    if (status != 200) {
      dev.consoleMsg('Failed TLS connection, status=' + status + '.');
      destroyDevice(dev);
      return;
    }
      ...
      if (amtlogicalelements.length > 0) {
        ...
        if (vs != null) {
          ...
          if (versionSplit.length >= 3) {
            dev.aquired.versionmaintenance = parseInt(versionSplit[2]);
          }
        }
      }
    }
  ...
  function processHelloDataEx3(stack, name, responses, status) {
    ...
    if (dev.aquired.pass == null) {
      dev.aquired.pass = getRandomAmtPassword();
    }
    ...
    attemptSyncClock(dev, function (dev) {
      ...
      attemptTlsSync(dev, function (dev) {
        ...
        if (obj.parent.amtManager != null) {
          obj.parent.amtManager.startAmtManagement(dev.nodeid, 3, dev.aquired.host);
        } // Request that Intel AMT manager take a look at this device.
  ...
  function attemptTlsSyncEx(stack, name, responses, status) {
    ...
    dev.consoleMsg(
      'Intel AMT has ' +
        xxCertificates.length +
        ' certificate(s) and ' +
        dev.policy.certPrivateKeys.length +
        ' private keys(s).'
    );
      ...
      generateKeyPairWithRetry(dev, function (stack, name, responses, status) {
        ...
        dev.amtstack.Enum(
          ...
          function (stack, name, responses, status, tag) {
            ...
            const cert = parent.amtManager.amtcert_createCertificate(
              certattributes,
              xxCaPrivateKey,
              DERKey,
              issuerattributes,
            ...
            dev.amtstack.AMT_PublicKeyManagementService_AddCertificate(
              ...
              function (stack, name, responses, status) {
                ...
                var localNdx = dev.policy.tlsSettings[0]['InstanceID'] == 'Intel(r) AMT LMS TLS Settings' ? 0 : 1,
  ...
  function destroyDevice(dev) {
    ...
    if (dev.amtstack != null) {
      ...
      delete dev.amtstack;
    } // Clean up the AMT stack.
    for (var i in dev) {
      delete dev[i];
    } // Aggressive cleanup or everything else.
  }
  ...
  function UpdateDevice(dev) {
    ...
    parent.db.Get(dev.nodeid, function (err, nodes) {
      if (nodes == null || nodes.length == 0) {
        ...
        if (dev.addr != dev.aquired.host) {
          devicename = dev.aquired.host.split('.')[0];
        }
        var device = {
          type: 'node',
          _id: dev.nodeid,
          ...
          domain: dev.domainid,
          intelamt: {
            ver: dev.aquired.version,
            user: dev.aquired.user,
            pass: dev.aquired.pass,
            tls: dev.aquired.tls,
            state: 2,
            realm: dev.aquired.realm
          }
        };
        if (dev.aquired.hash != null) {
        ...
        if (dev.aquired.controlMode == 1) {
          device.intelamt.flags = 2;
        } // CCM
        if (dev.aquired.controlMode == 2) {
          device.intelamt.flags = 4;
        } // ACM
        ...
        parent.DispatchEvent(parent.webserver.CreateMeshDispatchTargets(dev.meshid, [dev.nodeid]), obj, {
          ...
          msgid: 84,
          msgArgs: [devicename, mesh.name],
          msg: 'Added device ' + devicename + ' to device group ' + mesh.name,
          domain: dev.domainid
        });
      } else {
        ...
        if (typeof dev.aquired.warn == 'number') {
          if (dev.aquired.warn == 0 && device.intelamt.warn != null) {
            ...
            change = 1;
          } else if (dev.aquired.warn != device.intelamt.warn) {
  ...
  obj.logAmtActivation = function (domain, x) {
    ...
    try {
      parent.fs.appendFileSync(logpath, JSON.stringify(x) + '\r\n');
    } catch (ex) {
      console.log(ex);
      return false;
    }
  ...
  function isAmtDeviceValid(dev) {
    return obj.devices[dev.addr] != null;
  }
