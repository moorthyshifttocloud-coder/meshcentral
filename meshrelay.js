  ...
  try {
    if (logfile.text) {
        ...
        if (logfile.text == 1) {
          ...
          try {
            ...
            if (typeof x.action == 'string') {
              if (x.action == 'chat' && typeof x.msg == 'string') {
                out = utcDate.toUTCString() + ', ' + ((flags & 2 ? '--> ' : '<-- ') + x.msg + '\r\n');
              } else if (x.action == 'file' && typeof x.name == 'string' && typeof x.size == 'number') {
                out =
                  utcDate.toUTCString() +
                  ', ' +
                  ((flags & 2 ? '--> ' : '<-- ') +
      ...
      if (out != null) {
        ...
        require('fs').write(logfile.fd, block, 0, block.length, function () {
          func(logfile, tag);
        });
...
function CreateMeshRelayEx(parent, ws, req, domain, user, cookie) {
  ...
  if (cookie) {
    if (typeof cookie.expire == 'number' && cookie.expire <= currentTime) {
      try {
        ws.close();
        parent.parent.debug('relay', 'Relay: Expires cookie (' + req.clientIp + ')');
      } catch (e) {
        console.log(e);
      }
      return;
    }
    if (typeof cookie.nid == 'string') {
      req.query.nodeid = cookie.nid;
    }
  }
  var obj = {};
  obj.ws = ws;
  ...
  if (cookie != null && cookie.nouser == 1) {
    obj.nouser = true;
  } // This is a relay without user authentication
  ...
  if (cookie != null && typeof cookie.pid == 'string') {
    ...
    obj.HandleEvent = function (source, event, ids, id) {
      if (event.action == 'removedDeviceShare' && obj.pid == event.publicid) {
        closeBothSides();
      }
    };
  }
  ...
  if (user == null && obj.req.query != null && obj.req.query.rauth != null) {
    ...
    if (rcookie.ruserid != null) {
      obj.ruserid = rcookie.ruserid;
    } else if (rcookie.nouser === 1) {
      obj.rnouser = true;
    }
  }
  ...
  if (
    obj.id != null &&
    obj.id.startsWith('meshmessenger/') == false &&
    obj.user == null &&
    obj.ruserid == null &&
    obj.nouser !== true &&
    obj.rnouser !== true
    ...
    try {
      ws.close();
      parent.parent.debug('relay', 'Relay: Connection with no authentication (' + obj.req.clientIp + ')');
    } catch (e) {
      console.log(e);
    }
    return;
  ...
  if (req.query.slowrelay != null) {
    var sr = null;
    try {
      sr = parseInt(req.query.slowrelay);
    } catch (ex) {}
    if (typeof sr == 'number' && sr > 0 && sr < 1000) {
      obj.ws.slowRelay = sr;
    }
  }

  // Check if protocol is set in the cookie and if so replace req.query.p but only if its not already set or blank
  if (cookie != null && typeof cookie.p == 'number' && (obj.req.query.p === undefined || obj.req.query.p === '')) {
    obj.req.query.p = cookie.p;
  }

  // Patch Messenger protocol to 200
  if (obj.id != null && obj.id.startsWith('meshmessenger/') == true) {
    obj.req.query.p = 200;
  }
  ...
  obj.close = function (arg) {
    if (arg == 1 || arg == null) {
      try {
        ws.close();
        parent.parent.debug('relay', 'Relay: Soft disconnect (' + obj.req.clientIp + ')');
      } catch (e) {
        console.log(e);
      }
    } // Soft close, close the websocket
    if (arg == 2 || req.query.hd == 1) {
      try {
        ws._socket._parent.end();
        parent.parent.debug('relay', 'Relay: Hard disconnect (' + obj.req.clientIp + ')');
      } catch (e) {
        console.log(e);
      }
    ...
    // Unsubscribe
    if (obj.pid != null) {
      parent.parent.RemoveAllEventDispatch(obj);
    }
  };
  ...
  obj.sendAgentMessage = function (command, userid, domainid) {
    ...
    if (splitnodeid[0] == 'node' && splitnodeid[1] == domainid) {
      ...
      if (agent != null) {
        ...
        if (userid == null) {
          rights = MESHRIGHT_REMOTECONTROL;
        } else {
        ...
        if ((rights != null && mesh != null) || (rights & MESHRIGHT_REMOTECONTROL) != 0) {
          if (ws.sessionId) {
            command.sessionid = ws.sessionId;
          } // Set the session id, required for responses.
          command.rights = rights; // Add user rights flags to the message
          if (typeof command.consent == 'number') {
            command.consent = command.consent | mesh.consent;
          } else {
        ...
        if (routing != null) {
          ...
          if (rights != null || (rights & MESHRIGHT_REMOTECONTROL) != 0) {
            if (ws.sessionId) {
              command.fromSessionid = ws.sessionId;
            } // Set the session id, required for responses.
            command.rights = rights; // Add user rights flags to the message
            if (typeof command.consent == 'number') {
              command.consent = command.consent | mesh.consent;
  ...
  obj.pushStoredMessages = function () {
    if (obj.storedPushedMessages != null && obj.peer != null) {
      for (var i in obj.storedPushedMessages) {
        try {
          obj.peer.ws.send(JSON.stringify({ action: 'chat', msg: obj.storedPushedMessages[i] }));
        } catch (ex) {
          console.log(ex);
        }
      }
  ...
  obj.sendPeerImage = function () {
    if (
      ...
      obj.id.startsWith('meshmessenger/') &&
      obj.peer != null &&
      obj.user != null &&
      typeof obj.user.flags == 'number' &&
      obj.user.flags & 1
    ) {
      parent.db.Get('im' + obj.user._id, function (err, docs) {
        if (err == null && docs != null && docs.length == 1 && typeof docs[0].image == 'string') {
          try {
            obj.peer.ws.send(JSON.stringify({ ctrlChannel: '102938', type: 'image', image: docs[0].image }));
          } catch (ex) {}
        }
      });
    }
  ...
  function sendPing() {
    try {
      obj.ws.send('{"ctrlChannel":"102938","type":"ping"}');
    } catch (ex) {}
    try {
      if (obj.peer != null) {
        obj.peer.ws.send('{"ctrlChannel":"102938","type":"ping"}');
      }
    } catch (ex) {}
  }
  function sendPong() {
    try {
      obj.ws.send('{"ctrlChannel":"102938","type":"pong"}');
    } catch (ex) {}
    try {
      if (obj.peer != null) {
        obj.peer.ws.send('{"ctrlChannel":"102938","type":"pong"}');
      }
    } catch (ex) {}
  }
  ...
  function performRelay() {
    if (obj.id == null) {
      ...
      return null;
    } // Attempt to connect without id, drop this.
    ...
    if (obj.id.startsWith('meshmessenger/')) {
      if (obj.id.startsWith('meshmessenger/user/') == true && user == null) {
        try {
          obj.close();
        } catch (e) {}
        return null;
      } // If user-to-user, both sides need to be authenticated.
      var x = obj.id.split('/'),
        user1 = x[1] + '/' + x[2] + '/' + x[3],
        user2 = x[4] + '/' + x[5] + '/' + x[6];
      if (x[1] != 'user' && x[4] != 'user') {
        ...
        return null;
      } // MeshMessenger session must have at least one authenticated user
      if (x[1] == 'user' && x[4] == 'user') {
        // If this is a user-to-user session, you must be authenticated to join.
        if (user._id != user1 && user._id != user2) {
          try {
            obj.close();
          } catch (e) {}
          return null;
      ...
      if (relayinfo) {
        if (relayinfo.state == 1) {
          ...
          if (!obj.id.startsWith('meshmessenger/')) {
            ...
            if (parent.args.user != null) {
              // If the server is setup with a default user, correct the userid now.
              if (u1 != null) {
                u1 = 'user/' + domain.id + '/' + parent.args.user.toLowerCase();
              }
              if (u2 != null) {
                u2 = 'user/' + domain.id + '/' + parent.args.user.toLowerCase();
              }
            }
            if (u1 != u2 && obj.nouser !== true && relayinfo.peer1.nouser !== true) {
              ...
              parent.parent.debug(
                'relay',
                'Relay auth mismatch (' + u1 + ' != ' + u2 + '): ' + obj.id + ' (' + obj.req.clientIp + ')'
              );
              delete obj.id;
              delete obj.ws;
              delete obj.peer;
          ...
          // Remove the timeout
          if (relayinfo.timeout) {
            clearTimeout(relayinfo.timeout);
            delete relayinfo.timeout;
          }

          // Check the protocol in use
          req.query.p = parseInt(req.query.p);
          if (typeof req.query.p != 'number') {
            req.query.p = parseInt(obj.peer.req.query.p);
            if (typeof req.query.p != 'number') {
              req.query.p = 0;
            }
          }
          ...
          if (parent.trafficStats.relayIn[req.query.p] == null) {
            parent.trafficStats.relayIn[req.query.p] = 0;
          }
          if (parent.trafficStats.relayOut[req.query.p] == null) {
            parent.trafficStats.relayOut[req.query.p] = 0;
          }
          if (parent.trafficStats.relayCount[req.query.p] == null) {
            parent.trafficStats.relayCount[req.query.p] = 1;
          } else {
            parent.trafficStats.relayCount[req.query.p]++;
          }

          // Setup the agent PING/PONG timers unless requested not to
          if (
            obj.req.query.noping != 1 &&
            obj.peer.req != null &&
            obj.peer.req.query != null &&
            obj.peer.req.query.noping != 1
          ) {
            if (typeof parent.parent.args.agentping == 'number' && obj.pingtimer == null) {
              obj.pingtimer = setInterval(sendPing, parent.parent.args.agentping * 1000);
            } else if (typeof parent.parent.args.agentpong == 'number' && obj.pongtimer == null) {
              obj.pongtimer = setInterval(sendPong, parent.parent.args.agentpong * 1000);
            }
          }
          ...
          if (
            obj.req.query.p != null &&
            obj.req.query.nodeid != null &&
            sessionUser != null &&
            (domain.sessionrecording == true ||
              (typeof domain.sessionrecording == 'object' &&
                (domain.sessionrecording.protocols == null ||
                  domain.sessionrecording.protocols.indexOf(parseInt(obj.req.query.p)) >= 0)))
          ) {
            recordSession = true;
          }
            ...
            parent.db.Get(obj.req.query.nodeid, function (err, nodes) {
              ...
              if (
                ...
                (domain.sessionrecording.onlyselectedusers === true ||
                  domain.sessionrecording.onlyselectedusergroups === true ||
                ...
                if (sessionUser != null && domain.sessionrecording.onlyselectedusers === true) {
                  if (sessionUser.flags != null && (sessionUser.flags & 2) != 0) {
                    record = true;
                  }
                }
                ...
                if (record == false && node != null && domain.sessionrecording.onlyselecteddevicegroups === true) {
                  ...
                  if (mesh != null && mesh.flags != null && (mesh.flags & 4) != 0) {
                    record = true;
                  }
                }
                ...
                if (record == false && domain.sessionrecording.onlyselectedusergroups === true) {
                  ...
                  if (
                    sessionUser != null &&
                    sessionUser.links != null &&
                    sessionUser.links[node.meshid] == null &&
                    sessionUser.links[node._id] == null
                  ) {
                    ...
                    for (var i in sessionUser.links) {
                      ...
                      if (
                        ugrp != null &&
                        typeof ugrp.flags == 'number' &&
                        (ugrp.flags & 2) != 0 &&
                        ugrp.links != null &&
                        (ugrp.links[node.meshid] != null || ugrp.links[node._id] != null)
                      ) {
                        record = true;
                      }
                    }
              ...
              if (record == false) {
                try {
                  ws.send('c');
                } catch (ex) {} // Send connect to both peers
                try {
              ...
              if (sessionUser._id) {
                xusername = '-' + parent.common.makeFilename(sessionUser._id.split('/')[2]);
              }

              var now = new Date(Date.now());
              var xsessionid = obj.id;
              if (typeof xsessionid == 'string' && xsessionid.startsWith('meshmessenger/node/') == true) {
                xsessionid = 'Messenger';
              }
              var recFilename =
                'relaysession' +
                (domain.id == '' ? '' : '-') +
                ...
                now.getUTCFullYear() +
                '-' +
                parent.common.zeroPad(now.getUTCMonth() + 1, 2) +
                '-' +
                parent.common.zeroPad(now.getUTCDate(), 2) +
                '-' +
                parent.common.zeroPad(now.getUTCHours(), 2) +
                '-' +
                parent.common.zeroPad(now.getUTCMinutes(), 2) +
                '-' +
                parent.common.zeroPad(now.getUTCSeconds(), 2) +
                xusername +
                xdevicename +
                '-' +
                xsessionid +
                (xtextSession ? '.txt' : '.mcrec');
              var recFullFilename = null;
              if (domain.sessionrecording.filepath) {
                try {
                  parent.parent.fs.mkdirSync(domain.sessionrecording.filepath);
                } catch (e) {}
                recFullFilename = parent.parent.path.join(domain.sessionrecording.filepath, recFilename);
              } else {
                try {
                  parent.parent.fs.mkdirSync(parent.parent.recordpath);
                } catch (e) {}
              ...
              parent.parent.fs.open(recFullFilename, 'w', function (err, fd) {
                if (err != null) {
                  ...
                  if (node != null) {
                    logfile.nodeid = node._id;
                    logfile.meshid = node.meshid;
                    logfile.name = node.name;
                    logfile.icon = node.icon;
                  }
                  recordingEntry(logfile, 1, 0, firstBlock, function () {
                    try {
                      relayinfo.peer1.ws.logfile = ws.logfile = logfile;
                    } catch (ex) {
                      try {
                        ws.send('c');
                      } catch (ex) {} // Send connect to both peers, 'cr' indicates the session is being recorded.
                      try {
                        relayinfo.peer1.ws.send('c');
                      } catch (ex) {}
                      // Send any stored push messages
          ...
          parent.parent.debug(
            'relay',
            'Relay connected: ' + obj.id + ' (' + obj.req.clientIp + ' --> ' + obj.peer.req.clientIp + ')'
          );
          ...
          if (sessionUser != null) {
            var msg = 'Started relay session',
              msgid = 13;
            if ([1, 6, 8, 9].indexOf(obj.req.query.p) >= 0) {
              msg = 'Started terminal session';
              msgid = 14;
            } // admin shell, admin powershell, user shell, user powershell
            else if (obj.req.query.p == 2) {
              msg = 'Started desktop session';
              msgid = 15;
            } else if (obj.req.query.p == 5) {
              msg = 'Started file management session';
              msgid = 16;
            } else if (obj.req.query.p == 200) {
              msg = 'Started messenger session';
              msgid = 162;
            }
            var event = {
              etype: 'relay',
              ...
              username: sessionUser.name,
              msgid: msgid,
              msgArgs: [obj.id, obj.peer.req.clientIp, req.clientIp],
              msg: msg + ' \"' + obj.id + '\" from ' + obj.peer.req.clientIp + ' to ' + req.clientIp,
              protocol: req.query.p,
              nodeid: req.query.nodeid
            };
            if (obj.guestname) {
              event.guestname = obj.guestname;
            } else if (relayinfo.peer1.guestname) {
              event.guestname = relayinfo.peer1.guestname;
            } // If this is a sharing session, set the guest name here.
        ...
        if (obj.id.startsWith('meshmessenger/node/') == true && obj.authenticated) {
          obj.req.query.browser = 1;
        }
        ...
        if (obj.id.startsWith('meshmessenger/node/') == true && obj.authenticated && parent.parent.firebase != null) {
          ...
          parent.db.Get(nodeid, function (err, nodes) {
            if (err == null && nodes != null && nodes.length == 1 && typeof nodes[0].pmt == 'string') {
              if ((parent.GetNodeRights(obj.user, nodes[0].meshid, nodes[0]._id) & MESHRIGHT_CHATNOTIFY) != 0) {
                ...
                obj.msgurl =
                  req.headers.origin +
                  req.url.split('/.websocket')[0].split('/meshrelay.ashx').join('/messenger') +
                  '?id=' +
                  req.query.id;
              }
            }
          ...
          parent.parent.debug(
            'relay',
            'Relay holding: ' + obj.id + ' (' + obj.req.clientIp + ') ' + (obj.authenticated ? 'Authenticated' : '')
          );
    ...
    if (this.peer != null) {
      ...
      if (typeof data == 'string' && obj.node != null && obj.node.pmt != null) {
        ...
        if (
          ...
          command.action != 'chat' ||
          typeof command.msg != 'string' ||
          command.msg == ''
        )
          return;

        // Store pushed messages
        if (obj.storedPushedMessages == null) {
          obj.storedPushedMessages = [];
        }
        obj.storedPushedMessages.push(command.msg);
        while (obj.storedPushedMessages.length > 50) {
          obj.storedPushedMessages.shift();
        ...
        parent.parent.firebase.sendToDevice(obj.node, payload, options, function (id, err, errdesc) {
          if (err == null) {
            ...
            try {
              ws.send(JSON.stringify({ action: 'ctrl', value: 1 }));
            } catch (ex) {} // Push notification success
          } else {
            parent.parent.debug(
              'email',
            ...
            try {
              ws.send(JSON.stringify({ action: 'ctrl', value: 2 }));
            } catch (ex) {} // Push notification failed
          }
        });
      }
    }
  ...
  ws.on('error', function (err) {
    ...
    if (obj.relaySessionCounted) {
      parent.relaySessionCount--;
      delete obj.relaySessionCounted;
    }
    ...
    if (obj.id != null) {
      ...
      if (relayinfo != null) {
        if (relayinfo.state == 2) {
          ...
          try {
            inTraffc = ws._socket.bytesRead + peer.ws._socket.bytesRead;
          } catch (ex) {}
          try {
            outTraffc = ws._socket.bytesWritten + peer.ws._socket.bytesWritten;
          } catch (ex) {}

          // Disconnect the peer
          try {
            if (peer.relaySessionCounted) {
              parent.relaySessionCount--;
              delete peer.relaySessionCounted;
            }
          } catch (ex) {
            console.log(ex);
          }
          parent.parent.debug(
            'relay',
            'Relay disconnect: ' + obj.id + ' (' + obj.req.clientIp + ' --> ' + peer.req.clientIp + ')'
          );
          try {
            peer.ws.close();
          } catch (e) {} // Soft disconnect
          try {
            peer.ws._socket._parent.end();
          } catch (e) {} // Hard disconnect
          ...
          if (ws.time) {
            var msg = 'Ended relay session',
              msgid = 9;
            if ([1, 6, 8, 9].indexOf(obj.req.query.p) >= 0) {
              ((msg = 'Ended terminal session'), (msgid = 10));
            } // admin shell, admin powershell, user shell, user powershell
            else if (obj.req.query.p == 2) {
              ((msg = 'Ended desktop session'), (msgid = 11));
            } else if (obj.req.query.p == 5) {
              ((msg = 'Ended file management session'), (msgid = 12));
            } else if (obj.req.query.p == 200) {
              ((msg = 'Ended messenger session'), (msgid = 112));
            }
            ...
            if (user) {
              var event = {
                ...
                username: user.name,
                msgid: msgid,
                msgArgs: [obj.id, obj.req.clientIp, obj.peer.req.clientIp, Math.floor((Date.now() - ws.time) / 1000)],
                msg:
                  msg +
                  ' \"' +
                  obj.id +
                  ...
                  obj.req.clientIp +
                  ' to ' +
                  obj.peer.req.clientIp +
                  ', ' +
                  Math.floor((Date.now() - ws.time) / 1000) +
                  ' second(s)',
                protocol: obj.req.query.p,
                nodeid: obj.req.query.nodeid,
                bytesin: inTraffc,
                bytesout: outTraffc
              };
              if (obj.guestname) {
                event.guestname = obj.guestname;
              } else if (peer.guestname) {
                event.guestname = peer.guestname;
              } // If this is a sharing session, set the guest name here.
          ...
          delete peer.nodeid;
          delete peer.meshid;
          if (peer.pingtimer != null) {
            clearInterval(peer.pingtimer);
            delete peer.pingtimer;
          }
          if (peer.pongtimer != null) {
            clearInterval(peer.pongtimer);
            delete peer.pongtimer;
          }
        } else {
          ...
          setTimeout(function () {
            ...
            recordingEntry(
              ...
              function (logfile, tag) {
                ...
                if (domain.sessionrecording.index && domain.sessionrecording.index !== false) {
                  parent.parent.certificateOperations.acceleratorPerformOperation('indexMcRec', tag.logfile.filename);
                }

                // Compute session length
                var sessionLength = null;
                if (tag.logfile.startTime != null) {
                  sessionLength = Math.round((Date.now() - tag.logfile.startTime) / 1000) - 5;
                }
                ...
                if (xprotocol == null && logfile.text == 2) {
                  xprotocol = 200;
                }
                if (xprotocol != null) {
                ...
                if (tag.logfile.startTime) {
                  event.startTime = tag.logfile.startTime;
                  event.lengthTime = sessionLength;
                }
                if (tag.logfile.name) {
                  event.name = tag.logfile.name;
                }
                if (tag.logfile.icon) {
                  event.icon = tag.logfile.icon;
                }
  ...
  if (obj.authenticated) {
    ...
    if (cookie != null && cookie.nodeid != null && cookie.tcpport != null && cookie.domainid != null) {
      ...
      parent.db.Get(cookie.nodeid, function (err, docs) {
        ...
        if (obj.nouser !== true && (parent.GetNodeRights(obj.user, node.meshid, node._id) & 0x00200008) == 0) {
          console.log('ERR: Access denied (1)');
          try {
            obj.close();
          } catch (ex) {}
          return;
        ...
        const rcookieData = {};
        if (user != null) {
          rcookieData.ruserid = user._id;
        } else if (obj.nouser === true) {
          rcookieData.nouser = 1;
        }
        ...
        } // If there is no connection id, generate one.
        const command = {
          ...
          value:
            ...
            xdomain +
            'meshrelay.ashx?' +
            (obj.req.query.p != null ? 'p=' + obj.req.query.p + '&' : '') +
            'id=' +
            obj.id +
            '&rauth=' +
            rcookie,
          tcpport: cookie.tcpport,
          tcpaddr: cookie.tcpaddr,
          soptions: {}
        };
        if (user) {
        ...
        if (obj.sendAgentMessage(command, user ? user._id : null, cookie.domainid) == false) {
          delete obj.id;
          parent.parent.debug('relay', 'Relay: Unable to contact this agent (' + obj.req.clientIp + ')');
        }
        performRelay();
      });
      return obj;
      ...
      parent.db.Get(obj.req.query.nodeid, function (err, docs) {
        ...
        // Check if this user has permission to relay thru this computer (MESHRIGHT_REMOTECONTROL or MESHRIGHT_RELAY rights)
        if ((parent.GetNodeRights(obj.user, node.meshid, node._id) & 0x00200008) == 0) {
          console.log('ERR: Access denied (2)');
          try {
            obj.close();
          } catch (ex) {}
          return;
        ...
        if (obj.req.query.tcpport != null) {
          const command = {
            ...
            value:
              ...
              '&rauth=' +
              rcookie,
            tcpport: obj.req.query.tcpport,
            tcpaddr: obj.req.query.tcpaddr == null ? '127.0.0.1' : obj.req.query.tcpaddr,
            soptions: {}
          };
          if (typeof domain.consentmessages == 'object') {
            if (typeof domain.consentmessages.title == 'string') {
          ...
          if (obj.sendAgentMessage(command, user._id, domain.id) == false) {
            delete obj.id;
            parent.parent.debug('relay', 'Relay: Unable to contact this agent (' + obj.req.clientIp + ')');
          }
          ...
          const command = {
            ...
            value:
              ...
              '&rauth=' +
              rcookie,
            udpport: obj.req.query.udpport,
            udpaddr: obj.req.query.udpaddr == null ? '127.0.0.1' : obj.req.query.udpaddr,
            soptions: {}
          };
          if (typeof domain.consentmessages == 'object') {
            if (typeof domain.consentmessages.title == 'string') {
      ...
      cookie != null &&
      cookie.nid != null &&
      typeof cookie.r == 'number' &&
      typeof cookie.p == 'number' &&
      typeof cookie.cf == 'number' &&
      typeof cookie.gn == 'string'
    ) {
      // We have routing instructions in the cookie, but first, check user access for this node.
      parent.db.Get(cookie.nid, function (err, docs) {
        ...
        const command = {
          ...
          guestname: cookie.gn,
          consent: cookie.cf,
          remoteaddr: cleanRemoteAddr(obj.req.clientIp)
        };
        ...
        if (usages.indexOf(parseInt(obj.req.query.p)) < 0) {
          console.log('ERR: Invalid protocol usage');
          try {
            obj.close();
          } catch (e) {}
          return;
        ...
        parent.parent.debug('relay', 'Relay: Sending agent tunnel command: ' + JSON.stringify(command));
        if (obj.sendAgentMessage(command, user ? user._id : null, domain.id) == false) {
          delete obj.id;
          parent.parent.debug('relay', 'Relay: Unable to contact this agent (' + obj.req.clientIp + ')');
        }
      ...
      if (obj.req.query.nodeid != null && obj.req.query.nodeid.startsWith('node/')) {
        ...
        if (nodeSplit.length != 3 || nodeSplit[1] != domain.id) {
          console.log('ERR: Invalid NodeID');
          try {
            obj.close();
          } catch (e) {}
          return;
  ...
  function cleanUpRecordings() {
    if (
      ...
      (typeof domain.sessionrecording.maxrecordings == 'number' ||
        typeof domain.sessionrecording.maxrecordingsizemegabytes == 'number' ||
        typeof domain.sessionrecording.maxrecordingdays == 'number')
    ) {
      parent.cleanUpRecordingsActive = true;
      setTimeout(function () {
        var recPath = null,
          ...
          now = Date.now();
        if (domain.sessionrecording.filepath) {
          recPath = domain.sessionrecording.filepath;
        } else {
          recPath = parent.parent.recordpath;
        }
        fs.readdir(recPath, function (err, files) {
          ...
          for (var i in files) {
            if (files[i].endsWith('.mcrec')) {
              ...
              if (j > 0) {
                var stats = null;
                try {
                  stats = fs.statSync(parent.parent.path.join(recPath, files[i]));
                } catch (ex) {}
                if (stats != null) {
                  recfiles.push({ n: files[i], r: files[i].substring(j + 1), s: stats.size, t: stats.mtimeMs });
                }
              }
            }
          }
          ...
          for (var i in recfiles) {
            ...
            if (
              typeof domain.sessionrecording.maxrecordings == 'number' &&
              domain.sessionrecording.maxrecordings > 0 &&
              totalFiles >= domain.sessionrecording.maxrecordings
            ) {
              overQuota = true;
            } else if (
              typeof domain.sessionrecording.maxrecordingsizemegabytes == 'number' &&
              domain.sessionrecording.maxrecordingsizemegabytes > 0 &&
              totalSize >= domain.sessionrecording.maxrecordingsizemegabytes * 1048576
            ) {
              overQuota = true;
            } else if (
              typeof domain.sessionrecording.maxrecordingdays == 'number' &&
              domain.sessionrecording.maxrecordingdays > 0 &&
              (now - recfiles[i].t) / 1000 / 60 / 60 / 24 >= domain.sessionrecording.maxrecordingdays
            ) {
              overQuota = true;
            }
            if (overQuota) {
              fs.unlinkSync(parent.parent.path.join(recPath, recfiles[i].n));
            }
            totalFiles++;
...
function CreateLocalRelayEx(parent, ws, req, domain, user, cookie) {
  ...
  if (typeof protocolInUse != 'number') {
    protocolInUse = 0;
  }
  ...
  if (obj.user == null) {
    try {
      ws.close();
      parent.parent.debug('relay', 'LocalRelay: Connection with no authentication');
    } catch (e) {
      console.log(e);
    }
    return;
  ...
  if (cookie != null) {
    if (cookie.nodeid) {
      req.query.nodeid = cookie.nodeid;
    }
    if (cookie.tcpport) {
  ...
  if (req.query == null || req.query.nodeid == null || req.query.tcpport == null) {
    try {
      ws.close();
      parent.parent.debug('relay', 'LocalRelay: Connection with invalid arguments');
    } catch (e) {
      console.log(e);
    }
    return;
  ...
  if (
    nodeidsplit.length != 3 ||
    nodeidsplit[0] != 'node' ||
    nodeidsplit[1] != domain.id ||
    nodeidsplit[2].length < 10
  ) {
    try {
      ws.close();
      parent.parent.debug('relay', 'LocalRelay: Connection with invalid arguments');
  ...
  function dataAccounting() {
    ...
    const dataout =
      obj.client.bytesWritten - obj.client.bytesWrittenEx + (ws._socket.bytesWritten - ws._socket.bytesWrittenEx);
    ...
    // Add to counters
    if (parent.trafficStats.localRelayIn[protocolInUse]) {
      parent.trafficStats.localRelayIn[protocolInUse] += datain;
    } else {
      parent.trafficStats.localRelayIn[protocolInUse] = datain;
    }
    if (parent.trafficStats.localRelayOut[protocolInUse]) {
      parent.trafficStats.localRelayOut[protocolInUse] += dataout;
    } else {
      parent.trafficStats.localRelayOut[protocolInUse] = dataout;
    }
  }
  ...
  obj.close = function (arg) {
    ...
    var inTraffc = obj.ws._socket.bytesRead,
      outTraffc = obj.ws._socket.bytesWritten;
    if (obj.client != null) {
      inTraffc += obj.client.bytesRead;
      outTraffc += obj.client.bytesWritten;
    }
    ...
    if (arg == 1 || arg == null) {
      try {
        obj.ws.close();
        parent.parent.debug('relay', 'LocalRelay: Soft disconnect');
      } catch (e) {
        console.log(e);
      }
    } // Soft close, close the websocket
    if (arg == 2) {
      try {
        ...
        parent.parent.debug('relay', 'LocalRelay: Hard disconnect');
      } catch (e) {
        console.log(e);
      }
    } // Hard close, close the TCP socket
    ...
    if (obj.time) {
      ...
      if (req.query.p == 10) {
        protocolStr = 'RDP';
      } else if (req.query.p == 11) {
        protocolStr = 'SSH-TERM';
      } else if (req.query.p == 12) {
        protocolStr = 'VNC';
      } else if (req.query.p == 13) {
        protocolStr = 'SSH-FILES';
      } else if (req.query.p == 14) {
        protocolStr = 'Web-TCP';
      }
      var event = {
        etype: 'relay',
        ...
        username: obj.user.name,
        msgid: 121,
        msgArgs: [obj.id, protocolStr, obj.host, Math.floor((Date.now() - obj.time) / 1000)],
        msg:
          'Ended local relay session \"' +
          obj.id +
          '\", protocol ' +
          protocolStr +
          ' to ' +
          obj.host +
          ', ' +
          Math.floor((Date.now() - obj.time) / 1000) +
          ' second(s)',
        nodeid: obj.req.query.nodeid,
        protocol: req.query.p,
        in: inTraffc,
    ...
    parent.db.Get(obj.nodeid, function (err, docs) {
      ...
      obj.client.connect(obj.tcpport, node.host, function () {
        ...
        var event = {
          ...
          username: obj.user.name,
          msgid: 120,
          msgArgs: [obj.id, protocolStr, obj.host],
          msg: 'Started local relay session \"' + obj.id + '\", protocol ' + protocolStr + ' to ' + obj.host,
          nodeid: req.query.nodeid,
          protocol: req.query.p
        };
        if (obj.guestname) {
        ...
        // Count the session
        if (parent.trafficStats.localRelayCount[protocolInUse]) {
          parent.trafficStats.localRelayCount[protocolInUse] += 1;
        } else {
          parent.trafficStats.localRelayCount[protocolInUse] = 1;
        }
      ...
      obj.client.on('data', function (data) {
        ...
        try {
          this.pause();
          ws.send(data, this.clientResume);
        } catch (ex) {
          console.log(ex);
        }
      });
      obj.client.on('close', function () {
        obj.close();
      });
      obj.client.on('error', function (err) {
        obj.close();
      });
      obj.client.clientResume = function () {
        try {
          obj.client.resume();
        } catch (ex) {
          console.log(ex);
  ...
  ws.on('message', function (data) {
    if (typeof data != 'string') {
      try {
        ws._socket.pause();
        obj.client.write(data, ws.flushSink);
      } catch (ex) {}
    }
  }); // Perform relay
