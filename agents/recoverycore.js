...
function bsd_execv(name, agentfilename, sessionid) {
  ...
  try {
    ...
    if (sessionid != null) {
      sendConsoleText('Self Update failed: ' + e.toString(), sessionid);
    }
...
function windows_execve(name, agentfilename, sessionid) {
  ...
  if (name == null) {
    ...
    var updatesource = parms.find(function (v) {
      return v.startsWith('--updateSourcePath=');
    });
    if (updatesource == null) {
      parms.push('--updateSourcePath="' + cwd + agentfilename + '"');
      updatesource = (cwd + agentfilename).split('.exe');
      updatesource.pop();
      updatesource = updatesource.join('.exe');
      db = updatesource + '.db';
      updatesource =
        ' & move "' +
        updatedb +
        ...
        db +
        '"' +
        (' & erase "' + updatedb + '" & move "' + update + '" "' + updatesource + '.exe"');
    } else {
      ...
      updatesource =
        ...
        updatedb +
        '"' +
        (' & echo move "' + update + '" "' + updatesource + '.exe" & echo move "' + updatedb + '" "' + db + '"');
    }
  ...
  if (agentUpdate_Start._selfupdate != null) {
    ...
    if (require('MeshAgent').ARCHID == null && updateurl == null) {
      ...
      if (process.platform == 'win32') {
        ...
        if (
          parms.findIndex(function (val) {
            return val != null && (val.toUpperCase() == 'RUN' || val.toUpperCase() == 'CONNECT');
          }) >= 0
        ) {
          ...
          if (_getPotentialServiceNames().length > 0) {
            sendConsoleText(
              'Self update cannot continue because the installed agent (' +
                agents[0] +
                ') conflicts with the currently running Temp/Console agent...',
              sessionid
            );
            return;
          }
        ...
        try {
          ...
          if (!s.isMe()) {
            ...
            sendConsoleText(
              'Self Update cannot continue, this agent is not an instance of background service (' + name + ')',
              sessionid
            );
            return;
          }
        ...
        this._filehash.on('hash', function (h) {
          if (updateoptions != null && updateoptions.hash != null) {
            if (updateoptions.hash.toLowerCase() == h.toString('hex').toLowerCase()) {
              ...
              if (needStreamFix) {
                sendConsoleText(
                  'This is an older agent that may have an httpstream bug. On next retry will try to fetch the update differently...'
                );
                needStreamFix = false;
              }
            ...
            if (data.value != null) {
              ...
              if (xurl != null) {
                ...
                woptions.checkServerIdentity = function checkServerIdentity(certs) {
                  ...
                  try {
                    if (require('MeshAgent').ServerInfo.ControlChannelCertificate.fingerprint == certs[0].fingerprint)
                      return;
                  } catch (ex) {}
                ...
                tunnel.on('upgrade', function (response, s, head) {
                  ...
                  s.on('data', function (data) {
                    ...
                    if (this.httprequest.uploadFile && typeof data == 'object' && data[0] != 123) {
                      ...
                      this.write(
                        Buffer.from(JSON.stringify({ action: 'uploadack', reqid: this.httprequest.uploadFileid }))
                      ); // Ask for more data.
                      return;
                    }
                    ...
                    if (this.httprequest.state == 0) {
                      ...
                      if (this.httprequest.protocol == 0) {
                        ...
                        if (this.httprequest.protocol == 10) {
                          ...
                          try {
                            if (stats) {
                              this.httprequest.downloadFile = fs.createReadStream(this.httprequest.xoptions.file, {
                                flags: 'rbN'
                              });
                            }
                          } catch (e) {}
                          ...
                          if (process.platform == 'win32') {
                            ...
                            if (require('win-virtual-terminal').supported) {
                              ...
                              this.httprequest._dispatcher = require('win-dispatcher').dispatch({
                                modules: [
                                  { name: 'win-virtual-terminal', script: getJSModule('win-virtual-terminal') }
                                ],
                                launch: { module: 'win-virtual-terminal', method: 'Start', args: [cols, rows] }
                              });
                            ...
                            if (process.platform == 'linux') {
                              this.httprequest.process.stdin.write(" alias ls='ls --color=auto';clear\n");
                            }
                            this.httprequest.process.tunnel = this;
                            this.httprequest.process.on('exit', function (ecode, sig) {
                              this.tunnel.end();
                            });
                        ...
                        switch (cmd.action) {
                          ...
                          case 'rm': {
                            ...
                            for (var i in cmd.delfiles) {
                              try {
                                deleteFolderRecursive(path.join(cmd.path, cmd.delfiles[i]), cmd.rec);
                              } catch (e) {}
                            }
                            break;
                          }
                          ...
                          case 'findfile': {
                            ...
                            r.then(function () {
                              try {
                                this.socket.write(
                                  Buffer.from(JSON.stringify({ action: 'findfile', r: null, reqid: this.socket.reqid }))
                                );
                              } catch (ex) {}
                            });
                            break;
                          ...
                          case 'upload': {
                            ...
                            if (this.httprequest.uploadFile) {
                              this.write(
                                Buffer.from(
                                  JSON.stringify({ action: 'uploadstart', reqid: this.httprequest.uploadFileid })
                                )
                              );
                            }
                            break;
                          ...
                          case 'uploaddone': {
                            ...
                            if (this.httprequest.uploadFile) {
                              ...
                              this.write(
                                Buffer.from(
                                  JSON.stringify({ action: 'uploaddone', reqid: this.httprequest.uploadFileid })
                                )
                              ); // Indicate that we closed the file.
                              delete this.httprequest.uploadFile;
                              delete this.httprequest.uploadFileid;
                              delete this.httprequest.uploadFilePath;
                            }
                          ...
                          case 'uploadcancel': {
                            ...
                            if (this.httprequest.uploadFile) {
                              ...
                              this.write(
                                Buffer.from(
                                  JSON.stringify({ action: 'uploadcancel', reqid: this.httprequest.uploadFileid })
                                )
                              ); // Indicate that we closed the file.
                              delete this.httprequest.uploadFile;
                              delete this.httprequest.uploadFileid;
                ...
                tunnel.onerror = function (e) {
                  sendConsoleText('ERROR: ' + JSON.stringify(e));
                };
...
function processConsoleCommand(cmd, args, rights, sessionid) {
  try {
    ...
    switch (cmd) {
      ...
      case 'help':
        response =
          'Available commands are: agentupdate, agentupdateex, dbkeys, dbget, dbset, dbcompact, eval, netinfo, osinfo, setdebug, versions.';
        break;
      case '_descriptors':
      ...
      case 'tunnels': {
        ...
        for (var i in tunnels) {
          response += 'Tunnel #' + i + ', ' + tunnels[i].url + '\r\n';
        }
        if (response == '') {
...
function getDirectoryInfo(reqpath) {
  ...
  if ((reqpath == undefined || reqpath == '') && process.platform == 'win32') {
    ...
    } catch (e) {} // TODO: Anyway to get drive total size and free space? Could draw a progress bar.
    if (results != null) {
      for (var i = 0; i < results.length; ++i) {
        var drive = { n: results[i].name, t: 1 };
        if (results[i].type == 'REMOVABLE') {
          drive.dt = 'removable';
        } // TODO: See if this is USB/CDROM or something else, we can draw icons.
    ...
    if (results != null) {
      for (var i = 0; i < results.length; ++i) {
        if (results[i] != '.' && results[i] != '..') {
          var stat = null,
            p = path.join(reqpath, results[i]);
          //if (process.platform == "win32") { p = p.split('/').join('\\'); }
          try {
            stat = fs.statSync(p);
          } catch (e) {} // TODO: Get file size/date
