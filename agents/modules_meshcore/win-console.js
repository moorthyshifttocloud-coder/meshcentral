  ...
  if (process.platform == 'win32') {
    ...
    this._loadicon = function (imagePath) {
      var h = this._user32.LoadImageA(
        ...
        0,
        0,
        0x00000010 | 0x00008000 | 0x00000040
    ...
    this.SetTrayIcon = function SetTrayIcon(options) {
      ...
      data
        .Deref(this._Marshal.PointerSize == 4 ? 16 : 24, 4)
        .toBuffer()
        .writeUInt32LE(options.filter);

      if (!options.noBalloon) {
        trayType |= TrayIconFlags.NIF_INFO;
      }
      ...
      data
        .Deref(this._Marshal.PointerSize * 2, 4)
        .toBuffer()
        .writeUInt32LE(1);
      data
        .Deref(this._Marshal.PointerSize == 4 ? 12 : 20, 4)
        .toBuffer()
        .writeUInt32LE(trayType);
      data
        .Deref(this._Marshal.PointerSize == 4 ? 416 : 432, 4)
        .toBuffer()
        .writeUInt32LE(NOTIFYICON_VERSION_4);
      ...
      if (options.szTip) {
        Buffer.from(options.szTip).copy(szTip.toBuffer());
      }
      if (options.szInfo) {
        Buffer.from(options.szInfo).copy(szInfo.toBuffer());
      }
      if (options.szInfoTitle) {
        Buffer.from(options.szInfoTitle).copy(szInfoTitle.toBuffer());
      }
      ...
      retVal.MessagePump.on('hwnd', function onHwnd(h) {
        ...
        options.hwnd = h;
        h.pointerBuffer().copy(
          this.NotifyData.Deref(
            this.WindowsConsole._Marshal.PointerSize,
            this.WindowsConsole._Marshal.PointerSize
          ).toBuffer()
        );
