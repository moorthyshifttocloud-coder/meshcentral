...
function AmtManager(agent, db, isdebug) {
  var sendConsole = function (msg) {
    agent.SendCommand({ action: 'msg', type: 'console', value: msg });
  };
  var debug = function (msg) {
    if (isdebug) {
      sendConsole('amt-manager: ' + msg + '<br />');
    }
  };
  var amtMei = null,
    amtMeiState = 0;
  ...
  require('events').EventEmitter.call(obj, true).createEvent('stateChange_LMS').createEvent('portBinding_LMS');
  obj._lmsstate = 0;
  obj._mapping = [];
  ...
  obj.reset = function () {
    ...
    try {
      ...
      amtMei.on('error', function (e) {
        debug('MEI error');
        amtMei = null;
        amtMeiState = -1;
        obj.state = -1;
        if (obj.onStateChange != null) {
          obj.onStateChange(amtMeiState);
        }
      });
      amtMei.getVersion(function (result) {
        if (result == null) {
          ...
          if (rebindToMeiRetrys < 10) {
            setTimeout(obj.reset, 10000);
          }
        } else {
  ...
  obj.getMeiState = function (flags, func) {
    ...
    try {
      ...
      if (getMeiStateCache.MeiVersion != null) {
        amtMeiTmpState.MeiVersion = getMeiStateCache.MeiVersion;
      } else {
        amtMei.getProtocolVersion(function (result) {
          if (result != null) {
            getMeiStateCache.MeiVersion = amtMeiTmpState.MeiVersion = result;
          }
        });
      }
      if ((flags & 1) != 0) {
        if (getMeiStateCache.Versions != null) {
          ...
          amtMei.getVersion(function (result) {
            if (result) {
              getMeiStateCache.Versions = amtMeiTmpState.Versions = {};
              for (var version in result.Versions) {
                amtMeiTmpState.Versions[result.Versions[version].Description] = result.Versions[version].Version;
              }
            }
          });
        }
      }
      amtMei.getProvisioningMode(function (result) {
        if (result) {
          amtMeiTmpState.ProvisioningMode = result.mode;
        }
      });
      amtMei.getProvisioningState(function (result) {
        if (result) {
          amtMeiTmpState.ProvisioningState = result.state;
          if (result.state != 2) {
            amtMei.stopConfiguration(function () {});
          }
        }
      }); // 0: "Not Activated (Pre)", 1: "Not Activated (In)", 2: "Activated". Make sure to stop remote configuration if needed.
      amtMei.getEHBCState(function (result) {
        if (result != null && result.EHBC == true) {
          amtMeiTmpState.Flags += 1;
        }
      });
      amtMei.getControlMode(function (result) {
        if (result != null) {
          if (result.controlMode == 1) {
            amtMeiTmpState.Flags += 2;
          }
          if (result.controlMode == 2) {
            amtMeiTmpState.Flags += 4;
          }
        }
      }); // Flag 2 = CCM, 4 = ACM
      ...
      if ((flags & 8) != 0) {
        amtMei.getLanInterfaceSettings(0, function (result) {
          if (result) {
            ...
            var fqdn = null,
              interfaces = require('os').networkInterfaces(); // Look for the DNS suffix for the Intel AMT Ethernet interface
            for (var i in interfaces) {
              for (var j in interfaces[i]) {
                if (
                  interfaces[i][j].mac == result.mac &&
                  interfaces[i][j].fqdn != null &&
                  interfaces[i][j].fqdn != ''
                ) {
                  amtMeiTmpState.OsDnsSuffix = interfaces[i][j].fqdn;
                }
              }
            }
          }
        });
        amtMei.getLanInterfaceSettings(1, function (result) {
          if (result) {
            amtMeiTmpState.net1 = result;
          }
        });
      }
      if (getMeiStateCache.UUID != null) {
        amtMeiTmpState.UUID = getMeiStateCache.UUID;
      } else {
        amtMei.getUuid(function (result) {
          if (result != null && result.uuid != null) {
            getMeiStateCache.UUID = amtMeiTmpState.UUID = result.uuid;
          }
        });
      }
      if ((flags & 2) != 0) {
        amtMei.getLocalSystemAccount(function (x) {
          if (x != null && x.user && x.pass) {
            amtMeiTmpState.OsAdmin = { user: x.user, pass: x.pass };
          }
        });
      }
      amtMei.getDnsSuffix(function (result) {
        if (result != null) {
          amtMeiTmpState.DnsSuffix = result;
        }
        if ((flags & 4) == 0) {
          if (func != null) {
            func(amtMeiTmpState);
      ...
      if ((flags & 4) != 0) {
        amtMei.getHashHandles(function (handles) {
          if (handles != null && handles.length > 0) {
            amtMeiTmpState.Hashes = [];
          } else {
            func(amtMeiTmpState);
          }
          var exitOnCount = handles.length;
          for (var i = 0; i < handles.length; ++i) {
            this.getCertHashEntry(handles[i], function (hashresult) {
              amtMeiTmpState.Hashes.push(hashresult);
              if (--exitOnCount == 0) {
                if (func != null) {
                  func(amtMeiTmpState);
                }
  ...
  var handleAmtNotification = function (notifyMsg) {
    if (
      notifyMsg == null ||
      notifyMsg.Body == null ||
      notifyMsg.Body.MessageID == null ||
      notifyMsg.Body.MessageArguments == null
    )
      return null;
    var amtMessage = notifyMsg.Body.MessageID,
      amtMessageArg = notifyMsg.Body.MessageArguments[0],
      notify = null;
    ...
    if (notify != null) {
      agent.SendCommand({ action: 'msg', type: 'notify', value: notify, tag: 'general', amtMessage: amtMessage });
    }
  };
  ...
  obj.lmsreset = function () {
    ...
    try {
      ...
      amtLms.on('notify', function (data, options, code) {
