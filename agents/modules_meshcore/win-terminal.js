...
si.Deref(0, 4)
  .toBuffer()
  .writeUInt32LE(GM.PointerSize == 4 ? 68 : 104); // si.cb
si.Deref(GM.PointerSize == 4 ? 48 : 64, 2)
  .toBuffer()
  .writeUInt16LE(SW_HIDE | SW_MINIMIZE); // si.wShowWindow
si.Deref(GM.PointerSize == 4 ? 44 : 60, 4)
  .toBuffer()
  .writeUInt32LE(STARTF_USESHOWWINDOW); // si.dwFlags;
...
function windows_terminal() {
  ...
  this.SendCursorUpdate = function () {
    ...
    if (this._kernel32.GetConsoleScreenBufferInfo(this._stdoutput, newCsbi).Val == 0) {
      return;
    }
    if (
      newCsbi.Deref(4, 2).toBuffer().readUInt16LE() != this.currentX ||
      newCsbi.Deref(6, 2).toBuffer().readUInt16LE() != this.currentY
    ) {
      //
  ...
  this.ClearScreen = function () {
    ...
    if (this._kernel32.GetConsoleScreenBufferInfo(this._stdoutput, CONSOLE_SCREEN_BUFFER_INFO).Val == 0) {
      return;
    }

    var coordScreen = GM.CreateVariable(4);
    var dwConSize =
      CONSOLE_SCREEN_BUFFER_INFO.Deref(0, 2).toBuffer().readUInt16LE(0) *
      CONSOLE_SCREEN_BUFFER_INFO.Deref(2, 2).toBuffer().readUInt16LE(0);
    var cCharsWritten = GM.CreateVariable(4);

    // Fill the entire screen with blanks.
    if (
      this._kernel32.FillConsoleOutputCharacterA(
        this._stdoutput,
        32,
        dwConSize,
        coordScreen.Deref(0, 4).toBuffer().readUInt32LE(),
        cCharsWritten
      ).Val == 0
    ) {
      return;
    ...
    if (
      this._kernel32.FillConsoleOutputAttribute(
        this._stdoutput,
        CONSOLE_SCREEN_BUFFER_INFO.Deref(8, 2).toBuffer().readUInt16LE(0),
        dwConSize,
        coordScreen.Deref(0, 4).toBuffer().readUInt32LE(),
        cCharsWritten
      ).Val == 0
    ...
    rect
      .Deref(4, 2)
      .toBuffer()
      .writeUInt16LE(srWindow.readUInt16LE(4) - srWindow.readUInt16LE(0));
    rect
      .Deref(6, 2)
      .toBuffer()
      .writeUInt16LE(srWindow.readUInt16LE(6) - srWindow.readUInt16LE(2));
  ...
  this.StartEx = function Start(CONSOLE_SCREEN_WIDTH, CONSOLE_SCREEN_HEIGHT, terminalTarget) {
    ...
    rect
      .Deref(4, 2)
      .toBuffer()
      .writeUInt16LE(CONSOLE_SCREEN_WIDTH - 1);
    rect
      .Deref(6, 2)
      .toBuffer()
      .writeUInt16LE(CONSOLE_SCREEN_HEIGHT - 1);
    ...
    //
    if (
      this._kernel32.SetConsoleScreenBufferSize(this._stdoutput, coordScreen.Deref(0, 4).toBuffer().readUInt32LE())
        .Val == 0
    ) {
    ...
    this._stream._promise = new promise(function (res, rej) {
      this._res = res;
      this._rej = rej;
    });
  ...
  this._stop = function () {
    ...
    this.stopping = new promise(function (res, rej) {
      this._res = res;
      this._rej = rej;
    });
  ...
  this._hookThread = function () {
    ...
    var p = this._user32.SetWinEventHook.async(
      EVENT_CONSOLE_CARET,
      EVENT_CONSOLE_END_APPLICATION,
      0,
      this._ConsoleWinEventProc,
      0,
      0,
    ...
    //
    this._ConsoleWinEventProc.on(
      'GlobalCallback',
      function (hhook, dwEvent, hwnd, idObject, idChild, idEventThread, swmsEventTime) {
        //
        ...
        //
        if (!this.terminal.hwinEventHook || this.terminal.hwinEventHook.Val != hhook.Val) {
          return;
        }
        var buffer = null;
        ...
        switch (dwEvent.Val) {
          ...
          case EVENT_CONSOLE_UPDATE_REGION:
            ...
            if (this.terminal._scrollTimer == null) {
              buffer = this.terminal._GetScreenBuffer(
                LOWORD(idObject.Val),
                HIWORD(idObject.Val),
                LOWORD(idChild.Val),
                HIWORD(idChild.Val)
          ...
          case EVENT_CONSOLE_UPDATE_SIMPLE:
            ...
            var simplebuffer = {
              data: [Buffer.alloc(1, LOWORD(idChild.Val))],
              attributes: [HIWORD(idChild.Val)],
              width: 1,
              height: 1,
              x: LOWORD(idObject.Val),
              y: HIWORD(idObject.Val)
            };
          ...
          case EVENT_CONSOLE_END_APPLICATION:
            ...
            if (idObject.Val == this.terminal._hProcessID) {
              ...
              this.terminal._stop().then(function () {
                console.log('STOPPED');
              });
            }
            break;
  ...
  this._GetMessage = function () {
    ...
    if (this._user32.abort) {
      console.log('aborting loop');
      return;
    }
    this._user32.GetMessageA.async(this._user32.SetWinEventHook.async, MSG, 0, 0, 0).then(
      function (ret) {
        ...
        if (ret.Val != 0) {
          if (ret.Val == -1) {
            ...
            this.nativeProxy._user32.TranslateMessage.async(this.nativeProxy.user32.SetWinEventHook.async, MSG).then(
              function () {
                // Dispatches a message to a window procedure
                //console.log('DispatchMessage');
                this.nativeProxy._user32.DispatchMessageA.async(
                  this.nativeProxy.user32.SetWinEventHook.async,
                  MSG
                ).then(function () {
          ...
          this.nativeProxy.UnhookWinEvent.async(
            this.nativeProxy.terminal._user32.SetWinEventHook.async,
            this.nativeProxy.terminal.hwinEventHook
          ).then(
            function () {
              if (this.nativeProxy.terminal._hProcess == null) {
                return;
              }

              this.nativeProxy.terminal.stopping._res();
              if (
                this.nativeProxy.terminal._kernel32.TerminateProcess(this.nativeProxy.terminal._hProcess, 1067).Val == 0
              ) {
  ...
  this._WriteCharacter = function (key, bControlKey) {
    ...
    rec
      .Deref(12, 2)
      .toBuffer()
      .writeUInt16LE(this._user32.MapVirtualKeyA(this._user32.VkKeyScanA(key).Val, MAPVK_VK_TO_VSC).Val);

    var dwWritten = GM.CreateVariable(4);
    if (this._kernel32.WriteConsoleInputA(this._stdinput, rec, 1, dwWritten).Val == 0) {
      return false;
    }
  ...
  this._GetScreenBuffer = function (sx, sy, ex, ey) {
    ...
    if (this._kernel32.GetConsoleScreenBufferInfo(this._stdoutput, info).Val == 0) {
      throw 'Error getting screen buffer info';
    }
    ...
    size
      .Deref(0, 2)
      .toBuffer()
      .writeUInt16LE(ex - sx + 1, 0);
    size
      .Deref(2, 2)
      .toBuffer()
      .writeUInt16LE(ey - sy + 1, 0);
    ...
    if (
      this._kernel32.ReadConsoleOutputA(
        this._stdoutput,
        nBuffer,
        size.Deref(0, 4).toBuffer().readUInt32LE(),
        startCoord.Deref(0, 4).toBuffer().readUInt32LE(),
        region
      ).Val == 0
    ) {
  ...
  this._SendScroll = function _SendScroll(dx, dy) {
    ...
    if (this._scrollTimer || this._stream == null) {
      return;
    }
  ...
  this.StartCommand = function StartCommand(target) {
    if (
      this._kernel32.CreateProcessA(GM.CreateVariable(target), 0, 0, 0, 1, CREATE_NEW_PROCESS_GROUP, 0, 0, si, pi)
        .Val == 0
    ) {
    ...
    this._hProcessID = pi
      .Deref(GM.PointerSize == 4 ? 8 : 16, 4)
      .toBuffer()
      .readUInt32LE(); // pi.dwProcessId
...
function GetEsc(op, args) {
  return Buffer.from('\x1B[' + args.join(';') + op);
}
function MeshConsole(msg) {
  require('MeshAgent').SendCommand({ action: 'msg', type: 'console', value: JSON.stringify(msg) });
}
function TranslateLine(x, y, data, attributes) {
  ...
  } // If we get a single attribute, turn it into an array.
  ...
  for (i = 0; i < data.length; i++) {
    if (lastAttr != attributes[i]) {
      ...
      if (fb != fbright) {
        esc.push(2 - fb);
        fbright = fb;
      } // Set the bright foreground color if needed
      if (bb != bbright) {
        if (bb == 0) {
          esc.push(bcolor + 40);
        } else {
      ...
      } // Set bright Background color if needed

      if (esc.length > 0) {
        output.push(GetEsc('m', esc));
        esc = [];
      }
