...
function linux_identifiers() {
  ...
  if (!require('fs').existsSync('/sys/class/dmi/id')) {
    if (require('fs').existsSync('/sys/firmware/devicetree/base/model')) {
      if (require('fs').readFileSync('/sys/firmware/devicetree/base/model').toString().trim().startsWith('Raspberry')) {
        ...
        identifiers['board_serial'] = require('fs')
          .readFileSync('/sys/firmware/devicetree/base/serial-number')
          .toString()
          .trim();
        const memorySlots = [];
    ...
    for (var i in entries) {
      if (
        require('fs')
          .statSync('/sys/class/dmi/id/' + entries[i])
          .isFile()
      ) {
        try {
          ret[entries[i]] = require('fs')
            .readFileSync('/sys/class/dmi/id/' + entries[i])
            .toString()
            .trim();
        } catch (z) {}
        if (ret[entries[i]] == 'None') {
          delete ret[entries[i]];
        }
      }
    }
    entries = null;
  ...
  try {
    ...
    try {
      uefiExist = require('fs').existsSync('/sys/firmware/efi');
    } catch (ex) {
      uefiExist = false;
    }

    try {
      assumePi = require('fs').existsSync('/sys/firmware/devicetree/base/model');
    } catch (ex) {
      assumePi = false;
    }
    ...
    identifiers['bios_mode'] = 'Legacy / Unknown';
  }
  ...
  try {
    ...
    if (identifiers['cpu_name'] == '') {
      // CPU BLANK, check lscpu instead
      child = require('child_process').execFile('/bin/sh', ['sh']);
      child.stdout.str = '';
      child.stdout.on('data', dataHandler);
  ...
  child.stdin.write(
    "lspci | grep ' VGA ' | tr '\\n' '`' | awk '{ a=split($0,lines" +
      ',"`"); printf "["; for(i=1;i<a;++i) { split(lines[i],gpu,"r: "); printf "%s\\"%s\\"", (i==1?"":","),gpu[2]; } printf "]"; }\'\nexit\n'
  );
  child.waitExit();
  try {
    identifiers['gpu_name'] = JSON.parse(child.stdout.str.trim());
  } catch (xx) {}
  ...
  child.stdout.on('data', dataHandler);
  child.stdin.write(
    'df -T -x tmpfs -x devtmpfs -x efivarfs | awk \'NR==1 || $1 ~ ".+"{print $3, $4, $5, $7, $2}\' | awk \'NR>1 {printf "{\\"size\\":\\"%s\\",\\"used\\":\\"%s\\",\\"available\\":\\"%s\\",\\"mount_point\\":\\"%s\\",\\"type\\":\\"%s\\"},", $1, $2, $3, $4, $5}\' | sed \'$ s/,$//\' | awk \'BEGIN {printf "["} {printf "%s", $0} END {printf "]"}\'\nexit\n'
  );
  child.waitExit();
  try {
    ret.volumes = JSON.parse(child.stdout.str.trim());
  } catch (xx) {}
  ...
  try {
    ...
    if (batteries.length != 0) {
      ...
      for (var i in batteries) {
        ...
        for (var x in filesToRead) {
          try {
            const content = require('fs')
              .readFileSync('/sys/class/power_supply/' + batteries[i] + '/' + filesToRead[x])
              .toString()
              .trim();
        ...
        const toMilli = function (val) {
          return Math.round((val || 0) / 1000);
        }; // Convert from µ units to m units (divide by 1000)
        const batteryJson = {
          ...
          Health:
            thedata.energy_full && thedata.energy_full_design
              ? Math.floor((thedata.energy_full / thedata.energy_full_design) * 100)
              : 0,
          BatteryCharge:
            thedata.energy_now && thedata.energy_full
              ? Math.floor((thedata.energy_now / thedata.energy_full) * 100)
              : thedata.capacity
                ? thedata.capacity
                : 0
        };
      ...
      if (values.battery.length == 0) {
        delete values.battery;
      }
    }
  } catch (ex) {}
...
function windows_identifiers() {
  ...
  var values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_Bios', [
    'ReleaseDate',
    'Manufacturer',
    'SMBIOSBIOSVersion',
    'SerialNumber'
  ...
  values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_BaseBoard', [
    'Product',
    'SerialNumber',
    'Manufacturer',
    'Version'
  ...
  values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_SystemEnclosure', [
    'SerialNumber',
    'SMBIOSAssetTag',
    'Manufacturer'
  ]);
  ...
  values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_Processor', [
    'Caption',
    'DeviceID',
    'Manufacturer',
    'MaxClockSpeed',
  ...
  values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_VideoController', [
    'Name',
  ...
  values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_DiskDrive', [
    'Caption',
    'DeviceID',
    'Model',
    'Partitions',
  ...
  for (var gpuinfo in ret.windows.gpu) {
    if (ret.windows.gpu[gpuinfo].Name) {
      ret.identifiers.gpu_name.push(ret.windows.gpu[gpuinfo].Name);
    }
  }
  ...
  for (var dv in ret.windows.drives) {
    ret.identifiers.storage_devices.push({
      Caption: ret.windows.drives[dv].Caption,
      Model: ret.windows.drives[dv].Model,
      Size: ret.windows.drives[dv].Size
    });
  }

  try {
    ret.identifiers.cpu_name = ret.windows.cpu[0].Name;
  } catch (x) {}
  ...
  try {
    values = require('win-wmi').query('ROOT\\CIMV2\\Security\\MicrosoftTpm', 'SELECT * FROM Win32_Tpm', [
  ...
  try {
    ...
    var values2 = require('win-wmi').query('ROOT\\WMI', 'SELECT * FROM BatteryFullChargedCapacity', [
      'InstanceName',
      'FullChargedCapacity'
    ]);
    var values3 = require('win-wmi').query('ROOT\\WMI', 'SELECT * FROM BatteryRuntime', [
      'InstanceName',
      'EstimatedRuntime'
    ]);
    var values4 = require('win-wmi').query('ROOT\\WMI', 'SELECT * FROM BatteryStaticData', [
      'InstanceName',
      'Chemistry',
      'DesignedCapacity',
      'DeviceName',
    ...
    for (i = 0; i < values4.length; ++i) {
      ...
      if (values4[i].ManufactureDate) {
        if (values4[i].ManufactureDate.indexOf('*****') != -1) delete values4[i].ManufactureDate;
      }
    }
    var values5 = require('win-wmi').query('ROOT\\WMI', 'SELECT * FROM BatteryStatus', [
      'InstanceName',
      'ChargeRate',
      'Charging',
      'DischargeRate',
        ...
        for (var j = 0; j < values4.length; ++j) {
          if (values2[i].InstanceName == values4[j].InstanceName) {
            if (
              ...
              values4[j].DesignedCapacity > 0 &&
              values2[i].FullChargedCapacity &&
              values2[i].FullChargedCapacity > 0
            ) {
              values6[i] = {
                Health: Math.floor((values2[i].FullChargedCapacity / values4[j].DesignedCapacity) * 100),
                InstanceName: values2[i].InstanceName
              };
              if (values6[i].Health > 100) {
                values6[i].Health = 100;
              }
            } else {
        ...
        for (var j = 0; j < values5.length; ++j) {
          if (values2[i].InstanceName == values5[j].InstanceName) {
            if (
              values2[i].FullChargedCapacity &&
              values2[i].FullChargedCapacity > 0 &&
              values5[j].RemainingCapacity &&
              values5[j].RemainingCapacity > 0
            ) {
              values7[i] = {
...
function macos_identifiers() {
  ...
  child.stdin.write(
    'ioreg -d2 -c IOPlatformExpertDevice | grep board-id | awk -F= \'{ split($2, res, "\\""); print res[2]; }\'\nexit\n'
  );
  child.waitExit();
  ...
  child.stdin.write(
    'ioreg -d2 -c IOPlatformExpertDevice | grep IOPlatformSerialNumber | awk -F= \'{ split($2, res, "\\""); print res[2]; }\'\nexit\n'
  );
  child.waitExit();
  ...
  child.stdin.write(
    'ioreg -d2 -c IOPlatformExpertDevice | grep manufacturer | awk -F= \'{ split($2, res, "\\""); print res[2]; }\'\nexit\n'
  );
  child.waitExit();
  ...
  child.stdin.write(
    'ioreg -d2 -c IOPlatformExpertDevice | grep version | awk -F= \'{ split($2, res, "\\""); print res[2]; }\'\nexit\n'
  );
  child.waitExit();
  ...
  child.stdin.write(
    'ioreg -d2 -c IOPlatformExpertDevice | grep IOPlatformUUID | awk -F= \'{ split($2, res, "\\""); print res[2]; }\'\nexit\n'
  );
  child.waitExit();
  ...
  if (lines.length > 0) {
    const memorySlots = [];
    if (lines[2].trim().includes('Memory Slots:')) {
      // OLD MACS WITH SLOTS
      ...
      // NEW MACS WITHOUT SLOTS
      memorySlots.push({
        DeviceLocator: 'Onboard Memory',
        Size: lines[2].split(':')[1].trim(),
        PartNumber: lines[3].split(':')[1].trim(),
        Manufacturer: lines[4].split(':')[1].trim()
      });
    }
  ...
  child.stdout.on('data', dataHandler);
  child.stdin.write(
    'df -aHY | awk \'NR>1 {printf "{\\"size\\":\\"%s\\",\\"used\\":\\"%s\\",\\"available\\":\\"%s\\",\\"mount_point\\":\\"%s\\",\\"type\\":\\"%s\\"},", $3, $4, $5, $10, $2}\' | sed \'$ s/,$//\' | awk \'BEGIN {printf "["} {printf "%s", $0} END {printf "]"}\'\nexit\n'
  );
  child.waitExit();
  try {
    ...
    if (ret.darwin.volumes.length == 0) {
      // not sonima OS so dont show type for now
      child = require('child_process').execFile('/bin/sh', ['sh']);
      child.stdout.str = '';
      child.stdout.on('data', dataHandler);
      child.stdin.write(
        'df -aH | awk \'NR>1 {printf "{\\"size\\":\\"%s\\",\\"used\\":\\"%s\\",\\"available\\":\\"%s\\",\\"mount_point\\":\\"%s\\"},", $2, $3, $4, $9}\' | sed \'$ s/,$//\' | awk \'BEGIN {printf "["} {printf "%s", $0} END {printf "]"}\'\nexit\n'
      );
      child.waitExit();
      try {
        ret.darwin.volumes = JSON.parse(child.stdout.str.trim());
...
function win_chassisType() {
  ...
  try {
    var tokens = require('win-wmi-fixed').query('ROOT\\CIMV2', 'SELECT ChassisTypes FROM Win32_SystemEnclosure', [
      'ChassisTypes'
    ]);
    if (tokens[0]) {
...
function win_systemType() {
  try {
    var tokens = require('win-wmi').query('ROOT\\CIMV2', 'SELECT PCSystemType FROM Win32_ComputerSystem', [
      'PCSystemType'
    ]);
    if (tokens[0]) {
...
switch (process.platform) {
  ...
  case 'win32':
    module.exports = {
      _ObjectID: 'identifiers',
      get: windows_identifiers,
      chassisType: win_chassisType,
      formFactor: win_formFactor,
      systemType: win_systemType
    };
    break;
  case 'darwin':
  ...
  switch (process.platform) {
    ...
    case 'linux':
      ...
      for (var i in devices) {
        if (
          require('fs')
            .readFileSync('/sys/class/power_supply/' + devices[i] + '/type')
            .toString()
            .trim() == 'Battery'
        ) {
          ret = true;
...
module.exports.isVM = function isVM() {
  ...
  if (id.identifiers.board_vendor && id.identifiers.board_vendor == 'VMware, Inc.') {
    ret = true;
  }
