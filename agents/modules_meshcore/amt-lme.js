  ...
  this.write = function (chunk) {
    for (var args in arguments) {
      if (typeof arguments[args] == 'function') {
        this.once('drain', arguments[args]);
        break;
      }
    }
...
function lme_heci(options) {
  ...
  this.on('newListener', function (name, func) {
    if (name == 'connect' && this._LME._connected == true) {
      func.call(this);
    }
    if (name == 'error' && this._LME._error != null) {
      func.call(this, this._LME._error);
    }
  });
  if (options != null) {
  ...
  this._LME._binded = {};
  this._LME.LMS = this;
  this._LME.on('error', function (e) {
    this._error = e;
    this.LMS.emit('error', e);
  });
      ...
      switch (cmd) {
          ...
          switch (name) {
            case 'tcpip-forward':
              ...
              if (this[name][port] != null) {
                // Close the existing binding
                for (var i in this.sockets) {
                  var channel = this.sockets[i];
                  if (channel.localPort == port) {
                    this.sockets[i].end();
        ...
        case APF_CHANNEL_OPEN_CONFIRMATION:
          ...
          if (this.sockets[rChannel] != undefined) {
            ...
            this.sockets[rChannel].on('data', function (chunk) {
              if (!this.bufferedStream.write(chunk)) {
                this.pause();
              }
            });
        ...
        case APF_CHANNEL_WINDOW_ADJUST:
          ...
          if (this.sockets[rChannelId] != undefined) {
            ...
            if (
              !this.sockets[rChannelId].bufferedStream.isEmpty() &&
              this.sockets[rChannelId].bufferedStream.isWaiting()
            ) {
        ...
        case APF_CHANNEL_DATA:
          ...
          if (this.sockets != null && this.sockets[rChannelId] != undefined) {
            ...
            if (httpData != null || channel.data.length >= 8000) {
              ...
              if (xmlParser != null) {
                try {
                  notify = xmlParser.ParseWsman(httpData);
                } catch (e) {}
              }

              // Event the http data
              if (notify != null) {
                this.LMS.emit('notify', notify, channel.options, _lmsNotifyToCode(notify));
              }
        ...
        case APF_CHANNEL_OPEN:
          ...
          var originatorIp = chunk
            .slice(nameLen + 29 + hostToConnectLen, nameLen + 29 + hostToConnectLen + originatorIpLen)
            .toString();
          ...
          insocket.rxWindow = 0;
          insocket.options = {
            target: hostToConnect,
            targetPort: portToConnect,
            source: originatorIp,
  ...
  this.bindDuplexStream = function (duplexStream, remoteFamily, localPort) {
    ...
    if (this._LME.sockets == undefined) {
      this._LME.sockets = {};
    }
...
function parseHttp(httpData) {
  ...
  if (i == -1 || httpData.length < i + 2) {
    return null;
  }
  var headers = require('http-headers')(httpData.substring(0, i), true);
  var contentLength = parseInt(headers['content-length']);
  if (httpData.length >= contentLength + i + 4) {
    return httpData.substring(i + 4, i + 4 + contentLength);
  }
  return null;
}
...
function _lmsNotifyToCode(notify) {
  ...
  try {
    msgid += '-' + notify.Body.MessageArguments[0];
  } catch (e) {}
  return msgid;
}
