...
function getStatus() {
  ...
  if (seccenter != null) {
    ...
    if (seccenter.WscGetSecurityProviderHealth(WSC_SECURITY_PROVIDER_FIREWALL, status).Val == 0) {
      ret.firewall = statusString(status.toBuffer().readUInt32LE());
    }
    if (seccenter.WscGetSecurityProviderHealth(WSC_SECURITY_PROVIDER_ANTIVIRUS, status).Val == 0) {
      ret.antiVirus = statusString(status.toBuffer().readUInt32LE());
    }
    if (seccenter.WscGetSecurityProviderHealth(WSC_SECURITY_PROVIDER_AUTOUPDATE_SETTINGS, status).Val == 0) {
      ret.autoUpdate = statusString(status.toBuffer().readUInt32LE());
    }
  }
  return ret;
}
...
if (process.platform == 'win32' && seccenter != null) {
  ...
  require('events').EventEmitter.call(j, true).createEvent('changed');
  ...
  j._EV.on('GlobalCallback', function (p) {
    if (!this.ObjectToPtr_Verify(this.parent, p)) {
      return;
