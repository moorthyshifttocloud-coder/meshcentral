...
function windows_cpuUtilization() {
  ...
  var err = 0;
  if ((err = GM.pdh.PdhOpenQueryA(0, 0, p.cpu).Val) != 0) {
    p._rej(err);
    return;
  }
  ...
  if (
    (err = GM.pdh.PdhAddEnglishCounterA(
      p.cpu.Deref(),
      GM.CreateVariable('\\Processor(*)\\% Processor Time'),
      0,
      p.cpuTotal
    ).Val) != 0
  ) {
  ...
  if ((err = GM.pdh.PdhCollectQueryData(p.cpu.Deref()).Val != 0)) {
    p._rej(err);
    return;
  }
  p._timeout = setTimeout(
    function (po) {
      ...
      var e;
      if ((e = GM.pdh.PdhCollectQueryData(po.cpu.Deref()).Val != 0)) {
        po._rej(e);
        return;
      }

      if (
        (e = GM.pdh.PdhGetFormattedCounterArrayA(po.cpuTotal.Deref(), PDH_FMT_DOUBLE, bufSize, itemCount, 0).Val) ==
        -2147481646
      ) {
      ...
      if (
        (e = GM.pdh.PdhGetFormattedCounterArrayA(
          po.cpuTotal.Deref(),
          PDH_FMT_DOUBLE,
          bufSize,
          itemCount,
...
function windows_memUtilization() {
  ...
  ret.percentFree =
    (ret.MemFree.div(require('bignum')('1048576')).toNumber() /
      ret.MemTotal.div(require('bignum')('1048576')).toNumber()) *
    100; //.toFixed(2);
  ret.percentConsumed =
    (ret.MemTotal.sub(ret.MemFree).div(require('bignum')('1048576')).toNumber() /
      ret.MemTotal.div(require('bignum')('1048576')).toNumber()) *
    100; //.toFixed(2);
...
function linux_cpuUtilization() {
  ...
  for (var i in lines) {
    ...
    ((x = 0), (currSum = 0));
    while (columns[++x] == '');
    for (y = x; y < columns.length; ++y) {
      currSum += parseInt(columns[y]);
    }
...
function macos_memUtilization() {
  ...
  if (lines[0].length > 0) {
    ...
    var bdown = usage.split(',');
    if (bdown.length > 2) {
      // new style - PhysMem: 5750M used (1130M wired, 634M compressor), 1918M unused.
      mem.MemFree = parseInt(bdown[2].trim().split(' ')[0]);
    } else {
      // old style - PhysMem: 6683M used (1606M wired), 9699M unused.
...
function linux_thermals() {
  ...
  child.stdin.write(
    'for folder in /sys/class/thermal/thermal_zone*/; do [ -e "$folder/temp" ] && echo "$(cat "$folder/temp"),$(cat "$folder/type")"; done\nexit\n'
  );
  child.waitExit();
  ...
  child.stdin.write(
    'for mon in /sys/class/hwmon/hwmon*; do for label in "$mon"/temp*_label; do if [ -f $label ]; then echo $(cat "$label")___$(cat "${label%_*}_input"); fi; done; done;\nexit\n'
  );
  child.waitExit();
  if (child.stdout.str.trim() != '') {
    var lines = child.stdout.str.trim().split('\n');
...
function macos_thermals() {
  ...
  if (child.stdout.str.trim() != '') {
    ...
    child.stderr.on('data', function (c) {
      if (c.toString().split('unable to get smc values').length > 1) {
        // error getting sensors so just kill
        this.parent.kill();
        return;
      }
    });
...
switch (process.platform) {
  case 'linux':
    module.exports = {
      cpuUtilization: linux_cpuUtilization,
      memUtilization: linux_memUtilization,
      thermals: linux_thermals
    };
    break;
  case 'win32':
    module.exports = {
      cpuUtilization: windows_cpuUtilization,
      memUtilization: windows_memUtilization,
      thermals: windows_thermals
    };
    break;
  case 'darwin':
    module.exports = {
      cpuUtilization: macos_cpuUtilization,
      memUtilization: macos_memUtilization,
      thermals: macos_thermals
    };
    break;
}
