...
function trimObject(j) {
  var i;
  for (i in j) {
    if (j[i] == null || (typeof j[i] == 'string' && j[i] == '') || i.startsWith('__')) {
      delete j[i];
    }
  }
  if (j['SerialNumber'] < 0) {
    var tmp = Buffer.alloc(4);
    tmp.writeInt32LE(j['SerialNumber']);
    j['SerialNumber'] = tmp.readUInt32LE();
  }
  return j;
}
...
function windows_volumes() {
  ...
  var ret = {};
  var values = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_LogicalDisk', [
    'DeviceID',
    'VolumeName',
    'FileSystem',
    'Size',
  ...
  try {
    values = require('win-wmi').query(
      'ROOT\\CIMV2\\Security\\MicrosoftVolumeEncryption',
      'SELECT * FROM Win32_EncryptableVolume',
      ['DriveLetter', 'ConversionStatus', 'ProtectionStatus']
    );
    if (values[0]) {
      for (var i = 0; i < values.length; ++i) {
        ...
        ret[drive].volumeStatus = statuses.hasOwnProperty(values[i].ConversionStatus)
          ? statuses[values[i].ConversionStatus]
          : 'FullyDecrypted';
        ret[drive].protectionStatus =
          values[i].ProtectionStatus == 0 ? 'Off' : values[i].ProtectionStatus == 1 ? 'On' : 'Unknown';
        try {
          var foundIDMarkedLine = false,
            foundMarkedLine = false,
            identifier = '',
            password = '';
          var keychild = require('child_process').execFile(
            process.env['windir'] + '\\system32\\cmd.exe',
            ['/c', 'manage-bde -protectors -get ' + drive + ': -Type recoverypassword'],
            {}
          );
          keychild.stdout.str = '';
          keychild.stdout.on('data', function (c) {
            this.str += c.toString();
          });
          keychild.waitExit();
          ...
          for (var x = 0; x < lines.length; x++) {
            ...
            if (englishidpass || germanidpass || frenchidpass || englishpass || germanpass || frenchpass) {
              ...
              if (
                x + 1 < lines.length &&
                nextline !== '' &&
                (nextline.startsWith('ID:') || nextline.startsWith('ID :'))
              ) {
    ...
    p1._res(ret);
  } // just return volumes as cant get encryption/bitlocker
  return p1;
}
