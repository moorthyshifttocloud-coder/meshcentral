...
//
const ResultsFunctions = ['QueryInterface', 'AddRef', 'Release', 'Reset', 'Next', 'NextAsync', 'Clone', 'Skip'];
...
const QueryAsyncHandler = [
  ...
  },
  {
    ...
    parms: 5,
    name: 'SetStatus',
    func: function (j, lFlags, hResult, strParam, pObjParam) {
...
function enumerateProperties(j, fields) {
  ...
  if (fields != null && Array.isArray(fields)) {
    ...
    len = nme
      .Deref()
      .Deref(GM.PointerSize == 8 ? 24 : 16, 4)
      .toBuffer()
      .readUInt32LE();
    nn = GM.CreatePointer();
          ...
          switch (baseType) {
            ...
            case 0x0008: // VT_BSTR
              arrayValues.push(
                arrayData
                  .Deref()
                  .Deref(k * GM.PointerSize, GM.PointerSize)
                  .Deref().Wide2UTF8
              );
              break;
          }
...
function queryAsync(resourceString, queryString, fields) {
  ...
  handlers.locator = require('win-com').createInstance(
    require('win-com').CLSIDFromString(CLSID_WbemAdministrativeLocator),
    require('win-com').IID_IUnknown
  );
  ...
  if (
    handlers.locator.funcs.ConnectToServer(handlers.locator, resource, 0, 0, 0, 0, 0, 0, handlers.services).Val != 0
  ) {
    throw 'Error calling ConnectToService';
  }
  ...
  // Make the COM call
  if (
    handlers.services.funcs.ExecQueryAsync(
      handlers.services.Deref(),
      language,
      query,
...
function query(resourceString, queryString, fields) {
  ...
  var locator = require('win-com').createInstance(
    require('win-com').CLSIDFromString(CLSID_WbemAdministrativeLocator),
    require('win-com').IID_IUnknown
  );
  ...
  if (locator.funcs.ConnectToServer(locator, resource, 0, 0, 0, 0, 0, 0, services).Val != 0) {
    throw 'Error calling ConnectToService';
  }

  // Execute the Query
  services.funcs = require('win-com').marshalFunctions(services.Deref(), ServiceFunctions);
  if (services.funcs.ExecQuery(services.Deref(), language, query, WBEM_FLAG_BIDIRECTIONAL, 0, results).Val != 0) {
    throw 'Error in Query';
  }
