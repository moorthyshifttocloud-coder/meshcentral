...
function CreateAPFClient(parent, args) {
  ...
  obj.onSecureConnect = function onSecureConnect(resp, ws, head) {
    ...
    obj.forwardClient.ws.on('end', function () {
      ...
      if (obj.onChannelClosed) {
        obj.onChannelClosed(obj);
      }
    });
    ...
    if (typeof obj.args.conntype == 'number' && obj.args.conntype != 0) {
      ...
      if (obj.args.meiState != null) {
        SendJsonControl(obj.forwardClient.ws, { action: 'meiState', value: obj.args.meiState });
      }
    }
  ...
  obj.updateMeiState = function (state) {
    SendJsonControl(obj.forwardClient.ws, { action: 'meiState', value: state });
  };
  obj.sendMeiDeactivationState = function (state) {
    SendJsonControl(obj.forwardClient.ws, { action: 'deactivate', value: state });
  };
  obj.sendStartTlsHostConfigResponse = function (state) {
    SendJsonControl(obj.forwardClient.ws, { action: 'startTlsHostConfig', value: state });
  };
  obj.sendStopConfigurationResponse = function (state) {
    SendJsonControl(obj.forwardClient.ws, { action: 'stopConfiguration', value: state });
  };
  ...
  function SendProtocolVersion(socket, uuid) {
    var data =
      String.fromCharCode(APFProtocol.PROTOCOLVERSION) +
      IntToStr(1) +
      IntToStr(0) +
      IntToStr(0) +
      hex2rstr(strToGuid(uuid)) +
      binzerostring(64);
    socket.write(data);
  ...
  function SendUserAuthRequest(socket, user, pass) {
    ...
    var data =
      String.fromCharCode(APFProtocol.USERAUTH_REQUEST) +
      IntToStr(user.length) +
      user +
      IntToStr(service.length) +
      service;
    //password auth
  ...
  function SendGlobalRequestPfwd(socket, amthostname, amtport) {
    ...
    var data =
      String.fromCharCode(APFProtocol.GLOBAL_REQUEST) + IntToStr(tcpipfwd.length) + tcpipfwd + binzerostring(1, 1);
    ...
    switch (cmd) {
      case APFProtocol.SERVICE_ACCEPT: {
        var slen = ReadInt(data, 1),
          service = data.substring(5, 6 + slen);
        ...
        if (chan != null) {
          ...
          try {
            chan.write(Buffer.from(chan_data, 'binary'), function () {
              ...
              if (this.curInWindow > this.maxInWindow / 2) {
                SendChannelWindowAdjust(socket.ws, rcpt_chan, this.curInWindow);
                this.curInWindow = 0;
              }
            });
          } catch (ex) {
      ...
      case APFProtocol.JSON_CONTROL: {
        ...
        if (obj.onJsonControl) {
          var o = null;
          try {
            o = JSON.parse(data.substring(5, 5 + len));
          } catch (ex) {}
          if (o != null) {
            obj.onJsonControl(o);
          }
  ...
  function SendChannelOpenFailure(socket, chan_data) {
    socket.write(
      String.fromCharCode(APFProtocol.CHANNEL_OPEN_FAILURE) +
        IntToStr(chan_data.sender_chan) +
        IntToStr(2) +
        IntToStr(0) +
        IntToStr(0)
    );
  ...
  function SendChannelOpenConfirm(socket, chan_data) {
    socket.write(
      String.fromCharCode(APFProtocol.CHANNEL_OPEN_CONFIRMATION) +
        IntToStr(chan_data.sender_chan) +
        IntToStr(chan_data.sender_chan) +
        IntToStr(chan_data.window_size) +
        IntToStr(0xffffffff)
    );
  ...
  function SendChannelData(socket, chan, data) {
    socket.write(
      Buffer.concat([
        Buffer.from(String.fromCharCode(APFProtocol.CHANNEL_DATA) + IntToStr(chan) + IntToStr(data.length), 'binary'),
        data
      ])
    );
