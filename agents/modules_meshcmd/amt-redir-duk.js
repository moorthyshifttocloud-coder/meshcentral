...
module.exports = function CreateAmtRedirect(module) {
  ...
  obj.Start = function (host, port, user, pass, tls, tlsFingerprint, tlsoptions) {
    ...
    if (tls == true) {
      obj.socket = obj.tls.connect(
        { host: host, port: port, rejectUnauthorized: false, checkServerIdentity: obj.onCheckServerIdentity },
  ...
  obj.xxOnSocketConnected = function () {
    ...
    if (urlvars && urlvars['redirtrace']) {
      console.log('REDIR-CONNECTED');
    }
    //obj.Debug("Socket Connected");
    obj.xxStateChange(2);
    if (obj.protocol == 1)
      obj.xxSend(obj.RedirectStartSol); // TODO: Put these strings in higher level module to tighten code
    else if (obj.protocol == 2)
      obj.xxSend(obj.RedirectStartKvm); // Don't need these is the feature if not compiled-in.
      ...
      switch (obj.amtaccumulator[0]) {
        ...
        case 0x14: // AuthenticateSessionReply (20)
          ...
          for (i = 0; i < authDataLen; i++) {
            authData.push(obj.amtaccumulator[9 + i]);
          }
          ...
          if (authType == 0) {
            ...
            if (authData.indexOf(4) >= 0) {
              ...
              obj.xxSend(
                String.fromCharCode(0x13, 0x00, 0x00, 0x00, 0x04) +
                  IntToStrX(obj.user.length + obj.authuri.length + 8) +
                  String.fromCharCode(obj.user.length) +
                  obj.user +
                  String.fromCharCode(0x00, 0x00) +
                  String.fromCharCode(obj.authuri.length) +
                  obj.authuri +
            ...
            if (obj.digestRealmMatch && obj.digestRealmMatch != realm) {
              obj.Stop();
              return;
            }
            ...
            var digest = hex_md5(
              hex_md5(obj.user + ':' + realm + ':' + obj.pass) +
                ':' +
                nonce +
                ':' +
                extra +
                hex_md5('POST:' + obj.authuri)
            );
            var totallen =
              obj.user.length +
              realm.length +
            ...
            var buf = Buffer.concat([
              new Buffer([0x13, 0x00, 0x00, 0x00, authType]),
              new Buffer([totallen & 0xff, (totallen >> 8) & 0xff, 0x00, 0x00]),
              new Buffer([obj.user.length]),
              new Buffer(obj.user),
              new Buffer([realm.length]),
              new Buffer(realm),
              new Buffer([nonce.length]),
              new Buffer(nonce),
              new Buffer([obj.authuri.length]),
              new Buffer(obj.authuri),
              new Buffer([cnonce.length]),
              new Buffer(cnonce),
              new Buffer([snc.length]),
              new Buffer(snc),
              new Buffer([digest.length]),
              new Buffer(digest)
            ]);
            ...
            if (obj.protocol == 1) {
              ...
              obj.xxSend(
                String.fromCharCode(0x20, 0x00, 0x00, 0x00) +
                  ToIntStr(obj.amtsequence++) +
                  ToShortStr(MaxTxBuffer) +
                  ToShortStr(TxTimeout) +
                  ToShortStr(TxOverflowTimeout) +
                  ToShortStr(RxTimeout) +
                  ToShortStr(RxFlushTimeout) +
                  ToShortStr(Heartbeat) +
                  ToIntStr(0)
              );
            }
            if (obj.protocol == 2) {
  ...
  obj.xxSend = function (x) {
    if (urlvars && urlvars['redirtrace']) {
      console.log('REDIR-SEND(' + x.length + '): ' + rstr2hex(x));
    }
  ...
  obj.Send = function (x) {
    ...
    if (obj.protocol == 1) {
      obj.xxSend(String.fromCharCode(0x28, 0x00, 0x00, 0x00) + ToIntStr(obj.amtsequence++) + ToShortStr(x.length) + x);
    } else {
      obj.xxSend(x);
    }
  };
  ...
  obj.xxRandomValueHex = function (len) {
    var t = [],
      l = Math.floor(len / 2);
    for (var i = 0; i < l; i++) {
      t.push(obj.tls.generateRandomInteger('0', '255'));
    }
...
function hex_md5(a) {
  return md5hasher.syncHash(a).toString('hex').toLowerCase();
}
