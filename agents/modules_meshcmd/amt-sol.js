...
module.exports = function CreateAmtRemoteSol() {
  ...
  obj.xxStateChange = function (newstate) {
    if (obj.debug) console.log('SOL-StateChange', newstate);
    if (newstate == 0) {
      obj.Stop();
    }
    if (newstate == 3) {
  ...
  obj.Stop = function () {
    if (obj.debug) {
      console.log('SOL-Stop');
    }
  };
  obj.ProcessData = function (data) {
    if (obj.debug) {
      console.log('SOL-ProcessData', data);
    }
    if (obj.onData) {
      obj.onData(obj, data);
    }
  };
  obj.Send = function (text) {
    if (obj.debug) {
      console.log('SOL-Send', text);
    }
    obj.parent.Send(text);
  };
  return obj;
};
