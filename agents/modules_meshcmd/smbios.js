...
try {
  Object.defineProperty(String.prototype, 'replaceAll', {
    value: function replaceAll(oldVal, newVal) {
...
function SMBiosTables() {
      ...
      for (var i = 1; i < lines.length; ++i) {
        ...
        if (tokens.length > 1) {
          ...
          for (var strx in strings) {
            ...
            if (tmp && tmp[0] !== '"' && /^[0-9a-fA-F]+$/.test(tmp)) {
              stringsFinal.push(tmp);
            }
          }
  ...
  this.get = function get(callback) {
    if (process.platform == 'win32') {
      ...
      if (callback) {
        callback.apply(this, [this._parse(SMData)]);
        return;
      } else {
        return this._parse(SMData);
      }
    }
    if (process.platform == 'linux') {
      var MemoryStream = require('MemoryStream');
  ...
  this.parse = function parse(data) {
    ...
    try {
      if (JSON.stringify(r).length > 65535) {
        r = {};
      }
    } catch (ee) {}
    return r;
  ...
  this.systemInfo = function systemInfo(data) {
    ...
    if (data[1]) {
      ...
      retVal.uuid = [
        ...
        zeroLeftPad(uuid.slice(10).toString('hex').toLowerCase(), 12)
      ].join('-');
    ...
    if (data[130] && data[130].peek().slice(0, 4).toString() == '$AMT') {
      ...
      if (retVal.AMT) {
        ...
        if (data[131].peek() && data[131].peek().slice(52, 56).toString() == 'vPro') {
          ...
          if (settings[0] & 0x04) {
            retVal.TXT = settings[0] & 0x08 ? true : false;
          }
          if (settings[0] & 0x10) {
            retVal.VMX = settings[0] & 0x20 ? true : false;
          }
          retVal.MEBX =
            settings.readUInt16LE(4).toString() +
            '.' +
            settings.readUInt16LE(6).toString() +
            '.' +
            settings.readUInt16LE(8).toString() +
            '.' +
            settings.readUInt16LE(10).toString();

          var mecap = settings.slice(20, 32);
          retVal.ManagementEngine =
            mecap.readUInt16LE(6).toString() +
            '.' +
            mecap.readUInt16LE(4).toString() +
            '.' +
            mecap.readUInt16LE(10).toString() +
            '.' +
            mecap.readUInt16LE(8).toString();
