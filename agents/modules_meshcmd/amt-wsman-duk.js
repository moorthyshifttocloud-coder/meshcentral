...
function CreateWsmanComm(/*host, port, user, pass, tls, extra*/) {
  ...
  obj.PerformAjaxEx = function (postdata, callback, tag, url, action) {
    if (obj.FailAllError != 0) {
      if (obj.FailAllError != 999) {
        obj.gotNextMessagesError({ status: obj.FailAllError }, 'error', null, [postdata, callback, tag]);
      }
      return;
    }
    ...
    var request = {
      ...
      checkServerIdentity: function (cert) {
        /*console.log('checkServerIdentity', JSON.stringify(cert));*/
      }
    };
    ...
    req.on('error', function (err) {
      obj.gotNextMessagesError({ status: 600, error: '' + err }, 'error', null, [postdata, callback, tag]);
    });
  ...
  obj.gotNextMessagesError = function (request, status, errorThrown, callArgs) {
    ...
    if (obj.FailAllError != 999) {
      callArgs[1]({ Header: { HttpError: request.status, error: request.error } }, request.status, callArgs[2]);
    }
    obj.PerformNextAjax();
  };
