...
function amt_heci() {
  ...
  this.getCommand = function getCommand(chunk) {
    ...
    var ret = {
      IsResponse: (command & 0x800000) == 0x800000 ? true : false,
      Command: command & 0x7fffff,
      Status: chunk.length != 0 ? chunk.readUInt32LE(12) : -1,
      Data: chunk.length != 0 ? chunk.slice(16) : null
    };
    return ret;
  };
  ...
  this.sendCommand = function sendCommand() {
    ...
    g_internal._rq.enQueue({
      cmd: arguments[0],
      func: arguments[2],
      optional: args,
      send: arguments[1] == null ? header : Buffer.concat([header, arguments[1]])
    });
    if (!g_internal._amt) {
  ...
  this.getVersion = function getVersion(callback) {
    ...
    for (var i = 1; i < arguments.length; ++i) {
      optional.push(arguments[i]);
    }
    this.sendCommand(
      26,
      null,
      function (header, fn, opt) {
        if (header.Status == 0) {
          var i,
            CodeVersion = header.Data,
            val = { BiosVersion: CodeVersion.slice(0, g_internal._amt.BiosVersionLen).toString(), Versions: [] },
            v = CodeVersion.slice(g_internal._amt.BiosVersionLen + 4);
          for (i = 0; i < CodeVersion.readUInt32LE(g_internal._amt.BiosVersionLen); ++i) {
            val.Versions[i] = {
              Description: v.slice(2, v.readUInt16LE(0) + 2).toString(),
              Version: v
                .slice(
                  4 + g_internal._amt.UnicodeStringLen,
                  4 + g_internal._amt.UnicodeStringLen + v.readUInt16LE(2 + g_internal._amt.UnicodeStringLen)
                )
                .toString()
            };
            v = v.slice(4 + 2 * g_internal._amt.UnicodeStringLen);
          }
          if (val.BiosVersion.indexOf('\0') > 0) {
            val.BiosVersion = val.BiosVersion.substring(0, val.BiosVersion.indexOf('\0'));
          }
          opt.unshift(val);
        } else {
          opt.unshift(null);
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          var result = {};
          result.uuid = [
            zeroLeftPad(header.Data.readUInt32LE(0).toString(16), 8),
            ...
            zeroLeftPad(header.Data.slice(10).toString('hex').toLowerCase(), 12)
          ].join('-');
          opt.unshift(result);
        } else {
          opt.unshift(null);
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          ...
          if (result.state < 3) {
            result.stateStr = ['PRE', 'IN', 'POST'][result.state];
          }
          opt.unshift(result);
        } else {
          opt.unshift(null);
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          ...
          if (result.mode < 4) {
            result.modeStr = ['NONE', 'ENTERPRISE', 'SMALL_BUSINESS', 'REMOTE_ASSISTANCE'][result.mode];
          }
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          ...
          if (result.controlMode < 3) {
            result.controlModeStr = ['NONE_RPAT', 'CLIENT', 'ADMIN', 'REMOTE_ASSISTANCE'][result.controlMode];
          }
          opt.unshift(result);
        } else {
          opt.unshift(null);
  ...
  this.getMACAddresses = function getMACAddresses(callback) {
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          opt.unshift({
            DedicatedMAC: header.Data.slice(0, 6).toString('hex:'),
            HostMAC: header.Data.slice(6, 12).toString('hex:')
          });
        } else {
          opt.unshift({ DedicatedMAC: null, HostMAC: null });
        }
        fn.apply(this, opt);
      },
      callback,
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          ...
          if (resultLen > 0) {
            opt.unshift(header.Data.slice(2, 2 + resultLen).toString());
          } else {
            opt.unshift(null);
          }
        } else {
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0 && header.Data.length == 68) {
          opt.unshift({
            user: trim(header.Data.slice(0, 33).toString()),
            pass: trim(header.Data.slice(33, 67).toString()),
            raw: header.Data
          });
        } else {
          opt.unshift(null);
  ...
  this.getLanInterfaceSettings = function getLanInterfaceSettings(index, callback) {
    ...
    this.sendCommand(
      0x48,
      ifx,
      function onGetLanInterfaceSettings(header, fn, opt) {
        if (header.Status == 0) {
          var info = {};
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          ...
          opt.unshift({
            ...
            networkStatus: header.Data.readUInt32LE(0),
            remoteAccessStatus: header.Data.readUInt32LE(4),
            remoteAccessTrigger: header.Data.readUInt32LE(8),
  ...
  this.getProtocolVersion = function getProtocolVersion(callback) {
    ...
    if (!this._tmpSession) {
      this._tmpSession = heci.create();
      this._tmpSession.parent = this;
    }
    this._tmpSession.doIoctl(
      heci.IOCTL.HECI_VERSION,
      Buffer.alloc(5),
      Buffer.alloc(5),
      function (status, buffer, self, fn, opt) {
        if (status == 0) {
          var result =
            buffer.readUInt8(0).toString() +
            '.' +
            buffer.readUInt8(1).toString() +
            '.' +
            buffer.readUInt8(2).toString() +
            '.' +
            buffer.readUInt16BE(3).toString();
          opt.unshift(result);
          fn.apply(self, opt);
        } else {
          opt.unshift(null);
  ...
  this.startConfigurationHBased = function startConfigurationHBased(certHash, hostVpn, dnsSuffixList, func) {
    if (certHash == null || (certHash.length != 32 && certHash.length != 48)) {
      func({ status: -101 });
    }
    this.stopConfiguration(function (status) {
      if (status == 0) {
        ...
        var f = function tf() {
          delete tf.parent.xtimeout;
          tf.parent.startConfigurationHBasedEx(certHash, hostVpn, dnsSuffixList, func);
        };
        f.parent = this;
  ...
  this.startConfigurationHBasedEx = function startConfigurationHBased(certHash, hostVpn, dnsSuffixList, func) {
    ...
    if (dnsSuffixList != null) {
      ...
      var ptr = 73;
      for (var i = 0; i < dnsSuffixList.length; i++) {
        ptr += data.write(dnsSuffixList[i], ptr) + 1;
      } // Write up to 4 DNS Suffix with null seperation.
    }
    ...
    this.sendCommand(
      ...
      function (header, fn, opt) {
        if (header.Status == 0) {
          var amtHash = null;
          if (header.Data[0] == 2) {
            amtHash = header.Data.slice(1, 33);
          } // SHA256
          if (header.Data[0] == 3) {
            amtHash = header.Data.slice(1, 49);
          } // SHA384
