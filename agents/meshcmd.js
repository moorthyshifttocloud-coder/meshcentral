...
function onVerifyServer(clientName, certs) {
  ...
  try {
    for (var i in certs) {
      if (certs[i].fingerprint.replace(/:/g, '') == settings.serverhttpshash) {
        return;
      }
    }
  } catch (e) {}
...
function debug(level, message) {
  if (settings.debuglevel != null && settings.debuglevel >= level) {
    console.log(message);
  }
}
...
function md5hex(str) {
  return require('MD5Stream').create().syncHash(str).toString('hex');
}
...
function parceArguments(argv) {
  ...
  for (var i in argv) {
    ...
    if (argv[i].startsWith('--') == true) {
      var key = argv[i].substring(2).toLowerCase(),
        val = true;
...
function run(argv) {
  if (meshCmdVersion[0] == '*') {
    ...
    meshCmdVersion = ' v' + meshCmdVersion;
  }
  ...
  var actionfile = null;
  try {
    actionfile = fs.readFileSync(actionpath);
  } catch (e) {}
  if (actionpath != 'meshaction.txt' && actionfile == null) {
    console.log(
      'Unable to load \"' + actionpath + '\". Create this file or specify the location using --actionfile [filename].'
    );
    exit(1);
    return;
  }
  if (actionfile != null) {
    try {
      settings = JSON.parse(actionfile);
    } catch (e) {
      console.log(actionpath, e);
      exit(1);
      return;
    }
  } else {
    if (argv.length >= 2) {
      settings = { action: argv[1] };
    }
  }
  if (settings == null) {
    settings = {};
  ...
  for (var i in settings) {
    settings2[i.toLowerCase()] = settings[i];
  }
  settings = settings2;
  ...
  if (typeof args.authcookie == 'string') {
    settings.authcookie = args.authcookie;
  }
  if (typeof args.localport == 'string') {
    settings.localport = parseInt(args.localport);
  }
  if (typeof args.remotenodeid == 'string') {
    settings.remotenodeid = args.remotenodeid;
  }
  ...
  if (typeof args.passwordhex == 'string') {
    settings.password = Buffer.from(args.passwordhex, 'hex').toString();
  }
  ...
  if (typeof args.pass == 'string') {
    settings.password = args.pass;
  }
  if (typeof args.host == 'string') {
    settings.hostname = args.host;
  }
  if (typeof args.hostname == 'string') {
    settings.hostname = args.hostname;
  }
  if (typeof args.serverid == 'string') {
    settings.serverid = args.serverid;
  }
  if (typeof args.serverhttpshash == 'string') {
    settings.serverhttpshash = args.serverhttpshash;
  }
  if (typeof args.serverurl == 'string') {
    settings.serverurl = args.serverurl;
  }
  if (typeof args.remoteport == 'string') {
    settings.remoteport = parseInt(args.remoteport);
  }
  if (typeof args.remotetarget == 'string') {
    settings.remotetarget = args.remotetarget;
  }
  ...
  if (typeof args.debug == 'string') {
    settings.debuglevel = parseInt(args.debug);
  }
  if (typeof args.debugflags == 'string') {
    globalDebugFlags = parseInt(args.debugflags);
  }
  ...
  if (typeof args.floppy == 'string') {
    settings.floppy = args.floppy;
  }
  if (typeof args.cdrom == 'string') {
    settings.cdrom = args.cdrom;
  }
  if (typeof args.tag == 'string') {
    settings.tag = args.tag;
  }
  if (typeof args.scan == 'string') {
  ...
  if (typeof args.timeout == 'string') {
    settings.timeout = parseInt(args.timeout);
  }
  if (typeof args.iderstart == 'string') {
    settings.iderstart = args.iderstart;
  }
  if (typeof args.uuidoutput == 'string' || args.uuidoutput) {
    settings.uuidoutput = args.uuidoutput;
  }
  if (typeof args.desc == 'string') {
    settings.desc = args.desc;
  }
  if (typeof args.dnssuffix == 'string') {
    settings.dnssuffix = args.dnssuffix;
  }
  if (typeof args.create == 'string') {
    settings.create = args.create;
  }
  if (typeof args.delete == 'string') {
    settings.delete = args.delete;
  }
  if (args.bindany) {
    settings.bindany = true;
  }
  if (args.emailtoken) {
  ...
  if (argv.length > 1 && actions.indexOf(argv[1].toUpperCase()) >= 0) {
    settings.action = argv[1];
  }
  ...
  if (settings.debuglevel > 1) {
    if (settings.hostname) {
      console.log('Hostname HEX: ' + Buffer.from(settings.hostname, 'binary').toString('hex'));
    }
    if (settings.username) {
      console.log('Username HEX: ' + Buffer.from(settings.username, 'binary').toString('hex'));
    }
    if (settings.password) {
      console.log('Password HEX: ' + Buffer.from(settings.password, 'binary').toString('hex'));
    }
  }
  ...
  if (settings.action == 'help') {
    ...
    if (action == 'route') {
      console.log(
        "The route action is used along with a MeshCentral account to map a local TCP port to a remote port on any computer on your MeshCentral account. This action requires many arguments, to avoid specifying them all it's best to download the meshaction.txt file from the web site and place it in the current folder. Example usage:\r\n\r\n  (Place meshaction.txt file in current folder)\r\n  meshcmd route --pass myAccountPassword"
      );
    } else if (action == 'smbios') {
      console.log(
        "SMBios action will display this computer's system management BIOS information. Example usage:\r\n\r\n  meshcmd smbios --out smbios.txt\r\n"
      );
      ...
      console.log(
        "RawSMBios action will display this computer's system management BIOS information in raw hexdecimal form. Example usage:\r\n\r\n  meshcmd rawsmbios --out smbios.txt\r\n"
      );
      console.log('\r\Optional arguments:\r\n');
      console.log('  --output [filename]    Optional filename to write the results to.');
    } else if (action == 'amtinfo') {
      console.log(
        'AmtInfo action will get the version and activation state of Intel AMT on this computer. The command must be run on a computer with Intel AMT, must run as administrator and the Intel management driver must be installed. Example usage:\r\n\r\n  meshcmd amtinfo'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --json                 Display all Intel AMT state in JSON format.');
    } else if (action == 'amtinfojson') {
      console.log(
        'AmtInfoJson action get Intel AMT information about the computer and display it or send it to a server using HTTP. Example usage:\r\n\r\n  meshcmd amtinfojson --post https://example.com/capture'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --post [url]           Perform an HTTP POST of the data to the given URL.');
    } else if (action == 'amtversion' || action == 'amtversions') {
      console.log(
        'AmtVersions will display all version information about Intel AMT on this computer. The command must be run on a computer with Intel AMT, must run as administrator and the Intel management driver must be installed. Example usage:\r\n\r\n  meshcmd amtversions'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --json                 Display all Intel AMT state in JSON format.');
    } else if (action == 'amthashes') {
      console.log(
        'Amthashes will display all trusted activations hashes for Intel AMT. If the host is not specified, the hashes are read using the local MEI driver is used. These certificates hashes are used by Intel AMT when performing activation into ACM mode. Example usage:\r\n\r\n  meshcmd amthashes'
      );
      console.log('\r\nPossible arguments:\r\n');
      ...
      console.log(
        'Starts MicroLMS on this computer, allowing local access to Intel AMT on TCP ports 16992 and 16993 when applicable. The command must be run on a computer with Intel AMT, must run as administrator and the Intel management driver must be installed. These certificates hashes are used by Intel AMT when performing activation into ACM mode. Example usage:\r\n\r\n  meshcmd microlms'
      );
      console.log('\r\nPossible arguments:\r\n');
      ...
      console.log(
        'AmtCCM will attempt to activate Intel AMT on this computer into client control mode (CCM). The command must be run on a computer with Intel AMT, must run as administrator and the Intel management driver must be installed. Intel AMT must be in "pre-provisioning" state for this command to work and a administrator password must be provided.'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --password [password]     Admin password used to activate Intel AMT.');
    } else if (action == 'amtconfig') {
      console.log(
        'AmtConfig will attempt to activate and configure Intel AMT on this computer. The command must be run on a computer with Intel AMT, must run as administrator and the Intel management driver must be installed. Example usage:\r\n\r\n  meshcmd amtconfig --url [url]'
      );
      console.log('\r\nPossible arguments:\r\n');
      ...
      console.log(
        'AmtUUID action will get the unique identifier of the local or remote Intel AMT computer. By default, the local UUID is obtained unless a host is specified. Intel AMT must be activated for this command to work. Example usage:\r\n\r\n  meshcmd amtuuid --host 1.2.3.4 --user admin --pass mypassword --tls'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --host [hostname]      The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --user [username]      The Intel AMT login username, admin is default.');
      ...
      console.log(
        'AmtSaveState action will fetch all the entire state of Intel AMT and save it as a JSON file. This action will take multiple minutes to perform. The command will fetch the local computer state unless host is specified. Intel AMT must be ativated for this command to work. Example usage:\r\n\r\n  meshcmd amtsavestate --host 1.2.3.4 --user admin --pass mypassword --tls --output state.json'
      );
      console.log('\r\nPossible arguments:\r\n');
      ...
      console.log(
        'AmtPresence will heartbeat a local Intel AMT watchdog agent. Example usage:\r\n\r\n  meshcmd amtpresence --agent B4B6A24C-255E-A75C-F5E8-B00B4D946AA7'
      );
      console.log('\r\nPossible arguments:\r\n');
      console.log('  --user [username]      The Intel AMT login username, admin is default.');
      console.log('  --pass [password]      The Intel AMT login password.');
      ...
      console.log(
        'AmtNetwork is used to get/set Intel AMT network interface configuration. Example usage:\r\n\r\n  meshcmd amtnetwork --host 1.2.3.4 --user admin --pass mypassword --dhcp'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]         The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]         The Intel AMT login password.');
      ...
      console.log(
        'AmtFeatures is used to get/set Intel AMT feature configuration. Example usage:\r\n\r\n  meshcmd amtfeatures --host 1.2.3.4 --user admin --pass mypassword --tls --redir 1'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]         The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]         The Intel AMT login password.');
      ...
      console.log(
        'AmtAuditLog action will fetch the local or remote audit log. If used localy, no username/password is required. Example usage:\r\n\r\n  meshcmd amtauditlog --host 1.2.3.4 --user admin --pass mypassword --tls --output audit.json'
      );
      console.log('\r\nPossible arguments:\r\n');
      ...
      console.log(
        'AmtSCAN will look for Intel AMT device on the network. Example usage:\r\n\r\n  meshcmd amtscan --scan 192.168.1.0/24'
      );
      ...
      console.log(
        'AmtWifi is used to list, add or delete Intel AMT Wifi configuration. Example usage:\r\n\r\n  meshcmd amtwifi --host 1.2.3.4 --user admin --pass mypassword --list'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]         The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]         The Intel AMT login password.');
      ...
      console.log(
        'AmtWake is used to view/set/remote Intel AMT Wake Alarms. Example usage:\r\n\r\n  meshcmd amtwake --host 1.2.3.4 --user admin --pass mypassword --list'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]         The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]         The Intel AMT login password.');
      ...
      console.log(
        '     --time (hh:mm:ss)      Optional alarm time in hours:minutes:seconds format, default is 00:00:00.'
      );
      console.log(
        '     --interval (dd-hh-mm)  Optional alarm interval in days-hours-minutes format, default is alarm once.'
      );
      ...
      console.log(
        'AmtRPE is used to erase some elements of a remote Intel AMT platform. Example usage:\r\n\r\n  meshcmd amtrpe --host 1.2.3.4 --user admin --pass mypassword'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]         The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]         The Intel AMT login password.');
      ...
      console.log(
        'AmtDDNS is used to query and set the Intel AMT dynamic DNS settings. Example usage:\r\n\r\n  meshcmd amtddns --host 1.2.3.4 --user admin --pass mypassword'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]                The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]                The Intel AMT login password.');
      ...
      console.log(
        'AmtTerm is used to connect to the Serial-over-LAN port. Example usage:\r\n\r\n  meshcmd amtterm --host 1.2.3.4 --user admin --pass mypassword'
      );
      console.log('\r\nRequired arguments:\r\n');
      console.log('  --host [hostname]                The IP address or DNS name of Intel AMT, 127.0.0.1 is default.');
      console.log('  --pass [password]                The Intel AMT login password.');
  ...
  if (typeof settings.proxy == 'string') {
    var proxy = settings.proxy.split(':'),
      proxyport = proxy.length == 2 ? parseInt(proxy[1]) : 0;
    if (proxy.length != 2 || proxy[0].length < 1 || proxyport < 1 || proxyport > 65535) {
      console.log('Invalid \"proxy\" specified, use --proxy [hostname]:[port].');
      exit(1);
      return;
    }
    try {
  ...
  if (settings.action == 'amtstartconfig') {
    ...
    amtMei.on('error', function (e) {
      console.log('amtstartconfig error: ' + e);
      exit(1);
      return;
    });
    amtMei.startConfiguration(function (state) {
      if (state == 3) {
        console.log('Intel AMT is not in correct mode.');
      } else if (state == 1) {
        console.log('Intel AMT internal error.');
      } else if (state == 48) {
        console.log('Random generator not ready.');
      } else if (state == 49) {
        console.log('Certificate not ready.');
      } else if (state == 0) {
        console.log('Success.');
      } else {
    ...
    amtMei.on('error', function (e) {
      console.log('amtstopconfig error: ' + e);
      exit(1);
      return;
    });
    amtMei.stopConfiguration(function (state) {
      if (state == 3) {
        console.log('Intel AMT is not in in-provisionning mode.');
      } else if (state == 1) {
        console.log('Intel AMT internal error.');
      } else if (state == 0) {
        console.log('Success.');
    ...
    SMBiosTables.get(function (data) {
      ...
      if (settings.output == null) {
        ...
        fs.writeSync(file, Buffer.from(out, 'utf8'));
        fs.closeSync(file);
      }
      exit(0);
    });
          ...
          if (data[i][j].length > 0) {
            if (header == false) {
              out +=
                'Table type #' +
                i +
                (SMBiosTables.smTableTypes[i] == null ? '' : ', ' + SMBiosTables.smTableTypes[i]) +
                '\r\n';
              header = true;
            }
    ...
    if (
      settings.localport == null ||
      typeof settings.localport != 'number' ||
      settings.localport < 0 ||
      settings.localport > 65535
    ) {
      console.log('No or invalid \"localPort\" specified, use --localport [localport].');
      exit(1);
      return;
    }
    if (settings.remotenodeid == null || typeof settings.remotenodeid != 'string') {
      console.log('No or invalid \"remoteNodeId\" specified.');
      exit(1);
      return;
    }
    if (
      (settings.username == null || typeof settings.username != 'string' || settings.username == '') &&
      (settings.authcookie == null || typeof settings.authcookie != 'string' || settings.authcookie == '')
    ) {
      console.log('No or invalid \"username\" specified, use --username [username].');
      exit(1);
      return;
    }
    if (
      (settings.password == null || typeof settings.password != 'string' || settings.password == '') &&
      (settings.authcookie == null || typeof settings.authcookie != 'string' || settings.authcookie == '')
    ) {
      console.log('No or invalid \"password\" specified, use --password [password].');
      exit(1);
      return;
    }
    if (settings.serverid != null) {
      if (typeof settings.serverid != 'string' || settings.serverid.length != 96) {
        console.log('No or invalid \"serverId\" specified.');
        exit(1);
        return;
      }
    } else {
      if (
        settings.serverhttpshash == null ||
        typeof settings.serverhttpshash != 'string' ||
        settings.serverhttpshash.length != 96
      ) {
        console.log('No or invalid \"serverHttpsHash\" or \"serverId\" specified.');
        exit(1);
        return;
      }
    }
    if (
      settings.remoteport == null ||
      typeof settings.remoteport != 'number' ||
      settings.remoteport < 0 ||
      settings.remoteport > 65535
    ) {
      console.log('No or invalid \"remotePort\" specified, use --remoteport [remoteport].');
      exit(1);
      return;
    }
    ...
    amtMei.on('error', function (e) {
      console.log('amtversion error: ' + e);
      exit(1);
      return;
    });
      ...
      if (args.json) {
        ...
        for (var version in val.Versions) {
          ...
          if (val.Versions[version].Description == 'Sku') {
            var n = parseInt(val.Versions[version].Version),
              x = [],
              xx = 1;
            for (var i = 0; i < skuBits.length; i++) {
              if ((n & xx) != 0) {
                x.push(skuBits[i]);
              }
              xx = xx << 1;
      ...
      amtMei.getHashHandles(function (handles) {
        ...
        for (var i = 0; i < handles.length; ++i) {
          this.getCertHashEntry(handles[i], function (result) {
            var certState = [];
            if (result.isDefault) {
              certState.push('Default');
            }
            if (result.isActive) {
              certState.push('Active');
            } else {
              certState.push('Disabled');
            }
            ...
            if (--exitOnCount == 0) {
              if (args.json) {
                console.log(JSON.stringify(amtHashes, null, 2));
              }
              exit(0);
            }
          });
    ...
    try {
      amtMei.getVersion(function (result) {
        if (result) {
          for (var version in result.Versions) {
            if (result.Versions[version].Description == 'AMT') {
              mestate.ver = result.Versions[version].Version;
            }
            if (result.Versions[version].Description == 'Sku') {
              mestate.sku = parseInt(result.Versions[version].Version);
            }
          }
          ...
          amtMei.getProvisioningMode(function (result) {
            if (result) {
              mestate.ProvisioningMode = result;
            }
          });
          amtMei.getEHBCState(function (result) {
            if (result) {
              mestate.ehbc = result === true || (typeof result == 'object' && result.EHBC === true);
            }
          });
          amtMei.getControlMode(function (result) {
            if (result) {
              mestate.controlmode = result;
            }
          });
          amtMei.getMACAddresses(function (result) {
            if (result) {
              mestate.mac = result;
            }
          });
          ...
          amtMei.getUuid(function (result) {
            if (result != null && result.uuid != null) {
              mestate.uuid = result.uuid;
            }
          });
          amtMei.getRemoteAccessConnectionStatus(function (result) {
            if (result != null && result.status == 0) {
              mestate.networkStatus = result.networkStatus;
              mestate.remoteAccessStatus = result.remoteAccessStatus;
              mestate.remoteAccessTrigger = result.remoteAccessTrigger;
              mestate.mpsHostname = result.mpsHostname;
            }
          });
          amtMei.getDnsSuffix(function (result) {
            if (result) {
              mestate.DnsSuffix = result;
            }
            getAmtOsDnsSuffix(mestate, function () {
              if (args.json) {
                ...
                if (mestate.sku & 8) {
                  str = 'Intel AMT v' + mestate.ver;
                } else if (mestate.sku & 16) {
                  str = 'Intel SM v' + mestate.ver;
                }
                if (mestate.ProvisioningState.stateStr == 'PRE') {
                  str += ', pre-provisioning state';
                } else if (mestate.ProvisioningState.stateStr == 'IN') {
                  str += ', in-provisioning state';
                } else if (mestate.ProvisioningState.stateStr == 'POST') {
                  if (mestate.ProvisioningMode) {
                    if (mestate.controlmode) {
                      if (mestate.ProvisioningMode.modeStr == 'ENTERPRISE') {
                        str +=
                          ', activated in ' +
                          ['none', 'Client Control Mode (CCM)', 'Admin Control Mode (ACM)', 'remote assistance mode'][
                            mestate.controlmode.controlMode
                          ];
                      } else {
                ...
                str += '.';
                if (mestate.net0 != null) {
                  str +=
                    '\r\nWired ' +
                    (mestate.net0.enabled == 1 ? 'Enabled' : 'Disabled') +
                    (mestate.net0.dhcpEnabled == 1 ? ', DHCP' : ', Static') +
                    ', ' +
                    mestate.net0.mac +
                    (mestate.net0.address == '0.0.0.0' ? '' : ', ' + mestate.net0.address);
                }
                if (mestate.net1 != null) {
                  str +=
                    '\r\nWireless ' +
                    (mestate.net1.enabled == 1 ? 'Enabled' : 'Disabled') +
                    (mestate.net1.dhcpEnabled == 1 ? ', DHCP' : ', Static') +
                    ', ' +
                    mestate.net1.mac +
                    (mestate.net1.address == '0.0.0.0' ? '' : ', ' + mestate.net1.address);
                }
                  ...
                  if (mestate.DnsSuffix != null) {
                    ...
                    for (var i in interfaces) {
                      for (var j in interfaces[i]) {
                        if (
                          interfaces[i][j].mac == mestate.net0.mac &&
                          interfaces[i][j].fqdn != null &&
                          interfaces[i][j].fqdn != ''
                        ) {
                          fqdn = interfaces[i][j].fqdn;
                        }
                      }
                    }
                    if (fqdn != null) {
                ...
                if (typeof mestate.networkStatus == 'number') {
                  ...
                  if (mestate.remoteAccessStatus > 0 && mestate.mpsHostname != null && mestate.mpsHostname.length > 0) {
                    str +=
                      ...
                      mestate.mpsHostname +
                      ', ' +
                      ['User initiated', 'Alert', 'Periodic', 'Provisioning'][mestate.remoteAccessTrigger];
                  }
                }
                console.log(str + '.');
                exit(0);
          ...
          console.log(
            'Unable to perform MEI operations, try running as ' +
              (process.platform == 'win32' ? 'administrator.' : 'root.')
          );
          exit(1);
          return;
        }
      ...
      console.log(
        'Unable to perform MEI operations, try running as ' + (process.platform == 'win32' ? 'administrator.' : 'root.')
      );
      exit(1);
      return;
    }
    ...
    getMeiState(15, function (state) {
      ...
      if (typeof args.post == 'string') {
        ...
        req.on('error', function (e) {
          console.log('amtinfojson error: ' + e);
          exit(1);
        });
    ...
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
      console.log('No or invalid \"password\" specified, use --password [password].');
      exit(1);
      return;
    }
    if (settings.hostname == null || typeof settings.hostname != 'string' || settings.hostname == '') {
      settings.hostname = '127.0.0.1';
    }
    if (settings.username == null || typeof settings.username != 'string' || settings.username == '') {
      settings.username = 'admin';
    }
    if (settings.output == null || typeof settings.output != 'string' || settings.output == '') {
      console.log('No or invalid \"output\" file specified, use --output [filename].');
      exit(1);
      return;
    }
    ...
    getMeiState(0, function (xstate) {
      // Flags: 1 = Versions, 2 = OsAdmin, 4 = Hashes, 8 = Network
      if (xstate == null) {
        console.log(
          'Unable to get Intel AMT state, try running as ' + (process.platform == 'win32' ? 'administrator.' : 'root.')
        );
        exit(1);
        return;
      }
      startLms(
        function (state) {
          console.log(
            [
              'MicroLMS did not start. Must run as administrator or LMS already active.',
              'MicroLMS started.',
              'MicroLMS started, MeshCommander on HTTP/16994.',
              'MEI error'
            ][state]
          );
          if ((state == 0 || state == 3) && settings.noconsole === true) {
            exit(0);
          } else {
    ...
    if (settings.agent == null || typeof settings.agent != 'string' || settings.agent == '') {
      console.log('No or invalid \"agent\" specified, use --agent [agent].');
      exit(1);
      return;
    }
    ...
    // Start Intel AMT configuration
    if (settings.url == null || typeof settings.url != 'string' || settings.url == '') {
      console.log('No MeshCentral server URL specified, use --url [url].');
      exit(1);
      return;
    }
    if (settings.id == null || typeof settings.id != 'string' || settings.id == '') {
      console.log('No device group identifier specified, use --id [identifier].');
      exit(1);
      return;
    }
    ...
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
      console.log('No or invalid parameters specified, use --password [password] or --url [url].');
      exit(1);
      return;
    }
    settings.protocol = 'http:';
    ...
    if (settings.type == null || typeof settings.type != 'string' || settings.type == '') {
      console.log('Unprovisioning \"type\" must be specified, use --type [partial/full].');
      exit(1);
      return;
    }
    if (settings.username == null || typeof settings.username != 'string' || settings.username == '') {
    ...
    if (settings.scan == null || typeof settings.scan != 'string' || settings.scan == '') {
      console.log('No or invalid \"scan\" specified, use --scan [ip range].');
      exit(1);
      return;
    }
    console.log('Scanning: ' + settings.scan + '...');
    var AMTScannerModule = require('amt-scanner');
    var amtscanner = new AMTScannerModule(),
      r = '';
    amtscanner.scan(settings.scan, 2000, function (data) {
      if (data.length > 0) {
        ...
        for (var i in data) {
          ...
          if (x.provisioningState < 3) {
            r += ', ' + pstates[x.provisioningState];
          }
          if (x.provisioningState == 2) {
            r += ', ' + x.openPorts.join(', ');
          }
          r += '.';
        }
  ...
  } else if (settings.action == 'amtauditlog') {
    // Read the Intel AMT audit log
    if (settings.hostname != null) {
      if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
        console.log('No or invalid \"password\" specified, use --password [password].');
        exit(1);
    ...
    readAmtAuditLog();
  } else if (settings.action == 'amteventlog') {
    // Read the Intel AMT audit log
    if (settings.hostname != null) {
      if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
        console.log('No or invalid \"password\" specified, use --password [password].');
    ...
    readAmtEventLog();
  } else if (settings.action == 'amtider') {
    // Remote mount IDER image
    if (settings.hostname == null || typeof settings.hostname != 'string' || settings.hostname == '') {
      console.log('No or invalid \"hostname\" specified, use --hostname [password].');
      exit(1);
      return;
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    if (settings.floppy == null || typeof settings.floppy != 'string' || settings.floppy == '') {
      settings.floppy = null;
    }
    if (settings.cdrom == null || typeof settings.cdrom != 'string' || settings.cdrom == '') {
      settings.cdrom = null;
    }
    if (settings.floppy == null && settings.cdrom == null) {
      console.log('No or invalid \"floppy\" or \"cdrom\" specified, use --floppy [file] or --cdrom [file].');
      exit(1);
      return;
    }
    performIder();
  } else if (settings.action == 'amtnetwork') {
    // Perform remote Intel AMT wired IPv4 configuration operation
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
  ...
  } else if (settings.action == 'amtwifi') {
    // Perform remote Intel AMT Wifi configuration operation
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    if (args.add != null) {
      if (args.name == null || typeof args.name != 'string' || args.name == '') {
        console.log('Wifi profile name is required.');
        exit(1);
        return;
      }
      if (args.ssid == null || typeof args.ssid != 'string' || args.ssid == '') {
        console.log('Wifi SSID is required.');
        exit(1);
        return;
      }
      if (args.psk == null || typeof args.psk != 'string' || args.psk == '') {
        console.log('Wifi password is required.');
        exit(1);
        return;
      }
    }
  ...
  } else if (settings.action == 'amtwake') {
    // Perform remote Intel AMT wake alarm operations
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    if (args.del != null && (typeof args.del != 'string' || args.del == '')) {
      console.log('Alarm name is required (--del [name]).');
      exit(1);
      return;
    }
    if (args.add != null) {
      if (typeof args.add != 'string' || args.add == '') {
        console.log('Wake alarm name is required (--add [name]).');
        exit(1);
        return;
      }
      if (typeof args.date != 'string' || args.data == '') {
        console.log('Wake alarm date is required (--date [yyyy-mm-dd]).');
        exit(1);
        return;
      }
    }
    performAmtWakeConfig(args);
  } else if (settings.action == 'amtrpe') {
    // Perform Intel AMT remote platform erase operations
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
  ...
  } else if (settings.action == 'amtddns') {
    // Perform Intel AMT dynamic DNS get/set
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    if (args.set != null) {
      args.set = args.set.toLowerCase();
    }
    if (args.set != null && args.set != 'enabled' && args.set != 'dhcp' && args.set != 'disabled') {
      console.log('Intel AMT DDNS can only bet set to "enabled", "dhcp" or "disabled".');
    }
    if (args.interval != null) {
      args.interval = parseInt(args.interval);
      if (typeof args.interval != 'number' || isNaN(args.interval)) {
        console.log('Interval must be a number.');
        exit(1);
        return;
      }
      if (args.interval < 20) {
        console.log('Interval must be at least 20 minutes.');
        exit(1);
        return;
      }
    }
    if (args.ttl != null) {
      args.ttl = parseInt(args.ttl);
      if (typeof args.ttl != 'number' || isNaN(args.ttl)) {
        console.log('TTL must be a number.');
        exit(1);
        return;
      }
    }
    performAmtDynamicDNS(args);
  } else if (settings.action == 'amtfeatures') {
    // Perform remote Intel AMT feature configuration operation
    if (settings.hostname == null) {
      settings.hostname = '127.0.0.1';
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    performAmtTerm(args);
  } else if (settings.action == 'amtpower') {
    // Perform remote Intel AMT power operation
    if (settings.hostname == null || typeof settings.hostname != 'string' || settings.hostname == '') {
      console.log('No or invalid \"hostname\" specified, use --hostname [host].');
      exit(1);
      return;
    }
    if (settings.password == null || typeof settings.password != 'string' || settings.password == '') {
    ...
    if (args.poweron) {
      settings.poweraction = 2;
    }
    if (args.sleep) {
      settings.poweraction = 3;
    }
    if (args.powercycle) {
      settings.poweraction = 5;
    }
    if (args.poweroff) {
      settings.poweraction = 8;
    }
    if (args.hibernate) {
      settings.poweraction = 7;
    }
    if (args.reset) {
      settings.poweraction = 10;
    }
    ...
    if (args.bootdevice) {
      if (bootdevices.indexOf(args.bootdevice.toLowerCase()) >= 0) {
        ...
        console.log('Supported boot devices are pxe, hdd, cd, ider-floppy, ider-cdrom');
        exit(1);
        return;
      }
    }
...
function performAmtTrustedHashes() {
  ...
  if (typeof settings.create == 'string') {
    if (settings.name == null || typeof settings.name != 'string' || settings.name == '') {
      console.log('No or invalid \"name\" specified, use --name [name].');
      exit(1);
      return;
    }
    if (
      settings.create.length != 32 &&
      settings.create.length != 40 &&
      settings.create.length != 64 &&
      settings.create.length != 96
    ) {
      console.log('No or invalid \"create\" hash, must be in HEX format of length 30, 40, 64, 96.');
      exit(1);
      return;
    }
    if (Buffer.from(settings.create, 'hex').toString('hex') != settings.create.toUpperCase()) {
      console.log('No or invalid \"create\" specified, must be in HEX format.');
      exit(1);
      return;
    }
  ...
  if (typeof settings.delete == 'string') {
    if (
      settings.delete.length != 32 &&
      settings.delete.length != 40 &&
      settings.delete.length != 64 &&
      settings.delete.length != 96
    ) {
      console.log('No or invalid \"delete\" hash, must be in HEX format of length 30, 40, 64, 96.');
      exit(1);
      return;
    }
    if (Buffer.from(settings.delete, 'hex').toString('hex') != settings.delete.toUpperCase()) {
      console.log('No or invalid \"delete\" specified, must be in HEX format.');
      exit(1);
      return;
    }
  ...
  if (settings.hostname == '127.0.0.1' || settings.hostname.toLowerCase() == 'localhost') {
    settings.noconsole = true;
    startLms(performAmtTrustedHashesEx);
  } else {
...
function performAmtTrustedHashesEx2(stack, name, responses, status) {
  if (status != 200) {
    ...
    if (settings.create) {
      ...
      var instanceId = null;
      for (var i in r) {
        if (Buffer.from(r[i]['HashData'], 'base64').toString('hex') == settings.create) {
          instanceId = r[i]['InstanceID'];
        }
      }
      if (instanceId != null) {
        console.log('This trusted hash is already present.');
        exit(1);
        return;
      }
      ...
      if (hashtype == -1) {
        console.log('Invalid hash type', hash.length);
        exit(1);
        return;
      }
      ...
      amtstack.Create('AMT_ProvisioningCertificateHash', instance, function (stack, name, response, status) {
        if (status != 200) {
          console.log('ERROR: Failed to create trusted hash.', status, JSON.stringify(response, null, 2));
        } else {
          console.log('Done.');
        }
        exit(0);
      ...
      var instance = null;
      for (var i in r) {
        if (Buffer.from(r[i]['HashData'], 'base64').toString('hex') == settings.delete) {
          instance = r[i];
        }
      }
      if (instance == null) {
        console.log('This trusted hash not present.');
        exit(1);
        return;
      }
      ...
      amtstack.Delete('AMT_ProvisioningCertificateHash', instance, function (stack, name, response, status) {
        if (status != 200) {
          console.log('ERROR: Failed to delete trusted hash.', status, JSON.stringify(response, null, 2));
        } else {
          console.log('Done.');
        }
        exit(0);
      ...
      for (var i in r) {
        ...
        console.log(
          r[i]['Description'] +
            ...
            certState.join(', ') +
            ')\r\n  ' +
            hashTypes[r[i]['HashType']] +
            ': ' +
            Buffer.from(r[i]['HashData'], 'base64').toString('hex')
        );
      }
    }
    exit(0);
...
function performAmtAgentPresence() {
  startLms(function () {
    tempWatchdogTimer = setTimeout(performAmtAgentPresenceRegister, 3000);
  });
}
...
function performAmtAgentPresenceRegister() {
  ...
  watchdog = {
    DeviceID: Buffer.from(guidToStr(settings.agent.split('-').join('')).split('-').join(''), 'hex').toString('base64'),
    Retry: 0
  };
  amtstack.AMT_AgentPresenceWatchdog_RegisterAgent(performAmtAgentPresenceRegisterRetry, watchdog, watchdog.Seq, {
...
function performAmtAgentPresenceRegisterRetry(stack, name, response, status, watchdog) {
  if (status == 200 && response.Body.SessionSequenceNumber && response.Body.TimeoutInterval) {
    ...
    watchdog.Retry++;
    if (watchdog.Retry < 5) {
      tempWatchdogTimer = setTimeout(function () {
        amtstack.AMT_AgentPresenceWatchdog_RegisterAgent(performAmtAgentPresenceRegisterRetry, watchdog, watchdog.Seq, {
          DeviceID: watchdog.DeviceID
        });
      }, 1000);
...
function performAmtAgentPresenceAssert() {
  watchdog.Seq++;
  amtstack.AMT_AgentPresenceWatchdog_AssertPresence(watchdog.Seq, performAmtAgentPresenceAssertRetry, watchdog, 0, {
    DeviceID: watchdog.DeviceID
  });
}
...
function performAmtAgentPresenceEx5(stack, name, response, status, watchdog) {
  ...
  if (status == 200) {
    ...
    watchdog.Retry++;
    if (watchdog.Retry < 5) {
      amtstack.AMT_AgentPresenceWatchdog_AssertPresence(watchdog.Seq, performAmtAgentPresenceEx4, watchdog, 0, {
        DeviceID: watchdog.DeviceID
      });
    } else {
      console.log('Failed to assert presence on this watchdog.');
    ...
    if (settings.json) {
      ...
      for (var i in messages) {
        out +=
          messages[i].Time +
          ', ' +
          messages[i].EntityStr +
          ', ' +
          messages[i].Desc +
          ', ' +
          messages[i].EventSeverity +
          '\r\n';
      }
    }
    if ((settings.output == null || settings.output == '') && !settings.uuidoutput) {
      console.log(out);
      exit(1);
    } else {
      try {
        if (settings.output) {
          ...
          stack.Get('CIM_ComputerSystemPackage', function (obj, name, response, xstatus, tag) {
            if (xstatus == 200) {
              var eventlogsfile = path.join(
                destpath,
                guidToStr(response.Body.PlatformGUID.toLowerCase() + '_Event' + (settings.json ? '.json' : '.csv'))
              );
    ...
    if ((settings.output == null || settings.output == '') && !settings.uuidoutput) {
      ...
      try {
        if (settings.output) {
          ...
          stack.Get('CIM_ComputerSystemPackage', function (obj, name, response, xstatus, tag) {
            if (xstatus == 200) {
              var auditlogsfile = path.join(
                destpath,
                guidToStr(response.Body.PlatformGUID.toLowerCase() + '_Audit' + (settings.json ? '.json' : '.csv'))
              );
  ...
  webServer.on('upgrade', function (req, socket, head) {
    ...
    switch (req.url.split('?')[0]) {
      case '/webrelay.ashx': // MeshCommander relay channel
        ...
        if (webargs.p) {
          webargs.p = parseInt(webargs.p);
        }
        if (webargs.tls) {
          webargs.tls = parseInt(webargs.tls);
        }
        if (webargs.tls1only) {
          webargs.tls1only = parseInt(webargs.tls1only);
        }
        if (webargs.port) {
          webargs.port = parseInt(webargs.port);
        }
        ...
        if (webargs.tls == 0) {
          ...
          ws.forwardclient.on('connect', function () {
            debug(1, 'Connected TCP to ' + webargs.host + ':' + webargs.port + '.');
            this.pipe(this.ws, { end: false });
            this.ws.pipe(this, { end: false });
          });
          ws.forwardclient.on('error', function () {
            debug(1, 'TCP connection error to ' + webargs.host + ':' + webargs.port + '.');
            try {
              this.ws.end();
            } catch (e) {}
          });
          ...
          if (webargs.tls1only == 1) {
            tlsoptions.secureProtocol = 'TLSv1_method';
          }
          ws.forwardclient = tls.connect(tlsoptions, function () {
            debug(1, 'Connected TLS to ' + webargs.host + ':' + webargs.port + '.');
            this.pipe(this.ws, { end: false });
            this.ws.pipe(this, { end: false });
          });
          ws.forwardclient.on('error', function () {
            debug(1, 'TLS connection error to ' + webargs.host + ':' + webargs.port + '.');
            try {
              this.ws.end();
            } catch (e) {}
          });
        ...
        // Handle pipe closure
        ws.on('end', function () {
          debug(1, 'Disconnected from ' + webargs.host + ':' + webargs.port + '.');
          try {
            this.forwardclient.end();
          } catch (e) {}
          delete webServer.wsList[this.wsIndex];
        });
        ws.forwardclient.on('end', function () {
          try {
            this.ws.end();
          } catch (e) {}
        });
  ...
  webServer.on('request', function (req, rsp) {
    ...
    switch (req.url.split('?')[0]) {
      case '/': // Serve MeshCommander Web Application
        ...
        try {
          meshcommander = fs.readFileSync('meshcommander.htm');
        } catch (e) {}
        if (meshcommander != null) {
          ...
          rsp.writeHead(200, 'OK', {
            ...
            'Transfer-Encoding': 'chunked',
            ETag: FullSite_IntelAmtLocalWebApp_etag
          });
...
function configureAmt() {
  ...
  getMeiState(15, function (state) {
    ...
    if (state.ProvisioningState == null) {
      console.log('Intel AMT not ready for configuration.');
      exit(1);
      return;
    }
...
function configureAmt2(err, state) {
  ...
  if (typeof settings.password == 'string' && settings.password != '') {
    ...
    if (typeof settings.username == 'string' && settings.username != '') {
      state.amtuser = settings.username;
    }
  }

  // If a DNS suffix override is provided, use that
  if (settings.dnssuffix != null) {
    state.DnsSuffix = settings.dnssuffix;
  }

  // If a description is provided, send it to the server
  if (typeof settings.desc == 'string' && settings.desc != '') {
    state.desc = settings.desc;
  }
  ...
  if (apfarg.clientuuid == null || apfarg.clientuuid.length != 36) {
    ...
    settings.apftunnel.onChannelClosed = function () {
      exit(0);
    };
    try {
...
function configureJsonControl(data) {
  switch (data.action) {
    ...
    case 'mestate': // Request an updated MEI state
      getMeiState(15, function (state) {
        settings.apftunnel.updateMeiState(state);
      });
      break;
    case 'deactivate': // Request CCM deactivation
      ...
      try {
        ...
        settings.apftunnel.sendMeiDeactivationState(1);
        break;
      }
      amtMei.on('error', function (e) {
        settings.apftunnel.sendMeiDeactivationState(1);
      ...
      amtMei.unprovision(1, function (status) {
        settings.apftunnel.sendMeiDeactivationState(status);
      }); // 0 = Success
      break;
    case 'startTlsHostConfig': // Request start of host based TLS ACM activation
      ...
      try {
        ...
        settings.apftunnel.sendStartTlsHostConfigResponse({ state: -103 });
        break;
      }
      amtMei.on('error', function (e) {
        settings.apftunnel.sendStartTlsHostConfigResponse({ state: -104 });
      });
      amtMei.startConfigurationHBased(
...
function deactivateCCM() {
  ...
  amtMei.on('error', function (e) {
    console.log('deactivateCCM() error: ' + e);
    exit(1);
    return;
  });
  amtMei.unprovision(1, function (status) {
...
function activeToCCMEx2(stack, name, responses, status) {
  if (status != 200) {
    ...
    console.log('Client control mode activation not allowed');
    exit(1);
  } else {
    osamtstack.IPS_HostBasedSetupService_Setup(
      2,
      md5hex('admin:' + responses['AMT_GeneralSettings'].response['DigestRealm'] + ':' + settings.password).substring(
        0,
        32
      ),
      null,
  ...
  amtstack.Get('AMT_SetupAndConfigurationService', function (stack, name, responses, status) {
    if (status !== 200) {
      if (responses != null && responses.Header != null && typeof responses.Header.error == 'string') {
        ...
        if (status == 600) {
          console.log('Check that Intel AMT is in ACM mode and that the password is correct.');
        }
      } else {
      ...
      if (sacs.ZeroTouchConfigurationEnabled == true) {
        sacs.ZeroTouchConfigurationEnabled = true;
      }
      amtstack.Put('AMT_SetupAndConfigurationService', sacs, function (stack, name, responses, status) {
        if (settings.type == 'full') {
          amtstack.AMT_SetupAndConfigurationService_Unprovision(2, function (stack, name, responses, status) {
            if (status != 200) {
              console.log('Failed to fully unconfigure AMT, status ' + status);
              exit(1);
            } else if (responses.Body.ReturnValue != 0) {
              console.log('Unprovision failed: ' + responses.Body.ReturnValueStr);
              exit(1);
            } else {
              console.log('AMT fully unprovisioned.');
              exit(0);
            }
          });
        } else {
    ...
    for (var i = 0; i < handles.length; ++i) {
      this.getCertHashEntry(handles[i], function (result) {
        if (result.isActive == 1) {
          trustedHashes.push(result.certificateHash.toLowerCase());
        }
        if (--exitOnCount == 0) {
          func(tag);
        }
...
function getAmtUuidEx() {
  ...
  amtstack.Get('CIM_ComputerSystemPackage', function (obj, name, response, xstatus, tag) {
    if (xstatus == 200) {
      console.log('GUID: ' + guidToStr(response.Body.PlatformGUID.toLowerCase()));
    } else {
      console.log('Intel AMT is not available or not activated.');
    }
    exit(1);
  });
}
...
function onWsmanProcessChanged(a, b) {
  var x = Math.floor((a * 100) / b);
  if (x < IntelAmtEntireStateProgress) {
    IntelAmtEntireStateProgress = x;
    console.log(100 - x + '%');
  }
}
function saveEntireAmtStateOk2(stack, name, responses, status) {
  if (status == 600) {
    console.log('ERROR: Unable to connect to Intel(R) AMT.');
    exit(2);
  }
  IntelAmtEntireState['wsmanenums'] = responses;
  saveEntireAmtStateDone();
}
function saveEntireAmtStateOk3(stack, messages, status) {
  if (status == 600) {
    console.log('ERROR: Unable to connect to Intel(R) AMT.');
    exit(2);
  }
  IntelAmtEntireState['auditlog'] = messages;
  saveEntireAmtStateDone();
}
function saveEntireAmtStateOk4(stack, messages, tag, status) {
  if (status == 600) {
    console.log('ERROR: Unable to connect to Intel(R) AMT.');
    exit(2);
  }
  IntelAmtEntireState['eventlog'] = messages;
  saveEntireAmtStateDone();
}
...
function getAmtInfo(func, tag) {
  ...
  if (getAmtInfoFetching != null) {
    getAmtInfoFetching.push({ f: func, t: tag });
    return;
  }
  ...
  getAmtInfoFetchingTimer = setTimeout(function () {
    ...
    for (var i in getAmtInfoFetching) {
      if (getAmtInfoFetching[i].f != null) {
        getAmtInfoFetching[i].f(amtMeiTmpState, getAmtInfoFetching[i].t);
      }
    }
    ...
    amtMei.on('error', function (e) {
      console.log('getAmtInfo() error: ' + e);
      exit(1);
      return;
    });
  }, 3000);
  amtMei.getProtocolVersion(function (result) {
    if (result != null) {
      amtMeiTmpState.MeiVersion = result;
    }
  });
  amtMei.getVersion(function (val) {
    ...
    if (val != null) {
      for (var version in val.Versions) {
        amtMeiTmpState.Versions[val.Versions[version].Description] = val.Versions[version].Version;
      }
      amtMei.getProvisioningMode(function (result) {
        if (result != null) {
      ...
      amtMei.getHashHandles(function (handles) {
        ...
        for (var i = 0; i < handles.length; ++i) {
          amtMei.getCertHashEntry(handles[i], function (result) {
            amtMeiTmpState.TrustedHashes.push({
            ...
            if (--exitOnCount == 0) {
              amtMeiTmpState.Notifications = lmsNotifications;
...
function startLms(func, lmscommander, tag) {
  ...
  try {
    amtLms = new lme_heci({ debug: settings.lmsdebug, bindany: settings.bindany });
  } catch (ex) {
    if (func != null) {
      func(0, tag);
    }
  ...
  amtLms.on('notify', function (data, options, str, code) {
    if (code == 'iAMT0052-3') {
      ...
      while (lmsNotifications.length > 100) {
        lmsNotifications.shift();
      }
      var notifyBuf = Buffer.concat([Buffer.from('0900', 'hex'), Buffer.from(JSON.stringify(notify))]); // Add a notification
      for (var i in lmsControlSockets) {
        lmsControlSockets[i].write(notifyBuf);
      }
    }
  });
  ...
  amtLms.meshCommander.on('upgrade', function (req, socket, head) {
    ...
    switch (req.url.split('?')[0]) {
      case '/lms.ashx': // MeshCommander control channel (PTHI)
        ...
        socket.ws.on('end', function () {
          if (lmsControlSockets[this.id]) {
            delete lmsControlSockets[this.id]; /*console.log('removeControl', JSON.stringify(lmsControlSockets));*/
          }
        });
      ...
      case '/webrelay.ashx': // MeshCommander data channel (LME)
        ...
        if (amtLms.connected == true) {
          ...
          socket.relay.on('connect', function () {
            socket.ws.on('data', function (data) {
              socket.relay.write(data);
            });
            socket.ws.on('end', function () {
              socket.relay.end();
            });
          });
          socket.relay.on('data', function (data) {
            socket.ws.write(data);
          });
          socket.relay.on('close', function () {
            socket.ws.end();
          });
        }
        break;
  ...
  amtLms.meshCommander.on('request', function (req, rsp) {
    ...
    switch (req.url.split('?')[0]) {
      case '/': // Serve MeshCommander Web Application for LMS
        ...
        try {
          lmscommander = fs.readFileSync('lmscommander.htm');
        } catch (e) {}
...
function setupMeiOsAdmin(func, state, tag) {
  if (amtMei == null) {
    ...
    amtMei.getLocalSystemAccount(function (x) {
      if (x == null || x.user == null || x.pass == null) {
        if (func) {
          func(state, tag);
        }
        return;
      } // No OsAdmin, stop here.
      var transport = require('amt-wsman-duk');
      var wsman = require('amt-wsman');
      var amt = require('amt');
      ...
      amtMei.getVersion(function (x) {
        var amtver = null;
        try {
          for (var i in x.Versions) {
            if (x.Versions[i].Description == 'AMT') amtver = parseInt(x.Versions[i].Version.split('.')[0]);
          }
        } catch (e) {}
        if (amtver != null && amtver >= 12) {
          ...
          tempTimer = setInterval(function () {
            kvmGetData();
          }, 2000); // Start polling for KVM data.
...
function kvmDataGetResponse(stack, name, response, status, tag) {
  if (tag != 'skip' && status == 200 && response.Body.ReturnValue == 0) {
    var val = null;
    try {
      val = Buffer.from(response.Body.DataMessage, 'base64').toString();
    } catch (e) {
      return;
    }
    if (val != null) {
      kvmProcessData(response.Body.RealmsBitmap, response.Body.MessageId, val);
    }
  }
}
...
function kvmProcessData(realms, messageId, val) {
  ...
  if (data != null && data.action) {
    if (data.action == 'present') {
      kvmSetData(JSON.stringify({ action: 'present', ver: 1, platform: process.platform }));
    }
    if (data.action == 'offer') {
      ...
      webRtcDesktop.webrtc.on('dataChannel', function (rtcchannel) {
        ...
        webRtcDesktop.rtcchannel.on('end', function () {
          webRtcCleanUp();
        });
        webRtcDesktop.rtcchannel.on('data', function (x) {
          kvmCtrlData(this, x);
        });
...
function kvmCtrlData(channel, cmd) {
  if (cmd.length > 0 && cmd.charCodeAt(0) != 123) {
    ...
    if (this.fileupload != null) {
      ...
      if (header == 0x01000000 || header == 0x01000001) {
        ...
        if (header == 0x01000001) {
          fs.closeSync(this.fileupload.fp);
  ...
  try {
    ...
    console.error('Invalid JSON: ' + cmd);
    return;
  }
  ...
  switch (cmd.action) {
    ...
    case 'download': {
      ...
      if (cmd.sub == 'start') {
        ...
        if (this.filedownload != null) {
          channel.write({ action: 'download', sub: 'cancel', id: this.filedownload.id });
        ...
        if (this.filedownload) {
          channel.write({ action: 'download', sub: 'start', id: cmd.id });
        }
    ...
    case 'upload': {
      ...
      if (cmd.sub == 'start') {
        ...
        try {
          this.fileupload.fp = fs.openSync(filepath, 'wbN');
        } catch (e) {}
        if (this.fileupload.fp) {
          channel.write({ action: 'upload', sub: 'start', reqid: this.fileupload.reqid });
        } else {
          this.fileupload = null;
          channel.write({ action: 'upload', sub: 'error', reqid: this.fileupload.reqid });
        }
...
function webRtcCleanUp() {
  ...
  if (webRtcDesktop.rtcchannel) {
    try {
      webRtcDesktop.rtcchannel.close();
    } catch (e) {}
    try {
      webRtcDesktop.rtcchannel.removeAllListeners('data');
    } catch (e) {}
    try {
      webRtcDesktop.rtcchannel.removeAllListeners('end');
    } catch (e) {}
    delete webRtcDesktop.rtcchannel;
  }
  if (webRtcDesktop.webrtc) {
    try {
      webRtcDesktop.webrtc.close();
    } catch (e) {}
    try {
      webRtcDesktop.webrtc.removeAllListeners('connected');
    } catch (e) {}
    try {
      webRtcDesktop.webrtc.removeAllListeners('disconnected');
    } catch (e) {}
    try {
      webRtcDesktop.webrtc.removeAllListeners('dataChannel');
    } catch (e) {}
    delete webRtcDesktop.webrtc;
  }
  if (webRtcDesktop.kvm) {
    try {
      webRtcDesktop.kvm.end();
    } catch (e) {}
...
function startLmsWsmanResponse(stack, name, responses, status) {
  if (status == 600) {
    ...
    console.log('ERROR: Unable to get object from Intel(R) AMT, status = ' + status + '.');
  } else {
...
function processLmsControlData(data) {
  ...
  switch (cmdid) {
    case 1: { // Request basic Intel AMT information (CMD = 1)
      getAmtInfo(function (meinfo, socket) {
        meinfo.LoginMode = 2;
        socket.write(Buffer.concat([Buffer.from('0100', 'hex'), Buffer.from(JSON.stringify(meinfo))]));
      }, this);
      break;
    }
    case 2: { // Intel AMT MEI Unprovision (CMD = 2)
      ...
      amtMei.unprovision(
        ...
        function (status, socket) {
          ...
          data.writeUInt16LE(2, 0);
          data.writeUInt32LE(status, 2);
          socket.write(data);
        },
        this
      );
    ...
    case 3: { // Intel AMT MEI GetLocalSystemAccount (CMD = 3)
      amtMei.getLocalSystemAccount(function (account, socket) {
        socket.write(Buffer.concat([Buffer.from('030000000000', 'hex'), account.raw]));
      }, this);
      break;
    }
    case 4: { // Instruct Intel AMT to start remote configuration (CMD = 4)
      amtMei.startConfiguration(function (status, socket) {
        var data = Buffer.alloc(6);
        data.writeUInt16LE(7, 0);
        data.writeUInt32LE(status, 2);
        socket.write(data);
      }, this);
      break;
    ...
    case 5: { // Instruct Intel AMT to stop remote configuration (CMD = 5)
      amtMei.stopConfiguration(function (status, socket) {
        var data = Buffer.alloc(6);
        data.writeUInt16LE(7, 0);
        data.writeUInt32LE(status, 2);
        socket.write(data);
    ...
    case 6: { // Instruct Intel AMT connect CIRA (CMD = 6)
      amtMei.openUserInitiatedConnection(function (status, socket) {
        var data = Buffer.alloc(6);
        data.writeUInt16LE(7, 0);
        data.writeUInt32LE(status, 2);
        socket.write(data);
    ...
    case 7: { // Instruct Intel AMT disconnect CIRA (CMD = 7)
      amtMei.closeUserInitiatedConnection(function (status, socket) {
        var data = Buffer.alloc(6);
        data.writeUInt16LE(7, 0);
        data.writeUInt32LE(status, 2);
        socket.write(data);
    ...
    case 8: { // Get Intel AMT CIRA State (CMD = 8)
      amtMei.getRemoteAccessConnectionStatus(function (state, socket) {
        var data = Buffer.alloc(6);
        data.writeUInt16LE(8, 0);
        data.writeUInt32LE(state.status, 2);
        socket.write(Buffer.concat([data, state.raw]));
      }, this);
      break;
    }
    default:
...
function startRouter() {
  ...
  try {
    ...
    if (settings.serverid == null) {
      ...
      if (settings.authcookie != null) {
        ...
        if (xtoken != null) {
          options.headers = {
            host: options.host,
            'x-meshauth':
              Buffer.from(settings.username, 'binary').toString('base64') +
              ',' +
              Buffer.from(settings.password, 'binary').toString('base64') +
              ',' +
              Buffer.from(xtoken, 'binary').toString('base64')
          };
        } else {
          options.headers = {
            host: options.host,
      ...
      options.headers = { host: options.host, 'x-meshauth': '*' };
    } // Request inner authentication
    if (settings.loginkey) {
      xurlargs.push('key=' + settings.loginkey);
    }
    if (xurlargs.length > 0) {
      options.path += '?' + xurlargs.join('&');
    }
  } catch (e) {
    console.log('Unable to parse "serverUrl".');
    exit(1);
    return;
  }
  ...
  settings.websocket.on('error', function (ex) {
    console.log('Unable to connect to server: ' + JSON.stringify(ex));
    exit(1);
    return;
  });
  settings.websocket.on('response', function (rsp) {
    console.log('Unable to connect to server: ' + rsp.statusMessage + ' (' + rsp.statusCode + ')');
    exit(1);
    return;
  });
...
function OnServerWebSocket(msg, s, head) {
    ...
    switch (command.action) {
      case 'close': {
        if (command.cause == 'noauth') {
          ...
          console.log('Server disconnected: ' + command.msg);
        }
        exit(1);
        return;
      }
      ...
      case 'authcookie': {
        if (settings.acookie == null) {
          ...
          settings.renewCookieTimer = setInterval(function () {
            settings.webchannel.write('{"action":"authcookie"}');
          }, 600000); // Ask for new cookie every 10 minutes
          startRouterEx();
        } else {
          settings.acookie = command.cookie;
          settings.rcookie = command.rcookie;
      ...
      case 'serverAuth': {
        ...
        if (cert.getKeyHash().toString('hex') != settings.serverid) {
          console.log('Unable to authenticate the server, invalid server identifier.');
          exit(1);
          return;
        }

        // Hash the signed data and verify the server signature
        var signDataHash = hasher.syncHash(
          Buffer.concat([
            Buffer.from(settings.serverAuthClientNonce, 'base64'),
            Buffer.from(settings.meshServerTlsHash, 'hex'),
            Buffer.from(command.nonce, 'base64')
          ])
        );
        if (
          require('RSA').verify(
            require('RSA').TYPES.SHA384,
            cert,
            signDataHash,
            Buffer.from(command.signature, 'base64')
          ) == false
        ) {
          console.log('Unable to authenticate the server, invalid signature.');
          exit(1);
          return;
        }
  ...
  if (settings.serverid != null) {
    ...
    s.write(
      '{"action":"serverAuth","cnonce":"' +
        settings.serverAuthClientNonce +
        '","tlshash":"' +
        settings.meshServerTlsHash +
        '"}'
    ); // Ask for server authentication
  }
}
...
function startRouterEx() {
  ...
  tcpserver.on('error', function (e) {
    console.log('startRouterEx() Error: ' + JSON.stringify(e));
    exit(0);
    return;
  });
  try {
    ...
    console.log('Unable to bind to local TCP port ' + settings.localport + '.');
    exit(1);
    return;
  }
}
...
function OnTcpClientConnected(c) {
  try {
    ...
    try {
      options = http.parseUri(
        settings.serverurl +
          ...
          '&tcpport=' +
          settings.remoteport +
          (settings.remotetarget == null ? '' : '&tcpaddr=' + settings.remotetarget)
      );
    } catch (e) {
      console.log('Unable to parse "serverUrl".');
      exit(1);
    ...
    c.websocket.on('error', function (e) {
      console.log('OnTcpClientConnected() Error: ' + JSON.stringify(e));
    });
    c.websocket.end();
  } catch (e) {
    debug(2, e);
  ...
  s.on('data', function (msg) {
    if (this.parent.tunneling == false) {
      ...
      if (msg == 'c' || msg == 'cr') {
        ...
        this.pipe(this.parent.tcp, { dataTypeSkip: 1 });
...
function discoverMeshServer() {
  console.log('Looking for server...');
  discoveryInterval = setInterval(discoverMeshServerOnce, 5000);
  discoverMeshServerOnce();
}
...
function performIder() {
  if (settings.floppy != null && fs.existsSync(settings.floppy) == false) {
    console.log('Unable to floppy image file: ' + settings.floppy);
    exit();
    return;
  }
  if (settings.cdrom != null && fs.existsSync(settings.cdrom) == false) {
    console.log('Unable to CDROM image file: ' + settings.cdrom);
    exit();
    return;
  }
  var iderStarts = ['onreboot', 'graceful', 'now'];
  if (settings.iderstart != null && iderStarts.indexOf(settings.iderstart) < 0) {
    console.log('Unknown iderstart option: ' + settings.iderstart);
    exit();
    return;
  }
  try {
    ...
    if (settings.floppy) {
      try {
        if ((sfloppy = fs.statSync(settings.floppy))) {
          sfloppy.file = fs.openSync(settings.floppy, 'rbN');
        }
      } catch (ex) {
        console.log(ex);
        exit(1);
    ...
    if (settings.cdrom) {
      try {
        scdrom = fs.statSync(settings.cdrom);
        if (scdrom) {
          scdrom.file = fs.openSync(settings.cdrom, 'rbN');
        }
      } catch (ex) {
        console.log(ex);
        exit(1);
...
function iderSectorStats(mode, dev, mediaBlocks, lba, len) {
  if (iderIdleTimer != null) {
    clearTimeout(iderIdleTimer);
  }
  iderIdleTimer = setTimeout(function () {
...
function performAmtNetConfig(args) {
  if (settings.hostname == '127.0.0.1' || settings.hostname.toLowerCase() == 'localhost') {
    settings.noconsole = true;
    startLms(performAmtNetConfig0, false, args);
  } else {
...
function performAmtNetConfig1(stack, name, response, status, args) {
  if (status == 200) {
    ...
    for (var y in response['AMT_EthernetPortSettings'].responses) {
      ...
      if (z['WLANLinkProtectionLevel'] || y == 1) {
        amtwirelessif = y;
      } // Set the wireless interface, this seems to cover new wireless only computers and older computers with dual interfaces.
      if (y == 0) {
        if (amtwirelessif != y && z['MACAddress'] != '00-00-00-00-00-00') {
          amtwiredif = y;
        }
      } // On computers with only wireless, the wired interface will have a null MAC
    }
    ...
    if (args) {
      ...
      if (y['IpSyncEnabled'] == false && args.ipsync === '1') {
        x['IpSyncEnabled'] = true;
        docall = true;
      }
      if (y['IpSyncEnabled'] == true && args.ipsync === '0') {
      ...
      if (
        args.dhcp &&
        amtwiredif != -1 &&
        response['AMT_EthernetPortSettings'].responses[amtwiredif].DHCPEnabled == false
      ) {
        // Change to DHCP
        x['DHCPEnabled'] = true;
        docall = true;
        ...
        args.static &&
        amtwiredif != -1 &&
        response['AMT_EthernetPortSettings'].responses[amtwiredif].DHCPEnabled == true
      ) {
        // Change to STATIC
        x['DHCPEnabled'] = false;
        if (args.ip) {
          ...
          console.log('Missing IPv4 address, use --ip 1.2.3.4');
          exit(1);
        }
        if (args.subnet) {
          x['SubnetMask'] = args.subnet;
        } else {
          console.log('Missing IPv4 subnet, use --subnet 255.255.255.0');
          exit(1);
        }
        if (args.gateway) {
          x['DefaultGateway'] = args.gateway;
        }
        if (args.dns) {
          x['PrimaryDNS'] = args.dns;
        }
        if (args.dns2) {
          x['SecondaryDNS'] = args.dns2;
        }
        docall = true;
      }
      if (docall) {
    ...
    if (pendingAmtConfigActions == 0) {
      var maxlen = 0;
      if (amtwiredif != -1) {
        for (var i in response['AMT_EthernetPortSettings'].responses[amtwiredif]) {
          if (i.length > maxlen) {
            maxlen = i.length;
          }
        }
      ...
      if (amtwirelessif != -1) {
        for (var i in response['AMT_EthernetPortSettings'].responses[amtwirelessif]) {
          if (i.length > maxlen) {
            maxlen = i.length;
          }
        }
...
function performAmtWifiConfig(args) {
  if (settings.hostname == '127.0.0.1' || settings.hostname.toLowerCase() == 'localhost') {
    settings.noconsole = true;
    startLms(performAmtWifiConfig0, false, args);
  } else {
...
function performAmtWifiConfig1(stack, name, response, status, args) {
  if (status == 600) {
    console.log('Unable to login, check that Intel AMT password is correct.');
    exit(1);
    return;
  }
  if (status == 200) {
    ...
    for (var y in response['CIM_WiFiEndpointSettings'].responses) {
      ...
      wifiProfiles[n] = {
        Priority: z['Priority'],
        SSID: z['SSID'],
        AuthenticationMethod: z['AuthenticationMethod'],
        EncryptionMethod: z['EncryptionMethod']
      };
    }
    ...
    if (args) {
      if (args.add) {
        if (args.auth == null) {
          args.auth = 6;
        } // if not set, default to WPA2 PSK
        if (args.enc == null) {
          args.enc = 3;
        } // if not set, default to TKIP
        ...
        stack.AMT_WiFiPortConfigurationService_AddWiFiSettings(
          wifiep,
          wifiepsettinginput,
          null,
          null,
        ...
        for (var t in wifiProfiles) {
          ...
          console.log(
            'Profile Name: ' +
              ...
              w['SSID'] +
              '; Security: ' +
              wifiAuthMethod[w['AuthenticationMethod']] +
              '/' +
              wifiEncMethod[w['EncryptionMethod']]
          );
          wikiProfileCount++;
        }
        if (wikiProfileCount == 0) {
          console.log('  (No Wifi profiles stored)');
        }
        exit(0);
      }
    } else {
...
function prepareAlarmOccurenceTemplate(id, nm, start, interval, del) {
  return (
    ...
    id +
    '</s:InstanceID><s:StartTime><p:Datetime xmlns:p=\"http://schemas.dmtf.org/wbem/wscim/1/common\">' +
    start +
    '</p:Datetime></s:StartTime><s:Interval><p:Interval xmlns:p=\"http://schemas.dmtf.org/wbem/wscim/1/common\">' +
    interval +
    '</p:Interval></s:Interval><s:DeleteOnCompletion>' +
    del +
...
function performAmtWakeConfig(args) {
  if (settings.hostname == '127.0.0.1' || settings.hostname.toLowerCase() == 'localhost') {
    settings.noconsole = true;
    startLms(performAmtWakeConfig0, false, args);
  } else {
    ...
    if (args.list) {
      if (response.length == 0) {
        ...
        for (var i = 0; i < response.length; i++) {
          ...
          var t = waketime.toLocaleString(),
            tx = t.indexOf('.');
          if (tx >= 0) {
            t = t.substring(0, tx);
          }
          var details = response[i]['ElementName'] + ', wake on ' + t.replace(' ', ' at ');
          if (response[i]['Interval'] != null) {
            details += ' and each' + _fmtinterval(response[i]['Interval']['Interval']);
          }
          if (response[i]['DeleteOnCompletion'] == true) {
            details += ', delete when done';
          }
          console.log(details);
        }
      }
      ...
      for (var i = 0; i < response.length; i++) {
        if (response[i]['ElementName'] == args.del) {
          alarmFound = true;
        }
      }
      if (alarmFound == false) {
        console.log('Wake alarm ' + args.del + ' could not be found.');
        exit(0);
        return;
      }
      // Remote the alarm
      stack.Delete(
        ...
        function (stck, nm, resp, sts) {
          if (sts == 200) {
            ...
            console.log('Failed to delete wake alarm ' + args.del + '.');
          }
          exit(0);
        },
        0,
      ...
      var alarmFound = false;
      for (var i = 0; i < response.length; i++) {
        if (response[i]['ElementName'] == args.add) {
          alarmFound = true;
        }
      }
      if (alarmFound) {
        console.log('Wake alarm ' + args.add + ' already exists.');
        exit(0);
        return;
      }
      ...
      var alarm_starttime =
        _fmttimepad(t.getFullYear()) +
        '-' +
        _fmttimepad(t.getMonth() + 1) +
        '-' +
        _fmttimepad(t.getDate()) +
        'T' +
        _fmttimepad(t.getHours()) +
        ':' +
        _fmttimepad(t.getMinutes()) +
        ':' +
        _fmttimepad(t.getSeconds()) +
        'Z';
      ...
      stack.wsman.ExecMethodXml(
        amtstack.CompleteName('AMT_AlarmClockService'),
        'AddAlarm',
        tpl,
        function (ws, resuri, response, status) {
          if (status != 200) {
            console.log('Failed to add alarm. Status: ' + status + '. Verify the alarm is for a future time.');
          } else if (response.Body['ReturnValue'] != 0) {
            console.log(
              'Failed to add alarm ' + response.Body['ReturnValueStr'] + '. Verify the alarm is for a future time.'
            );
          } else {
            console.log('Done.');
          }
...
function performAmtDynamicDNS1(stack, name, response, status, args) {
  ...
  if (status == 200) {
    ...
    if (args.set || args.ttl || args.interval) {
      ...
      if (args.interval) {
        body['DDNSPeriodicUpdateInterval'] = args.interval;
      }
      if (args.ttl) {
        body['DDNSTTL'] = args.ttl;
      }
      amtstack.Put(
        'AMT_GeneralSettings',
        body,
        function (stack, name, response, status, args) {
          if (status == 200) {
            delete args.set;
            delete args.ttl;
...
function performAmtPlatformErase(args) {
  if (!settings.tls) {
    console.log('Remote Platfrom Erase (RPE) is only supported over TLS, add --tls');
    exit(1);
    return;
  }
  var transport = require('amt-wsman-duk');
...
function performAmtPlatformErase1(stack, name, response, status, args) {
  ...
  if (status == 200) {
    ...
    if (platfromEraseSupport == null) {
      console.log('Remote Platfrom Erase (RPE) is not supported on this platform');
      exit(1);
      return;
    }
    var supportedRpeFeatures = [];
    if (platfromEraseSupport == 0) {
      supportedRpeFeatures.push('None');
    } else {
      if (platfromEraseSupport & (1 << 1)) {
        supportedRpeFeatures.push('Pyrite Revert (--pyrite)');
      }
      if (platfromEraseSupport & (1 << 2)) {
        supportedRpeFeatures.push('Secure Erase All SSDs (--ssd)');
      }
      if (platfromEraseSupport & (1 << 6)) {
        supportedRpeFeatures.push('TPM Clear (--tpm)');
      }
      if (platfromEraseSupport & (1 << 25)) {
        supportedRpeFeatures.push('Clear BIOS NVM Variables (--nvm)');
      }
      if (platfromEraseSupport & (1 << 26)) {
        supportedRpeFeatures.push('BIOS Reload of Golden Configuration (--bios)');
      }
      if (platfromEraseSupport & (1 << 31)) {
        supportedRpeFeatures.push('CSME Unconfigure (--csme)');
      }
    }
    ...
    if ((rpeflags | platfromEraseSupport) != platfromEraseSupport) {
      console.log('Unable to perform unsupported RPE operation.');
      exit(1);
      return;
    }
    ...
    if (settings.powerAction == 0) {
      console.log('--reset or --poweron is required to perform RPE action.');
      exit(1);
      return;
    }
    ...
    if (newEnabledState < 32768) {
      newEnabledState = 32768;
    }
...
function performAmtPlatformErase2(stack, name, response, status, args) {
  ...
  if (status == 200) {
    if (response.Body['ReturnValueStr'] != 'SUCCESS') {
      console.log('Error, ' + response.Body['ReturnValueStr'] + '.');
      exit(1);
    } else {
      console.log('Checking enabled RPE state...');
      amtstack.BatchEnum(null, ['*CIM_BootService'], performAmtPlatformErase2b, args);
    }
  } else {
    console.log('Error, status ' + status + '.');
    exit(1);
...
function performAmtPlatformErase2b(stack, name, response, status, args) {
  ...
  if (status == 200) {
    ...
    if ((enabledState & 2) == 0) {
      console.log("RPE can't be enabled, check RPE is enabled in BIOS.");
      exit(1);
    } else {
      performAmtPlatformErase3(args);
    }
...
function performAmtPlatformErase3(args) {
  ...
  var tlv = makeUefiBootParam(1, settings.rpeflags, 4),
    tlvlen = 1;
  if (settings.rpeflags & 2 && typeof args.pyrite == 'string') {
    tlv += makeUefiBootParam(10, args.pyrite);
    tlvlen++;
  }
  if (settings.rpeflags & 4 && typeof args.ssd == 'string') {
    tlv += makeUefiBootParam(20, args.ssd);
    tlvlen++;
  }
  ...
  if (status == 200) {
    ...
    amtstack.CIM_BootConfigSetting_ChangeBootOrder(
      ...
      function (stack, name, response, status) {
        if (status != 200) {
          console.log(
            'PUT CIM_BootConfigSetting_ChangeBootOrder, error #' +
              status +
              (response.Header && response.Header.WsmanError ? ', ' + response.Header.WsmanError : '')
          );
          exit(1);
          return;
        }
        if (response.Body['ReturnValue'] != 0) {
          messagebox('Error, change boot order returns ' + response.Body.ReturnValueStr);
          exit(1);
          return;
        }
...
function performAmtFeatureConfig(args) {
  if (settings.hostname == '127.0.0.1' || settings.hostname.toLowerCase() == 'localhost') {
    settings.noconsole = true;
    startLms(performAmtFeatureConfig0, false, args);
  } else {
...
function performAmtFeatureConfig0(state, args) {
  ...
  amtstack.BatchEnum(
    null,
    ['*IPS_OptInService', '*AMT_RedirectionService', '*CIM_KVMRedirectionSAP'],
  ...
  if (status == 200) {
    ...
    if (args) {
      if (args.userconsent == 'none' && optinrequired != 0) {
        ...
        amtstack.Put(
          'IPS_OptInService',
          response['IPS_OptInService'].response,
          function (stack, name, response, status) {
            if (--pendingAmtConfigActions == 0) {
              performAmtFeatureConfig0();
            }
          }
        );
    ...
    if (args) {
      if (redir == false && (args.redir == 'enabled' || args.redir == 1)) {
        response['AMT_RedirectionService'].response['ListenerEnabled'] = true;
        redirportchange = true;
      }
      if (redir == true && (args.redir == 'disabled' || args.redir == 0)) {
        response['AMT_RedirectionService'].response['ListenerEnabled'] = false;
        redirportchange = true;
      }
      if (sol == false && (args.sol == 'enabled' || args.sol == 1)) {
        sol = true;
        redirchange = true;
      }
      if (sol == true && (args.sol == 'disabled' || args.sol == 0)) {
        sol = false;
        redirchange = true;
      }
      if (ider == false && (args.ider == 'enabled' || args.ider == 1)) {
        ider = true;
        redirchange = true;
      }
      if (ider == true && (args.ider == 'disabled' || args.ider == 0)) {
        ider = false;
        redirchange = true;
      }
      if (redirportchange) {
        ...
        amtstack.Put(
          ...
          response['AMT_RedirectionService'].response,
          function (stack, name, response, status) {
            if (--pendingAmtConfigActions == 0) {
              performAmtFeatureConfig0();
            }
      ...
      if (redirchange) {
        pendingAmtConfigActions++;
        amtstack.AMT_RedirectionService_RequestStateChange(
          32768 + ((ider ? 1 : 0) + (sol ? 2 : 0)),
          function (stack, name, response, status) {
            if (--pendingAmtConfigActions == 0) {
              performAmtFeatureConfig0();
            }
    ...
    if (response['CIM_KVMRedirectionSAP'] != null) {
      kvm =
        (response['CIM_KVMRedirectionSAP'].response['EnabledState'] == 6 &&
          response['CIM_KVMRedirectionSAP'].response['RequestedState'] == 2) ||
        response['CIM_KVMRedirectionSAP'].response['EnabledState'] == 2 ||
        response['CIM_KVMRedirectionSAP'].response['EnabledState'] == 6;
      if (args) {
        if (kvm == false && (args.kvm == 'enabled' || args.kvm == 1)) {
          kvm = true;
          kvmchange = true;
        }
        if (kvm == true && (args.kvm == 'disabled' || args.kvm == 0)) {
          kvm = false;
          kvmchange = true;
        }
        if (kvmchange) {
          pendingAmtConfigActions++;
          amtstack.CIM_KVMRedirectionSAP_RequestStateChange(kvm ? 2 : 3, 0, function (stack, name, response, status) {
            if (--pendingAmtConfigActions == 0) {
              performAmtFeatureConfig0();
            }
          });
    ...
    if (pendingAmtConfigActions == 0) {
      if (optinrequired == 0) {
        console.log('User Consent         : None');
      } else if (optinrequired == 1) {
        console.log('User Consent         : KVM');
      } else if (optinrequired == 0xffffffff) {
        console.log('User Consent         : All');
      } else {
        console.log('User Consent         : ' + optinrequired);
      }
      ...
      if (response['CIM_KVMRedirectionSAP'] != null) {
        console.log('Remote desktop (KVM) : ' + (kvm ? 'Enabled' : 'Disabled'));
      }
      exit(0);
    }
  } else {
...
function powerActionResponse1(stack, name, response, status) {
  if (status != 200) {
    console.log('Unable to get AMT_BootSettingData');
    exit(1);
    return;
  }
  var r = response.Body;
  ...
  amtstack.CIM_BootConfigSetting_ChangeBootOrder(
    ...
    function (stack, name, response, status) {
      if (status != 200) {
        console.log('PUT CIM_BootConfigSetting_ChangeBootOrder failed');
        exit(1);
        return;
      }
      if (response.Body['ReturnValue'] != 0) {
        console.log('(1) Change Boot Order returns ' + response.Body.ReturnValueStr);
        exit(1);
        return;
      }
...
function powerActionResponse2(stack, name, response, status, tag) {
  if (status != 200) {
    console.log('PUT AMT_BootSettingData failed.');
    exit(1);
    return;
  }
...
function powerActionResponse3(stack, name, response, status) {
  if (status != 200) {
    console.log('SetBootConfigRole failed.');
    exit(1);
    return;
  }
...
function getMeiState(flags, func) {
  ...
  try {
    ...
    amtMei.getVersion(function (result) {
      ...
      }); // 0: "Not Activated (Pre)", 1: "Not Activated (In)", 2: "Activated"
      amtMei.getEHBCState(function (result) {
        if (result != null && result.EHBC == true) {
          amtMeiTmpState.Flags += 1;
        }
        ...
        amtMei.getLanInterfaceSettings(0, function (result) {
          if (result) {
            ...
            for (var i in interfaces) {
              for (var j in interfaces[i]) {
                if (
                  interfaces[i][j].mac == amtMeiTmpState.net0.mac &&
                  interfaces[i][j].fqdn != null &&
                  interfaces[i][j].fqdn != ''
                ) {
      ...
      amtMei.getUuid(function (result) {
        if (result != null && result.uuid != null) {
          amtMeiTmpState.UUID = result.uuid;
        }
      });
      if ((flags & 2) != 0) {
        amtMei.getLocalSystemAccount(function (x) {
...
function parseUrlArguments(url) {
  ...
  for (var i in x) {
    var j = x[i].indexOf('=');
    if (j > 0) {
      r[x[i].substring(0, j).toLowerCase()] = x[i].substring(j + 1);
    }
  }
  return r;
}
...
for (var i in process.argv) {
  if (process.argv[i].toLowerCase() == 'install') {
    process.argv[i] = '-install';
  }
  if (process.argv[i].toLowerCase() == 'uninstall') {
    process.argv[i] = '-uninstall';
  }
  if (
    process.argv[i].toLowerCase() == 'microlms' ||
    process.argv[i].toLowerCase() == 'amtlms' ||
    process.argv[i].toLowerCase() == 'lms'
  ) {
...
if (serviceName == null) {
  if (process.execPath.includes('MicroLMS')) {
    serviceName = 'MicroLMS';
  } else if (process.execPath.includes('LMS')) {
    serviceName = 'LMS';
  } else if (process.execPath.includes('MeshCommander')) {
    serviceName = 'MeshCommander';
  }
  if (serviceName == null) {
    for (var i in process.argv) {
  ...
  var meshcmdService = new serviceHost({
    name: serviceName,
  ...
  meshcmdService.on('serviceStop', function onStop() {
    console.log('Stopping service');
    exit(0);
  }); // The console.log() is for debugging, will be ignored unless "console.setDestination()" is set.
  ...
  meshcmdService.on('normalStart', function onNormalStart() {
    try {
      ...
      console.log('onNormalStart() error: ' + e);
    }
  });
  meshcmdService.run();
}
