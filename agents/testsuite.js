...
function objToString(x, p, ret) {
  ...
  var r = '{' + (ret ? '\r\n' : ' ');
  for (var i in x) {
    r += addPad(p + 2, ret) + i + ': ' + objToString(x[i], p + 2, ret) + (ret ? '\r\n' : ' ');
  }
...
function parseUrl(url) {
  ...
  if (y.length == 1) {
    options.port = x[0] == 'https:' || x[0] == 'wss:' ? 443 : 80;
  } else {
    options.port = parseInt(y[1]);
  }
...

{
  ...
  sha256.hashString = function (x) {
    if (x == '81B637D8FCD2C6DA6359E6963113A1170DE795E4B725B84D1E0B4CFD9EC58CE9') {
      console.log('Test 1 - OK: ' + x);
    } else {
      console.log('Test 1 - FAIL: ' + x);
    }
  };
  sha256.write('bob');
  sha256.end();
