...
if (lang != 'en') {
  for (var i in translation['en']) {
    ...
    if (translation[lang][i] == null) {
      translation[lang][i] = translation['en'][i];
    }
  }
}
...
var s = null,
  buttons = [translation[lang].cancel],
  skip = false;
var serviceName = msh.meshServiceName ? msh.meshServiceName : 'meshagent';

try {
  s = require('service-manager').manager.getService(serviceName);
} catch (e) {}
...
if (msh.displayName) {
  connectArgs.push('--displayName="' + msh.displayName + '"');
}
if (msh.agentName) {
...
function _install(parms) {
  ...
  if (msh.companyName) {
    parms.unshift('--companyName="' + msh.companyName + '"');
  }
  if (msh.displayName) {
    parms.unshift('--displayName="' + msh.displayName + '"');
  }
...
if ((msh.InstallFlags & 1) == 1) {
  ...
  if (process.argv.includes('-connect')) {
    ...
    global._child.stdout.on('data', function (c) {});
    global._child.stderr.on('data', function (c) {});
    global._child.on('exit', function (code) {
      process.exit(code);
    });
...
if (!skip) {
  if (process.argv.includes('-install') || process.argv.includes('-update')) {
    ...
    if (
      !require('message-box').kdialog &&
      (require('message-box').zenity == null || !require('message-box').zenity.extra)
    ) {
...
if (!skip) {
  if (!s) {
    ...
    msg =
      translation[lang].agent +
      ': ' +
      (s.isRunning() ? translation[lang].status[1] : translation[lang].status[2]) +
      '\n';
  }
  ...
  p.then(function (v) {
    switch (v) {
      ...
      case translation[lang].setup:
        var d = require('message-box').create(displayName, msg, 99999, [
          translation[lang].update,
          translation[lang].uninstall,
