...
//   if (pathstr.split('\\*').join('').toLowerCase() == process.env['windir'].toLowerCase()) {
//     sysnative = true;
//   }
...
//     case 'win32':
//       try {
//         ret = require('win-wmi').query('ROOT\\CIMV2', 'SELECT * FROM Win32_ComputerSystem', [
//           'Name',
//           'Domain',
//           'PartOfDomain'
//         ])[0];
...
//             ret = {
//               Name: hostname.substring(0, hostname.length - names.peek().length - 1),
//               Domain: names.peek(),
//               PartOfDomain: true
//             };
//             break;
...
//       'SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Authentication\\LogonUI\\SessionData\\' + sessionId,
//       'LoggedOnUser'
//     );
...
//   // 4 Microsoft Account
//   try {
//     const userAccounts = require('win-registry').QueryKey(
//       require('win-registry').HKEY.LocalMachine,
//       'SOFTWARE\\Microsoft\\IdentityStore\\LogonCache\\D7F9888F-E3FC-49b0-9EA6-A85B5F392A4F'
//     );
...
//   }
//   if (id != null) {
//     Object.defineProperty(require('MeshAgent'), 'ARCHID', { value: id });
//   }
// }
...
//     users: function () {
//       var r = {};
//       require('user-sessions').Current(function (c) {
//         r = c;
//       });
//       if (process.platform != 'win32') {
//         for (var i in r) {
...
//   try {
//     var writtenSize = 0,
//       actualSize = Math.floor(require('fs').statSync(process.execPath).size / 1024);
//     var serviceName = _MSH().serviceName
//       ? _MSH().serviceName
//       : require('_agentNodeId').serviceName()
//         ? require('_agentNodeId').serviceName()
//         : 'Mesh Agent';
//     try {
//       writtenSize = require('win-registry').QueryKey(
//         require('win-registry').HKEY.LocalMachine,
//         'Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\' + serviceName,
//         'EstimatedSize'
//       );
//     } catch (ex) {}
//     if (writtenSize != actualSize) {
//       try {
//         require('win-registry').WriteKey(
//           require('win-registry').HKEY.LocalMachine,
//           'Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\' + serviceName,
//           'EstimatedSize',
//           actualSize
...
//     var meshCheck = false;
//     try {
//       meshCheck = require('service-manager').manager.getService(svcname).isMe();
//     } catch (ex) {}
//     if (meshCheck && require('win-bcd').isSafeModeService && !require('win-bcd').isSafeModeService(svcname)) {
...
//   if (process.platform == 'win32') {
//     try {
//       var writtenDisplayVersion = 0,
//         actualDisplayVersion = process.versions.commitDate.toString();
//       var serviceName = _MSH().serviceName
//         ? _MSH().serviceName
//         : require('_agentNodeId').serviceName()
//           ? require('_agentNodeId').serviceName()
//           : 'Mesh Agent';
//       try {
//         writtenDisplayVersion = require('win-registry').QueryKey(
//           require('win-registry').HKEY.LocalMachine,
//           'Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\' + serviceName,
//           'DisplayVersion'
//         );
//       } catch (ex) {}
//       if (writtenDisplayVersion != actualDisplayVersion) {
//         try {
//           require('win-registry').WriteKey(
//             require('win-registry').HKEY.LocalMachine,
//             'Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\' + serviceName,
//             'DisplayVersion',
//             actualDisplayVersion
//           );
//         } catch (ex) {}
//       }
//     } catch (ex) {}
...
//   });
//   child.stdin.write(
//     'cat /Library/LaunchDaemons/meshagent_osx64_LaunchDaemon.plist | tr \'\n\' \'\.\' | awk \'{split($0, a, "<key>KeepAlive</key>"); split(a[2], b, "<"); split(b[2], c, ">"); '
//   );
//   child.stdin.write(
//     ' if(c[1]=="dict"){ split(a[2], d, "</dict>"); if(split(d[1], truval, "<true/>")>1) { split(truval[1], kn1, "<key>"); split(kn1[2], kn2, "</key>"); print kn2[1]; } }'
//   );
...
//         fix +=
//           "    child.stdin.write('launchctl unload /Library/LaunchDaemons/meshagent_osx64_LaunchDaemon.plist\\n');\n";
//         fix +=
//           "    child.stdin.write('launchctl load /Library/LaunchDaemons/meshagent_osx64_LaunchDaemon.plist\\n');\n";
...
//   if (obj.amtevents == null) {
//     obj.amtevents = [];
//   }
//   var d = new Date(),
//     e = zeroPad(d.getHours(), 2) + ':' + zeroPad(d.getMinutes(), 2) + ':' + zeroPad(d.getSeconds(), 2) + ', ' + msg;
//   obj.amtevents.push(e);
//   if (obj.amtevents.length > 100) {
//     obj.amtevents.splice(0, obj.amtevents.length - 100);
//   }
//   if (obj.showamtevent) {
//     require('MeshAgent').SendCommand({ action: 'msg', type: 'console', value: e });
//   }
// }
...
// if (process.platform != 'win32') {
//   try {
//     require('fs').unlinkSync(process.cwd() + '/DAIPC');
//   } catch (ex) {}
// }
// obj.DAIPC.IPCPATH =
//   process.platform == 'win32' ? '\\\\.\\pipe\\' + require('_agentNodeId')() + '-DAIPC' : process.cwd() + '/DAIPC';
// try {
//   obj.DAIPC.listen({ path: obj.DAIPC.IPCPATH, writableAll: true, maxConnections: 5 });
// } catch (ex) {}
// obj.DAIPC._daipc = [];
...
//           try {
//             mesh.SendCommand({ action: 'sessions', type: 'help', value: help });
//           } catch (ex) {}
//           MeshServerLogEx(
//             98,
//             [this._registered, data.value],
//             'Help Requested, user: ' + this._registered + ', details: ' + data.value,
//             null
//           );
//           break;
//         case 'cancelhelp':
...
//           try {
//             mesh.SendCommand({ action: 'sessions', type: 'help', value: {} });
//           } catch (ex) {}
//           break;
//         case 'register':
...
//             apps[data.value] = 1;
//             try {
//               mesh.SendCommand({ action: 'sessions', type: 'app', value: apps });
//             } catch (ex) {}
//             this._send({
//               cmd: 'serverstate',
//               value: meshServerConnectionState,
//               url: require('MeshAgent').ConnectedServer,
//               amt: amt != null
//             });
//           }
//           break;
...
//         case 'amtstate':
//           if (amt == null) return;
//           var func = function amtStateFunc(state) {
//             if (state != null) {
//               amtStateFunc.pipe._send({ cmd: 'amtstate', value: state });
//             }
//           };
//           func.pipe = this;
...
//         case 'getUserImage':
//           try {
//             mesh.SendCommand({ action: 'getUserImage', userid: data.userid, pipe: true });
//           } catch (ex) {}
//           break;
//         case 'console':
//           if (debugConsole) {
...
//     if (obj.DAIPC._daipc[i]._registered != null) {
//       obj.DAIPC._daipc[i]._send(x);
//     }
//   }
// }
...
//   if (obj.DAIPC == null || obj.DAIPC._daipc == null) return;
//   for (var i in obj.DAIPC._daipc) {
//     if (obj.DAIPC._daipc[i]._registered == appid) {
//       obj.DAIPC._daipc[i]._send(x);
//     }
//   }
// }
...
//   var apps = {};
//   for (var i in obj.DAIPC._daipc) {
//     if (apps[obj.DAIPC._daipc[i]._registered] == null) {
//       apps[obj.DAIPC._daipc[i]._registered] = 1;
//     } else {
//       apps[obj.DAIPC._daipc[i]._registered]++;
//     }
//   }
//   try {
//     mesh.SendCommand({ action: 'sessions', type: 'app', value: apps });
...
//   for (var i = obj.DAIPC._daipc.length - 1; i >= 0; i--) {
//     if (obj.DAIPC._daipc[i] === pipe) {
//       obj.DAIPC._daipc.splice(i, 1);
//     }
//   }
...
//   if (cert.root.pfx) {
//     ddb.Put('SelfNodeCert', cert.root.pfx);
//   }
//   if (cert.tls) {
//     ddb.Put('SelfNodeTlsCert', cert.tls.pfx);
//   }
//   if (proxyConfig) {
...
//     action: 'msg',
//     type: 'console',
//     value: 'Diagnostic Agent Registered [' + nodeid.length + '/' + nodeid + ']'
//   });

//   delete ddb;

//   // Set a recurrent task, to run the Diagnostic Agent every 2 days
//   require('task-scheduler').create({
//     name: 'meshagentDiagnostic/periodicStart',
//     daily: 2,
//     time:
//       require('tls').generateRandomInteger('0', '23') +
//       ':' +
//       require('tls').generateRandomInteger('0', '59').padStart(2, '0'),
//     service: 'meshagentDiagnostic'
//   });
...
//         try {
//           data = require('fs')
//             .readFileSync(process.cwd() + 'batterystate.txt')
//             .toString();
//         } catch (ex) {}
...
//               require('MeshAgent').SendCommand({ action: 'battery', state: data[0], level: level });
//             }
//           }
//         }
//       } catch (ex) {}
...
//         _battLevelChanged.self.SendCommand({
//           action: 'battery',
//           state: _battLevelChanged.self._currentPowerState,
//           level: val
//         });
//       };
...
//         _powerChanged.self.SendCommand({
//           action: 'battery',
//           state: val == 'AC' ? 'ac' : 'dc',
//           level: _powerChanged.self._currentBatteryLevel
//         });
//       };
...
// var meshCoreObj = {
//   action: 'coreinfo',
//   value: require('MeshAgent').coreHash
//     ? (process.versions.compileTime ? process.versions.compileTime : '').split(', ')[1].replace('  ', ' ') +
//       ', ' +
//       crc32c(require('MeshAgent').coreHash)
//     : 'MeshCore v6',
//   caps: 14,
//   root: require('user-sessions').isRoot()
...
//     var u = [],
//       a = users.Active;
//     if (meshCoreObj.lusers == null) {
//       meshCoreObj.lusers = [];
//     }
//     if (meshCoreObj.upnusers == null) {
//       meshCoreObj.upnusers = [];
//     }
//     var ret = getDomainInfo();
//     for (var i = 0; i < a.length; i++) {
//       var un = a[i].Domain ? a[i].Domain + '\\' + a[i].Username : a[i].Username;
//       if (user && locked && JSON.stringify(a[i]) === JSON.stringify(user)) {
//         if (meshCoreObj.lusers.indexOf(un) == -1) {
//           meshCoreObj.lusers.push(un);
//         }
//       } else if (user && !locked && JSON.stringify(a[i]) === JSON.stringify(user)) {
//         meshCoreObj.lusers.splice(meshCoreObj.lusers.indexOf(un), 1);
//       }
//       if (u.indexOf(un) == -1) {
//         u.push(un);
//       } // Only push users in the list once.
...
//         } else if (getJoinState() == 4) {
//           // One account with Microsoft Account
//           var userobj = getLogonCacheKeys();
//           if (userobj && userobj.length > 0) {
//             for (var j = 0; j < userobj.length; j++) {
//               if (
...
//   userSession.on('changed', function () {
//     onUserSessionChanged(null, false);
//   });
//   userSession.emit('changed');
//   userSession.on('locked', function (user) {
//     if (user != undefined && user != null) {
//       onUserSessionChanged(user, true);
//     }
//   });
//   userSession.on('unlocked', function (user) {
//     if (user != undefined && user != null) {
//       onUserSessionChanged(user, false);
//     }
//   });
// } catch (ex) {}
...
//     }
//     if (state.remoteaddr) {
//       msg.remoteaddr = state.remoteaddr;
//     }
//     if (state.guestname) {
//       msg.guestname = state.guestname;
//     }
//   }
...
//           mesh.SendCommand({ action: 'smbios', value: SMBiosTablesRaw });
//         }
...
//             mesh.SendCommand({ action: 'lmsinfo', value: { ports: map.keys() } });
//           });
//           amt.on('stateChange_LMS', function (v) {
//             if (!meshCoreObj.intelamt) {
//               meshCoreObj.intelamt = {};
//             }
//             meshCoreObj.intelamt.microlms = v;
//             meshCoreObjChanged();
//           }); // 0 = Disabled, 1 = Connecting, 2 = Connected
//           amt.onStateChange = function (state) {
//             if (state == 2) {
...
//   wifiScanner.on('accessPoint', function (data) {
//     sendConsoleText('wifiScanner: ' + data);
//   });
// } catch (ex) {
//   wifiScannerLib = null;
//   wifiScanner = null;
...
//   if (getIpLocationDataExInProgress == true) {
//     return false;
//   }
...
//   for (var i in x.netif) {
//     try {
//       if (x.netif[i].gatewaymac) {
//         delete x.netif[i].gatewaymac;
//       }
//     } catch (ex) {}
//   }
...
//     publicLocationInfo = JSON.parse(publicLocationInfo);
//   }
//   if (publicLocationInfo == null) {
...
//   for (var i in x) {
//     if (i != '_ObjectID') {
//       r += addPad(p + 2, pad) + i + ': ' + objToString(x[i], p + 2, pad, ret) + (ret ? '\r\n' : ' ');
//     }
//   }
...
//     return getServerTargetUrl(url.substring(2));
//   }
//   return url;
// }
...
//   if (require('MeshAgent')._promises == null) {
//     require('MeshAgent')._promises = {};
//   }
//   require('MeshAgent')._promises[ret._hashCode()] = ret;
//   require('MeshAgent').SendCommand({
//     action: 'getUserImage',
//     userid: userid,
//     promise: ret._hashCode(),
//     sentDefault: true
//   });
//   return ret;
// }
...
//   if (
//     require('MeshAgent').ServerInfo == null ||
//     require('MeshAgent').ServerInfo.ControlChannelCertificate == null ||
//     certs[0].digest == null
//   ) {
...
//   if (
//     certs[0].fingerprint != null &&
//     require('MeshAgent').ServerInfo.ControlChannelCertificate.fingerprint == certs[0].fingerprint
//   )
//     return; // Control channel certificate matches using public key hash

//   // Check that the certificate is the one expected by the server, fail if not.
//   if (
//     tunnel_checkServerIdentity.servertlshash != null &&
//     tunnel_checkServerIdentity.servertlshash.toLowerCase() != certs[0].digest.split(':').join('').toLowerCase()
//   ) {
...
//                 if (typeof data.perMessageDeflate == 'boolean') {
//                   woptions.perMessageDeflate = data.perMessageDeflate;
//                 }
...
//                 if (global._MSH && _MSH().LocalConsent != null) {
//                   tunnel.consent |= parseInt(_MSH().LocalConsent);
//                 }
...
//                 tunnel.realname =
//                   (data.realname ? data.realname : data.username) + (data.guestname ? ' - ' + data.guestname : '');
...
//                 tunnel.consentTimeout =
//                   tunnel.soptions && tunnel.soptions.consentTimeout ? tunnel.soptions.consentTimeout : 30;
//                 tunnel.consentAutoAccept = tunnel.soptions && tunnel.soptions.consentAutoAccept === true;
//                 tunnel.consentAutoAcceptIfNoUser =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfNoUser === true;
//                 tunnel.consentAutoAcceptIfDesktopNoUser =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfDesktopNoUser === true;
//                 tunnel.consentAutoAcceptIfTerminalNoUser =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfTerminalNoUser === true;
//                 tunnel.consentAutoAcceptIfFileNoUser =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfFileNoUser === true;
//                 tunnel.consentAutoAcceptIfLocked =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfLocked === true;
//                 tunnel.consentAutoAcceptIfDesktopLocked =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfDesktopLocked === true;
//                 tunnel.consentAutoAcceptIfTerminalLocked =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfTerminalLocked === true;
//                 tunnel.consentAutoAcceptIfFileLocked =
//                   tunnel.soptions && tunnel.soptions.consentAutoAcceptIfFileLocked === true;
...
//                   msgid = 0;
//                 if (data.protocol == 'kvm' && tunnels[i].protocol == 2) {
//                   msgid = 134;
//                   disconnect = true;
//                 } else if (data.protocol == 'terminal' && tunnels[i].protocol == 1) {
//                   msgid = 135;
//                   disconnect = true;
//                 } else if (data.protocol == 'files' && tunnels[i].protocol == 5) {
//                   msgid = 136;
//                   disconnect = true;
//                 } else if (data.protocol == 'tcp' && tunnels[i].tcpport != null) {
//                   msgid = 137;
//                   disconnect = true;
//                 } else if (data.protocol == 'udp' && tunnels[i].udpport != null) {
//                   msgid = 137;
//                   disconnect = true;
//                 }
//                 if (disconnect) {
...
//                 18,
//                 [data.title, data.msg],
//                 'Displaying message box, title=' + data.title + ', message=' + data.msg,
//                 data
//               );
//               if (process.platform == 'win32') {
...
//                     global._clientmessage.then(function () {
//                       global._clientmessage = null;
//                     });
//                   } catch (z) {
//                     try {
//                       require('message-box')
//                         .create(data.title, data.msg, 120)
//                         .then(function () {})
//                         .catch(function () {});
//                     } catch (ex) {}
//                   }
...
//               } catch (ex) {
//                 sendConsoleText('pskill: ' + JSON.stringify(ex));
//               }
//             }
//             break;
//           }
...
//                   var cmd =
//                     '(Get-WmiObject -Class win32_userAccount -Filter "SID=\'' + service.installedBy + '\'").Caption';
//                   var replydata = '';
//                   var pws = require('child_process').execFile(
//                     process.env['windir'] + '\\System32\\WindowsPowerShell\\v1.0\\powershell.exe',
//                     ['powershell', '-noprofile', '-nologo', '-command', '-'],
//                     {}
//                   );
//                   pws.descriptorMetadata = 'UserSIDPowerShell';
//                   pws.stdout.on('data', function (c) {
//                     replydata += c.toString();
//                   });
//                   pws.stderr.on('data', function (c) {
//                     replydata += c.toString();
//                   });
//                   pws.stdin.write(cmd + '\r\nexit\r\n');
...
//             var services = null;
//             try {
//               services = require('service-manager').manager.enumerateService();
//             } catch (ex) {}
//             if (services != null) {
//               mesh.SendCommand({
//                 action: 'msg',
...
//                     '-b64exec',
//                     ''
//                   ],
//                   { type: stype, uid: sid }
//                 );
//                 child.stdout.str = '';
...
//                   type: 'deskBackground',
//                   sessionid: data.sessionid,
//                   data: current != '' ? '' : require('MeshAgent')._wallpaper
//                 });
//               } else {
...
//                   require('MeshAgent')._wallpaper = current;
//                 }
//                 require('linux-gnome-helpers').setDesktopWallpaper(
//                   id,
//                   current != '/dev/null' ? undefined : require('MeshAgent')._wallpaper
//                 );
//                 mesh.SendCommand({
//                   action: 'msg',
//                   type: 'deskBackground',
//                   sessionid: data.sessionid,
//                   data: current != '/dev/null' ? '' : require('MeshAgent')._wallpaper
//                 });
//               }
//             } catch (ex) {
//               sendConsoleText(ex);
...
//                 url: data.url,
//                 sessionid: data.sessionid,
//                 success: openUserDesktopUrl(data.url) != null
//               });
//             }
//             break;
//           }
...
//                 22,
//                 [data.data.length],
//                 'Setting clipboard content, ' + data.data.length + ' byte(s)',
//                 data
//               );
//               if (require('MeshAgent').isService) {
//                 if (process.platform != 'win32') {
...
//                   }
//                   this._dispatcher = require('win-dispatcher').dispatch({
//                     user: user,
//                     modules: [
//                       {
//                         name: 'clip-dispatch',
//                         script:
//                           "module.exports = { dispatch: function dispatch(val) { require('clipboard')(val); process.exit(); } };"
//                       }
//                     ],
//                     launch: { module: 'clip-dispatch', method: 'dispatch', args: [clipargs] }
//                   });
...
//             if (data.appid != null) {
//               sendToRegisteredApp(data.appid, data.value);
//             } else {
//               broadcastToRegisteredApps(data.value);
//             }
//             break;
//           case 'alertbox': {
...
//                 158,
//                 [data.title, data.msg],
//                 'Displaying alert box, title=' + data.title + ', message=' + data.msg,
//                 data
//               );
//               try {
//                 require('message-box')
//                   .create(data.title, data.msg, 9999, 1)
//                   .then(function () {})
//                   .catch(function () {});
//               } catch (ex) {}
//             }
...
//               if (results != null && data.hash != results.hash) {
...
//           sendConsoleText("Run commands can't execute, already busy.");
//           break;
//         }
...
//             mesh.cmdchild = require('child_process').execFile(
//               process.env['windir'] + '\\system32\\cmd.exe',
//               ['cmd'],
//               options
//             );
//             mesh.cmdchild.descriptorMetadata = 'UserCommandsShell';
//             mesh.cmdchild.stdout.on('data', function (c) {
//               replydata += c.toString();
//               sendConsoleText(c.toString());
//             });
//             mesh.cmdchild.stderr.on('data', function (c) {
//               replydata += c.toString();
//               sendConsoleText(c.toString());
//             });
...
//           } catch (ex) {}
//           var js =
//             "require('service-manager').manager.getService('" +
//             agentName +
//             "').stop(); require('service-manager').manager.uninstallService('" +
//             agentName +
//             "'); process.exit();";
//           this.child = require('child_process').execFile(
//             process.execPath,
//             [
//               process.platform == 'win32' ? process.execPath.split('\\').pop() : process.execPath.split('/').pop(),
//               '-b64exec',
//               Buffer.from(js).toString('base64')
//             ],
//             { type: 4, detached: true }
//           );
//         }
//         break;
//       case 'poweraction': {
...
//           MeshServerLogEx(
//             25,
//             [data.actiontype, forced],
//             'Performing power action=' + data.actiontype + ', forced=' + forced,
//             data
//           );
...
//           mesh.SendCommand({ action: 'iplocation', type: 'publicip', value: location });
//         });
//         break;
//       }
//       case 'toast': {
...
//             26,
//             [data.title, data.msg],
//             'Displaying toast message, title=' + data.title + ', message=' + data.msg,
//             data
//           );
//           data.msg = data.msg.split('\r').join('\\r').split('\n').join('\\n');
//           try {
//             require('toaster').Toast(data.title, data.msg);
//           } catch (ex) {}
//         }
//         break;
//       }
...
//             apftunnel.onChannelClosed = function () {
//               addAmtEvent('LMS tunnel closed.');
//               apftunnel = null;
//             };
//             try {
//               apftunnel.connect();
...
//         if (process.platform == 'win32') {
//           coreDumpPath = process.coreDumpLocation;
//         } else {
//           coreDumpPath = process.cwd() != '//' ? fs.existsSync(process.cwd() + 'core') : null;
//         }
...
//             if (coretime > agenttime) {
//               r.exists = db.Get('CoreDumpTime') != coredate;
//             }
//           } catch (ex) {}
//         }
//         if (r.exists == true) {
...
//           delete data.pipe;
//           delete data.action;
//           data.cmd = 'meshToolInfo';
//           broadcastToRegisteredApps(data);
//         }
//         if (data.tag == 'info') {
//           sendConsoleText(JSON.stringify(data, null, 2));
//         }
//         if (data.tag == 'install') {
...
//         try {
//           mesh.SendCommand(JSON.stringify({ action: 'errorlog', log: require('util-agentlog').read(data.startTime) }));
//         } catch (ex) {}
//         break;
//       default:
//         // Unknown action, ignore it.
...
//   if (process.platform == 'win32' || mestate.net0 == null || mestate.net0.mac == null) {
//     func(mestate);
//     return;
//   }
//   try {
...
//   }
//   require('linux-dhcp')
//     .client.info(mestate.net0.mac)
//     .then(
//       function (d) {
//         if (typeof d.options == 'object' && typeof d.options.domainname == 'string') {
//           mestate.OsDnsSuffix = d.options.domainname;
//         }
//         func(mestate);
//       },
//       function (e) {
...
//     // If the tunnel certificate matches the control channel certificate, accept the connection
//     try {
//       if (require('MeshAgent').ServerInfo.ControlChannelCertificate.digest == certs[0].digest) return;
//     } catch (ex) {}
//     try {
//       if (require('MeshAgent').ServerInfo.ControlChannelCertificate.fingerprint == certs[0].fingerprint) return;
//     } catch (ex) {}
//     // Check that the certificate is the one expected by the server, fail if not.
...
//     if (
//       checkServerIdentity.servertlshash != null &&
//       checkServerIdentity.servertlshash.toLowerCase() != certs[0].digest.split(':').join('').toLowerCase()
//     ) {
//       throw new Error('BadCert');
//     }
//   };
...
//   trustedDownloads[downloadoptions.name].dl.on('error', function (e) {
//     downloadoptions.func(downloadoptions, false);
//     delete trustedDownloads[downloadoptions.name];
//   });
...
//       if (downloadoptions.hash != null && downloadoptions.hash.toLowerCase() != h.toString('hex').toLowerCase()) {
//         downloadoptions.func(downloadoptions, false);
//         delete trustedDownloads[downloadoptions.name];
//         return;
//       }
//       downloadoptions.func(downloadoptions, true);
//     });
//     img.pipe(this._file);
...
//     addAmtEvent(data.msg);
//   } // Add console message to AMT event log
//   if (data.action == 'mestate') {
//     amt.getMeiState(15, function (state) {
//       apftunnel.updateMeiState(state);
//     });
//   } // Update the MEI state
//   if (data.action == 'close') {
//     try {
//       apftunnel.disconnect();
//     } catch (ex) {}
//     apftunnel = null;
//   } // Close the CIRA-LMS connection
//   if (amt.amtMei != null) {
//     if (data.action == 'deactivate') {
//       // Request CCM deactivation
//       amt.amtMei.unprovision(1, function (status) {
//         if (apftunnel) apftunnel.sendMeiDeactivationState(status);
//       }); // 0 = Success
//     }
//     if (data.action == 'startTlsHostConfig') {
//       // Request start of host based TLS ACM activation
//       amt.amtMei.startConfigurationHBased(
...
//       );
//     }
//     if (data.action == 'stopConfiguration') {
//       // Request Intel AMT stop configuration.
//       amt.amtMei.stopConfiguration(function (status) {
//         apftunnel.sendStopConfigurationResponse(status);
//       });
//     }
//   }
// }
...
//     } catch (ex) {}
//   } // If requested, create the local folder.
...
//   var agentFileHttpRequest = http.request(agentFileHttpOptions, function (response) {
//     response.xparent = this;
//     try {
//       response.xfile = fs.createWriteStream(this.xpath, { flags: 'wbN' });
//       response.pipe(response.xfile);
//       response.end = function () {
//         delete agentFileHttpRequests[this.xparent.xurlpath];
//         delete this.xparent;
//         serverFetchFile();
//       };
//     } catch (ex) {
...
//     }
//   });
//   agentFileHttpRequest.on('error', function (ex) {
//     sendConsoleText(ex);
//     delete agentFileHttpRequests[this.xurlpath];
//     serverFetchFile();
//   });
...
//   for (var i in o) {
//     if (i.indexOf(' ') >= 0) {
//       o[i.split(' ').join('_')] = o[i];
//       delete o[i];
//     }
//     replaceSpacesWithUnderscoresRec(o[i]);
//   }
// }
...
//       try {
//         delete x.FreePhysicalMemory;
//       } catch (ex) {}
//       try {
//         delete x.FreeSpaceInPagingFiles;
//       } catch (ex) {}
//       try {
//         delete x.FreeVirtualMemory;
//       } catch (ex) {}
//       try {
//         delete x.LocalDateTime;
//       } catch (ex) {}
//       try {
//         delete x.MaxProcessMemorySize;
//       } catch (ex) {}
//       try {
//         delete x.TotalVirtualMemorySize;
//       } catch (ex) {}
//       try {
//         delete x.TotalVisibleMemorySize;
//       } catch (ex) {}
//       try {
//         if (results.hardware.windows.memory) {
//           for (var i in results.hardware.windows.memory) {
//             delete results.hardware.windows.memory[i].Node;
//           }
//         }
...
//         }
//         if (results.hardware.windows.partitions) {
//           for (var i in results.hardware.windows.partitions) {
//             delete results.hardware.windows.partitions[i].Node;
//           }
//         }
//       } catch (ex) {}
//       if (x.LastBootUpTime) {
//         // detect windows uptime
//         var thedate = {
...
//         };
//         var thelastbootuptime = new Date(
//           thedate.year,
//           thedate.month,
//           thedate.day,
//           thedate.hours,
...
//         if (differenceInMilliseconds < 300000) {
//           // computer uptime less than 5 minutes
...
//           t: 1,
//           dt: results[i].type,
//           s: results[i].size ? results[i].size : 0,
//           f: results[i].free ? results[i].free : 0
//         };
...
//     }
//     var results = null,
//       xpath = obj.path.join(reqpath, '*');
//     //if (process.platform == "win32") { xpath = xpath.split('/').join('\\'); }
//     try {
//       results = fs.readdirSync(xpath);
//     } catch (ex) {}
//     try {
//       if (results != null && results.length == 0 && fs.existsSync(reqpath) == false) {
//         results = null;
//       }
//     } catch (ex) {}
//     if (results != null) {
...
//           var stat = null,
//             p = obj.path.join(reqpath, results[i]);
//           //if (process.platform == "win32") { p = p.split('/').join('\\'); }
//           try {
//             stat = fs.statSync(p);
//           } catch (ex) {} // TODO: Get file size/date
...
//     if (this.tcpaddr != null) {
//       connectionOptions.host = this.tcpaddr;
//     } else {
...
//       if (tunnelUserCount.tcp[userid] == null) {
//         tunnelUserCount.tcp[userid] = 1;
//       } else {
//         tunnelUserCount.tcp[userid]++;
//       }
//       try {
//         mesh.SendCommand({ action: 'sessions', type: 'tcp', value: tunnelUserCount.tcp });
//       } catch (ex) {}
...
//     if (s.httprequest.userid != null) {
//       var userid = getUserIdAndGuestNameFromHttpRequest(s.httprequest);
//       if (tunnelUserCount.udp[userid] == null) {
//         tunnelUserCount.udp[userid] = 1;
//       } else {
//         tunnelUserCount.udp[userid]++;
//       }
//       try {
//         mesh.SendCommand({ action: 'sessions', type: 'udp', value: tunnelUserCount.tcp });
//       } catch (ex) {}
//       broadcastSessionsToRegisteredApps();
//     }
//   } else {
...
//   if (request.guestname == null) return request.userid;
//   else return request.guestuserid + '/guest:' + Buffer.from(request.guestname).toString('base64');
// }
...
//     this.udprelay.send(
//       data,
//       parseInt(this.udprelay.udpport),
//       this.udprelay.udpaddr ? this.udprelay.udpaddr : '127.0.0.1'
//     );
//   }
// }
...
//     if (this.httprequest._dispatcher.close) {
//       this.httprequest._dispatcher.close();
//     }
...
//     // Look for a TSID
//     var tsid = null;
//     if (this.httprequest.xoptions != null && typeof this.httprequest.xoptions.tsid == 'number') {
//       tsid = this.httprequest.xoptions.tsid;
//     }
...
//       if (tunnelUserCount.tcp[userid] != null) {
//         tunnelUserCount.tcp[userid]--;
//         if (tunnelUserCount.tcp[userid] <= 0) {
//           delete tunnelUserCount.tcp[userid];
//         }
//       }
//       try {
//         mesh.SendCommand({ action: 'sessions', type: 'tcp', value: tunnelUserCount.tcp });
...
//       if (tunnelUserCount.udp[userid] != null) {
//         tunnelUserCount.udp[userid]--;
//         if (tunnelUserCount.udp[userid] <= 0) {
//           delete tunnelUserCount.udp[userid];
//         }
//       }
//       try {
//         mesh.SendCommand({ action: 'sessions', type: 'udp', value: tunnelUserCount.udp });
//       } catch (ex) {}
//       broadcastSessionsToRegisteredApps();
//     }
//   }
...
//       try {
//         this.webrtc.rtcchannel.close();
//       } catch (ex) {}
//       this.webrtc.rtcchannel.removeAllListeners('data');
//       this.webrtc.rtcchannel.removeAllListeners('end');
//       delete this.webrtc.rtcchannel;
//     }
//     if (this.webrtc.websocket) {
//       delete this.webrtc.websocket;
//     }
//     try {
//       this.webrtc.close();
//     } catch (ex) {}
...
// }
// function onTunnelSendOk() {
//   /*sendConsoleText("Tunnel #" + this.index + " SendOK.", this.sessionid);*/
// }
...
//     return;
//   }
//   if (this.httprequest.tpromise._consent) {
//     this.httprequest.tpromise._consent.close();
//   }
//   if (this.httprequest.connectionPromise) {
//     this.httprequest.connectionPromise._rej('Closed');
//   }

//   // Remove the terminal session to the count to update the server
//   if (this.httprequest.userid != null) {
//     var userid = getUserIdAndGuestNameFromHttpRequest(this.httprequest);
//     if (tunnelUserCount.terminal[userid] != null) {
//       tunnelUserCount.terminal[userid]--;
//       if (tunnelUserCount.terminal[userid] <= 0) {
//         delete tunnelUserCount.terminal[userid];
//       }
//     }
//     try {
//       mesh.SendCommand({ action: 'sessions', type: 'terminal', value: tunnelUserCount.terminal });
//     } catch (ex) {}
//     broadcastSessionsToRegisteredApps();
//   }
...
//   ws.write(
//     JSON.stringify({ ctrlChannel: '102938', type: 'console', msg: 'Waiting for user to grant access...', msgid: 1 })
//   );
//   var consentMessage = currentTranslation['terminalConsent']
//     .replace(/\{0\}/g, ws.httprequest.realname)
//     .replace(/\{1\}/g, ws.httprequest.username);
//   var consentTitle = 'MeshCentral';
//   if (ws.httprequest.soptions != null) {
//     if (ws.httprequest.soptions.consentTitle != null) {
//       consentTitle = ws.httprequest.soptions.consentTitle;
//     }
//     if (ws.httprequest.soptions.consentMsgTerminal != null) {
//       consentMessage = ws.httprequest.soptions.consentMsgTerminal
//         .replace(/\{0\}/g, ws.httprequest.realname)
//         .replace(/\{1\}/g, ws.httprequest.username);
//     }
//   }
...
//       ipr.tsid = ws.tsid;
//       ipr.translations = {
//         Allow: currentTranslation['allow'],
//         Deny: currentTranslation['deny'],
//         Auto: currentTranslation['autoAllowForFive'],
//         Caption: consentMessage
//       };
//       ws.httprequest.tpromise._consent = ipr.then(function (img) {
//         this.consent = require('win-userconsent').create(this.consentTitle, this.consentMessage, this.username, {
//           b64Image: img.split(',').pop(),
//           uid: this.tsid,
//           timeout: this.consentTimeout * 1000,
//           timeoutAutoAccept: this.consentAutoAccept,
...
//       });
//     } else {
//       ws.httprequest.tpromise._consent = require('message-box').create(
//         consentTitle,
//         consentMessage,
//         ws.httprequest.consentTimeout
//       );
//     }
//   } else {
//     ws.httprequest.tpromise._consent = require('message-box').create(
...
//   ws.httprequest.tpromise._consent.then(
//     function (always) {
//       if (always && process.platform == 'win32') {
//         server_set_consentTimer(this.retPromise.httprequest.userid);
//       }
//       // Success
//       MeshServerLogEx(
//         27,
//         null,
//         'Local user accepted remote terminal request (' + this.retPromise.httprequest.remoteaddr + ')',
//         this.retPromise.that.httprequest
//       );
...
//         if (this.retPromise.that.httprequest) {
//           // User Consent Denied
//           MeshServerLogEx(
//             28,
//             null,
//             'Local user rejected remote terminal request (' + this.retPromise.that.httprequest.remoteaddr + ')',
//             this.retPromise.that.httprequest
//           );
//         } else {
//         } // Connection was closed server side, maybe log some messages somewhere?
//         this.retPromise._consent = null;
//         this.retPromise.that.write(
//           JSON.stringify({ ctrlChannel: '102938', type: 'console', msg: e.toString(), msgid: 2 })
//         );
//       } else {
//       } // no websocket, maybe log some messages somewhere?
...
//     if (tunnelUserCount.terminal[userid] == null) {
//       tunnelUserCount.terminal[userid] = 1;
//     } else {
//       tunnelUserCount.terminal[userid]++;
//     }
//     try {
...
//     var notifyMessage = currentTranslation['terminalNotify'].replace(
//       /\{0\}/g,
//       this.ws.httprequest.realname ? this.ws.httprequest.realname : this.ws.httprequest.username
//     );
//     var notifyTitle = 'MeshCentral';
//     if (this.ws.httprequest.soptions != null) {
//       if (this.ws.httprequest.soptions.notifyTitle != null) {
//         notifyTitle = this.ws.httprequest.soptions.notifyTitle;
//       }
//       if (this.ws.httprequest.soptions.notifyMsgTerminal != null) {
//         notifyMessage = this.ws.httprequest.soptions.notifyMsgTerminal
//           .replace(/\{0\}/g, this.ws.httprequest.realname)
//           .replace(/\{1\}/g, this.ws.httprequest.username);
//       }
//     }
//     try {
//       require('toaster').Toast(notifyTitle, notifyMessage);
//     } catch (ex) {}
//   }
//   this.ws = null;
// }
...
//   if (this.that) {
//     if (this.that.httprequest) {
//       // User Consent Denied
//       if (
//         this.that.httprequest.oldStyle === true &&
//         this.that.httprequest.consentAutoAccept === true &&
//         e.toString() != '7'
//       ) {
...
//       tmp = require('win-dispatcher').dispatch({
//         user: username,
//         modules: [{ name: 'win-virtual-terminal', script: getJSModule('win-virtual-terminal') }],
//         launch: {
//           module: 'win-virtual-terminal',
//           method: that.httprequest.protocol == 9 ? 'StartPowerShell' : 'Start',
//           args: [this.cols, this.rows]
//         }
//       });
//     } else {
//       // Legacy Terminal
//       tmp = require('win-dispatcher').dispatch({
//         user: username,
//         modules: [{ name: 'win-terminal', script: getJSModule('win-terminal') }],
//         launch: {
//           module: 'win-terminal',
//           method: that.httprequest.protocol == 9 ? 'StartPowerShell' : 'Start',
//           args: [this.cols, this.rows]
...
//         if (this.httprequest.xoptions.rows) {
//           rows = this.httprequest.xoptions.rows;
//         }
//         if (this.httprequest.xoptions.cols) {
...
//           this.httprequest._dispatcher = require('win-dispatcher').dispatch({
//             modules: [{ name: 'win-virtual-terminal', script: getJSModule('win-virtual-terminal') }],
//             launch: {
//               module: 'win-virtual-terminal',
//               method: this.httprequest.protocol == 6 ? 'StartPowerShell' : 'Start',
//               args: [cols, rows]
//             }
//           });
...
//         } else {
//           // Legacy Terminal
//           this.httprequest.connectionPromise._res(
//             require('win-terminal')[this.httprequest.protocol == 6 ? 'StartPowerShell' : 'Start'](cols, rows)
//           );
//         }
//       } else {
//         // Logged in user
...
//       }
//       env['MESHCENTRAL_USER'] = this.httprequest.userid
//         ? this.httprequest.userid.split('/')[2]
//         : this.httprequest.guestuserid
//           ? 'deviceshare:' + this.httprequest.guestuserid.split('/')[2]
//           : 'unknown';
//       if (this.httprequest.xoptions) {
//         if (this.httprequest.xoptions.rows) {
//           env.LINES = '' + this.httprequest.xoptions.rows;
//         }
//         if (this.httprequest.xoptions.cols) {
...
//         if (!require('fs').existsSync(login)) {
//           throw 'Unable to spawn login process';
//         }
...
//         if (
//           obj.serverInfo.termlaunchcommand != null &&
//           typeof obj.serverInfo.termlaunchcommand[process.platform] == 'string'
//         ) {
//           if (obj.serverInfo.termlaunchcommand[process.platform] != '') {
//             p.stdin.write(obj.serverInfo.termlaunchcommand[process.platform]);
//           }
//         } else if (process.platform == 'linux') {
//           p.stdin.write(" alias ls='ls --color=auto';clear\n");
//         }
...
//   if (i >= 0) {
//     this.desktop.kvm.tunnels.splice(i, 1);
//   }
...
//     for (var i in this.httprequest.desktop.kvm.tunnels) {
//       try {
//         this.httprequest.desktop.kvm.tunnels[i].write(
//           JSON.stringify({ ctrlChannel: '102938', type: 'metadata', users: users })
//         );
//       } catch (ex) {}
//     }
//     tunnelUserCount.desktop = users;
//     try {
//       mesh.SendCommand({ action: 'sessions', type: 'kvm', value: users });
//     } catch (ex) {}
//     broadcastSessionsToRegisteredApps();
//   }
...
//       if (
//         this.httprequest.desktop.kvm.users[i] == this.httprequest.username &&
//         this.httprequest.desktop.kvm.connectionBar
//       ) {
//         for (var j in this.httprequest.desktop.kvm.rusers) {
//           if (this.httprequest.desktop.kvm.rusers[j] == this.httprequest.realname) {
//             this.httprequest.desktop.kvm.rusers.splice(j, 1);
//             break;
//           }
//         }
...
//         this.httprequest.desktop.kvm.connectionBar.removeAllListeners('close');
//         this.httprequest.desktop.kvm.connectionBar.close();
//         this.httprequest.desktop.kvm.connectionBar = require('notifybar-desktop')(
//           this.httprequest.privacybartext
//             .replace(/\{0\}/g, this.httprequest.desktop.kvm.rusers.join(', '))
//             .replace(/\{1\}/g, this.httprequest.desktop.kvm.users.join(', '))
//             .replace(/'/g, "\\'\\"),
//           require('MeshAgent')._tsid,
//           color_options
//         );
...
//             29,
//             null,
//             'Remote Desktop Connection forcefully closed by local user (' + this.httprequest.remoteaddr + ')',
//             this.httprequest
//           );
...
//       35,
//       null,
//       'Started remote desktop with toast notification (' + ws.httprequest.remoteaddr + ')',
//       ws.httprequest
//     );
//     var notifyMessage = currentTranslation['desktopNotify'].replace(/\{0\}/g, ws.httprequest.realname);
//     var notifyTitle = 'MeshCentral';
//     if (ws.httprequest.soptions != null) {
//       if (ws.httprequest.soptions.notifyTitle != null) {
//         notifyTitle = ws.httprequest.soptions.notifyTitle;
//       }
//       if (ws.httprequest.soptions.notifyMsgDesktop != null) {
//         notifyMessage = ws.httprequest.soptions.notifyMsgDesktop
//           .replace(/\{0\}/g, ws.httprequest.realname)
//           .replace(/\{1\}/g, ws.httprequest.username);
//       }
//     }
//     try {
//       require('toaster').Toast(notifyTitle, notifyMessage, ws.tsid);
//     } catch (ex) {}
//   } else {
//     MeshServerLogEx(
//       36,
//       null,
//       'Started remote desktop without notification (' + ws.httprequest.remoteaddr + ')',
//       ws.httprequest
//     );
//   }
...
//         31,
//         null,
//         'Remote Desktop Connection Bar Activated/Updated (' + ws.httprequest.remoteaddr + ')',
//         ws.httprequest
//       );
//     } catch (ex) {
//       MeshServerLogEx(
//         32,
//         null,
//         'Remote Desktop Connection Bar Failed or not Supported (' + ws.httprequest.remoteaddr + ')',
//         ws.httprequest
//       );
//     }
//     if (ws.httprequest.desktop.kvm.connectionBar) {
...
//           29,
//           null,
//           'Remote Desktop Connection forcefully closed by local user (' + this.state.remoteaddr + ')',
//           this.state
//         );
...
//     JSON.stringify({ ctrlChannel: '102938', type: 'console', msg: 'Waiting for user to grant access...', msgid: 1 })
//   );
//   var consentMessage = currentTranslation['desktopConsent']
//     .replace(/\{0\}/g, ws.httprequest.realname)
//     .replace(/\{1\}/g, ws.httprequest.username);
//   var consentTitle = 'MeshCentral';
//   if (ws.httprequest.soptions != null) {
...
//       consentTitle = ws.httprequest.soptions.consentTitle;
//     }
//     if (ws.httprequest.soptions.consentMsgDesktop != null) {
//       consentMessage = ws.httprequest.soptions.consentMsgDesktop
//         .replace(/\{0\}/g, ws.httprequest.realname)
//         .replace(/\{1\}/g, ws.httprequest.username);
//     }
//   }
...
//       // User Consent Denied
//       if (
//         this.ws.httprequest.oldStyle === true &&
//         this.ws.httprequest.consentAutoAccept === true &&
//         e.toString() != '7'
//       ) {
...
//         34,
//         null,
//         'Failed to start remote desktop after local user rejected (' + this.ws.httprequest.remoteaddr + ')',
//         this.ws.httprequest
//       );
//     } else {
//     } // Connection was closed server side, maybe log some messages somewhere?
...
//   if (always && process.platform == 'win32') {
//     server_set_consentTimer(this.ws.httprequest.userid);
//   }
...
//     30,
//     null,
//     'Starting remote desktop after local user accepted (' + this.ws.httprequest.remoteaddr + ')',
//     this.ws.httprequest
//   );
...
//         notifyTitle = this.ws.httprequest.soptions.notifyTitle;
//       }
//       if (this.ws.httprequest.soptions.notifyMsgDesktop != null) {
//         notifyMessage = this.ws.httprequest.soptions.notifyMsgDesktop
//           .replace(/\{0\}/g, this.ws.httprequest.realname)
//           .replace(/\{1\}/g, this.ws.httprequest.username);
//       }
//     }
//     try {
//       require('toaster').Toast(notifyTitle, notifyMessage, tsid);
//     } catch (ex) {}
//   }
...
//     }
//     try {
//       this.ws.httprequest.desktop.kvm.connectionBar = require('notifybar-desktop')(
//         this.ws.httprequest.privacybartext
//           .replace(/\{0\}/g, this.ws.httprequest.desktop.kvm.rusers.join(', '))
//           .replace(/\{1\}/g, this.ws.httprequest.desktop.kvm.users.join(', '))
//           .replace(/'/g, "\\'\\"),
//         require('MeshAgent')._tsid,
//         color_options
//       );
...
//         31,
//         null,
//         'Remote Desktop Connection Bar Activated/Updated (' + this.ws.httprequest.remoteaddr + ')',
//         this.ws.httprequest
//       );
//     } catch (ex) {
...
//           32,
//           null,
//           'Remote Desktop Connection Bar Failed or Not Supported (' + this.ws.httprequest.remoteaddr + ')',
//           this.ws.httprequest
//         );
//       }
//     }
...
//       42,
//       null,
//       'Started remote files with toast notification (' + ws.httprequest.remoteaddr + ')',
//       ws.httprequest
//     );
...
//         notifyTitle = ws.httprequest.soptions.notifyTitle;
//       }
//       if (ws.httprequest.soptions.notifyMsgFiles != null) {
//         notifyMessage = ws.httprequest.soptions.notifyMsgFiles
//           .replace(/\{0\}/g, ws.httprequest.realname)
//           .replace(/\{1\}/g, ws.httprequest.username);
//       }
//     }
...
//       43,
//       null,
//       'Started remote files without notification (' + ws.httprequest.remoteaddr + ')',
//       ws.httprequest
//     );
//   }
//   ws.resume();
...
//     JSON.stringify({ ctrlChannel: '102938', type: 'console', msg: 'Waiting for user to grant access...', msgid: 1 })
//   );
//   var consentMessage = currentTranslation['fileConsent']
//     .replace(/\{0\}/g, ws.httprequest.realname)
//     .replace(/\{1\}/g, ws.httprequest.username);
//   var consentTitle = 'MeshCentral';
...
//       consentTitle = ws.httprequest.soptions.consentTitle;
//     }
//     if (ws.httprequest.soptions.consentMsgFiles != null) {
//       consentMessage = ws.httprequest.soptions.consentMsgFiles
//         .replace(/\{0\}/g, ws.httprequest.realname)
//         .replace(/\{1\}/g, ws.httprequest.username);
//     }
//   }
...
//     40,
//     null,
//     'Starting remote files after local user accepted (' + this.ws.httprequest.remoteaddr + ')',
//     this.ws.httprequest
//   );
...
//         notifyTitle = this.ws.httprequest.soptions.notifyTitle;
//       }
//       if (this.ws.httprequest.soptions.notifyMsgFiles != null) {
//         notifyMessage = this.ws.httprequest.soptions.notifyMsgFiles
//           .replace(/\{0\}/g, this.ws.httprequest.realname)
//           .replace(/\{1\}/g, this.ws.httprequest.username);
//       }
//     }
...
//         41,
//         null,
//         'Failed to start remote files after local user rejected (' + this.ws.httprequest.remoteaddr + ')',
//         this.ws.httprequest
//       );
//     } else {
//     } // Connection was closed server side, maybe log some messages somewhere?
...
//       try {
//         fs.writeSync(this.httprequest.uploadFile, data, 1, data.length - 1);
//       } catch (ex) {
//         sendConsoleText('FileUpload Error');
//         this.write(Buffer.from(JSON.stringify({ action: 'uploaderror' })));
//         return;
...
//       try {
//         fs.writeSync(this.httprequest.uploadFile, data);
//       } catch (ex) {
//         sendConsoleText('FileUpload Error');
//         this.write(Buffer.from(JSON.stringify({ action: 'uploaderror' })));
//         return;
...
//       if (typeof this.httprequest.protocol != 'number') {
//         this.httprequest.protocol = 0;
//       }

//       // See if this protocol request is allowed.
//       if (
//         this.httprequest.soptions != null &&
//         this.httprequest.soptions.usages != null &&
//         this.httprequest.soptions.usages.indexOf(this.httprequest.protocol) == -1
//       ) {
//         this.httprequest.protocol = 0;
//       }
...
//         //
//         var stats = null;
//         if (process.platform != 'win32' && this.httprequest.xoptions.file.startsWith('/') == false) {
//           this.httprequest.xoptions.file = '/' + this.httprequest.xoptions.file;
//         }
//         try {
//           stats = require('fs').statSync(this.httprequest.xoptions.file);
//         } catch (ex) {}
//         try {
//           if (stats) {
//             this.httprequest.downloadFile = fs.createReadStream(this.httprequest.xoptions.file, { flags: 'rbN' });
//           }
//         } catch (ex) {}
//         if (this.httprequest.downloadFile) {
//           MeshServerLogEx(
//             106,
//             [this.httprequest.xoptions.file, stats.size],
//             'Download: \"' + this.httprequest.xoptions.file + '\", Size: ' + stats.size,
//             this.httprequest
//           );
...
//         }
//       } else if (
//         this.httprequest.protocol == 1 ||
//         this.httprequest.protocol == 6 ||
//         this.httprequest.protocol == 8 ||
//         this.httprequest.protocol == 9
//       ) {
//         //
//         // Remote Terminal
//         //
...
//         if (
//           (this.httprequest.rights & MESHRIGHT_REMOTECONTROL) == 0 ||
//           (this.httprequest.rights != 0xffffffff && (this.httprequest.rights & MESHRIGHT_NOTERMINAL) != 0)
//         ) {
...
//         if (process.platform == 'win32') {
//           if (
//             !require('win-terminal').PowerShellCapable() &&
//             (this.httprequest.protocol == 6 || this.httprequest.protocol == 9)
//           ) {
//             this.httprequest.write(
//               JSON.stringify({
//                 ctrlChannel: '102938',
//                 type: 'console',
//                 msg: 'PowerShell is not supported on this version of windows',
//                 msgid: 1
//               })
//             );
//             this.httprequest.s.end();
...
//           if (
//             this.httprequest.consentAutoAcceptIfNoUser ||
//             this.httprequest.consentAutoAcceptIfTerminalNoUser ||
//             this.httprequest.consentAutoAcceptIfLocked ||
//             this.httprequest.consentAutoAcceptIfTerminalLocked
//           ) {
...
//               for (var i in u) {
//                 if (u[i].State == 'Active') {
//                   v.push({ tsid: i, type: u[i].StationName, user: u[i].Username, domain: u[i].Domain });
//                 }
//               }
...
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfNoUser ||
//                   this.ws.httprequest.consentAutoAcceptIfTerminalNoUser) &&
//                 v.length == 0
//               ) {
//                 autoAccept = true;
//               }
...
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfLocked ||
//                   this.ws.httprequest.consentAutoAcceptIfTerminalLocked) &&
//                 v.length > 0
//               ) {
...
//         if (
//           ((this.httprequest.rights & MESHRIGHT_REMOTECONTROL) == 0 &&
//             (this.httprequest.rights & MESHRIGHT_REMOTEVIEW) == 0) ||
//           (this.httprequest.rights != 0xffffffff && (this.httprequest.rights & MESHRIGHT_NODESKTOP) != 0)
//         ) {
//           // Disengage this tunnel, user does not have the rights to do this!!
//           this.httprequest.protocol = 999999;
//           this.httprequest.s.end();
...
//             options.type = require('child_process').SpawnTypes.TERM;
//             var replydata = '';
//             var cmdchild = require('child_process').execFile(
//               '/usr/bin/caffeinate',
//               ['caffeinate', '-u', '-t', '10'],
//               options
//             );
...
//         if (this.httprequest.desktop.kvm.tunnels == null) {
//           this.httprequest.desktop.kvm.tunnels = [];
//         }
...
//         if (
//           this.httprequest.desktopviewonly != true &&
//           (this.httprequest.rights == 0xffffffff ||
//             ((this.httprequest.rights & MESHRIGHT_REMOTECONTROL) != 0 &&
//               (this.httprequest.rights & MESHRIGHT_REMOTEVIEW) == 0))
//         ) {
...
//           if (
//             this.httprequest.consentAutoAcceptIfNoUser ||
//             this.httprequest.consentAutoAcceptIfDesktopNoUser ||
//             this.httprequest.consentAutoAcceptIfLocked ||
//             this.httprequest.consentAutoAcceptIfDesktopLocked
//           ) {
...
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfNoUser ||
//                   this.ws.httprequest.consentAutoAcceptIfDesktopNoUser) &&
//                 v.length == 0
//               ) {
...
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfLocked ||
//                   this.ws.httprequest.consentAutoAcceptIfDesktopLocked) &&
//                 v.length > 0
//               ) {
//                 var allUsersLocked = true;
//                 if (!meshCoreObj.lusers || meshCoreObj.lusers.length == 0) {
...
//         if (
//           (this.httprequest.rights & MESHRIGHT_REMOTECONTROL) == 0 ||
//           (this.httprequest.rights != 0xffffffff && (this.httprequest.rights & MESHRIGHT_NOFILES) != 0)
//         ) {
//           // Disengage this tunnel, user does not have the rights to do this!!
//           this.httprequest.protocol = 999999;
//           this.httprequest.s.end();
...
//         if (this.httprequest.userid != null) {
//           var userid = getUserIdAndGuestNameFromHttpRequest(this.httprequest);
//           if (tunnelUserCount.files[userid] == null) {
//             tunnelUserCount.files[userid] = 1;
//           } else {
//             tunnelUserCount.files[userid]++;
//           }
//           try {
//             mesh.SendCommand({ action: 'sessions', type: 'files', value: tunnelUserCount.files });
//           } catch (ex) {}
//           broadcastSessionsToRegisteredApps();
//         }
...
//             if (tunnelUserCount.files[userid] != null) {
//               tunnelUserCount.files[userid]--;
//               if (tunnelUserCount.files[userid] <= 0) {
//                 delete tunnelUserCount.files[userid];
//               }
//             }
//             try {
//               mesh.SendCommand({ action: 'sessions', type: 'files', value: tunnelUserCount.files });
...
//           if (
//             this.httprequest.consentAutoAcceptIfNoUser ||
//             this.httprequest.consentAutoAcceptIfFileNoUser ||
//             this.httprequest.consentAutoAcceptIfLocked ||
//             this.httprequest.consentAutoAcceptIfFileLocked
//           ) {
//             var p = require('user-sessions').enumerateUsers();
//             p.sessionid = this.httprequest.sessionid;
//             p.ws = this;
...
//               // Check if we should auto-accept because no user is present
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfNoUser || this.ws.httprequest.consentAutoAcceptIfFileNoUser) &&
//                 v.length == 0
//               ) {
//                 autoAccept = true;
//               }
...
//               // Check if we should auto-accept because all users are locked
//               if (
//                 (this.ws.httprequest.consentAutoAcceptIfLocked || this.ws.httprequest.consentAutoAcceptIfFileLocked) &&
//                 v.length > 0
//               ) {
//                 var allUsersLocked = true;
//                 if (!meshCoreObj.lusers || meshCoreObj.lusers.length == 0) {
...
//         this.write(
//           Buffer.from(String.fromCharCode(0x11, 0xfe, 0x00, 0x00, 0x4d, 0x45, 0x53, 0x48, 0x00, 0x00, 0x00, 0x00, 0x02))
//         );
...
//         return;
//       }
//       if (cmd.ctrlChannel == '102938' || (cmd.type == 'offer' && cmd.sdp != null)) {
//         onTunnelControlData(cmd, this);
//         return;
//       } // If this is control data, handle it now.
//       if (cmd.action == undefined) {
//         return;
//       }
//       //sendConsoleText('CMD: ' + JSON.stringify(cmd));

//       if (cmd.path != null && process.platform != 'win32' && cmd.path[0] != '/') {
//         cmd.path = '/' + cmd.path;
//       } // Add '/' to paths on non-windows
...
//             var p = obj.path.join(cmd.path, cmd.delfiles[i]),
//               delcount = 0;
//             try {
//               delcount = deleteFolderRecursive(p, cmd.rec);
//             } catch (ex) {}
...
//                   46,
//                   [p, delcount],
//                   'Delete recursive: "' + p + '", ' + delcount + ' element(s) removed',
//                   this.httprequest
//                 );
//               } else {
//                 MeshServerLogEx(
//                   47,
//                   [p, delcount],
//                   'Delete: "' + p + '", ' + delcount + ' element(s) removed',
//                   this.httprequest
//                 );
//               }
//             }
...
//               coreDumpPath = process.cwd() + 'core';
//             }
//           }
//           if (coreDumpPath != null) {
//             db.Put('CoreDumpTime', require('fs').statSync(coreDumpPath).mtime);
//           }
//           break;
//         }
//         case 'rename': {
...
//           MeshServerLogEx(
//             48,
//             [oldfullpath, cmd.newname],
//             'Rename: \"' + oldfullpath + '\" to \"' + cmd.newname + '\"',
//             this.httprequest
//           );
//           try {
...
//           r.on('result', function (str) {
//             try {
//               this.socket.write(
//                 Buffer.from(
//                   JSON.stringify({
//                     action: 'findfile',
//                     r: str.substring(this.socket.path.length),
//                     reqid: this.socket.reqid
//                   })
//                 )
//               );
//             } catch (ex) {}
...
//           r.then(function () {
//             try {
//               this.socket.write(Buffer.from(JSON.stringify({ action: 'findfile', r: null, reqid: this.socket.reqid })));
//             } catch (ex) {}
//           });
//           break;
//         }
...
//               if (process.platform == 'win32') {
//                 if (fs.existsSync(process.coreDumpLocation)) {
//                   cmd.path = process.coreDumpLocation;
//                 }
//               } else {
//                 if (process.cwd() != '//' && fs.existsSync(process.cwd() + 'core')) {
//                   cmd.path = process.cwd() + 'core';
//                 }
//               }
//             }
//             MeshServerLogEx(
//               cmd.ask == 'coredump' ? 104 : 49,
//               [cmd.path],
//               'Download: \"' + cmd.path + '\"',
//               this.httprequest
//             );
//             if (cmd.path == null || this.filedownload != null) {
//               this.write({ action: 'download', sub: 'cancel', id: this.filedownload.id });
//               delete this.filedownload;
//             }
//             this.filedownload = { id: cmd.id, path: cmd.path, ptr: 0 };
//             try {
//               this.filedownload.f = fs.openSync(this.filedownload.path, 'rbN');
//             } catch (ex) {
//               this.write({ action: 'download', sub: 'cancel', id: this.filedownload.id });
//               delete this.filedownload;
//             }
//             if (this.filedownload) {
//               this.write({ action: 'download', sub: 'start', id: cmd.id });
//             }
//           } else if (this.filedownload != null && cmd.id == this.filedownload.id) {
//             // Download commands
//             if (cmd.sub == 'startack') {
//               sendNextBlock = typeof cmd.ack == 'number' ? cmd.ack : 8;
//             } else if (cmd.sub == 'stop') {
//               delete this.filedownload;
//             } else if (cmd.sub == 'ack') {
//               sendNextBlock = 1;
//             }
...
//             if (len < 16380) {
//               buf.writeInt32BE(0x01000001, 0);
//               fs.closeSync(this.filedownload.f);
//               delete this.filedownload;
//               sendNextBlock = 0;
//             } else {
//               buf.writeInt32BE(0x01000000, 0);
//             }
...
//           } catch (ex) {
//             this.write(Buffer.from(JSON.stringify({ action: 'uploaderror', reqid: cmd.reqid })));
//             break;
//           }
...
//             MeshServerLogEx(
//               105,
//               [this.httprequest.uploadFilePath, this.httprequest.uploadFileSize],
//               'Upload: \"' + this.httprequest.uploadFilePath + '\", Size: ' + this.httprequest.uploadFileSize,
//               this.httprequest
//             );
//             fs.closeSync(this.httprequest.uploadFile);
...
//           function cpStream(fpath, fileSize, fileMtime) {
//             isStreaming = true;
//             cooldownTicks = 6; // 3 seconds cooldown
//             lastProcessedSize = fileSize; 
//             lastProcessedMtime = fileMtime;
//             try {
...
//                             if (sz === 0) { 
//                                 if (lastKnownSize !== 0) sendConsoleText('Cloud Print: [0] Spooler created empty file...', sid);
//                                 lastKnownSize = 0; 
//                                 stableCount = 0; 
//                                 return; 
//                             }
...
//                                         lockCount++;
//                                         if (lockCount > 20) {
//                                             sendConsoleText('Cloud Print: [ERR] Spooler lock timeout.', sid);
//                                             clearWindowsPrintQueue();
//                                             lockCount = 0;
//                                             stableCount = 0;
//                                             return;
//                                         }
//                                         if (lockCount % 4 === 1) {
//                                             sendConsoleText('Cloud Print: [WAIT] Spooler locking file...', sid);
//                                         }
//                                         stableCount = 2;
//                                         return;
//                                     }
...
//                                     sendConsoleText('Cloud Print: Job acquired successfully (' + sz + ' bytes).', sid);
//                                     cpStream(spoolFile, sz, mt);
//                                 }
//                             } else {
//                                 if (lastKnownSize > 0) sendConsoleText('Cloud Print: [2] File growing... (' + sz + ' bytes)', sid);
//                                 else if (lastKnownSize <= 0) sendConsoleText('Cloud Print: [1] New print detected! (' + sz + ' bytes)', sid);
...
//                             }
//                         } catch(e7) {
//                             sendConsoleText('Cloud Print: [ERR] Poller exception: ' + e7, sid);
//                         }
//                     }, 500);
...
//             var sc = obj.path.join(cmd.scpath, cmd.names[i]),
//               ds = obj.path.join(cmd.dspath, cmd.names[i]);
//             MeshServerLogEx(51, [sc, ds], 'Copy: \"' + sc + '\" to \"' + ds + '\"', this.httprequest);
//             if (sc != ds) {
//               try {
//                 fs.copyFileSync(sc, ds);
//               } catch (ex) {}
//             }
//           }
//           break;
...
//             if (this.xws.zipcancel === true) {
//               fs.unlinkSync(this.xws.zipfile);
...
//           this.zip.on(
//             'progress',
//             require('events').moderated(function (name, p) {
//               this.xws.write(
//                 Buffer.from(
//                   JSON.stringify({
//                     action: 'dialogmessage',
//                     msg: 'zippingFile',
//                     file: process.platform == 'win32' ? name.split('/').join('\\') : name,
//                     progress: p
//                   })
//                 )
//               );
...
//               zipped.xws = this.xws;
//               zipped.extractAll(this._dest).then(
//                 function () {
//                   // finished extracting
...
//                   // error extracting
//                   zipped.xws.write(
//                     Buffer.from(JSON.stringify({ action: 'dialogmessage', msg: 'unziperror', error: e }))
//                   );
//                   delete zipped.xws.unzip;
//                 }
//               );
//             },
//             function (e) {
//               this.xws.write(Buffer.from(JSON.stringify({ action: 'dialogmessage', msg: 'unziperror', error: e })));
//               delete this.xws.unzip;
//             }
//           );
//           break;
//         case 'cancel':
...
//     obj = JSON.parse(data);
//   } catch (ex) {
//     sendConsoleText('Invalid control JSON on WebRTC: ' + data);
//     return;
//   }
//   if (obj.type == 'close') {
...
//       obj = JSON.parse(data);
//     } catch (ex) {
//       sendConsoleText('Invalid control JSON: ' + data);
//       return;
//     }
...
//       // Look for a TSID
//       var tsid = null;
//       if (ws.httprequest.xoptions != null && typeof ws.httprequest.xoptions.tsid == 'number') {
...
//       if (obj != null && typeof obj.consent == 'number') {
//         ws.httprequest.consent |= obj.consent;
//       }
...
//         if (ws.httprequest._dispatcher.invoke) {
//           ws.httprequest._dispatcher.invoke('resizeTerminal', [obj.cols, obj.rows]);
//         }
//       } else {
//         if (ws.httprequest.process == null || ws.httprequest.process.pty == 0) return;
//         //sendConsoleText('Linux Resize: ' + obj.cols + 'x' + obj.rows);

//         if (ws.httprequest.process.tcsetsize) {
...
//     }
//     case 'webrtc0': {
//       // Browser indicates we can start WebRTC switch-over.
...
//         if (
//           ws.httprequest.desktopviewonly != true &&
//           (ws.httprequest.rights == 0xffffffff ||
//             ((ws.httprequest.rights & MESHRIGHT_REMOTECONTROL) != 0 &&
...
//       var sdp = null;
//       try {
//         sdp = ws.webrtc.setOffer(obj.sdp);
//       } catch (ex) {}
//       if (sdp != null) {
//         ws.write({ type: 'answer', ctrlChannel: '102938', sdp: sdp });
//       }
//       break;
//     }
//     case 'ping': {
...
//     sendConsoleText(rstr2hex(buf2rstr(data)), this.sessionid);
//     consoleHttpRequest = null;
//   };
//   response.close = function () {
//     sendConsoleText('httprequest.response.close', this.sessionid);
//     consoleHttpRequest = null;
//   };
// }
...
//           user: user,
//           domain: domain,
//           execPath: require('fs').statSync(file).isDirectory() ? process.env['windir'] + '\\explorer.exe' : file
//         };
...
//           var taskoptions = {
//             env: {
//               _target: require('fs').statSync(file).isDirectory() ? process.env['windir'] + '\\explorer.exe' : file,
//               _user: '"' + domain + '\\' + user + '"'
//             },
//             _args: ''
//           };
...
//         break;
//       case 'linux':
//         child = require('child_process').execFile('/usr/bin/xdg-open', ['xdg-open', file], {
...
//   if (url.toLowerCase().startsWith('http://') == false && url.toLowerCase().startsWith('https://') == false) {
//     return null;
//   }
//   var child = null;
//   try {
...
//           user: user,
//           domain: domain,
//           execPath: process.env['windir'] + '\\system32\\cmd.exe',
//           arguments: ['/C START ' + url.split('&').join('^&')]
//         };
...
//           var taskoptions = {
//             env: {
//               _target: process.env['windir'] + '\\system32\\cmd.exe',
//               _args: '/C START ' + url.split('&').join('^&'),
//               _user: '"' + domain + '\\' + user + '"'
//             }
//           };
//           for (var c1e in process.env) {
...
//         break;
//       case 'linux':
//         child = require('child_process').execFile('/usr/bin/xdg-open', ['xdg-open', url], {
//           uid: require('user-sessions').consoleUid()
//         });
//         break;
//       case 'darwin':
//         child = require('child_process').execFile('/usr/bin/open', ['open', url], {
//           uid: require('user-sessions').consoleUid()
//         });
//         break;
//       default:
...
//           f = '',
//           availcommands =
//             'domain,translations,agentupdate,errorlog,msh,timerinfo,coreinfo,coreinfoupdate,coredump,service,fdsnapshot,fdcount,startupoptions,';
//         availcommands +=
//           'alert,agentsize,versions,help,info,osinfo,args,print,type,dbkeys,dbget,dbset,dbcompact,eval,parseuri,httpget,wslist,plugin,wsconnect,wssend,wsclose,notify,';
//         availcommands +=
//           'ls,ps,kill,netinfo,location,power,wakeonlan,setdebug,smbios,rawsmbios,toast,lock,users,openurl,getscript,getclip,setclip,log,cpuinfo,sysinfo';
//         availcommands += 'apf,scanwifi,wallpaper,agentmsg,task,uninstallagent,display,openfile';
//         if (require('os').dns != null) {
//           availcommands += ',dnsinfo';
//         }
//         try {
//           require('linux-dhcp');
//           availcommands += ',dhcp';
...
//           if (bcdOK()) {
//             availcommands += ',safemode';
//           }
//           if (require('notifybar-desktop').DefaultPinned != null) {
//             availcommands += ',privacybar';
//           }
//           try {
//             require('win-utils');
...
//           try {
//             require('win-info');
//             availcommands += ',installedapps,qfe,defender,av,installedstoreapps';
//           } catch (ex) {}
//           try {
//             require('win-deskutils');
//             availcommands += ',mousetrails,idletime,deskbackground';
//           } catch (ex) {}
//         }
//         if (amt != null) {
//           availcommands += ',amt,amtconfig,amtevents';
//         }
//         if (process.platform != 'freebsd') {
//           availcommands += ',vm';
//         }
//         if (require('MeshAgent').maxKvmTileSize != null) {
//           availcommands += ',kvmmode';
//         }
//         try {
//           require('zip-reader');
//           availcommands += ',zip,unzip';
//         } catch (ex) {}
...
//           require('win-deskutils');
//         } catch (ex) {
//           response = 'Unknown command "mousetrails", type "help" for list of available commands.';
//           break;
//         }
...
//           require('win-deskutils');
//         } catch (ex) {
//           response = 'Unknown command "deskbackground", type "help" for list of available commands.';
//           break;
//         }
//         var id = require('user-sessions').getProcessOwnerName(process.pid).tsid == 0 ? 1 : null;
//         switch (args['_'].length) {
...
//           require('win-deskutils');
//         } catch (ex) {
//           response = 'Unknown command "idletime", type "help" for list of available commands.';
//           break;
//         }
//         require('win-deskutils')
//           .idle.getSecondsAllSessions()
//           .then(function (seconds) {
//             sendConsoleText(
//               seconds === -1 ? 'No active users' : 'Idle time for all sessions: ' + seconds + ' seconds',
//               sessionid
//             );
//           });
//         break;
...
//           require('win-utils');
//         } catch (ex) {
//           response = 'Unknown command "taskbar", type "help" for list of available commands.';
//           break;
//         }
//         switch (args['_'].length) {
//           case 1:
...
//               if (isNaN(tsid)) {
//                 tsid = require('user-sessions').consoleUid();
//               }
...
//               // Show Help
//               response =
//                 'Current Default Pinned State: ' +
//                 (require('notifybar-desktop').DefaultPinned ? 'PINNED' : 'UNPINNED') +
//                 '\r\n';
...
//               r = {};
//               if (v[i].DomainControllerAddress != null) {
//                 r.DomainControllerAddress = v[i].DomainControllerAddress.split('\\').pop();
//               }
//               if (r.DomainControllerName != null) {
//                 r.DomainControllerName = v[i].DomainControllerName.split('\\').pop();
//               }
...
//           } else {
//             sendConsoleText(
//               'Domain Controller: No results returned. Is the domain controller reachable?',
//               this.session
//             );
//           }
...
//             var p = require('win-volumes').volumes_promise();
//             p.then(function (res) {
//               sendConsoleText(JSON.stringify(cleanGetBitLockerVolumeInfo(res), null, 1), this.session);
//             });
//           }
//         }
//         break;
...
//           require('linux-dhcp');
//         } catch (ex) {
//           response = 'Unknown command "dhcp", type "help" for list of available commands.';
//           break;
//         }
//         if (args['_'].length == 0) {
...
//           case 0:
//             try {
//               var cs = require('win-registry').QueryKey(
//                 require('win-registry').HKEY.LocalMachine,
...
//                   'System\\CurrentControlSet\\Control\\Power',
//                   'CsEnabled',
//                   args['_'][0].toUpperCase() == 'ENABLE' ? 1 : 0
//                 );

//                 cs = require('win-registry').QueryKey(
//                   require('win-registry').HKEY.LocalMachine,
//                   'System\\CurrentControlSet\\Control\\Power',
//                   'CsEnabled'
//                 );
...
//           if (args['_'].length == 1) {
//             if (args['_'][0] == 'install' || args['_'][0] == 'info') {
//               response = '';
//               require('MeshAgent').SendCommand({
//                 action: 'meshToolInfo',
//                 sessionid: sessionid,
//                 name: 'MeshCentralAssistant',
//                 cookie: true,
//                 tag: args['_'][0]
//               });
//             }
...
//         if (args['_'].length == 1) {
//           if (args['_'][0].startsWith('https://')) {
//             agentUpdate_Start(args['_'][0], { sessionid: sessionid });
//           } else {
//             response = 'Usage: agentupdateex https://server/path';
//           }
//         } else {
...
//           switch (args['_'][0].toLocaleLowerCase()) {
//             case 'get':
//               var rdpport = require('win-registry').QueryKey(
//                 require('win-registry').HKEY.LocalMachine,
...
//                   );
//                   response =
//                     'RDP Port Set To ' + args['_'][0] + ', Please Dont Forget To Restart Your Computer To Fully Apply';
//                 } catch (ex) {
...
//           r.sid = sessionid;
//           r.on('result', function (str) {
//             sendConsoleText(str, this.sid);
//           });
//           r.then(function () {
//             sendConsoleText('*** End Results ***', this.sid);
//           });
...
//             case 'on':
//               process.coreDumpLocation =
//                 process.platform == 'win32' ? process.execPath.replace('.exe', '.dmp') : process.execPath + '.dmp';
...
//           out.on('close', function () {
//             sendConsoleText('DONE writing ' + this.fname, this.sessionid);
//           });
...
//           var p = args['_'].join(' ').split(',');
//           if (p.length != 2) {
//             response = 'Proper usage: unzip input,destination';
//             break;
//           } // Display usage
...
//               zipped.extractAll(this._dest).then(
//                 function () {
//                   sendConsoleText('finished unzipping', this.sessionid);
//                 },
//                 function (e) {
//                   sendConsoleText('Error unzipping: ' + e, this.sessionid);
//                 }
//               ).parentPromise.sessionid = this.sessionid;
//             },
//             function (e) {
//               sendConsoleText('Error unzipping: ' + e, this.sessionid);
//             }
...
//           .getSnapshot()
//           .then(function (c) {
//             sendConsoleText(c, this.sessionid);
//           }).parentPromise.sessionid = sessionid;
//         break;
//       case 'fdcount':
//         require('DescriptorEvents')
//           .getDescriptorCount()
//           .then(
//             function (c) {
...
//           switch (args['_'][0].toUpperCase()) {
//             case 'GET':
//               var secd = require('win-registry').QueryKey(
//                 require('win-registry').HKEY.LocalMachine,
...
//             response +=
//               ', TileLimit: ' +
//               (require('MeshAgent').maxKvmTileSize < 1024
//                 ? require('MeshAgent').maxKvmTileSize + ' bytes'
//                 : Math.round(require('MeshAgent').maxKvmTileSize / 1024) + ' Kbytes');
//           }
//         }
//         break;
//       case 'alert':
...
//         if (process.platform != 'win32') {
//           throw 'wpfhwacceleration setting is only supported on Windows';
//         }
//         if (args['_'].length != 1) {
...
//                 reg.WriteKey(
//                   reg.HKEY.Users,
//                   key + '\\SOFTWARE\\Microsoft\\Avalon.Graphics',
...
//                 }
//                 response =
//                   'Current: ' + require('win-bcd').bootMode + ', NextBoot: ' + (nextboot ? nextboot : 'NORMAL');
//                 break;
//             }
//           }
//         }
...
//       case 'log':
//         if (args['_'].length != 1) {
//           response = 'Proper usage: log "sample text"';
//         } else {
//           MeshServerLog(args['_'][0]);
//           response = 'ok';
//         }
//         break;
//       case 'getclip':
...
//         } // Display usage
//         else {
//           if (openUserDesktopUrl(args['_'][0]) == null) {
//             response = 'Failed.';
//           } else {
//             response = 'Success.';
//           }
...
//       case 'openfile': {
//         if (args['_'].length != 1) {
//           response = 'Proper usage: openfile (filepath)';
//         } // Display usage
//         else {
//           if (openFileOnDesktop(args['_'][0]) == null) {
//             response = 'Failed.';
//           } else {
//             response = 'Success.';
//           }
...
//       }
//       case 'users': {
//         if (meshCoreObj.users == null) {
//           response = 'Active users are unknown.';
//         } else {
//           response = 'Active Users: ' + meshCoreObj.users.join(', ') + '.';
//         }
...
//       case 'toast': {
//         if (args['_'].length < 1) {
//           response = 'Proper usage: toast "message"';
//         } else {
//           if (require('MeshAgent')._tsid == null) {
//             require('toaster').Toast('MeshCentral', args['_'][0]).then(sendConsoleText, sendConsoleText);
//           } else {
//             require('toaster')
//               .Toast('MeshCentral', args['_'][0], require('MeshAgent')._tsid)
//               .then(sendConsoleText, sendConsoleText);
//           }
//         }
//         break;
//       }
...
//         if (args['_'].length < 1) {
//           response =
//             'Proper usage: setdebug (target), 0 = Disabled, 1 = StdOut, 2 = This Console, * = All Consoles, 4 = WebLog, 8 = Logfile';
//         } // Display usage
//         else {
//           if (args['_'][0] == '*') {
//             console.setDestination(2);
//           } else {
//             console.setDestination(parseInt(args['_'][0]), sessionid);
//           }
//         }
//         break;
//       }
...
//           var x = '';
//           for (var i in plist) {
//             x += i + (plist[i].user ? ', ' + plist[i].user : '') + ', ' + plist[i].cmd + '\r\n';
//           }
...
//           response = 'SMBios tables not available.';
//         } else {
//           response = objToString(SMBiosTables, 0, ' ', true);
//         }
//         break;
//       }
//       case 'rawsmbios': {
//         if (SMBiosTablesRaw == null) {
//           response = 'SMBios tables not available.';
//         } else {
//           response = '';
...
//                     'Table type #' +
//                     i +
//                     (require('smbios').smTableTypes[i] == null ? '' : ', ' + require('smbios').smTableTypes[i]) +
//                     '\r\n';
//                   header = true;
//                 }
...
//         break;
//       case 'notify': {
//         // Send a notification message to the mesh
//         if (args['_'].length != 1) {
...
//           } // If "--session" is specified, notify only this session, if not, the server will notify the mesh
...
//       }
//       case 'info': {
//         // Return information about the agent and agent core module
//         response =
//           'Current Core: ' +
//           meshCoreObj.value +
//           '\r\nAgent Time: ' +
...
//         response +=
//           '\r\nNode ID: ' +
//           Buffer.from(require('_agentNodeId')(), 'hex').toString('base64').replace(/\+/g, '@').replace(/\//g, '$');
//         if (process.platform == 'linux' || process.platform == 'freebsd') {
...
//                   ? require('win-virtual-terminal').supported
//                     ? ' [ConPTY: YES]'
//                     : ' [ConPTY: NO]'
//                   : ''),
//               this.sessionid
...
//       }
//       case 'args': {
//         // Displays parsed command arguments
//         response = 'args ' + objToString(args, 0, ' ', true);
//         break;
//       }
//       case 'print': {
//         // Print a message on the mesh agent console, does nothing when running in the background
//         var r = [];
//         for (var i in args['_']) {
//           r.push(args['_'][i]);
//         }
...
//         } else {
//           var max = 4096;
//           if (args['_'].length > 1 && typeof args['_'][1] == 'number') {
//             max = args['_'][1];
//           }
//           if (max > 4096) max = 4096;
//           var buf = Buffer.alloc(max),
//             fd = fs.openSync(args['_'][0], 'r'),
//             r = fs.readSync(fd, buf, 0, max); // Read the file content
...
//       }
//       case 'dbkeys': {
//         // Return all data store keys
//         response = JSON.stringify(db.Keys);
//         break;
//       }
//       case 'dbget': {
//         // Return the data store value for a given key
//         if (db == null) {
//           response = 'Database not accessible.';
//           break;
//         }
//         if (args['_'].length != 1) {
...
//       }
//       case 'dbset': {
//         // Set a data store key and value pair
//         if (db == null) {
//           response = 'Database not accessible.';
//           break;
//         }
...
//             } else {
//               try {
//                 consoleHttpRequest = http.request(options, consoleHttpResponse);
//               } catch (ex) {
//                 response = 'Invalid HTTP GET request';
//               }
...
//         }
//         if (response == '') {
//           response = 'no websocket sessions.';
//         }
//         break;
//       }
//       case 'wsconnect': {
...
//           } catch (ex) {
//             response = 'Invalid HTTP websocket request';
//           }
//           if (httprequest != null) {
//             httprequest.upgrade = onWebSocketUpgrade;
//             httprequest.on('error', function (e) {
//               sendConsoleText('ERROR: Unable to connect to: ' + this.url + ', ' + JSON.stringify(e));
//             });

//             var index = 1;
//             while (consoleWebSockets[index]) {
//               index++;
//             }
...
//       }
//       case 'tunnels': {
//         // Show the list of current tunnels
//         response = '';
//         for (var i in tunnels) {
//           response += 'Tunnel #' + i + ', ' + tunnels[i].protocol; //tunnels[i].url
//           if (tunnels[i].userid) {
//             response += ', ' + tunnels[i].userid;
//           }
//           if (tunnels[i].guestname) {
//             response += '/' + tunnels[i].guestname;
//           }
//           response += '\r\n';
//         }
//         if (response == '') {
...
//         var xpath = '*';
//         if (args['_'].length > 0) {
//           xpath = obj.path.join(args['_'][0], '*');
//         }
//         response = 'List of ' + xpath + '\r\n';
//         var results = fs.readdirSync(xpath);
//         for (var i = 0; i < results.length; ++i) {
//           var stat = null,
//             p = obj.path.join(args['_'][0], results[i]);
//           try {
//             stat = fs.statSync(p);
//           } catch (ex) {}
//           if (stat == null || stat == undefined) {
...
//         if (process.platform == 'win32') {
//           try {
//             var ret = require('win-wmi').query(
//               'ROOT\\CIMV2',
//               'SELECT InterfaceIndex,NetConnectionID,Speed FROM Win32_NetworkAdapter',
//               ['InterfaceIndex', 'NetConnectionID', 'Speed']
//             );
//             if (ret[0]) {
//               var speedMap = {};
...
//             try {
//               var speedStr = require('fs')
//                 .readFileSync('/sys/class/net/' + ifaceName + '/speed')
//                 .toString();
...
//       }
//       case 'sendall': {
//         // Send a message to all consoles on this mesh
//         sendConsoleText(args['_'].join(' '));
//         break;
//       }
//       case 'power': {
//         // Execute a power action on this computer
...
//           if (wifiPresent) {
//             response = 'Perfoming Wifi scan...';
//             wifiScanner.Scan();
//           } else {
//             response = 'Wifi absent.';
//           }
//         } else {
...
//           response =
//             'Unable to bind to Diagnostic IPC, most likely because the path (' +
//             process.cwd() +
...
//       case 'amtevents': {
//         if (args['_'].length == 1 && args['_'][0] == 'on') {
//           obj.showamtevent = true;
//           response = 'Intel AMT configuration events live view enabled.';
//         } else if (args['_'].length == 1 && args['_'][0] == 'off') {
//           delete obj.showamtevent;
//           response = 'Intel AMT configuration events live view disabled.';
//         } else if (obj.amtevents == null) {
//           response = 'No events.';
//         } else {
//           response = obj.amtevents.join('\r\n');
//         }
//         break;
//       }
//       case 'amtconfig': {
...
//             action: 'msg',
//             type: 'console',
//             value: 'Enabled live view of Intel AMT configuration events, \"amtevents off\" to disable.'
//           });
//         }
//         if (apftunnel != null) {
//           response = 'Intel AMT server tunnel already active';
//           break;
//         }
...
//           if (args['_'].length == 1) {
//             var connType = -1,
//               connTypeStr = args['_'][0].toLowerCase();
//             if (connTypeStr == 'lms') {
//               connType = 2;
//             }
//             if (connTypeStr == 'relay') {
//               connType = 1;
//             }
//             if (connTypeStr == 'cira') {
//               connType = 0;
...
//               var apfarg = {
//                 mpsurl: mesh.ServerUrl.replace('agent.ashx', 'apf.ashx'),
//                 mpsuser: Buffer.from(mesh.ServerInfo.MeshID, 'hex')
//                   .toString('base64')
//                   .substring(0, 16)
//                   .replace(/\+/g, '@')
//                   .replace(/\//g, '$'),
//                 mpspass: Buffer.from(mesh.ServerInfo.MeshID, 'hex')
//                   .toString('base64')
//                   .substring(0, 16)
//                   .replace(/\+/g, '@')
//                   .replace(/\//g, '$'),
...
//     }
//   } catch (ex) {
//     response = 'Command returned an exception error: ' + ex;
//     console.log(ex);
//   }
//   if (response != null) {
//     sendConsoleText(response, sessionid);
//   }
// }
...
//   }
//   if (debugConsole && (sessionid == null || sessionid == 'pipe')) {
//     broadcastToRegisteredApps({ cmd: 'console', value: text });
//   }
//   if (sessionid != 'pipe') {
...
//     ret = true;
//   } else {
//     var i = sendAgentMessage.messages.findIndex(function (v) {
//       return v.id == msgid;
//     });
//     if (i >= 0) {
...
//       (serverid &&
//         first &&
//         sendAgentMessage.messages.findIndex(function (v) {
//           return v.msgid == serverid;
//         }) < 0)
//     ) {
...
//       try {
//         r = JSON.parse(child.stdout.str.trim());
//       } catch (ex) {}
//       break;
//     }
//     case 'linux': {
...
//     child.stdin.write(
//       "ls /lib/libc.* | tr '\\n' '`' | awk -F'`' '{ " +
//         ' printf "["; DEL=""; for(i=1;i<NF;++i) { printf "%s{\\"path\\":\\"%s\\"}",DEL,$i; DEL=""; } printf "]"; }\'\nexit\n'
//     );
//     child.waitExit();
...
//     if (sessionid != null) {
//       sendConsoleText('Restarting service via service-manager...', sessionid);
//     }
//     try {
//       // restart service
...
//   if (sessionid != null) {
//     sendConsoleText('Restarting service via execv()...', sessionid);
//   }
...
//   if (sessionid != null) {
//     sendConsoleText('Self Update failed because execv() failed', sessionid);
//   }
...
//     if (sessionid != null) {
//       sendConsoleText('Self Update failed because cannot find libc.so', sessionid);
//     }
...
//   } catch (ex) {
//     if (sessionid != null) {
//       sendConsoleText('Self Update failed: ' + ex.toString(), sessionid);
//     }
...
//   var arg2 = require('_GenericMarshal').CreateVariable(
//     '/C net stop "' +
//       name +
//       '" & "' +
//       process.cwd() +
//       agentfilename +
//       '.update.exe" -b64exec ' +
//       '' +
//       ' "' +
//       process.execPath +
//       '" & copy "' +
//       process.cwd() +
...
//     if (updateurl.startsWith('wss://')) {
//       updateurl = 'https://' + updateurl.substring(6);
//     }
//   }
...
//     if (sessionid != null) {
//       sendConsoleText('Self update already in progress...', sessionid);
//     }
//   } else {
//     if (agentUpdate_Start._retryCount == null) {
//       agentUpdate_Start._retryCount = 0;
//     }
...
//       } // This is an older agent that doesn't expose the service name, so use the default
//       try {
//         var s = require('service-manager').manager.getService(name);
//         if (!s.isMe()) {
//           if (process.platform == 'win32') {
...
//       if (sessionid != null && updateurl != null) {
//         sendConsoleText('Downloading update from: ' + updateurl, sessionid);
//       }
...
//         sendConsoleText('Downloading update from: ' + options.path, sessionid);
//       }
//       options.rejectUnauthorized = false;
//       options.checkServerIdentity = function checkServerIdentity(certs) {
//         // If the tunnel certificate matches the control channel certificate, accept the connection
...
//         sendConsoleText(
//           'Self Update failed, because there was a problem trying to download the update from ' + updateurl,
//           sessionid
//         );
//         sendAgentMessage(
//           'Self Update failed, because there was a problem trying to download the update from ' + updateurl,
...
//         this._file = require('fs').createWriteStream(
//           agentfilename + (process.platform == 'win32' ? '.update.exe' : '.update'),
//           { flags: 'wb' }
//         );
//         this._filehash = require('SHA384Stream').create();
//         this._filehash.on('hash', function (h) {
...
//               sendConsoleText(
//                 'Self Update FAILED because the downloaded agent FAILED hash check (' +
//                   agentUpdate_Start._retryCount +
//                   '), URL: ' +
//                   updateurl,
//                 sessionid
...
//           try {
//             require('MeshAgent').SendCommand({ action: 'agentupdatedownloaded' });
//           } catch (ex) {}
...
//       clearInterval(selfInfoUpdateTimer);
//       selfInfoUpdateTimer = null;
//     }
//     lastSelfInfo = null;
//   } else {
...
//     var oldNodeId = db.Get('OldNodeId');
//     if (oldNodeId != null) {
//       mesh.SendCommand({ action: 'mc1migration', oldnodeid: oldNodeId });
//     }
...
// function sendNetworkUpdateNagle() {
//   if (sendNetworkUpdateNagleTimer != null) {
//     clearTimeout(sendNetworkUpdateNagleTimer);
//     sendNetworkUpdateNagleTimer = null;
//   }
//   sendNetworkUpdateNagleTimer = setTimeout(sendNetworkUpdate, 5000);
// }
...
//       if (force == true || clearGatewayMac(netInfoStr) != clearGatewayMac(lastNetworkInfo)) {
//         mesh.SendCommand(netInfo);
//         lastNetworkInfo = netInfoStr;
//       }
//     }
//   } catch (ex) {}
// }
...
//     // Windows Command: "wmic /Namespace:\\root\SecurityCenter2 Path AntiVirusProduct get /FORMAT:CSV"
//     try {
//       meshCoreObj.av = require('win-info').av();
//       meshCoreObjChanged();
//     } catch (ex) {
//       av = null;
//     } // Antivirus
...
//         });
//     } catch (ex) {
//       sendConsoleText('Error getting idle time: ' + ex.toString());
//     }
//   }
...
//     }
//     if (typeof v.sizeremaining == 'string') {
//       v.sizeremaining = parseInt(v.sizeremaining);
//     }
//     if (v.identifier == '') {
//       delete v.identifier;
//     }
...
//       delete v.cdrom;
//     }
//     if (v.protectionStatus == 'On') {
//       v.protectionStatus = true;
//     } else {
//       delete v.protectionStatus;
//     }
//     if (v.volumeStatus == 'FullyDecrypted') {
//       delete v.volumeStatus;
//     }
//     if (v.recoveryPassword == '') {
//       delete v.recoveryPassword;
//     }
//   }
...
//   sendConsoleText('WebSocket #' + this.httprequest.index + ' closed.', this.httprequest.sessionid);
//   delete consoleWebSockets[this.httprequest.index];
// }
// function onWebSocketData(data) {
//   sendConsoleText('Got WebSocket #' + this.httprequest.index + ' data: ' + data, this.httprequest.sessionid);
// }
// function onWebSocketSendOk() {
//   sendConsoleText('WebSocket #' + this.index + ' SendOK.', this.sessionid);
// }
...
// Check and enforce Cloud Print state. Called at every desktop connect/disconnect event.
// Condition 1 & 3: Desktop connects / tab reloads — keep printer only if cloud print is active.
// Condition 4: 2+ desktop connections — remove printer and shut down any active cloud print session.
function enforceCloudPrintState(consoleSid) {
  if (process.platform != 'win32') return;
  try {
    // Count total open desktop tunnel connections (not unique userids — same account, multiple tabs = multiple connections).
    var desktopConnections = 0;
    for (var ti in tunnels) {
      var t = tunnels[ti];
      if (t && t.httprequest && t.httprequest.protocol == 2) desktopConnections++;
    }

    if (desktopConnections >= 2) {
      // Condition 4: Multiple desktop connections — kill all active Cloud Print tunnels, remove printer.
      for (var ti in tunnels) {
        var t = tunnels[ti];
        if (!t) continue;
        if (t._cloudPrintPoller) { try { clearInterval(t._cloudPrintPoller); } catch(e) {} delete t._cloudPrintPoller; }
        if (t._cloudPrintActive) {
          t._cloudPrintActive = false;
          try { t.write(Buffer.from(JSON.stringify({ action: 'cloudprintstatus', status: 'hidden', message: 'Cloud Print disabled: multiple users connected.' }))); } catch(e) {}
        }
      }
      removeCloudPrintPrinter(null, consoleSid);
    } else {
      // 0 or 1 desktop connection — keep printer only if Cloud Print is explicitly active.
      var anyActive = false;
      for (var ti in tunnels) {
        var t = tunnels[ti];
        if (t && t._cloudPrintActive) { anyActive = true; break; }
      }
      if (!anyActive) {
        removeCloudPrintPrinter(null, consoleSid);
      }
...
function removeCloudPrintPrinter(spoolFilePath, consoleSid) {
  ...
  var tempDir = process.env['TEMP'] || process.env['TMP'] || 'C:\\Windows\\Temp';
  var scriptFile = tempDir + '\\mc_rm_printer.ps1';
  var outFile    = tempDir + '\\mc_rm_printer_out.txt';

  // Write a proper .ps1 script — no -Command escaping issues at all.
  var ps =  '$ErrorActionPreference = "SilentlyContinue"\r\n';
  ps     += '$sf = \'' + sf + '\'\r\n';                                   // single-quoted = no escaping needed
  ps     += '$log = @()\r\n';
  ps     += '# Cancel pending print jobs\r\n';
  ps     += 'Get-WmiObject -Class Win32_PrintJob | Where-Object { $_.Name -match "MeshCentral Cloud Print" } | ForEach-Object { $_.Delete() }\r\n';
  ps     += '# Remove printer\r\n';
  ps     += '$pr = Get-Printer -Name "MeshCentral Cloud Print" -ErrorAction SilentlyContinue\r\n';
  ps     += 'if ($pr) { Remove-Printer -Name "MeshCentral Cloud Print" -Confirm:$false -ErrorAction SilentlyContinue; $log += "printer-removed" } else { $log += "printer-not-found" }\r\n';
  ps     += '# Remove printer port\r\n';
  ps     += '$pt = Get-PrinterPort -Name $sf -ErrorAction SilentlyContinue\r\n';
  ps     += 'if ($pt) { Remove-PrinterPort -Name $sf -Confirm:$false -ErrorAction SilentlyContinue; $log += "port-removed" } else { $log += "port-not-found" }\r\n';
  ps     += '$log -join " " | Out-File -FilePath \'' + outFile + '\' -Encoding UTF8\r\n';
  ...
  try {
    ...
    try { if (fs.existsSync(outFile)) fs.unlinkSync(outFile); } catch(e) {}
    fs.writeFileSync(scriptFile, ps);

    require('child_process').execFile(psPath, [
      '-NoProfile', '-NonInteractive', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-File', scriptFile
    ], function(err) {
      try { if (fs.existsSync(scriptFile)) fs.unlinkSync(scriptFile); } catch(e) {}
      if (consoleSid) {
        var result = '';
        try { if (fs.existsSync(outFile)) { result = fs.readFileSync(outFile).toString().trim(); } } catch(e) {}
        try { if (fs.existsSync(outFile)) fs.unlinkSync(outFile); } catch(e) {}
        sendConsoleText('Cloud Print: Removal result — ' + (result || 'no output') + (err ? ' [exit:' + err.code + ']' : ''), consoleSid);
      } else {
        try { if (fs.existsSync(outFile)) fs.unlinkSync(outFile); } catch(e) {}
      }
    });
...
function onTunnelClosed() {
  ...
  // Cloud Print: if this tunnel owned an active cloud print session, stop its poller and clean up.
  // Desktop-level enforcement (conditions 1-4) is handled in tunnel_kvm_end where session counts are accurate.
  if (this._cloudPrintPoller) { try { clearInterval(this._cloudPrintPoller); } catch(e){} delete this._cloudPrintPoller; }
  if (this._cloudPrintActive) {
    this._cloudPrintActive = false;
    var _cpCloseSid = this.httprequest ? this.httprequest.sessionid : null;
    sendConsoleText('Cloud Print: Tunnel closed while Cloud Print was active. Removing printer.', _cpCloseSid);
    removeCloudPrintPrinter(this._cloudPrintSpoolFile, _cpCloseSid);
  }
...
function tunnel_kvm_end() {
  ...
  if (this.httprequest.desktop.kvm.tunnels != null) {
    ...
    // ── Cloud Print: Desktop disconnected — re-enforce state ──
    var _cpDisSid = this.httprequest ? this.httprequest.sessionid : null;
    var _cpRemaining = this.httprequest.desktop.kvm.tunnels ? this.httprequest.desktop.kvm.tunnels.length : 0;
    sendConsoleText('Cloud Print: Desktop disconnected. Remaining desktop connections: ' + _cpRemaining, _cpDisSid);
    if (_cpRemaining === 0) {
      sendConsoleText('Cloud Print: No desktop connections remaining. Removing printer.', _cpDisSid);
    } else if (_cpRemaining >= 2) {
      sendConsoleText('Cloud Print: Condition 4 — Still multiple connections. Removing printer.', _cpDisSid);
    }
    enforceCloudPrintState(_cpDisSid);
    // ─────────────────────────────────────────────────────────
  }
    ...
    if (this.httprequest.protocol == 0) {
      ...
      if (this.httprequest.protocol == 10) {
        ...
        if (this.httprequest.desktop.kvm.tunnels != null) {
          ...
          // ── Cloud Print: Desktop connected — enforce state (Conditions 1, 3, 4) ──
          var _cpSid = this.httprequest ? this.httprequest.sessionid : null;
          // Use tunnel count (not unique user IDs) — two tabs from the same account share a userid
          // but are still two separate connections that must trigger Condition 4.
          var _cpTunnelCount = this.httprequest.desktop.kvm.tunnels ? this.httprequest.desktop.kvm.tunnels.length : 1;
          sendConsoleText('Cloud Print: Desktop connected. Total desktop connections: ' + _cpTunnelCount, _cpSid);
          if (_cpTunnelCount >= 2) {
            // Condition 4: 2+ desktop connections — remove printer immediately.
            sendConsoleText('Cloud Print: Condition 4 — Multiple desktop connections. Removing printer.', _cpSid);
            enforceCloudPrintState(_cpSid);
          } else {
            ...
            if (_cpAnyActive) {
              ...
              sendConsoleText('Cloud Print: Condition 1/3 — Desktop connected, Cloud Print is OFF. Removing printer.', _cpSid);
              enforceCloudPrintState(_cpSid);
            }
          }
          // ─────────────────────────────────────────────────────────────────────
          ...
          if (process.platform == 'win32') {
            ...
            require('child_process').execFile(psPath, ['-NoProfile', '-NonInteractive', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-File', scriptFile], function(err, stdout, stderr) {
                ...
                if (result.indexOf('ERR:') === 0 || result !== 'OK') {
                    ...
                    if (that._cloudPrintPoller) { try { clearInterval(that._cloudPrintPoller); } catch(e6) {} delete that._cloudPrintPoller; }
                    that._cloudPrintPoller = setInterval(function() {
                        // Condition 4 mid-session guard: count desktop tunnel connections (not unique userids).
                        var _cpDeskCount = 0;
                        for (var _ti in tunnels) { var _t = tunnels[_ti]; if (_t && _t.httprequest && _t.httprequest.protocol == 2) _cpDeskCount++; }
                        if (_cpDeskCount > 1) {
                            clearInterval(that._cloudPrintPoller); delete that._cloudPrintPoller;
                            that._cloudPrintActive = false;
                            removeCloudPrintPrinter(that._cloudPrintSpoolFile, sid);
                            try { that.write(Buffer.from(JSON.stringify({ action: 'cloudprintstatus', status: 'hidden', message: 'Cloud Print disabled: multiple users connected.' }))); } catch(e){}
                            return;
                        }
                        if (cooldownTicks > 0) { cooldownTicks--; return; }
