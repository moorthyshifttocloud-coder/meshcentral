...
module.exports.CreateSwarmServer = function (parent, db, args, certificates) {
  ...
  obj.stats = {
    blockedConnect: 0,
    connectCount: 0,
    clientCertConnectCount: 0,
    noCertConnectCount: 0,
    bytesIn: 0,
    bytesOut: 0,
    httpGetRequest: 0,
    pushedAgents: {},
  ...
  obj.server = tls.createServer(
    {
      key: certificates.swarmserver.key,
      cert: certificates.swarmserver.cert,
      requestCert: true,
  ...
  obj.server
    .listen(args.swarmport, function () {
      console.log('MeshCentral Legacy Swarm Server running on ' + certificates.CommonName + ':' + args.swarmport + '.');
      obj.parent.updateServerState('swarm-port', args.swarmport);
    })
    .on('error', function (err) {
      console.error('ERROR: MeshCentral Swarm Server server port ' + args.swarmport + ' is not available.');
      if (args.exactports) {
        process.exit();
      }
    });
  ...
  function loadMigrationAgents() {
    var migrationAgentsDir = null,
      migrationAgentsPath = obj.parent.path.join(obj.parent.datapath, 'migrationagents');
    try {
      migrationAgentsDir = obj.parent.fs.readdirSync(migrationAgentsPath);
    } catch (e) {}
    if (migrationAgentsDir != null) {
      for (var i in migrationAgentsDir) {
        if (migrationAgentsDir[i].toLowerCase().startsWith('meshagent-')) {
          ...
          if (obj.migrationAgents[agentArch] == null) {
            obj.migrationAgents[agentArch] = {};
          }
          if (obj.migrationAgents[agentArch][agentVersion] == null) {
            obj.migrationAgents[agentArch][agentVersion] = {
              arch: agentArch,
              ver: agentVersion,
              path: migrationAgentName
            };
          }
        }
      }
  ...
  function onData(data) {
    if (this.relaySocket) {
      var ps = this;
      try {
        this.relaySocket.write(data, 'binary', function () {
          ps.resume();
        });
      } catch (ex) {}
      return;
    ...
    if (this.tag.first == true) {
      ...
      if (this.tag.accumulator.substring(0, 3) == 'GET' || this.tag.accumulator.substring(0, 3) == 'POS') {
        ...
        this.pause();
        var relaySocket = tls.connect(obj.args.port, { rejectUnauthorized: false }, function () {
          this.write(this.parentSocket.tag.accumulator);
          this.parentSocket.resume();
        });
        relaySocket.on('data', function (data) {
          try {
            var rs = this;
            this.pause();
            this.parentSocket.write(data, 'binary', function () {
              rs.resume();
            });
          } catch (ex) {}
        });
        relaySocket.on('error', function (err) {
          try {
            this.parentSocket.end();
          } catch (ex) {}
        });
        relaySocket.on('end', function () {
          try {
            this.parentSocket.end();
          } catch (ex) {}
        });
    ...
    switch (cmd) {
      case LegacyMeshProtocol.NODEPUSH: {
        ...
        if (nodeblock != null && nodeblock.agenttype != null && nodeblock.agentversion != null) {
          if (socket.pingTimer == null) {
            socket.pingTimer = setInterval(function () {
              obj.SendCommand(socket, LegacyMeshProtocol.PING);
            }, 20000);
          }
          parent.debug('swarmcmd', 'NODEPUSH:' + JSON.stringify(nodeblock));

          // Log the agent type
          if (obj.stats.agenttype[nodeblock.agenttype] == null) {
            obj.stats.agenttype[nodeblock.agenttype] = 1;
          } else {
            obj.stats.agenttype[nodeblock.agenttype]++;
          }
          ...
          if (actionCount > 2) {
            ...
            if (nodeblock.agentversion < 201) {
              nextAgentVersion = 201;
            } // If less then 201, move to transitional MC1 agent.
            if (nodeblock.agentversion == 201) {
              nextAgentVersion = 202;
            } // If at 201, move to first MC2 agent.

            // See if we need to start the agent update
            if (
              nextAgentVersion > 0 &&
              obj.migrationAgents[nodeblock.agenttype] != null &&
              obj.migrationAgents[nodeblock.agenttype][nextAgentVersion] != null
            ) {
              // Start the update
              ...
              // Update stats
              if (obj.stats.pushedAgents[nodeblock.agenttype] == null) {
                obj.stats.pushedAgents[nodeblock.agenttype] = {};
              }
              if (obj.stats.pushedAgents[nodeblock.agenttype][nextAgentVersion] == null) {
                obj.stats.pushedAgents[nodeblock.agenttype][nextAgentVersion] = 1;
              } else {
                obj.stats.pushedAgents[nodeblock.agenttype][nextAgentVersion]++;
              }
              ...
              obj.parent.taskLimiter.launch(
                function (socket, taskid, taskLimiterQueue) {
                  if (socket.xclosed == 1) {
        ...
        switch (statecmd) {
          case 6: {
            ...
            if (socket.tag.update != null) {
              // Send an agent block
              if (socket.tag.update.binary == null) {
                socket.tag.update.binary = obj.parent.fs.readFileSync(socket.tag.update.path);
              }
              var l = Math.min(socket.tag.update.binary.length - socket.tag.updatePtr, 16384);
              obj.SendCommand(
                socket,
                LegacyMeshProtocol.GETSTATE,
                common.IntToStr(6) +
                  common.IntToStr(socket.tag.updatePtr) +
                  socket.tag.update.binary.toString('binary', socket.tag.updatePtr, socket.tag.updatePtr + l)
              ); // agent.SendQuery(6, AgentFileLen + AgentBlock);
              ...
              if (socket.tag.updatePtr >= socket.tag.update.binary.length) {
                ...
                obj.SendCommand(
                  socket,
                  LegacyMeshProtocol.GETSTATE,
                  common.IntToStr(7) + common.IntToStr(socket.tag.update.binary.length)
                ); //agent.SendQuery(7, AgentFileLen);
  ...
  function onConnection(socket) {
    ...
    if (checkSwarmIpAddress(socket, obj.args.swarmallowedip) == false) {
      obj.stats.blockedConnect++;
      parent.debug('swarm', 'New blocked agent connection');
      return;
    }
    ...
    if (socket.tag.clientCert == null || socket.tag.clientCert.subject == null) {
      obj.stats.noCertConnectCount++;
    } else {
      obj.stats.clientCertConnectCount++;
    }
    ...
    socket.addListener('close', function () {
      ...
      if (this.relaySocket) {
        try {
          ...
          this.relaySocket.removeAllListeners(['data', 'end', 'error']);
          delete this.relaySocket;
        } catch (ex) {}
      }
      ...
      if (this.tag) {
        if (this.tag.accumulator) {
          delete this.tag.accumulator;
        }
        if (this.tag.clientCert) {
          delete this.tag.clientCert;
        }
        delete this.tag;
      }
  ...
  obj.decodeNodeBlock = function (data) {
    try {
      ...
      var info = {},
        ptr = 68,
        der = forge.asn1.fromDer(forge.util.createBuffer(data, 'binary'));
      ...
      while (ptr < binarydata.length) {
        var btyp = common.ReadShort(binarydata, ptr),
          blen = common.ReadShort(binarydata, ptr + 2),
          bdata = binarydata.substring(ptr + 4, ptr + 4 + blen);
        switch (btyp) {
          case 1: {
            // PBST_COMPUTERINFO
            ...
            if (xx >= 0) {
              info.agentname = info.agentname.substring(0, xx);
            }
            info.agentosdesc = bdata.substring(64 + 8, 64 + 64 + 8);
            xx = info.agentosdesc.indexOf('\u0000');
            if (xx >= 0) {
              info.agentosdesc = info.agentosdesc.substring(0, xx);
            }
            return info;
          }
          default: {
  ...
  function checkSwarmIpAddress(socket, allowedIpList) {
    ...
    try {
      ...
      if (ip) {
        for (var i = 0; i < allowedIpList.length; i++) {
          if (require('ipcheck').match(ip, allowedIpList[i])) {
            return true;
          }
        }
      }
