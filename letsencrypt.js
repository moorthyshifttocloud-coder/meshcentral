...
module.exports.CreateLetsEncrypt = function (parent) {
  ...
  obj.log = function (str) {
    ...
    while (obj.events.length > 200) {
      obj.events.shift();
    } // Keep only 200 last events.
  };
  obj.events = [];
  ...
  try {
    obj.parent.fs.mkdirSync(obj.certPath);
  } catch (e) {}

  // Hook up GreenLock to the redirection server
  if (obj.parent.config.settings.rediraliasport === 80) {
    obj.redirWebServerHooked = true;
  } else if (obj.parent.config.settings.rediraliasport == null && obj.parent.redirserver.port == 80) {
    obj.redirWebServerHooked = true;
  }
  ...
  obj.challenge = function (token, hostname, func) {
    if (obj.challenges[token] != null) {
      obj.log('Succesful response to challenge.');
    } else {
      obj.log('Failed to respond to challenge, token: ' + token + ', table: ' + JSON.stringify(obj.challenges) + '.');
    }
    func(obj.challenges[token]);
  };
  ...
  obj.getCertificate = function (certs, func) {
    ...
    obj.zerossl =
      typeof obj.parent.config.letsencrypt.zerossl == 'object' ? obj.parent.config.letsencrypt.zerossl : false;
    obj.log('Getting certs from local store (' + (obj.runAsProduction ? 'Production' : 'Staging') + ')');
    if (certs.CommonName.indexOf('.') == -1) {
      obj.configErr = 'Add "cert" value to settings in config.json before using Let\'s Encrypt.';
    ...
    if (obj.parent.config.letsencrypt == null) {
      obj.configErr = "No Let's Encrypt configuration";
      parent.addServerWarning(obj.configErr);
      obj.log('WARNING: ' + obj.configErr);
      func(certs);
      return;
    }
    if (obj.parent.config.letsencrypt.email == null) {
      obj.configErr = "Let's Encrypt email address not specified.";
      parent.addServerWarning(obj.configErr);
      obj.log('WARNING: ' + obj.configErr);
      func(certs);
      return;
    ...
    if (
      obj.parent.redirserver == null ||
      (typeof obj.parent.config.settings.rediraliasport === 'number' &&
        obj.parent.config.settings.rediraliasport !== 80) ||
      (obj.parent.config.settings.rediraliasport == null && obj.parent.redirserver.port !== 80)
    ) {
      obj.configErr = "Redirection web server must be active on port 80 for Let's Encrypt to work.";
      parent.addServerWarning(obj.configErr);
      obj.log('WARNING: ' + obj.configErr);
      func(certs);
      return;
    }
    if (obj.redirWebServerHooked !== true) {
      obj.configErr = "Redirection web server not setup for Let's Encrypt to work.";
      parent.addServerWarning(obj.configErr);
      obj.log('WARNING: ' + obj.configErr);
      func(certs);
      return;
    ...
    if (
      obj.parent.config.letsencrypt.rsakeysize != null &&
      obj.parent.config.letsencrypt.rsakeysize !== 2048 &&
      obj.parent.config.letsencrypt.rsakeysize !== 3072 &&
      obj.parent.config.letsencrypt.rsakeysize !== 4096
    ) {
      obj.configErr = "Invalid Let's Encrypt certificate key size, must be 2048, 3072 or 4096.";
      parent.addServerWarning(obj.configErr);
      obj.log('WARNING: ' + obj.configErr);
      func(certs);
      return;
    }
    if (obj.checkInterval == null) {
      obj.checkInterval = setInterval(obj.checkRenewCertificate, 86400000);
    } // Call certificate check every 24 hours.
    obj.configOk = true;

    // Get the list of domains
    obj.leDomains = [certs.CommonName];
    if (obj.parent.config.letsencrypt.names != null) {
      if (typeof obj.parent.config.letsencrypt.names == 'string') {
        obj.parent.config.letsencrypt.names = obj.parent.config.letsencrypt.names.split(',');
      }
      obj.parent.config.letsencrypt.names.map(function (s) {
        return s.trim();
      }); // Trim each name
      if (
        typeof obj.parent.config.letsencrypt.names != 'object' ||
        obj.parent.config.letsencrypt.names.length == null
      ) {
        console.log("ERROR: Let's Encrypt names must be an array in config.json.");
        func(certs);
        return;
      }
    ...
    if (obj.fs.existsSync(certFile) && obj.fs.existsSync(keyFile)) {
      ...
      if (altNames) {
        for (var i = 0; i < altNames.altNames.length; i++) {
          var acn = altNames.altNames[i].value.toLowerCase();
          if (obj.certNames.indexOf(acn) == -1) {
            obj.certNames.push(acn);
          }
        }
      }
      ...
      for (var i in obj.parent.config.domains) {
        if (
          ...
          obj.parent.certificateOperations.compareCertificateNames(obj.certNames, obj.parent.config.domains[i].dns)
        ) {
  ...
  obj.checkRenewCertificate = function () {
    if (obj.pendingRequest == true) {
      obj.log('Request for certificate is in process.');
      return 4;
    }
  ...
  obj.requestCertificate = function () {
    ...
    if (obj.configOk == false) {
      obj.log("Can't request cert, invalid configuration.");
      return;
    }
    if (acme.forge == null) {
      obj.log('Forge not setup in ACME, unable to continue.');
      return;
    }
    ...
    acme.forge
      .createPrivateKey(
        obj.parent.config.letsencrypt.rsakeysize != null ? obj.parent.config.letsencrypt.rsakeysize : 2048
      )
      .then(
        function (accountKey) {
          ...
          if (obj.zerossl) {
            if (obj.zerossl.kid == '') {
              obj.log("EAB KID hasn't been set, invalid configuration.");
              return;
            }
            if (obj.zerossl.hmackey == '') {
              obj.log("EAB HMAC KEY hasn't been set, invalid configuration.");
              return;
            }
            ...
            if (obj.custom.hmackey == '') {
              obj.log("EAB HMAC KEY hasn't been set, invalid configuration.");
              return;
            }
            if (obj.custom.server == '') {
              obj.log("Custom ACME server URL hasn't been set, invalid configuration.");
              return;
            }
            obj.client = new acme.Client({
            ...
            obj.client = new acme.Client({
              directoryUrl: obj.runAsProduction
          ...
          var certRequest = {
            ...
            keySize: obj.parent.config.letsencrypt.rsakeysize != null ? obj.parent.config.letsencrypt.rsakeysize : 2048
          };
          if (obj.leDomains.length > 1) {
            certRequest.altNames = obj.leDomains;
          }
          acme.forge.createCsr(certRequest).then(
            function (r) {
              obj.csr = r[1];
  ...
  obj.getStats = function () {
    ...
    if (obj.configErr) {
      r.error = 'WARNING: ' + obj.configErr;
    }
    if (obj.certExpire) {
      r.cert = 'Present';
      r.daysLeft = Math.floor((obj.certExpire - new Date()) / 86400000);
    } else {
      r.cert = 'None';
    }
    return r;
