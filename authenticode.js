...
function createSelfSignedCert(args) {
  ...
  var attrs = [];
  if (typeof args.cn == 'string') {
    attrs.push({ name: 'commonName', value: args.cn });
  }
  if (typeof args.country == 'string') {
    attrs.push({ name: 'countryName', value: args.country });
  }
  if (typeof args.state == 'string') {
    attrs.push({ name: 'ST', value: args.state });
  }
  if (typeof args.locality == 'string') {
    attrs.push({ name: 'localityName', value: args.locality });
  }
  if (typeof args.org == 'string') {
    attrs.push({ name: 'organizationName', value: args.org });
  }
  if (typeof args.orgunit == 'string') {
    attrs.push({ name: 'OU', value: args.orgunit });
  }
...
function loadBitmap(bitmapFile) {
  var bitmapData = null;
  try {
    bitmapData = fs.readFileSync(bitmapFile);
  } catch (ex) {}
...
function loadIcon(iconFile) {
  var iconData = null;
  try {
    iconData = fs.readFileSync(iconFile);
  } catch (ex) {}
...
function loadCertificates(pemFileNames) {
  ...
  if (typeof pemFileNames == 'string') {
    pemFileNames = [pemFileNames];
  }
    ...
    try {
        ...
        if (k >= 0) {
          certs.push(
            pki.certificateFromPem(
              '-----BEGIN CERTIFICATE-----' + pemCerts[j].substring(0, k) + '-----END CERTIFICATE-----'
            )
          );
        }
      }
        ...
        if (k >= 0) {
          keys.push(
            pki.privateKeyFromPem(
              '-----BEGIN RSA PRIVATE KEY-----' + PemKeys[j].substring(0, k) + '-----END RSA PRIVATE KEY-----'
            )
          );
        }
      }
      ...
      for (var j in PemKeys) {
        ...
        if (k >= 0) {
          keys.push(
            pki.privateKeyFromPem(
              '-----BEGIN PRIVATE KEY-----' + PemKeys[j].substring(0, k) + '-----END PRIVATE KEY-----'
            )
          );
        }
      }
...
function createAuthenticodeHandler(path) {
  ...
  function openFile() {
    ...
    if (obj.filesize < 160 + obj.header.peHeaderLocation) {
      obj.close();
      return false;
    } // Invalid SizeOfHeaders.
    if (readFileSlice(obj.header.peHeaderLocation, 4).toString('hex') != '50450000') {
      obj.close();
      return false;
    } // Invalid PE header, must start with "PE" (HEX: 50 45 00 00).
    ...
    switch (
      obj.header.peStandard.magic // Check magic value
    ) {
      case 0x020b:
        obj.header.pe32plus = 1;
        break;
      case 0x010b:
        obj.header.pe32plus = 0;
        break;
      default: {
        obj.close();
        return false;
    ...
    if (obj.header.pe32plus == 0) {
      obj.header.peStandard.baseOfData = optinalHeader.readUInt32LE(24);
    }
    ...
    obj.header.dataDirectories.exportTable = {
      addr: optinalHeader.readUInt32LE(96 + pePlusOffset),
      size: optinalHeader.readUInt32LE(100 + pePlusOffset)
    };
    obj.header.dataDirectories.importTable = {
      addr: optinalHeader.readUInt32LE(104 + pePlusOffset),
      size: optinalHeader.readUInt32LE(108 + pePlusOffset)
    };
    obj.header.dataDirectories.resourceTable = {
      addr: optinalHeader.readUInt32LE(112 + pePlusOffset),
      size: optinalHeader.readUInt32LE(116 + pePlusOffset)
    }; // Same as .rsrc virtual address & size
    obj.header.dataDirectories.exceptionTableAddr = {
      addr: optinalHeader.readUInt32LE(120 + pePlusOffset),
      size: optinalHeader.readUInt32LE(124 + pePlusOffset)
    }; // Same as .pdata virtual address & size
    obj.header.dataDirectories.certificateTable = {
      addr: optinalHeader.readUInt32LE(128 + pePlusOffset),
      size: optinalHeader.readUInt32LE(132 + pePlusOffset)
    };
    obj.header.dataDirectories.baseRelocationTable = {
      addr: optinalHeader.readUInt32LE(136 + pePlusOffset),
      size: optinalHeader.readUInt32LE(140 + pePlusOffset)
    }; // Same as .reloc virtual address & size
    obj.header.dataDirectories.debug = {
      addr: optinalHeader.readUInt32LE(144 + pePlusOffset),
      size: optinalHeader.readUInt32LE(148 + pePlusOffset)
    };
    // obj.header.dataDirectories.architecture = optinalHeader.readBigUInt64LE(152 + pePlusOffset); // Must be zero
    obj.header.dataDirectories.globalPtr = {
      addr: optinalHeader.readUInt32LE(160 + pePlusOffset),
      size: optinalHeader.readUInt32LE(164 + pePlusOffset)
    };
    obj.header.dataDirectories.tLSTable = {
      addr: optinalHeader.readUInt32LE(168 + pePlusOffset),
      size: optinalHeader.readUInt32LE(172 + pePlusOffset)
    };
    obj.header.dataDirectories.loadConfigTable = {
      addr: optinalHeader.readUInt32LE(176 + pePlusOffset),
      size: optinalHeader.readUInt32LE(180 + pePlusOffset)
    };
    obj.header.dataDirectories.boundImport = {
      addr: optinalHeader.readUInt32LE(184 + pePlusOffset),
      size: optinalHeader.readUInt32LE(188 + pePlusOffset)
    };
    obj.header.dataDirectories.iAT = {
      addr: optinalHeader.readUInt32LE(192 + pePlusOffset),
      size: optinalHeader.readUInt32LE(196 + pePlusOffset)
    };
    obj.header.dataDirectories.delayImportDescriptor = {
      addr: optinalHeader.readUInt32LE(200 + pePlusOffset),
      size: optinalHeader.readUInt32LE(204 + pePlusOffset)
    };
    obj.header.dataDirectories.clrRuntimeHeader = {
      addr: optinalHeader.readUInt32LE(208 + pePlusOffset),
      size: optinalHeader.readUInt32LE(212 + pePlusOffset)
    };
    ...
    for (var i = 0; i < obj.header.coff.numberOfSections; i++) {
      ...
      if (buf[0] != 46 && buf[0] != 95) {
        obj.close();
        return false;
      } // Name of the section must start with a dot or underscore. If not, something is wrong.
      var sectionName = buf.slice(0, 8).toString().trim('\0');
      var j = sectionName.indexOf('\0');
      if (j >= 0) {
        sectionName = sectionName.substring(0, j);
    ...
    if (obj.header.signed) {
      ...
      if (obj.filesize < obj.header.sigpos + obj.header.siglen) {
        obj.close();
        return false;
      } // Executable file too short to contain the signature block.

      // Remove the padding if needed
      var i,
        pkcs7raw = readFileSlice(obj.header.sigpos + 8, obj.header.siglen - 8);
      var derlen = forge.asn1.getBerValueLength(forge.util.createBuffer(pkcs7raw.slice(1, 5))) + 4;
      if (derlen != pkcs7raw.length) {
        pkcs7raw = pkcs7raw.slice(0, derlen);
      }
      ...
      var pkcs7der = null,
        valid = false;
      try {
        pkcs7der = forge.asn1.fromDer(forge.util.createBuffer(pkcs7raw));
      } catch (ex) {}
      try {
        valid =
          pkcs7der != null &&
          forge.asn1.derToOid(pkcs7der.value[1].value[0].value[2].value[0].value) == '1.3.6.1.4.1.311.2.1.4';
      } catch (ex) {}
      if (pkcs7der == null) {
        ...
        if (pkcs7 == null || pkcs7content == null) {
          ...
          try {
            for (var i in pkcs7.rawCapture.authenticatedAttributes) {
              if (
                ...
                forge.asn1.derToOid(pkcs7.rawCapture.authenticatedAttributes[i].value[0].value) ==
                  obj.Oids.SPC_SP_OPUS_INFO_OBJID
              ) {
                for (var j in pkcs7.rawCapture.authenticatedAttributes[i].value[1].value[0].value) {
                  if (
                    ...
                    if (v.startsWith('http://') || v.startsWith('https://') || v.length % 2 == 1) {
                      obj.signingAttribs.push(v);
                    } else {
                      var r = ''; // This string value is in UCS2 format, convert it to a normal string.
                      for (var k = 0; k < v.length; k += 2) {
                        r += String.fromCharCode((v.charCodeAt(k + 8) << 8) + v.charCodeAt(k + 1));
                      }
          ...
          switch (hashAlgoOid) {
            case forge.pki.oids.sha256: {
              obj.fileHashAlgo = 'sha256';
              break;
            }
            case forge.pki.oids.sha384: {
              obj.fileHashAlgo = 'sha384';
              break;
            }
            case forge.pki.oids.sha512: {
              obj.fileHashAlgo = 'sha512';
              break;
            }
            case forge.pki.oids.sha224: {
              obj.fileHashAlgo = 'sha224';
              break;
            }
            case forge.pki.oids.md5: {
              obj.fileHashAlgo = 'md5';
              break;
            }
          }
          ...
          if (obj.fileHashAlgo != null) {
            obj.fileHashActual = obj.getHash(obj.fileHashAlgo);
          }
        }
      }
    }
  ...
  obj.timeStampRequest = function (args, func) {
    ...
    const asn1obj = asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
    ...
    httpRequest(options, requestBody, function (err, data) {
      ...
      try {
        timepkcs7der = forge.asn1.fromDer(forge.util.createBuffer(Buffer.from(data, 'base64').toString('binary')));
      } catch (ex) {
        func('Unable to parse time-stamp response: ' + ex);
        return;
      }
      ...
      var pkcs7der = null;
      try {
        var pkcs7der = forge.asn1.fromDer(
          forge.util.createBuffer(Buffer.from(obj.getRawSignatureBlock(), 'base64').toString('binary'))
        );
        ...
        for (var i in timeasn1Certs) {
          pkcs7der.value[1].value[0].value[3].value.push(timeasn1Certs[i]);
        }
        ...
        for (var i in pkcs7der.value[1].value[0].value[4].value[0].value) {
          ...
          if (j.tagClass != 128 || j.type != 1) {
            newValues.push(j);
          } // If this is not a time stamp, add it to out new list.
        }
        ...
        const asn1obj2 = asn1.create(asn1.Class.CONTEXT_SPECIFIC, 1, true, [
          asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
        ...
        if (padding > 0) {
          fs.writeSync(output, Buffer.alloc(padding, 0));
        }
  ...
  function readResourceTable(ptr, offset) {
    ...
    for (var i = 0; i < totalResources; i++) {
      ...
      if ((offsetToData & 0x80000000) != 0) {
        resource.table = readResourceTable(ptr, offsetToData - 0x80000000);
      } else {
        resource.item = readResourceItem(ptr, offsetToData);
      }
  ...
  function readLenPrefixUnicodeString(ptr) {
    ...
    var buf = readFileSlice(ptr + 2, nameLen * 2),
      name = '';
    for (var i = 0; i < nameLen; i++) {
      name += String.fromCharCode(buf.readUInt16LE(i * 2));
    }
    return name;
  }
  ...
  function generateResourceSection(resources) {
    ...
    if (resSizeTotal % fileAlign != 0) {
      resSizeTotal += fileAlign - (resSizeTotal % fileAlign);
    }
    ...
    const resPointers = {
      tables: 0,
      items: resSizes.tables,
      names: resSizes.tables + resSizes.items,
      data: resSizes.tables + resSizes.items + resSizes.names
    };
  ...
  function getResourceSectionSize(resources, sizes) {
    ...
    for (var i in resources.entries) {
      ...
      if (resources.entries[i].table) {
        getResourceSectionSize(resources.entries[i].table, sizes);
  ...
  function createResourceSection(resources, buf, resPointers) {
    var numberOfNamedEntries = 0,
      numberOfIdEntries = 0,
      ptr = resPointers.tables;
    ...
    for (var i in resources.entries) {
      ...
      if (resources.entries[i].table) {
        ...
        if (resources.entries[i].item.buffer) {
          ...
          const actualPtr =
            resources.entries[i].item.offsetToData -
            obj.header.sections['.rsrc'].virtualAddr +
            obj.header.sections['.rsrc'].rawAddr;
  ...
  function stringToUnicode(str, buf, offset) {
    ...
    for (var i = 0; i < str.length; i++) {
      buf.writeInt16LE(str.charCodeAt(i), offset + i * 2);
    }
  }
  ...
  obj.getBitmapInfo = function () {
    const r = {},
      ptr = obj.header.sections['.rsrc'].rawAddr;
  ...
  obj.setBitmapInfo = function (bitmapInfo) {
    ...
    const bitmapEntry = {
      name: resourceDefaultNames.bitmaps,
      table: { characteristics: 0, timeDateStamp: 0, majorVersion: 0, minorVersion: 0, entries: [] }
    };
    for (var i in bitmapInfo) {
      ...
      const bitmapItemEntry = {
        ...
        table: {
          ...
          majorVersion: 0,
          minorVersion: 0,
          entries: [{ name: 1033, item: { buffer: bitmapInfo[i], codePage: 0 } }]
        }
      };
    ...
    const names = [];
    for (var i = 0; i < obj.resources.entries.length; i++) {
      names.push(obj.resources.entries[i].name);
    }
    names.sort(resSort);
    var newEntryOrder = [];
    for (var i in names) {
      for (var j = 0; j < obj.resources.entries.length; j++) {
        if (obj.resources.entries[j].name == names[i]) {
          newEntryOrder.push(obj.resources.entries[j]);
        }
      }
    }
  ...
  obj.setIconInfo = function (iconInfo) {
    ...
    for (var i = 0; i < obj.resources.entries.length; i++) {
      if (
        obj.resources.entries[i].name != resourceDefaultNames.icon &&
        obj.resources.entries[i].name != resourceDefaultNames.iconGroups
      ) {
        resourcesEntries.push(obj.resources.entries[i]);
      }
    }
    ...
    for (var i in iconInfo) {
      ...
      for (var j in iconInfo[i].icons) {
        xicons[nextIconNumber++] = iconInfo[i].icons[j];
      }
    ...
    const iconsEntry = {
      name: resourceDefaultNames.icon,
      table: { characteristics: 0, timeDateStamp: 0, majorVersion: 0, minorVersion: 0, entries: [] }
    };
    for (var i in iconInfo) {
      for (var j in iconInfo[i].icons) {
        ...
        const iconItemEntry = {
          ...
          table: {
            ...
            majorVersion: 0,
            minorVersion: 0,
            entries: [{ name: 1033, item: { buffer: iconInfo[i].icons[j].icon, codePage: 0 } }]
          }
        };
    ...
    const groupEntry = {
      name: resourceDefaultNames.iconGroups,
      table: { characteristics: 0, timeDateStamp: 0, majorVersion: 0, minorVersion: 0, entries: [] }
    };
    for (var i in iconInfo) {
      ...
      const groupItemEntry = {
        ...
        table: {
          ...
          majorVersion: 0,
          minorVersion: 0,
          entries: [{ name: 1033, item: { buffer: buf, codePage: 0 } }]
        }
      };
  ...
  obj.getVersionInfo = function () {
    var r = {},
      info = readVersionInfo(getVersionInfoData(), 0);
    ...
    if (StringFileInfo == null || StringFileInfo.stringTable == null || StringFileInfo.stringTable.strings == null)
      return null;
    const strings = StringFileInfo.stringTable.strings;
    for (var i in strings) {
      r[strings[i].key] = strings[i].value;
    }
    r['~FileVersion'] =
      (info.fixedFileInfo.dwFileVersionMS >> 16) +
      '.' +
      (info.fixedFileInfo.dwFileVersionMS & 0xffff) +
      '.' +
      (info.fixedFileInfo.dwFileVersionLS >> 16) +
      '.' +
      (info.fixedFileInfo.dwFileVersionLS & 0xffff);
    r['~ProductVersion'] =
      (info.fixedFileInfo.dwProductVersionMS >> 16) +
      '.' +
      (info.fixedFileInfo.dwProductVersionMS & 0xffff) +
      '.' +
      (info.fixedFileInfo.dwProductVersionLS >> 16) +
      '.' +
      (info.fixedFileInfo.dwProductVersionLS & 0xffff);
    return r;
  };
  ...
  obj.setVersionInfo = function (versions) {
    ...
    for (var i in versions) {
      if (!i.startsWith('~')) {
        stringArray.push({ key: i, value: versions[i] });
      }
    }
    ...
    if (versions['~ProductVersion'] != null) {
      ...
      info.fixedFileInfo.dwProductVersionMS =
        (parseInt(ProductVersionSplit[0]) << 16) + parseInt(ProductVersionSplit[1]);
      info.fixedFileInfo.dwProductVersionLS =
        (parseInt(ProductVersionSplit[2]) << 16) + parseInt(ProductVersionSplit[3]);
    }
  ...
  function writeVersionInfo(bufArray, info) {
    ...
    if (info.stringFiles != null) {
      wLength += writeStringFileInfo(bufArray, info.stringFiles);
    }
  ...
  function writeStringFileInfo(bufArray, stringFiles) {
    ...
    for (var i in stringFiles) {
      ...
      if (stringFiles[i].szKey == 'VarFileInfo') {
        l += 4;
      } // TODO: This is a hack, not sure what the correct code should be
      ...
      if (stringFiles[i].szKey == 'StringFileInfo') {
        wLength += writeStringTableStruct(bufArray, stringFiles[i].stringTable);
      }
      if (stringFiles[i].szKey == 'VarFileInfo') {
        wLength += writeVarFileInfoStruct(bufArray, stringFiles[i].varFileInfo);
      }
  ...
  function writeStringTableStruct(bufArray, stringTable) {
    ...
    if (stringTable.strings) {
      wLength += writeStringStructs(bufArray, stringTable.strings);
    }
  ...
  function readStringFilesStruct(buf, ptr, len) {
    ...
    while (ptr < startPtr + len) {
      ...
      if (r.szKey == 'StringFileInfo') {
        r.stringTable = readStringTableStruct(buf, ptr + 36);
      }
      if (r.szKey == 'VarFileInfo') {
        r.varFileInfo = readVarFileInfoStruct(buf, ptr + 32);
      }
      t.push(r);
      ptr += wLength;
      ptr = padPointer(ptr);
  ...
  function readStringStructs(buf, ptr, len) {
    ...
    while (ptr + 6 < startPtr + len) {
      ...
      if (wType == 1) {
        r.value = unicodeToString(buf.slice(ptr + wLength - wValueLength * 2, ptr + wLength - 2));
      } // String value
      if (wType == 2) {
        r.value = buf.slice(ptr + wLength - wValueLength * 2, ptr + wLength);
      } // Binary value
  ...
  function padPointer(ptr) {
    return ptr + (ptr % 4 == 0 ? 0 : 4 - (ptr % 4));
  }
  ...
  obj.getHash = function (algo) {
    ...
    runHash(
      hash,
      obj.header.peHeaderLocation + 152 + obj.header.pe32plus * 16 + 8,
      obj.header.sigpos > 0 ? obj.header.sigpos : obj.filesize
    );
    return hash.digest();
  };
  ...
  obj.getHashOfFile = function (fd, algo, filesize) {
    ...
    runHashOnFile(
      fd,
      hash,
      obj.header.peHeaderLocation + 88 + 4,
      obj.header.peHeaderLocation + 152 + obj.header.pe32plus * 16
    );
    runHashOnFile(
      fd,
      hash,
      obj.header.peHeaderLocation + 152 + obj.header.pe32plus * 16 + 8,
      obj.header.sigpos > 0 ? obj.header.sigpos : filesize
    );
    return hash.digest();
  };
  ...
  obj.getHashNoResources = function (algo) {
    if (obj.header.sections['.rsrc'] == null) {
      return obj.getHash(algo);
    } // No resources in this executable, return a normal hash
  ...
  function runHash(hash, start, end) {
    var ptr = start;
    while (ptr < end) {
      const buf = readFileSlice(ptr, Math.min(65536, end - ptr));
      hash.update(buf);
      ptr += buf.length;
    }
  }
  ...
  function runChecksum() {
    var ptr = 0,
      c = createChecksum((obj.header.peOptionalHeaderLocation + 64) / 4);
    while (ptr < obj.filesize) {
      const buf = readFileSlice(ptr, Math.min(65536, obj.filesize - ptr));
      c.update(buf);
      ptr += buf.length;
    }
    return c.digest();
  ...
  function runChecksumOnFile(fd, filesize, checksumLocation) {
    var ptr = 0,
      c = createChecksum(checksumLocation),
      buf = Buffer.alloc(65536);
    while (ptr < filesize) {
      var len = fs.readSync(fd, buf, 0, Math.min(65536, filesize - ptr), ptr);
      c.update(buf, len);
      ptr += len;
    }
    return c.digest();
  ...
  obj.sign = function (cert, args, func) {
    if (cert == null) {
      cert = createSelfSignedCert({ cn: 'Test' });
    }
    ...
    if (args.hash == 'sha256') {
      hashOid = forge.pki.oids.sha256;
      fileHash = obj.getHash('sha256');
    }
    if (args.hash == 'sha384') {
      hashOid = forge.pki.oids.sha384;
      fileHash = obj.getHash('sha384');
    }
    if (args.hash == 'sha512') {
      hashOid = forge.pki.oids.sha512;
      fileHash = obj.getHash('sha512');
    }
    if (args.hash == 'sha224') {
      hashOid = forge.pki.oids.sha224;
      fileHash = obj.getHash('sha224');
    }
    if (args.hash == 'md5') {
      hashOid = forge.pki.oids.md5;
      fileHash = obj.getHash('md5');
    }
    if (hashOid == null) {
      func('Invalid signing hash: ' + args.hash);
      return;
    }
              ...
              value: [
                {
                  ...
                  value: '\u0000',
                  bitStringContents: '\u0000',
                  original: { tagClass: 0, type: 3, constructed: false, composed: false, value: '\u0000' }
                },
                {
                  tagClass: 128,
                  type: 0,
                  ...
                  value: [
                    {
                      ...
                      constructed: true,
                      composed: true,
                      value: [{ tagClass: 128, type: 0, constructed: false, composed: false, value: '' }]
                    }
                  ]
                }
              ]
          ...
          value: [
            {
              ...
              composed: true,
              value: [
                { tagClass: 0, type: 6, constructed: false, composed: false, value: forge.asn1.oidToDer(hashOid).data },
                { tagClass: 0, type: 5, constructed: false, composed: false, value: '' }
              ]
            },
            { tagClass: 0, type: 4, constructed: false, composed: false, value: fileHash.toString('binary') }
          ]
        }
      ]
    };
    xp7.contentInfo = forge.asn1.create(forge.asn1.Class.UNIVERSAL, forge.asn1.Type.SEQUENCE, true, [
      forge.asn1.create(
    ...
    if (typeof args.desc == 'string' || typeof args.url == 'string') {
      ...
      if (args.desc != null) {
        // Encode description as big-endian unicode.
        var desc = '',
          ucs = Buffer.from(args.desc, 'ucs2').toString();
        for (var k = 0; k < ucs.length; k += 2) {
          desc += String.fromCharCode(ucs.charCodeAt(k + 1), ucs.charCodeAt(k));
        }
        codeSigningAttributes.value.push({
          tagClass: 128,
          type: 0,
          constructed: true,
          composed: true,
          value: [{ tagClass: 128, type: 0, constructed: false, composed: false, value: desc }]
        });
      }
      if (args.url != null) {
        codeSigningAttributes.value.push({
          ...
          constructed: true,
          composed: true,
          value: [{ tagClass: 128, type: 0, constructed: false, composed: false, value: args.url }]
        });
      }
    ...
    xp7.sign();
    var p7signature = Buffer.from(
      forge.pkcs7.messageToPem(xp7).split('-----BEGIN PKCS7-----')[1].split('-----END PKCS7-----')[0],
      'base64'
    );
        ...
        asn1.create(asn1.Class.UNIVERSAL, asn1.Type.SEQUENCE, true, [
          ...
          asn1.create(asn1.Class.CONTEXT_SPECIFIC, 0, true, [
            asn1.create(
              ...
              false,
              pkcs7.rawCapture.signature.toString('binary')
            ) // Signature here
          ])
        ])
      ]);
  ...
  function signEx(args, p7signature, filesize, func) {
    ...
    if (output == null) {
      func('Unable to open output file: ' + args.out);
      return;
    }
    var tmp,
      written = 0,
      executableSize = obj.header.sigpos ? obj.header.sigpos : filesize;
  ...
  obj.writeExecutable = function (args, cert, func) {
    ...
    for (var i in obj.header.sections) {
      ...
      if (i == '.rsrc') {
        ...
        if (virtualAddressPadding != 0) {
          virtualAddress += 4096 - virtualAddressPadding;
        }
      } else {
        // Change the location of any other section if located after the resource section
        if (section.rawAddr > resPtr) {
          fullHeader.writeUInt32LE(section.rawAddr + resDeltaSize, section.ptr + 20);
        }
    ...
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.exportTable.addr),
      obj.header.peOptionalHeaderLocation + 96 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.importTable.addr),
      obj.header.peOptionalHeaderLocation + 104 + pePlusOffset
    );
    ...
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.exceptionTableAddr.addr),
      obj.header.peOptionalHeaderLocation + 120 + pePlusOffset
    );
    ...
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.baseRelocationTable.addr),
      obj.header.peOptionalHeaderLocation + 136 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.debug.addr),
      obj.header.peOptionalHeaderLocation + 144 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.globalPtr.addr),
      obj.header.peOptionalHeaderLocation + 160 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.tLSTable.addr),
      obj.header.peOptionalHeaderLocation + 168 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.loadConfigTable.addr),
      obj.header.peOptionalHeaderLocation + 176 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.boundImport.addr),
      obj.header.peOptionalHeaderLocation + 184 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.iAT.addr),
      obj.header.peOptionalHeaderLocation + 192 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.delayImportDescriptor.addr),
      obj.header.peOptionalHeaderLocation + 200 + pePlusOffset
    );
    fullHeader.writeUInt32LE(
      correctDirectoryValue(oldSections, newSections, obj.header.dataDirectories.clrRuntimeHeader.addr),
      obj.header.peOptionalHeaderLocation + 208 + pePlusOffset
    );
    ...
    if (cert != null) {
      ...
      if (args.hash == 'sha256') {
        hashOid = forge.pki.oids.sha256;
        fileHash = obj.getHashOfFile(output, 'sha256', written);
      }
      if (args.hash == 'sha384') {
        hashOid = forge.pki.oids.sha384;
        fileHash = obj.getHashOfFile(output, 'sha384', written);
      }
      if (args.hash == 'sha512') {
        hashOid = forge.pki.oids.sha512;
        fileHash = obj.getHashOfFile(output, 'sha512', written);
      }
      if (args.hash == 'sha224') {
        hashOid = forge.pki.oids.sha224;
        fileHash = obj.getHashOfFile(output, 'sha224', written);
      }
      if (args.hash == 'md5') {
        hashOid = forge.pki.oids.md5;
        fileHash = obj.getHashOfFile(output, 'md5', written);
      }
      if (hashOid == null) {
        func('Bad hash method OID');
        return;
      }
...
function start() {
  ...
  if (
    [
      ...
      'timestamp',
      'signblock'
    ].indexOf(process.argv[2].toLowerCase()) == -1
  ) {
  ...
  if (args.exe) {
    ...
    var stats = null;
    try {
      stats = require('fs').statSync(args.exe);
    } catch (ex) {}
    if (stats == null) {
      console.log('Unable to executable open file: ' + args.exe);
      return;
    }
    exe = createAuthenticodeHandler(args.exe);
    if (exe == null) {
      console.log('Unable to parse executable file: ' + args.exe);
      return;
    }
  }
  ...
  if (exe != null) {
    ...
    for (var i in versionProperties) {
      ...
      if (args[propl] && args[propl] != versionStrings[prop]) {
        versionStrings[prop] = args[propl];
        resChanges = true;
      }
    }
    if (args['fileversionnumber'] != null) {
      const fileVerSplit = args['fileversionnumber'].split('.');
      if (fileVerSplit.length != 4) {
        console.log('--fileversionnumber must be of format n.n.n.n, for example: 1.2.3.4');
        return;
      }
      for (var i in fileVerSplit) {
        var n = parseInt(fileVerSplit[i]);
        if (n < 0 || n > 65535) {
          console.log('--fileversionnumber numbers must be between 0 and 65535.');
          return;
        }
      }
      if (args['fileversionnumber'] != versionStrings['~FileVersion']) {
        versionStrings['~FileVersion'] = args['fileversionnumber'];
        resChanges = true;
      }
    }
    if (args['productversionnumber'] != null) {
      const productVerSplit = args['productversionnumber'].split('.');
      if (productVerSplit.length != 4) {
        console.log('--productversionnumber must be of format n.n.n.n, for example: 1.2.3.4');
        return;
      }
      for (var i in productVerSplit) {
        var n = parseInt(productVerSplit[i]);
        if (n < 0 || n > 65535) {
          console.log('--productversionnumber numbers must be between 0 and 65535.');
          return;
        }
      }
      if (args['productversionnumber'] != versionStrings['~ProductVersion']) {
        versionStrings['~ProductVersion'] = args['productversionnumber'];
        resChanges = true;
      }
    }
    if (resChanges == true) {
      exe.setVersionInfo(versionStrings);
    }
  }
  ...
  if (exe != null) {
    ...
    if (typeof args['removeicongroup'] == 'string') {
      // If --removeicongroup is used, it's to remove an existing icon group
      const groupsToRemove = args['removeicongroup'].split(',');
      for (var i in groupsToRemove) {
        if (icons[groupsToRemove[i]] != null) {
          delete icons[groupsToRemove[i]];
          resChanges = true;
        }
      }
    } else if (typeof args['removeicongroup'] == 'number') {
      if (icons[args['removeicongroup']] != null) {
        delete icons[args['removeicongroup']];
        resChanges = true;
      }
    }
    if (typeof args['removebitmap'] == 'string') {
      // If --removebitmap is used
      const bitmapsToRemove = args['removebitmap'].split(',');
      for (var i in bitmapsToRemove) {
        if (bitmaps[bitmapsToRemove[i]] != null) {
          delete bitmaps[bitmapsToRemove[i]];
          resChanges = true;
        }
      }
    } else if (typeof args['removebitmap'] == 'number') {
      if (bitmaps[args['removebitmap']] != null) {
        delete bitmaps[args['removebitmap']];
        resChanges = true;
      }
    }
    if (typeof args['icon'] == 'string') {
      // If --icon is used, it's to add or replace an existing icon group
      const iconToAddSplit = args['icon'].split(',');
      if (iconToAddSplit.length != 2) {
        console.log('The --icon format is: --icon [number],[file].');
        return;
      }
      ...
      if (icon == null) {
        console.log('Unable to load icon: ' + iconFile);
        return;
      }
      if (icons[iconName] != null) {
        ...
        if (iconHash != iconHash2) {
          icons[iconName] = icon;
          resChanges = true;
        } // If different, replace the icon group
      } else {
        icons[iconName] = icon; // We are adding an icon group
        resChanges = true;
      }
    }
    if (typeof args['bitmap'] == 'string') {
      // If --bitmap is used, it's to add or replace an existing bitmap
      const bitmapToAddSplit = args['bitmap'].split(',');
      if (bitmapToAddSplit.length != 2) {
        console.log('The --bitmap format is: --bitmap [number],[file].');
        return;
      }
      ...
      if (bitmap == null) {
        console.log('Unable to load bitmap: ' + bitmapFile);
        return;
      }
      if (bitmaps[bitmapName] != null) {
        ...
        if (bitmapHash != bitmapHash2) {
          bitmaps[bitmapName] = bitmap;
          resChanges = true;
        } // If different, replace the new bitmap
      } else {
  ...
  if (command == 'info') {
    // Get signature information about an executable
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
    if (args.json) {
      var r = {},
        stringInfo = exe.getVersionInfo();
      if (stringInfo != null) {
        r.versionInfo = {};
        r.stringInfo = {};
        for (var i in stringInfo) {
          if (i.startsWith('~')) {
            r.versionInfo[i.substring(1)] = stringInfo[i];
          } else {
            r.stringInfo[i] = stringInfo[i];
          }
        }
      }
      if (exe.fileHashAlgo != null) {
        r.signture = {};
        if (exe.fileHashAlgo != null) {
          r.signture.hashMethod = exe.fileHashAlgo;
        }
        if (exe.fileHashSigned != null) {
          r.signture.hashSigned = exe.fileHashSigned.toString('hex');
        }
        if (exe.fileHashActual != null) {
          r.signture.hashActual = exe.fileHashActual.toString('hex');
        }
        if (exe.signingAttribs && exe.signingAttribs.length > 0) {
          r.signture.attributes = exe.signingAttribs;
        }
      }
      ...
      if (versionInfo != null) {
        ...
        for (var i in versionInfo) {
          if (i.startsWith('~') == true) {
            console.log('  ' + i.substring(1) + ': ' + versionInfo[i] + '');
          }
        }
      ...
      if (exe.fileHashAlgo != null) {
        ...
        if (exe.fileHashSigned != null) {
          console.log('  Signed Hash:', exe.fileHashSigned.toString('hex'));
        }
        if (exe.fileHashActual != null) {
          console.log('  Actual Hash:', exe.fileHashActual.toString('hex'));
        }
      } else {
        console.log('  This file is not signed.');
      }
      if (exe.signingAttribs && exe.signingAttribs.length > 0) {
        console.log('Signature Attributes:');
        for (var i in exe.signingAttribs) {
          console.log('  ' + exe.signingAttribs[i]);
        }
      }
    }
  }
  if (command == 'header') {
    // Display the full executable header in JSON format
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
    ...
    for (var i in exe.header.sections) {
      if (i == '.text') {
        sizeOfCode += exe.header.sections[i].rawSize;
      } else {
        sizeOfInitializedData += exe.header.sections[i].rawSize;
      }
      if (exe.header.sections[i].rawAddr != ptr) {
        console.log(
          'WARNING: ' +
            i +
            ' section should have a rawAddr or ' +
            ptr +
            ', but has ' +
            exe.header.sections[i].rawAddr +
            ' instead.'
        );
      }
      ptr += exe.header.sections[i].rawSize;
    }
    if (exe.header.peStandard.sizeOfCode != sizeOfCode) {
      console.log('WARNING: Size of code is ' + exe.header.peStandard.sizeOfCode + ', should be ' + sizeOfCode + '.');
    }
    if (exe.header.peStandard.sizeOfInitializedData != sizeOfInitializedData) {
      console.log(
        'WARNING: Size of initialized data is ' +
          exe.header.peStandard.sizeOfInitializedData +
          ', should be ' +
  ...
  if (command == 'sections') {
    // Display sections in CSV format
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
    var csvHeader = 'section';
    for (var i in exe.header.sections['.text']) {
      csvHeader += ',' + i;
    }
    console.log(csvHeader);
    for (var i in exe.header.sections) {
      var csvData = i;
      for (var j in exe.header.sections[i]) {
        csvData += ',' + exe.header.sections[i][j];
      }
      console.log(csvData);
    }
  }
  if (command == 'sign') {
    ...
    if (typeof args.hash == 'string') {
      args.hash = args.hash.toLowerCase();
      if (['md5', 'sha224', 'sha256', 'sha384', 'sha512'].indexOf(args.hash) == -1) {
        console.log('Invalid hash method, must be SHA256 or SHA384');
        return;
      }
    }
    if (args.hash == null) {
    ...
    if (cert == null) {
      console.log('Unable to load certificate and/or private key, generating test certificate.');
      cert = createSelfSignedCert({ cn: 'Test' });
    }
    if (resChanges == false) {
      console.log('Signing to ' + args.out);
      exe.sign(cert, args, function (err) {
        // Simple signing, copy most of the original file.
        if (err == null) {
          console.log('Done.');
        } else {
          console.log(err);
      ...
      exe.writeExecutable(args, cert, function (err) {
        // Signing with resources decoded and re-encoded.
        if (err == null) {
          console.log('Done.');
        } else {
          console.log(err);
  ...
  if (command == 'unsign') {
    ...
    if (resChanges == false) {
      ...
      exe.writeExecutable(args, null, function (err) {
        // Unsigning with resources decoded and re-encoded.
        if (err == null) {
          console.log('Done.');
        } else {
          console.log(err);
  ...
  if (command == 'createcert') {
    ...
    if (typeof args.out != 'string') {
      console.log('Missing --out [filename]');
      return;
    }
    if (typeof args.cn != 'string') {
      console.log('Missing --cn [name]');
      return;
    }
    if (typeof args.serial == 'string') {
      if (args.serial != parseInt(args.serial)) {
        console.log('Invalid serial number.');
        return;
      } else {
        args.serial = parseInt(args.serial);
      }
    }
    if (typeof args.serial == 'number') {
      args.serial = '0' + args.serial;
    } // Serial number must be a integer string with a single leading '0'
  ...
  if (command == 'bitmaps') {
    ...
    if (args.json) {
      ...
      if (bitmapInfo != null) {
        ...
        for (var i in bitmapInfo) {
          console.log('  ' + i + ': ' + bitmapInfo[i].length + ' byte' + (bitmapInfo[i].length > 1 ? 's' : '') + '.');
        }
      }
    }
  }
  if (command == 'savebitmap') {
    // Save an bitmap to file
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
    ...
    if (typeof args.bitmap != 'number') {
      console.log('Missing or incorrect --bitmap [number]');
      return;
    }
    const bitmapInfo = exe.getBitmapInfo();
    if (bitmapInfo[args.bitmap] == null) {
      console.log('Unknown bitmap: ' + args.bitmap);
      return;
    }
          ...
          for (var j in iconInfo[i].icons) {
            console.log(
              '    Icon ' +
                j +
                ': ' +
                (iconInfo[i].icons[j].width == 0 ? 256 : iconInfo[i].icons[j].width) +
                'x' +
                (iconInfo[i].icons[j].height == 0 ? 256 : iconInfo[i].icons[j].height) +
                ', size: ' +
                iconInfo[i].icons[j].icon.length
            );
          }
        }
      }
  ...
  if (command == 'saveicon') {
    ...
    var icon = null;
    for (var i in iconInfo) {
      if (iconInfo[i].icons[args.icon]) {
        icon = iconInfo[i].icons[args.icon];
      }
    }
    if (icon == null) {
      console.log('Unknown icon: ' + args.icon);
      return;
    }
  ...
  if (command == 'saveicons') {
    // Save an icon group to file
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
    ...
    if (typeof args.icongroup != 'number') {
      console.log('Missing or incorrect --icongroup [number]');
      return;
    }
    const iconInfo = exe.getIconInfo();
    const iconGroup = iconInfo[args.icongroup];
    if (iconGroup == null) {
      console.log('Invalid or incorrect --icongroup [number]');
      return;
    }
    ...
    // Store each icon
    for (var i in iconGroup.icons) {
      iconFileData.push(iconGroup.icons[i].icon);
    }
  ...
  if (command == 'signblock') {
    // Display the raw signature block of the executable in hex
    if (exe == null) {
      console.log('Missing --exe [filename]');
      return;
    }
  ...
  if (command == 'timestamp') {
    ...
    if (exe.signature == null) {
      console.log('Executable is not signed.');
      return;
    }
    if (typeof args.time != 'string') {
      console.log('Missing --time [url]');
      return;
    }
    createOutFile(args, args.exe);
